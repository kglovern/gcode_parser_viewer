...
export function createToolpathStreams(args: {
  ...
  scene: any;
}): { streams: ToolpathStreamState[]; bounds: any | null } {
...
export function disposeToolpathStreams(scene: any, streams: readonly ToolpathStreamState[]): void {
  for (const stream of streams) {
    scene.remove(stream.line);
    stream.line.geometry.dispose();
    (stream.line.material as any).dispose();
  }
}
...
export function refreshToolpathStreamOpacities(args: {
  ...
  for (const stream of args.streams) {
    const material = stream.line.material as any;
...
export function applyStreamGreyCursor(args: {
  ...
  const attr = args.stream.line.geometry.getAttribute("color") as any;
