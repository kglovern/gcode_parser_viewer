import { buildMovementVerticesFromLines } from "../threejs-core";
...
export class GCodeSVGRenderer {
  ...
  private project = (x: number, y: number, z: number): Pt2 => {
    ...
    // Rotate around Y axis (azimuth) — uses cached trig
    const rx = dx * this.cosRotY + dz * this.sinRotY;
    const ry = dy;
    const rz = -dx * this.sinRotY + dz * this.cosRotY;
    // Rotate around X axis (elevation) — uses cached trig
    const fx = rx;
  ...
  private renderBbox(): void {
    ...
    // 8 projected corners: bottom face then top face
    ...
    // Bottom face, top face, 4 vertical edges
    const d = [
      // bottom face
    ...
    // Font size: 4% of longest projected dimension
    ...
    // X label: midpoint of bottom front edge (b0→b1), offset outward
    setLabel(
      this.bboxLabelX,
      mid(b0, b1),
    ...
    // Y label: midpoint of bottom right edge (b1→b2), offset outward
    setLabel(
      this.bboxLabelY,
      mid(b1, b2),
    ...
    // Z label: midpoint of front-left vertical edge (b0→t0), offset outward
    setLabel(
      this.bboxLabelZ,
      mid(b0, t0),
  ...
  private onPointerMove = (e: PointerEvent): void => {
    ...
    if (this.dragMode === 'orbit') {
      // Scale rotation speed by viewBox size relative to element size
      ...
      this.updateTrig(); // recompute once per drag event, not per vertex
      this.fitView();
      this.scheduleDraw();
    } else {
      // Pan: translate viewBox in screen space
      const rect = this.svg.getBoundingClientRect();
      this.viewBox = {
        ...this.viewBox,
  ...
  private onPointerUp = (e: PointerEvent): void => {
    ...
    if (wasOrbit) {
      this.rebuildAndRender(false); // full precision on release
    }
  };
...
// Push point `pos` away from `interior` by `gap` units
...
// Full quality: 2 decimal places is sub-0.01mm, more than enough for display
function f(n: number): string {
  return n.toFixed(2);
}

// Draft (during drag): integer precision — much shorter strings, faster SVG parse
