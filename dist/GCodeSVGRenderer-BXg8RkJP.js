...
import { OrbitControls as nP } from "three/examples/jsm/controls/OrbitControls.js";
import { GCodeVirtualizer as BA, buildWorkerToolpathStreams as TP, buildToolpathGeometryFromLinesBatched as oP, buildMovementVerticesFromLines as zP, buildWorkerSegmentGroups as wP } from "./gviewer.js";
import { STLLoader as EP } from "three/examples/jsm/loaders/STLLoader.js";
function aP(r) {
  const Q = [];
  let B = null;
  for (const A of r.specs) {
    if (A.positions.length === 0 || A.opacity <= 0)
      continue;
    const t = A.positions.length / 3, P = A.colors ? A.colors.slice() : nt(A.kind, t, r.options), e = new Float32Array(P.length);
    e.set(P);
    const i = new k.BufferGeometry();
    i.setAttribute("position", new k.BufferAttribute(A.positions, 3));
    const v = new k.BufferAttribute(e, 3);
    v.setUsage(k.DynamicDrawUsage), i.setAttribute("color", v), i.computeBoundingBox();
    const s = new k.LineBasicMaterial({
      vertexColors: !0,
      transparent: !0,
      opacity: A.opacity
    }), c = new k.LineSegments(i, s);
...
function kP(r, Q) {
  for (const B of r) {
    const A = nt(B.kind, B.totalVertices, Q);
...
function UP(r) {
  const Q = AA(r.options.render.theme.rapidOpacity ?? 0.3);
...
function DP(r) {
  ...
  if (B === A)
    return;
  const t = r.stream.line.geometry.getAttribute("color"), P = lP(r.options), e = Math.min(A, B), i = Math.max(A, B);
  if (B > A)
    for (let v = A; v < B; v += 1) {
      const s = v * 3;
      r.stream.simColors[s] = P.r, r.stream.simColors[s + 1] = P.g, r.stream.simColors[s + 2] = P.b;
    }
  else {
    const v = B * 3, s = A * 3;
    r.stream.simColors.set(r.stream.baseColors.subarray(v, s), v);
  }
  t.clearUpdateRanges(), t.addUpdateRange(e * 3, (i - e) * 3), t.needsUpdate = !0, r.stream.greyCursorVertex = B;
}
function nt(r, Q, B) {
  const A = B.render.theme.colors, t = B.mode.laser ? A.laser ?? A.cutting : A.cutting, P = new k.Color(r === "rapid" ? A.rapid : t), e = new Float32Array(Q * 3);
  for (let i = 0; i < Q; i += 1) {
    const v = i * 3;
    e[v] = P.r, e[v + 1] = P.g, e[v + 2] = P.b;
  }
  return e;
}
function lP(r) {
  const Q = r.render.theme.colors, B = new k.Color(r.render.theme.background), A = new k.Color(Q.processed ?? Q.cutting), t = B.clone().lerp(A, 0.65);
  return { r: t.r, g: t.g, b: t.b };
}
function uP(r) {
  if (r.bucketCount <= 1)
    return AA(r.baseOpacity);
  const Q = r.bucketIndex / (r.bucketCount - 1);
  return AA(r.baseOpacity * Q);
}
function AA(r) {
...
async function NP(r, Q, B, A) {
  var M, b, j;
  const t = Math.max(2, Math.floor(B)), P = (A == null ? void 0 : A.arcSegments) ?? 30, e = (A == null ? void 0 : A.checkpointEveryLines) ?? 2e3, i = ((M = A == null ? void 0 : A.batch) == null ? void 0 : M.yieldEveryLines) ?? 1e4, v = ((b = A == null ? void 0 : A.batch) == null ? void 0 : b.shouldAbort) ?? (() => !1), s = (j = A == null ? void 0 : A.batch) == null ? void 0 : j.onProgress;
  if (r.length === 0) {
    const d = {
      xMin: -1,
      xMax: 1,
  ...
  let c = 1 / 0, n = -1 / 0, o = 1 / 0, z = -1 / 0, w = 1 / 0, T = -1 / 0;
  const E = new BA({
    onLinearMove(d) {
      if (d.modals.motion === "G0") return;
      const G = d.transformedStart ?? d.start, f = d.transformedEnd ?? d.end;
      for (const H of [G, f])
        H.X < c && (c = H.X), H.X > n && (n = H.X), H.Y < o && (o = H.Y), H.Y > z && (z = H.Y), H.Z < w && (w = H.Z), H.Z > T && (T = H.Z);
    },
    onArcMove(d) {
      const G = d.transformedStart ?? d.start, f = d.transformedEnd ?? d.end, H = d.transformedMax ?? d.max;
      for (const F of [G, f, H])
        F.X < c && (c = F.X), F.X > n && (n = F.X), F.Y < o && (o = F.Y), F.Y > z && (z = F.Y), F.Z < w && (w = F.Z), F.Z > T && (T = F.Z);
    }
  });
  for (const d of r)
    E.processLine(d);
  isFinite(c) || (c = -1, n = 1, o = -1, z = 1, w = 0, T = 0), c === n && (c -= Q * 2, n += Q * 2), o === z && (o -= Q * 2, z += Q * 2), c -= Q, n += Q, o -= Q, z += Q;
  const h = T;
  let a = w - Q;
  a >= h && (a = h - 1);
  const D = { xMin: c, xMax: n, yMin: o, yMax: z, zTop: h, zBot: a }, l = new Float32Array(t * t).fill(h), U = /* @__PURE__ */ new Map();
  U.set(0, new Float32Array(l));
  const O = [];
  let u = 0, N = 0;
  const C = new BA({
    onLinearMove(d) {
      ...
      O.push(H), tA(H, l, D, Q, t);
    },
    onArcMove(d) {
      const G = d.transformedStart ?? d.start, f = d.transformedEnd ?? d.end, H = d.transformedCenter ?? d.center, F = OP(G, f, H, d.plane, d.motion, P);
      for (const X of F) {
        const nQ = { lineIndex: N, ...X };
        O.push(nQ), tA(nQ, l, D, Q, t);
      }
    }
  });
  for (let d = 0; d < r.length && !v(); d++) {
    N = d;
    const G = r[d];
    G !== void 0 && C.processLine(G), d - u >= e && (U.set(d, new Float32Array(l)), u = d), d % i === 0 && d > 0 && (s == null || s(d, r.length), await new Promise((f) => setTimeout(f, 0)));
  }
  return s == null || s(r.length, r.length), { moves: O, checkpoints: U, slabBounds: D, resolution: t, toolRadius: Q };
}
function tA(r, Q, B, A, t) {
  const P = (B.xMax - B.xMin) / t, e = (B.yMax - B.yMin) / t, i = Math.hypot(r.x1 - r.x0, r.y1 - r.y0), v = Math.max(1e-3, Math.min(P, e) * 0.5), s = Math.max(1, Math.ceil(i / v)), c = Math.ceil(A / P), n = Math.ceil(A / e), o = A * A;
  for (let z = 0; z <= s; z++) {
    const w = z / s, T = r.x0 + (r.x1 - r.x0) * w, E = r.y0 + (r.y1 - r.y0) * w, h = r.z0 + (r.z1 - r.z0) * w, a = Math.floor((T - B.xMin) / P), D = Math.floor((E - B.yMin) / e);
    for (let l = -c; l <= c; l++) {
      const U = a + l;
      if (U < 0 || U >= t) continue;
      const O = B.xMin + (U + 0.5) * P, u = (O - T) * (O - T);
      if (!(u > o))
        for (let N = -n; N <= n; N++) {
          const C = D + N;
          if (C < 0 || C >= t) continue;
          const M = B.yMin + (C + 0.5) * e;
          if (u + (M - E) * (M - E) > o) continue;
          const j = C * t + U;
          Q[j] > h && (Q[j] = h);
        }
    }
...
function mB(r, Q) {
  ...
  for (const P of Q.moves)
    P.lineIndex > B && P.lineIndex <= r && tA(P, t, Q.slabBounds, Q.toolRadius, Q.resolution);
  return t;
}
function yB(r, Q, B, A) {
  if (B <= 0) return;
...
function OP(r, Q, B, A, t, P) {
  const e = Math.max(1, P);
  let i, v, s;
  A === "G18" ? (i = "Z", v = "X", s = "Y") : A === "G19" ? (i = "Y", v = "Z", s = "X") : (i = "X", v = "Y", s = "Z");
  const c = Math.hypot(r[i] - B[i], r[v] - B[v]), n = Math.atan2(r[v] - B[v], r[i] - B[i]), o = Math.atan2(Q[v] - B[v], Q[i] - B[i]);
  let z;
  t === "G2" ? (z = o - n, z > 0 && (z -= Math.PI * 2)) : (z = o - n, z < 0 && (z += Math.PI * 2));
  const w = [];
  let T = { ...r };
  for (let E = 1; E <= e; E++) {
    const h = E / e, a = n + z * h, D = h, l = B[i] + c * Math.cos(a), U = B[v] + c * Math.sin(a), O = r[s] + (Q[s] - r[s]) * D, u = { X: 0, Y: 0, Z: 0 };
    u[i] = l, u[v] = U, u[s] = O, w.push({
      x0: T.X,
      y0: T.Y,
      z0: T.Z,
      x1: u.X,
...
function dP() {
  ...
  for (let t = 0; t < A; t++) {
    const P = Math.random() * r, e = 1 + Math.random() * 2, i = Math.floor(139 + (Math.random() - 0.5) * 40), v = Math.floor(94 + (Math.random() - 0.5) * 30), s = Math.floor(60 + (Math.random() - 0.5) * 20);
    B.strokeStyle = `rgb(${i},${v},${s})`, B.lineWidth = e, B.beginPath(), B.moveTo(0, P), B.lineTo(r, P), B.stroke();
  }
  for (let t = 0; t < 300; t++) {
    const P = Math.random() * r, e = Math.random() * r, i = 20 + Math.random() * 60, v = (Math.random() - 0.5) * 0.3, s = Math.floor(100 + Math.random() * 80), c = Math.floor(60 + Math.random() * 50), n = Math.floor(30 + Math.random() * 40);
    B.strokeStyle = `rgba(${s},${c},${n},0.35)`, B.lineWidth = 0.5 + Math.random() * 1, B.beginPath(), B.moveTo(P, e), B.quadraticCurveTo(
      P + i * 0.5 * Math.cos(v),
      e + i * 0.5 * Math.sin(v) * 0.1,
      P + i * Math.cos(v * 0.5),
      e + i * 0.15
    ), B.stroke();
  }
  return uB = new k.CanvasTexture(Q), uB;
}
function LP(r, Q) {
  const B = new k.Group();
  B.name = "gviewer:sim3d-slab";
  const A = r.xMax - r.xMin, t = r.yMax - r.yMin, P = r.zTop - r.zBot, e = (r.xMin + r.xMax) / 2, i = (r.yMin + r.yMax) / 2, v = (r.zBot + r.zTop) / 2, s = dP();
  s.wrapS = k.RepeatWrapping, s.wrapT = k.RepeatWrapping, s.repeat.set(A / 100, t / 100);
  const c = new k.MeshStandardMaterial({
    map: s,
    roughness: 0.85,
    metalness: 0,
    side: k.FrontSide
  }), n = new k.MeshStandardMaterial({
    color: 8016432,
    roughness: 0.9,
    metalness: 0,
    side: k.DoubleSide
  }), o = new k.MeshStandardMaterial({
    color: 7030822,
    roughness: 0.9,
    metalness: 0
  }), z = new k.PlaneGeometry(A, t, Q - 1, Q - 1), w = Q * Q, T = new Float32Array(w * 3), E = z.attributes.position;
  for (let j = 0; j < w; j++)
    T[j * 3] = E.getX(j), T[j * 3 + 1] = E.getY(j), T[j * 3 + 2] = 0;
  const h = new k.BufferAttribute(T, 3);
  h.usage = k.DynamicDrawUsage, z.setAttribute("position", h), z.computeVertexNormals();
  const a = new k.Mesh(z, c);
  a.name = "gviewer:sim3d-top", a.position.set(e, i, r.zTop), B.add(a);
  const D = new k.PlaneGeometry(A, P);
  D.rotateX(Math.PI / 2);
  const l = new k.Mesh(D, n);
  l.position.set(e, r.yMax, v), B.add(l);
  const U = new k.PlaneGeometry(A, P);
  U.rotateX(Math.PI / 2);
  const O = new k.Mesh(U, n);
  O.position.set(e, r.yMin, v), B.add(O);
  const u = new k.PlaneGeometry(P, t);
  u.rotateY(-Math.PI / 2);
  const N = new k.Mesh(u, n);
  N.position.set(r.xMax, i, v), B.add(N);
  const C = new k.PlaneGeometry(P, t);
  C.rotateY(-Math.PI / 2);
  const M = new k.Mesh(C, n);
  M.position.set(r.xMin, i, v), B.add(M);
  const b = new k.Mesh(new k.PlaneGeometry(A, t), o);
  return b.position.set(e, i, r.zBot), B.add(b), { group: B, topMesh: a, topPositionAttr: h, slabBounds: r, resolution: Q };
}
function IB(r, Q) {
  const { topPositionAttr: B, slabBounds: A, resolution: t } = r, P = t * t;
  for (let e = 0; e < P; e++) {
    const i = Math.floor(e / t), v = e % t, s = (t - 1 - i) * t + v;
    B.setZ(e, Q[s] - A.zTop);
  }
...
function OQ(r, Q, B = {}) {
  const { opacity: A = 1, size: t = 10 } = B, P = new k.Object3D(), e = 100, i = document.createElement("canvas"), v = i.getContext("2d");
  if (!v)
    throw new Error("Missing canvas 2D context.");
  v.font = `normal ${e}px Arial`;
  const c = v.measureText(r).width;
  i.width = c, i.height = e, v.font = `normal ${e}px Arial`, v.textAlign = "center", v.textBaseline = "middle", v.fillStyle = Q, v.fillText(r, c / 2, e / 2);
  const n = new k.Texture(i);
  n.needsUpdate = !0, n.minFilter = k.LinearFilter;
  const o = new k.SpriteMaterial({
    map: n,
    transparent: !0,
    opacity: A
  });
  ...
  const z = new k.Sprite(o);
  return z.scale.set(c / e * t, t, 1), P.add(z), P;
}
function jP(r) {
  r.traverse((Q) => {
    var A;
    if (!(Q instanceof k.Sprite))
      return;
    const B = Q.material;
...
const FP = 25.4;
function GP(r, Q) {
  const B = new k.Group(), A = new k.Box3Helper(
    r.clone(),
    new k.Color(Q.render.theme.colors.boundingBox)
  ), t = A.material;
  return t.transparent = !0, t.opacity = 0.12, t.depthWrite = !1, B.add(A), Q.boundingBox.labels && B.add(...fP(r.clone(), Q)), B;
}
function fP(r, Q) {
  const B = Q.render.theme.colors.boundingBox, A = 0.32, t = (D) => Math.abs(D) < 1e-9 ? "0" : D.toFixed(3), P = (D) => Q.units === "in" ? `${t(D / FP)} in` : `${t(D)} mm`, e = r.min, i = r.max, v = (D) => OQ(D, B, { size: 4, opacity: A }), s = v(P(e.x)), c = v(P(i.x)), n = v(P(e.y)), o = v(P(i.y)), z = v(P(e.z)), w = v(P(i.z)), T = new k.Vector3();
  r.getSize(T);
  const E = Math.max(2, Math.max(T.x, T.y, T.z) * 0.02), h = (e.x + i.x) / 2, a = (e.y + i.y) / 2;
  return s.position.set(e.x - E, a, e.z - E), c.position.set(i.x + E, a, e.z - E), n.position.set(h, e.y - E, e.z - E), o.position.set(h, i.y + E, e.z - E), z.position.set(h, a, e.z - E), w.position.set(h, a, i.z + E), [s, c, n, o, z, w];
}
function bP(r) {
  r.traverse((Q) => {
    var B;
    if (Q instanceof k.LineSegments || Q instanceof k.Line) {
      Q.geometry.dispose();
      const A = Q.material;
...
function xA(r) {
  const Q = new k.Vector3(), B = {
    right: new k.Vector3(1, 0, 0),
    left: new k.Vector3(-1, 0, 0),
    front: new k.Vector3(0, -1, 0),
    back: new k.Vector3(0, 1, 0),
    top: new k.Vector3(0, 0, 1),
    bottom: new k.Vector3(0, 0, -1)
  };
  switch (r) {
    case "front":
      Q.copy(B.front);
...
const PA = 25.4;
function mP(r) {
  const B = Math.max(1, r.sizeMm) / 2, A = r.units === "mm" ? 10 : PA, t = Math.max(0, Math.floor(B / A)), P = [], e = [], i = (s, c, n, o, z) => {
    z.push(s, c, 0, n, o, 0);
  };
  for (let s = -t; s <= t; s += 1) {
    const c = s * A;
  ...
  const v = new k.Group();
  return e.length > 0 && v.add(mA(e, r.theme.colors.grid.minor, 0.25)), P.length > 0 && v.add(mA(P, r.theme.colors.grid.major, 0.4)), v;
}
function mA(r, Q, B) {
  const A = new k.BufferGeometry();
  A.setAttribute("position", new k.Float32BufferAttribute(r, 3));
  const t = new k.LineBasicMaterial({
    color: new k.Color(Q),
    transparent: !0,
    opacity: B,
    depthWrite: !1
  });
  return new k.LineSegments(A, t);
}
function yP(r) {
  r.traverse((Q) => {
    if (!(Q instanceof k.LineSegments))
      return;
    Q.geometry.dispose();
    const B = Q.material;
...
function IP(r) {
  const Q = new k.Group(), B = Math.max(1, r.sizeWorld) / 2, A = Math.max(1, r.depthWorld), t = Math.max(1, Math.min(B / 12, 30)), P = t * 0.6, e = (w) => new k.LineDashedMaterial({
    color: new k.Color(w),
    dashSize: t,
    gapSize: P,
    transparent: !0,
    ...
    new k.BufferGeometry().setFromPoints([
      new k.Vector3(-B, 0, 0),
      new k.Vector3(B, 0, 0)
    ]),
  ...
  s.computeLineDistances(), Q.add(i, v, s);
  const c = Math.max(2, t), n = OQ("X", r.theme.colors.axes.x, { size: 5 });
  n.position.set(B + c, 0, 0);
  const o = OQ("Y", r.theme.colors.axes.y, { size: 5 });
  o.position.set(0, B + c, 0);
  const z = OQ("Z", r.theme.colors.axes.z, { size: 5 });
  return z.position.set(0, 0, A + c), Q.add(n, o, z), Q;
}
function SP(r) {
  r.traverse((Q) => {
    var B;
    if (Q instanceof k.Line) {
      Q.geometry.dispose();
      const A = Q.material;
      Array.isArray(A) ? A.forEach((t) => t.dispose()) : A.dispose();
      return;
...
function YP(r) {
  const Q = new k.Group(), B = Math.max(1, r.sizeMm) / 2, A = r.units === "mm" ? 10 : 1, t = A, P = r.units === "mm" ? B : B / PA, e = A * 2, i = r.units === "mm" ? 1 : PA, v = 0.5, s = 4, c = 0.1, n = 5;
  for (let o = t; o <= P + 1e-6; o += e) {
    const z = o * i;
    if (z > B + 1e-6)
      continue;
    const w = r.theme.colors.axes.x, T = r.theme.colors.axes.y, E = { opacity: v, size: s }, h = OQ(String(o), w, E);
    h.position.set(z, n, c);
    const a = OQ(String(-o), w, E);
    a.position.set(-z, n, c);
    const D = OQ(String(o), T, E);
    D.position.set(-n, z, c);
    const l = OQ(String(-o), T, E);
    l.position.set(-n, -z, c), Q.add(h, a, D, l);
  }
  return Q;
}
function DQ(r) {
...
var vQ = {
  ...
  overwrite: !1,
  delay: 0
}, kA, Z, m, zQ = 1e8, x = 1 / zQ, eA = Math.PI * 2, VP = eA / 4, JP = 0, ot = Math.sqrt, RP = Math.cos, XP = Math.sin, K = function(Q) {
  ...
  return Y(Q) || K(Q);
}, zt = typeof ArrayBuffer == "function" && ArrayBuffer.isView || function() {
}, q = Array.isArray, KP = /random\([^)]+\)/g, ZP = /,\s*/g, yA = /(?:-?\.?\d|\.)+/gi, wt = /[-+=.]*\d+[.e\-+]*\d*[e\-+]*\d*/g, KQ = /[-+=.]*\d+[.e-]*\d*[a-z%]*/g, SB = /[-+=.]*\d+\.?\d*(?:e-|e\+)?\d*/gi, Et = /[+-]=-?[.\d]+/, WP = /[^,'"\[\]\s]+/gi, qP = /^[+\-=e\s\d]*\d+[.\d]*([a-z]*|%)\s*$/i, I, EQ, iA, lA, sQ = {}, MB = {}, at, ht = function(Q) {
  return (MB = $Q(Q, sQ)) && tQ;
}, uA = function(Q, B) {
  ...
  suppressEvents: !0
}, CA = {}, HQ = [], rA = {}, Ut, PQ = {}, YB = {}, IA = 30, OB = [], NA = "", OA = function(Q) {
  var B = Q[0], A, t;
  ...
  for (t = Q.length; t--; )
    Q[t] && (Q[t]._gsap || (Q[t]._gsap = new St(Q[t], A))) || Q.splice(t, 1);
  return Q;
}, SQ = function(Q) {
  return Q._gsap || OA(wQ(Q))[0]._gsap;
}, Dt = function(Q, B, A) {
  ...
  for (rA = {}, HQ.length = 0, A = 0; A < Q; A++)
    t = B[A], t && t._lazy && (t.render(t._lazy[0], t._lazy[1], !0)._lazy = 0);
}, dA = function(Q) {
  return !!(Q._initted || Q._startAt || Q.add);
}, lt = function(Q, B, A, t) {
  HQ.length && !Z && HB(), Q.render(B, A, !!(Z && B < 0 && dA(Q))), HQ.length && !Z && HB();
}, ut = function(Q) {
  var B = parseFloat(Q);
  return (B || B === 0) && (Q + "").match(WP).length < 2 ? B : K(Q) ? Q.trim() : Q;
}, Ct = function(Q) {
  return Q;
}, cQ = function(Q, B) {
  for (var A in B)
  ...
  var B = Q.parent || I, A = Q.keyframes ? Be(q(Q.keyframes)) : cQ;
  if ($(Q.inherit))
    for (; B; )
  ...
  var A;
  if ((B._time || !B._dur && B._initted || B._start < Q._time && (B._dur || !B.add)) && (A = FB(Q.rawTime(), B), (!B._dur || DB(0, B.totalDuration(), A) - B._tTime > x) && B.render(A, !0)), YQ(Q, B)._dp && Q._initted && Q._time >= Q._dur && Q._ts) {
  ...
  return B.parent && FQ(B), B._start = y((uQ(A) ? A : A || Q !== I ? oQ(Q, A, B) : Q._time) + B._delay), B._end = y(B._start + (B.totalDuration() / Math.abs(B.timeScale()) || 0)), Nt(Q, B, "_first", "_last", Q._sort ? "_start" : 0), sA(B) || (Q._recent = B), t || Ot(Q, B), Q._ts < 0 && xB(Q, Q._tTime), Q;
}, dt = function(Q, B) {
  return (sQ.ScrollTrigger || uA("scrollTrigger", B)) && sQ.ScrollTrigger.create(B, Q);
}, Lt = function(Q, B, A, t, P) {
  if (MA(Q, B, P), !Q._initted)
    return 1;
  ...
  var P = Q.ratio, e = B < 0 || !B && (!Q._start && ee(Q) && !(!Q._initted && sA(Q)) || (Q._ts < 0 || Q._dp._ts < 0) && !sA(Q)) ? 0 : 1, i = Q._rDelay, v = 0, s, c, n;
  if (i && Q._repeat && (v = DB(0, Q._tDur, B), c = QB(v, i), Q._yoyo && c & 1 && (e = 1 - e), c !== QB(Q._tTime, i) && (P = 1 - e, Q.vars.repeatRefresh && Q._initted && Q.invalidate())), e !== P || Z || t || Q._zTime === x || !B && Q._zTime) {
    if (!Q._initted && Lt(Q, B, t, A, v))
      return;
    for (n = Q._zTime, Q._zTime = B || (A ? x : 0), A || (A = B && !n), Q.ratio = e, Q._from && (e = 1 - e), Q._time = 0, Q._tTime = v, s = Q._pt; s; )
      s.r(e, s.d), s = s._next;
    B < 0 && vA(Q, B, A, !0), Q._onUpdate && !A && iQ(Q, "onUpdate"), v && Q._repeat && !A && Q.parent && iQ(Q, "onRepeat"), (B >= Q._tDur || B < 0) && Q.ratio === e && (e && FQ(Q, 1), !A && !Z && (iQ(Q, e ? "onComplete" : "onReverseComplete", !0), Q._prom && Q._prom()));
  ...
  var t = Q.labels, P = Q._recent || ve, e = Q.duration() >= zQ ? P.endTime(!1) : Q._dur, i, v, s;
  return K(B) && (isNaN(B) || B in t) ? (v = B.charAt(0), s = B.substr(-1) === "%", i = B.indexOf("="), v === "<" || v === ">" ? (i >= 0 && (B = B.replace(/=/, "")), (v === "<" ? P._start : P.endTime(P._repeat >= 0)) + (parseFloat(B.substr(1)) || 0) * (s ? (i < 0 ? P : A).totalDuration() / 100 : 1)) : i < 0 ? (B in t || (t[B] = e), t[B]) : (v = parseFloat(B.charAt(i - 1) + B.substr(i + 1)), s && A && (v = v / 100 * (q(A) ? A[0] : A).totalDuration()), i > 1 ? r(Q, B.substr(0, i - 1), A) + v : e + v)) : B == null ? e : +B;
}, nB = function(Q, B, A) {
  var t = uQ(B[1]), P = (t ? 2 : 1) + (Q < 2 ? 0 : 1), e = B[P], i, v;
  if (t && (e.duration = B[1]), e.parent = A, Q) {
    for (i = e, v = A; v && !("immediateRender" in i); )
      i = v.vars.defaults || {}, v = $(v.vars.inherit) && v.parent;
  ...
  return !K(Q) || !(B = qP.exec(Q)) ? "" : B[1];
}, se = function(Q, B, A) {
  return bQ(A, function(t) {
    return DB(Q, B, t);
  });
}, cA = [].slice, Mt = function(Q, B) {
  return Q && kQ(Q) && "length" in Q && (!B && !Q.length || Q.length - 1 in Q && kQ(Q[0])) && !Q.nodeType && Q !== EQ;
}, ce = function(Q, B, A) {
  return A === void 0 && (A = []), Q.forEach(function(t) {
    var P;
    return K(t) && !B || Mt(t, 1) ? (P = A).push.apply(P, wQ(t)) : A.push(t);
  }) || A;
}, wQ = function(Q, B, A) {
  return m && !B && m.selector ? m.selector(Q) : K(Q) && !A && (iA || !AB()) ? cA.call((B || lA).querySelectorAll(Q), 0) : q(Q) ? ce(Q, A) : Mt(Q) ? cA.call(Q, 0) : Q ? [Q] : [];
}, nA = function(Q) {
  return Q = wQ(Q)[0] || zB("Invalid scope") || {}, function(B) {
    var A = Q.current || Q.nativeElement || Q;
    return wQ(B, A.querySelectorAll ? A : A === Q ? zB("Invalid scope") || lA.createElement("div") : Q);
  };
}, Ht = function(Q) {
  ...
  var B = kQ(Q) ? Q : {
    each: Q
  }, A = VQ(B.ease), t = B.from || 0, P = parseFloat(B.base) || 0, e = {}, i = t > 0 && t < 1, v = isNaN(t) || i, s = B.axis, c = t, n = t;
  return K(t) ? c = n = {
    center: 0.5,
    edges: 0.5,
    end: 1
  }[t] || 0 : !i && v && (c = t[0], n = t[1]), function(o, z, w) {
    var T = (w || B).length, E = e[T], h, a, D, l, U, O, u, N, C;
    if (!E) {
      if (C = B.grid === "auto" ? 0 : (B.grid || [1, zQ])[1], !C) {
        for (u = -zQ; u < (u = w[C++].getBoundingClientRect().left) && C < T; )
          ;
        C < T && C--;
      }
      for (E = e[T] = [], h = v ? Math.min(C, T) * c - 0.5 : t % C, a = C === zQ ? 0 : v ? T * n / C - 0.5 : t / C | 0, u = 0, N = zQ, O = 0; O < T; O++)
        D = O % C - h, l = a - (O / C | 0), E[O] = U = s ? Math.abs(s === "y" ? l : D) : ot(D * D + l * l), U > u && (u = U), U < N && (N = U);
      t === "random" && Ht(E), E.max = u - N, E.min = N, E.v = T = (parseFloat(B.amount) || parseFloat(B.each) * (C > T ? T - 1 : s ? s === "y" ? T / C : C : Math.max(C, T / C)) || 0) * (t === "edges" ? -1 : 1), E.b = T < 0 ? P - T : P, E.u = W(B.amount || B.each) || 0, A = A && T < 0 ? ue(A) : A;
    }
    return T = (E[o] - E.min) / E.max || 0, y(E.b + (A ? A(T) : T) * E.v) + E.u;
  };
}, TA = function(Q) {
  ...
  return !A && kQ(Q) && (t = A = Q.radius || zQ, Q.values ? (Q = wQ(Q.values), (P = !uQ(Q[0])) && (t *= t)) : Q = TA(Q.increment)), bQ(B, A ? Y(Q) ? function(e) {
    ...
    for (var i = parseFloat(P ? e.x : e), v = parseFloat(P ? e.y : 0), s = zQ, c = 0, n = Q.length, o, z; n--; )
      P ? (o = Q[n].x - i, z = Q[n].y - v, o = o * o + z * z) : o = Math.abs(Q[n] - i), o < s && (s = o, c = n);
    return c = !t || s <= t ? Q[c] : e, P || c === e || uQ(e) ? c : c + W(e);
  } : TA(Q));
}, Gt = function(Q, B, A, t) {
  ...
  var t = B - Q;
  return q(Q) ? ft(Q, r(0, Q.length), B) : bQ(A, function(P) {
    return (t + (P - Q) % t) % t + Q;
  });
}, we = function r(Q, B, A) {
  var t = B - Q, P = t * 2;
  return q(Q) ? ft(Q, r(0, Q.length - 1), B) : bQ(A, function(e) {
    return e = (P + (e - Q) % P) % P || 0, Q + (e > t ? P - e : e);
  });
}, EB = function(Q) {
  return Q.replace(KP, function(B) {
    var A = B.indexOf("[") + 1, t = B.substring(A || 7, A ? B.indexOf("]") : B.length - 1).split(ZP);
    return Gt(A ? t : +t[0], A ? 0 : +t[1], +t[2] || 1e-5);
  });
}, bt = function(Q, B, A, t, P) {
  var e = B - Q, i = t - A;
  return bQ(P, function(v) {
    return A + ((v - Q) / e * i || 0);
  });
}, Ee = function r(Q, B, A, t) {
  var P = isNaN(Q + B) ? 0 : function(z) {
    return (1 - z) * Q + z * B;
  };
  if (!P) {
    var e = K(Q), i = {}, v, s, c, n, o;
    if (A === !0 && (t = 1) && (A = null), e)
      Q = {
        p: Q
      }, B = {
      ...
      for (c = [], n = Q.length, o = n - 2, s = 1; s < n; s++)
        c.push(r(Q[s - 1], Q[s]));
      n--, P = function(w) {
        w *= n;
        var T = Math.min(o, ~~w);
    ...
    if (!c) {
      for (v in B)
        LA.call(i, Q, v, "get", B[v]);
      P = function(w) {
        return FA(w, i) || (e ? Q.p : Q);
      };
    }
  }
  return bQ(A, P);
}, JA = function(Q, B, A) {
  var t = Q.labels, P = zQ, e, i, v;
  for (e in t)
    i = t[e] - B, i < 0 == !!A && i && P > (i = Math.abs(i)) && (v = e, P = i);
  return v;
}, iQ = function(Q, B, A) {
  var t = Q.vars, P = t[B], e = m, i = Q._ctx, v, s, c;
  if (P)
    return v = t[B + "Params"], s = t.callbackScope || Q, A && HQ.length && HB(), i && (m = i), c = v ? P.apply(s, v) : P.call(s), m = e, c;
}, vB = function(Q) {
  ...
  if (Q)
    if (Q = !Q.name && Q.default || Q, DA() || Q.headless) {
      ...
      if (AB(), Q !== t) {
        if (PQ[B])
          return;
        cQ(t, cQ(jB(Q, P), e)), $Q(t.prototype, $Q(P, jB(Q, e))), PQ[t.prop = B] = t, Q.targetTest && (OB.push(t), CA[B] = 1), B = (B === "css" ? "CSS" : B.charAt(0).toUpperCase() + B.substr(1)) + "Plugin";
      }
      kt(B, t), Q.register && Q.register(tQ, t, BQ);
    } else
      gt.push(Q);
}, p = 255, sB = {
  aqua: [0, p, p],
  ...
  var t = Q ? uQ(Q) ? [Q >> 16, Q >> 8 & p, Q & p] : 0 : sB.black, P, e, i, v, s, c, n, o, z, w;
  if (!t) {
    if (Q.substr(-1) === "," && (Q = Q.substr(0, Q.length - 1)), sB[Q])
      t = sB[Q];
    else if (Q.charAt(0) === "#") {
      ...
      if (t = w = Q.match(yA), !B)
        v = +t[0] % 360 / 360, s = +t[1] / 100, c = +t[2] / 100, e = c <= 0.5 ? c * (s + 1) : c + s - c * s, P = c * 2 - e, t.length > 3 && (t[3] *= 1), t[0] = VB(v + 1 / 3, P, e), t[1] = VB(v, P, e), t[2] = VB(v - 1 / 3, P, e);
      else if (~Q.indexOf("="))
        return t = Q.match(wt), A && t.length < 4 && (t[3] = 1), t;
    } else
      t = Q.match(yA) || sB.transparent;
    t = t.map(Number);
  }
  return B && !w && (P = t[0] / p, e = t[1] / p, i = t[2] / p, n = Math.max(P, e, i), o = Math.min(P, e, i), c = (n + o) / 2, n === o ? v = s = 0 : (z = n - o, s = c > 0.5 ? z / (2 - n - o) : z / (n + o), v = n === P ? (e - i) / z + (e < i ? 6 : 0) : n === e ? (i - P) / z + 2 : (P - e) / z + 4, v *= 60), t[0] = ~~(v + 0.5), t[1] = ~~(s * 100 + 0.5), t[2] = ~~(c * 100 + 0.5)), A && t.length < 4 && (t[3] = 1), t;
}, mt = function(Q) {
  var B = [], A = [], t = -1;
  ...
  var t = "", P = (Q + t).match(jQ), e = B ? "hsla(" : "rgba(", i = 0, v, s, c, n;
  if (!P)
    return Q;
  if (P = P.map(function(o) {
    return (o = xt(o, B, 1)) && e + (B ? o[0] + "," + o[1] + "%," + o[2] + "%," + o[3] : o.join(",")) + ")";
  }), A && (c = mt(Q), v = A.c, v.join(t) !== c.c.join(t)))
    for (s = Q.replace(jQ, "1").split(KQ), n = s.length - 1; i < n; i++)
      t += s[i] + (~v.indexOf(i) ? P.shift() || e + "0,0,0,0)" : (c.length ? c : P.length ? P : A).shift());
  if (!s)
    for (s = Q.split(jQ), n = s.length - 1; i < n; i++)
      t += s[i] + P[i];
  return t + s[n];
}, jQ = function() {
...
}(), ae = /hsl[a]?\(/, yt = function(Q) {
  var B = Q.join(" "), A;
  if (jQ.lastIndex = 0, jQ.test(B))
    return A = ae.test(B), Q[1] = RA(Q[1], A), Q[0] = RA(Q[0], A, mt(Q[1])), !0;
}, aB, eQ = function() {
  var r = Date.now, Q = 500, B = 33, A = r(), t = A, P = 1e3 / 240, e = P, i = [], v, s, c, n, o, z, w = function T(E) {
    var h = r() - t, a = E === !0, D, l, U, O;
    if ((h > Q || h < 0) && (A += h - B), t += h, U = t - A, D = U - e, (D > 0 || a) && (O = ++n.frame, o = U - n.time * 1e3, n.time = U = U / 1e3, e += D + (D >= P ? 4 : P - D), l = 1), a || (v = s(T)), l)
  ...
  return n = {
    ...
    wake: function() {
      at && (!iA && DA() && (EQ = iA = window, lA = EQ.document || {}, sQ.gsap = tQ, (EQ.gsapVersions || (EQ.gsapVersions = [])).push(tQ.version), ht(MB || EQ.GreenSockGlobals || !EQ.gsap && EQ || {}), gt.forEach(pt)), c = typeof requestAnimationFrame < "u" && requestAnimationFrame, v && n.sleep(), s = c || function(E) {
        return setTimeout(E, e - n.time * 1e3 + 1 | 0);
      }, aB = 1, w(2));
    },
    sleep: function() {
      (c ? cancelAnimationFrame : clearTimeout)(v), aB = 0, s = wB;
    },
    ...
    add: function(E, h, a) {
      var D = h ? function(l, U, O, u) {
        E(l, U, O, u), n.remove(D);
      } : E;
      return n.remove(E), i[a ? "unshift" : "push"](D), AB(), D;
    },
    remove: function(E, h) {
  ...
  return !aB && eQ.wake();
}, g = {}, he = /^[\d.\-M][\d.\-,\s]/, ke = /["']/g, Ue = function(Q) {
  for (var B = {}, A = Q.substr(1, Q.length - 3).split(":"), t = A[0], P = 1, e = A.length, i, v, s; P < e; P++)
    v = A[P], i = P !== e - 1 ? v.lastIndexOf(",") : v.length, s = v.substr(0, i), B[t] = isNaN(s) ? s.replace(ke, "").trim() : +s, t = v.substr(i + 1).trim();
  return B;
}, De = function(Q) {
  ...
  return A && B.length > 1 && A.config ? A.config.apply(null, ~Q.indexOf("{") ? [Ue(B[1])] : De(Q).split(",").map(ut)) : g._CE && he.test(Q) ? g._CE("", Q) : A;
}, ue = function(Q) {
  return function(B) {
    return 1 - Q(1 - B);
  };
}, VQ = function(Q, B) {
  return Q && (Y(Q) ? Q : g[Q] || le(Q)) || B;
}, RQ = function(Q, B, A, t) {
  A === void 0 && (A = function(v) {
    return 1 - B(1 - v);
  }), t === void 0 && (t = function(v) {
    return v < 0.5 ? B(v * 2) / 2 : 1 - B((1 - v) * 2) / 2;
  });
  var P = {
    easeIn: B,
    easeOut: A,
  ...
  return QQ(Q, function(i) {
    g[i] = sQ[i] = P, g[e = i.toLowerCase()] = A;
    for (var v in P)
      g[e + (v === "easeIn" ? ".in" : v === "easeOut" ? ".out" : ".inOut")] = g[i + "." + v] = P[v];
  }), P;
}, It = function(Q) {
  return function(B) {
  ...
  var t = B >= 1 ? B : 1, P = (A || (Q ? 0.3 : 0.45)) / (B < 1 ? B : 1), e = P / eA * (Math.asin(1 / t) || 0), i = function(c) {
    return c === 1 ? 1 : t * Math.pow(2, -10 * c) * XP((c - e) * P) + 1;
  }, v = Q === "out" ? i : Q === "in" ? function(s) {
    return 1 - i(1 - s);
  } : It(i);
  return P = eA / P, v.config = function(s, c) {
    return r(Q, s, c);
  }, v;
}, RB = function r(Q, B) {
  B === void 0 && (B = 1.70158);
...
RQ("Sine", function(r) {
  return r === 1 ? 1 : -RP(r * VP) + 1;
});
RQ("Back", RB("in"), RB("out"), RB());
g.SteppedEase = g.steps = sQ.SteppedEase = {
  config: function(Q, B) {
    Q === void 0 && (Q = 1);
...
var St = function(Q, B) {
  this.id = JP++, Q._gsap = this, this.target = Q, this.harness = B, this.get = B ? B.get : Dt, this.set = B ? B.getSetter : jA;
  ...
  return Q.delay = function(A) {
    ...
    if (P && P.smoothChildTiming && this._ts) {
      for (xB(this, A), !P._dp || P.parent || Ot(P, this); P && P.parent; )
    ...
    return (this._tTime !== A || !this._dur && !t || this._initted && Math.abs(this._zTime) === x || !this._initted && this._dur && A || !A && !this._initted && (this.add || this._ptLookup)) && (this._ts || (this._pTime = A), lt(this, A, t)), this;
  }, Q.time = function(A, t) {
    return arguments.length ? this.totalTime(Math.min(this.totalDuration(), A + YA(this)) % (this._dur + this._rDelay) || (A ? this._dur : 0), t) : this._time;
    ...
    return arguments.length ? this.totalTime(this.duration() * (this._yoyo && !(this.iteration() & 1) ? 1 - A : A) + YA(this), t) : this.duration() ? Math.min(1, this._time / this._dur) : this.rawTime() > 0 ? 1 : 0;
    ...
    return this._rts = +A || 0, this._ts = this._ps || A === -x ? 0 : this._rts, this.totalTime(DB(-Math.abs(this._delay), this.totalDuration(), P), t !== !1), pB(this), te(this);
  }, Q.paused = function(A) {
    ...
    A === void 0 && (A = $P);
    var t = Z;
    return Z = A, dA(this) && (this.timeline && this.timeline.revert(A), this.totalTime(-0.01, A.suppressEvents)), this.data !== "nested" && A.kill !== !1 && this.kill(), Z = t, this;
    ...
    return arguments.length ? (this._repeat = A === 1 / 0 ? -2 : A, VA(this)) : this._repeat === -2 ? 1 / 0 : this._repeat;
  }, Q.repeatDelay = function(A) {
    if (arguments.length) {
      var t = this._time;
      return this._rDelay = A, VA(this), t ? this.time(t) : this;
    }
    return this._rDelay;
  }, Q.yoyo = function(A) {
    return arguments.length ? (this._yoyo = A, this) : this._yoyo;
  }, Q.seek = function(A, t) {
    return this.totalTime(oQ(this, A), $(t));
    ...
    return new Promise(function(e) {
      var i = Y(A) ? A : Ct, v = function() {
        var c = t.then;
        t.then = null, P && P(), Y(i) && (i = i(t)) && (i.then || i === t) && (t.then = c), e(i), t.then = c;
      };
      t._initted && t.totalProgress() === 1 && t._ts >= 0 || !t._tTime && t._ts < 0 ? v() : t._prom = v;
    });
  }, Q.kill = function() {
    vB(this);
  }, r;
...
var _ = /* @__PURE__ */ function(r) {
  ...
  function Q(A, t) {
    var P;
    return A === void 0 && (A = {}), P = r.call(this, A) || this, P.labels = {}, P.smoothChildTiming = !!A.smoothChildTiming, P.autoRemoveChildren = !!A.autoRemoveChildren, P._sort = $(A.sortChildren), I && aQ(A.parent || I, DQ(P), t), A.reversed && P.reverse(), A.paused && P.paused(!0), A.scrollTrigger && dt(DQ(P), A.scrollTrigger), P;
  }
  var B = Q.prototype;
  return B.to = function(t, P, e) {
    ...
    return P.duration = 0, P.parent = this, cB(P).repeatDelay || (P.repeat = 0), P.immediateRender = !!P.immediateRender, new J(t, P, oQ(this, e), 1), this;
  }, B.call = function(t, P, e) {
    return aQ(this, J.delayedCall(0, t, P), e);
  }, B.staggerTo = function(t, P, e, i, v, s, c) {
    return e.duration = P, e.stagger = e.stagger || i, e.onComplete = s, e.onCompleteParams = c, e.parent = this, new J(t, e, oQ(this, v)), this;
  }, B.staggerFrom = function(t, P, e, i, v, s, c) {
    return e.runBackwards = 1, cB(e).immediateRender = $(e.immediateRender), this.staggerTo(t, P, e, i, v, s, c);
  }, B.staggerFromTo = function(t, P, e, i, v, s, c, n) {
    return i.startAt = e, cB(i).immediateRender = $(i.immediateRender), this.staggerTo(t, P, i, v, s, c, n);
  }, B.render = function(t, P, e) {
    var i = this._time, v = this._dirty ? this.totalDuration() : this._tDur, s = this._dur, c = t <= 0 ? 0 : y(t), n = this._zTime < 0 != t < 0 && (this._initted || !s), o, z, w, T, E, h, a, D, l, U, O, u;
    if (this !== I && c > v && t >= 0 && (c = v), c !== this._tTime || e || n) {
      if (i !== this._time && s && (c += this._time - i, t += this._time - i), o = c, l = this._start, D = this._ts, h = !D, n && (s || (i = this._zTime), (t || !P) && (this._zTime = t)), this._repeat) {
        if (O = this._yoyo, E = s + this._rDelay, this._repeat < -1 && t < 0)
          return this.totalTime(E * 100 + t, P, e);
        if (o = y(c % E), c === v ? (T = this._repeat, o = s) : (U = y(c / E), T = ~~U, T && T === U && (o = s, T--), o > s && (o = s)), U = QB(this._tTime, E), !i && this._tTime && U !== T && this._tTime - U * E - this._dur <= 0 && (U = T), O && T & 1 && (o = s - o, u = 1), T !== U && !this._lock) {
          var N = O && U & 1, C = N === (O && T & 1);
          if (T < U && (N = !N), i = N ? 0 : c % s ? s : c, this._lock = 1, this.render(i || (u ? 0 : y(T * E)), P, !s)._lock = 0, this._tTime = c, !P && this.parent && iQ(this, "onRepeat"), this.vars.repeatRefresh && !u && (this.invalidate()._lock = 1, U = T), i && i !== this._time || h !== !this._ts || this.vars.onRepeat && !this.parent && !this._act)
            return this;
          if (s = this._dur, v = this._tDur, C && (this._lock = 2, i = N ? s : -1e-4, this.render(i, !0), this.vars.repeatRefresh && !u && this.invalidate()), this._lock = 0, !this._ts && !h)
            return this;
        }
      }
      if (this._hasPause && !this._forcing && this._lock < 2 && (a = re(this, y(i), y(o)), a && (c -= o - (o = a._start))), this._tTime = c, this._time = o, this._act = !!D, this._initted || (this._onUpdate = this.vars.onUpdate, this._initted = 1, this._zTime = t, i = 0), !i && c && s && !P && !U && (iQ(this, "onStart"), this._tTime !== c))
        return this;
      if (o >= i && t >= 0)
        for (z = this._first; z; ) {
          if (w = z._next, (z._act || o >= z._start) && z._ts && a !== z) {
            if (z.parent !== this)
              return this.render(t, P, e);
            if (z.render(z._ts > 0 ? (o - z._start) * z._ts : (z._dirty ? z.totalDuration() : z._tDur) + (o - z._start) * z._ts, P, e), o !== this._time || !this._ts && !h) {
        ...
        for (var M = t < 0 ? t : o; z; ) {
          if (w = z._prev, (z._act || M <= z._end) && z._ts && a !== z) {
            if (z.parent !== this)
              return this.render(t, P, e);
            if (z.render(z._ts > 0 ? (M - z._start) * z._ts : (z._dirty ? z.totalDuration() : z._tDur) + (M - z._start) * z._ts, P, e || Z && dA(z)), o !== this._time || !this._ts && !h) {
      ...
      if (a && !P && (this.pause(), a.render(o >= i ? 0 : -x)._zTime = o >= i ? 1 : -1, this._ts))
        return this._start = l, pB(this), this.render(t, P, e);
      this._onUpdate && !P && iQ(this, "onUpdate", !0), (c === v && this._tTime >= this.totalDuration() || !c && i) && (l === this._start || Math.abs(D) !== Math.abs(this._ts)) && (this._lock || ((t || !s) && (c === v && this._ts > 0 || !c && this._ts < 0) && FQ(this, 1), !P && !(t < 0 && !i) && (c || i || !v) && (iQ(this, c === v && t >= 0 ? "onComplete" : "onReverseComplete", !0), this._prom && !(c < v && this.timeScale() > 0) && this._prom())));
    }
    return this;
  }, B.add = function(t, P) {
    var e = this;
    if (uQ(P) || (P = oQ(this, P, t)), !(t instanceof hB)) {
      if (q(t))
    ...
    for (var v = [], s = this._first; s; )
      s._start >= i && (s instanceof J ? P && v.push(s) : (e && v.push(s), t && v.push.apply(v, s.getChildren(!0, P, e)))), s = s._next;
    return v;
    ...
    return this.labels[t] = oQ(this, P), this;
  }, B.removeLabel = function(t) {
    return delete this.labels[t], this;
  }, B.addPause = function(t, P, e) {
    var i = J.delayedCall(0, P || wB, e);
    return i.data = "isPause", this._hasPause = 1, aQ(this, i, oQ(this, t));
  }, B.removePause = function(t) {
    var P = this._first;
    for (t = oQ(this, t); P; )
      P._start === t && P.data === "isPause" && FQ(P), P = P._next;
  }, B.killTweensOf = function(t, P, e) {
    for (var i = this.getTweensOf(t, e), v = i.length; v--; )
      dQ !== i[v] && i[v].kill(t, P);
    return this;
  }, B.getTweensOf = function(t, P) {
    for (var e = [], i = wQ(t), v = this._first, s = uQ(P), c; v; )
      v instanceof J ? Qe(v._targets, i) && (s ? (!dQ || v._initted && v._ts) && v.globalTime(0) <= P && v.globalTime(v.totalDuration()) > P : !P || v.isActive()) && e.push(v) : (c = v.getTweensOf(i, P)).length && e.push.apply(e, c), v = v._next;
    return e;
  }, B.tweenTo = function(t, P) {
    P = P || {};
    var e = this, i = oQ(e, t), v = P, s = v.startAt, c = v.onStart, n = v.onStartParams, o = v.immediateRender, z, w = J.to(e, cQ({
      ease: P.ease || "none",
      lazy: !1,
      immediateRender: !1,
      time: i,
      overwrite: "auto",
      duration: P.duration || Math.abs((i - (s && "time" in s ? s.time : e._time)) / e.timeScale()) || x,
      onStart: function() {
        if (e.pause(), !z) {
          var E = P.duration || Math.abs((i - (s && "time" in s ? s.time : e._time)) / e.timeScale());
          w._dur !== E && BB(w, E, 0, 1).render(w._time, !0, !0), z = 1;
        }
        c && c.apply(w, n || []);
      }
    }, P));
    return o ? w.render(0) : w;
    ...
    return t === void 0 && (t = this._time), JA(this, oQ(this, t));
  }, B.previousLabel = function(t) {
    return t === void 0 && (t = this._time), JA(this, oQ(this, t), 1);
  }, B.currentLabel = function(t) {
    return arguments.length ? this.seek(t, !0) : this.previousLabel(this._time + x);
  }, B.shiftChildren = function(t, P, e) {
    e === void 0 && (e = 0);
    var i = this._first, v = this.labels, s;
    for (t = y(t); i; )
    ...
    var P = 0, e = this, i = e._last, v = zQ, s, c, n;
    if (arguments.length)
      return e.timeScale((e._repeat < 0 ? e.duration() : e.totalDuration()) / (e.reversed() ? -t : t));
    if (e._dirty) {
      for (n = e.parent; i; )
        s = i._prev, i._dirty && i.totalDuration(), c = i._start, c > v && e._sort && i._ts && !e._lock ? (e._lock = 1, aQ(e, i, c - i._delay, 1)._lock = 0) : v = c, c < 0 && i._ts && (P -= c, (!n && !e._dp || n && n.smoothChildTiming) && (e._start += y(c / e._ts), e._time -= c, e._tTime -= c), e.shiftChildren(-c, !1, -1 / 0), v = 0), i._end > P && i._ts && (P = i._end), i = s;
      BB(e, e === I && e._time > P ? e._time : P, 1, 1), e._dirty = 0;
    }
    return e._tDur;
  }, Q.updateRoot = function(t) {
    if (I._ts && (lt(I, FB(t, I)), Ut = eQ.frame), eQ.frame >= IA) {
      IA += vQ.autoSleep || 120;
      var P = I._first;
      if ((!P || !P._ts) && vQ.autoSleep && eQ._listeners.length < 2) {
        for (; P && !P._ts; )
          P = P._next;
        P || eQ.sleep();
      }
...
var Ce = function(Q, B, A, t, P, e, i) {
  var v = new BQ(this._pt, Q, B, 0, 1, Kt, null, P), s = 0, c = 0, n, o, z, w, T, E, h, a;
  for (v.b = A, v.e = t, A += "", t += "", (h = ~t.indexOf("random(")) && (t = EB(t)), e && (a = [A, t], e(a, Q, B), A = a[0], t = a[1]), o = A.match(SB) || []; n = SB.exec(t); )
    w = n[0], T = t.substring(s, n.index), z ? z = (z + 1) % 5 : T.substr(-5) === "rgba(" && (z = 1), w !== o[c++] && (E = parseFloat(o[c - 1]) || 0, v._pt = {
      _next: v._pt,
      p: T || c === 1 ? T : ",",
  ...
  return v.c = s < t.length ? t.substring(s, t.length) : "", v.fp = i, (Et.test(t) || h) && (v.e = 0), this._pt = v, v;
}, LA = function(Q, B, A, t, P, e, i, v, s, c) {
  Y(t) && (t = t(P || 0, Q, e));
  var n = Q[B], o = A !== "get" ? A : Y(n) ? s ? Q[B.indexOf("set") || !Y(Q["get" + B.substr(3)]) ? B : "get" + B.substr(3)](s) : Q[B]() : n, z = Y(n) ? s ? Me : Rt : HA, w;
  if (K(t) && (~t.indexOf("random(") && (t = EB(t)), t.charAt(1) === "=" && (w = WQ(o, t) + (W(o) || 0), (w || w === 0) && (t = w))), !c || o !== t || oA)
    return !isNaN(o * t) && t !== "" ? (w = new BQ(this._pt, Q, B, +o || 0, t - (o || 0), typeof n == "boolean" ? je : Xt, 0, z), s && (w.fp = s), i && w.modifier(i, this, Q), this._pt = w) : (!n && !(B in Q) && uA(B, t), Ce.call(this, Q, B, o, t, z, v || vQ.stringFilter, s));
}, Ne = function(Q, B, A, t, P) {
  if (Y(Q) && (Q = TB(Q, P, B, A, t)), !kQ(Q) || Q.style && Q.nodeType || q(Q) || zt(Q))
    return K(Q) ? TB(Q, P, B, A, t) : Q;
  var e = {}, i;
  for (i in Q)
    e[i] = TB(Q[i], P, B, A, t);
  return e;
}, Yt = function(Q, B, A, t, P, e) {
  var i, v, s, c;
  if (PQ[Q] && (i = new PQ[Q]()).init(P, i.rawVars ? B[Q] : Ne(B[Q], t, P, e, A), A, t, e) !== !1 && (A._pt = v = new BQ(A._pt, P, Q, 0, 1, i.render, i, 0, i.priority), A !== ZQ))
    for (s = A._ptLookup[A._targets.indexOf(P)], c = i._props.length; c--; )
      s[i._props[c]] = v;
  return i;
}, dQ, oA, MA = function r(Q, B, A) {
  var t = Q.vars, P = t.ease, e = t.startAt, i = t.immediateRender, v = t.lazy, s = t.onUpdate, c = t.runBackwards, n = t.yoyoEase, o = t.keyframes, z = t.autoRevert, w = Q._dur, T = Q._startAt, E = Q._targets, h = Q.parent, a = h && h.data === "nested" ? h.vars.targets : E, D = Q._overwrite === "auto" && !kA, l = Q.timeline, U = t.easeReverse || n, O, u, N, C, M, b, j, d, G, f, H, F, X;
  if (l && (!o || !P) && (P = "none"), Q._ease = VQ(P, oB.ease), Q._rEase = U && (VQ(U) || Q._ease), Q._from = !l && !!t.runBackwards, Q._from && (Q.ratio = 1), !l || o && !t.stagger) {
    if (d = E[0] ? SQ(E[0]).harness : 0, F = d && t[d.prop], O = jB(t, CA), T && (T._zTime < 0 && T.progress(1), B < 0 && c && i && !z ? T.render(-1, !0) : T.revert(c && w ? NB : _P), T._lazy = 0), e) {
    ...
    for (Q._pt = Q._ptCache = 0, v = w && $(v) || v && !w, u = 0; u < E.length; u++) {
      if (M = E[u], j = M._gsap || OA(E)[u]._gsap, Q._ptLookup[u] = f = {}, rA[j.id] && HQ.length && HB(), H = a === E ? u : a.indexOf(M), d && (G = new d()).init(M, F || O, Q, H, a) !== !1 && (Q._pt = C = new BQ(Q._pt, M, G.name, 0, 1, G.render, G, 0, G.priority), G._props.forEach(function(nQ) {
        f[nQ] = C;
      }), G.priority && (b = 1)), !d || F)
        for (N in O)
          PQ[N] && (G = Yt(N, O, Q, H, M, a)) ? G.priority && (b = 1) : f[N] = C = LA.call(Q, M, N, "get", O[N], H, a, 0, t.stringFilter);
      Q._op && Q._op[u] && Q.kill(M, Q._op[u]), D && Q._pt && (dQ = Q, I.killTweensOf(M, f, Q.globalTime(B)), X = !Q.parent, dQ = 0), Q._pt && v && (rA[j.id] = 1);
    }
    b && Zt(Q), Q._onInit && Q._onInit(Q);
  }
  Q._onUpdate = s, Q._initted = (!Q._op || Q._pt) && !X, o && B <= 0 && l.render(zQ, !0, !0);
}, Oe = function(Q, B, A, t, P, e, i, v) {
  var s = (Q._pt && Q._ptCache || (Q._ptCache = {}))[B], c, n, o, z;
  if (!s)
    for (s = Q._ptCache[B] = [], o = Q._ptLookup, z = Q._targets.length; z--; ) {
      if (c = o[z][B], c && c.d && c.d._pt)
        for (c = c.d._pt; c && c.p !== B && c.fp !== B; )
          c = c._next;
      if (!c)
        return oA = 1, Q.vars[B] = "+=0", MA(Q, i), oA = 0, v ? zB(B + " not eligible for reset. Try splitting into individual properties") : 1;
      s.push(c);
    }
  for (z = s.length; z--; )
    n = s[z], c = n._pt || n, c.s = (t || t === 0) && !P ? t : c.s + (t || 0) + e * c.c, c.c = A - c.s, n.e && (n.e = V(A) + W(n.e)), n.b && (n.b = c.s + W(n.b));
}, de = function(Q, B) {
  var A = Q[0] ? SQ(Q[0]).harness : 0, t = A && A.aliases, P, e, i, v;
  if (!t)
    return B;
  P = $Q({}, B);
  for (e in t)
    if (e in P)
      for (v = t[e].split(","), i = v.length; i--; )
        P[v[i]] = P[e];
  return P;
}, Le = function(Q, B, A, t) {
  var P = B.ease || t || "power1.inOut", e, i;
  if (q(B))
    i = A[Q] || (A[Q] = []), B.forEach(function(v, s) {
      return i.push({
        t: s / (B.length - 1) * 100,
        v,
        e: P
...
}, TB = function(Q, B, A, t, P) {
  return Y(Q) ? Q.call(B, A, t, P) : K(Q) && ~Q.indexOf("random(") ? EB(Q) : Q;
}, Vt = NA + "repeat,repeatDelay,yoyo,repeatRefresh,yoyoEase,easeReverse,autoRevert", Jt = {};
QQ(Vt + ",id,stagger,delay,duration,paused,scrollTrigger", function(r) {
  return Jt[r] = 1;
});
var J = /* @__PURE__ */ function(r) {
  ...
  function Q(A, t, P, e) {
    ...
    var v = i.vars, s = v.duration, c = v.delay, n = v.immediateRender, o = v.stagger, z = v.overwrite, w = v.keyframes, T = v.defaults, E = v.scrollTrigger, h = t.parent || I, a = (q(A) || zt(A) ? uQ(A[0]) : "length" in t) ? [A] : wQ(A), D, l, U, O, u, N, C, M;
    if (i._targets = a.length ? OA(a) : zB("GSAP target " + A + " not found. https://gsap.com", !vQ.nullTargetWarn) || [], i._ptLookup = [], i._overwrite = z, w || o || CB(s) || CB(c)) {
      t = i.vars;
      var b = t.easeReverse || t.yoyoEase;
      if (D = i.timeline = new _({
        data: "nested",
        defaults: T || {},
        targets: h && h.data === "nested" ? h.vars.targets : a
      }), D.kill(), D.parent = D._dp = DQ(i), D._start = 0, o || CB(s) || CB(c)) {
        if (O = a.length, C = o && jt(o), kQ(o))
          for (u in o)
            ~Vt.indexOf(u) && (M || (M = {}), M[u] = o[u]);
        for (l = 0; l < O; l++)
          U = jB(t, Jt), U.stagger = 0, b && (U.easeReverse = b), M && $Q(U, M), N = a[l], U.duration = +TB(s, DQ(i), l, N, a), U.delay = (+TB(c, DQ(i), l, N, a) || 0) - i._delay, !o && O === 1 && U.delay && (i._delay = c = U.delay, i._start += c, U.delay = 0), D.to(N, U, C ? C(l, N, a) : 0), D._ease = g.none;
        D.duration() ? s = c = 0 : i.timeline = 0;
      } else if (w) {
        cB(cQ(D.vars.defaults, {
          ease: "none"
        })), D._ease = VQ(w.ease || t.ease || "none");
        var j = 0, d, G, f;
        if (q(w))
          ...
          U = {};
          for (u in w)
            u === "ease" || u === "easeEach" || Le(u, w[u], U, w.easeEach);
          for (u in U)
            for (d = U[u].sort(function(H, F) {
              return H.t - F.t;
            }), j = 0, l = 0; l < d.length; l++)
              G = d[l], f = {
                ease: G.e,
                duration: (G.t - (l ? d[l - 1].t : 0)) / 100 * s
              }, f[u] = G.v, D.to(a, f, j), j += f.duration;
          D.duration() < s && D.to({}, {
            duration: s - D.duration()
          });
        }
      }
      s || i.duration(s = D.duration());
    } else
      i.timeline = 0;
    return z === !0 && !kA && (dQ = DQ(i), I.killTweensOf(a), dQ = 0), aQ(h, DQ(i), P), t.reversed && i.reverse(), t.paused && i.paused(!0), (n || !s && !w && i._start === y(h._time) && $(n) && Pe(DQ(i)) && h.data !== "nested") && (i._tTime = -x, i.render(Math.max(0, -c) || 0)), E && dt(DQ(i), E), i;
  }
  var B = Q.prototype;
  return B.render = function(t, P, e) {
    var i = this._time, v = this._tDur, s = this._dur, c = t < 0, n = t > v - x && !c ? v : t < x ? 0 : t, o, z, w, T, E, h, a, D;
    if (!s)
      ie(this, t, P, e);
    else if (n !== this._tTime || !t || e || !this._initted && this._tTime || this._startAt && this._zTime < 0 !== c || this._lazy) {
      if (o = n, D = this.timeline, this._repeat) {
        if (T = s + this._rDelay, this._repeat < -1 && c)
          return this.totalTime(T * 100 + t, P, e);
        if (o = y(n % T), n === v ? (w = this._repeat, o = s) : (E = y(n / T), w = ~~E, w && w === E ? (o = s, w--) : o > s && (o = s)), h = this._yoyo && w & 1, h && (o = s - o), E = QB(this._tTime, T), o === i && !e && this._initted && w === E)
          return this._tTime = n, this;
        w !== E && this.vars.repeatRefresh && !h && !this._lock && o !== T && this._initted && (this._lock = e = 1, this.render(y(T * w), !0).invalidate()._lock = 0);
      }
      if (!this._initted) {
        if (Lt(this, c ? t : o, e, P, n))
      ...
      if (this._rEase) {
        ...
        if (l !== this._inv) {
          var U = l ? i : s - i;
          this._inv = l, this._from && (this.ratio = 1 - this.ratio), this._invRatio = this.ratio, this._invTime = i, this._invRecip = U ? (l ? -1 : 1) / U : 0, this._invScale = l ? -this.ratio : 1 - this.ratio, this._invEase = l ? this._rEase : this._ease;
        }
        this.ratio = a = this._invRatio + this._invScale * this._invEase((o - this._invTime) * this._invRecip);
      } else
        this.ratio = a = this._ease(o / s);
      if (this._from && (this.ratio = a = 1 - a), this._tTime = n, this._time = o, !this._act && this._ts && (this._act = 1, this._lazy = 0), !i && n && !P && !E && (iQ(this, "onStart"), this._tTime !== n))
        return this;
      for (z = this._pt; z; )
        z.r(a, z.d), z = z._next;
      D && D.render(t < 0 ? t : D._dur * D._ease(o / this._dur), P, e) || this._startAt && (this._zTime = t), this._onUpdate && !P && (c && vA(this, t, P, e), iQ(this, "onUpdate")), this._repeat && w !== E && this.vars.onRepeat && !P && this.parent && iQ(this, "onRepeat"), (n === this._tDur || !n) && this._tTime === n && (c && !this._onUpdate && vA(this, t, !0, !0), (t || !s) && (n === this._tDur && this._ts > 0 || !n && this._ts < 0) && FQ(this, 1), !P && !(c && !i) && (n || i || h) && (iQ(this, n === v ? "onComplete" : "onReverseComplete", !0), this._prom && !(n < v && this.timeScale() > 0) && this._prom()));
    }
    return this;
  }, B.targets = function() {
  ...
  }, B.resetTo = function(t, P, e, i, v) {
    aB || eQ.wake(), this._ts || this.play();
    var s = Math.min(this._dur, (this._dp._time - this._start) * this._ts), c;
    return this._initted || MA(this, s), c = this._ease(s / this._dur), Oe(this, t, P, e, i, c, s, v) ? this.resetTo(t, P, e, i, 1) : (xB(this, 0), this.parent || Nt(this._dp, this, "_first", "_last", this._dp._sort ? "_start" : 0), this.render(0));
  }, B.kill = function(t, P) {
    if (P === void 0 && (P = "all"), !t && (!P || P === "all"))
      return this._lazy = this._pt = 0, this.parent ? vB(this) : this.scrollTrigger && this.scrollTrigger.kill(!!Z), this;
    if (this.timeline) {
      var e = this.timeline.totalDuration();
      return this.timeline.killTweensOf(t, P, dQ && dQ.vars.overwrite !== !0)._first || vB(this), this.parent && e !== this.timeline.totalDuration() && BB(this, this._dur * this.timeline._tDur / e, 0, 1), this;
    }
    var i = this._targets, v = t ? wQ(t) : i, s = this._ptLookup, c = this._pt, n, o, z, w, T, E, h;
    if ((!P || P === "all") && Ae(i, v))
      return P === "all" && (this._pt = 0), vB(this);
    for (n = this._op = this._op || [], P !== "all" && (K(P) && (T = {}, QQ(P, function(a) {
      return T[a] = 1;
    }), P = T), P = de(i, P)), h = i.length; h--; )
      if (~v.indexOf(i[h])) {
        o = s[h], P === "all" ? (n[h] = P, w = o, z = {}) : (z = n[h] = n[h] || {}, w = P);
        for (T in w)
          E = o && o[T], E && ((!("kill" in E.d) || E.d.kill(T) === !0) && gB(this, E, "_pt"), delete o[T]), z !== "all" && (z[T] = 1);
      }
    return this._initted && !this._pt && c && vB(this), this;
  }, Q.to = function(t, P) {
...
QQ("staggerTo,staggerFrom,staggerFromTo", function(r) {
  J[r] = function() {
    var Q = new _(), B = cA.call(arguments, 0);
    return B.splice(r === "staggerFromTo" ? 5 : 4, 0, 0), Q[r].apply(Q, B);
  };
});
var HA = function(Q, B, A) {
  return Q[B] = A;
}, Rt = function(Q, B, A) {
  return Q[B](A);
}, Me = function(Q, B, A, t) {
  ...
  return Y(Q[B]) ? Rt : UA(Q[B]) && Q.setAttribute ? He : HA;
}, Xt = function(Q, B) {
  ...
  function r(B, A, t, P, e, i, v, s, c) {
    this.t = A, this.s = P, this.c = e, this.p = t, this.r = i || Xt, this.d = v || this, this.set = s || HA, this.pr = c || 0, this._next = B, B && (B._prev = this);
  }
  var Q = r.prototype;
  return Q.modifier = function(A, t, P) {
    this.mSet = this.mSet || this.set, this.set = fe, this.m = A, this.mt = P, this.tween = t;
  }, r;
}();
QQ(NA + "parent,duration,ease,delay,overwrite,runBackwards,startAt,yoyo,immediateRender,repeat,repeatDelay,data,paused,reversed,lazy,callbackScope,stringFilter,id,yoyoEase,stagger,inherit,repeatRefresh,keyframes,autoRevert,scrollTrigger,easeReverse", function(r) {
  return CA[r] = 1;
});
sQ.TweenMax = sQ.TweenLite = J;
sQ.TimelineLite = sQ.TimelineMax = _;
I = new _({
  sortChildren: !1,
  defaults: oB,
...
vQ.stringFilter = yt;
var JQ = [], dB = {}, be = [], XA = 0, ge = 0, XB = function(Q) {
  return (dB[Q] || be).map(function(B) {
    return B();
  });
}, zA = function() {
  var Q = Date.now(), B = [];
  Q - XA > 2 && (XB("matchMediaInit"), JQ.forEach(function(A) {
    var t = A.queries, P = A.conditions, e, i, v, s;
    for (i in t)
      e = EQ.matchMedia(t[i]).matches, e && (v = 1), e !== P[i] && (P[i] = e, s = 1);
    s && (A.revert(), v && B.push(A));
  ...
  function r(B, A) {
    this.selector = A && nA(A), this.data = [], this._r = [], this.isReverted = !1, this.id = ge++, B && this.add(B);
  }
  var Q = r.prototype;
  return Q.add = function(A, t, P) {
    Y(A) && (P = t, t = A, A = Y);
    var e = this, i = function() {
      var s = m, c = e.selector, n;
      return s && s !== e && s.data.push(e), P && (e.selector = nA(P)), m = e, n = t.apply(e, arguments), Y(n) && e._r.push(n), m = s, e.selector = c, e.isReverted = !1, n;
    };
    return e.last = i, A === Y ? i(e, function(v) {
      return e.add(null, v);
    }) : A ? e[A] = i : i;
  }, Q.ignore = function(A) {
    var t = m;
    ...
    var P = this;
    if (A ? function() {
      for (var i = P.getTweens(), v = P.data.length, s; v--; )
        s = P.data[v], s.data === "isFlip" && (s.revert(), s.getChildren(!0, !0, !1).forEach(function(c) {
      ...
      for (i.map(function(c) {
        ...
        s = P.data[v], s instanceof _ ? s.data !== "nested" && (s.scrollTrigger && s.scrollTrigger.revert(), s.kill()) : !(s instanceof J) && s.revert && s.revert(A);
  ...
  return Q.add = function(A, t, P) {
    ...
    var e = new Wt(0, P || this.scope), i = e.conditions = {}, v, s, c;
    m && !e.selector && (e.selector = m.selector), this.contexts.push(e), t = e.add("onMatch", t), e.queries = A;
    for (s in A)
      s === "all" ? c = 1 : (v = EQ.matchMedia(A[s]), v && (JQ.indexOf(e) < 0 && JQ.push(e), (i[s] = v.matches) && (c = 1), v.addListener ? v.addListener(zA) : v.addEventListener("change", zA)));
  ...
  getProperty: function(Q, B, A, t) {
    K(Q) && (Q = wQ(Q)[0]);
    var P = SQ(Q || {}).get, e = A ? Ct : ut;
    return A === "native" && (A = ""), Q && (B ? e((PQ[B] && PQ[B].get || P)(Q, B, A, t)) : function(i, v, s) {
      return e((PQ[i] && PQ[i].get || P)(Q, i, v, s));
    });
  },
  quickSetter: function(Q, B, A) {
    ...
    Q = Q[0] || {};
    var e = PQ[B], i = SQ(Q), v = i.harness && (i.harness.aliases || {})[B] || B, s = e ? function(c) {
      var n = new e();
      ZQ._pt = 0, n.init(Q, A ? c + A : c, ZQ, 0, [Q]), n.render(1, n), ZQ._pt && FA(1, ZQ);
    } : i.set(Q, v);
    return e ? s : function(c) {
      return s(Q, v, A ? c + A : c, i, 1);
    };
  },
  quickTo: function(Q, B, A) {
    var t, P = tQ.to(Q, cQ((t = {}, t[B] = "+=0.1", t.paused = !0, t.stagger = 0, t), A || {})), e = function(v, s, c) {
      return P.resetTo(B, v, s, c);
    };
    return e.tween = P, e;
  },
  ...
  defaults: function(Q) {
    return Q && Q.ease && (Q.ease = VQ(Q.ease, oB.ease)), SA(oB, Q || {});
  },
  config: function(Q) {
    return SA(vQ, Q || {});
  },
  registerEffect: function(Q) {
    ...
    (t || "").split(",").forEach(function(i) {
      return i && !PQ[i] && !sQ[i] && zB(B + " effect requires " + i + " plugin.");
    }), YB[B] = function(i, v, s) {
      return A(wQ(i), cQ(v || {}, P), s);
    }, e && (_.prototype[B] = function(i, v, s) {
      return this.add(YB[B](i, kQ(v) ? v : (s = v) && {}, this), s);
    });
  },
...
var xe = function(Q, B) {
  ...
  for (t in B)
    for (P = A.length; P--; )
      e = Q._ptLookup[P][t], e && (e = e.d) && (e._pt && (e = xe(e, t)), e && e.modifier && e.modifier(B[t], Q, A[P], t));
}, KB = function(Q, B) {
  return {
    name: Q,
    headless: 1,
  ...
  init: function(Q, B, A, t, P) {
    ...
    this.tween = A;
    for (e in B)
      v = Q.getAttribute(e) || "", i = this.add(Q, "setAttribute", (v || 0) + "", B[e], t, P, 0, 0, e), i.op = e, i.b = v, this._props.push(e);
  },
  render: function(Q, B) {
    for (var A = B._pt; A; )
...
}, KB("roundProps", TA), KB("modifiers"), KB("snap", Ft)) || GB;
 ...
 * @author: Jack Doyle, jack@greensock.com
*/
var KA, LQ, qQ, GA, IQ, ZA, fA, ye = function() {
  return typeof window < "u";
}, CQ = {}, yQ = 180 / Math.PI, _Q = Math.PI / 180, XQ = Math.atan2, WA = 1e8, bA = /([A-Z])/g, Ie = /(left|right|width|margin|padding|x)/i, Se = /[\s,\(]\S/, hQ = {
  ...
  if (Q in CQ && P) {
    if (this.tfm = this.tfm || {}, Q !== "transform")
      Q = hQ[Q] || Q, ~Q.indexOf(",") ? Q.split(",").forEach(function(i) {
        return A.tfm[i] = lQ(t, i);
      }) : this.tfm[Q] = e.x ? e[Q] : lQ(t, Q), Q === AQ && (this.tfm.zOrigin = e.zOrigin);
    else
  ...
  for (P = 0; P < Q.length; P += 3)
    Q[P + 1] ? Q[P + 1] === 2 ? B[Q[P]](Q[P + 2]) : B[Q[P]] = Q[P + 2] : Q[P + 2] ? A[Q[P]] = Q[P + 2] : A.removeProperty(Q[P].substr(0, 2) === "--" ? Q[P] : Q[P].replace(bA, "-$1").toLowerCase());
  if (this.tfm) {
    for (e in this.tfm)
      t[e] = this.tfm[e];
    t.svg && (t.renderTransform(), B.setAttribute("data-svg-origin", this.svgo || "")), P = fA(), (!P || !P.isStart) && !A[S] && ($t(A), t.zOrigin && A[AQ] && (A[AQ] += " " + t.zOrigin + "px", t.zOrigin = 0, t.renderTransform()), t.uncache = 1);
  }
}, QP = function(Q, B) {
  var A = {
    target: Q,
  ...
  return t[B] || t.getPropertyValue(B.replace(bA, "-$1").toLowerCase()) || t.getPropertyValue(B) || !A && r(Q, tB(B) || B, 1) || "";
}, qA = "O,Moz,ms,Ms,Webkit".split(","), tB = function(Q, B, A) {
  var t = B || IQ, P = t.style, e = 5;
  if (Q in P && !A)
    return Q;
  for (Q = Q.charAt(0).toUpperCase() + Q.substr(1); e-- && !(qA[e] + Q in P); )
    ;
  return e < 0 ? null : (e === 3 ? "ms" : e >= 0 ? qA[e] : "") + Q;
}, aA = function() {
  ye() && window.document && (KA = window, LQ = KA.document, qQ = LQ.documentElement, IQ = EA("div") || {
    style: {}
  }, EA("div"), S = tB(S), AQ = S + "Origin", IQ.style.cssText = "border-width:0;line-height:0;position:absolute;padding:0", BP = !!tB("perspective"), fA = tQ.core.reverting, GA = 1);
}, _A = function(Q) {
  var B = Q.ownerSVGElement, A = EA("svg", B && B.getAttribute("xmlns") || "http://www.w3.org/2000/svg"), t = Q.cloneNode(!0), P;
  ...
  return B && (B.width || B.height) || A || (B = _A(Q)), B && !B.width && !B.x && !B.y ? {
    x: +$A(Q, ["x", "cx", "x1"]) || 0,
    y: +$A(Q, ["y", "cy", "y1"]) || 0,
    width: 0,
    height: 0
  ...
  return !!(Q.getCTM && (!Q.parentNode || Q.ownerSVGElement) && AP(Q));
}, GQ = function(Q, B) {
  if (B) {
    var A = Q.style, t;
    B in CQ && B !== AQ && (B = S), A.removeProperty ? (t = B.substr(0, 2), (t === "ms" || B.substr(0, 6) === "webkit") && (B = "-" + B), A.removeProperty(t === "--" ? B : B.replace(bA, "-$1").toLowerCase())) : A.removeAttribute(B);
  }
}, MQ = function(Q, B, A, t, P, e) {
  var i = new BQ(Q._pt, B, A, 0, 1, e ? _t : qt);
  ...
  var P = parseFloat(A) || 0, e = (A + "").trim().substr((P + "").length) || "px", i = IQ.style, v = Ie.test(B), s = Q.tagName.toLowerCase() === "svg", c = (s ? "client" : "offset") + (v ? "Width" : "Height"), n = 100, o = t === "px", z = t === "%", w, T, E, h;
  if (t === e || !P || Qt[t] || Qt[e])
    return P;
  if (e !== "px" && !o && (P = r(Q, B, A, "px")), h = Q.getCTM && tP(Q), (z || e === "%") && (CQ[B] || ~B.indexOf("adius")))
    return w = h ? Q.getBBox()[v ? "width" : "height"] : Q[c], V(z ? P / w * n : P / 100 * w);
  if (i[v ? "width" : "height"] = n + (o ? e : t), T = t !== "rem" && ~B.indexOf("adius") || t === "em" && Q.appendChild && !s ? Q : Q.parentNode, h && (T = (Q.ownerSVGElement || {}).parentNode), (!T || T === LQ || !T.appendChild) && (T = LQ.body), E = T._gsap, E && z && E.width && v && E.time === eQ.time && !E.uncache)
    return V(P / E.width * n);
  if (z && (B === "height" || B === "width")) {
    var a = Q.style[B];
    Q.style[B] = n + t, w = Q[c], a ? Q.style[B] = a : GQ(Q, B);
  } else
    (z || e === "%") && !Bi[rQ(T, "display")] && (i.position = rQ(Q, "position")), T === Q && (i.position = "static"), T.appendChild(IQ), w = IQ[c], T.removeChild(IQ), i.position = "absolute";
  return v && z && (E = SQ(T), E.time = eQ.time, E.width = T[c]), V(o ? w * P / n : w && P ? n / w * P : 0);
}, lQ = function(Q, B, A, t) {
  var P;
  return GA || aA(), B in hQ && B !== "transform" && (B = hQ[B], ~B.indexOf(",") && (B = B.split(",")[0])), CQ[B] && B !== "transform" ? (P = UB(Q, t), P = B !== "transformOrigin" ? P[B] : P.svg ? P.origin : bB(rQ(Q, AQ)) + " " + P.zOrigin + "px") : (P = Q.style[B], (!P || P === "auto" || t || ~(P + "").indexOf("calc(")) && (P = fB[B] && fB[B](Q, B, A) || rQ(Q, B) || Dt(Q, B) || (B === "opacity" ? 1 : 0))), A && !~(P + "").trim().indexOf(" ") ? fQ(Q, B, P, A) + A : P;
}, Ai = function(Q, B, A, t) {
  if (!A || A === "none") {
    var P = tB(B, Q, 1), e = P && rQ(Q, P, 1);
    e && e !== A ? (B = P, A = e) : B === "borderColor" && (A = rQ(Q, "borderTopColor"));
  }
  var i = new BQ(this._pt, Q.style, B, 0, 1, Kt), v = 0, s = 0, c, n, o, z, w, T, E, h, a, D, l, U;
  if (i.b = A, i.e = t, A += "", t += "", t.substring(0, 6) === "var(--" && (t = rQ(Q, t.substring(4, t.indexOf(")")))), t === "auto" && (T = Q.style[B], Q.style[B] = t, t = rQ(Q, B) || t, T ? Q.style[B] = T : GQ(Q, B)), c = [A, t], yt(c), A = c[0], t = c[1], o = A.match(KQ) || [], U = t.match(KQ) || [], U.length) {
    for (; n = KQ.exec(t); )
      E = n[0], a = t.substring(v, n.index), w ? w = (w + 1) % 5 : (a.substr(-5) === "rgba(" || a.substr(-5) === "hsla(") && (w = 1), E !== (T = o[s++] || "") && (z = parseFloat(T) || 0, l = T.substr((z + "").length), E.charAt(1) === "=" && (E = WQ(z, E) + l), h = parseFloat(E), D = E.substr((h + "").length), v = KQ.lastIndex - D.length, D || (D = D || vQ.units[B] || l, v === t.length && (t += D, i.e += D)), l !== D && (z = fQ(Q, B, T, D) || 0), i._pt = {
        _next: i._pt,
        p: a || s === 1 ? a : ",",
        //note: SVG spec allows omission of comma/space when a negative sign is wedged between two numbers, like 2.5-5.3 instead of 2.5,-5.3 but when tweening, the negative value may switch to positive, so we insert the comma just in case.
        s: z,
    ...
    i.c = v < t.length ? t.substring(v, t.length) : "";
  } else
    i.r = B === "display" && t === "none" ? _t : qt;
  return Et.test(t) && (i.e = 0), this._pt = i, i;
}, Bt = {
  top: "0%",
  bottom: "100%",
  left: "0%",
  ...
  return (A === "top" || A === "bottom" || t === "left" || t === "right") && (Q = A, A = t, t = Q), B[0] = Bt[A] || A, B[1] = Bt[t] || t, B.join(" ");
}, Pi = function(Q, B) {
  if (B.tween && B.tween._time === B.tween._dur) {
    var A = B.t, t = A.style, P = B.u, e = A._gsap, i, v, s;
    if (P === "all" || P === !0)
      t.cssText = "", v = 1;
    else
      for (P = P.split(","), s = P.length; --s > -1; )
        i = P[s], CQ[i] && (v = 1, i = i === "transformOrigin" ? AQ : S), GQ(A, i);
    v && (GQ(A, S), e && (e.svg && A.removeAttribute("transform"), t.scale = t.rotate = t.translate = "none", UB(A, 1), e.uncache = 1, $t(t)));
  }
}, fB = {
  clearProps: function(Q, B, A, t, P) {
    if (P.data !== "isFromStart") {
      var e = Q._pt = new BQ(Q._pt, B, A, 0, 0, Pi);
  ...
  */
}, kB = [1, 0, 0, 1, 0, 0], PP = {}, eP = function(Q) {
  return Q === "matrix(1, 0, 0, 1, 0, 0)" || Q === "none" || !Q;
}, At = function(Q) {
  var B = rQ(Q, S);
  return eP(B) ? kB : B.substr(7).match(wt).map(V);
}, gA = function(Q, B) {
  var A = Q._gsap || SQ(Q), t = Q.style, P = At(Q), e, i, v, s;
  return A.svg && Q.getAttribute("transform") ? (v = Q.transform.baseVal.consolidate().matrix, P = [v.a, v.b, v.c, v.d, v.e, v.f], P.join(",") === "1,0,0,1,0,0" ? kB : P) : (P === kB && !Q.offsetParent && Q !== qQ && !A.svg && (v = t.display, t.display = "block", e = Q.parentNode, (!e || !Q.offsetParent && !Q.getBoundingClientRect().width) && (s = 1, i = Q.nextElementSibling, qQ.appendChild(Q)), P = At(Q), v ? t.display = v : GQ(Q, "display"), s && (i ? e.insertBefore(Q, i) : e ? e.appendChild(Q) : qQ.removeChild(Q))), B && P.length > 6 ? [P[0], P[1], P[4], P[5], P[12], P[13]] : P);
}, hA = function(Q, B, A, t, P, e) {
  var i = Q._gsap, v = P || gA(Q, !0), s = i.xOrigin || 0, c = i.yOrigin || 0, n = i.xOffset || 0, o = i.yOffset || 0, z = v[0], w = v[1], T = v[2], E = v[3], h = v[4], a = v[5], D = B.split(" "), l = parseFloat(D[0]) || 0, U = parseFloat(D[1]) || 0, O, u, N, C;
  A ? v !== kB && (u = z * E - w * T) && (N = l * (E / u) + U * (-T / u) + (T * a - E * h) / u, C = l * (-w / u) + U * (z / u) - (z * a - w * h) / u, l = N, U = C) : (O = AP(Q), l = O.x + (~D[0].indexOf("%") ? l / 100 * O.width : l), U = O.y + (~(D[1] || D[0]).indexOf("%") ? U / 100 * O.height : U)), t || t !== !1 && i.smooth ? (h = l - s, a = U - c, i.xOffset = n + (h * z + a * T) - h, i.yOffset = o + (h * w + a * E) - a) : i.xOffset = i.yOffset = 0, i.xOrigin = l, i.yOrigin = U, i.smooth = !!t, i.origin = B, i.originIsAbsolute = !!A, Q.style[AQ] = "0px 0px", e && (MQ(e, i, "xOrigin", s, l), MQ(e, i, "yOrigin", c, U), MQ(e, i, "xOffset", n, i.xOffset), MQ(e, i, "yOffset", o, i.yOffset)), Q.setAttribute("data-svg-origin", l + " " + U);
}, UB = function(Q, B) {
  var A = Q._gsap || new St(Q);
  if ("x" in A && !B && !A.uncache)
    return A;
  var t = Q.style, P = A.scaleX < 0, e = "px", i = "deg", v = getComputedStyle(Q), s = rQ(Q, AQ) || "0", c, n, o, z, w, T, E, h, a, D, l, U, O, u, N, C, M, b, j, d, G, f, H, F, X, nQ, NQ, gQ, TQ, R, UQ, pQ;
  return c = n = o = T = E = h = a = D = l = 0, z = w = 1, A.svg = !!(Q.getCTM && tP(Q)), v.translate && ((v.translate !== "none" || v.scale !== "none" || v.rotate !== "none") && (t[S] = (v.translate !== "none" ? "translate3d(" + (v.translate + " 0 0").split(" ").slice(0, 3).join(", ") + ") " : "") + (v.rotate !== "none" ? "rotate(" + v.rotate + ") " : "") + (v.scale !== "none" ? "scale(" + v.scale.split(" ").join(",") + ") " : "") + (v[S] !== "none" ? v[S] : "")), t.scale = t.rotate = t.translate = "none"), u = gA(Q, A.svg), A.svg && (A.uncache ? (X = Q.getBBox(), s = A.xOrigin - X.x + "px " + (A.yOrigin - X.y) + "px", F = "") : F = !B && Q.getAttribute("data-svg-origin"), hA(Q, F || s, !!F || A.originIsAbsolute, A.smooth !== !1, u)), U = A.xOrigin || 0, O = A.yOrigin || 0, u !== kB && (b = u[0], j = u[1], d = u[2], G = u[3], c = f = u[4], n = H = u[5], u.length === 6 ? (z = Math.sqrt(b * b + j * j), w = Math.sqrt(G * G + d * d), T = b || j ? XQ(j, b) * yQ : 0, a = d || G ? XQ(d, G) * yQ + T : 0, a && (w *= Math.abs(Math.cos(a * _Q))), A.svg && (c -= U - (U * b + O * d), n -= O - (U * j + O * G))) : (pQ = u[6], R = u[7], NQ = u[8], gQ = u[9], TQ = u[10], UQ = u[11], c = u[12], n = u[13], o = u[14], N = XQ(pQ, TQ), E = N * yQ, N && (C = Math.cos(-N), M = Math.sin(-N), F = f * C + NQ * M, X = H * C + gQ * M, nQ = pQ * C + TQ * M, NQ = f * -M + NQ * C, gQ = H * -M + gQ * C, TQ = pQ * -M + TQ * C, UQ = R * -M + UQ * C, f = F, H = X, pQ = nQ), N = XQ(-d, TQ), h = N * yQ, N && (C = Math.cos(-N), M = Math.sin(-N), F = b * C - NQ * M, X = j * C - gQ * M, nQ = d * C - TQ * M, UQ = G * M + UQ * C, b = F, j = X, d = nQ), N = XQ(j, b), T = N * yQ, N && (C = Math.cos(N), M = Math.sin(N), F = b * C + j * M, X = f * C + H * M, j = j * C - b * M, H = H * C - f * M, b = F, f = X), E && Math.abs(E) + Math.abs(T) > 359.9 && (E = T = 0, h = 180 - h), z = V(Math.sqrt(b * b + j * j + d * d)), w = V(Math.sqrt(H * H + pQ * pQ)), N = XQ(f, H), a = Math.abs(N) > 2e-4 ? N * yQ : 0, l = UQ ? 1 / (UQ < 0 ? -UQ : UQ) : 0), A.svg && (F = Q.getAttribute("transform"), A.forceCSS = Q.setAttribute("transform", "") || !eP(rQ(Q, S)), F && Q.setAttribute("transform", F))), Math.abs(a) > 90 && Math.abs(a) < 270 && (P ? (z *= -1, a += T <= 0 ? 180 : -180, T += T <= 0 ? 180 : -180) : (w *= -1, a += a <= 0 ? 180 : -180)), B = B || A.uncache, A.x = c - ((A.xPercent = c && (!B && A.xPercent || (Math.round(Q.offsetWidth / 2) === Math.round(-c) ? -50 : 0))) ? Q.offsetWidth * A.xPercent / 100 : 0) + e, A.y = n - ((A.yPercent = n && (!B && A.yPercent || (Math.round(Q.offsetHeight / 2) === Math.round(-n) ? -50 : 0))) ? Q.offsetHeight * A.yPercent / 100 : 0) + e, A.z = o + e, A.scaleX = V(z), A.scaleY = V(w), A.rotation = V(T) + i, A.rotationX = V(E) + i, A.rotationY = V(h) + i, A.skewX = a + i, A.skewY = D + i, A.transformPerspective = l + e, (A.zOrigin = parseFloat(s.split(" ")[2]) || !B && A.zOrigin || 0) && (t[AQ] = bB(s)), A.xOffset = A.yOffset = 0, A.force3D = vQ.force3D, A.renderTransform = A.svg ? ii : BP ? iP : ei, A.uncache = 0, A;
}, bB = function(Q) {
  ...
  B.z = "0px", B.rotationY = B.rotationX = "0deg", B.force3D = 0, iP(Q, B);
}, xQ = "0deg", PB = "0px", mQ = ") ", iP = function(Q, B) {
  var A = B || this, t = A.xPercent, P = A.yPercent, e = A.x, i = A.y, v = A.z, s = A.rotation, c = A.rotationY, n = A.rotationX, o = A.skewX, z = A.skewY, w = A.scaleX, T = A.scaleY, E = A.transformPerspective, h = A.force3D, a = A.target, D = A.zOrigin, l = "", U = h === "auto" && Q && Q !== 1 || h === !0;
  if (D && (n !== xQ || c !== xQ)) {
    var O = parseFloat(c) * _Q, u = Math.sin(O), N = Math.cos(O), C;
    O = parseFloat(n) * _Q, C = Math.cos(O), e = ZB(a, e, u * C * -D), i = ZB(a, i, -Math.sin(O) * -D), v = ZB(a, v, N * C * -D + D);
  }
  E !== PB && (l += "perspective(" + E + mQ), (t || P) && (l += "translate(" + t + "%, " + P + "%) "), (U || e !== PB || i !== PB || v !== PB) && (l += v !== PB || U ? "translate3d(" + e + ", " + i + ", " + v + ") " : "translate(" + e + ", " + i + mQ), s !== xQ && (l += "rotate(" + s + mQ), c !== xQ && (l += "rotateY(" + c + mQ), n !== xQ && (l += "rotateX(" + n + mQ), (o !== xQ || z !== xQ) && (l += "skew(" + o + ", " + z + mQ), (w !== 1 || T !== 1) && (l += "scale(" + w + ", " + T + mQ), a.style[S] = l || "translate(0, 0)";
}, ii = function(Q, B) {
  var A = B || this, t = A.xPercent, P = A.yPercent, e = A.x, i = A.y, v = A.rotation, s = A.skewX, c = A.skewY, n = A.scaleX, o = A.scaleY, z = A.target, w = A.xOrigin, T = A.yOrigin, E = A.xOffset, h = A.yOffset, a = A.forceCSS, D = parseFloat(e), l = parseFloat(i), U, O, u, N, C;
  v = parseFloat(v), s = parseFloat(s), c = parseFloat(c), c && (c = parseFloat(c), s += c, v += c), v || s ? (v *= _Q, s *= _Q, U = Math.cos(v) * n, O = Math.sin(v) * n, u = Math.sin(v - s) * -o, N = Math.cos(v - s) * o, s && (c *= _Q, C = Math.tan(s - c), C = Math.sqrt(1 + C * C), u *= C, N *= C, c && (C = Math.tan(c), C = Math.sqrt(1 + C * C), U *= C, O *= C)), U = V(U), O = V(O), u = V(u), N = V(N)) : (U = n, N = o, O = u = 0), (D && !~(e + "").indexOf("px") || l && !~(i + "").indexOf("px")) && (D = fQ(z, "x", e, "px"), l = fQ(z, "y", i, "px")), (w || T || E || h) && (D = V(D + w - (w * U + T * u) + E), l = V(l + T - (w * O + T * N) + h)), (t || P) && (C = z.getBBox(), D = V(D + t / 100 * C.width), l = V(l + P / 100 * C.height)), C = "matrix(" + U + "," + O + "," + u + "," + N + "," + D + "," + l + ")", z.setAttribute("transform", C), a && (z.style[S] = C);
}, ri = function(Q, B, A, t, P) {
  var e = 360, i = K(P), v = parseFloat(P) * (i && ~P.indexOf("rad") ? yQ : 1), s = v - t, c = t + s + "deg", n, o;
  return i && (n = P.split("_")[1], n === "short" && (s %= e, s !== s % (e / 2) && (s += s < 0 ? e : -e)), n === "cw" && s < 0 ? s = (s + e * WA) % e - ~~(s / e) * e : n === "ccw" && s > 0 && (s = (s - e * WA) % e - ~~(s / e) * e)), Q._pt = o = new BQ(Q._pt, B, A, t, s, Ye), o.e = c, o.u = "deg", Q._props.push(A), o;
}, tt = function(Q, B) {
  for (var A in B)
    Q[A] = B[A];
  return Q;
}, vi = function(Q, B, A) {
  var t = tt({}, A._gsap), P = "perspective,force3D,transformOrigin,svgOrigin", e = A.style, i, v, s, c, n, o, z, w;
  t.svg ? (s = A.getAttribute("transform"), A.setAttribute("transform", ""), e[S] = B, i = UB(A, 1), GQ(A, S), A.setAttribute("transform", s)) : (s = getComputedStyle(A)[S], e[S] = B, i = UB(A, 1), e[S] = s);
  for (v in CQ)
    s = t[v], c = i[v], s !== c && P.indexOf(v) < 0 && (z = W(s), w = W(c), n = z !== w ? fQ(A, v, s, w) : parseFloat(s), o = parseFloat(c), Q._pt = new BQ(Q._pt, i, v, n, o - n, wA), Q._pt.u = w || 0, Q._props.push(v));
  tt(i, t);
};
QQ("padding,margin,Width,Radius", function(r, Q) {
  ...
  fB[Q > 1 ? "border" + r : r] = function(i, v, s, c, n) {
    var o, z;
    if (arguments.length < 4)
      return o = e.map(function(w) {
        return lQ(i, w, s);
      }), z = o.join(" "), z.split(o[0]).length === 5 ? o[0] : z;
    o = (c + "").split(" "), z = {}, e.forEach(function(w, T) {
      return z[w] = o[T] = o[T] || o[(T - 1) / 2 | 0];
    }), i.init(v, z, n);
  };
});
var rP = {
  ...
  init: function(Q, B, A, t, P) {
    var e = this._props, i = Q.style, v = A.vars.startAt, s, c, n, o, z, w, T, E, h, a, D, l, U, O, u, N, C;
    GA || aA(), this.styles = this.styles || QP(Q), N = this.styles.props, this.tween = A;
    for (T in B)
      if (T !== "autoRound" && (c = B[T], !(PQ[T] && Yt(T, B, A, t, Q, P)))) {
        if (z = typeof c, w = fB[T], z === "function" && (c = c.call(A, t, Q, P), z = typeof c), z === "string" && ~c.indexOf("random(") && (c = EB(c)), w)
          w(this, Q, T, c, A) && (u = 1);
        else if (T.substr(0, 2) === "--")
          s = (getComputedStyle(Q).getPropertyValue(T) + "").trim(), c += "", jQ.lastIndex = 0, jQ.test(s) || (E = W(s), h = W(c), h ? E !== h && (s = fQ(Q, T, s, h) + h) : E && (c += E)), this.add(i, "setProperty", s, c, t, P, 0, 0, T), e.push(T), N.push(T, 0, i[T]);
        else if (z !== "undefined") {
          if (v && T in v ? (s = typeof v[T] == "function" ? v[T].call(A, t, Q, P) : v[T], K(s) && ~s.indexOf("random(") && (s = EB(s)), W(s + "") || s === "auto" || (s += vQ.units[T] || W(lQ(Q, T)) || ""), (s + "").charAt(1) === "=" && (s = lQ(Q, T))) : s = lQ(Q, T), o = parseFloat(s), a = z === "string" && c.charAt(1) === "=" && c.substr(0, 2), a && (c = c.substr(2)), n = parseFloat(c), T in hQ && (T === "autoAlpha" && (o === 1 && lQ(Q, "visibility") === "hidden" && n && (o = 0), N.push("visibility", 0, i.visibility), MQ(this, i, "visibility", o ? "inherit" : "hidden", n ? "inherit" : "hidden", !n)), T !== "scale" && T !== "transform" && (T = hQ[T], ~T.indexOf(",") && (T = T.split(",")[0]))), D = T in CQ, D) {
            if (this.styles.save(T), C = c, z === "string" && c.substring(0, 6) === "var(--") {
            ...
            if (l || (U = Q._gsap, U.renderTransform && !B.parseTransform || UB(Q, B.parseTransform), O = B.smoothOrigin !== !1 && U.smooth, l = this._pt = new BQ(this._pt, i, S, 0, 1, U.renderTransform, U, 0, -1), l.dep = 1), T === "scale")
              this._pt = new BQ(this._pt, U, "scaleY", U.scaleY, (a ? WQ(U.scaleY, a + n) : n) - U.scaleY || 0, wA), this._pt.u = 0, e.push("scaleY", T), T += "X";
            else if (T === "transformOrigin") {
              N.push(AQ, 0, i[AQ]), c = ti(c), U.svg ? hA(Q, c, 0, O, 0, this) : (h = parseFloat(c.split(" ")[2]) || 0, h !== U.zOrigin && MQ(this, U, "zOrigin", U.zOrigin, h), MQ(this, i, T, bB(s), bB(c)));
              continue;
            } else if (T === "svgOrigin") {
              hA(Q, c, 1, O, 0, this);
              continue;
            } else if (T in PP) {
              ri(this, U, T, o, a ? WQ(o, a + c) : c);
              continue;
            } else if (T === "smoothOrigin") {
              MQ(this, U, "smooth", U.smooth, c);
              continue;
            } else if (T === "force3D") {
              U[T] = c;
              continue;
          ...
          } else T in i || (T = tB(T) || T);
          if (D || (n || n === 0) && (o || o === 0) && !Se.test(c) && T in i)
            E = (s + "").substr((o + "").length), n || (n = 0), h = W(c) || (T in vQ.units ? vQ.units[T] : E), E !== h && (o = fQ(Q, T, s, h)), this._pt = new BQ(this._pt, D ? U : i, T, o, (a ? WQ(o, a + n) : n) - o, !D && (h === "px" || T === "zIndex") && B.autoRound !== !1 ? Re : wA), this._pt.u = h || 0, D && C !== c ? (this._pt.b = s, this._pt.e = C, this._pt.r = Je) : E !== h && h !== "%" && (this._pt.b = s, this._pt.r = Ve);
          else if (T in i)
            Ai.call(this, Q, T, s, a ? a + c : c);
          else if (T in Q)
            this.add(Q, T, s || Q[T], a ? a + c : c, t, P);
          else if (T !== "parseTransform") {
            uA(T, c);
            continue;
          }
          D || (T in i ? N.push(T, 0, i[T]) : typeof Q[T] == "function" ? N.push(T, 2, Q[T]()) : N.push(T, 1, s || Q[T])), e.push(T);
        }
      }
    u && Zt(this);
  },
  ...
  getSetter: function(Q, B, A) {
    var t = hQ[B];
    return t && t.indexOf(",") < 0 && (B = t), B in CQ && B !== AQ && (Q._gsap.x || lQ(Q, "x")) ? A && ZA === A ? B === "scale" ? We : Ze : (ZA = A || {}) && (B === "scale" ? qe : _e) : Q.style && !UA(Q.style[B]) ? Xe : ~B.indexOf("-") ? Ke : jA(Q, B);
  },
  core: {
    _removeProperty: GQ,
    _getMatrix: gA
...
tQ.core.getStyleSaver = QP;
(function(r, Q, B, A) {
  var t = QQ(r + "," + Q + "," + B, function(P) {
    CQ[P] = 1;
  });
  QQ(Q, function(P) {
    vQ.units[P] = "deg", PP[P] = 1;
...
function Ti(r) {
  const Q = Math.max(1e-3, r), B = Q * 1.6, A = Q * 0.7, t = new k.ConeGeometry(A, B, 3, 1, !1);
  return t.rotateX(-Math.PI / 2), t.translate(0, 0, B / 2), t;
}
function oi(r) {
  const B = Math.max(1e-3, r) * 0.65, A = new k.SphereGeometry(B, 18, 12);
  return A.translate(0, 0, B), A;
}
function sP(r) {
  const Q = atob(si), B = new Uint8Array(Q.length);
  for (let s = 0; s < Q.length; s++) B[s] = Q.charCodeAt(s);
  const t = new EP().parse(B.buffer);
  t.rotateX(-Math.PI / 2), t.computeBoundingBox();
  const P = t.boundingBox, e = new k.Vector3();
  P.getSize(e);
  const i = Math.max(1e-3, r) * 1.6 / Math.max(e.x, e.y, e.z, 1e-3);
  t.scale(i, i, i), t.computeBoundingBox();
  const v = t.boundingBox;
  return t.translate(
    -(v.min.x + v.max.x) / 2,
    -(v.min.y + v.max.y) / 2,
    -v.min.z
...
function zi(r, Q) {
  const B = Math.max(1e-3, r), A = B * 3, t = B * 0.15, P = B * 0.35, e = lB(Q), i = new k.CylinderGeometry(0, t, A, 8, 1, !1);
  i.rotateX(-Math.PI / 2), i.translate(0, 0, A / 2);
  const v = new k.CylinderGeometry(0, P, A, 8, 1, !0);
  v.rotateX(-Math.PI / 2), v.translate(0, 0, A / 2);
  const s = new k.MeshBasicMaterial({
    color: new k.Color(et),
    transparent: !0,
    opacity: e,
    depthTest: !1,
    depthWrite: !1,
    blending: k.AdditiveBlending
  }), c = new k.MeshBasicMaterial({
    color: new k.Color(et),
    transparent: !0,
    opacity: e * 0.3,
    depthTest: !1,
  ...
  n.renderOrder = 1e3;
  const o = new k.Mesh(v, c);
  o.renderOrder = 1e3;
  const z = new k.Group();
  return z.add(n), z.add(o), z;
}
function wi(r) {
  const Q = Math.max(1e-3, r.bit.size);
  return r.bit.type === "triangle" ? Ti(Q) : r.bit.type === "drill" ? sP(Q) : oi(Q);
}
function cP(r) {
  return r.bit.colorSource === "custom" ? r.bit.color : ci;
}
function Ei(r) {
  const Q = lB(r.bit.opacity), B = sP(Math.max(1e-3, r.bit.size)), A = new k.MeshStandardMaterial({
    color: new k.Color(cP(r)),
    metalness: 0.75,
    roughness: 0.3,
    transparent: Q < 1,
    opacity: Q,
...
function ai(r) {
  const Q = lB(r.bit.opacity), B = wi(r), A = new k.MeshBasicMaterial({
    color: new k.Color(vP),
    transparent: Q < 1,
    opacity: Q,
    side: k.DoubleSide,
    depthTest: !1,
...
function it(r) {
  return r.bit.type === "laser" ? zi(r.bit.size, r.bit.opacity) : r.bit.type === "drill" ? Ei(r) : ai(r);
}
function rt(r) {
  if (r instanceof k.Mesh)
    r.geometry.dispose(), r.material.dispose();
  else if (r instanceof k.Group)
    for (const Q of r.children)
      Q instanceof k.Mesh && (Q.geometry.dispose(), Q.material.dispose());
}
function hi(r) {
  const Q = new k.Group();
  ...
  e = v(B.bit.spinRpm);
  const s = (n) => {
    const o = n.bit.type, z = B.bit.type, w = n.bit.size, T = B.bit.size, E = o !== z, h = w !== T, a = lB(n.bit.opacity);
    if (E || h)
      Q.remove(A), rt(A), A = it(n), Q.add(A), i == null || i.kill(), e == null || e.kill(), e = v(n.bit.spinRpm), P && e.timeScale(1).play();
    else if (o === "laser") {
      const l = A, [U, O] = l.children, u = U.material, N = O.material;
      u.opacity = a, u.needsUpdate = !0, N.opacity = a * 0.3, N.needsUpdate = !0;
    } else {
      const U = A.material;
      U.color = new k.Color(o === "drill" ? cP(n) : vP), U.opacity = a, U.transparent = a < 1, U.needsUpdate = !0;
    }
  ...
  return {
    ...
    setOptions: (n) => s(n),
    setTarget: (n, o) => {
      const z = !!(o != null && o.immediate), w = Number(n.x) || 0, T = Number(n.y) || 0, E = Number(n.z) || 0;
      if (n.a, z || B.bit.tweenMs <= 0) {
        t = null, Q.position.set(w, T, E);
        return;
      }
      t = {
    ...
    update: (n) => {
      if (!t) return;
      const o = (n - t.startedAt) / t.duration, z = ni(o);
      Q.position.set(
        k.MathUtils.lerp(t.from.x, t.to.x, z),
        k.MathUtils.lerp(t.from.y, t.to.y, z),
        k.MathUtils.lerp(t.from.z, t.to.z, z)
      ), o >= 1 && (Q.position.set(t.to.x, t.to.y, t.to.z), t = null);
    },
    dispose: () => {
      i == null || i.kill(), e == null || e.kill(), Q.remove(A), rt(A);
    }
  };
}
const ki = 25.4;
class di {
  constructor(Q) {
    this.viewCube = null, this.resizeObserver = null, this.onWindowResize = null, this.animationFrameId = null, this.gridGroup = null, this.axesGroup = null, this.gridLabelsGroup = null, this.boundingBoxGroup = null, this.bitMarker = null, this.preLaserBitType = "drill", this.toolpathStreams = [], this.toolpathCutBucketCount = 1, this.toolpathRotationA = 0, this.sim3dHandle = null, this.currentLines = [], this.linePositions = null, this.renderSequence = 0, this.currentBounds = null, this.cameraFocusTransition = null, this.id = Q.id, this.container = Q.container, this.callbacks = Q.callbacks ?? {}, this.options = st(CP, Q.options), this.canvas = document.createElement("canvas"), this.canvas.style.width = "100%", this.canvas.style.height = "100%", this.canvas.style.display = "block", this.container.appendChild(this.canvas), gP(this.container), this.renderer = new k.WebGLRenderer({
      canvas: this.canvas,
      antialias: this.options.render.antialias
    }), this.renderer.setPixelRatio(window.devicePixelRatio), this.renderer.setClearColor(this.options.render.theme.background, 1), this.scene = new k.Scene(), this.scene.add(new k.AmbientLight(16777215, 0.7)), this.scene.add(new k.DirectionalLight(16777215, 0.6)), this.toolpathRoot = new k.Group(), this.toolpathRoot.name = "gviewer:toolpath-root", this.scene.add(this.toolpathRoot), this.camera = new k.PerspectiveCamera(this.options.camera.fov, 1, 0.1, 1e5), this.camera.up.set(0, 0, 1), this.camera.position.set(
      this.options.camera.initialPosition.x,
      this.options.camera.initialPosition.y,
      this.options.camera.initialPosition.z
    ), this.controls = new nP(this.camera, this.renderer.domElement), this.controls.enableDamping = this.options.camera.orbit.enableDamping, this.controls.update(), this.viewCubeCorrection = new k.Matrix4().makeRotationX(k.MathUtils.degToRad(90)), this.viewCube = new HP({
      container: this.container,
      onSelectView: (B) => {
        const A = new k.Vector3();
        let t;
        if (this.currentBounds) {
          this.currentBounds.getCenter(A);
          const P = new k.Vector3();
          this.currentBounds.getSize(P);
          const e = Math.max(P.x, P.y, 1) / 2, i = k.MathUtils.degToRad(this.camera.fov);
          t = e / Math.tan(i / 2) * 1.25 + P.z;
        } else
          t = this.camera.position.distanceTo(this.controls.target);
        this.startSnapToView(B, A, Math.max(1e-6, t), 400);
      }
  ...
  setToolpathRotationA(Q) {
    const B = Number(Q) || 0;
    this.toolpathRotationA = B, this.toolpathRoot.rotation.x = k.MathUtils.degToRad(B);
  }
  setCallbacks(Q) {
    this.callbacks = Q;
  }
  ...
  startSnapToView(Q, B, A, t) {
    const P = xA(Q), e = B.clone().add(P.multiplyScalar(A)), i = Math.max(1, A);
  ...
  setOptions(Q) {
    ...
    this.options = st(this.options, Q), (B.render.antialias !== this.options.render.antialias || B.render.theme.background !== this.options.render.theme.background) && this.renderer.setClearColor(this.options.render.theme.background, 1), (B.render.theme.opacity !== this.options.render.theme.opacity || B.render.theme.rapidOpacity !== this.options.render.theme.rapidOpacity) && this.refreshToolpathOpacities(), B.camera.fov !== this.options.camera.fov && (this.camera.fov = this.options.camera.fov, this.camera.updateProjectionMatrix()), B.camera.orbit.enableDamping !== this.options.camera.orbit.enableDamping && (this.controls.enableDamping = this.options.camera.orbit.enableDamping);
    ...
    if (B.sim3d.erosionPasses !== this.options.sim3d.erosionPasses && this.sim3dHandle) {
      const o = this.sim3dHandle.currentLine, z = mB(o, this.sim3dHandle.data);
  ...
  ensureBitMarker() {
    this.bitMarker || (this.bitMarker = hi(this.options), this.scene.add(this.bitMarker.object));
  }
  worldSizes() {
    const Q = this.options.units === "in" ? ki : 1;
    return {
  ...
  renderGridAndAxes() {
    ...
    ), this.setAxesGroup(IP({ sizeWorld: Q, depthWorld: B, theme: this.options.render.theme }));
  }
  refreshGridLabels() {
    ...
    this.setGridLabelsGroup(
      YP({ sizeMm: Q, units: this.options.units, theme: this.options.render.theme })
    );
  }
  refreshBoundingBox() {
    ...
    this.setBoundingBoxGroup(GP(this.currentBounds, this.options));
  }
  setBitMarker(Q) {
  ...
  setGridGroup(Q) {
    this.gridGroup && (this.scene.remove(this.gridGroup), yP(this.gridGroup), this.gridGroup = null), Q && (this.gridGroup = Q, this.scene.add(Q));
  }
  setAxesGroup(Q) {
    this.axesGroup && (this.scene.remove(this.axesGroup), SP(this.axesGroup), this.axesGroup = null), Q && (this.axesGroup = Q, this.scene.add(Q));
  }
  setGridLabelsGroup(Q) {
    this.gridLabelsGroup && (this.scene.remove(this.gridLabelsGroup), jP(this.gridLabelsGroup), this.gridLabelsGroup = null), Q && (this.gridLabelsGroup = Q, this.scene.add(Q));
  }
  setBoundingBoxGroup(Q) {
    this.boundingBoxGroup && (this.scene.remove(this.boundingBoxGroup), bP(this.boundingBoxGroup), this.boundingBoxGroup = null), Q && (this.boundingBoxGroup = Q, this.scene.add(Q));
  }
  setGeometryEmpty() {
    this.setSim3dHandle(null), hP(this.toolpathRoot, this.toolpathStreams), this.toolpathStreams = [], this.toolpathCutBucketCount = 1, this.linePositions = null, this.currentBounds = null, this.emitBoundsChanged(), this.refreshBoundingBox();
  }
  ...
  refreshToolpathColors() {
    kP(this.toolpathStreams, this.options);
  }
  ...
  buildLinePositions() {
    ...
    const e = new BA({
      onLinearMove: (i) => {
        const v = i.transformedEnd ?? i.end;
        B = v.X, A = v.Y, t = v.Z, P = i.end.A;
      },
      onArcMove: (i) => {
        const v = i.transformedEnd ?? i.end;
        B = v.X, A = v.Y, t = v.Z, P = i.end.A;
    ...
    for (let i = 0; i < Q; i++) {
      const v = this.currentLines[i];
      v && e.processLine(v), this.linePositions[i * 4] = B, this.linePositions[i * 4 + 1] = A, this.linePositions[i * 4 + 2] = t, this.linePositions[i * 4 + 3] = P;
    }
  }
  async renderScene() {
    ...
    try {
      ...
      const B = await oP(this.currentLines, {
  ...
  async buildAndApplySim3d(Q) {
    ...
    let t;
    try {
      t = await NP(this.currentLines, B, A, {
        arcSegments: this.options.geometry.arcSegments,
        batch: {
          shouldAbort: () => Q !== this.renderSequence,
          onProgress: (i, v) => {
  ...
  startCameraFocus(Q) {
    ...
    const A = new k.Vector3();
    Q.getSize(A);
    const t = Math.max(A.x, A.y, 1) / 2, P = k.MathUtils.degToRad(this.camera.fov), e = t / Math.tan(P / 2) * 1.25 + A.z, i = xA("front-top-left"), v = B.clone().add(i.multiplyScalar(e)), s = Math.max(A.x, A.y, A.z, 1);
    this.camera.near = s / 1e3, this.camera.far = s * 50, this.camera.updateProjectionMatrix(), this.cameraFocusTransition && (this.controls.enableDamping = this.cameraFocusTransition.dampingEnabled), this.cameraFocusTransition = {
      startedAt: performance.now(),
  ...
  updateCameraFocusTransition() {
    if (!this.cameraFocusTransition)
      return;
    const B = performance.now() - this.cameraFocusTransition.startedAt, A = Math.min(1, Math.max(0, B / this.cameraFocusTransition.duration)), t = pP(A);
...
class Li {
  constructor(Q, B) {
    this.crosshairPos = null, this.crosshairVisible = !1, this.pathEls = [], this.segmentGroups = [], this.workerMode = !1, this.viewBox = { x: 0, y: 0, w: 100, h: 100 }, this.rotX = eB, this.rotY = iB, this.centerX = 0, this.centerY = 0, this.centerZ = 0, this.focalLength = 500, this.cosRotX = 1, this.sinRotX = 0, this.cosRotY = 1, this.sinRotY = 0, this.dragMode = "none", this.activePointers = /* @__PURE__ */ new Map(), this.pinchLastDist = 0, this.pinchLastMid = { x: 0, y: 0 }, this.rafPending = !1, this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.bounds = { minX: 0, minY: 0, maxX: 0, maxY: 0, minZ: 0, maxZ: 0, empty: !0 }, this.project = (A, t, P) => {
      const e = A - this.centerX, i = t - this.centerY, v = P - this.centerZ, s = e * this.cosRotY + v * this.sinRotY, c = i, n = -e * this.sinRotY + v * this.cosRotY, o = s, z = c * this.cosRotX - n * this.sinRotX, w = c * this.sinRotX + n * this.cosRotX;
      if (this.options.projectionMode === "perspective") {
        const T = this.focalLength / (this.focalLength + w);
        return { x: o * T, y: -z * T };
      }
      return { x: o, y: -z };
    }, this.onWheel = (A) => {
      A.preventDefault();
      const t = A.deltaY > 0 ? 1.1 : 1 / 1.1, P = this.svgPoint(A.clientX, A.clientY), { x: e, y: i, w: v, h: s } = this.viewBox;
      this.viewBox = {
        x: P.x - (P.x - e) * t,
        y: P.y - (P.y - i) * t,
        w: v * t,
      ...
      if (this.activePointers.set(A.pointerId, { x: A.clientX, y: A.clientY }), this.activePointers.size === 2) {
        const [i, v] = Array.from(this.activePointers.values()), s = Math.hypot(v.x - i.x, v.y - i.y), c = { x: (i.x + v.x) / 2, y: (i.y + v.y) / 2 };
        if (this.pinchLastDist > 0) {
          const w = this.pinchLastDist / s, T = this.svgPoint(c.x, c.y), { x: E, y: h, w: a, h: D } = this.viewBox;
          this.viewBox = {
            x: T.x - (T.x - E) * w,
            y: T.y - (T.y - h) * w,
            w: a * w,
        ...
        const n = this.svg.getBoundingClientRect(), o = c.x - this.pinchLastMid.x, z = c.y - this.pinchLastMid.y;
        this.viewBox = {
          ...this.viewBox,
          x: this.viewBox.x - o / n.width * this.viewBox.w,
          y: this.viewBox.y - z / n.height * this.viewBox.h
        }, this.pinchLastDist = s, this.pinchLastMid = c, this.applyViewBox();
        return;
      }
      if (this.dragMode === "none") return;
      const P = A.clientX - t.x, e = A.clientY - t.y;
      if (this.dragMode === "orbit") {
        const i = this.svg.getBoundingClientRect(), v = Math.PI / Math.min(i.width, i.height);
        this.rotY += P * v, this.rotX += e * v, this.rotX = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, this.rotX)), this.updateTrig(), this.fitView(), this.scheduleDraw();
      } else {
      ...
      A.preventDefault();
    }, this.options = { ...Di, ...B }, this.svg = document.createElementNS("http://www.w3.org/2000/svg", "svg"), this.svg.style.cssText = "width:100%;height:100%;display:block;cursor:grab;user-select:none;touch-action:none;", this.svg.setAttribute("preserveAspectRatio", "xMidYMid meet"), this.svg.setAttribute("xmlns", "http://www.w3.org/2000/svg"), this.bboxPath = WB(), this.pathLayer = document.createElementNS("http://www.w3.org/2000/svg", "g"), this.bboxLabelX = qB("middle", "hanging"), this.bboxLabelY = qB("start", "middle"), this.bboxLabelZ = qB("middle", "auto"), this.originMarker = document.createElementNS("http://www.w3.org/2000/svg", "circle"), this.originMarker.setAttribute("stroke-opacity", "0.4"), this.originMarker.setAttribute("visibility", "hidden"), this.crosshairEl = WB(), this.crosshairEl.setAttribute("fill", "none"), this.crosshairEl.setAttribute("visibility", "hidden"), this.svg.appendChild(this.bboxPath), this.svg.appendChild(this.pathLayer), this.svg.appendChild(this.bboxLabelX), this.svg.appendChild(this.bboxLabelY), this.svg.appendChild(this.bboxLabelZ), this.svg.appendChild(this.originMarker), this.svg.appendChild(this.crosshairEl), Q.appendChild(this.svg), this.applyOptions(), this.bindEvents();
  }
  // ── Public API ────────────────────────────────────────────────────────────
  loadFromLines(Q) {
    const { rapid: B, cutting: A } = zP(Q, {
      arcSegments: this.options.arcSegments
    });
    if (this.rapidVerts = B, this.cutVerts = A, this.workerMode = !1, this.syncSegmentGroupsFromLines(), this.bounds = QA(B, A), !this.bounds.empty) {
  ...
  loadFromWorkerData(Q) {
    const B = wP(Q);
    if (this.workerMode = !0, this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.segmentGroups = B.map((A) => ({ color: A.hexColor, opacity: A.opacity, verts: A.positions })), this.bounds = QA(...B.map((A) => A.positions)), !this.bounds.empty) {
      this.centerX = (this.bounds.minX + this.bounds.maxX) / 2, this.centerY = (this.bounds.minY + this.bounds.maxY) / 2, this.centerZ = (this.bounds.minZ + this.bounds.maxZ) / 2;
      const A = Math.hypot(
        this.bounds.maxX - this.bounds.minX,
        this.bounds.maxY - this.bounds.minY,
  ...
  loadFromPrecomputedGroups(Q) {
    ...
    })), this.bounds = QA(...this.segmentGroups.map((B) => B.verts)), !this.bounds.empty) {
      this.centerX = (this.bounds.minX + this.bounds.maxX) / 2, this.centerY = (this.bounds.minY + this.bounds.maxY) / 2, this.centerZ = (this.bounds.minZ + this.bounds.maxZ) / 2;
      const B = Math.hypot(
        this.bounds.maxX - this.bounds.minX,
        this.bounds.maxY - this.bounds.minY,
  ...
  rebuildAndRender(Q = !1) {
    const B = Q ? li : L, A = String(this.options.strokeWidth);
    ...
    for (let t = 0; t < this.segmentGroups.length; t++) {
      const { color: P, opacity: e, verts: i } = this.segmentGroups[t], v = this.pathEls[t];
      v.setAttribute("stroke", P), v.setAttribute("stroke-opacity", String(e)), v.setAttribute("stroke-width", A), v.setAttribute("stroke-linecap", "round"), v.setAttribute("stroke-linejoin", "round"), v.setAttribute("d", ui(i, this.project, B));
    }
  ...
  fitView() {
    ...
    const { minX: Q, maxX: B, minY: A, maxY: t, minZ: P, maxZ: e } = this.bounds, i = this.project, v = i(Q, A, P), s = i(B, A, P), c = i(B, t, P), n = i(Q, t, P), o = i(Q, A, e), z = i(B, A, e), w = i(B, t, e), T = i(Q, t, e), E = Math.hypot(s.x - v.x, s.y - v.y), h = Math.hypot(n.x - v.x, n.y - v.y), a = Math.hypot(o.x - v.x, o.y - v.y), D = Math.max(E, h, a) * 0.07, l = D * 0.5, U = ct(rB(v, s), rB(c, n), l), O = [v, s, c, n, o, z, w, T], u = Math.min(...O.map((R) => R.y)), N = Math.max(...O.map((R) => R.y)), C = Math.min(...O.map((R) => R.x)), M = Math.max(...O.map((R) => R.x)), b = { x: M + l, y: (u + N) / 2 }, j = { x: b.x + D * 5, y: b.y }, d = { x: (C + M) / 2, y: u - D }, G = [v, s, c, n, o, z, w, T, U, b, j, d];
    let f = 1 / 0, H = 1 / 0, F = -1 / 0, X = -1 / 0;
    for (const R of G)
      R.x < f && (f = R.x), R.y < H && (H = R.y), R.x > F && (F = R.x), R.y > X && (X = R.y);
    const nQ = F - f, NQ = X - H, gQ = Math.max(nQ, NQ) * 0.08, TQ = this.options.padding + gQ;
    this.viewBox = {
      x: f - TQ,
      y: H - TQ,
      w: nQ + TQ * 2,
  ...
  renderBbox() {
    ...
    const { minX: Q, maxX: B, minY: A, maxY: t, minZ: P, maxZ: e } = this.bounds, i = this.project, v = i(Q, A, P), s = i(B, A, P), c = i(B, t, P), n = i(Q, t, P), o = i(Q, A, e), z = i(B, A, e), w = i(B, t, e), T = i(Q, t, e), E = [
      `M${L(v.x)} ${L(v.y)}L${L(s.x)} ${L(s.y)}L${L(c.x)} ${L(c.y)}L${L(n.x)} ${L(n.y)}Z`,
      `M${L(o.x)} ${L(o.y)}L${L(z.x)} ${L(z.y)}L${L(w.x)} ${L(w.y)}L${L(T.x)} ${L(T.y)}Z`,
      `M${L(v.x)} ${L(v.y)}L${L(o.x)} ${L(o.y)}`,
      `M${L(s.x)} ${L(s.y)}L${L(z.x)} ${L(z.y)}`,
      `M${L(c.x)} ${L(c.y)}L${L(w.x)} ${L(w.y)}`,
      `M${L(n.x)} ${L(n.y)}L${L(T.x)} ${L(T.y)}`
    ].join("");
    this.bboxPath.setAttribute("d", E);
    const h = Math.hypot(s.x - v.x, s.y - v.y), a = Math.hypot(n.x - v.x, n.y - v.y), D = Math.hypot(o.x - v.x, o.y - v.y), l = Math.max(h, a, D) * 0.07, U = l * 0.5;
    _B(this.bboxLabelX, rB(v, s), ct(rB(v, s), rB(c, n), U), l, `X: ${$B(B - Q)}`);
    const O = [v, s, c, n, o, z, w, T], u = Math.min(...O.map((d) => d.y)), N = Math.max(...O.map((d) => d.y)), C = Math.min(...O.map((d) => d.x)), M = Math.max(...O.map((d) => d.x)), b = { x: M + U, y: (u + N) / 2 };
    _B(this.bboxLabelY, b, b, l, `Y: ${$B(t - A)}`);
    const j = { x: (C + M) / 2, y: u - l };
    _B(this.bboxLabelZ, j, j, l, `Z: ${$B(e - P)}`);
  }
  renderOriginMarker() {
  ...
  renderCrosshairMarker() {
    ...
    const { x: Q, y: B, z: A } = this.crosshairPos, t = this.project(Q, B, A), P = Math.min(this.viewBox.w, this.viewBox.h) * 0.025, e = P * 0.25, i = t.x, v = t.y, s = [
      `M${L(i - P)} ${L(v)}L${L(i - e)} ${L(v)}`,
      `M${L(i + e)} ${L(v)}L${L(i + P)} ${L(v)}`,
      `M${L(i)} ${L(v - P)}L${L(i)} ${L(v - e)}`,
      `M${L(i)} ${L(v + e)}L${L(i)} ${L(v + P)}`
    ].join("");
    this.crosshairEl.setAttribute("d", s), this.crosshairEl.setAttribute("stroke-width", L(this.options.strokeWidth * 2)), this.crosshairEl.setAttribute("stroke-linecap", "round"), this.crosshairEl.setAttribute("visibility", "visible");
  }
  bindEvents() {
...
function ui(r, Q, B) {
  ...
  for (let i = 0; i + 5 < r.length; i += 6) {
    const v = Q(r[i], r[i + 1], r[i + 2]), s = Q(r[i + 3], r[i + 4], r[i + 5]);
    Math.abs(v.x - P) < t && Math.abs(v.y - e) < t ? A.push(`L${B(s.x)} ${B(s.y)}`) : A.push(`M${B(v.x)} ${B(v.y)}L${B(s.x)} ${B(s.y)}`), P = s.x, e = s.y;
  }
  return A.join("");
}
function QA(...r) {
  ...
  for (const v of r)
    for (let s = 0; s + 5 < v.length; s += 6) {
      const c = v[s], n = v[s + 1], o = v[s + 2], z = v[s + 3], w = v[s + 4], T = v[s + 5];
      c < Q && (Q = c), n < B && (B = n), o < A && (A = o), c > t && (t = c), n > P && (P = n), o > e && (e = o), z < Q && (Q = z), w < B && (B = w), T < A && (A = T), z > t && (t = z), w > P && (P = w), T > e && (e = T), i = !1;
    }
