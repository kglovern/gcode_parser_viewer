import * as h from "three";
import { OrbitControls as ue } from "three/examples/jsm/controls/OrbitControls.js";
import { GCodeVirtualizer as et, buildWorkerToolpathStreams as Ue, buildToolpathGeometryFromLinesBatched as De, buildMovementVerticesFromLines as de, buildWorkerSegmentGroups as Ce } from "./gviewer.js";
import { STLLoader as Ne } from "three/examples/jsm/loaders/STLLoader.js";
function Oe(r) {
  const Q = [];
  let B = null;
  for (const t of r.specs) {
    if (t.positions.length === 0 || t.opacity <= 0)
      continue;
    const A = t.positions.length / 3, e = t.colors ? t.colors.slice() : lA(t.kind, A, r.options), P = new Float32Array(e.length);
    P.set(e);
    const i = new h.BufferGeometry();
    i.setAttribute("position", new h.BufferAttribute(t.positions, 3));
    const s = new h.BufferAttribute(P, 3);
    s.setUsage(h.DynamicDrawUsage), i.setAttribute("color", s), i.computeBoundingBox();
    const v = new h.LineBasicMaterial({
      vertexColors: !0,
      transparent: !0,
      opacity: t.opacity
    }), c = new h.LineSegments(i, v);
    r.scene.add(c), Q.push({
      line: c,
      baseColors: e,
      simColors: P,
      prefixEndVertex: t.prefixEndVertex,
      totalVertices: A,
      greyCursorVertex: 0,
      kind: t.kind,
      cutBucketIndex: t.cutBucketIndex,
      workerColors: t.colors ? e : void 0
    }), i.boundingBox && (B = B ? B.union(i.boundingBox) : i.boundingBox.clone());
  }
  return { streams: Q, bounds: B };
}
function Le(r, Q) {
  for (const B of Q)
    r.remove(B.line), B.line.geometry.dispose(), B.line.material.dispose();
}
function Me(r, Q) {
  for (const B of r) {
    if (B.workerColors) continue;
    const t = lA(B.kind, B.totalVertices, Q);
    B.baseColors = t, B.simColors.set(t), B.greyCursorVertex = 0;
    const A = B.line.geometry.getAttribute("color");
    A.clearUpdateRanges(), A.addUpdateRange(0, B.simColors.length), A.needsUpdate = !0;
  }
}
function He(r) {
  const Q = Pt(r.options.render.theme.rapidOpacity ?? 0.3);
  for (const B of r.streams) {
    const t = B.line.material;
    if (B.kind === "rapid") {
      t.opacity = Q;
      continue;
    }
    t.opacity = fe({
      bucketIndex: B.cutBucketIndex ?? 0,
      bucketCount: r.cutBucketCount,
      baseOpacity: r.options.render.theme.opacity
    });
  }
}
function je(r) {
  const Q = r.stream.totalVertices, B = Math.max(0, Math.min(Q, Math.floor(r.nextCursorVertex))), t = Math.max(0, Math.min(Q, Math.floor(r.stream.greyCursorVertex)));
  if (B === t)
    return;
  const A = r.stream.line.geometry.getAttribute("color"), e = Fe(r.options), P = Math.min(t, B), i = Math.max(t, B);
  if (B > t)
    for (let s = t; s < B; s += 1) {
      const v = s * 3;
      r.stream.simColors[v] = e.r, r.stream.simColors[v + 1] = e.g, r.stream.simColors[v + 2] = e.b;
    }
  else {
    const s = B * 3, v = t * 3;
    r.stream.simColors.set(r.stream.baseColors.subarray(s, v), s);
  }
  A.clearUpdateRanges(), A.addUpdateRange(P * 3, (i - P) * 3), A.needsUpdate = !0, r.stream.greyCursorVertex = B;
}
function lA(r, Q, B) {
  const t = B.render.theme.colors, A = B.mode.laser ? t.laser ?? t.cutting : t.cutting, e = new h.Color(r === "rapid" ? t.rapid : A), P = new Float32Array(Q * 3);
  for (let i = 0; i < Q; i += 1) {
    const s = i * 3;
    P[s] = e.r, P[s + 1] = e.g, P[s + 2] = e.b;
  }
  return P;
}
function Fe(r) {
  const Q = r.render.theme.colors, B = new h.Color(r.render.theme.background), t = new h.Color(Q.processed ?? Q.cutting), A = B.clone().lerp(t, 0.65);
  return { r: A.r, g: A.g, b: A.b };
}
function fe(r) {
  if (r.bucketCount <= 1)
    return Pt(r.baseOpacity);
  const Q = r.bucketIndex / (r.bucketCount - 1);
  return Pt(r.baseOpacity * Q);
}
function Pt(r) {
  return Number.isFinite(r) ? Math.max(0, Math.min(1, r)) : 0;
}
const Vt = {
  background: "#111827",
  opacity: 0.9,
  rapidOpacity: 0.3,
  colors: {
    rapid: "#0ef6ae",
    cutting: "#3e85c7",
    laser: "#a855f7",
    processed: "#6b7280",
    boundingBox: "#e2e8f0",
    machineBed: "#fbbf24",
    machineBedKeepout: "#df3b3b",
    grid: { major: "#2f3840", minor: "#1f252b" },
    axes: { x: "#df3b3b", y: "#06b881", z: "#295d8d" }
  }
}, Ge = {
  units: "mm",
  mode: { laser: !1, sim3d: !1 },
  sim3d: { toolDiameter: 6.35, resolution: 256, showToolpath: !1, erosionPasses: 2 },
  bit: {
    enabled: !0,
    type: "drill",
    size: 4.05,
    opacity: 0.9,
    tweenMs: 140,
    colorSource: "cutting",
    color: Vt.colors.cutting,
    spinRpm: 300
  },
  progress: { mode: "grey" },
  grid: { sizeX: 1e3, sizeY: 1e3, axisDepth: 200, labels: !0, bounds: null },
  boundingBox: { visible: !1, labels: !1 },
  machineBed: { visible: !1, min: null, max: null, keepout: null },
  geometry: { arcSegments: 30, batching: { progressEveryLines: 5e3, yieldEveryLines: 5e4 } },
  render: { antialias: !0, theme: Vt },
  camera: {
    fov: 45,
    focusDurationMs: 900,
    orbit: { enableDamping: !0 },
    initialPosition: { x: 0, y: -200, z: 200 }
  }
};
async function be(r, Q, B, t) {
  var M, b, G;
  const A = Math.max(2, Math.floor(B)), e = (t == null ? void 0 : t.arcSegments) ?? 30, P = (t == null ? void 0 : t.checkpointEveryLines) ?? 2e3, i = ((M = t == null ? void 0 : t.batch) == null ? void 0 : M.yieldEveryLines) ?? 1e4, s = ((b = t == null ? void 0 : t.batch) == null ? void 0 : b.shouldAbort) ?? (() => !1), v = (G = t == null ? void 0 : t.batch) == null ? void 0 : G.onProgress;
  if (r.length === 0) {
    const C = {
      xMin: -1,
      xMax: 1,
      yMin: -1,
      yMax: 1,
      zTop: 1,
      zBot: 0
    }, j = new Float32Array(A * A).fill(C.zTop), F = /* @__PURE__ */ new Map();
    return F.set(0, new Float32Array(j)), { moves: [], checkpoints: F, slabBounds: C, resolution: A, toolRadius: Q };
  }
  let c = 1 / 0, n = -1 / 0, T = 1 / 0, z = -1 / 0, w = 1 / 0, o = -1 / 0;
  const a = new et({
    onLinearMove(C) {
      if (C.modals.motion === "G0") return;
      const j = C.transformedStart ?? C.start, F = C.transformedEnd ?? C.end;
      for (const H of [j, F])
        H.X < c && (c = H.X), H.X > n && (n = H.X), H.Y < T && (T = H.Y), H.Y > z && (z = H.Y), H.Z < w && (w = H.Z), H.Z > o && (o = H.Z);
    },
    onArcMove(C) {
      const j = C.transformedStart ?? C.start, F = C.transformedEnd ?? C.end, H = C.transformedMax ?? C.max;
      for (const f of [j, F, H])
        f.X < c && (c = f.X), f.X > n && (n = f.X), f.Y < T && (T = f.Y), f.Y > z && (z = f.Y), f.Z < w && (w = f.Z), f.Z > o && (o = f.Z);
    }
  });
  for (const C of r)
    a.processLine(C);
  isFinite(c) || (c = -1, n = 1, T = -1, z = 1, w = 0, o = 0), c === n && (c -= Q * 2, n += Q * 2), T === z && (T -= Q * 2, z += Q * 2), c -= Q, n += Q, T -= Q, z += Q;
  const E = o;
  let l = w - Q;
  l >= E && (l = E - 1);
  const u = { xMin: c, xMax: n, yMin: T, yMax: z, zTop: E, zBot: l }, U = new Float32Array(A * A).fill(E), k = /* @__PURE__ */ new Map();
  k.set(0, new Float32Array(U));
  const O = [];
  let D = 0, N = 0;
  const d = new et({
    onLinearMove(C) {
      const j = C.transformedStart ?? C.start, F = C.transformedEnd ?? C.end, H = {
        lineIndex: N,
        x0: j.X,
        y0: j.Y,
        z0: j.Z,
        x1: F.X,
        y1: F.Y,
        z1: F.Z
      };
      O.push(H), it(H, U, u, Q, A);
    },
    onArcMove(C) {
      const j = C.transformedStart ?? C.start, F = C.transformedEnd ?? C.end, H = C.transformedCenter ?? C.center, f = ge(j, F, H, C.plane, C.motion, e);
      for (const I of f) {
        const R = { lineIndex: N, ...I };
        O.push(R), it(R, U, u, Q, A);
      }
    }
  });
  for (let C = 0; C < r.length && !s(); C++) {
    N = C;
    const j = r[C];
    j !== void 0 && d.processLine(j), C - D >= P && (k.set(C, new Float32Array(U)), D = C), C % i === 0 && C > 0 && (v == null || v(C, r.length), await new Promise((F) => setTimeout(F, 0)));
  }
  return v == null || v(r.length, r.length), { moves: O, checkpoints: k, slabBounds: u, resolution: A, toolRadius: Q };
}
function it(r, Q, B, t, A) {
  const e = (B.xMax - B.xMin) / A, P = (B.yMax - B.yMin) / A, i = Math.hypot(r.x1 - r.x0, r.y1 - r.y0), s = Math.max(1e-3, Math.min(e, P) * 0.5), v = Math.max(1, Math.ceil(i / s)), c = Math.ceil(t / e), n = Math.ceil(t / P), T = t * t;
  for (let z = 0; z <= v; z++) {
    const w = z / v, o = r.x0 + (r.x1 - r.x0) * w, a = r.y0 + (r.y1 - r.y0) * w, E = r.z0 + (r.z1 - r.z0) * w, l = Math.floor((o - B.xMin) / e), u = Math.floor((a - B.yMin) / P);
    for (let U = -c; U <= c; U++) {
      const k = l + U;
      if (k < 0 || k >= A) continue;
      const O = B.xMin + (k + 0.5) * e, D = (O - o) * (O - o);
      if (!(D > T))
        for (let N = -n; N <= n; N++) {
          const d = u + N;
          if (d < 0 || d >= A) continue;
          const M = B.yMin + (d + 0.5) * P;
          if (D + (M - a) * (M - a) > T) continue;
          const G = d * A + k;
          Q[G] > E && (Q[G] = E);
        }
    }
  }
}
function SB(r, Q) {
  let B = 0;
  for (const e of Q.checkpoints.keys())
    e <= r && e > B && (B = e);
  const t = Q.checkpoints.get(B), A = t ? new Float32Array(t) : new Float32Array(Q.resolution * Q.resolution).fill(Q.slabBounds.zTop);
  for (const e of Q.moves)
    e.lineIndex > B && e.lineIndex <= r && it(e, A, Q.slabBounds, Q.toolRadius, Q.resolution);
  return A;
}
function YB(r, Q, B, t) {
  if (B <= 0) return;
  const A = 1e-3, e = Q * Q, P = new Float32Array(e);
  for (let i = 0; i < B; i++) {
    P.set(r);
    for (let s = 0; s < Q; s++)
      for (let v = 0; v < Q; v++) {
        if (P[s * Q + v] < t - A) continue;
        let n = t;
        for (let T = -1; T <= 1; T++) {
          const z = s + T;
          if (!(z < 0 || z >= Q))
            for (let w = -1; w <= 1; w++) {
              if (w === 0 && T === 0) continue;
              const o = v + w;
              if (o < 0 || o >= Q) continue;
              const a = P[z * Q + o];
              a < n && (n = a);
            }
        }
        n < t - A && (r[s * Q + v] = n);
      }
  }
}
function ge(r, Q, B, t, A, e) {
  const P = Math.max(1, e);
  let i, s, v;
  t === "G18" ? (i = "Z", s = "X", v = "Y") : t === "G19" ? (i = "Y", s = "Z", v = "X") : (i = "X", s = "Y", v = "Z");
  const c = Math.hypot(r[i] - B[i], r[s] - B[s]), n = Math.atan2(r[s] - B[s], r[i] - B[i]), T = Math.atan2(Q[s] - B[s], Q[i] - B[i]);
  let z;
  A === "G2" ? (z = T - n, z > 0 && (z -= Math.PI * 2)) : (z = T - n, z < 0 && (z += Math.PI * 2));
  const w = [];
  let o = { ...r };
  for (let a = 1; a <= P; a++) {
    const E = a / P, l = n + z * E, u = E, U = B[i] + c * Math.cos(l), k = B[s] + c * Math.sin(l), O = r[v] + (Q[v] - r[v]) * u, D = { X: 0, Y: 0, Z: 0 };
    D[i] = U, D[s] = k, D[v] = O, w.push({
      x0: o.X,
      y0: o.Y,
      z0: o.Z,
      x1: D.X,
      y1: D.Y,
      z1: D.Z
    }), o = D;
  }
  return w;
}
let NB = null;
function pe() {
  if (NB) return NB;
  const r = 512, Q = document.createElement("canvas");
  Q.width = r, Q.height = r;
  const B = Q.getContext("2d");
  B.fillStyle = "#8B5E3C", B.fillRect(0, 0, r, r);
  const t = Math.floor(40 + Math.random() * 20);
  for (let A = 0; A < t; A++) {
    const e = Math.random() * r, P = 1 + Math.random() * 2, i = Math.floor(139 + (Math.random() - 0.5) * 40), s = Math.floor(94 + (Math.random() - 0.5) * 30), v = Math.floor(60 + (Math.random() - 0.5) * 20);
    B.strokeStyle = `rgb(${i},${s},${v})`, B.lineWidth = P, B.beginPath(), B.moveTo(0, e), B.lineTo(r, e), B.stroke();
  }
  for (let A = 0; A < 300; A++) {
    const e = Math.random() * r, P = Math.random() * r, i = 20 + Math.random() * 60, s = (Math.random() - 0.5) * 0.3, v = Math.floor(100 + Math.random() * 80), c = Math.floor(60 + Math.random() * 50), n = Math.floor(30 + Math.random() * 40);
    B.strokeStyle = `rgba(${v},${c},${n},0.35)`, B.lineWidth = 0.5 + Math.random() * 1, B.beginPath(), B.moveTo(e, P), B.quadraticCurveTo(
      e + i * 0.5 * Math.cos(s),
      P + i * 0.5 * Math.sin(s) * 0.1,
      e + i * Math.cos(s * 0.5),
      P + i * 0.15
    ), B.stroke();
  }
  return NB = new h.CanvasTexture(Q), NB;
}
function me(r, Q) {
  const B = new h.Group();
  B.name = "gviewer:sim3d-slab";
  const t = r.xMax - r.xMin, A = r.yMax - r.yMin, e = r.zTop - r.zBot, P = (r.xMin + r.xMax) / 2, i = (r.yMin + r.yMax) / 2, s = (r.zBot + r.zTop) / 2, v = pe();
  v.wrapS = h.RepeatWrapping, v.wrapT = h.RepeatWrapping, v.repeat.set(t / 100, A / 100);
  const c = new h.MeshStandardMaterial({
    map: v,
    roughness: 0.85,
    metalness: 0,
    side: h.FrontSide
  }), n = new h.MeshStandardMaterial({
    color: 8016432,
    roughness: 0.9,
    metalness: 0,
    side: h.DoubleSide
  }), T = new h.MeshStandardMaterial({
    color: 7030822,
    roughness: 0.9,
    metalness: 0
  }), z = new h.PlaneGeometry(t, A, Q - 1, Q - 1), w = Q * Q, o = new Float32Array(w * 3), a = z.attributes.position;
  for (let G = 0; G < w; G++)
    o[G * 3] = a.getX(G), o[G * 3 + 1] = a.getY(G), o[G * 3 + 2] = 0;
  const E = new h.BufferAttribute(o, 3);
  E.usage = h.DynamicDrawUsage, z.setAttribute("position", E), z.computeVertexNormals();
  const l = new h.Mesh(z, c);
  l.name = "gviewer:sim3d-top", l.position.set(P, i, r.zTop), B.add(l);
  const u = new h.PlaneGeometry(t, e);
  u.rotateX(Math.PI / 2);
  const U = new h.Mesh(u, n);
  U.position.set(P, r.yMax, s), B.add(U);
  const k = new h.PlaneGeometry(t, e);
  k.rotateX(Math.PI / 2);
  const O = new h.Mesh(k, n);
  O.position.set(P, r.yMin, s), B.add(O);
  const D = new h.PlaneGeometry(e, A);
  D.rotateY(-Math.PI / 2);
  const N = new h.Mesh(D, n);
  N.position.set(r.xMax, i, s), B.add(N);
  const d = new h.PlaneGeometry(e, A);
  d.rotateY(-Math.PI / 2);
  const M = new h.Mesh(d, n);
  M.position.set(r.xMin, i, s), B.add(M);
  const b = new h.Mesh(new h.PlaneGeometry(t, A), T);
  return b.position.set(P, i, r.zBot), B.add(b), { group: B, topMesh: l, topPositionAttr: E, slabBounds: r, resolution: Q };
}
function VB(r, Q) {
  const { topPositionAttr: B, slabBounds: t, resolution: A } = r, e = A * A;
  for (let P = 0; P < e; P++) {
    const i = Math.floor(P / A), s = P % A, v = (A - 1 - i) * A + s;
    B.setZ(P, Q[v] - t.zTop);
  }
  B.needsUpdate = !0, r.topMesh.geometry.computeVertexNormals();
}
function xe(r) {
  r.group.traverse((Q) => {
    if (Q instanceof h.Mesh) {
      Q.geometry.dispose();
      const B = Q.material;
      Array.isArray(B) ? B.forEach((t) => {
        t.map && t.map.dispose(), t.dispose();
      }) : (B.map && B.map.dispose(), B.dispose());
    }
  });
}
const ye = ".gViewer-viewcube{position:absolute;left:60px;bottom:60px;width:84px;height:84px;z-index:9999;perspective:600px;pointer-events:auto}.gViewer-viewcube__cube{position:absolute;top:0;right:0;bottom:0;left:0;transform-style:preserve-3d;transition:transform 60ms linear}.gViewer-viewcube__face{position:absolute;width:84px;height:84px;display:flex;align-items:center;justify-content:center;font:600 11px system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;letter-spacing:.06em;text-transform:uppercase;color:#f0f5fce0;background:#0c1014b3;border:1px solid rgba(120,140,160,.35);border-radius:14px;cursor:pointer;-webkit-user-select:none;user-select:none;padding:0}.gViewer-viewcube__face:hover{background:#0c1014db;border-color:#a0bedc8c}.gViewer-viewcube__face.is-active{border-color:#0ef6aef2;box-shadow:0 0 0 1px #0ef6ae59,0 10px 30px #00000059;color:#0ef6aef2}.gViewer-viewcube__face--front{transform:translateZ(42px)}.gViewer-viewcube__face--back{transform:rotateY(180deg) translateZ(42px)}.gViewer-viewcube__face--left{transform:rotateY(-90deg) translateZ(42px)}.gViewer-viewcube__face--right{transform:rotateY(90deg) translateZ(42px)}.gViewer-viewcube__face--top{transform:rotateX(-90deg) translateZ(42px)}.gViewer-viewcube__face--bottom{transform:rotateX(90deg) translateZ(42px)}", Jt = "gviewer-viewcube-style";
function Ie() {
  if (typeof document > "u" || document.getElementById(Jt)) return;
  const r = document.createElement("style");
  r.id = Jt, r.setAttribute("data-gviewer", "viewcube"), r.textContent = ye, document.head.appendChild(r);
}
class Se {
  constructor(Q) {
    Ie(), this.onSelectView = Q.onSelectView, this.faceButtons = /* @__PURE__ */ new Map(), this.root = document.createElement("div"), this.root.className = "gViewer-viewcube", this.cube = document.createElement("div"), this.cube.className = "gViewer-viewcube__cube", this.root.appendChild(this.cube);
    const B = [
      { view: "front", className: "gViewer-viewcube__face--front", label: "Front" },
      { view: "back", className: "gViewer-viewcube__face--back", label: "Back" },
      { view: "left", className: "gViewer-viewcube__face--left", label: "Left" },
      { view: "right", className: "gViewer-viewcube__face--right", label: "Right" },
      { view: "top", className: "gViewer-viewcube__face--top", label: "Top" },
      { view: "bottom", className: "gViewer-viewcube__face--bottom", label: "Bottom" }
    ];
    for (const t of B) {
      const A = document.createElement("button");
      A.type = "button", A.className = `gViewer-viewcube__face ${t.className}`, A.textContent = t.label, A.addEventListener("click", () => this.onSelectView(t.view)), this.cube.appendChild(A), this.faceButtons.set(t.view, A);
    }
    Q.container.appendChild(this.root);
  }
  setActiveFace(Q) {
    for (const [B, t] of this.faceButtons.entries())
      B === Q ? t.classList.add("is-active") : t.classList.remove("is-active");
  }
  setRotationMatrix3d(Q) {
    const B = Array.from({ length: 16 }, (t, A) => {
      const e = Q[A] ?? 0;
      return Number.isFinite(e) ? String(Math.abs(e) < 1e-12 ? 0 : Number(e.toFixed(8))) : "0";
    });
    this.cube.style.transform = `matrix3d(${B.join(",")})`;
  }
  dispose() {
    this.root.remove();
  }
}
function MQ(r, Q, B = {}) {
  const { opacity: t = 1, size: A = 10 } = B, e = new h.Object3D(), P = 100, i = document.createElement("canvas"), s = i.getContext("2d");
  if (!s)
    throw new Error("Missing canvas 2D context.");
  s.font = `normal ${P}px Arial`;
  const c = s.measureText(r).width;
  i.width = c, i.height = P, s.font = `normal ${P}px Arial`, s.textAlign = "center", s.textBaseline = "middle", s.fillStyle = Q, s.fillText(r, c / 2, P / 2);
  const n = new h.Texture(i);
  n.needsUpdate = !0, n.minFilter = h.LinearFilter;
  const T = new h.SpriteMaterial({
    map: n,
    transparent: !0,
    opacity: t
  });
  e.textHeight = A, e.textWidth = c / P * e.textHeight, B.textAlign === "left" ? e.position.x = (B.x ?? 0) + (e.textWidth ?? 0) / 2 : B.textAlign === "right" ? e.position.x = (B.x ?? 0) - (e.textWidth ?? 0) / 2 : e.position.x = B.x ?? 0, B.textBaseline === "top" ? e.position.y = (B.y ?? 0) - (e.textHeight ?? 0) / 2 : B.textBaseline === "bottom" ? e.position.y = (B.y ?? 0) + (e.textHeight ?? 0) / 2 : e.position.y = B.y ?? 0, e.position.z = B.z ?? 0;
  const z = new h.Sprite(T);
  return z.scale.set(c / P * A, A, 1), e.add(z), e;
}
function Ye(r) {
  r.traverse((Q) => {
    var t;
    if (!(Q instanceof h.Sprite))
      return;
    const B = Q.material;
    (t = B.map) == null || t.dispose(), B.dispose();
  });
}
const Ve = 25.4;
function Je(r, Q) {
  const B = new h.Group(), t = new h.Box3Helper(
    r.clone(),
    new h.Color(Q.render.theme.colors.boundingBox)
  ), A = t.material;
  return A.transparent = !0, A.opacity = 0.65, A.depthWrite = !1, A.linewidth = 2, B.add(t), Q.boundingBox.labels && B.add(...Re(r.clone(), Q)), B;
}
function Re(r, Q) {
  const B = Q.render.theme.colors.boundingBox, t = 0.8, A = (u) => Math.abs(u) < 1e-9 ? "0" : u.toFixed(3), e = (u) => Q.units === "in" ? `${A(u / Ve)} in` : `${A(u)} mm`, P = r.min, i = r.max, s = (u) => MQ(u, B, { size: 4, opacity: t }), v = s(e(P.x)), c = s(e(i.x)), n = s(e(P.y)), T = s(e(i.y)), z = s(e(P.z)), w = s(e(i.z)), o = new h.Vector3();
  r.getSize(o);
  const a = Math.max(2, Math.max(o.x, o.y, o.z) * 0.02), E = (P.x + i.x) / 2, l = (P.y + i.y) / 2;
  return v.position.set(P.x - a, l, P.z - a), c.position.set(i.x + a, l, P.z - a), n.position.set(E, P.y - a, P.z - a), T.position.set(E, i.y + a, P.z - a), z.position.set(E, l, P.z - a), w.position.set(E, l, i.z + a), [v, c, n, T, z, w];
}
function Xe(r) {
  r.traverse((Q) => {
    var B;
    if (Q instanceof h.LineSegments || Q instanceof h.Line) {
      Q.geometry.dispose();
      const t = Q.material;
      Array.isArray(t) ? t.forEach((A) => A.dispose()) : t.dispose();
      return;
    }
    if (Q instanceof h.Sprite) {
      const t = Q.material;
      (B = t.map) == null || B.dispose(), t.dispose();
    }
  });
}
function Rt(r, Q, B) {
  const t = new Float32Array([
    r.x,
    r.y,
    0,
    Q.x,
    r.y,
    0,
    Q.x,
    Q.y,
    0,
    r.x,
    Q.y,
    0,
    r.x,
    r.y,
    0
  ]), A = new h.BufferGeometry();
  A.setAttribute("position", new h.BufferAttribute(t, 3));
  const e = new h.LineBasicMaterial({
    color: new h.Color(B),
    transparent: !0,
    opacity: 0.35,
    depthWrite: !1
  });
  return new h.LineLoop(A, e);
}
function Ke(r, Q, B, t = 10) {
  const A = [], e = r.x - Q.y, P = Q.x - r.y;
  for (let v = e; v <= P; v += t) {
    const c = Math.max(r.x, r.y + v), n = Math.min(Q.x, Q.y + v);
    n > c && A.push(c, c - v, 0, n, n - v, 0);
  }
  const i = new h.BufferGeometry();
  i.setAttribute("position", new h.BufferAttribute(new Float32Array(A), 3));
  const s = new h.LineBasicMaterial({
    color: new h.Color(B),
    transparent: !0,
    opacity: 0.35,
    depthWrite: !1
  });
  return new h.LineSegments(i, s);
}
function Ze(r, Q, B, t) {
  const A = new h.Group();
  if (A.add(Rt(r, Q, B.render.theme.colors.machineBed)), t) {
    const e = B.render.theme.colors.machineBedKeepout;
    A.add(Rt(t.min, t.max, e)), A.add(Ke(t.min, t.max, e));
  }
  return A;
}
function We(r) {
  r.traverse((Q) => {
    if (Q instanceof h.LineSegments || Q instanceof h.Line) {
      Q.geometry.dispose();
      const B = Q.material;
      Array.isArray(B) ? B.forEach((t) => t.dispose()) : B.dispose();
    }
  });
}
function qe(r) {
  const Q = window.getComputedStyle(r);
  (Q.position === "static" || !Q.position) && (r.style.position = "relative"), r.style.overflow || (r.style.overflow = "hidden");
}
function _e(r) {
  if (r < 0.5)
    return 4 * r * r * r;
  const Q = -2 * r + 2;
  return 1 - Q * Q * Q / 2;
}
function Xt(r) {
  const Q = new h.Vector3(), B = {
    right: new h.Vector3(1, 0, 0),
    left: new h.Vector3(-1, 0, 0),
    front: new h.Vector3(0, -1, 0),
    back: new h.Vector3(0, 1, 0),
    top: new h.Vector3(0, 0, 1),
    bottom: new h.Vector3(0, 0, -1)
  };
  switch (r) {
    case "front":
      Q.copy(B.front);
      break;
    case "back":
      Q.copy(B.back);
      break;
    case "left":
      Q.copy(B.left);
      break;
    case "right":
      Q.copy(B.right);
      break;
    case "top":
      Q.copy(B.top);
      break;
    case "bottom":
      Q.copy(B.bottom);
      break;
    case "front-top-left":
      Q.copy(B.front).add(B.top).add(B.left);
      break;
    case "front-top-right":
      Q.copy(B.front).add(B.top).add(B.right);
      break;
    case "front-bottom-left":
      Q.copy(B.front).add(B.bottom).add(B.left);
      break;
    case "front-bottom-right":
      Q.copy(B.front).add(B.bottom).add(B.right);
      break;
    case "back-top-left":
      Q.copy(B.back).add(B.top).add(B.left);
      break;
    case "back-top-right":
      Q.copy(B.back).add(B.top).add(B.right);
      break;
    case "back-bottom-left":
      Q.copy(B.back).add(B.bottom).add(B.left);
      break;
    case "back-bottom-right":
      Q.copy(B.back).add(B.bottom).add(B.right);
      break;
    default:
      Q.copy(B.front);
      break;
  }
  return Q.normalize();
}
function $e(r, Q) {
  const B = r.x - Q.x, t = r.y - Q.y, A = r.z - Q.z, e = Math.abs(B), P = Math.abs(t), i = Math.abs(A);
  return i >= e && i >= P ? A >= 0 ? "top" : "bottom" : e >= P ? B >= 0 ? "right" : "left" : t >= 0 ? "back" : "front";
}
const kA = 25.4;
function ut(r) {
  if (r.bounds)
    return {
      minX: r.bounds.min.x,
      maxX: r.bounds.max.x,
      minY: r.bounds.min.y,
      maxY: r.bounds.max.y
    };
  const Q = Math.max(1, r.sizeXMm) / 2, B = Math.max(1, r.sizeYMm) / 2;
  return { minX: -Q, maxX: Q, minY: -B, maxY: B };
}
function QP(r) {
  const { minX: Q, maxX: B, minY: t, maxY: A } = ut(r), e = r.units === "mm" ? 10 : kA, P = [], i = [], s = (w, o, a, E, l) => {
    l.push(w, o, 0, a, E, 0);
  }, v = Math.ceil(t / e) * e, c = Math.floor(A / e) * e;
  for (let w = v; w <= c + 1e-6; w += e)
    s(Q, w, B, w, Math.abs(w) < 1e-6 ? P : i);
  const n = Math.ceil(Q / e) * e, T = Math.floor(B / e) * e;
  for (let w = n; w <= T + 1e-6; w += e)
    s(w, t, w, A, Math.abs(w) < 1e-6 ? P : i);
  const z = new h.Group();
  return i.length > 0 && z.add(Kt(i, r.theme.colors.grid.minor, 0.25)), P.length > 0 && z.add(Kt(P, r.theme.colors.grid.major, 0.4)), z;
}
function Kt(r, Q, B) {
  const t = new h.BufferGeometry();
  t.setAttribute("position", new h.Float32BufferAttribute(r, 3));
  const A = new h.LineBasicMaterial({
    color: new h.Color(Q),
    transparent: !0,
    opacity: B,
    depthWrite: !1
  });
  return new h.LineSegments(t, A);
}
function BP(r) {
  r.traverse((Q) => {
    if (!(Q instanceof h.LineSegments))
      return;
    Q.geometry.dispose();
    const B = Q.material;
    Array.isArray(B) ? B.forEach((t) => t.dispose()) : B.dispose();
  });
}
function tP(r) {
  const Q = new h.Group(), { minX: B, maxX: t, minY: A, maxY: e } = ut({
    sizeXMm: r.sizeXWorld,
    sizeYMm: r.sizeYWorld,
    bounds: r.bounds
  }), P = Math.max(1, r.depthWorld), i = Math.max(1, Math.min(Math.max(t - B, e - A) / 24, 30)), s = i * 0.6, v = (E) => new h.LineDashedMaterial({
    color: new h.Color(E),
    dashSize: i,
    gapSize: s,
    transparent: !0,
    opacity: 0.75,
    depthWrite: !1
  }), c = new h.Line(
    new h.BufferGeometry().setFromPoints([
      new h.Vector3(B, 0, 0),
      new h.Vector3(t, 0, 0)
    ]),
    v(r.theme.colors.axes.x)
  );
  c.computeLineDistances();
  const n = new h.Line(
    new h.BufferGeometry().setFromPoints([
      new h.Vector3(0, A, 0),
      new h.Vector3(0, e, 0)
    ]),
    v(r.theme.colors.axes.y)
  );
  n.computeLineDistances();
  const T = new h.Line(
    new h.BufferGeometry().setFromPoints([
      new h.Vector3(0, 0, 0),
      new h.Vector3(0, 0, P)
    ]),
    v(r.theme.colors.axes.z)
  );
  T.computeLineDistances(), Q.add(c, n, T);
  const z = Math.max(2, i), w = MQ("X", r.theme.colors.axes.x, { size: 5 });
  w.position.set(t + z, 0, 0);
  const o = MQ("Y", r.theme.colors.axes.y, { size: 5 });
  o.position.set(0, e + z, 0);
  const a = MQ("Z", r.theme.colors.axes.z, { size: 5 });
  return a.position.set(0, 0, P + z), Q.add(w, o, a), Q;
}
function AP(r) {
  r.traverse((Q) => {
    var B;
    if (Q instanceof h.Line) {
      Q.geometry.dispose();
      const t = Q.material;
      Array.isArray(t) ? t.forEach((A) => A.dispose()) : t.dispose();
      return;
    }
    if (Q instanceof h.Sprite) {
      const t = Q.material;
      (B = t.map) == null || B.dispose(), t.dispose();
    }
  });
}
function eP(r) {
  const Q = new h.Group(), { minX: B, maxX: t, minY: A, maxY: e } = ut(r), P = r.units === "mm" ? 10 : 1, i = P * 2, s = r.units === "mm" ? 1 : kA, v = 0.5, c = 4, n = 0.1, T = 5, z = r.theme.colors.axes.x, w = r.theme.colors.axes.y, o = { opacity: v, size: c };
  for (let a = P; ; a += i) {
    const E = a * s;
    if (E > t + 1e-6)
      break;
    const l = MQ(String(a), z, o);
    l.position.set(E, T, n), Q.add(l);
  }
  for (let a = P; ; a += i) {
    const E = a * s;
    if (-E < B - 1e-6)
      break;
    const l = MQ(String(-a), z, o);
    l.position.set(-E, T, n), Q.add(l);
  }
  for (let a = P; ; a += i) {
    const E = a * s;
    if (E > e + 1e-6)
      break;
    const l = MQ(String(a), w, o);
    l.position.set(-T, E, n), Q.add(l);
  }
  for (let a = P; ; a += i) {
    const E = a * s;
    if (-E < A - 1e-6)
      break;
    const l = MQ(String(-a), w, o);
    l.position.set(-T, -E, n), Q.add(l);
  }
  return Q;
}
function dQ(r) {
  if (r === void 0)
    throw new ReferenceError("this hasn't been initialised - super() hasn't been called");
  return r;
}
function uA(r, Q) {
  r.prototype = Object.create(Q.prototype), r.prototype.constructor = r, r.__proto__ = Q;
}
/*!
 * GSAP 3.15.0
 * https://gsap.com
 *
 * @license Copyright 2008-2026, GreenSock. All rights reserved.
 * Subject to the terms at https://gsap.com/standard-license
 * @author: Jack Doyle, jack@greensock.com
*/
var zQ = {
  autoSleep: 120,
  force3D: "auto",
  nullTargetWarn: 1,
  units: {
    lineHeight: ""
  }
}, aB = {
  duration: 0.5,
  overwrite: !1,
  delay: 0
}, Ut, q, y, hQ = 1e8, m = 1 / hQ, rt = Math.PI * 2, PP = rt / 4, iP = 0, UA = Math.sqrt, rP = Math.cos, sP = Math.sin, Z = function(Q) {
  return typeof Q == "string";
}, J = function(Q) {
  return typeof Q == "function";
}, NQ = function(Q) {
  return typeof Q == "number";
}, Dt = function(Q) {
  return typeof Q > "u";
}, DQ = function(Q) {
  return typeof Q == "object";
}, AQ = function(Q) {
  return Q !== !1;
}, dt = function() {
  return typeof window < "u";
}, OB = function(Q) {
  return J(Q) || Z(Q);
}, DA = typeof ArrayBuffer == "function" && ArrayBuffer.isView || function() {
}, $ = Array.isArray, vP = /random\([^)]+\)/g, cP = /,\s*/g, Zt = /(?:-?\.?\d|\.)+/gi, dA = /[-+=.]*\d+[.e\-+]*\d*[e\-+]*\d*/g, qQ = /[-+=.]*\d+[.e-]*\d*[a-z%]*/g, JB = /[-+=.]*\d+\.?\d*(?:e-|e\+)?\d*/gi, CA = /[+-]=-?[.\d]+/, nP = /[^,'"\[\]\s]+/gi, oP = /^[+\-=e\s\d]*\d+[.\d]*([a-z]*|%)\s*$/i, Y, kQ, st, Ct, wQ = {}, FB = {}, NA, OA = function(Q) {
  return (FB = tB(Q, wQ)) && rQ;
}, Nt = function(Q, B) {
  return console.warn("Invalid property", Q, "set to", B, "Missing plugin? gsap.registerPlugin()");
}, EB = function(Q, B) {
  return !B && console.warn(Q);
}, LA = function(Q, B) {
  return Q && (wQ[Q] = B) && FB && (FB[Q] = B) || wQ;
}, hB = function() {
  return 0;
}, TP = {
  suppressEvents: !0,
  isStart: !0,
  kill: !1
}, LB = {
  suppressEvents: !0,
  kill: !1
}, zP = {
  suppressEvents: !0
}, Ot = {}, fQ = [], vt = {}, MA, cQ = {}, RB = {}, Wt = 30, MB = [], Lt = "", Mt = function(Q) {
  var B = Q[0], t, A;
  if (DQ(B) || J(B) || (Q = [Q]), !(t = (B._gsap || {}).harness)) {
    for (A = MB.length; A-- && !MB[A].targetTest(B); )
      ;
    t = MB[A];
  }
  for (A = Q.length; A--; )
    Q[A] && (Q[A]._gsap || (Q[A]._gsap = new qA(Q[A], t))) || Q.splice(A, 1);
  return Q;
}, YQ = function(Q) {
  return Q._gsap || Mt(lQ(Q))[0]._gsap;
}, HA = function(Q, B, t) {
  return (t = Q[B]) && J(t) ? Q[B]() : Dt(t) && Q.getAttribute && Q.getAttribute(B) || t;
}, eQ = function(Q, B) {
  return (Q = Q.split(",")).forEach(B) || Q;
}, X = function(Q) {
  return Math.round(Q * 1e5) / 1e5 || 0;
}, S = function(Q) {
  return Math.round(Q * 1e7) / 1e7 || 0;
}, $Q = function(Q, B) {
  var t = B.charAt(0), A = parseFloat(B.substr(2));
  return Q = parseFloat(Q), t === "+" ? Q + A : t === "-" ? Q - A : t === "*" ? Q * A : Q / A;
}, wP = function(Q, B) {
  for (var t = B.length, A = 0; Q.indexOf(B[A]) < 0 && ++A < t; )
    ;
  return A < t;
}, fB = function() {
  var Q = fQ.length, B = fQ.slice(0), t, A;
  for (vt = {}, fQ.length = 0, t = 0; t < Q; t++)
    A = B[t], A && A._lazy && (A.render(A._lazy[0], A._lazy[1], !0)._lazy = 0);
}, Ht = function(Q) {
  return !!(Q._initted || Q._startAt || Q.add);
}, jA = function(Q, B, t, A) {
  fQ.length && !q && fB(), Q.render(B, t, !!(q && B < 0 && Ht(Q))), fQ.length && !q && fB();
}, FA = function(Q) {
  var B = parseFloat(Q);
  return (B || B === 0) && (Q + "").match(nP).length < 2 ? B : Z(Q) ? Q.trim() : Q;
}, fA = function(Q) {
  return Q;
}, aQ = function(Q, B) {
  for (var t in B)
    t in Q || (Q[t] = B[t]);
  return Q;
}, aP = function(Q) {
  return function(B, t) {
    for (var A in t)
      A in B || A === "duration" && Q || A === "ease" || (B[A] = t[A]);
  };
}, tB = function(Q, B) {
  for (var t in B)
    Q[t] = B[t];
  return Q;
}, qt = function r(Q, B) {
  for (var t in B)
    t !== "__proto__" && t !== "constructor" && t !== "prototype" && (Q[t] = DQ(B[t]) ? r(Q[t] || (Q[t] = {}), B[t]) : B[t]);
  return Q;
}, GB = function(Q, B) {
  var t = {}, A;
  for (A in Q)
    A in B || (t[A] = Q[A]);
  return t;
}, TB = function(Q) {
  var B = Q.parent || Y, t = Q.keyframes ? aP($(Q.keyframes)) : aQ;
  if (AQ(Q.inherit))
    for (; B; )
      t(Q, B.vars.defaults), B = B.parent || B._dp;
  return Q;
}, EP = function(Q, B) {
  for (var t = Q.length, A = t === B.length; A && t-- && Q[t] === B[t]; )
    ;
  return t < 0;
}, GA = function(Q, B, t, A, e) {
  var P = Q[A], i;
  if (e)
    for (i = B[e]; P && P[e] > i; )
      P = P._prev;
  return P ? (B._next = P._next, P._next = B) : (B._next = Q[t], Q[t] = B), B._next ? B._next._prev = B : Q[A] = B, B._prev = P, B.parent = B._dp = Q, B;
}, xB = function(Q, B, t, A) {
  t === void 0 && (t = "_first"), A === void 0 && (A = "_last");
  var e = B._prev, P = B._next;
  e ? e._next = P : Q[t] === B && (Q[t] = P), P ? P._prev = e : Q[A] === B && (Q[A] = e), B._next = B._prev = B.parent = null;
}, bQ = function(Q, B) {
  Q.parent && (!B || Q.parent.autoRemoveChildren) && Q.parent.remove && Q.parent.remove(Q), Q._act = 0;
}, VQ = function(Q, B) {
  if (Q && (!B || B._end > Q._dur || B._start < 0))
    for (var t = Q; t; )
      t._dirty = 1, t = t.parent;
  return Q;
}, hP = function(Q) {
  for (var B = Q.parent; B && B.parent; )
    B._dirty = 1, B.totalDuration(), B = B.parent;
  return Q;
}, ct = function(Q, B, t, A) {
  return Q._startAt && (q ? Q._startAt.revert(LB) : Q.vars.immediateRender && !Q.vars.autoRevert || Q._startAt.render(B, !0, A));
}, lP = function r(Q) {
  return !Q || Q._ts && r(Q.parent);
}, _t = function(Q) {
  return Q._repeat ? AB(Q._tTime, Q = Q.duration() + Q._rDelay) * Q : 0;
}, AB = function(Q, B) {
  var t = Math.floor(Q = S(Q / B));
  return Q && t === Q ? t - 1 : t;
}, bB = function(Q, B) {
  return (Q - B._start) * B._ts + (B._ts >= 0 ? 0 : B._dirty ? B.totalDuration() : B._tDur);
}, yB = function(Q) {
  return Q._end = S(Q._start + (Q._tDur / Math.abs(Q._ts || Q._rts || m) || 0));
}, IB = function(Q, B) {
  var t = Q._dp;
  return t && t.smoothChildTiming && Q._ts && (Q._start = S(t._time - (Q._ts > 0 ? B / Q._ts : ((Q._dirty ? Q.totalDuration() : Q._tDur) - B) / -Q._ts)), yB(Q), t._dirty || VQ(t, Q)), Q;
}, bA = function(Q, B) {
  var t;
  if ((B._time || !B._dur && B._initted || B._start < Q._time && (B._dur || !B.add)) && (t = bB(Q.rawTime(), B), (!B._dur || dB(0, B.totalDuration(), t) - B._tTime > m) && B.render(t, !0)), VQ(Q, B)._dp && Q._initted && Q._time >= Q._dur && Q._ts) {
    if (Q._dur < Q.duration())
      for (t = Q; t._dp; )
        t.rawTime() >= 0 && t.totalTime(t._tTime), t = t._dp;
    Q._zTime = -m;
  }
}, uQ = function(Q, B, t, A) {
  return B.parent && bQ(B), B._start = S((NQ(t) ? t : t || Q !== Y ? EQ(Q, t, B) : Q._time) + B._delay), B._end = S(B._start + (B.totalDuration() / Math.abs(B.timeScale()) || 0)), GA(Q, B, "_first", "_last", Q._sort ? "_start" : 0), nt(B) || (Q._recent = B), A || bA(Q, B), Q._ts < 0 && IB(Q, Q._tTime), Q;
}, gA = function(Q, B) {
  return (wQ.ScrollTrigger || Nt("scrollTrigger", B)) && wQ.ScrollTrigger.create(B, Q);
}, pA = function(Q, B, t, A, e) {
  if (Ft(Q, B, e), !Q._initted)
    return 1;
  if (!t && Q._pt && !q && (Q._dur && Q.vars.lazy !== !1 || !Q._dur && Q.vars.lazy) && MA !== nQ.frame)
    return fQ.push(Q), Q._lazy = [e, A], 1;
}, kP = function r(Q) {
  var B = Q.parent;
  return B && B._ts && B._initted && !B._lock && (B.rawTime() < 0 || r(B));
}, nt = function(Q) {
  var B = Q.data;
  return B === "isFromStart" || B === "isStart";
}, uP = function(Q, B, t, A) {
  var e = Q.ratio, P = B < 0 || !B && (!Q._start && kP(Q) && !(!Q._initted && nt(Q)) || (Q._ts < 0 || Q._dp._ts < 0) && !nt(Q)) ? 0 : 1, i = Q._rDelay, s = 0, v, c, n;
  if (i && Q._repeat && (s = dB(0, Q._tDur, B), c = AB(s, i), Q._yoyo && c & 1 && (P = 1 - P), c !== AB(Q._tTime, i) && (e = 1 - P, Q.vars.repeatRefresh && Q._initted && Q.invalidate())), P !== e || q || A || Q._zTime === m || !B && Q._zTime) {
    if (!Q._initted && pA(Q, B, A, t, s))
      return;
    for (n = Q._zTime, Q._zTime = B || (t ? m : 0), t || (t = B && !n), Q.ratio = P, Q._from && (P = 1 - P), Q._time = 0, Q._tTime = s, v = Q._pt; v; )
      v.r(P, v.d), v = v._next;
    B < 0 && ct(Q, B, t, !0), Q._onUpdate && !t && oQ(Q, "onUpdate"), s && Q._repeat && !t && Q.parent && oQ(Q, "onRepeat"), (B >= Q._tDur || B < 0) && Q.ratio === P && (P && bQ(Q, 1), !t && !q && (oQ(Q, P ? "onComplete" : "onReverseComplete", !0), Q._prom && Q._prom()));
  } else Q._zTime || (Q._zTime = B);
}, UP = function(Q, B, t) {
  var A;
  if (t > B)
    for (A = Q._first; A && A._start <= t; ) {
      if (A.data === "isPause" && A._start > B)
        return A;
      A = A._next;
    }
  else
    for (A = Q._last; A && A._start >= t; ) {
      if (A.data === "isPause" && A._start < B)
        return A;
      A = A._prev;
    }
}, eB = function(Q, B, t, A) {
  var e = Q._repeat, P = S(B) || 0, i = Q._tTime / Q._tDur;
  return i && !A && (Q._time *= P / Q._dur), Q._dur = P, Q._tDur = e ? e < 0 ? 1e10 : S(P * (e + 1) + Q._rDelay * e) : P, i > 0 && !A && IB(Q, Q._tTime = Q._tDur * i), Q.parent && yB(Q), t || VQ(Q.parent, Q), Q;
}, $t = function(Q) {
  return Q instanceof tQ ? VQ(Q) : eB(Q, Q._dur);
}, DP = {
  _start: 0,
  endTime: hB,
  totalDuration: hB
}, EQ = function r(Q, B, t) {
  var A = Q.labels, e = Q._recent || DP, P = Q.duration() >= hQ ? e.endTime(!1) : Q._dur, i, s, v;
  return Z(B) && (isNaN(B) || B in A) ? (s = B.charAt(0), v = B.substr(-1) === "%", i = B.indexOf("="), s === "<" || s === ">" ? (i >= 0 && (B = B.replace(/=/, "")), (s === "<" ? e._start : e.endTime(e._repeat >= 0)) + (parseFloat(B.substr(1)) || 0) * (v ? (i < 0 ? e : t).totalDuration() / 100 : 1)) : i < 0 ? (B in A || (A[B] = P), A[B]) : (s = parseFloat(B.charAt(i - 1) + B.substr(i + 1)), v && t && (s = s / 100 * ($(t) ? t[0] : t).totalDuration()), i > 1 ? r(Q, B.substr(0, i - 1), t) + s : P + s)) : B == null ? P : +B;
}, zB = function(Q, B, t) {
  var A = NQ(B[1]), e = (A ? 2 : 1) + (Q < 2 ? 0 : 1), P = B[e], i, s;
  if (A && (P.duration = B[1]), P.parent = t, Q) {
    for (i = P, s = t; s && !("immediateRender" in i); )
      i = s.vars.defaults || {}, s = AQ(s.vars.inherit) && s.parent;
    P.immediateRender = AQ(i.immediateRender), Q < 2 ? P.runBackwards = 1 : P.startAt = B[e - 1];
  }
  return new K(B[0], P, B[e + 1]);
}, mQ = function(Q, B) {
  return Q || Q === 0 ? B(Q) : B;
}, dB = function(Q, B, t) {
  return t < Q ? Q : t > B ? B : t;
}, _ = function(Q, B) {
  return !Z(Q) || !(B = oP.exec(Q)) ? "" : B[1];
}, dP = function(Q, B, t) {
  return mQ(t, function(A) {
    return dB(Q, B, A);
  });
}, ot = [].slice, mA = function(Q, B) {
  return Q && DQ(Q) && "length" in Q && (!B && !Q.length || Q.length - 1 in Q && DQ(Q[0])) && !Q.nodeType && Q !== kQ;
}, CP = function(Q, B, t) {
  return t === void 0 && (t = []), Q.forEach(function(A) {
    var e;
    return Z(A) && !B || mA(A, 1) ? (e = t).push.apply(e, lQ(A)) : t.push(A);
  }) || t;
}, lQ = function(Q, B, t) {
  return y && !B && y.selector ? y.selector(Q) : Z(Q) && !t && (st || !PB()) ? ot.call((B || Ct).querySelectorAll(Q), 0) : $(Q) ? CP(Q, t) : mA(Q) ? ot.call(Q, 0) : Q ? [Q] : [];
}, Tt = function(Q) {
  return Q = lQ(Q)[0] || EB("Invalid scope") || {}, function(B) {
    var t = Q.current || Q.nativeElement || Q;
    return lQ(B, t.querySelectorAll ? t : t === Q ? EB("Invalid scope") || Ct.createElement("div") : Q);
  };
}, xA = function(Q) {
  return Q.sort(function() {
    return 0.5 - Math.random();
  });
}, yA = function(Q) {
  if (J(Q))
    return Q;
  var B = DQ(Q) ? Q : {
    each: Q
  }, t = JQ(B.ease), A = B.from || 0, e = parseFloat(B.base) || 0, P = {}, i = A > 0 && A < 1, s = isNaN(A) || i, v = B.axis, c = A, n = A;
  return Z(A) ? c = n = {
    center: 0.5,
    edges: 0.5,
    end: 1
  }[A] || 0 : !i && s && (c = A[0], n = A[1]), function(T, z, w) {
    var o = (w || B).length, a = P[o], E, l, u, U, k, O, D, N, d;
    if (!a) {
      if (d = B.grid === "auto" ? 0 : (B.grid || [1, hQ])[1], !d) {
        for (D = -hQ; D < (D = w[d++].getBoundingClientRect().left) && d < o; )
          ;
        d < o && d--;
      }
      for (a = P[o] = [], E = s ? Math.min(d, o) * c - 0.5 : A % d, l = d === hQ ? 0 : s ? o * n / d - 0.5 : A / d | 0, D = 0, N = hQ, O = 0; O < o; O++)
        u = O % d - E, U = l - (O / d | 0), a[O] = k = v ? Math.abs(v === "y" ? U : u) : UA(u * u + U * U), k > D && (D = k), k < N && (N = k);
      A === "random" && xA(a), a.max = D - N, a.min = N, a.v = o = (parseFloat(B.amount) || parseFloat(B.each) * (d > o ? o - 1 : v ? v === "y" ? o / d : d : Math.max(d, o / d)) || 0) * (A === "edges" ? -1 : 1), a.b = o < 0 ? e - o : e, a.u = _(B.amount || B.each) || 0, t = t && o < 0 ? mP(t) : t;
    }
    return o = (a[T] - a.min) / a.max || 0, S(a.b + (t ? t(o) : o) * a.v) + a.u;
  };
}, zt = function(Q) {
  var B = Math.pow(10, ((Q + "").split(".")[1] || "").length);
  return function(t) {
    var A = S(Math.round(parseFloat(t) / Q) * Q * B);
    return (A - A % 1) / B + (NQ(t) ? 0 : _(t));
  };
}, IA = function(Q, B) {
  var t = $(Q), A, e;
  return !t && DQ(Q) && (A = t = Q.radius || hQ, Q.values ? (Q = lQ(Q.values), (e = !NQ(Q[0])) && (A *= A)) : Q = zt(Q.increment)), mQ(B, t ? J(Q) ? function(P) {
    return e = Q(P), Math.abs(e - P) <= A ? e : P;
  } : function(P) {
    for (var i = parseFloat(e ? P.x : P), s = parseFloat(e ? P.y : 0), v = hQ, c = 0, n = Q.length, T, z; n--; )
      e ? (T = Q[n].x - i, z = Q[n].y - s, T = T * T + z * z) : T = Math.abs(Q[n] - i), T < v && (v = T, c = n);
    return c = !A || v <= A ? Q[c] : P, e || c === P || NQ(P) ? c : c + _(P);
  } : zt(Q));
}, SA = function(Q, B, t, A) {
  return mQ($(Q) ? !B : t === !0 ? !!(t = 0) : !A, function() {
    return $(Q) ? Q[~~(Math.random() * Q.length)] : (t = t || 1e-5) && (A = t < 1 ? Math.pow(10, (t + "").length - 2) : 1) && Math.floor(Math.round((Q - t / 2 + Math.random() * (B - Q + t * 0.99)) / t) * t * A) / A;
  });
}, NP = function() {
  for (var Q = arguments.length, B = new Array(Q), t = 0; t < Q; t++)
    B[t] = arguments[t];
  return function(A) {
    return B.reduce(function(e, P) {
      return P(e);
    }, A);
  };
}, OP = function(Q, B) {
  return function(t) {
    return Q(parseFloat(t)) + (B || _(t));
  };
}, LP = function(Q, B, t) {
  return VA(Q, B, 0, 1, t);
}, YA = function(Q, B, t) {
  return mQ(t, function(A) {
    return Q[~~B(A)];
  });
}, MP = function r(Q, B, t) {
  var A = B - Q;
  return $(Q) ? YA(Q, r(0, Q.length), B) : mQ(t, function(e) {
    return (A + (e - Q) % A) % A + Q;
  });
}, HP = function r(Q, B, t) {
  var A = B - Q, e = A * 2;
  return $(Q) ? YA(Q, r(0, Q.length - 1), B) : mQ(t, function(P) {
    return P = (e + (P - Q) % e) % e || 0, Q + (P > A ? e - P : P);
  });
}, lB = function(Q) {
  return Q.replace(vP, function(B) {
    var t = B.indexOf("[") + 1, A = B.substring(t || 7, t ? B.indexOf("]") : B.length - 1).split(cP);
    return SA(t ? A : +A[0], t ? 0 : +A[1], +A[2] || 1e-5);
  });
}, VA = function(Q, B, t, A, e) {
  var P = B - Q, i = A - t;
  return mQ(e, function(s) {
    return t + ((s - Q) / P * i || 0);
  });
}, jP = function r(Q, B, t, A) {
  var e = isNaN(Q + B) ? 0 : function(z) {
    return (1 - z) * Q + z * B;
  };
  if (!e) {
    var P = Z(Q), i = {}, s, v, c, n, T;
    if (t === !0 && (A = 1) && (t = null), P)
      Q = {
        p: Q
      }, B = {
        p: B
      };
    else if ($(Q) && !$(B)) {
      for (c = [], n = Q.length, T = n - 2, v = 1; v < n; v++)
        c.push(r(Q[v - 1], Q[v]));
      n--, e = function(w) {
        w *= n;
        var o = Math.min(T, ~~w);
        return c[o](w - o);
      }, t = B;
    } else A || (Q = tB($(Q) ? [] : {}, Q));
    if (!c) {
      for (s in B)
        jt.call(i, Q, s, "get", B[s]);
      e = function(w) {
        return bt(w, i) || (P ? Q.p : Q);
      };
    }
  }
  return mQ(t, e);
}, QA = function(Q, B, t) {
  var A = Q.labels, e = hQ, P, i, s;
  for (P in A)
    i = A[P] - B, i < 0 == !!t && i && e > (i = Math.abs(i)) && (s = P, e = i);
  return s;
}, oQ = function(Q, B, t) {
  var A = Q.vars, e = A[B], P = y, i = Q._ctx, s, v, c;
  if (e)
    return s = A[B + "Params"], v = A.callbackScope || Q, t && fQ.length && fB(), i && (y = i), c = s ? e.apply(v, s) : e.call(v), y = P, c;
}, nB = function(Q) {
  return bQ(Q), Q.scrollTrigger && Q.scrollTrigger.kill(!!q), Q.progress() < 1 && oQ(Q, "onInterrupt"), Q;
}, _Q, JA = [], RA = function(Q) {
  if (Q)
    if (Q = !Q.name && Q.default || Q, dt() || Q.headless) {
      var B = Q.name, t = J(Q), A = B && !t && Q.init ? function() {
        this._props = [];
      } : Q, e = {
        init: hB,
        render: bt,
        add: jt,
        kill: KP,
        modifier: XP,
        rawVars: 0
      }, P = {
        targetTest: 0,
        get: 0,
        getSetter: Gt,
        aliases: {},
        register: 0
      };
      if (PB(), Q !== A) {
        if (cQ[B])
          return;
        aQ(A, aQ(GB(Q, e), P)), tB(A.prototype, tB(e, GB(Q, P))), cQ[A.prop = B] = A, Q.targetTest && (MB.push(A), Ot[B] = 1), B = (B === "css" ? "CSS" : B.charAt(0).toUpperCase() + B.substr(1)) + "Plugin";
      }
      LA(B, A), Q.register && Q.register(rQ, A, PQ);
    } else
      JA.push(Q);
}, p = 255, oB = {
  aqua: [0, p, p],
  lime: [0, p, 0],
  silver: [192, 192, 192],
  black: [0, 0, 0],
  maroon: [128, 0, 0],
  teal: [0, 128, 128],
  blue: [0, 0, p],
  navy: [0, 0, 128],
  white: [p, p, p],
  olive: [128, 128, 0],
  yellow: [p, p, 0],
  orange: [p, 165, 0],
  gray: [128, 128, 128],
  purple: [128, 0, 128],
  green: [0, 128, 0],
  red: [p, 0, 0],
  pink: [p, 192, 203],
  cyan: [0, p, p],
  transparent: [p, p, p, 0]
}, XB = function(Q, B, t) {
  return Q += Q < 0 ? 1 : Q > 1 ? -1 : 0, (Q * 6 < 1 ? B + (t - B) * Q * 6 : Q < 0.5 ? t : Q * 3 < 2 ? B + (t - B) * (2 / 3 - Q) * 6 : B) * p + 0.5 | 0;
}, XA = function(Q, B, t) {
  var A = Q ? NQ(Q) ? [Q >> 16, Q >> 8 & p, Q & p] : 0 : oB.black, e, P, i, s, v, c, n, T, z, w;
  if (!A) {
    if (Q.substr(-1) === "," && (Q = Q.substr(0, Q.length - 1)), oB[Q])
      A = oB[Q];
    else if (Q.charAt(0) === "#") {
      if (Q.length < 6 && (e = Q.charAt(1), P = Q.charAt(2), i = Q.charAt(3), Q = "#" + e + e + P + P + i + i + (Q.length === 5 ? Q.charAt(4) + Q.charAt(4) : "")), Q.length === 9)
        return A = parseInt(Q.substr(1, 6), 16), [A >> 16, A >> 8 & p, A & p, parseInt(Q.substr(7), 16) / 255];
      Q = parseInt(Q.substr(1), 16), A = [Q >> 16, Q >> 8 & p, Q & p];
    } else if (Q.substr(0, 3) === "hsl") {
      if (A = w = Q.match(Zt), !B)
        s = +A[0] % 360 / 360, v = +A[1] / 100, c = +A[2] / 100, P = c <= 0.5 ? c * (v + 1) : c + v - c * v, e = c * 2 - P, A.length > 3 && (A[3] *= 1), A[0] = XB(s + 1 / 3, e, P), A[1] = XB(s, e, P), A[2] = XB(s - 1 / 3, e, P);
      else if (~Q.indexOf("="))
        return A = Q.match(dA), t && A.length < 4 && (A[3] = 1), A;
    } else
      A = Q.match(Zt) || oB.transparent;
    A = A.map(Number);
  }
  return B && !w && (e = A[0] / p, P = A[1] / p, i = A[2] / p, n = Math.max(e, P, i), T = Math.min(e, P, i), c = (n + T) / 2, n === T ? s = v = 0 : (z = n - T, v = c > 0.5 ? z / (2 - n - T) : z / (n + T), s = n === e ? (P - i) / z + (P < i ? 6 : 0) : n === P ? (i - e) / z + 2 : (e - P) / z + 4, s *= 60), A[0] = ~~(s + 0.5), A[1] = ~~(v * 100 + 0.5), A[2] = ~~(c * 100 + 0.5)), t && A.length < 4 && (A[3] = 1), A;
}, KA = function(Q) {
  var B = [], t = [], A = -1;
  return Q.split(GQ).forEach(function(e) {
    var P = e.match(qQ) || [];
    B.push.apply(B, P), t.push(A += P.length + 1);
  }), B.c = t, B;
}, BA = function(Q, B, t) {
  var A = "", e = (Q + A).match(GQ), P = B ? "hsla(" : "rgba(", i = 0, s, v, c, n;
  if (!e)
    return Q;
  if (e = e.map(function(T) {
    return (T = XA(T, B, 1)) && P + (B ? T[0] + "," + T[1] + "%," + T[2] + "%," + T[3] : T.join(",")) + ")";
  }), t && (c = KA(Q), s = t.c, s.join(A) !== c.c.join(A)))
    for (v = Q.replace(GQ, "1").split(qQ), n = v.length - 1; i < n; i++)
      A += v[i] + (~s.indexOf(i) ? e.shift() || P + "0,0,0,0)" : (c.length ? c : e.length ? e : t).shift());
  if (!v)
    for (v = Q.split(GQ), n = v.length - 1; i < n; i++)
      A += v[i] + e[i];
  return A + v[n];
}, GQ = function() {
  var r = "(?:\\b(?:(?:rgb|rgba|hsl|hsla)\\(.+?\\))|\\B#(?:[0-9a-f]{3,4}){1,2}\\b", Q;
  for (Q in oB)
    r += "|" + Q + "\\b";
  return new RegExp(r + ")", "gi");
}(), FP = /hsl[a]?\(/, ZA = function(Q) {
  var B = Q.join(" "), t;
  if (GQ.lastIndex = 0, GQ.test(B))
    return t = FP.test(B), Q[1] = BA(Q[1], t), Q[0] = BA(Q[0], t, KA(Q[1])), !0;
}, kB, nQ = function() {
  var r = Date.now, Q = 500, B = 33, t = r(), A = t, e = 1e3 / 240, P = e, i = [], s, v, c, n, T, z, w = function o(a) {
    var E = r() - A, l = a === !0, u, U, k, O;
    if ((E > Q || E < 0) && (t += E - B), A += E, k = A - t, u = k - P, (u > 0 || l) && (O = ++n.frame, T = k - n.time * 1e3, n.time = k = k / 1e3, P += u + (u >= e ? 4 : e - u), U = 1), l || (s = v(o)), U)
      for (z = 0; z < i.length; z++)
        i[z](k, T, O, a);
  };
  return n = {
    time: 0,
    frame: 0,
    tick: function() {
      w(!0);
    },
    deltaRatio: function(a) {
      return T / (1e3 / (a || 60));
    },
    wake: function() {
      NA && (!st && dt() && (kQ = st = window, Ct = kQ.document || {}, wQ.gsap = rQ, (kQ.gsapVersions || (kQ.gsapVersions = [])).push(rQ.version), OA(FB || kQ.GreenSockGlobals || !kQ.gsap && kQ || {}), JA.forEach(RA)), c = typeof requestAnimationFrame < "u" && requestAnimationFrame, s && n.sleep(), v = c || function(a) {
        return setTimeout(a, P - n.time * 1e3 + 1 | 0);
      }, kB = 1, w(2));
    },
    sleep: function() {
      (c ? cancelAnimationFrame : clearTimeout)(s), kB = 0, v = hB;
    },
    lagSmoothing: function(a, E) {
      Q = a || 1 / 0, B = Math.min(E || 33, Q);
    },
    fps: function(a) {
      e = 1e3 / (a || 240), P = n.time * 1e3 + e;
    },
    add: function(a, E, l) {
      var u = E ? function(U, k, O, D) {
        a(U, k, O, D), n.remove(u);
      } : a;
      return n.remove(a), i[l ? "unshift" : "push"](u), PB(), u;
    },
    remove: function(a, E) {
      ~(E = i.indexOf(a)) && i.splice(E, 1) && z >= E && z--;
    },
    _listeners: i
  }, n;
}(), PB = function() {
  return !kB && nQ.wake();
}, g = {}, fP = /^[\d.\-M][\d.\-,\s]/, GP = /["']/g, bP = function(Q) {
  for (var B = {}, t = Q.substr(1, Q.length - 3).split(":"), A = t[0], e = 1, P = t.length, i, s, v; e < P; e++)
    s = t[e], i = e !== P - 1 ? s.lastIndexOf(",") : s.length, v = s.substr(0, i), B[A] = isNaN(v) ? v.replace(GP, "").trim() : +v, A = s.substr(i + 1).trim();
  return B;
}, gP = function(Q) {
  var B = Q.indexOf("(") + 1, t = Q.indexOf(")"), A = Q.indexOf("(", B);
  return Q.substring(B, ~A && A < t ? Q.indexOf(")", t + 1) : t);
}, pP = function(Q) {
  var B = (Q + "").split("("), t = g[B[0]];
  return t && B.length > 1 && t.config ? t.config.apply(null, ~Q.indexOf("{") ? [bP(B[1])] : gP(Q).split(",").map(FA)) : g._CE && fP.test(Q) ? g._CE("", Q) : t;
}, mP = function(Q) {
  return function(B) {
    return 1 - Q(1 - B);
  };
}, JQ = function(Q, B) {
  return Q && (J(Q) ? Q : g[Q] || pP(Q)) || B;
}, XQ = function(Q, B, t, A) {
  t === void 0 && (t = function(s) {
    return 1 - B(1 - s);
  }), A === void 0 && (A = function(s) {
    return s < 0.5 ? B(s * 2) / 2 : 1 - B((1 - s) * 2) / 2;
  });
  var e = {
    easeIn: B,
    easeOut: t,
    easeInOut: A
  }, P;
  return eQ(Q, function(i) {
    g[i] = wQ[i] = e, g[P = i.toLowerCase()] = t;
    for (var s in e)
      g[P + (s === "easeIn" ? ".in" : s === "easeOut" ? ".out" : ".inOut")] = g[i + "." + s] = e[s];
  }), e;
}, WA = function(Q) {
  return function(B) {
    return B < 0.5 ? (1 - Q(1 - B * 2)) / 2 : 0.5 + Q((B - 0.5) * 2) / 2;
  };
}, KB = function r(Q, B, t) {
  var A = B >= 1 ? B : 1, e = (t || (Q ? 0.3 : 0.45)) / (B < 1 ? B : 1), P = e / rt * (Math.asin(1 / A) || 0), i = function(c) {
    return c === 1 ? 1 : A * Math.pow(2, -10 * c) * sP((c - P) * e) + 1;
  }, s = Q === "out" ? i : Q === "in" ? function(v) {
    return 1 - i(1 - v);
  } : WA(i);
  return e = rt / e, s.config = function(v, c) {
    return r(Q, v, c);
  }, s;
}, ZB = function r(Q, B) {
  B === void 0 && (B = 1.70158);
  var t = function(P) {
    return P ? --P * P * ((B + 1) * P + B) + 1 : 0;
  }, A = Q === "out" ? t : Q === "in" ? function(e) {
    return 1 - t(1 - e);
  } : WA(t);
  return A.config = function(e) {
    return r(Q, e);
  }, A;
};
eQ("Linear,Quad,Cubic,Quart,Quint,Strong", function(r, Q) {
  var B = Q < 5 ? Q + 1 : Q;
  XQ(r + ",Power" + (B - 1), Q ? function(t) {
    return Math.pow(t, B);
  } : function(t) {
    return t;
  }, function(t) {
    return 1 - Math.pow(1 - t, B);
  }, function(t) {
    return t < 0.5 ? Math.pow(t * 2, B) / 2 : 1 - Math.pow((1 - t) * 2, B) / 2;
  });
});
g.Linear.easeNone = g.none = g.Linear.easeIn;
XQ("Elastic", KB("in"), KB("out"), KB());
(function(r, Q) {
  var B = 1 / Q, t = 2 * B, A = 2.5 * B, e = function(i) {
    return i < B ? r * i * i : i < t ? r * Math.pow(i - 1.5 / Q, 2) + 0.75 : i < A ? r * (i -= 2.25 / Q) * i + 0.9375 : r * Math.pow(i - 2.625 / Q, 2) + 0.984375;
  };
  XQ("Bounce", function(P) {
    return 1 - e(1 - P);
  }, e);
})(7.5625, 2.75);
XQ("Expo", function(r) {
  return Math.pow(2, 10 * (r - 1)) * r + r * r * r * r * r * r * (1 - r);
});
XQ("Circ", function(r) {
  return -(UA(1 - r * r) - 1);
});
XQ("Sine", function(r) {
  return r === 1 ? 1 : -rP(r * PP) + 1;
});
XQ("Back", ZB("in"), ZB("out"), ZB());
g.SteppedEase = g.steps = wQ.SteppedEase = {
  config: function(Q, B) {
    Q === void 0 && (Q = 1);
    var t = 1 / Q, A = Q + (B ? 0 : 1), e = B ? 1 : 0, P = 1 - m;
    return function(i) {
      return ((A * dB(0, P, i) | 0) + e) * t;
    };
  }
};
aB.ease = g["quad.out"];
eQ("onComplete,onUpdate,onStart,onRepeat,onReverseComplete,onInterrupt", function(r) {
  return Lt += r + "," + r + "Params,";
});
var qA = function(Q, B) {
  this.id = iP++, Q._gsap = this, this.target = Q, this.harness = B, this.get = B ? B.get : HA, this.set = B ? B.getSetter : Gt;
}, uB = /* @__PURE__ */ function() {
  function r(B) {
    this.vars = B, this._delay = +B.delay || 0, (this._repeat = B.repeat === 1 / 0 ? -2 : B.repeat || 0) && (this._rDelay = B.repeatDelay || 0, this._yoyo = !!B.yoyo || !!B.yoyoEase), this._ts = 1, eB(this, +B.duration, 1, 1), this.data = B.data, y && (this._ctx = y, y.data.push(this)), kB || nQ.wake();
  }
  var Q = r.prototype;
  return Q.delay = function(t) {
    return t || t === 0 ? (this.parent && this.parent.smoothChildTiming && this.startTime(this._start + t - this._delay), this._delay = t, this) : this._delay;
  }, Q.duration = function(t) {
    return arguments.length ? this.totalDuration(this._repeat > 0 ? t + (t + this._rDelay) * this._repeat : t) : this.totalDuration() && this._dur;
  }, Q.totalDuration = function(t) {
    return arguments.length ? (this._dirty = 0, eB(this, this._repeat < 0 ? t : (t - this._repeat * this._rDelay) / (this._repeat + 1))) : this._tDur;
  }, Q.totalTime = function(t, A) {
    if (PB(), !arguments.length)
      return this._tTime;
    var e = this._dp;
    if (e && e.smoothChildTiming && this._ts) {
      for (IB(this, t), !e._dp || e.parent || bA(e, this); e && e.parent; )
        e.parent._time !== e._start + (e._ts >= 0 ? e._tTime / e._ts : (e.totalDuration() - e._tTime) / -e._ts) && e.totalTime(e._tTime, !0), e = e.parent;
      !this.parent && this._dp.autoRemoveChildren && (this._ts > 0 && t < this._tDur || this._ts < 0 && t > 0 || !this._tDur && !t) && uQ(this._dp, this, this._start - this._delay);
    }
    return (this._tTime !== t || !this._dur && !A || this._initted && Math.abs(this._zTime) === m || !this._initted && this._dur && t || !t && !this._initted && (this.add || this._ptLookup)) && (this._ts || (this._pTime = t), jA(this, t, A)), this;
  }, Q.time = function(t, A) {
    return arguments.length ? this.totalTime(Math.min(this.totalDuration(), t + _t(this)) % (this._dur + this._rDelay) || (t ? this._dur : 0), A) : this._time;
  }, Q.totalProgress = function(t, A) {
    return arguments.length ? this.totalTime(this.totalDuration() * t, A) : this.totalDuration() ? Math.min(1, this._tTime / this._tDur) : this.rawTime() >= 0 && this._initted ? 1 : 0;
  }, Q.progress = function(t, A) {
    return arguments.length ? this.totalTime(this.duration() * (this._yoyo && !(this.iteration() & 1) ? 1 - t : t) + _t(this), A) : this.duration() ? Math.min(1, this._time / this._dur) : this.rawTime() > 0 ? 1 : 0;
  }, Q.iteration = function(t, A) {
    var e = this.duration() + this._rDelay;
    return arguments.length ? this.totalTime(this._time + (t - 1) * e, A) : this._repeat ? AB(this._tTime, e) + 1 : 1;
  }, Q.timeScale = function(t, A) {
    if (!arguments.length)
      return this._rts === -m ? 0 : this._rts;
    if (this._rts === t)
      return this;
    var e = this.parent && this._ts ? bB(this.parent._time, this) : this._tTime;
    return this._rts = +t || 0, this._ts = this._ps || t === -m ? 0 : this._rts, this.totalTime(dB(-Math.abs(this._delay), this.totalDuration(), e), A !== !1), yB(this), hP(this);
  }, Q.paused = function(t) {
    return arguments.length ? (this._ps !== t && (this._ps = t, t ? (this._pTime = this._tTime || Math.max(-this._delay, this.rawTime()), this._ts = this._act = 0) : (PB(), this._ts = this._rts, this.totalTime(this.parent && !this.parent.smoothChildTiming ? this.rawTime() : this._tTime || this._pTime, this.progress() === 1 && Math.abs(this._zTime) !== m && (this._tTime -= m)))), this) : this._ps;
  }, Q.startTime = function(t) {
    if (arguments.length) {
      this._start = S(t);
      var A = this.parent || this._dp;
      return A && (A._sort || !this.parent) && uQ(A, this, this._start - this._delay), this;
    }
    return this._start;
  }, Q.endTime = function(t) {
    return this._start + (AQ(t) ? this.totalDuration() : this.duration()) / Math.abs(this._ts || 1);
  }, Q.rawTime = function(t) {
    var A = this.parent || this._dp;
    return A ? t && (!this._ts || this._repeat && this._time && this.totalProgress() < 1) ? this._tTime % (this._dur + this._rDelay) : this._ts ? bB(A.rawTime(t), this) : this._tTime : this._tTime;
  }, Q.revert = function(t) {
    t === void 0 && (t = zP);
    var A = q;
    return q = t, Ht(this) && (this.timeline && this.timeline.revert(t), this.totalTime(-0.01, t.suppressEvents)), this.data !== "nested" && t.kill !== !1 && this.kill(), q = A, this;
  }, Q.globalTime = function(t) {
    for (var A = this, e = arguments.length ? t : A.rawTime(); A; )
      e = A._start + e / (Math.abs(A._ts) || 1), A = A._dp;
    return !this.parent && this._sat ? this._sat.globalTime(t) : e;
  }, Q.repeat = function(t) {
    return arguments.length ? (this._repeat = t === 1 / 0 ? -2 : t, $t(this)) : this._repeat === -2 ? 1 / 0 : this._repeat;
  }, Q.repeatDelay = function(t) {
    if (arguments.length) {
      var A = this._time;
      return this._rDelay = t, $t(this), A ? this.time(A) : this;
    }
    return this._rDelay;
  }, Q.yoyo = function(t) {
    return arguments.length ? (this._yoyo = t, this) : this._yoyo;
  }, Q.seek = function(t, A) {
    return this.totalTime(EQ(this, t), AQ(A));
  }, Q.restart = function(t, A) {
    return this.play().totalTime(t ? -this._delay : 0, AQ(A)), this._dur || (this._zTime = -m), this;
  }, Q.play = function(t, A) {
    return t != null && this.seek(t, A), this.reversed(!1).paused(!1);
  }, Q.reverse = function(t, A) {
    return t != null && this.seek(t || this.totalDuration(), A), this.reversed(!0).paused(!1);
  }, Q.pause = function(t, A) {
    return t != null && this.seek(t, A), this.paused(!0);
  }, Q.resume = function() {
    return this.paused(!1);
  }, Q.reversed = function(t) {
    return arguments.length ? (!!t !== this.reversed() && this.timeScale(-this._rts || (t ? -m : 0)), this) : this._rts < 0;
  }, Q.invalidate = function() {
    return this._initted = this._act = 0, this._zTime = -m, this;
  }, Q.isActive = function() {
    var t = this.parent || this._dp, A = this._start, e;
    return !!(!t || this._ts && this._initted && t.isActive() && (e = t.rawTime(!0)) >= A && e < this.endTime(!0) - m);
  }, Q.eventCallback = function(t, A, e) {
    var P = this.vars;
    return arguments.length > 1 ? (A ? (P[t] = A, e && (P[t + "Params"] = e), t === "onUpdate" && (this._onUpdate = A)) : delete P[t], this) : P[t];
  }, Q.then = function(t) {
    var A = this, e = A._prom;
    return new Promise(function(P) {
      var i = J(t) ? t : fA, s = function() {
        var c = A.then;
        A.then = null, e && e(), J(i) && (i = i(A)) && (i.then || i === A) && (A.then = c), P(i), A.then = c;
      };
      A._initted && A.totalProgress() === 1 && A._ts >= 0 || !A._tTime && A._ts < 0 ? s() : A._prom = s;
    });
  }, Q.kill = function() {
    nB(this);
  }, r;
}();
aQ(uB.prototype, {
  _time: 0,
  _start: 0,
  _end: 0,
  _tTime: 0,
  _tDur: 0,
  _dirty: 0,
  _repeat: 0,
  _yoyo: !1,
  parent: null,
  _initted: !1,
  _rDelay: 0,
  _ts: 1,
  _dp: 0,
  ratio: 0,
  _zTime: -m,
  _prom: 0,
  _ps: !1,
  _rts: 1
});
var tQ = /* @__PURE__ */ function(r) {
  uA(Q, r);
  function Q(t, A) {
    var e;
    return t === void 0 && (t = {}), e = r.call(this, t) || this, e.labels = {}, e.smoothChildTiming = !!t.smoothChildTiming, e.autoRemoveChildren = !!t.autoRemoveChildren, e._sort = AQ(t.sortChildren), Y && uQ(t.parent || Y, dQ(e), A), t.reversed && e.reverse(), t.paused && e.paused(!0), t.scrollTrigger && gA(dQ(e), t.scrollTrigger), e;
  }
  var B = Q.prototype;
  return B.to = function(A, e, P) {
    return zB(0, arguments, this), this;
  }, B.from = function(A, e, P) {
    return zB(1, arguments, this), this;
  }, B.fromTo = function(A, e, P, i) {
    return zB(2, arguments, this), this;
  }, B.set = function(A, e, P) {
    return e.duration = 0, e.parent = this, TB(e).repeatDelay || (e.repeat = 0), e.immediateRender = !!e.immediateRender, new K(A, e, EQ(this, P), 1), this;
  }, B.call = function(A, e, P) {
    return uQ(this, K.delayedCall(0, A, e), P);
  }, B.staggerTo = function(A, e, P, i, s, v, c) {
    return P.duration = e, P.stagger = P.stagger || i, P.onComplete = v, P.onCompleteParams = c, P.parent = this, new K(A, P, EQ(this, s)), this;
  }, B.staggerFrom = function(A, e, P, i, s, v, c) {
    return P.runBackwards = 1, TB(P).immediateRender = AQ(P.immediateRender), this.staggerTo(A, e, P, i, s, v, c);
  }, B.staggerFromTo = function(A, e, P, i, s, v, c, n) {
    return i.startAt = P, TB(i).immediateRender = AQ(i.immediateRender), this.staggerTo(A, e, i, s, v, c, n);
  }, B.render = function(A, e, P) {
    var i = this._time, s = this._dirty ? this.totalDuration() : this._tDur, v = this._dur, c = A <= 0 ? 0 : S(A), n = this._zTime < 0 != A < 0 && (this._initted || !v), T, z, w, o, a, E, l, u, U, k, O, D;
    if (this !== Y && c > s && A >= 0 && (c = s), c !== this._tTime || P || n) {
      if (i !== this._time && v && (c += this._time - i, A += this._time - i), T = c, U = this._start, u = this._ts, E = !u, n && (v || (i = this._zTime), (A || !e) && (this._zTime = A)), this._repeat) {
        if (O = this._yoyo, a = v + this._rDelay, this._repeat < -1 && A < 0)
          return this.totalTime(a * 100 + A, e, P);
        if (T = S(c % a), c === s ? (o = this._repeat, T = v) : (k = S(c / a), o = ~~k, o && o === k && (T = v, o--), T > v && (T = v)), k = AB(this._tTime, a), !i && this._tTime && k !== o && this._tTime - k * a - this._dur <= 0 && (k = o), O && o & 1 && (T = v - T, D = 1), o !== k && !this._lock) {
          var N = O && k & 1, d = N === (O && o & 1);
          if (o < k && (N = !N), i = N ? 0 : c % v ? v : c, this._lock = 1, this.render(i || (D ? 0 : S(o * a)), e, !v)._lock = 0, this._tTime = c, !e && this.parent && oQ(this, "onRepeat"), this.vars.repeatRefresh && !D && (this.invalidate()._lock = 1, k = o), i && i !== this._time || E !== !this._ts || this.vars.onRepeat && !this.parent && !this._act)
            return this;
          if (v = this._dur, s = this._tDur, d && (this._lock = 2, i = N ? v : -1e-4, this.render(i, !0), this.vars.repeatRefresh && !D && this.invalidate()), this._lock = 0, !this._ts && !E)
            return this;
        }
      }
      if (this._hasPause && !this._forcing && this._lock < 2 && (l = UP(this, S(i), S(T)), l && (c -= T - (T = l._start))), this._tTime = c, this._time = T, this._act = !!u, this._initted || (this._onUpdate = this.vars.onUpdate, this._initted = 1, this._zTime = A, i = 0), !i && c && v && !e && !k && (oQ(this, "onStart"), this._tTime !== c))
        return this;
      if (T >= i && A >= 0)
        for (z = this._first; z; ) {
          if (w = z._next, (z._act || T >= z._start) && z._ts && l !== z) {
            if (z.parent !== this)
              return this.render(A, e, P);
            if (z.render(z._ts > 0 ? (T - z._start) * z._ts : (z._dirty ? z.totalDuration() : z._tDur) + (T - z._start) * z._ts, e, P), T !== this._time || !this._ts && !E) {
              l = 0, w && (c += this._zTime = -m);
              break;
            }
          }
          z = w;
        }
      else {
        z = this._last;
        for (var M = A < 0 ? A : T; z; ) {
          if (w = z._prev, (z._act || M <= z._end) && z._ts && l !== z) {
            if (z.parent !== this)
              return this.render(A, e, P);
            if (z.render(z._ts > 0 ? (M - z._start) * z._ts : (z._dirty ? z.totalDuration() : z._tDur) + (M - z._start) * z._ts, e, P || q && Ht(z)), T !== this._time || !this._ts && !E) {
              l = 0, w && (c += this._zTime = M ? -m : m);
              break;
            }
          }
          z = w;
        }
      }
      if (l && !e && (this.pause(), l.render(T >= i ? 0 : -m)._zTime = T >= i ? 1 : -1, this._ts))
        return this._start = U, yB(this), this.render(A, e, P);
      this._onUpdate && !e && oQ(this, "onUpdate", !0), (c === s && this._tTime >= this.totalDuration() || !c && i) && (U === this._start || Math.abs(u) !== Math.abs(this._ts)) && (this._lock || ((A || !v) && (c === s && this._ts > 0 || !c && this._ts < 0) && bQ(this, 1), !e && !(A < 0 && !i) && (c || i || !s) && (oQ(this, c === s && A >= 0 ? "onComplete" : "onReverseComplete", !0), this._prom && !(c < s && this.timeScale() > 0) && this._prom())));
    }
    return this;
  }, B.add = function(A, e) {
    var P = this;
    if (NQ(e) || (e = EQ(this, e, A)), !(A instanceof uB)) {
      if ($(A))
        return A.forEach(function(i) {
          return P.add(i, e);
        }), this;
      if (Z(A))
        return this.addLabel(A, e);
      if (J(A))
        A = K.delayedCall(0, A);
      else
        return this;
    }
    return this !== A ? uQ(this, A, e) : this;
  }, B.getChildren = function(A, e, P, i) {
    A === void 0 && (A = !0), e === void 0 && (e = !0), P === void 0 && (P = !0), i === void 0 && (i = -hQ);
    for (var s = [], v = this._first; v; )
      v._start >= i && (v instanceof K ? e && s.push(v) : (P && s.push(v), A && s.push.apply(s, v.getChildren(!0, e, P)))), v = v._next;
    return s;
  }, B.getById = function(A) {
    for (var e = this.getChildren(1, 1, 1), P = e.length; P--; )
      if (e[P].vars.id === A)
        return e[P];
  }, B.remove = function(A) {
    return Z(A) ? this.removeLabel(A) : J(A) ? this.killTweensOf(A) : (A.parent === this && xB(this, A), A === this._recent && (this._recent = this._last), VQ(this));
  }, B.totalTime = function(A, e) {
    return arguments.length ? (this._forcing = 1, !this._dp && this._ts && (this._start = S(nQ.time - (this._ts > 0 ? A / this._ts : (this.totalDuration() - A) / -this._ts))), r.prototype.totalTime.call(this, A, e), this._forcing = 0, this) : this._tTime;
  }, B.addLabel = function(A, e) {
    return this.labels[A] = EQ(this, e), this;
  }, B.removeLabel = function(A) {
    return delete this.labels[A], this;
  }, B.addPause = function(A, e, P) {
    var i = K.delayedCall(0, e || hB, P);
    return i.data = "isPause", this._hasPause = 1, uQ(this, i, EQ(this, A));
  }, B.removePause = function(A) {
    var e = this._first;
    for (A = EQ(this, A); e; )
      e._start === A && e.data === "isPause" && bQ(e), e = e._next;
  }, B.killTweensOf = function(A, e, P) {
    for (var i = this.getTweensOf(A, P), s = i.length; s--; )
      HQ !== i[s] && i[s].kill(A, e);
    return this;
  }, B.getTweensOf = function(A, e) {
    for (var P = [], i = lQ(A), s = this._first, v = NQ(e), c; s; )
      s instanceof K ? wP(s._targets, i) && (v ? (!HQ || s._initted && s._ts) && s.globalTime(0) <= e && s.globalTime(s.totalDuration()) > e : !e || s.isActive()) && P.push(s) : (c = s.getTweensOf(i, e)).length && P.push.apply(P, c), s = s._next;
    return P;
  }, B.tweenTo = function(A, e) {
    e = e || {};
    var P = this, i = EQ(P, A), s = e, v = s.startAt, c = s.onStart, n = s.onStartParams, T = s.immediateRender, z, w = K.to(P, aQ({
      ease: e.ease || "none",
      lazy: !1,
      immediateRender: !1,
      time: i,
      overwrite: "auto",
      duration: e.duration || Math.abs((i - (v && "time" in v ? v.time : P._time)) / P.timeScale()) || m,
      onStart: function() {
        if (P.pause(), !z) {
          var a = e.duration || Math.abs((i - (v && "time" in v ? v.time : P._time)) / P.timeScale());
          w._dur !== a && eB(w, a, 0, 1).render(w._time, !0, !0), z = 1;
        }
        c && c.apply(w, n || []);
      }
    }, e));
    return T ? w.render(0) : w;
  }, B.tweenFromTo = function(A, e, P) {
    return this.tweenTo(e, aQ({
      startAt: {
        time: EQ(this, A)
      }
    }, P));
  }, B.recent = function() {
    return this._recent;
  }, B.nextLabel = function(A) {
    return A === void 0 && (A = this._time), QA(this, EQ(this, A));
  }, B.previousLabel = function(A) {
    return A === void 0 && (A = this._time), QA(this, EQ(this, A), 1);
  }, B.currentLabel = function(A) {
    return arguments.length ? this.seek(A, !0) : this.previousLabel(this._time + m);
  }, B.shiftChildren = function(A, e, P) {
    P === void 0 && (P = 0);
    var i = this._first, s = this.labels, v;
    for (A = S(A); i; )
      i._start >= P && (i._start += A, i._end += A), i = i._next;
    if (e)
      for (v in s)
        s[v] >= P && (s[v] += A);
    return VQ(this);
  }, B.invalidate = function(A) {
    var e = this._first;
    for (this._lock = 0; e; )
      e.invalidate(A), e = e._next;
    return r.prototype.invalidate.call(this, A);
  }, B.clear = function(A) {
    A === void 0 && (A = !0);
    for (var e = this._first, P; e; )
      P = e._next, this.remove(e), e = P;
    return this._dp && (this._time = this._tTime = this._pTime = 0), A && (this.labels = {}), VQ(this);
  }, B.totalDuration = function(A) {
    var e = 0, P = this, i = P._last, s = hQ, v, c, n;
    if (arguments.length)
      return P.timeScale((P._repeat < 0 ? P.duration() : P.totalDuration()) / (P.reversed() ? -A : A));
    if (P._dirty) {
      for (n = P.parent; i; )
        v = i._prev, i._dirty && i.totalDuration(), c = i._start, c > s && P._sort && i._ts && !P._lock ? (P._lock = 1, uQ(P, i, c - i._delay, 1)._lock = 0) : s = c, c < 0 && i._ts && (e -= c, (!n && !P._dp || n && n.smoothChildTiming) && (P._start += S(c / P._ts), P._time -= c, P._tTime -= c), P.shiftChildren(-c, !1, -1 / 0), s = 0), i._end > e && i._ts && (e = i._end), i = v;
      eB(P, P === Y && P._time > e ? P._time : e, 1, 1), P._dirty = 0;
    }
    return P._tDur;
  }, Q.updateRoot = function(A) {
    if (Y._ts && (jA(Y, bB(A, Y)), MA = nQ.frame), nQ.frame >= Wt) {
      Wt += zQ.autoSleep || 120;
      var e = Y._first;
      if ((!e || !e._ts) && zQ.autoSleep && nQ._listeners.length < 2) {
        for (; e && !e._ts; )
          e = e._next;
        e || nQ.sleep();
      }
    }
  }, Q;
}(uB);
aQ(tQ.prototype, {
  _lock: 0,
  _hasPause: 0,
  _forcing: 0
});
var xP = function(Q, B, t, A, e, P, i) {
  var s = new PQ(this._pt, Q, B, 0, 1, Ae, null, e), v = 0, c = 0, n, T, z, w, o, a, E, l;
  for (s.b = t, s.e = A, t += "", A += "", (E = ~A.indexOf("random(")) && (A = lB(A)), P && (l = [t, A], P(l, Q, B), t = l[0], A = l[1]), T = t.match(JB) || []; n = JB.exec(A); )
    w = n[0], o = A.substring(v, n.index), z ? z = (z + 1) % 5 : o.substr(-5) === "rgba(" && (z = 1), w !== T[c++] && (a = parseFloat(T[c - 1]) || 0, s._pt = {
      _next: s._pt,
      p: o || c === 1 ? o : ",",
      //note: SVG spec allows omission of comma/space when a negative sign is wedged between two numbers, like 2.5-5.3 instead of 2.5,-5.3 but when tweening, the negative value may switch to positive, so we insert the comma just in case.
      s: a,
      c: w.charAt(1) === "=" ? $Q(a, w) - a : parseFloat(w) - a,
      m: z && z < 4 ? Math.round : 0
    }, v = JB.lastIndex);
  return s.c = v < A.length ? A.substring(v, A.length) : "", s.fp = i, (CA.test(A) || E) && (s.e = 0), this._pt = s, s;
}, jt = function(Q, B, t, A, e, P, i, s, v, c) {
  J(A) && (A = A(e || 0, Q, P));
  var n = Q[B], T = t !== "get" ? t : J(n) ? v ? Q[B.indexOf("set") || !J(Q["get" + B.substr(3)]) ? B : "get" + B.substr(3)](v) : Q[B]() : n, z = J(n) ? v ? VP : Be : ft, w;
  if (Z(A) && (~A.indexOf("random(") && (A = lB(A)), A.charAt(1) === "=" && (w = $Q(T, A) + (_(T) || 0), (w || w === 0) && (A = w))), !c || T !== A || wt)
    return !isNaN(T * A) && A !== "" ? (w = new PQ(this._pt, Q, B, +T || 0, A - (T || 0), typeof n == "boolean" ? RP : te, 0, z), v && (w.fp = v), i && w.modifier(i, this, Q), this._pt = w) : (!n && !(B in Q) && Nt(B, A), xP.call(this, Q, B, T, A, z, s || zQ.stringFilter, v));
}, yP = function(Q, B, t, A, e) {
  if (J(Q) && (Q = wB(Q, e, B, t, A)), !DQ(Q) || Q.style && Q.nodeType || $(Q) || DA(Q))
    return Z(Q) ? wB(Q, e, B, t, A) : Q;
  var P = {}, i;
  for (i in Q)
    P[i] = wB(Q[i], e, B, t, A);
  return P;
}, _A = function(Q, B, t, A, e, P) {
  var i, s, v, c;
  if (cQ[Q] && (i = new cQ[Q]()).init(e, i.rawVars ? B[Q] : yP(B[Q], A, e, P, t), t, A, P) !== !1 && (t._pt = s = new PQ(t._pt, e, Q, 0, 1, i.render, i, 0, i.priority), t !== _Q))
    for (v = t._ptLookup[t._targets.indexOf(e)], c = i._props.length; c--; )
      v[i._props[c]] = s;
  return i;
}, HQ, wt, Ft = function r(Q, B, t) {
  var A = Q.vars, e = A.ease, P = A.startAt, i = A.immediateRender, s = A.lazy, v = A.onUpdate, c = A.runBackwards, n = A.yoyoEase, T = A.keyframes, z = A.autoRevert, w = Q._dur, o = Q._startAt, a = Q._targets, E = Q.parent, l = E && E.data === "nested" ? E.vars.targets : a, u = Q._overwrite === "auto" && !Ut, U = Q.timeline, k = A.easeReverse || n, O, D, N, d, M, b, G, C, j, F, H, f, I;
  if (U && (!T || !e) && (e = "none"), Q._ease = JQ(e, aB.ease), Q._rEase = k && (JQ(k) || Q._ease), Q._from = !U && !!A.runBackwards, Q._from && (Q.ratio = 1), !U || T && !A.stagger) {
    if (C = a[0] ? YQ(a[0]).harness : 0, f = C && A[C.prop], O = GB(A, Ot), o && (o._zTime < 0 && o.progress(1), B < 0 && c && i && !z ? o.render(-1, !0) : o.revert(c && w ? LB : TP), o._lazy = 0), P) {
      if (bQ(Q._startAt = K.set(a, aQ({
        data: "isStart",
        overwrite: !1,
        parent: E,
        immediateRender: !0,
        lazy: !o && AQ(s),
        startAt: null,
        delay: 0,
        onUpdate: v && function() {
          return oQ(Q, "onUpdate");
        },
        stagger: 0
      }, P))), Q._startAt._dp = 0, Q._startAt._sat = Q, B < 0 && (q || !i && !z) && Q._startAt.revert(LB), i && w && B <= 0 && t <= 0) {
        B && (Q._zTime = B);
        return;
      }
    } else if (c && w && !o) {
      if (B && (i = !1), N = aQ({
        overwrite: !1,
        data: "isFromStart",
        //we tag the tween with as "isFromStart" so that if [inside a plugin] we need to only do something at the very END of a tween, we have a way of identifying this tween as merely the one that's setting the beginning values for a "from()" tween. For example, clearProps in CSSPlugin should only get applied at the very END of a tween and without this tag, from(...{height:100, clearProps:"height", delay:1}) would wipe the height at the beginning of the tween and after 1 second, it'd kick back in.
        lazy: i && !o && AQ(s),
        immediateRender: i,
        //zero-duration tweens render immediately by default, but if we're not specifically instructed to render this tween immediately, we should skip this and merely _init() to record the starting values (rendering them immediately would push them to completion which is wasteful in that case - we'd have to render(-1) immediately after)
        stagger: 0,
        parent: E
        //ensures that nested tweens that had a stagger are handled properly, like gsap.from(".class", {y: gsap.utils.wrap([-100,100]), stagger: 0.5})
      }, O), f && (N[C.prop] = f), bQ(Q._startAt = K.set(a, N)), Q._startAt._dp = 0, Q._startAt._sat = Q, B < 0 && (q ? Q._startAt.revert(LB) : Q._startAt.render(-1, !0)), Q._zTime = B, !i)
        r(Q._startAt, m, m);
      else if (!B)
        return;
    }
    for (Q._pt = Q._ptCache = 0, s = w && AQ(s) || s && !w, D = 0; D < a.length; D++) {
      if (M = a[D], G = M._gsap || Mt(a)[D]._gsap, Q._ptLookup[D] = F = {}, vt[G.id] && fQ.length && fB(), H = l === a ? D : l.indexOf(M), C && (j = new C()).init(M, f || O, Q, H, l) !== !1 && (Q._pt = d = new PQ(Q._pt, M, j.name, 0, 1, j.render, j, 0, j.priority), j._props.forEach(function(R) {
        F[R] = d;
      }), j.priority && (b = 1)), !C || f)
        for (N in O)
          cQ[N] && (j = _A(N, O, Q, H, M, l)) ? j.priority && (b = 1) : F[N] = d = jt.call(Q, M, N, "get", O[N], H, l, 0, A.stringFilter);
      Q._op && Q._op[D] && Q.kill(M, Q._op[D]), u && Q._pt && (HQ = Q, Y.killTweensOf(M, F, Q.globalTime(B)), I = !Q.parent, HQ = 0), Q._pt && s && (vt[G.id] = 1);
    }
    b && ee(Q), Q._onInit && Q._onInit(Q);
  }
  Q._onUpdate = v, Q._initted = (!Q._op || Q._pt) && !I, T && B <= 0 && U.render(hQ, !0, !0);
}, IP = function(Q, B, t, A, e, P, i, s) {
  var v = (Q._pt && Q._ptCache || (Q._ptCache = {}))[B], c, n, T, z;
  if (!v)
    for (v = Q._ptCache[B] = [], T = Q._ptLookup, z = Q._targets.length; z--; ) {
      if (c = T[z][B], c && c.d && c.d._pt)
        for (c = c.d._pt; c && c.p !== B && c.fp !== B; )
          c = c._next;
      if (!c)
        return wt = 1, Q.vars[B] = "+=0", Ft(Q, i), wt = 0, s ? EB(B + " not eligible for reset. Try splitting into individual properties") : 1;
      v.push(c);
    }
  for (z = v.length; z--; )
    n = v[z], c = n._pt || n, c.s = (A || A === 0) && !e ? A : c.s + (A || 0) + P * c.c, c.c = t - c.s, n.e && (n.e = X(t) + _(n.e)), n.b && (n.b = c.s + _(n.b));
}, SP = function(Q, B) {
  var t = Q[0] ? YQ(Q[0]).harness : 0, A = t && t.aliases, e, P, i, s;
  if (!A)
    return B;
  e = tB({}, B);
  for (P in A)
    if (P in e)
      for (s = A[P].split(","), i = s.length; i--; )
        e[s[i]] = e[P];
  return e;
}, YP = function(Q, B, t, A) {
  var e = B.ease || A || "power1.inOut", P, i;
  if ($(B))
    i = t[Q] || (t[Q] = []), B.forEach(function(s, v) {
      return i.push({
        t: v / (B.length - 1) * 100,
        v: s,
        e
      });
    });
  else
    for (P in B)
      i = t[P] || (t[P] = []), P === "ease" || i.push({
        t: parseFloat(Q),
        v: B[P],
        e
      });
}, wB = function(Q, B, t, A, e) {
  return J(Q) ? Q.call(B, t, A, e) : Z(Q) && ~Q.indexOf("random(") ? lB(Q) : Q;
}, $A = Lt + "repeat,repeatDelay,yoyo,repeatRefresh,yoyoEase,easeReverse,autoRevert", Qe = {};
eQ($A + ",id,stagger,delay,duration,paused,scrollTrigger", function(r) {
  return Qe[r] = 1;
});
var K = /* @__PURE__ */ function(r) {
  uA(Q, r);
  function Q(t, A, e, P) {
    var i;
    typeof A == "number" && (e.duration = A, A = e, e = null), i = r.call(this, P ? A : TB(A)) || this;
    var s = i.vars, v = s.duration, c = s.delay, n = s.immediateRender, T = s.stagger, z = s.overwrite, w = s.keyframes, o = s.defaults, a = s.scrollTrigger, E = A.parent || Y, l = ($(t) || DA(t) ? NQ(t[0]) : "length" in A) ? [t] : lQ(t), u, U, k, O, D, N, d, M;
    if (i._targets = l.length ? Mt(l) : EB("GSAP target " + t + " not found. https://gsap.com", !zQ.nullTargetWarn) || [], i._ptLookup = [], i._overwrite = z, w || T || OB(v) || OB(c)) {
      A = i.vars;
      var b = A.easeReverse || A.yoyoEase;
      if (u = i.timeline = new tQ({
        data: "nested",
        defaults: o || {},
        targets: E && E.data === "nested" ? E.vars.targets : l
      }), u.kill(), u.parent = u._dp = dQ(i), u._start = 0, T || OB(v) || OB(c)) {
        if (O = l.length, d = T && yA(T), DQ(T))
          for (D in T)
            ~$A.indexOf(D) && (M || (M = {}), M[D] = T[D]);
        for (U = 0; U < O; U++)
          k = GB(A, Qe), k.stagger = 0, b && (k.easeReverse = b), M && tB(k, M), N = l[U], k.duration = +wB(v, dQ(i), U, N, l), k.delay = (+wB(c, dQ(i), U, N, l) || 0) - i._delay, !T && O === 1 && k.delay && (i._delay = c = k.delay, i._start += c, k.delay = 0), u.to(N, k, d ? d(U, N, l) : 0), u._ease = g.none;
        u.duration() ? v = c = 0 : i.timeline = 0;
      } else if (w) {
        TB(aQ(u.vars.defaults, {
          ease: "none"
        })), u._ease = JQ(w.ease || A.ease || "none");
        var G = 0, C, j, F;
        if ($(w))
          w.forEach(function(H) {
            return u.to(l, H, ">");
          }), u.duration();
        else {
          k = {};
          for (D in w)
            D === "ease" || D === "easeEach" || YP(D, w[D], k, w.easeEach);
          for (D in k)
            for (C = k[D].sort(function(H, f) {
              return H.t - f.t;
            }), G = 0, U = 0; U < C.length; U++)
              j = C[U], F = {
                ease: j.e,
                duration: (j.t - (U ? C[U - 1].t : 0)) / 100 * v
              }, F[D] = j.v, u.to(l, F, G), G += F.duration;
          u.duration() < v && u.to({}, {
            duration: v - u.duration()
          });
        }
      }
      v || i.duration(v = u.duration());
    } else
      i.timeline = 0;
    return z === !0 && !Ut && (HQ = dQ(i), Y.killTweensOf(l), HQ = 0), uQ(E, dQ(i), e), A.reversed && i.reverse(), A.paused && i.paused(!0), (n || !v && !w && i._start === S(E._time) && AQ(n) && lP(dQ(i)) && E.data !== "nested") && (i._tTime = -m, i.render(Math.max(0, -c) || 0)), a && gA(dQ(i), a), i;
  }
  var B = Q.prototype;
  return B.render = function(A, e, P) {
    var i = this._time, s = this._tDur, v = this._dur, c = A < 0, n = A > s - m && !c ? s : A < m ? 0 : A, T, z, w, o, a, E, l, u;
    if (!v)
      uP(this, A, e, P);
    else if (n !== this._tTime || !A || P || !this._initted && this._tTime || this._startAt && this._zTime < 0 !== c || this._lazy) {
      if (T = n, u = this.timeline, this._repeat) {
        if (o = v + this._rDelay, this._repeat < -1 && c)
          return this.totalTime(o * 100 + A, e, P);
        if (T = S(n % o), n === s ? (w = this._repeat, T = v) : (a = S(n / o), w = ~~a, w && w === a ? (T = v, w--) : T > v && (T = v)), E = this._yoyo && w & 1, E && (T = v - T), a = AB(this._tTime, o), T === i && !P && this._initted && w === a)
          return this._tTime = n, this;
        w !== a && this.vars.repeatRefresh && !E && !this._lock && T !== o && this._initted && (this._lock = P = 1, this.render(S(o * w), !0).invalidate()._lock = 0);
      }
      if (!this._initted) {
        if (pA(this, c ? A : T, P, e, n))
          return this._tTime = 0, this;
        if (i !== this._time && !(P && this.vars.repeatRefresh && w !== a))
          return this;
        if (v !== this._dur)
          return this.render(A, e, P);
      }
      if (this._rEase) {
        var U = T < i;
        if (U !== this._inv) {
          var k = U ? i : v - i;
          this._inv = U, this._from && (this.ratio = 1 - this.ratio), this._invRatio = this.ratio, this._invTime = i, this._invRecip = k ? (U ? -1 : 1) / k : 0, this._invScale = U ? -this.ratio : 1 - this.ratio, this._invEase = U ? this._rEase : this._ease;
        }
        this.ratio = l = this._invRatio + this._invScale * this._invEase((T - this._invTime) * this._invRecip);
      } else
        this.ratio = l = this._ease(T / v);
      if (this._from && (this.ratio = l = 1 - l), this._tTime = n, this._time = T, !this._act && this._ts && (this._act = 1, this._lazy = 0), !i && n && !e && !a && (oQ(this, "onStart"), this._tTime !== n))
        return this;
      for (z = this._pt; z; )
        z.r(l, z.d), z = z._next;
      u && u.render(A < 0 ? A : u._dur * u._ease(T / this._dur), e, P) || this._startAt && (this._zTime = A), this._onUpdate && !e && (c && ct(this, A, e, P), oQ(this, "onUpdate")), this._repeat && w !== a && this.vars.onRepeat && !e && this.parent && oQ(this, "onRepeat"), (n === this._tDur || !n) && this._tTime === n && (c && !this._onUpdate && ct(this, A, !0, !0), (A || !v) && (n === this._tDur && this._ts > 0 || !n && this._ts < 0) && bQ(this, 1), !e && !(c && !i) && (n || i || E) && (oQ(this, n === s ? "onComplete" : "onReverseComplete", !0), this._prom && !(n < s && this.timeScale() > 0) && this._prom()));
    }
    return this;
  }, B.targets = function() {
    return this._targets;
  }, B.invalidate = function(A) {
    return (!A || !this.vars.runBackwards) && (this._startAt = 0), this._pt = this._op = this._onUpdate = this._lazy = this.ratio = 0, this._ptLookup = [], this.timeline && this.timeline.invalidate(A), r.prototype.invalidate.call(this, A);
  }, B.resetTo = function(A, e, P, i, s) {
    kB || nQ.wake(), this._ts || this.play();
    var v = Math.min(this._dur, (this._dp._time - this._start) * this._ts), c;
    return this._initted || Ft(this, v), c = this._ease(v / this._dur), IP(this, A, e, P, i, c, v, s) ? this.resetTo(A, e, P, i, 1) : (IB(this, 0), this.parent || GA(this._dp, this, "_first", "_last", this._dp._sort ? "_start" : 0), this.render(0));
  }, B.kill = function(A, e) {
    if (e === void 0 && (e = "all"), !A && (!e || e === "all"))
      return this._lazy = this._pt = 0, this.parent ? nB(this) : this.scrollTrigger && this.scrollTrigger.kill(!!q), this;
    if (this.timeline) {
      var P = this.timeline.totalDuration();
      return this.timeline.killTweensOf(A, e, HQ && HQ.vars.overwrite !== !0)._first || nB(this), this.parent && P !== this.timeline.totalDuration() && eB(this, this._dur * this.timeline._tDur / P, 0, 1), this;
    }
    var i = this._targets, s = A ? lQ(A) : i, v = this._ptLookup, c = this._pt, n, T, z, w, o, a, E;
    if ((!e || e === "all") && EP(i, s))
      return e === "all" && (this._pt = 0), nB(this);
    for (n = this._op = this._op || [], e !== "all" && (Z(e) && (o = {}, eQ(e, function(l) {
      return o[l] = 1;
    }), e = o), e = SP(i, e)), E = i.length; E--; )
      if (~s.indexOf(i[E])) {
        T = v[E], e === "all" ? (n[E] = e, w = T, z = {}) : (z = n[E] = n[E] || {}, w = e);
        for (o in w)
          a = T && T[o], a && ((!("kill" in a.d) || a.d.kill(o) === !0) && xB(this, a, "_pt"), delete T[o]), z !== "all" && (z[o] = 1);
      }
    return this._initted && !this._pt && c && nB(this), this;
  }, Q.to = function(A, e) {
    return new Q(A, e, arguments[2]);
  }, Q.from = function(A, e) {
    return zB(1, arguments);
  }, Q.delayedCall = function(A, e, P, i) {
    return new Q(e, 0, {
      immediateRender: !1,
      lazy: !1,
      overwrite: !1,
      delay: A,
      onComplete: e,
      onReverseComplete: e,
      onCompleteParams: P,
      onReverseCompleteParams: P,
      callbackScope: i
    });
  }, Q.fromTo = function(A, e, P) {
    return zB(2, arguments);
  }, Q.set = function(A, e) {
    return e.duration = 0, e.repeatDelay || (e.repeat = 0), new Q(A, e);
  }, Q.killTweensOf = function(A, e, P) {
    return Y.killTweensOf(A, e, P);
  }, Q;
}(uB);
aQ(K.prototype, {
  _targets: [],
  _lazy: 0,
  _startAt: 0,
  _op: 0,
  _onInit: 0
});
eQ("staggerTo,staggerFrom,staggerFromTo", function(r) {
  K[r] = function() {
    var Q = new tQ(), B = ot.call(arguments, 0);
    return B.splice(r === "staggerFromTo" ? 5 : 4, 0, 0), Q[r].apply(Q, B);
  };
});
var ft = function(Q, B, t) {
  return Q[B] = t;
}, Be = function(Q, B, t) {
  return Q[B](t);
}, VP = function(Q, B, t, A) {
  return Q[B](A.fp, t);
}, JP = function(Q, B, t) {
  return Q.setAttribute(B, t);
}, Gt = function(Q, B) {
  return J(Q[B]) ? Be : Dt(Q[B]) && Q.setAttribute ? JP : ft;
}, te = function(Q, B) {
  return B.set(B.t, B.p, Math.round((B.s + B.c * Q) * 1e6) / 1e6, B);
}, RP = function(Q, B) {
  return B.set(B.t, B.p, !!(B.s + B.c * Q), B);
}, Ae = function(Q, B) {
  var t = B._pt, A = "";
  if (!Q && B.b)
    A = B.b;
  else if (Q === 1 && B.e)
    A = B.e;
  else {
    for (; t; )
      A = t.p + (t.m ? t.m(t.s + t.c * Q) : Math.round((t.s + t.c * Q) * 1e4) / 1e4) + A, t = t._next;
    A += B.c;
  }
  B.set(B.t, B.p, A, B);
}, bt = function(Q, B) {
  for (var t = B._pt; t; )
    t.r(Q, t.d), t = t._next;
}, XP = function(Q, B, t, A) {
  for (var e = this._pt, P; e; )
    P = e._next, e.p === A && e.modifier(Q, B, t), e = P;
}, KP = function(Q) {
  for (var B = this._pt, t, A; B; )
    A = B._next, B.p === Q && !B.op || B.op === Q ? xB(this, B, "_pt") : B.dep || (t = 1), B = A;
  return !t;
}, ZP = function(Q, B, t, A) {
  A.mSet(Q, B, A.m.call(A.tween, t, A.mt), A);
}, ee = function(Q) {
  for (var B = Q._pt, t, A, e, P; B; ) {
    for (t = B._next, A = e; A && A.pr > B.pr; )
      A = A._next;
    (B._prev = A ? A._prev : P) ? B._prev._next = B : e = B, (B._next = A) ? A._prev = B : P = B, B = t;
  }
  Q._pt = e;
}, PQ = /* @__PURE__ */ function() {
  function r(B, t, A, e, P, i, s, v, c) {
    this.t = t, this.s = e, this.c = P, this.p = A, this.r = i || te, this.d = s || this, this.set = v || ft, this.pr = c || 0, this._next = B, B && (B._prev = this);
  }
  var Q = r.prototype;
  return Q.modifier = function(t, A, e) {
    this.mSet = this.mSet || this.set, this.set = ZP, this.m = t, this.mt = e, this.tween = A;
  }, r;
}();
eQ(Lt + "parent,duration,ease,delay,overwrite,runBackwards,startAt,yoyo,immediateRender,repeat,repeatDelay,data,paused,reversed,lazy,callbackScope,stringFilter,id,yoyoEase,stagger,inherit,repeatRefresh,keyframes,autoRevert,scrollTrigger,easeReverse", function(r) {
  return Ot[r] = 1;
});
wQ.TweenMax = wQ.TweenLite = K;
wQ.TimelineLite = wQ.TimelineMax = tQ;
Y = new tQ({
  sortChildren: !1,
  defaults: aB,
  autoRemoveChildren: !0,
  id: "root",
  smoothChildTiming: !0
});
zQ.stringFilter = ZA;
var RQ = [], HB = {}, WP = [], tA = 0, qP = 0, WB = function(Q) {
  return (HB[Q] || WP).map(function(B) {
    return B();
  });
}, at = function() {
  var Q = Date.now(), B = [];
  Q - tA > 2 && (WB("matchMediaInit"), RQ.forEach(function(t) {
    var A = t.queries, e = t.conditions, P, i, s, v;
    for (i in A)
      P = kQ.matchMedia(A[i]).matches, P && (s = 1), P !== e[i] && (e[i] = P, v = 1);
    v && (t.revert(), s && B.push(t));
  }), WB("matchMediaRevert"), B.forEach(function(t) {
    return t.onMatch(t, function(A) {
      return t.add(null, A);
    });
  }), tA = Q, WB("matchMedia"));
}, Pe = /* @__PURE__ */ function() {
  function r(B, t) {
    this.selector = t && Tt(t), this.data = [], this._r = [], this.isReverted = !1, this.id = qP++, B && this.add(B);
  }
  var Q = r.prototype;
  return Q.add = function(t, A, e) {
    J(t) && (e = A, A = t, t = J);
    var P = this, i = function() {
      var v = y, c = P.selector, n;
      return v && v !== P && v.data.push(P), e && (P.selector = Tt(e)), y = P, n = A.apply(P, arguments), J(n) && P._r.push(n), y = v, P.selector = c, P.isReverted = !1, n;
    };
    return P.last = i, t === J ? i(P, function(s) {
      return P.add(null, s);
    }) : t ? P[t] = i : i;
  }, Q.ignore = function(t) {
    var A = y;
    y = null, t(this), y = A;
  }, Q.getTweens = function() {
    var t = [];
    return this.data.forEach(function(A) {
      return A instanceof r ? t.push.apply(t, A.getTweens()) : A instanceof K && !(A.parent && A.parent.data === "nested") && t.push(A);
    }), t;
  }, Q.clear = function() {
    this._r.length = this.data.length = 0;
  }, Q.kill = function(t, A) {
    var e = this;
    if (t ? function() {
      for (var i = e.getTweens(), s = e.data.length, v; s--; )
        v = e.data[s], v.data === "isFlip" && (v.revert(), v.getChildren(!0, !0, !1).forEach(function(c) {
          return i.splice(i.indexOf(c), 1);
        }));
      for (i.map(function(c) {
        return {
          g: c._dur || c._delay || c._sat && !c._sat.vars.immediateRender ? c.globalTime(0) : -1 / 0,
          t: c
        };
      }).sort(function(c, n) {
        return n.g - c.g || -1 / 0;
      }).forEach(function(c) {
        return c.t.revert(t);
      }), s = e.data.length; s--; )
        v = e.data[s], v instanceof tQ ? v.data !== "nested" && (v.scrollTrigger && v.scrollTrigger.revert(), v.kill()) : !(v instanceof K) && v.revert && v.revert(t);
      e._r.forEach(function(c) {
        return c(t, e);
      }), e.isReverted = !0;
    }() : this.data.forEach(function(i) {
      return i.kill && i.kill();
    }), this.clear(), A)
      for (var P = RQ.length; P--; )
        RQ[P].id === this.id && RQ.splice(P, 1);
  }, Q.revert = function(t) {
    this.kill(t || {});
  }, r;
}(), _P = /* @__PURE__ */ function() {
  function r(B) {
    this.contexts = [], this.scope = B, y && y.data.push(this);
  }
  var Q = r.prototype;
  return Q.add = function(t, A, e) {
    DQ(t) || (t = {
      matches: t
    });
    var P = new Pe(0, e || this.scope), i = P.conditions = {}, s, v, c;
    y && !P.selector && (P.selector = y.selector), this.contexts.push(P), A = P.add("onMatch", A), P.queries = t;
    for (v in t)
      v === "all" ? c = 1 : (s = kQ.matchMedia(t[v]), s && (RQ.indexOf(P) < 0 && RQ.push(P), (i[v] = s.matches) && (c = 1), s.addListener ? s.addListener(at) : s.addEventListener("change", at)));
    return c && A(P, function(n) {
      return P.add(null, n);
    }), this;
  }, Q.revert = function(t) {
    this.kill(t || {});
  }, Q.kill = function(t) {
    this.contexts.forEach(function(A) {
      return A.kill(t, !0);
    });
  }, r;
}(), gB = {
  registerPlugin: function() {
    for (var Q = arguments.length, B = new Array(Q), t = 0; t < Q; t++)
      B[t] = arguments[t];
    B.forEach(function(A) {
      return RA(A);
    });
  },
  timeline: function(Q) {
    return new tQ(Q);
  },
  getTweensOf: function(Q, B) {
    return Y.getTweensOf(Q, B);
  },
  getProperty: function(Q, B, t, A) {
    Z(Q) && (Q = lQ(Q)[0]);
    var e = YQ(Q || {}).get, P = t ? fA : FA;
    return t === "native" && (t = ""), Q && (B ? P((cQ[B] && cQ[B].get || e)(Q, B, t, A)) : function(i, s, v) {
      return P((cQ[i] && cQ[i].get || e)(Q, i, s, v));
    });
  },
  quickSetter: function(Q, B, t) {
    if (Q = lQ(Q), Q.length > 1) {
      var A = Q.map(function(c) {
        return rQ.quickSetter(c, B, t);
      }), e = A.length;
      return function(c) {
        for (var n = e; n--; )
          A[n](c);
      };
    }
    Q = Q[0] || {};
    var P = cQ[B], i = YQ(Q), s = i.harness && (i.harness.aliases || {})[B] || B, v = P ? function(c) {
      var n = new P();
      _Q._pt = 0, n.init(Q, t ? c + t : c, _Q, 0, [Q]), n.render(1, n), _Q._pt && bt(1, _Q);
    } : i.set(Q, s);
    return P ? v : function(c) {
      return v(Q, s, t ? c + t : c, i, 1);
    };
  },
  quickTo: function(Q, B, t) {
    var A, e = rQ.to(Q, aQ((A = {}, A[B] = "+=0.1", A.paused = !0, A.stagger = 0, A), t || {})), P = function(s, v, c) {
      return e.resetTo(B, s, v, c);
    };
    return P.tween = e, P;
  },
  isTweening: function(Q) {
    return Y.getTweensOf(Q, !0).length > 0;
  },
  defaults: function(Q) {
    return Q && Q.ease && (Q.ease = JQ(Q.ease, aB.ease)), qt(aB, Q || {});
  },
  config: function(Q) {
    return qt(zQ, Q || {});
  },
  registerEffect: function(Q) {
    var B = Q.name, t = Q.effect, A = Q.plugins, e = Q.defaults, P = Q.extendTimeline;
    (A || "").split(",").forEach(function(i) {
      return i && !cQ[i] && !wQ[i] && EB(B + " effect requires " + i + " plugin.");
    }), RB[B] = function(i, s, v) {
      return t(lQ(i), aQ(s || {}, e), v);
    }, P && (tQ.prototype[B] = function(i, s, v) {
      return this.add(RB[B](i, DQ(s) ? s : (v = s) && {}, this), v);
    });
  },
  registerEase: function(Q, B) {
    g[Q] = JQ(B);
  },
  parseEase: function(Q, B) {
    return arguments.length ? JQ(Q, B) : g;
  },
  getById: function(Q) {
    return Y.getById(Q);
  },
  exportRoot: function(Q, B) {
    Q === void 0 && (Q = {});
    var t = new tQ(Q), A, e;
    for (t.smoothChildTiming = AQ(Q.smoothChildTiming), Y.remove(t), t._dp = 0, t._time = t._tTime = Y._time, A = Y._first; A; )
      e = A._next, (B || !(!A._dur && A instanceof K && A.vars.onComplete === A._targets[0])) && uQ(t, A, A._start - A._delay), A = e;
    return uQ(Y, t, 0), t;
  },
  context: function(Q, B) {
    return Q ? new Pe(Q, B) : y;
  },
  matchMedia: function(Q) {
    return new _P(Q);
  },
  matchMediaRefresh: function() {
    return RQ.forEach(function(Q) {
      var B = Q.conditions, t, A;
      for (A in B)
        B[A] && (B[A] = !1, t = 1);
      t && Q.revert();
    }) || at();
  },
  addEventListener: function(Q, B) {
    var t = HB[Q] || (HB[Q] = []);
    ~t.indexOf(B) || t.push(B);
  },
  removeEventListener: function(Q, B) {
    var t = HB[Q], A = t && t.indexOf(B);
    A >= 0 && t.splice(A, 1);
  },
  utils: {
    wrap: MP,
    wrapYoyo: HP,
    distribute: yA,
    random: SA,
    snap: IA,
    normalize: LP,
    getUnit: _,
    clamp: dP,
    splitColor: XA,
    toArray: lQ,
    selector: Tt,
    mapRange: VA,
    pipe: NP,
    unitize: OP,
    interpolate: jP,
    shuffle: xA
  },
  install: OA,
  effects: RB,
  ticker: nQ,
  updateRoot: tQ.updateRoot,
  plugins: cQ,
  globalTimeline: Y,
  core: {
    PropTween: PQ,
    globals: LA,
    Tween: K,
    Timeline: tQ,
    Animation: uB,
    getCache: YQ,
    _removeLinkedListItem: xB,
    reverting: function() {
      return q;
    },
    context: function(Q) {
      return Q && y && (y.data.push(Q), Q._ctx = y), y;
    },
    suppressOverwrites: function(Q) {
      return Ut = Q;
    }
  }
};
eQ("to,from,fromTo,delayedCall,set,killTweensOf", function(r) {
  return gB[r] = K[r];
});
nQ.add(tQ.updateRoot);
_Q = gB.to({}, {
  duration: 0
});
var $P = function(Q, B) {
  for (var t = Q._pt; t && t.p !== B && t.op !== B && t.fp !== B; )
    t = t._next;
  return t;
}, Qi = function(Q, B) {
  var t = Q._targets, A, e, P;
  for (A in B)
    for (e = t.length; e--; )
      P = Q._ptLookup[e][A], P && (P = P.d) && (P._pt && (P = $P(P, A)), P && P.modifier && P.modifier(B[A], Q, t[e], A));
}, qB = function(Q, B) {
  return {
    name: Q,
    headless: 1,
    rawVars: 1,
    //don't pre-process function-based values or "random()" strings.
    init: function(A, e, P) {
      P._onInit = function(i) {
        var s, v;
        if (Z(e) && (s = {}, eQ(e, function(c) {
          return s[c] = 1;
        }), e = s), B) {
          s = {};
          for (v in e)
            s[v] = B(e[v]);
          e = s;
        }
        Qi(i, e);
      };
    }
  };
}, rQ = gB.registerPlugin({
  name: "attr",
  init: function(Q, B, t, A, e) {
    var P, i, s;
    this.tween = t;
    for (P in B)
      s = Q.getAttribute(P) || "", i = this.add(Q, "setAttribute", (s || 0) + "", B[P], A, e, 0, 0, P), i.op = P, i.b = s, this._props.push(P);
  },
  render: function(Q, B) {
    for (var t = B._pt; t; )
      q ? t.set(t.t, t.p, t.b, t) : t.r(Q, t.d), t = t._next;
  }
}, {
  name: "endArray",
  headless: 1,
  init: function(Q, B) {
    for (var t = B.length; t--; )
      this.add(Q, t, Q[t] || 0, B[t], 0, 0, 0, 0, 0, 1);
  }
}, qB("roundProps", zt), qB("modifiers"), qB("snap", IA)) || gB;
K.version = tQ.version = rQ.version = "3.15.0";
NA = 1;
dt() && PB();
g.Power0;
g.Power1;
g.Power2;
g.Power3;
g.Power4;
g.Linear;
g.Quad;
g.Cubic;
g.Quart;
g.Quint;
g.Strong;
g.Elastic;
g.Back;
g.SteppedEase;
g.Bounce;
g.Sine;
g.Expo;
g.Circ;
/*!
 * CSSPlugin 3.15.0
 * https://gsap.com
 *
 * Copyright 2008-2026, GreenSock. All rights reserved.
 * Subject to the terms at https://gsap.com/standard-license
 * @author: Jack Doyle, jack@greensock.com
*/
var AA, jQ, QB, gt, SQ, eA, pt, Bi = function() {
  return typeof window < "u";
}, OQ = {}, IQ = 180 / Math.PI, BB = Math.PI / 180, KQ = Math.atan2, PA = 1e8, mt = /([A-Z])/g, ti = /(left|right|width|margin|padding|x)/i, Ai = /[\s,\(]\S/, UQ = {
  autoAlpha: "opacity,visibility",
  scale: "scaleX,scaleY",
  alpha: "opacity"
}, Et = function(Q, B) {
  return B.set(B.t, B.p, Math.round((B.s + B.c * Q) * 1e4) / 1e4 + B.u, B);
}, ei = function(Q, B) {
  return B.set(B.t, B.p, Q === 1 ? B.e : Math.round((B.s + B.c * Q) * 1e4) / 1e4 + B.u, B);
}, Pi = function(Q, B) {
  return B.set(B.t, B.p, Q ? Math.round((B.s + B.c * Q) * 1e4) / 1e4 + B.u : B.b, B);
}, ii = function(Q, B) {
  return B.set(B.t, B.p, Q === 1 ? B.e : Q ? Math.round((B.s + B.c * Q) * 1e4) / 1e4 + B.u : B.b, B);
}, ri = function(Q, B) {
  var t = B.s + B.c * Q;
  B.set(B.t, B.p, ~~(t + (t < 0 ? -0.5 : 0.5)) + B.u, B);
}, ie = function(Q, B) {
  return B.set(B.t, B.p, Q ? B.e : B.b, B);
}, re = function(Q, B) {
  return B.set(B.t, B.p, Q !== 1 ? B.b : B.e, B);
}, si = function(Q, B, t) {
  return Q.style[B] = t;
}, vi = function(Q, B, t) {
  return Q.style.setProperty(B, t);
}, ci = function(Q, B, t) {
  return Q._gsap[B] = t;
}, ni = function(Q, B, t) {
  return Q._gsap.scaleX = Q._gsap.scaleY = t;
}, oi = function(Q, B, t, A, e) {
  var P = Q._gsap;
  P.scaleX = P.scaleY = t, P.renderTransform(e, P);
}, Ti = function(Q, B, t, A, e) {
  var P = Q._gsap;
  P[B] = t, P.renderTransform(e, P);
}, V = "transform", iQ = V + "Origin", zi = function r(Q, B) {
  var t = this, A = this.target, e = A.style, P = A._gsap;
  if (Q in OQ && e) {
    if (this.tfm = this.tfm || {}, Q !== "transform")
      Q = UQ[Q] || Q, ~Q.indexOf(",") ? Q.split(",").forEach(function(i) {
        return t.tfm[i] = CQ(A, i);
      }) : this.tfm[Q] = P.x ? P[Q] : CQ(A, Q), Q === iQ && (this.tfm.zOrigin = P.zOrigin);
    else
      return UQ.transform.split(",").forEach(function(i) {
        return r.call(t, i, B);
      });
    if (this.props.indexOf(V) >= 0)
      return;
    P.svg && (this.svgo = A.getAttribute("data-svg-origin"), this.props.push(iQ, B, "")), Q = V;
  }
  (e || B) && this.props.push(Q, B, e[Q]);
}, se = function(Q) {
  Q.translate && (Q.removeProperty("translate"), Q.removeProperty("scale"), Q.removeProperty("rotate"));
}, wi = function() {
  var Q = this.props, B = this.target, t = B.style, A = B._gsap, e, P;
  for (e = 0; e < Q.length; e += 3)
    Q[e + 1] ? Q[e + 1] === 2 ? B[Q[e]](Q[e + 2]) : B[Q[e]] = Q[e + 2] : Q[e + 2] ? t[Q[e]] = Q[e + 2] : t.removeProperty(Q[e].substr(0, 2) === "--" ? Q[e] : Q[e].replace(mt, "-$1").toLowerCase());
  if (this.tfm) {
    for (P in this.tfm)
      A[P] = this.tfm[P];
    A.svg && (A.renderTransform(), B.setAttribute("data-svg-origin", this.svgo || "")), e = pt(), (!e || !e.isStart) && !t[V] && (se(t), A.zOrigin && t[iQ] && (t[iQ] += " " + A.zOrigin + "px", A.zOrigin = 0, A.renderTransform()), A.uncache = 1);
  }
}, ve = function(Q, B) {
  var t = {
    target: Q,
    props: [],
    revert: wi,
    save: zi
  };
  return Q._gsap || rQ.core.getCache(Q), B && Q.style && Q.nodeType && B.split(",").forEach(function(A) {
    return t.save(A);
  }), t;
}, ce, ht = function(Q, B) {
  var t = jQ.createElementNS ? jQ.createElementNS((B || "http://www.w3.org/1999/xhtml").replace(/^https/, "http"), Q) : jQ.createElement(Q);
  return t && t.style ? t : jQ.createElement(Q);
}, TQ = function r(Q, B, t) {
  var A = getComputedStyle(Q);
  return A[B] || A.getPropertyValue(B.replace(mt, "-$1").toLowerCase()) || A.getPropertyValue(B) || !t && r(Q, iB(B) || B, 1) || "";
}, iA = "O,Moz,ms,Ms,Webkit".split(","), iB = function(Q, B, t) {
  var A = B || SQ, e = A.style, P = 5;
  if (Q in e && !t)
    return Q;
  for (Q = Q.charAt(0).toUpperCase() + Q.substr(1); P-- && !(iA[P] + Q in e); )
    ;
  return P < 0 ? null : (P === 3 ? "ms" : P >= 0 ? iA[P] : "") + Q;
}, lt = function() {
  Bi() && window.document && (AA = window, jQ = AA.document, QB = jQ.documentElement, SQ = ht("div") || {
    style: {}
  }, ht("div"), V = iB(V), iQ = V + "Origin", SQ.style.cssText = "border-width:0;line-height:0;position:absolute;padding:0", ce = !!iB("perspective"), pt = rQ.core.reverting, gt = 1);
}, rA = function(Q) {
  var B = Q.ownerSVGElement, t = ht("svg", B && B.getAttribute("xmlns") || "http://www.w3.org/2000/svg"), A = Q.cloneNode(!0), e;
  A.style.display = "block", t.appendChild(A), QB.appendChild(t);
  try {
    e = A.getBBox();
  } catch {
  }
  return t.removeChild(A), QB.removeChild(t), e;
}, sA = function(Q, B) {
  for (var t = B.length; t--; )
    if (Q.hasAttribute(B[t]))
      return Q.getAttribute(B[t]);
}, ne = function(Q) {
  var B, t;
  try {
    B = Q.getBBox();
  } catch {
    B = rA(Q), t = 1;
  }
  return B && (B.width || B.height) || t || (B = rA(Q)), B && !B.width && !B.x && !B.y ? {
    x: +sA(Q, ["x", "cx", "x1"]) || 0,
    y: +sA(Q, ["y", "cy", "y1"]) || 0,
    width: 0,
    height: 0
  } : B;
}, oe = function(Q) {
  return !!(Q.getCTM && (!Q.parentNode || Q.ownerSVGElement) && ne(Q));
}, gQ = function(Q, B) {
  if (B) {
    var t = Q.style, A;
    B in OQ && B !== iQ && (B = V), t.removeProperty ? (A = B.substr(0, 2), (A === "ms" || B.substr(0, 6) === "webkit") && (B = "-" + B), t.removeProperty(A === "--" ? B : B.replace(mt, "-$1").toLowerCase())) : t.removeAttribute(B);
  }
}, FQ = function(Q, B, t, A, e, P) {
  var i = new PQ(Q._pt, B, t, 0, 1, P ? re : ie);
  return Q._pt = i, i.b = A, i.e = e, Q._props.push(t), i;
}, vA = {
  deg: 1,
  rad: 1,
  turn: 1
}, ai = {
  grid: 1,
  flex: 1
}, pQ = function r(Q, B, t, A) {
  var e = parseFloat(t) || 0, P = (t + "").trim().substr((e + "").length) || "px", i = SQ.style, s = ti.test(B), v = Q.tagName.toLowerCase() === "svg", c = (v ? "client" : "offset") + (s ? "Width" : "Height"), n = 100, T = A === "px", z = A === "%", w, o, a, E;
  if (A === P || !e || vA[A] || vA[P])
    return e;
  if (P !== "px" && !T && (e = r(Q, B, t, "px")), E = Q.getCTM && oe(Q), (z || P === "%") && (OQ[B] || ~B.indexOf("adius")))
    return w = E ? Q.getBBox()[s ? "width" : "height"] : Q[c], X(z ? e / w * n : e / 100 * w);
  if (i[s ? "width" : "height"] = n + (T ? P : A), o = A !== "rem" && ~B.indexOf("adius") || A === "em" && Q.appendChild && !v ? Q : Q.parentNode, E && (o = (Q.ownerSVGElement || {}).parentNode), (!o || o === jQ || !o.appendChild) && (o = jQ.body), a = o._gsap, a && z && a.width && s && a.time === nQ.time && !a.uncache)
    return X(e / a.width * n);
  if (z && (B === "height" || B === "width")) {
    var l = Q.style[B];
    Q.style[B] = n + A, w = Q[c], l ? Q.style[B] = l : gQ(Q, B);
  } else
    (z || P === "%") && !ai[TQ(o, "display")] && (i.position = TQ(Q, "position")), o === Q && (i.position = "static"), o.appendChild(SQ), w = SQ[c], o.removeChild(SQ), i.position = "absolute";
  return s && z && (a = YQ(o), a.time = nQ.time, a.width = o[c]), X(T ? w * e / n : w && e ? n / w * e : 0);
}, CQ = function(Q, B, t, A) {
  var e;
  return gt || lt(), B in UQ && B !== "transform" && (B = UQ[B], ~B.indexOf(",") && (B = B.split(",")[0])), OQ[B] && B !== "transform" ? (e = DB(Q, A), e = B !== "transformOrigin" ? e[B] : e.svg ? e.origin : mB(TQ(Q, iQ)) + " " + e.zOrigin + "px") : (e = Q.style[B], (!e || e === "auto" || A || ~(e + "").indexOf("calc(")) && (e = pB[B] && pB[B](Q, B, t) || TQ(Q, B) || HA(Q, B) || (B === "opacity" ? 1 : 0))), t && !~(e + "").trim().indexOf(" ") ? pQ(Q, B, e, t) + t : e;
}, Ei = function(Q, B, t, A) {
  if (!t || t === "none") {
    var e = iB(B, Q, 1), P = e && TQ(Q, e, 1);
    P && P !== t ? (B = e, t = P) : B === "borderColor" && (t = TQ(Q, "borderTopColor"));
  }
  var i = new PQ(this._pt, Q.style, B, 0, 1, Ae), s = 0, v = 0, c, n, T, z, w, o, a, E, l, u, U, k;
  if (i.b = t, i.e = A, t += "", A += "", A.substring(0, 6) === "var(--" && (A = TQ(Q, A.substring(4, A.indexOf(")")))), A === "auto" && (o = Q.style[B], Q.style[B] = A, A = TQ(Q, B) || A, o ? Q.style[B] = o : gQ(Q, B)), c = [t, A], ZA(c), t = c[0], A = c[1], T = t.match(qQ) || [], k = A.match(qQ) || [], k.length) {
    for (; n = qQ.exec(A); )
      a = n[0], l = A.substring(s, n.index), w ? w = (w + 1) % 5 : (l.substr(-5) === "rgba(" || l.substr(-5) === "hsla(") && (w = 1), a !== (o = T[v++] || "") && (z = parseFloat(o) || 0, U = o.substr((z + "").length), a.charAt(1) === "=" && (a = $Q(z, a) + U), E = parseFloat(a), u = a.substr((E + "").length), s = qQ.lastIndex - u.length, u || (u = u || zQ.units[B] || U, s === A.length && (A += u, i.e += u)), U !== u && (z = pQ(Q, B, o, u) || 0), i._pt = {
        _next: i._pt,
        p: l || v === 1 ? l : ",",
        //note: SVG spec allows omission of comma/space when a negative sign is wedged between two numbers, like 2.5-5.3 instead of 2.5,-5.3 but when tweening, the negative value may switch to positive, so we insert the comma just in case.
        s: z,
        c: E - z,
        m: w && w < 4 || B === "zIndex" ? Math.round : 0
      });
    i.c = s < A.length ? A.substring(s, A.length) : "";
  } else
    i.r = B === "display" && A === "none" ? re : ie;
  return CA.test(A) && (i.e = 0), this._pt = i, i;
}, cA = {
  top: "0%",
  bottom: "100%",
  left: "0%",
  right: "100%",
  center: "50%"
}, hi = function(Q) {
  var B = Q.split(" "), t = B[0], A = B[1] || "50%";
  return (t === "top" || t === "bottom" || A === "left" || A === "right") && (Q = t, t = A, A = Q), B[0] = cA[t] || t, B[1] = cA[A] || A, B.join(" ");
}, li = function(Q, B) {
  if (B.tween && B.tween._time === B.tween._dur) {
    var t = B.t, A = t.style, e = B.u, P = t._gsap, i, s, v;
    if (e === "all" || e === !0)
      A.cssText = "", s = 1;
    else
      for (e = e.split(","), v = e.length; --v > -1; )
        i = e[v], OQ[i] && (s = 1, i = i === "transformOrigin" ? iQ : V), gQ(t, i);
    s && (gQ(t, V), P && (P.svg && t.removeAttribute("transform"), A.scale = A.rotate = A.translate = "none", DB(t, 1), P.uncache = 1, se(A)));
  }
}, pB = {
  clearProps: function(Q, B, t, A, e) {
    if (e.data !== "isFromStart") {
      var P = Q._pt = new PQ(Q._pt, B, t, 0, 0, li);
      return P.u = A, P.pr = -10, P.tween = e, Q._props.push(t), 1;
    }
  }
  /* className feature (about 0.4kb gzipped).
  , className(plugin, target, property, endValue, tween) {
  	let _renderClassName = (ratio, data) => {
  			data.css.render(ratio, data.css);
  			if (!ratio || ratio === 1) {
  				let inline = data.rmv,
  					target = data.t,
  					p;
  				target.setAttribute("class", ratio ? data.e : data.b);
  				for (p in inline) {
  					_removeProperty(target, p);
  				}
  			}
  		},
  		_getAllStyles = (target) => {
  			let styles = {},
  				computed = getComputedStyle(target),
  				p;
  			for (p in computed) {
  				if (isNaN(p) && p !== "cssText" && p !== "length") {
  					styles[p] = computed[p];
  				}
  			}
  			_setDefaults(styles, _parseTransform(target, 1));
  			return styles;
  		},
  		startClassList = target.getAttribute("class"),
  		style = target.style,
  		cssText = style.cssText,
  		cache = target._gsap,
  		classPT = cache.classPT,
  		inlineToRemoveAtEnd = {},
  		data = {t:target, plugin:plugin, rmv:inlineToRemoveAtEnd, b:startClassList, e:(endValue.charAt(1) !== "=") ? endValue : startClassList.replace(new RegExp("(?:\\s|^)" + endValue.substr(2) + "(?![\\w-])"), "") + ((endValue.charAt(0) === "+") ? " " + endValue.substr(2) : "")},
  		changingVars = {},
  		startVars = _getAllStyles(target),
  		transformRelated = /(transform|perspective)/i,
  		endVars, p;
  	if (classPT) {
  		classPT.r(1, classPT.d);
  		_removeLinkedListItem(classPT.d.plugin, classPT, "_pt");
  	}
  	target.setAttribute("class", data.e);
  	endVars = _getAllStyles(target, true);
  	target.setAttribute("class", startClassList);
  	for (p in endVars) {
  		if (endVars[p] !== startVars[p] && !transformRelated.test(p)) {
  			changingVars[p] = endVars[p];
  			if (!style[p] && style[p] !== "0") {
  				inlineToRemoveAtEnd[p] = 1;
  			}
  		}
  	}
  	cache.classPT = plugin._pt = new PropTween(plugin._pt, target, "className", 0, 0, _renderClassName, data, 0, -11);
  	if (style.cssText !== cssText) { //only apply if things change. Otherwise, in cases like a background-image that's pulled dynamically, it could cause a refresh. See https://gsap.com/forums/topic/20368-possible-gsap-bug-switching-classnames-in-chrome/.
  		style.cssText = cssText; //we recorded cssText before we swapped classes and ran _getAllStyles() because in cases when a className tween is overwritten, we remove all the related tweening properties from that class change (otherwise class-specific stuff can't override properties we've directly set on the target's style object due to specificity).
  	}
  	_parseTransform(target, true); //to clear the caching of transforms
  	data.css = new gsap.plugins.css();
  	data.css.init(target, changingVars, tween);
  	plugin._props.push(...data.css._props);
  	return 1;
  }
  */
}, UB = [1, 0, 0, 1, 0, 0], Te = {}, ze = function(Q) {
  return Q === "matrix(1, 0, 0, 1, 0, 0)" || Q === "none" || !Q;
}, nA = function(Q) {
  var B = TQ(Q, V);
  return ze(B) ? UB : B.substr(7).match(dA).map(X);
}, xt = function(Q, B) {
  var t = Q._gsap || YQ(Q), A = Q.style, e = nA(Q), P, i, s, v;
  return t.svg && Q.getAttribute("transform") ? (s = Q.transform.baseVal.consolidate().matrix, e = [s.a, s.b, s.c, s.d, s.e, s.f], e.join(",") === "1,0,0,1,0,0" ? UB : e) : (e === UB && !Q.offsetParent && Q !== QB && !t.svg && (s = A.display, A.display = "block", P = Q.parentNode, (!P || !Q.offsetParent && !Q.getBoundingClientRect().width) && (v = 1, i = Q.nextElementSibling, QB.appendChild(Q)), e = nA(Q), s ? A.display = s : gQ(Q, "display"), v && (i ? P.insertBefore(Q, i) : P ? P.appendChild(Q) : QB.removeChild(Q))), B && e.length > 6 ? [e[0], e[1], e[4], e[5], e[12], e[13]] : e);
}, kt = function(Q, B, t, A, e, P) {
  var i = Q._gsap, s = e || xt(Q, !0), v = i.xOrigin || 0, c = i.yOrigin || 0, n = i.xOffset || 0, T = i.yOffset || 0, z = s[0], w = s[1], o = s[2], a = s[3], E = s[4], l = s[5], u = B.split(" "), U = parseFloat(u[0]) || 0, k = parseFloat(u[1]) || 0, O, D, N, d;
  t ? s !== UB && (D = z * a - w * o) && (N = U * (a / D) + k * (-o / D) + (o * l - a * E) / D, d = U * (-w / D) + k * (z / D) - (z * l - w * E) / D, U = N, k = d) : (O = ne(Q), U = O.x + (~u[0].indexOf("%") ? U / 100 * O.width : U), k = O.y + (~(u[1] || u[0]).indexOf("%") ? k / 100 * O.height : k)), A || A !== !1 && i.smooth ? (E = U - v, l = k - c, i.xOffset = n + (E * z + l * o) - E, i.yOffset = T + (E * w + l * a) - l) : i.xOffset = i.yOffset = 0, i.xOrigin = U, i.yOrigin = k, i.smooth = !!A, i.origin = B, i.originIsAbsolute = !!t, Q.style[iQ] = "0px 0px", P && (FQ(P, i, "xOrigin", v, U), FQ(P, i, "yOrigin", c, k), FQ(P, i, "xOffset", n, i.xOffset), FQ(P, i, "yOffset", T, i.yOffset)), Q.setAttribute("data-svg-origin", U + " " + k);
}, DB = function(Q, B) {
  var t = Q._gsap || new qA(Q);
  if ("x" in t && !B && !t.uncache)
    return t;
  var A = Q.style, e = t.scaleX < 0, P = "px", i = "deg", s = getComputedStyle(Q), v = TQ(Q, iQ) || "0", c, n, T, z, w, o, a, E, l, u, U, k, O, D, N, d, M, b, G, C, j, F, H, f, I, R, QQ, sQ, W, x, BQ, vQ;
  return c = n = T = o = a = E = l = u = U = 0, z = w = 1, t.svg = !!(Q.getCTM && oe(Q)), s.translate && ((s.translate !== "none" || s.scale !== "none" || s.rotate !== "none") && (A[V] = (s.translate !== "none" ? "translate3d(" + (s.translate + " 0 0").split(" ").slice(0, 3).join(", ") + ") " : "") + (s.rotate !== "none" ? "rotate(" + s.rotate + ") " : "") + (s.scale !== "none" ? "scale(" + s.scale.split(" ").join(",") + ") " : "") + (s[V] !== "none" ? s[V] : "")), A.scale = A.rotate = A.translate = "none"), D = xt(Q, t.svg), t.svg && (t.uncache ? (I = Q.getBBox(), v = t.xOrigin - I.x + "px " + (t.yOrigin - I.y) + "px", f = "") : f = !B && Q.getAttribute("data-svg-origin"), kt(Q, f || v, !!f || t.originIsAbsolute, t.smooth !== !1, D)), k = t.xOrigin || 0, O = t.yOrigin || 0, D !== UB && (b = D[0], G = D[1], C = D[2], j = D[3], c = F = D[4], n = H = D[5], D.length === 6 ? (z = Math.sqrt(b * b + G * G), w = Math.sqrt(j * j + C * C), o = b || G ? KQ(G, b) * IQ : 0, l = C || j ? KQ(C, j) * IQ + o : 0, l && (w *= Math.abs(Math.cos(l * BB))), t.svg && (c -= k - (k * b + O * C), n -= O - (k * G + O * j))) : (vQ = D[6], x = D[7], QQ = D[8], sQ = D[9], W = D[10], BQ = D[11], c = D[12], n = D[13], T = D[14], N = KQ(vQ, W), a = N * IQ, N && (d = Math.cos(-N), M = Math.sin(-N), f = F * d + QQ * M, I = H * d + sQ * M, R = vQ * d + W * M, QQ = F * -M + QQ * d, sQ = H * -M + sQ * d, W = vQ * -M + W * d, BQ = x * -M + BQ * d, F = f, H = I, vQ = R), N = KQ(-C, W), E = N * IQ, N && (d = Math.cos(-N), M = Math.sin(-N), f = b * d - QQ * M, I = G * d - sQ * M, R = C * d - W * M, BQ = j * M + BQ * d, b = f, G = I, C = R), N = KQ(G, b), o = N * IQ, N && (d = Math.cos(N), M = Math.sin(N), f = b * d + G * M, I = F * d + H * M, G = G * d - b * M, H = H * d - F * M, b = f, F = I), a && Math.abs(a) + Math.abs(o) > 359.9 && (a = o = 0, E = 180 - E), z = X(Math.sqrt(b * b + G * G + C * C)), w = X(Math.sqrt(H * H + vQ * vQ)), N = KQ(F, H), l = Math.abs(N) > 2e-4 ? N * IQ : 0, U = BQ ? 1 / (BQ < 0 ? -BQ : BQ) : 0), t.svg && (f = Q.getAttribute("transform"), t.forceCSS = Q.setAttribute("transform", "") || !ze(TQ(Q, V)), f && Q.setAttribute("transform", f))), Math.abs(l) > 90 && Math.abs(l) < 270 && (e ? (z *= -1, l += o <= 0 ? 180 : -180, o += o <= 0 ? 180 : -180) : (w *= -1, l += l <= 0 ? 180 : -180)), B = B || t.uncache, t.x = c - ((t.xPercent = c && (!B && t.xPercent || (Math.round(Q.offsetWidth / 2) === Math.round(-c) ? -50 : 0))) ? Q.offsetWidth * t.xPercent / 100 : 0) + P, t.y = n - ((t.yPercent = n && (!B && t.yPercent || (Math.round(Q.offsetHeight / 2) === Math.round(-n) ? -50 : 0))) ? Q.offsetHeight * t.yPercent / 100 : 0) + P, t.z = T + P, t.scaleX = X(z), t.scaleY = X(w), t.rotation = X(o) + i, t.rotationX = X(a) + i, t.rotationY = X(E) + i, t.skewX = l + i, t.skewY = u + i, t.transformPerspective = U + P, (t.zOrigin = parseFloat(v.split(" ")[2]) || !B && t.zOrigin || 0) && (A[iQ] = mB(v)), t.xOffset = t.yOffset = 0, t.force3D = zQ.force3D, t.renderTransform = t.svg ? ui : ce ? we : ki, t.uncache = 0, t;
}, mB = function(Q) {
  return (Q = Q.split(" "))[0] + " " + Q[1];
}, _B = function(Q, B, t) {
  var A = _(B);
  return X(parseFloat(B) + parseFloat(pQ(Q, "x", t + "px", A))) + A;
}, ki = function(Q, B) {
  B.z = "0px", B.rotationY = B.rotationX = "0deg", B.force3D = 0, we(Q, B);
}, xQ = "0deg", sB = "0px", yQ = ") ", we = function(Q, B) {
  var t = B || this, A = t.xPercent, e = t.yPercent, P = t.x, i = t.y, s = t.z, v = t.rotation, c = t.rotationY, n = t.rotationX, T = t.skewX, z = t.skewY, w = t.scaleX, o = t.scaleY, a = t.transformPerspective, E = t.force3D, l = t.target, u = t.zOrigin, U = "", k = E === "auto" && Q && Q !== 1 || E === !0;
  if (u && (n !== xQ || c !== xQ)) {
    var O = parseFloat(c) * BB, D = Math.sin(O), N = Math.cos(O), d;
    O = parseFloat(n) * BB, d = Math.cos(O), P = _B(l, P, D * d * -u), i = _B(l, i, -Math.sin(O) * -u), s = _B(l, s, N * d * -u + u);
  }
  a !== sB && (U += "perspective(" + a + yQ), (A || e) && (U += "translate(" + A + "%, " + e + "%) "), (k || P !== sB || i !== sB || s !== sB) && (U += s !== sB || k ? "translate3d(" + P + ", " + i + ", " + s + ") " : "translate(" + P + ", " + i + yQ), v !== xQ && (U += "rotate(" + v + yQ), c !== xQ && (U += "rotateY(" + c + yQ), n !== xQ && (U += "rotateX(" + n + yQ), (T !== xQ || z !== xQ) && (U += "skew(" + T + ", " + z + yQ), (w !== 1 || o !== 1) && (U += "scale(" + w + ", " + o + yQ), l.style[V] = U || "translate(0, 0)";
}, ui = function(Q, B) {
  var t = B || this, A = t.xPercent, e = t.yPercent, P = t.x, i = t.y, s = t.rotation, v = t.skewX, c = t.skewY, n = t.scaleX, T = t.scaleY, z = t.target, w = t.xOrigin, o = t.yOrigin, a = t.xOffset, E = t.yOffset, l = t.forceCSS, u = parseFloat(P), U = parseFloat(i), k, O, D, N, d;
  s = parseFloat(s), v = parseFloat(v), c = parseFloat(c), c && (c = parseFloat(c), v += c, s += c), s || v ? (s *= BB, v *= BB, k = Math.cos(s) * n, O = Math.sin(s) * n, D = Math.sin(s - v) * -T, N = Math.cos(s - v) * T, v && (c *= BB, d = Math.tan(v - c), d = Math.sqrt(1 + d * d), D *= d, N *= d, c && (d = Math.tan(c), d = Math.sqrt(1 + d * d), k *= d, O *= d)), k = X(k), O = X(O), D = X(D), N = X(N)) : (k = n, N = T, O = D = 0), (u && !~(P + "").indexOf("px") || U && !~(i + "").indexOf("px")) && (u = pQ(z, "x", P, "px"), U = pQ(z, "y", i, "px")), (w || o || a || E) && (u = X(u + w - (w * k + o * D) + a), U = X(U + o - (w * O + o * N) + E)), (A || e) && (d = z.getBBox(), u = X(u + A / 100 * d.width), U = X(U + e / 100 * d.height)), d = "matrix(" + k + "," + O + "," + D + "," + N + "," + u + "," + U + ")", z.setAttribute("transform", d), l && (z.style[V] = d);
}, Ui = function(Q, B, t, A, e) {
  var P = 360, i = Z(e), s = parseFloat(e) * (i && ~e.indexOf("rad") ? IQ : 1), v = s - A, c = A + v + "deg", n, T;
  return i && (n = e.split("_")[1], n === "short" && (v %= P, v !== v % (P / 2) && (v += v < 0 ? P : -P)), n === "cw" && v < 0 ? v = (v + P * PA) % P - ~~(v / P) * P : n === "ccw" && v > 0 && (v = (v - P * PA) % P - ~~(v / P) * P)), Q._pt = T = new PQ(Q._pt, B, t, A, v, ei), T.e = c, T.u = "deg", Q._props.push(t), T;
}, oA = function(Q, B) {
  for (var t in B)
    Q[t] = B[t];
  return Q;
}, Di = function(Q, B, t) {
  var A = oA({}, t._gsap), e = "perspective,force3D,transformOrigin,svgOrigin", P = t.style, i, s, v, c, n, T, z, w;
  A.svg ? (v = t.getAttribute("transform"), t.setAttribute("transform", ""), P[V] = B, i = DB(t, 1), gQ(t, V), t.setAttribute("transform", v)) : (v = getComputedStyle(t)[V], P[V] = B, i = DB(t, 1), P[V] = v);
  for (s in OQ)
    v = A[s], c = i[s], v !== c && e.indexOf(s) < 0 && (z = _(v), w = _(c), n = z !== w ? pQ(t, s, v, w) : parseFloat(v), T = parseFloat(c), Q._pt = new PQ(Q._pt, i, s, n, T - n, Et), Q._pt.u = w || 0, Q._props.push(s));
  oA(i, A);
};
eQ("padding,margin,Width,Radius", function(r, Q) {
  var B = "Top", t = "Right", A = "Bottom", e = "Left", P = (Q < 3 ? [B, t, A, e] : [B + e, B + t, A + t, A + e]).map(function(i) {
    return Q < 2 ? r + i : "border" + i + r;
  });
  pB[Q > 1 ? "border" + r : r] = function(i, s, v, c, n) {
    var T, z;
    if (arguments.length < 4)
      return T = P.map(function(w) {
        return CQ(i, w, v);
      }), z = T.join(" "), z.split(T[0]).length === 5 ? T[0] : z;
    T = (c + "").split(" "), z = {}, P.forEach(function(w, o) {
      return z[w] = T[o] = T[o] || T[(o - 1) / 2 | 0];
    }), i.init(s, z, n);
  };
});
var ae = {
  name: "css",
  register: lt,
  targetTest: function(Q) {
    return Q.style && Q.nodeType;
  },
  init: function(Q, B, t, A, e) {
    var P = this._props, i = Q.style, s = t.vars.startAt, v, c, n, T, z, w, o, a, E, l, u, U, k, O, D, N, d;
    gt || lt(), this.styles = this.styles || ve(Q), N = this.styles.props, this.tween = t;
    for (o in B)
      if (o !== "autoRound" && (c = B[o], !(cQ[o] && _A(o, B, t, A, Q, e)))) {
        if (z = typeof c, w = pB[o], z === "function" && (c = c.call(t, A, Q, e), z = typeof c), z === "string" && ~c.indexOf("random(") && (c = lB(c)), w)
          w(this, Q, o, c, t) && (D = 1);
        else if (o.substr(0, 2) === "--")
          v = (getComputedStyle(Q).getPropertyValue(o) + "").trim(), c += "", GQ.lastIndex = 0, GQ.test(v) || (a = _(v), E = _(c), E ? a !== E && (v = pQ(Q, o, v, E) + E) : a && (c += a)), this.add(i, "setProperty", v, c, A, e, 0, 0, o), P.push(o), N.push(o, 0, i[o]);
        else if (z !== "undefined") {
          if (s && o in s ? (v = typeof s[o] == "function" ? s[o].call(t, A, Q, e) : s[o], Z(v) && ~v.indexOf("random(") && (v = lB(v)), _(v + "") || v === "auto" || (v += zQ.units[o] || _(CQ(Q, o)) || ""), (v + "").charAt(1) === "=" && (v = CQ(Q, o))) : v = CQ(Q, o), T = parseFloat(v), l = z === "string" && c.charAt(1) === "=" && c.substr(0, 2), l && (c = c.substr(2)), n = parseFloat(c), o in UQ && (o === "autoAlpha" && (T === 1 && CQ(Q, "visibility") === "hidden" && n && (T = 0), N.push("visibility", 0, i.visibility), FQ(this, i, "visibility", T ? "inherit" : "hidden", n ? "inherit" : "hidden", !n)), o !== "scale" && o !== "transform" && (o = UQ[o], ~o.indexOf(",") && (o = o.split(",")[0]))), u = o in OQ, u) {
            if (this.styles.save(o), d = c, z === "string" && c.substring(0, 6) === "var(--") {
              if (c = TQ(Q, c.substring(4, c.indexOf(")"))), c.substring(0, 5) === "calc(") {
                var M = Q.style.perspective;
                Q.style.perspective = c, c = TQ(Q, "perspective"), M ? Q.style.perspective = M : gQ(Q, "perspective");
              }
              n = parseFloat(c);
            }
            if (U || (k = Q._gsap, k.renderTransform && !B.parseTransform || DB(Q, B.parseTransform), O = B.smoothOrigin !== !1 && k.smooth, U = this._pt = new PQ(this._pt, i, V, 0, 1, k.renderTransform, k, 0, -1), U.dep = 1), o === "scale")
              this._pt = new PQ(this._pt, k, "scaleY", k.scaleY, (l ? $Q(k.scaleY, l + n) : n) - k.scaleY || 0, Et), this._pt.u = 0, P.push("scaleY", o), o += "X";
            else if (o === "transformOrigin") {
              N.push(iQ, 0, i[iQ]), c = hi(c), k.svg ? kt(Q, c, 0, O, 0, this) : (E = parseFloat(c.split(" ")[2]) || 0, E !== k.zOrigin && FQ(this, k, "zOrigin", k.zOrigin, E), FQ(this, i, o, mB(v), mB(c)));
              continue;
            } else if (o === "svgOrigin") {
              kt(Q, c, 1, O, 0, this);
              continue;
            } else if (o in Te) {
              Ui(this, k, o, T, l ? $Q(T, l + c) : c);
              continue;
            } else if (o === "smoothOrigin") {
              FQ(this, k, "smooth", k.smooth, c);
              continue;
            } else if (o === "force3D") {
              k[o] = c;
              continue;
            } else if (o === "transform") {
              Di(this, c, Q);
              continue;
            }
          } else o in i || (o = iB(o) || o);
          if (u || (n || n === 0) && (T || T === 0) && !Ai.test(c) && o in i)
            a = (v + "").substr((T + "").length), n || (n = 0), E = _(c) || (o in zQ.units ? zQ.units[o] : a), a !== E && (T = pQ(Q, o, v, E)), this._pt = new PQ(this._pt, u ? k : i, o, T, (l ? $Q(T, l + n) : n) - T, !u && (E === "px" || o === "zIndex") && B.autoRound !== !1 ? ri : Et), this._pt.u = E || 0, u && d !== c ? (this._pt.b = v, this._pt.e = d, this._pt.r = ii) : a !== E && E !== "%" && (this._pt.b = v, this._pt.r = Pi);
          else if (o in i)
            Ei.call(this, Q, o, v, l ? l + c : c);
          else if (o in Q)
            this.add(Q, o, v || Q[o], l ? l + c : c, A, e);
          else if (o !== "parseTransform") {
            Nt(o, c);
            continue;
          }
          u || (o in i ? N.push(o, 0, i[o]) : typeof Q[o] == "function" ? N.push(o, 2, Q[o]()) : N.push(o, 1, v || Q[o])), P.push(o);
        }
      }
    D && ee(this);
  },
  render: function(Q, B) {
    if (B.tween._time || !pt())
      for (var t = B._pt; t; )
        t.r(Q, t.d), t = t._next;
    else
      B.styles.revert();
  },
  get: CQ,
  aliases: UQ,
  getSetter: function(Q, B, t) {
    var A = UQ[B];
    return A && A.indexOf(",") < 0 && (B = A), B in OQ && B !== iQ && (Q._gsap.x || CQ(Q, "x")) ? t && eA === t ? B === "scale" ? ni : ci : (eA = t || {}) && (B === "scale" ? oi : Ti) : Q.style && !Dt(Q.style[B]) ? si : ~B.indexOf("-") ? vi : Gt(Q, B);
  },
  core: {
    _removeProperty: gQ,
    _getMatrix: xt
  }
};
rQ.utils.checkPrefix = iB;
rQ.core.getStyleSaver = ve;
(function(r, Q, B, t) {
  var A = eQ(r + "," + Q + "," + B, function(e) {
    OQ[e] = 1;
  });
  eQ(Q, function(e) {
    zQ.units[e] = "deg", Te[e] = 1;
  }), UQ[A[13]] = r + "," + Q, eQ(t, function(e) {
    var P = e.split(":");
    UQ[P[1]] = A[P[0]];
  });
})("x,y,z,scale,scaleX,scaleY,xPercent,yPercent", "rotation,rotationX,rotationY,skewX,skewY", "transform,transformOrigin,svgOrigin,force3D,smoothOrigin,transformPerspective", "0:translateX,1:translateY,2:translateZ,8:rotate,8:rotationZ,8:rotateZ,9:rotateX,10:rotateY");
eQ("x,y,z,top,right,bottom,left,width,height,fontSize,padding,margin,perspective", function(r) {
  zQ.units[r] = "px";
});
rQ.registerPlugin(ae);
var jB = rQ.registerPlugin(ae) || rQ;
jB.core.Tween;
const di = "", TA = 0.3, Ee = "#c9883d", Ci = "#eef1f4", zA = "#a855f7";
function CB(r) {
  return Number.isFinite(r) ? Math.max(0, Math.min(1, r)) : 0;
}
function Ni(r) {
  const Q = CB(r);
  return 1 - Math.pow(1 - Q, 3);
}
function Oi(r) {
  const Q = Math.max(1e-3, r), B = Q * 1.6, t = Q * 0.7, A = new h.ConeGeometry(t, B, 3, 1, !1);
  return A.rotateX(-Math.PI / 2), A.translate(0, 0, B / 2), A;
}
function Li(r) {
  const B = Math.max(1e-3, r) * 0.65, t = new h.SphereGeometry(B, 18, 12);
  return t.translate(0, 0, B), t;
}
function Mi(r, Q) {
  r.computeBoundingBox();
  const B = r.boundingBox, t = Math.max(1e-6, B.max.z - B.min.z), A = r.getAttribute("position"), e = new Float32Array(A.count * 2);
  for (let P = 0; P < A.count; P++) {
    const i = A.getX(P), s = A.getY(P), v = A.getZ(P);
    e[P * 2] = Math.atan2(s, i) / (2 * Math.PI) + 0.5, e[P * 2 + 1] = (v - B.min.z) / t * Q;
  }
  r.setAttribute("uv", new h.BufferAttribute(e, 2));
}
function he(r) {
  const Q = atob(di), B = new Uint8Array(Q.length);
  for (let v = 0; v < Q.length; v++) B[v] = Q.charCodeAt(v);
  const A = new Ne().parse(B.buffer);
  A.rotateX(-Math.PI / 2), A.computeBoundingBox();
  const e = A.boundingBox, P = new h.Vector3();
  e.getSize(P);
  const i = Math.max(1e-3, r) * 1.6 / Math.max(P.x, P.y, P.z, 1e-3);
  A.scale(i, i, i), A.computeBoundingBox();
  const s = A.boundingBox;
  return A.translate(
    -(s.min.x + s.max.x) / 2,
    -(s.min.y + s.max.y) / 2,
    -s.min.z
  ), Mi(A, 6), A.computeVertexNormals(), A;
}
let $B = null;
function Hi() {
  if ($B) return $B;
  const r = 64, Q = 256, B = document.createElement("canvas");
  B.width = r, B.height = Q;
  const t = B.getContext("2d");
  t.fillStyle = "#808080", t.fillRect(0, 0, r, Q);
  const A = 10;
  for (let i = 0; i < A; i++) {
    const s = i / A * r, v = i % 2 === 0 ? 150 : 90;
    t.fillStyle = `rgb(${v},${v},${v})`, t.fillRect(s, 0, r / A / 2, Q);
  }
  const e = t.getImageData(0, 0, r, Q);
  for (let i = 0; i < e.data.length; i += 4) {
    const s = (Math.random() - 0.5) * 40;
    e.data[i] = Math.min(255, Math.max(0, e.data[i] + s)), e.data[i + 1] = e.data[i], e.data[i + 2] = e.data[i];
  }
  t.putImageData(e, 0, 0);
  const P = new h.CanvasTexture(B);
  return P.wrapS = h.RepeatWrapping, P.wrapT = h.RepeatWrapping, $B = P, P;
}
function ji(r, Q) {
  const B = Math.max(1e-3, r), t = B * 1.6, A = B * 0.08, e = B * 0.187, P = CB(Q), i = new h.CylinderGeometry(0, A, t, 8, 1, !1);
  i.rotateX(-Math.PI / 2), i.translate(0, 0, t / 2);
  const s = new h.CylinderGeometry(0, e, t, 8, 1, !0);
  s.rotateX(-Math.PI / 2), s.translate(0, 0, t / 2);
  const v = new h.MeshBasicMaterial({
    color: new h.Color(zA),
    transparent: !0,
    opacity: P,
    depthTest: !1,
    depthWrite: !1,
    blending: h.AdditiveBlending
  }), c = new h.MeshBasicMaterial({
    color: new h.Color(zA),
    transparent: !0,
    opacity: P * 0.3,
    depthTest: !1,
    depthWrite: !1,
    blending: h.AdditiveBlending,
    side: h.DoubleSide
  }), n = new h.Mesh(i, v);
  n.renderOrder = 1e3;
  const T = new h.Mesh(s, c);
  T.renderOrder = 1e3;
  const z = new h.Group();
  return z.add(n), z.add(T), z;
}
function Fi(r) {
  const Q = Math.max(1e-3, r.bit.size);
  return r.bit.type === "triangle" ? Oi(Q) : r.bit.type === "drill" ? he(Q) : Li(Q);
}
function le(r) {
  return r.bit.colorSource === "custom" ? r.bit.color : Ci;
}
function fi(r) {
  const Q = CB(r.bit.opacity), B = he(Math.max(1e-3, r.bit.size)), t = new h.Color(le(r)), A = Hi(), e = new h.MeshStandardMaterial({
    color: t,
    metalness: 0.8,
    roughness: 0.35,
    roughnessMap: A,
    bumpMap: A,
    bumpScale: 0.03,
    // Self-illuminate slightly so the bit stays legible as a UI marker
    // regardless of viewing angle/lighting, matching the old visualizer's
    // brighter, always-visible look.
    emissive: t,
    emissiveIntensity: 0.12,
    transparent: Q < 1,
    opacity: Q,
    depthTest: !1,
    depthWrite: !1
  }), P = new h.Mesh(B, e);
  return P.renderOrder = 1e3, P;
}
function Gi(r) {
  const Q = CB(r.bit.opacity), B = Fi(r), t = new h.MeshBasicMaterial({
    color: new h.Color(Ee),
    transparent: Q < 1,
    opacity: Q,
    side: h.DoubleSide,
    depthTest: !1,
    depthWrite: !1
  }), A = new h.Mesh(B, t);
  return A.renderOrder = 1e3, A;
}
function wA(r) {
  return r.bit.type === "laser" ? ji(r.bit.size, r.bit.opacity) : r.bit.type === "drill" ? fi(r) : Gi(r);
}
function aA(r) {
  if (r instanceof h.Mesh)
    r.geometry.dispose(), r.material.dispose();
  else if (r instanceof h.Group)
    for (const Q of r.children)
      Q instanceof h.Mesh && (Q.geometry.dispose(), Q.material.dispose());
}
function bi(r) {
  const Q = new h.Group();
  Q.name = "gviewer:bit-marker", Q.visible = !!r.bit.enabled;
  let B = r, t = wA(B);
  Q.add(t);
  let A = null, e = !1, P = null, i = null;
  const s = (n) => {
    const T = Math.max(1, n);
    return jB.to(t.rotation, {
      z: `+=${Math.PI * 2}`,
      duration: 60 / T,
      repeat: -1,
      ease: "none",
      paused: !0
    });
  };
  P = s(B.bit.spinRpm);
  const v = (n) => {
    const T = n.bit.type, z = B.bit.type, w = n.bit.size, o = B.bit.size, a = T !== z, E = w !== o, l = CB(n.bit.opacity);
    if (a || E)
      Q.remove(t), aA(t), t = wA(n), Q.add(t), i == null || i.kill(), P == null || P.kill(), P = s(n.bit.spinRpm), e && P.timeScale(1).play();
    else if (T === "laser") {
      const U = t, [k, O] = U.children, D = k.material, N = O.material;
      D.opacity = l, D.needsUpdate = !0, N.opacity = l * 0.3, N.needsUpdate = !0;
    } else {
      const k = t.material, O = new h.Color(T === "drill" ? le(n) : Ee);
      k.color = O, T === "drill" && (k.emissive = O), k.opacity = l, k.transparent = l < 1, k.needsUpdate = !0;
    }
    B = n, Q.visible = !!n.bit.enabled;
  };
  return {
    object: Q,
    setVisible: (n) => {
      Q.visible = !!n;
    },
    setOptions: (n) => v(n),
    setTarget: (n, T) => {
      const z = !!(T != null && T.immediate), w = Number(n.x) || 0, o = Number(n.y) || 0, a = Number(n.z) || 0;
      if (n.a, z || B.bit.tweenMs <= 0) {
        A = null, Q.position.set(w, o, a);
        return;
      }
      A = {
        startedAt: performance.now(),
        duration: Math.max(1, B.bit.tweenMs),
        from: { x: Q.position.x, y: Q.position.y, z: Q.position.z },
        to: { x: w, y: o, z: a }
      };
    },
    setSpinning: (n) => {
      const T = !!n;
      if (T !== e)
        if (e = T, i == null || i.kill(), e)
          P == null || P.play(), i = jB.to(P, {
            timeScale: 1,
            duration: TA,
            ease: "power1.out"
          });
        else {
          const z = P;
          i = jB.to(P, {
            timeScale: 0,
            duration: TA,
            ease: "power1.in",
            onComplete: () => z == null ? void 0 : z.pause()
          });
        }
    },
    update: (n) => {
      if (!A) return;
      const T = (n - A.startedAt) / A.duration, z = Ni(T);
      Q.position.set(
        h.MathUtils.lerp(A.from.x, A.to.x, z),
        h.MathUtils.lerp(A.from.y, A.to.y, z),
        h.MathUtils.lerp(A.from.z, A.to.z, z)
      ), T >= 1 && (Q.position.set(A.to.x, A.to.y, A.to.z), A = null);
    },
    dispose: () => {
      i == null || i.kill(), P == null || P.kill(), Q.remove(t), aA(t);
    }
  };
}
const gi = 25.4, pi = 500, mi = 300;
class Xi {
  constructor(Q) {
    this.viewCube = null, this.resizeObserver = null, this.onWindowResize = null, this.animationFrameId = null, this.gridGroup = null, this.axesGroup = null, this.gridLabelsGroup = null, this.boundingBoxGroup = null, this.machineBedGroup = null, this.bitMarker = null, this.preLaserBitType = "drill", this.toolpathStreams = [], this.toolpathCutBucketCount = 1, this.toolpathRotationA = 0, this.lastBitPosition = { x: 0, y: 0, z: 0, a: 0 }, this.sim3dHandle = null, this.currentLines = [], this.linePositions = null, this.renderSequence = 0, this.currentBounds = null, this.cameraFocusTransition = null, this.cameraFollowRequested = !1, this.cameraFollowInterrupted = !1, this.cameraFollowOffset = null, this.id = Q.id, this.container = Q.container, this.callbacks = Q.callbacks ?? {}, this.options = hA(Ge, Q.options), this.canvas = document.createElement("canvas"), this.canvas.style.width = "100%", this.canvas.style.height = "100%", this.canvas.style.display = "block", this.container.appendChild(this.canvas), qe(this.container), this.renderer = new h.WebGLRenderer({
      canvas: this.canvas,
      antialias: this.options.render.antialias
    }), this.renderer.setPixelRatio(window.devicePixelRatio), this.renderer.setClearColor(this.options.render.theme.background, 1), this.scene = new h.Scene(), this.scene.add(new h.AmbientLight(16777215, 0.8));
    const B = new h.DirectionalLight(16777215, 0.9);
    B.position.set(1, -1, 1.5), this.scene.add(B);
    const t = new h.DirectionalLight(16777215, 0.5);
    t.position.set(-1, 1, 1), this.scene.add(t), this.toolpathRoot = new h.Group(), this.toolpathRoot.name = "gviewer:toolpath-root", this.scene.add(this.toolpathRoot), this.camera = new h.PerspectiveCamera(this.options.camera.fov, 1, 0.1, 1e5), this.camera.up.set(0, 0, 1), this.camera.position.set(
      this.options.camera.initialPosition.x,
      this.options.camera.initialPosition.y,
      this.options.camera.initialPosition.z
    ), this.controls = new ue(this.camera, this.renderer.domElement), this.controls.enableDamping = this.options.camera.orbit.enableDamping, this.controls.addEventListener("start", () => {
      this.cameraFollowRequested && (this.cameraFollowInterrupted = !0);
    }), this.controls.update(), this.viewCubeCorrection = new h.Matrix4().makeRotationX(h.MathUtils.degToRad(90)), this.viewCube = new Se({
      container: this.container,
      onSelectView: (A) => {
        const e = new h.Vector3();
        let P;
        if (this.currentBounds) {
          this.currentBounds.getCenter(e);
          const i = new h.Vector3();
          this.currentBounds.getSize(i);
          const s = Math.max(i.x, i.y, 1) / 2, v = h.MathUtils.degToRad(this.camera.fov);
          P = s / Math.tan(v / 2) * 1.25 + i.z;
        } else
          P = this.camera.position.distanceTo(this.controls.target);
        this.startSnapToView(A, e, Math.max(1e-6, P), 400);
      }
    }), typeof ResizeObserver < "u" ? (this.resizeObserver = new ResizeObserver(() => this.resize()), this.resizeObserver.observe(this.container)) : (this.onWindowResize = () => this.resize(), window.addEventListener("resize", this.onWindowResize)), this.renderGridAndAxes(), this.refreshGridLabels(), this.refreshBoundingBox(), this.refreshMachineBed(), this.ensureBitMarker(), this.resize(), this.startAnimationLoop();
  }
  setBitPosition(Q, B) {
    var t;
    if (this.ensureBitMarker(), this.lastBitPosition = { ...this.lastBitPosition, ...Q }, (t = this.bitMarker) == null || t.setTarget(Q, B), this.cameraFollowRequested && !this.cameraFollowInterrupted && this.cameraFollowOffset) {
      const A = new h.Vector3(this.lastBitPosition.x, this.lastBitPosition.y, this.controls.target.z), e = A.clone().add(this.cameraFollowOffset);
      this.startCameraLerp(A, e, mi, "linear");
    }
  }
  /**
   * Enable or disable "follow tool" camera tracking. While enabled, every
   * subsequent setBitPosition() call pans the camera+target in X/Y to keep
   * the tool centered, holding the camera→target offset captured at engage
   * time constant — which is what preserves viewing angle and height. A
   * user-initiated camera drag (OrbitControls "start") interrupts an active
   * session; it only re-arms on the next disabled→enabled edge.
   */
  setCameraFollowEnabled(Q) {
    if (Q === this.cameraFollowRequested)
      return;
    if (this.cameraFollowRequested = Q, this.cameraFollowInterrupted = !1, !Q) {
      this.cameraFollowOffset = null;
      return;
    }
    this.cameraFollowOffset = this.camera.position.clone().sub(this.controls.target);
    const B = new h.Vector3(this.lastBitPosition.x, this.lastBitPosition.y, this.controls.target.z), t = B.clone().add(this.cameraFollowOffset);
    this.startCameraLerp(B, t, pi);
  }
  setBitVisible(Q) {
    var B;
    this.ensureBitMarker(), (B = this.bitMarker) == null || B.setVisible(Q);
  }
  setBitSpinning(Q) {
    var B;
    this.ensureBitMarker(), (B = this.bitMarker) == null || B.setSpinning(Q);
  }
  setToolpathRotationA(Q) {
    const B = Number(Q) || 0;
    this.toolpathRotationA = B, this.toolpathRoot.rotation.x = h.MathUtils.degToRad(B);
  }
  setCallbacks(Q) {
    this.callbacks = Q;
  }
  hideUntilLine(Q, B) {
    const t = B ?? this.options.progress.mode, A = Math.floor(Q);
    for (const e of this.toolpathStreams) {
      const P = A < 0 ? 0 : e.prefixEndVertex[Math.min(A, e.prefixEndVertex.length - 1)];
      t === "grey" ? (e.line.geometry.setDrawRange(0, e.totalVertices), je({ stream: e, nextCursorVertex: P, options: this.options })) : e.line.geometry.setDrawRange(P, Math.max(0, e.totalVertices - P));
    }
  }
  seekToLine(Q, B) {
    if (this.hideUntilLine(Q, B), this.linePositions && this.currentLines.length > 0) {
      const t = Math.max(0, Math.min(Math.floor(Q), this.currentLines.length - 1)), A = {
        x: this.linePositions[t * 4],
        y: this.linePositions[t * 4 + 1],
        z: this.linePositions[t * 4 + 2],
        a: this.linePositions[t * 4 + 3]
      };
      this.setBitPosition(A, { immediate: !0 });
    }
    if (this.sim3dHandle) {
      const t = Math.max(0, Math.min(Math.floor(Q), this.currentLines.length - 1));
      if (t !== this.sim3dHandle.currentLine) {
        const A = SB(t, this.sim3dHandle.data);
        YB(A, this.sim3dHandle.data.resolution, this.options.sim3d.erosionPasses, this.sim3dHandle.data.slabBounds.zTop), VB(this.sim3dHandle.slab, A), this.sim3dHandle.currentLine = t;
      }
    }
  }
  showAll() {
    for (const Q of this.toolpathStreams)
      Q.line.geometry.setDrawRange(0, Q.totalVertices);
  }
  resetColors() {
    for (const Q of this.toolpathStreams) {
      Q.greyCursorVertex = 0, Q.simColors.set(Q.baseColors);
      const B = Q.line.geometry.getAttribute("color");
      B.clearUpdateRanges(), B.addUpdateRange(0, Q.simColors.length), B.needsUpdate = !0;
    }
  }
  snapCameraToView(Q, B = {}) {
    const t = Math.max(0, Math.floor(B.durationMs ?? 240)), A = this.controls.target.clone(), e = this.camera.position.distanceTo(A), P = Math.max(1e-6, B.distance ?? e);
    this.startSnapToView(Q, A, P, t);
  }
  /**
   * Enable or disable orbit *rotation* while leaving pan and zoom untouched.
   *
   * Used to pin the camera to a fixed view (e.g. top-down) so a press-and-hold
   * gesture is not interpreted as a rotate-drag and the picked plane stays
   * stable under the cursor. Picking (screenToWorld) works from any camera
   * orientation, so this is a UX lock, not a correctness requirement.
   */
  setRotateEnabled(Q) {
    this.controls.enableRotate = Q;
  }
  /**
   * Convert a viewport pixel (clientX/clientY, e.g. from a PointerEvent) into a
   * point on a horizontal toolpath plane, in the scene's coordinate space.
   *
   * A ray is cast from the camera through the pixel and intersected with the
   * plane Z = `planeZ`, which defaults to the bit's current Z so the pick lands
   * on the plane the tool is sitting on. Returns null when the pixel is outside
   * a laid-out canvas or the ray is parallel to the plane (no intersection).
   *
   * The result is in world/scene space. The toolpath root sits at the origin,
   * so for a normal (non-rotary) file this equals the gcode work coordinate.
   * For a rotary file the toolpath root is rotated about X, so the returned XY
   * is not a meaningful work coordinate — callers supporting only XY moves
   * should gate on file type.
   */
  screenToWorld(Q, B, t) {
    const A = this.canvas.getBoundingClientRect();
    if (A.width === 0 || A.height === 0)
      return null;
    const e = new h.Vector2(
      (Q - A.left) / A.width * 2 - 1,
      -((B - A.top) / A.height) * 2 + 1
    ), P = new h.Raycaster();
    P.setFromCamera(e, this.camera);
    const i = (t == null ? void 0 : t.planeZ) ?? this.lastBitPosition.z ?? 0, s = new h.Plane(new h.Vector3(0, 0, 1), -i), v = new h.Vector3();
    return P.ray.intersectPlane(s, v) ? { x: v.x, y: v.y, z: v.z } : null;
  }
  /**
   * Project a point in world/scene space to a viewport pixel (clientX/clientY),
   * the inverse of screenToWorld and in the same coordinate space it consumes.
   *
   * The point is projected through the camera to normalized device coordinates
   * and mapped back onto the laid-out canvas rect, so overlays drawn in fixed
   * (viewport) coordinates line up with picked scene points and track pan/zoom.
   *
   * `z` is the point's height in scene space (default 0); it changes the
   * projected pixel under the perspective camera, so callers placing markers
   * off the Z=0 plane should pass it. Returns null when the canvas has no layout.
   */
  worldToScreen(Q, B, t = 0) {
    const A = this.canvas.getBoundingClientRect();
    if (A.width === 0 || A.height === 0)
      return null;
    const e = new h.Vector3(Q, B, t).project(this.camera);
    return {
      x: A.left + (e.x + 1) / 2 * A.width,
      y: A.top + (1 - e.y) / 2 * A.height
    };
  }
  startSnapToView(Q, B, t, A) {
    const e = Xt(Q), P = B.clone().add(e.multiplyScalar(t)), i = Math.max(1, t);
    this.camera.near = i / 1e3, this.camera.far = i * 50, this.camera.updateProjectionMatrix(), this.startCameraLerp(B, P, A);
  }
  // Shared tween kickoff for any camera position+target move (view snapping,
  // model focus, tool-follow engage/track): saves/restores damping around the
  // transition and hands off to updateCameraFocusTransition() each frame.
  startCameraLerp(Q, B, t, A = "easeInOutCubic") {
    this.cameraFocusTransition && (this.controls.enableDamping = this.cameraFocusTransition.dampingEnabled), this.cameraFocusTransition = {
      startedAt: performance.now(),
      duration: t,
      fromPosition: this.camera.position.clone(),
      toPosition: B,
      fromTarget: this.controls.target.clone(),
      toTarget: Q,
      dampingEnabled: this.controls.enableDamping,
      easing: A
    }, this.controls.enableDamping = !1;
  }
  async loadFromUrl(Q, B = {}) {
    this.emitProgress({ state: "indeterminate", label: "Loading file..." });
    const t = await fetch(Q, { signal: B.signal });
    if (!t.ok)
      throw this.emitProgress({ state: "hidden" }), new Error(`Failed to load gcode: ${t.statusText}`);
    const A = await t.text();
    await this.loadFromText(A);
  }
  async loadFromFile(Q) {
    this.emitProgress({ state: "indeterminate", label: "Loading file..." });
    const B = await Q.text();
    await this.loadFromText(B);
  }
  async loadFromText(Q) {
    await this.loadFromLines(Q.split(/\r?\n/));
  }
  async loadFromLines(Q) {
    this.currentLines = Array.from(Q), await this.renderScene();
  }
  async loadFromWorkerData(Q) {
    this.currentLines = [];
    const { rapid: B, cut: t } = Ue(Q), A = (Q.toolchangeCount ?? 0) > 0;
    this.setToolpathGeometry({
      rapid: {
        positions: B.positions,
        colors: void 0,
        prefixEndVertex: B.prefixEndVertex
      },
      cuts: [{
        positions: t.positions,
        colors: A ? t.colors : void 0,
        prefixEndVertex: t.prefixEndVertex
      }],
      cutBucketCount: 1
    });
  }
  unload() {
    this.currentLines = [], this.setGeometryEmpty(), this.emitProgress({ state: "hidden" });
  }
  setOptions(Q) {
    var T, z, w, o, a, E, l, u, U, k, O, D, N, d, M, b, G, C, j, F, H, f, I, R, QQ, sQ, W, x, BQ, vQ, rB, yt, It, St;
    const B = this.options;
    this.options = hA(this.options, Q), (B.render.antialias !== this.options.render.antialias || B.render.theme.background !== this.options.render.theme.background) && this.renderer.setClearColor(this.options.render.theme.background, 1), (B.render.theme.opacity !== this.options.render.theme.opacity || B.render.theme.rapidOpacity !== this.options.render.theme.rapidOpacity) && this.refreshToolpathOpacities(), B.camera.fov !== this.options.camera.fov && (this.camera.fov = this.options.camera.fov, this.camera.updateProjectionMatrix()), B.camera.orbit.enableDamping !== this.options.camera.orbit.enableDamping && (this.controls.enableDamping = this.options.camera.orbit.enableDamping);
    const t = ((T = B.grid.bounds) == null ? void 0 : T.min.x) !== ((z = this.options.grid.bounds) == null ? void 0 : z.min.x) || ((w = B.grid.bounds) == null ? void 0 : w.min.y) !== ((o = this.options.grid.bounds) == null ? void 0 : o.min.y) || ((a = B.grid.bounds) == null ? void 0 : a.max.x) !== ((E = this.options.grid.bounds) == null ? void 0 : E.max.x) || ((l = B.grid.bounds) == null ? void 0 : l.max.y) !== ((u = this.options.grid.bounds) == null ? void 0 : u.max.y), A = B.units !== this.options.units || B.grid.sizeX !== this.options.grid.sizeX || B.grid.sizeY !== this.options.grid.sizeY || B.grid.axisDepth !== this.options.grid.axisDepth || t, e = B.render.theme.colors.grid.major !== this.options.render.theme.colors.grid.major || B.render.theme.colors.grid.minor !== this.options.render.theme.colors.grid.minor || B.render.theme.colors.axes.x !== this.options.render.theme.colors.axes.x || B.render.theme.colors.axes.y !== this.options.render.theme.colors.axes.y || B.render.theme.colors.axes.z !== this.options.render.theme.colors.axes.z;
    (A || e) && this.renderGridAndAxes(), (B.grid.labels !== this.options.grid.labels || B.units !== this.options.units || B.grid.sizeX !== this.options.grid.sizeX || B.grid.sizeY !== this.options.grid.sizeY || t || e) && this.refreshGridLabels(), (B.boundingBox.visible !== this.options.boundingBox.visible || B.boundingBox.labels !== this.options.boundingBox.labels || B.units !== this.options.units || B.render.theme.colors.boundingBox !== this.options.render.theme.colors.boundingBox) && this.refreshBoundingBox(), (B.machineBed.visible !== this.options.machineBed.visible || ((U = B.machineBed.min) == null ? void 0 : U.x) !== ((k = this.options.machineBed.min) == null ? void 0 : k.x) || ((O = B.machineBed.min) == null ? void 0 : O.y) !== ((D = this.options.machineBed.min) == null ? void 0 : D.y) || ((N = B.machineBed.max) == null ? void 0 : N.x) !== ((d = this.options.machineBed.max) == null ? void 0 : d.x) || ((M = B.machineBed.max) == null ? void 0 : M.y) !== ((b = this.options.machineBed.max) == null ? void 0 : b.y) || ((C = (G = B.machineBed.keepout) == null ? void 0 : G.min) == null ? void 0 : C.x) !== ((F = (j = this.options.machineBed.keepout) == null ? void 0 : j.min) == null ? void 0 : F.x) || ((f = (H = B.machineBed.keepout) == null ? void 0 : H.min) == null ? void 0 : f.y) !== ((R = (I = this.options.machineBed.keepout) == null ? void 0 : I.min) == null ? void 0 : R.y) || ((sQ = (QQ = B.machineBed.keepout) == null ? void 0 : QQ.max) == null ? void 0 : sQ.x) !== ((x = (W = this.options.machineBed.keepout) == null ? void 0 : W.max) == null ? void 0 : x.x) || ((vQ = (BQ = B.machineBed.keepout) == null ? void 0 : BQ.max) == null ? void 0 : vQ.y) !== ((yt = (rB = this.options.machineBed.keepout) == null ? void 0 : rB.max) == null ? void 0 : yt.y) || B.render.theme.colors.machineBed !== this.options.render.theme.colors.machineBed || B.render.theme.colors.machineBedKeepout !== this.options.render.theme.colors.machineBedKeepout) && this.refreshMachineBed();
    const v = B.render.theme.colors.rapid !== this.options.render.theme.colors.rapid || B.render.theme.colors.cutting !== this.options.render.theme.colors.cutting || B.render.theme.colors.laser !== this.options.render.theme.colors.laser || B.render.theme.background !== this.options.render.theme.background || B.render.theme.colors.processed !== this.options.render.theme.colors.processed;
    if (v && this.refreshToolpathColors(), (B.bit.enabled !== this.options.bit.enabled || B.bit.type !== this.options.bit.type || B.bit.size !== this.options.bit.size || B.bit.opacity !== this.options.bit.opacity || B.bit.colorSource !== this.options.bit.colorSource || B.bit.color !== this.options.bit.color || v) && (this.ensureBitMarker(), (It = this.bitMarker) == null || It.setOptions(this.options)), B.mode.laser !== this.options.mode.laser && (this.options.mode.laser ? this.options.bit.type !== "laser" && (this.preLaserBitType = this.options.bit.type, this.options = { ...this.options, bit: { ...this.options.bit, type: "laser" } }) : this.options = { ...this.options, bit: { ...this.options.bit, type: this.preLaserBitType } }, this.ensureBitMarker(), (St = this.bitMarker) == null || St.setOptions(this.options), this.currentLines.length > 0 && this.renderScene()), B.mode.sim3d !== this.options.mode.sim3d)
      if (!this.options.mode.sim3d)
        this.setSim3dHandle(null), this.setToolpathStreamsVisible(!0);
      else {
        this.renderScene();
        return;
      }
    if ((B.sim3d.toolDiameter !== this.options.sim3d.toolDiameter || B.sim3d.resolution !== this.options.sim3d.resolution) && this.options.mode.sim3d) {
      this.renderScene();
      return;
    }
    if (B.sim3d.erosionPasses !== this.options.sim3d.erosionPasses && this.sim3dHandle) {
      const ke = this.sim3dHandle.currentLine, Yt = SB(ke, this.sim3dHandle.data);
      YB(Yt, this.sim3dHandle.data.resolution, this.options.sim3d.erosionPasses, this.sim3dHandle.data.slabBounds.zTop), VB(this.sim3dHandle.slab, Yt);
    }
    B.sim3d.showToolpath !== this.options.sim3d.showToolpath && this.options.mode.sim3d && this.setToolpathStreamsVisible(this.options.sim3d.showToolpath);
  }
  getOptions() {
    return this.options;
  }
  resize() {
    const Q = this.container.getBoundingClientRect(), B = Math.max(
      1,
      Math.floor(Q.width || this.container.clientWidth || window.innerWidth)
    ), t = Math.max(
      1,
      Math.floor(Q.height || this.container.clientHeight || window.innerHeight)
    );
    this.renderer.setSize(B, t, !1), this.camera.aspect = B / t, this.camera.updateProjectionMatrix();
  }
  focusToModel() {
    this.currentBounds && this.startCameraFocus(this.currentBounds);
  }
  resetCamera() {
    this.cameraFocusTransition = null, this.controls.enableDamping = this.options.camera.orbit.enableDamping, this.controls.target.set(0, 0, 0), this.camera.position.set(
      this.options.camera.initialPosition.x,
      this.options.camera.initialPosition.y,
      this.options.camera.initialPosition.z
    ), this.controls.update();
  }
  getBounds() {
    return this.currentBounds ? {
      min: {
        x: this.currentBounds.min.x,
        y: this.currentBounds.min.y,
        z: this.currentBounds.min.z
      },
      max: {
        x: this.currentBounds.max.x,
        y: this.currentBounds.max.y,
        z: this.currentBounds.max.z
      }
    } : null;
  }
  dispose() {
    var Q;
    this.renderSequence += 1, this.animationFrameId !== null && (cancelAnimationFrame(this.animationFrameId), this.animationFrameId = null), this.resizeObserver && (this.resizeObserver.disconnect(), this.resizeObserver = null), this.onWindowResize && (window.removeEventListener("resize", this.onWindowResize), this.onWindowResize = null), this.setGeometryEmpty(), this.setGridGroup(null), this.setAxesGroup(null), this.setGridLabelsGroup(null), this.setBoundingBoxGroup(null), this.setMachineBedGroup(null), this.setBitMarker(null), this.controls.dispose(), this.renderer.dispose(), (Q = this.viewCube) == null || Q.dispose(), this.viewCube = null, this.container.removeChild(this.canvas);
  }
  emitProgress(Q) {
    var B, t;
    (t = (B = this.callbacks).onProgress) == null || t.call(B, { id: this.id, ...Q });
  }
  emitBoundsChanged() {
    var Q, B;
    (B = (Q = this.callbacks).onBoundsChanged) == null || B.call(Q, { id: this.id, bounds: this.getBounds() });
  }
  startAnimationLoop() {
    const Q = () => {
      var t;
      this.animationFrameId = requestAnimationFrame(Q);
      const B = performance.now();
      this.controls.update(), this.updateCameraFocusTransition(), this.updateViewCubeRotation(), (t = this.bitMarker) == null || t.update(B), this.renderer.render(this.scene, this.camera);
    };
    Q();
  }
  ensureBitMarker() {
    this.bitMarker || (this.bitMarker = bi(this.options), this.scene.add(this.bitMarker.object));
  }
  worldSizes() {
    const Q = this.options.units === "in" ? gi : 1;
    return {
      sizeXMm: Math.max(1, this.options.grid.sizeX * Q),
      sizeYMm: Math.max(1, this.options.grid.sizeY * Q),
      axisDepthMm: Math.max(1, this.options.grid.axisDepth * Q),
      // Bounds are literal absolute mm (unlike sizeX/sizeY), so they don't
      // go through the units scale round-trip above.
      bounds: this.options.grid.bounds ?? null
    };
  }
  renderGridAndAxes() {
    const { sizeXMm: Q, sizeYMm: B, axisDepthMm: t, bounds: A } = this.worldSizes();
    this.setGridGroup(
      QP({
        units: this.options.units,
        sizeXMm: Q,
        sizeYMm: B,
        bounds: A,
        theme: this.options.render.theme
      })
    ), this.setAxesGroup(
      tP({
        sizeXWorld: Q,
        sizeYWorld: B,
        depthWorld: t,
        bounds: A,
        theme: this.options.render.theme
      })
    );
  }
  refreshGridLabels() {
    if (!this.options.grid.labels) {
      this.setGridLabelsGroup(null);
      return;
    }
    const { sizeXMm: Q, sizeYMm: B, bounds: t } = this.worldSizes();
    this.setGridLabelsGroup(
      eP({ sizeXMm: Q, sizeYMm: B, bounds: t, units: this.options.units, theme: this.options.render.theme })
    );
  }
  refreshBoundingBox() {
    if (!this.options.boundingBox.visible || !this.currentBounds) {
      this.setBoundingBoxGroup(null);
      return;
    }
    this.setBoundingBoxGroup(Je(this.currentBounds, this.options));
  }
  refreshMachineBed() {
    const { visible: Q, min: B, max: t, keepout: A } = this.options.machineBed;
    if (!Q || !B || !t) {
      this.setMachineBedGroup(null);
      return;
    }
    this.setMachineBedGroup(Ze(B, t, this.options, A));
  }
  setBitMarker(Q) {
    this.bitMarker && (this.scene.remove(this.bitMarker.object), this.bitMarker.dispose()), this.bitMarker = Q, this.bitMarker && this.scene.add(this.bitMarker.object);
  }
  setGridGroup(Q) {
    this.gridGroup && (this.scene.remove(this.gridGroup), BP(this.gridGroup), this.gridGroup = null), Q && (this.gridGroup = Q, this.scene.add(Q));
  }
  setAxesGroup(Q) {
    this.axesGroup && (this.scene.remove(this.axesGroup), AP(this.axesGroup), this.axesGroup = null), Q && (this.axesGroup = Q, this.scene.add(Q));
  }
  setGridLabelsGroup(Q) {
    this.gridLabelsGroup && (this.scene.remove(this.gridLabelsGroup), Ye(this.gridLabelsGroup), this.gridLabelsGroup = null), Q && (this.gridLabelsGroup = Q, this.scene.add(Q));
  }
  setBoundingBoxGroup(Q) {
    this.boundingBoxGroup && (this.scene.remove(this.boundingBoxGroup), Xe(this.boundingBoxGroup), this.boundingBoxGroup = null), Q && (this.boundingBoxGroup = Q, this.scene.add(Q));
  }
  setMachineBedGroup(Q) {
    this.machineBedGroup && (this.scene.remove(this.machineBedGroup), We(this.machineBedGroup), this.machineBedGroup = null), Q && (this.machineBedGroup = Q, this.scene.add(Q));
  }
  setGeometryEmpty() {
    this.setSim3dHandle(null), Le(this.toolpathRoot, this.toolpathStreams), this.toolpathStreams = [], this.toolpathCutBucketCount = 1, this.linePositions = null, this.currentBounds = null, this.emitBoundsChanged(), this.refreshBoundingBox();
  }
  setToolpathGeometry(Q) {
    this.setGeometryEmpty(), this.toolpathCutBucketCount = Math.max(1, Math.floor(Q.cutBucketCount));
    const B = [
      {
        kind: "rapid",
        cutBucketIndex: null,
        positions: Q.rapid.positions,
        prefixEndVertex: Q.rapid.prefixEndVertex,
        colors: Q.rapid.colors,
        opacity: EA(this.options.render.theme.rapidOpacity ?? 0.3)
      },
      ...Q.cuts.map((e, P) => ({
        kind: "cut",
        cutBucketIndex: P,
        positions: e.positions,
        prefixEndVertex: e.prefixEndVertex,
        colors: e.colors,
        opacity: this.cutBucketOpacity(P)
      }))
    ], { streams: t, bounds: A } = Oe({
      specs: B,
      options: this.options,
      scene: this.toolpathRoot
    });
    this.toolpathStreams = t, this.currentBounds = A ? A.clone() : null, this.emitBoundsChanged(), this.refreshBoundingBox(), this.setToolpathRotationA(this.toolpathRotationA);
  }
  refreshToolpathColors() {
    Me(this.toolpathStreams, this.options);
  }
  refreshToolpathOpacities() {
    He({
      streams: this.toolpathStreams,
      options: this.options,
      cutBucketCount: this.toolpathCutBucketCount
    });
  }
  cutBucketOpacity(Q) {
    const B = this.options.render.theme.opacity, t = this.toolpathCutBucketCount;
    if (t <= 1)
      return B;
    const A = Q / (t - 1);
    return EA(B * A);
  }
  buildLinePositions() {
    const Q = this.currentLines.length;
    this.linePositions = new Float32Array(Q * 4);
    let B = 0, t = 0, A = 0, e = 0;
    const P = new et({
      onLinearMove: (i) => {
        const s = i.transformedEnd ?? i.end;
        B = s.X, t = s.Y, A = s.Z, e = i.end.A;
      },
      onArcMove: (i) => {
        const s = i.transformedEnd ?? i.end;
        B = s.X, t = s.Y, A = s.Z, e = i.end.A;
      }
    });
    for (let i = 0; i < Q; i++) {
      const s = this.currentLines[i];
      s && P.processLine(s), this.linePositions[i * 4] = B, this.linePositions[i * 4 + 1] = t, this.linePositions[i * 4 + 2] = A, this.linePositions[i * 4 + 3] = e;
    }
  }
  async renderScene() {
    if (this.currentLines.length === 0) {
      this.setGeometryEmpty(), this.emitProgress({ state: "hidden" });
      return;
    }
    const Q = this.renderSequence += 1;
    try {
      this.options.mode.laser ? this.emitProgress({ state: "indeterminate", label: "Scanning power..." }) : this.emitProgress({
        state: "determinate",
        label: "Building geometry...",
        processed: 0,
        total: this.currentLines.length
      });
      const B = await De(this.currentLines, {
        arcSegments: this.options.geometry.arcSegments,
        bucketCount: 16,
        laserMode: this.options.mode.laser,
        batch: {
          everyLines: this.options.geometry.batching.progressEveryLines,
          yieldEveryLines: this.options.geometry.batching.yieldEveryLines,
          shouldAbort: () => Q !== this.renderSequence,
          onProgress: (t, A) => {
            Q === this.renderSequence && this.emitProgress({ state: "determinate", label: "Building geometry...", processed: t, total: A });
          }
        }
      });
      if (Q !== this.renderSequence)
        return;
      this.setToolpathGeometry({
        rapid: B.rapid,
        cuts: B.cuts,
        cutBucketCount: B.cutBucketCount
      }), this.buildLinePositions(), this.options.mode.sim3d && (this.setToolpathStreamsVisible(this.options.sim3d.showToolpath), await this.buildAndApplySim3d(Q));
    } catch (B) {
      if (B instanceof Error && B.message === "Aborted.")
        return;
      throw B;
    } finally {
      Q === this.renderSequence && this.emitProgress({ state: "hidden" });
    }
  }
  setSim3dHandle(Q) {
    this.sim3dHandle && (this.scene.remove(this.sim3dHandle.slab.group), xe(this.sim3dHandle.slab), this.sim3dHandle = null), Q && (this.sim3dHandle = Q, this.scene.add(Q.slab.group));
  }
  setToolpathStreamsVisible(Q) {
    for (const B of this.toolpathStreams)
      B.line.visible = Q;
  }
  async buildAndApplySim3d(Q) {
    this.emitProgress({ state: "indeterminate", label: "Building 3D simulation..." });
    const B = this.options.sim3d.toolDiameter / 2, t = this.options.sim3d.resolution;
    let A;
    try {
      A = await be(this.currentLines, B, t, {
        arcSegments: this.options.geometry.arcSegments,
        batch: {
          shouldAbort: () => Q !== this.renderSequence,
          onProgress: (i, s) => {
            Q === this.renderSequence && this.emitProgress({
              state: "determinate",
              label: "Building 3D simulation...",
              processed: i,
              total: s
            });
          },
          yieldEveryLines: this.options.geometry.batching.yieldEveryLines
        }
      });
    } catch {
      return;
    }
    if (Q !== this.renderSequence) return;
    const e = SB(0, A);
    YB(e, t, this.options.sim3d.erosionPasses, A.slabBounds.zTop);
    const P = me(A.slabBounds, t);
    VB(P, e), this.setSim3dHandle({ data: A, slab: P, currentLine: 0 });
  }
  startCameraFocus(Q) {
    const B = new h.Vector3();
    Q.getCenter(B);
    const t = new h.Vector3();
    Q.getSize(t);
    const A = Math.max(t.x, t.y, 1) / 2, e = h.MathUtils.degToRad(this.camera.fov), P = A / Math.tan(e / 2) * 1.25 + t.z, i = Xt("front-top-left"), s = B.clone().add(i.multiplyScalar(P)), v = Math.max(t.x, t.y, t.z, 1);
    this.camera.near = v / 1e3, this.camera.far = v * 50, this.camera.updateProjectionMatrix(), this.startCameraLerp(B, s, this.options.camera.focusDurationMs);
  }
  updateCameraFocusTransition() {
    if (!this.cameraFocusTransition)
      return;
    const B = performance.now() - this.cameraFocusTransition.startedAt, t = Math.min(1, Math.max(0, B / this.cameraFocusTransition.duration)), A = this.cameraFocusTransition.easing === "linear" ? t : _e(t);
    if (this.camera.position.lerpVectors(this.cameraFocusTransition.fromPosition, this.cameraFocusTransition.toPosition, A), this.controls.target.lerpVectors(this.cameraFocusTransition.fromTarget, this.cameraFocusTransition.toTarget, A), t >= 1) {
      const e = this.cameraFocusTransition.dampingEnabled;
      this.camera.position.copy(this.cameraFocusTransition.toPosition), this.controls.target.copy(this.cameraFocusTransition.toTarget), this.cameraFocusTransition = null, this.controls.enableDamping = !1, this.controls.update(), this.controls.enableDamping = e;
    }
  }
  updateViewCubeRotation() {
    if (!this.viewCube)
      return;
    const Q = this.camera.quaternion.clone();
    typeof Q.invert == "function" ? Q.invert() : typeof Q.inverse == "function" && Q.inverse();
    const B = new h.Matrix4();
    B.makeRotationFromQuaternion(Q), B.multiply(this.viewCubeCorrection), this.viewCube.setRotationMatrix3d(B.elements);
    const t = $e(this.camera.position, this.controls.target);
    this.viewCube.setActiveFace(t);
  }
}
function EA(r) {
  return Number.isFinite(r) ? Math.max(0, Math.min(1, r)) : 0;
}
function hA(r, Q) {
  var t, A, e, P;
  if (!Q)
    return { ...r };
  const B = xi(r.render.theme, (t = Q.render) == null ? void 0 : t.theme);
  return {
    ...r,
    ...Q,
    mode: { ...r.mode, ...Q.mode },
    sim3d: { ...r.sim3d, ...Q.sim3d },
    bit: { ...r.bit, ...Q.bit },
    progress: { ...r.progress, ...Q.progress },
    grid: { ...r.grid, ...Q.grid },
    boundingBox: { ...r.boundingBox, ...Q.boundingBox },
    machineBed: { ...r.machineBed, ...Q.machineBed },
    geometry: {
      ...r.geometry,
      ...Q.geometry,
      batching: { ...r.geometry.batching, ...(A = Q.geometry) == null ? void 0 : A.batching }
    },
    render: {
      ...r.render,
      ...Q.render,
      theme: B
    },
    camera: {
      ...r.camera,
      ...Q.camera,
      orbit: { ...r.camera.orbit, ...(e = Q.camera) == null ? void 0 : e.orbit },
      initialPosition: { ...r.camera.initialPosition, ...(P = Q.camera) == null ? void 0 : P.initialPosition }
    }
  };
}
function xi(r, Q) {
  var B, t;
  return Q ? {
    ...r,
    ...Q,
    colors: {
      ...r.colors,
      ...Q.colors,
      grid: { ...r.colors.grid, ...(B = Q.colors) == null ? void 0 : B.grid },
      axes: { ...r.colors.axes, ...(t = Q.colors) == null ? void 0 : t.axes }
    }
  } : r;
}
const yi = {
  rapidColor: "#0ef6ae",
  cutColor: "#3e85c7",
  boundingBoxColor: "#d0d0d0",
  strokeWidth: 0.5,
  arcSegments: 30,
  padding: 5,
  projectionMode: "isometric",
  showOrigin: !0,
  originColor: "#ffffff",
  crosshairColor: "#ffffff"
}, vB = 0, cB = 0;
class Ki {
  constructor(Q, B) {
    this.crosshairPos = null, this.crosshairVisible = !1, this.pathEls = [], this.segmentGroups = [], this.workerMode = !1, this.viewBox = { x: 0, y: 0, w: 100, h: 100 }, this.rotX = vB, this.rotY = cB, this.centerX = 0, this.centerY = 0, this.centerZ = 0, this.focalLength = 500, this.cosRotX = 1, this.sinRotX = 0, this.cosRotY = 1, this.sinRotY = 0, this.dragMode = "none", this.activePointers = /* @__PURE__ */ new Map(), this.pinchLastDist = 0, this.pinchLastMid = { x: 0, y: 0 }, this.rafPending = !1, this.overlayRafPending = !1, this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.bounds = { minX: 0, minY: 0, maxX: 0, maxY: 0, minZ: 0, maxZ: 0, empty: !0 }, this.hasZInfo = !0, this.project = (t, A, e) => {
      if (this.options.projectionMode === "top")
        return { x: t, y: -A };
      const P = t - this.centerX, i = A - this.centerY, s = e - this.centerZ, v = P * this.cosRotY + s * this.sinRotY, c = i, n = -P * this.sinRotY + s * this.cosRotY, T = v, z = c * this.cosRotX - n * this.sinRotX, w = c * this.sinRotX + n * this.cosRotX;
      if (this.options.projectionMode === "perspective") {
        const o = this.focalLength / (this.focalLength + w);
        return { x: T * o, y: -z * o };
      }
      return { x: T, y: -z };
    }, this.onWheel = (t) => {
      t.preventDefault();
      const A = t.deltaY > 0 ? 1.1 : 1 / 1.1, e = this.svgPoint(t.clientX, t.clientY), { x: P, y: i, w: s, h: v } = this.viewBox;
      this.viewBox = {
        x: e.x - (e.x - P) * A,
        y: e.y - (e.y - i) * A,
        w: s * A,
        h: v * A
      }, this.applyViewBox();
    }, this.onPointerDown = (t) => {
      if (this.activePointers.set(t.pointerId, { x: t.clientX, y: t.clientY }), this.svg.setPointerCapture(t.pointerId), t.preventDefault(), this.activePointers.size === 1)
        this.dragMode = "pan", this.svg.style.cursor = "grabbing";
      else if (this.activePointers.size === 2) {
        this.dragMode = "pan", this.svg.style.cursor = "move";
        const [A, e] = Array.from(this.activePointers.values());
        this.pinchLastDist = Math.hypot(e.x - A.x, e.y - A.y), this.pinchLastMid = { x: (A.x + e.x) / 2, y: (A.y + e.y) / 2 };
      }
    }, this.onPointerMove = (t) => {
      if (!this.activePointers.has(t.pointerId)) return;
      const A = this.activePointers.get(t.pointerId);
      if (this.activePointers.set(t.pointerId, { x: t.clientX, y: t.clientY }), this.activePointers.size === 2) {
        const [i, s] = Array.from(this.activePointers.values()), v = Math.hypot(s.x - i.x, s.y - i.y), c = { x: (i.x + s.x) / 2, y: (i.y + s.y) / 2 };
        if (this.pinchLastDist > 0) {
          const w = this.pinchLastDist / v, o = this.svgPoint(c.x, c.y), { x: a, y: E, w: l, h: u } = this.viewBox;
          this.viewBox = {
            x: o.x - (o.x - a) * w,
            y: o.y - (o.y - E) * w,
            w: l * w,
            h: u * w
          };
        }
        const n = this.svg.getBoundingClientRect(), T = c.x - this.pinchLastMid.x, z = c.y - this.pinchLastMid.y;
        this.viewBox = {
          ...this.viewBox,
          x: this.viewBox.x - T / n.width * this.viewBox.w,
          y: this.viewBox.y - z / n.height * this.viewBox.h
        }, this.pinchLastDist = v, this.pinchLastMid = c, this.applyViewBox();
        return;
      }
      if (this.dragMode === "none") return;
      const e = t.clientX - A.x, P = t.clientY - A.y;
      if (this.dragMode === "orbit") {
        const i = this.svg.getBoundingClientRect(), s = Math.PI / Math.min(i.width, i.height);
        this.rotY += e * s, this.rotX += P * s, this.rotX = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, this.rotX)), this.updateTrig(), this.fitView(), this.scheduleDraw();
      } else {
        const i = this.svg.getBoundingClientRect();
        this.viewBox = {
          ...this.viewBox,
          x: this.viewBox.x - e / i.width * this.viewBox.w,
          y: this.viewBox.y - P / i.height * this.viewBox.h
        }, this.applyViewBox();
      }
    }, this.onPointerUp = (t) => {
      if (this.activePointers.delete(t.pointerId), this.svg.releasePointerCapture(t.pointerId), this.activePointers.size === 1) {
        const [A] = this.activePointers.values();
        this.pinchLastMid = { x: A.x, y: A.y }, this.dragMode = "pan", this.svg.style.cursor = "grabbing";
      } else this.activePointers.size === 0 && (this.dragMode = "none", this.svg.style.cursor = "grab");
    }, this.onContextMenu = (t) => {
      t.preventDefault();
    }, this.options = { ...yi, ...B }, this.svg = document.createElementNS("http://www.w3.org/2000/svg", "svg"), this.svg.style.cssText = "width:100%;height:100%;display:block;cursor:grab;user-select:none;touch-action:none;", this.svg.setAttribute("preserveAspectRatio", "xMidYMid meet"), this.svg.setAttribute("xmlns", "http://www.w3.org/2000/svg"), this.bboxPath = Qt(), this.pathLayer = document.createElementNS("http://www.w3.org/2000/svg", "g"), this.bboxLabelX = Bt("middle", "hanging"), this.bboxLabelY = Bt("start", "middle"), this.bboxLabelZ = Bt("middle", "auto"), this.originMarker = document.createElementNS("http://www.w3.org/2000/svg", "circle"), this.originMarker.setAttribute("stroke-opacity", "0.4"), this.originMarker.setAttribute("visibility", "hidden"), this.crosshairEl = Qt(), this.crosshairEl.setAttribute("fill", "none"), this.crosshairEl.setAttribute("visibility", "hidden"), this.svg.appendChild(this.bboxPath), this.svg.appendChild(this.pathLayer), this.svg.appendChild(this.bboxLabelX), this.svg.appendChild(this.bboxLabelY), this.svg.appendChild(this.bboxLabelZ), this.svg.appendChild(this.originMarker), this.svg.appendChild(this.crosshairEl), Q.appendChild(this.svg), this.applyOptions(), this.bindEvents();
  }
  // ── Public API ────────────────────────────────────────────────────────────
  loadFromLines(Q) {
    const { rapid: B, cutting: t } = de(Q, {
      arcSegments: this.options.arcSegments
    });
    if (this.rapidVerts = B, this.cutVerts = t, this.workerMode = !1, this.syncSegmentGroupsFromLines(), this.hasZInfo = !0, this.bounds = At([
      { verts: B, stride: 6 },
      { verts: t, stride: 6 }
    ]), !this.bounds.empty) {
      this.centerX = (this.bounds.minX + this.bounds.maxX) / 2, this.centerY = (this.bounds.minY + this.bounds.maxY) / 2, this.centerZ = (this.bounds.minZ + this.bounds.maxZ) / 2;
      const A = Math.hypot(
        this.bounds.maxX - this.bounds.minX,
        this.bounds.maxY - this.bounds.minY,
        this.bounds.maxZ - this.bounds.minZ
      );
      this.focalLength = A * 2;
    }
    this.rotX = vB, this.rotY = cB, this.updateTrig(), this.fitView(), this.rebuildAndRender();
  }
  loadFromFile(Q) {
    return new Promise((B, t) => {
      const A = new FileReader();
      A.onload = (e) => {
        var i;
        const P = (i = e.target) == null ? void 0 : i.result;
        if (typeof P != "string") {
          t(new Error("Failed to read file"));
          return;
        }
        this.loadFromText(P), B();
      }, A.onerror = () => t(new Error("FileReader error")), A.readAsText(Q);
    });
  }
  loadFromText(Q) {
    this.loadFromLines(Q.split(/\r?\n/));
  }
  clear() {
    this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.workerMode = !1, this.segmentGroups = [], this.hasZInfo = !0, this.bounds = { minX: 0, minY: 0, maxX: 0, maxY: 0, minZ: 0, maxZ: 0, empty: !0 }, this.rebuildAndRender();
  }
  loadFromWorkerData(Q) {
    const B = Ce(Q);
    if (this.workerMode = !0, this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.segmentGroups = B.map((t) => ({
      color: t.hexColor,
      opacity: t.opacity,
      verts: t.positions,
      stride: 6
    })), this.hasZInfo = !0, this.bounds = At(B.map((t) => ({ verts: t.positions, stride: 6 }))), !this.bounds.empty) {
      this.centerX = (this.bounds.minX + this.bounds.maxX) / 2, this.centerY = (this.bounds.minY + this.bounds.maxY) / 2, this.centerZ = (this.bounds.minZ + this.bounds.maxZ) / 2;
      const t = Math.hypot(
        this.bounds.maxX - this.bounds.minX,
        this.bounds.maxY - this.bounds.minY,
        this.bounds.maxZ - this.bounds.minZ
      );
      this.focalLength = t * 2;
    }
    this.rotX = vB, this.rotY = cB, this.updateTrig(), this.fitView(), this.rebuildAndRender();
  }
  loadFromPrecomputedGroups(Q, B) {
    this.workerMode = !0, this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.segmentGroups = Q.map((A) => ({
      color: A.hexColor,
      opacity: A.opacity,
      verts: new Float32Array(A.positionsBuffer, 0, A.positionsLen),
      stride: A.stride ?? 6
    })), this.bounds = At(this.segmentGroups);
    const t = (B == null ? void 0 : B.minZ) !== void 0 && (B == null ? void 0 : B.maxZ) !== void 0;
    if (t && !this.bounds.empty && (this.bounds.minZ = B.minZ, this.bounds.maxZ = B.maxZ), this.hasZInfo = t || this.segmentGroups.every((A) => A.stride === 6), !this.bounds.empty) {
      this.centerX = (this.bounds.minX + this.bounds.maxX) / 2, this.centerY = (this.bounds.minY + this.bounds.maxY) / 2, this.centerZ = (this.bounds.minZ + this.bounds.maxZ) / 2;
      const A = Math.hypot(
        this.bounds.maxX - this.bounds.minX,
        this.bounds.maxY - this.bounds.minY,
        this.bounds.maxZ - this.bounds.minZ
      );
      this.focalLength = A * 2;
    }
    this.rotX = vB, this.rotY = cB, this.updateTrig(), this.fitView(), this.rebuildAndRender();
  }
  syncSegmentGroupsFromLines() {
    this.segmentGroups = [
      { color: this.options.rapidColor, opacity: 0.5, verts: this.rapidVerts, stride: 6 },
      { color: this.options.cutColor, opacity: 1, verts: this.cutVerts, stride: 6 }
    ];
  }
  resetView() {
    this.rotX = vB, this.rotY = cB, this.updateTrig(), this.fitView(), this.rebuildAndRender();
  }
  setOptions(Q) {
    this.options = { ...this.options, ...Q }, this.applyOptions(), this.rebuildAndRender();
  }
  setProjectionMode(Q) {
    this.options = { ...this.options, projectionMode: Q }, this.rebuildAndRender();
  }
  getSVGElement() {
    return this.svg;
  }
  setBitPosition(Q) {
    this.crosshairPos = Q, this.crosshairVisible = !0, this.scheduleOverlayDraw();
  }
  setBitVisible(Q) {
    this.crosshairVisible = Q, this.scheduleOverlayDraw();
  }
  dispose() {
    this.svg.removeEventListener("wheel", this.onWheel), this.svg.removeEventListener("pointerdown", this.onPointerDown), this.svg.removeEventListener("pointermove", this.onPointerMove), this.svg.removeEventListener("pointerup", this.onPointerUp), this.svg.removeEventListener("pointercancel", this.onPointerUp), this.svg.removeEventListener("contextmenu", this.onContextMenu), this.svg.remove();
  }
  // ── Projection ────────────────────────────────────────────────────────────
  updateTrig() {
    this.cosRotX = Math.cos(this.rotX), this.sinRotX = Math.sin(this.rotX), this.cosRotY = Math.cos(this.rotY), this.sinRotY = Math.sin(this.rotY);
  }
  // ── Rendering ─────────────────────────────────────────────────────────────
  rebuildAndRender(Q = !1) {
    this.rebuildToolpaths(Q), this.renderOverlays();
  }
  // Expensive: re-projects and re-serialises every vertex into SVG path `d`
  // strings. Only call when geometry, projection, or stroke width changes —
  // never for bit-position/crosshair updates (use scheduleOverlayDraw instead).
  rebuildToolpaths(Q = !1) {
    const B = Q ? Ii : L, t = String(this.options.strokeWidth), A = this.simplifyTolerance();
    for (; this.pathEls.length < this.segmentGroups.length; ) {
      const e = Qt();
      this.pathLayer.appendChild(e), this.pathEls.push(e);
    }
    for (; this.pathEls.length > this.segmentGroups.length; )
      this.pathLayer.removeChild(this.pathEls.pop());
    for (let e = 0; e < this.segmentGroups.length; e++) {
      const { color: P, opacity: i, verts: s, stride: v } = this.segmentGroups[e], c = this.pathEls[e];
      c.setAttribute("stroke", P), c.setAttribute("stroke-opacity", String(i)), c.setAttribute("stroke-width", t), c.setAttribute("stroke-linecap", "round"), c.setAttribute("stroke-linejoin", "round"), c.setAttribute(
        "d",
        v === 4 ? Si(s, B, A) : Yi(s, this.project, B, A)
      );
    }
  }
  // Cheap O(1) overlays that don't depend on toolpath geometry.
  renderOverlays() {
    this.renderBbox(), this.renderOriginMarker(), this.renderCrosshairMarker(), this.applyViewBox();
  }
  // World-space distance below which consecutive projected points are merged.
  // Derived from the bounds diagonal so it scales with model size and stays
  // deterministic (independent of zoom / layout timing).
  simplifyTolerance() {
    return this.bounds.empty ? 0 : Math.hypot(
      this.bounds.maxX - this.bounds.minX,
      this.bounds.maxY - this.bounds.minY,
      this.bounds.maxZ - this.bounds.minZ
    ) / 1500;
  }
  scheduleDraw() {
    this.rafPending || (this.rafPending = !0, requestAnimationFrame(() => {
      this.rafPending = !1, this.rebuildAndRender(!0);
    }));
  }
  // rAF-coalesced redraw of just the overlays (bbox/origin/crosshair/viewBox).
  // Used for bit-position updates so a running job never rebuilds the toolpath.
  scheduleOverlayDraw() {
    this.overlayRafPending || (this.overlayRafPending = !0, requestAnimationFrame(() => {
      this.overlayRafPending = !1, this.renderOverlays();
    }));
  }
  fitView() {
    if (this.bounds.empty) {
      this.viewBox = { x: -50, y: -50, w: 100, h: 100 };
      return;
    }
    const { minX: Q, maxX: B, minY: t, maxY: A, minZ: e, maxZ: P } = this.bounds, i = this.project, s = i(Q, t, e), v = i(B, t, e), c = i(B, A, e), n = i(Q, A, e), T = i(Q, t, P), z = i(B, t, P), w = i(B, A, P), o = i(Q, A, P), a = Math.hypot(v.x - s.x, v.y - s.y), E = Math.hypot(n.x - s.x, n.y - s.y), l = Math.hypot(T.x - s.x, T.y - s.y), u = Math.max(a, E, l) * 0.07, U = u * 0.5, k = tt(LQ(s, v), LQ(c, n), U), O = [s, v, c, n, T, z, w, o], D = Math.min(...O.map((x) => x.y)), N = Math.max(...O.map((x) => x.y)), d = Math.min(...O.map((x) => x.x)), M = Math.max(...O.map((x) => x.x)), b = { x: M + U, y: (D + N) / 2 }, G = { x: b.x + u * 5, y: b.y }, C = { x: (d + M) / 2, y: D - u }, j = [s, v, c, n, T, z, w, o, k, b, G, C];
    let F = 1 / 0, H = 1 / 0, f = -1 / 0, I = -1 / 0;
    for (const x of j)
      x.x < F && (F = x.x), x.y < H && (H = x.y), x.x > f && (f = x.x), x.y > I && (I = x.y);
    const R = f - F, QQ = I - H, sQ = Math.max(R, QQ) * 0.08, W = this.options.padding + sQ;
    this.viewBox = {
      x: F - W,
      y: H - W,
      w: R + W * 2,
      h: QQ + W * 2
    };
  }
  applyViewBox() {
    const { x: Q, y: B, w: t, h: A } = this.viewBox;
    this.svg.setAttribute("viewBox", `${L(Q)} ${L(B)} ${L(t)} ${L(A)}`);
  }
  applyOptions() {
    const { boundingBoxColor: Q, strokeWidth: B, originColor: t } = this.options;
    this.workerMode || this.syncSegmentGroupsFromLines(), this.bboxPath.setAttribute("stroke", Q), this.bboxPath.setAttribute("stroke-width", String(B * 0.6)), this.bboxPath.setAttribute("fill", Q), this.bboxPath.setAttribute("fill-opacity", "0.05");
    for (const A of [this.bboxLabelX, this.bboxLabelY, this.bboxLabelZ])
      A.setAttribute("fill", Q);
    this.originMarker.setAttribute("fill", t), this.originMarker.setAttribute("stroke", "#000000"), this.crosshairEl.setAttribute("stroke", this.options.crosshairColor);
  }
  renderBbox() {
    if (this.bounds.empty) {
      this.bboxPath.setAttribute("d", ""), this.bboxLabelX.setAttribute("visibility", "hidden"), this.bboxLabelY.setAttribute("visibility", "hidden"), this.bboxLabelZ.setAttribute("visibility", "hidden");
      return;
    }
    const { minX: Q, maxX: B, minY: t, maxY: A, minZ: e, maxZ: P } = this.bounds, i = this.project;
    if (this.options.projectionMode === "top") {
      const C = i(Q, t, 0), j = i(B, t, 0), F = i(B, A, 0), H = i(Q, A, 0);
      this.bboxPath.setAttribute(
        "d",
        `M${L(C.x)} ${L(C.y)}L${L(j.x)} ${L(j.y)}L${L(F.x)} ${L(F.y)}L${L(H.x)} ${L(H.y)}Z`
      );
      const f = Math.abs(j.x - C.x), I = Math.abs(H.y - C.y), R = Math.max(f, I) * 0.07, QQ = R * 0.5;
      ZQ(this.bboxLabelX, LQ(C, j), tt(LQ(C, j), LQ(F, H), QQ), R, `X: ${WQ(B - Q)}`);
      const sQ = Math.min(C.y, j.y, F.y, H.y), W = Math.max(C.y, j.y, F.y, H.y), x = Math.max(C.x, j.x, F.x, H.x), BQ = Math.min(C.x, j.x, F.x, H.x), vQ = { x: x + QQ, y: (sQ + W) / 2 };
      if (ZQ(this.bboxLabelY, vQ, vQ, R, `Y: ${WQ(A - t)}`), this.hasZInfo) {
        const rB = { x: (BQ + x) / 2, y: sQ - R };
        ZQ(this.bboxLabelZ, rB, rB, R, `Z: ${WQ(P - e)}`);
      } else
        this.bboxLabelZ.setAttribute("visibility", "hidden");
      return;
    }
    const s = i(Q, t, e), v = i(B, t, e), c = i(B, A, e), n = i(Q, A, e), T = i(Q, t, P), z = i(B, t, P), w = i(B, A, P), o = i(Q, A, P), a = [
      `M${L(s.x)} ${L(s.y)}L${L(v.x)} ${L(v.y)}L${L(c.x)} ${L(c.y)}L${L(n.x)} ${L(n.y)}Z`,
      `M${L(T.x)} ${L(T.y)}L${L(z.x)} ${L(z.y)}L${L(w.x)} ${L(w.y)}L${L(o.x)} ${L(o.y)}Z`,
      `M${L(s.x)} ${L(s.y)}L${L(T.x)} ${L(T.y)}`,
      `M${L(v.x)} ${L(v.y)}L${L(z.x)} ${L(z.y)}`,
      `M${L(c.x)} ${L(c.y)}L${L(w.x)} ${L(w.y)}`,
      `M${L(n.x)} ${L(n.y)}L${L(o.x)} ${L(o.y)}`
    ].join("");
    this.bboxPath.setAttribute("d", a);
    const E = Math.hypot(v.x - s.x, v.y - s.y), l = Math.hypot(n.x - s.x, n.y - s.y), u = Math.hypot(T.x - s.x, T.y - s.y), U = Math.max(E, l, u) * 0.07, k = U * 0.5;
    ZQ(this.bboxLabelX, LQ(s, v), tt(LQ(s, v), LQ(c, n), k), U, `X: ${WQ(B - Q)}`);
    const O = [s, v, c, n, T, z, w, o], D = Math.min(...O.map((C) => C.y)), N = Math.max(...O.map((C) => C.y)), d = Math.min(...O.map((C) => C.x)), M = Math.max(...O.map((C) => C.x)), b = { x: M + k, y: (D + N) / 2 };
    ZQ(this.bboxLabelY, b, b, U, `Y: ${WQ(A - t)}`);
    const G = { x: (d + M) / 2, y: D - U };
    ZQ(this.bboxLabelZ, G, G, U, `Z: ${WQ(P - e)}`);
  }
  renderOriginMarker() {
    if (!this.options.showOrigin || this.bounds.empty) {
      this.originMarker.setAttribute("visibility", "hidden");
      return;
    }
    const Q = this.project(0, 0, 0), B = Math.min(this.viewBox.w, this.viewBox.h) * 0.018;
    this.originMarker.setAttribute("cx", L(Q.x)), this.originMarker.setAttribute("cy", L(Q.y)), this.originMarker.setAttribute("r", L(B)), this.originMarker.setAttribute("stroke-width", L(B * 0.25)), this.originMarker.setAttribute("visibility", "visible");
  }
  renderCrosshairMarker() {
    if (!this.crosshairVisible || this.crosshairPos === null) {
      this.crosshairEl.setAttribute("visibility", "hidden");
      return;
    }
    const { x: Q, y: B, z: t } = this.crosshairPos, A = this.project(Q, B, t), e = Math.min(this.viewBox.w, this.viewBox.h) * 0.025, P = e * 0.25, i = A.x, s = A.y, v = [
      `M${L(i - e)} ${L(s)}L${L(i - P)} ${L(s)}`,
      `M${L(i + P)} ${L(s)}L${L(i + e)} ${L(s)}`,
      `M${L(i)} ${L(s - e)}L${L(i)} ${L(s - P)}`,
      `M${L(i)} ${L(s + P)}L${L(i)} ${L(s + e)}`
    ].join("");
    this.crosshairEl.setAttribute("d", v), this.crosshairEl.setAttribute("stroke-width", L(this.options.strokeWidth * 2)), this.crosshairEl.setAttribute("stroke-linecap", "round"), this.crosshairEl.setAttribute("visibility", "visible");
  }
  bindEvents() {
    this.svg.addEventListener("wheel", this.onWheel, { passive: !1 }), this.svg.addEventListener("pointerdown", this.onPointerDown), this.svg.addEventListener("pointermove", this.onPointerMove), this.svg.addEventListener("pointerup", this.onPointerUp), this.svg.addEventListener("pointercancel", this.onPointerUp), this.svg.addEventListener("contextmenu", this.onContextMenu);
  }
  svgPoint(Q, B) {
    const t = this.svg.getBoundingClientRect(), { x: A, y: e, w: P, h: i } = this.viewBox;
    return {
      x: A + (Q - t.left) / t.width * P,
      y: e + (B - t.top) / t.height * i
    };
  }
}
function Qt() {
  const r = document.createElementNS("http://www.w3.org/2000/svg", "path");
  return r.setAttribute("fill", "none"), r;
}
function Bt(r, Q) {
  const B = document.createElementNS("http://www.w3.org/2000/svg", "text");
  return B.setAttribute("text-anchor", r), B.setAttribute("dominant-baseline", Q), B;
}
function ZQ(r, Q, B, t, A) {
  r.setAttribute("x", L(B.x)), r.setAttribute("y", L(B.y)), r.setAttribute("font-size", L(t)), r.textContent = A, r.setAttribute("visibility", "visible");
}
function LQ(r, Q) {
  return { x: (r.x + Q.x) / 2, y: (r.y + Q.y) / 2 };
}
function tt(r, Q, B) {
  const t = r.x - Q.x, A = r.y - Q.y, e = Math.hypot(t, A) || 1;
  return { x: r.x + t / e * B, y: r.y + A / e * B };
}
function L(r) {
  return r.toFixed(2);
}
function Ii(r) {
  return Math.round(r) + "";
}
function WQ(r) {
  return r.toFixed(2);
}
function Si(r, Q, B = 0) {
  if (r.length === 0) return "";
  const t = [], A = 1e-6, e = B * B;
  let P = NaN, i = NaN, s = NaN, v = NaN, c = NaN, n = NaN, T = !1;
  for (let z = 0; z + 3 < r.length; z += 4) {
    const w = r[z], o = -r[z + 1], a = r[z + 2], E = -r[z + 3];
    if (Math.abs(w - P) < A && Math.abs(o - i) < A) {
      const l = a - s, u = E - v;
      e > 0 && l * l + u * u < e ? (c = a, n = E, T = !0) : (t.push(`L${Q(a)} ${Q(E)}`), s = a, v = E, T = !1);
    } else
      T && (t.push(`L${Q(c)} ${Q(n)}`), T = !1), t.push(`M${Q(w)} ${Q(o)}L${Q(a)} ${Q(E)}`), s = a, v = E;
    P = a, i = E;
  }
  return T && t.push(`L${Q(c)} ${Q(n)}`), t.join("");
}
function Yi(r, Q, B, t = 0) {
  if (r.length === 0) return "";
  const A = [], e = 1e-6, P = t * t;
  let i = NaN, s = NaN, v = NaN, c = NaN, n = NaN, T = NaN, z = !1;
  for (let w = 0; w + 5 < r.length; w += 6) {
    const o = Q(r[w], r[w + 1], r[w + 2]), a = Q(r[w + 3], r[w + 4], r[w + 5]);
    if (Math.abs(o.x - i) < e && Math.abs(o.y - s) < e) {
      const E = a.x - v, l = a.y - c;
      P > 0 && E * E + l * l < P ? (n = a.x, T = a.y, z = !0) : (A.push(`L${B(a.x)} ${B(a.y)}`), v = a.x, c = a.y, z = !1);
    } else
      z && (A.push(`L${B(n)} ${B(T)}`), z = !1), A.push(`M${B(o.x)} ${B(o.y)}L${B(a.x)} ${B(a.y)}`), v = a.x, c = a.y;
    i = a.x, s = a.y;
  }
  return z && A.push(`L${B(n)} ${B(T)}`), A.join("");
}
function At(r) {
  let Q = 1 / 0, B = 1 / 0, t = 1 / 0, A = -1 / 0, e = -1 / 0, P = -1 / 0, i = !0, s = !1;
  for (const { verts: v, stride: c } of r)
    if (c === 4)
      for (let n = 0; n + 3 < v.length; n += 4) {
        const T = v[n], z = v[n + 1], w = v[n + 2], o = v[n + 3];
        T < Q && (Q = T), z < B && (B = z), T > A && (A = T), z > e && (e = z), w < Q && (Q = w), o < B && (B = o), w > A && (A = w), o > e && (e = o), i = !1;
      }
    else
      for (let n = 0; n + 5 < v.length; n += 6) {
        const T = v[n], z = v[n + 1], w = v[n + 2], o = v[n + 3], a = v[n + 4], E = v[n + 5];
        T < Q && (Q = T), z < B && (B = z), w < t && (t = w), T > A && (A = T), z > e && (e = z), w > P && (P = w), o < Q && (Q = o), a < B && (B = a), E < t && (t = E), o > A && (A = o), a > e && (e = a), E > P && (P = E), i = !1, s = !0;
      }
  return !i && !s && (t = 0, P = 0), { minX: Q, minY: B, maxX: A, maxY: e, minZ: t, maxZ: P, empty: i };
}
export {
  Ki as G,
  Se as V,
  Xi as a,
  Vt as b,
  Ge as d
};
