...
import { GCodeVirtualizer as rB, buildWorkerToolpathStreams as Me, buildToolpathGeometryFromLinesBatched as He, buildMovementVerticesFromLines as je, buildWorkerSegmentGroups as Fe } from "./gviewer.js";
import { OrbitControls as ZB } from "three/examples/jsm/controls/OrbitControls.js";
import { STLLoader as fe } from "three/examples/jsm/loaders/STLLoader.js";
function Ge(i) {
  const Q = [];
  let t = null;
  for (const B of i.specs) {
    if (B.positions.length === 0 || B.opacity <= 0)
      continue;
    const A = B.positions.length / 3, e = B.colors ? B.colors.slice() : LA(B.kind, A, i.options), P = new Float32Array(e.length);
    P.set(e);
    const r = new l.BufferGeometry();
    r.setAttribute("position", new l.BufferAttribute(B.positions, 3));
    const s = new l.BufferAttribute(P, 3);
    ...
    }), r.boundingBox && (t = t ? t.union(r.boundingBox) : r.boundingBox.clone());
  }
  return { streams: Q, bounds: t };
}
function be(i, Q) {
  for (const t of Q)
    i.remove(t.line), t.line.geometry.dispose(), t.line.material.dispose();
}
function pe(i, Q) {
  for (const t of i) {
    if (t.workerColors) continue;
    const B = LA(t.kind, t.totalVertices, Q);
    t.baseColors = B, t.simColors.set(B), t.greyCursorVertex = 0;
    const A = t.line.geometry.getAttribute("color");
    A.clearUpdateRanges(), A.addUpdateRange(0, t.simColors.length), A.needsUpdate = !0;
  }
}
function ge(i) {
  const Q = sB(i.options.render.theme.rapidOpacity ?? 0.3);
  for (const t of i.streams) {
    const B = t.line.material;
    if (t.kind === "rapid") {
      B.opacity = Q;
      continue;
    }
    B.opacity = ye({
      bucketIndex: t.cutBucketIndex ?? 0,
      bucketCount: i.cutBucketCount,
      baseOpacity: i.options.render.theme.opacity
    });
  }
}
function me(i) {
  const Q = i.stream.totalVertices, t = Math.max(0, Math.min(Q, Math.floor(i.nextCursorVertex))), B = Math.max(0, Math.min(Q, Math.floor(i.stream.greyCursorVertex)));
  if (t === B)
    return;
...
function LA(i, Q, t) {
  ...
  for (let r = 0; r < Q; r += 1) {
    const s = r * 3;
    P[s] = e.r, P[s + 1] = e.g, P[s + 2] = e.b;
  }
  return P;
}
function xe(i) {
  const Q = i.render.theme.colors, t = new l.Color(i.render.theme.background), B = new l.Color(Q.processed ?? Q.cutting), A = t.clone().lerp(B, 0.65);
  return { r: A.r, g: A.g, b: A.b };
}
function ye(i) {
  if (i.bucketCount <= 1)
    return sB(i.baseOpacity);
  const Q = i.bucketIndex / (i.bucketCount - 1);
  return sB(i.baseOpacity * Q);
}
function sB(i) {
  return Number.isFinite(i) ? Math.max(0, Math.min(1, i)) : 0;
}
const WB = {
  background: "#111827",
  opacity: 0.9,
...
async function Ye(i, Q, t, B) {
  var M, b, G;
  const A = Math.max(2, Math.floor(t)), e = (B == null ? void 0 : B.arcSegments) ?? 30, P = (B == null ? void 0 : B.checkpointEveryLines) ?? 2e3, r = ((M = B == null ? void 0 : B.batch) == null ? void 0 : M.yieldEveryLines) ?? 1e4, s = ((b = B == null ? void 0 : B.batch) == null ? void 0 : b.shouldAbort) ?? (() => !1), v = (G = B == null ? void 0 : B.batch) == null ? void 0 : G.onProgress;
  if (i.length === 0) {
    const C = {
      xMin: -1,
      xMax: 1,
    ...
    return F.set(0, new Float32Array(j)), { moves: [], checkpoints: F, slabBounds: C, resolution: A, toolRadius: Q };
  }
  ...
  for (const C of i)
    w.processLine(C);
  isFinite(c) || (c = -1, n = 1, T = -1, z = 1, a = 0, o = 0), c === n && (c -= Q * 2, n += Q * 2), T === z && (T -= Q * 2, z += Q * 2), c -= Q, n += Q, T -= Q, z += Q;
  const E = o;
  let h = a - Q;
  h >= E && (h = E - 1);
  ...
  const d = new rB({
    onLinearMove(C) {
      ...
      O.push(H), vB(H, U, u, Q, A);
    },
    onArcMove(C) {
      const j = C.transformedStart ?? C.start, F = C.transformedEnd ?? C.end, H = C.transformedCenter ?? C.center, f = Se(j, F, H, C.plane, C.motion, e);
      for (const I of f) {
        const R = { lineIndex: N, ...I };
        O.push(R), vB(R, U, u, Q, A);
      }
    }
  });
  for (let C = 0; C < i.length && !s(); C++) {
    N = C;
    const j = i[C];
    j !== void 0 && d.processLine(j), C - D >= P && (k.set(C, new Float32Array(U)), D = C), C % r === 0 && C > 0 && (v == null || v(C, i.length), await new Promise((F) => setTimeout(F, 0)));
  }
  return v == null || v(i.length, i.length), { moves: O, checkpoints: k, slabBounds: u, resolution: A, toolRadius: Q };
}
function vB(i, Q, t, B, A) {
  const e = (t.xMax - t.xMin) / A, P = (t.yMax - t.yMin) / A, r = Math.hypot(i.x1 - i.x0, i.y1 - i.y0), s = Math.max(1e-3, Math.min(e, P) * 0.5), v = Math.max(1, Math.ceil(r / s)), c = Math.ceil(B / e), n = Math.ceil(B / P), T = B * B;
  for (let z = 0; z <= v; z++) {
    const a = z / v, o = i.x0 + (i.x1 - i.x0) * a, w = i.y0 + (i.y1 - i.y0) * a, E = i.z0 + (i.z1 - i.z0) * a, h = Math.floor((o - t.xMin) / e), u = Math.floor((w - t.yMin) / P);
    for (let U = -c; U <= c; U++) {
      const k = h + U;
      if (k < 0 || k >= A) continue;
      const O = t.xMin + (k + 0.5) * e, D = (O - o) * (O - o);
      if (!(D > T))
        for (let N = -n; N <= n; N++) {
          const d = u + N;
          if (d < 0 || d >= A) continue;
          const M = t.yMin + (d + 0.5) * P;
...
function Vt(i, Q) {
  let t = 0;
  for (const e of Q.checkpoints.keys())
    e <= i && e > t && (t = e);
  const B = Q.checkpoints.get(t), A = B ? new Float32Array(B) : new Float32Array(Q.resolution * Q.resolution).fill(Q.slabBounds.zTop);
  for (const e of Q.moves)
    e.lineIndex > t && e.lineIndex <= i && vB(e, A, Q.slabBounds, Q.toolRadius, Q.resolution);
  return A;
}
function Jt(i, Q, t, B) {
  if (t <= 0) return;
  const A = 1e-3, e = Q * Q, P = new Float32Array(e);
  for (let r = 0; r < t; r++) {
    P.set(i);
    for (let s = 0; s < Q; s++)
      for (let v = 0; v < Q; v++) {
        if (P[s * Q + v] < B - A) continue;
        let n = B;
        for (let T = -1; T <= 1; T++) {
          const z = s + T;
          if (!(z < 0 || z >= Q))
...
function Se(i, Q, t, B, A, e) {
  const P = Math.max(1, e);
  let r, s, v;
  B === "G18" ? (r = "Z", s = "X", v = "Y") : B === "G19" ? (r = "Y", s = "Z", v = "X") : (r = "X", s = "Y", v = "Z");
  const c = Math.hypot(i[r] - t[r], i[s] - t[s]), n = Math.atan2(i[s] - t[s], i[r] - t[r]), T = Math.atan2(Q[s] - t[s], Q[r] - t[r]);
  let z;
  A === "G2" ? (z = T - n, z > 0 && (z -= Math.PI * 2)) : (z = T - n, z < 0 && (z += Math.PI * 2));
  const a = [];
  let o = { ...i };
  for (let w = 1; w <= P; w++) {
    const E = w / P, h = n + z * E, u = E, U = t[r] + c * Math.cos(h), k = t[s] + c * Math.sin(h), O = i[v] + (Q[v] - i[v]) * u, D = { X: 0, Y: 0, Z: 0 };
...
function Ve() {
  if (Nt) return Nt;
  const i = 512, Q = document.createElement("canvas");
  Q.width = i, Q.height = i;
  const t = Q.getContext("2d");
  t.fillStyle = "#8B5E3C", t.fillRect(0, 0, i, i);
  const B = Math.floor(40 + Math.random() * 20);
  for (let A = 0; A < B; A++) {
    const e = Math.random() * i, P = 1 + Math.random() * 2, r = Math.floor(139 + (Math.random() - 0.5) * 40), s = Math.floor(94 + (Math.random() - 0.5) * 30), v = Math.floor(60 + (Math.random() - 0.5) * 20);
    t.strokeStyle = `rgb(${r},${s},${v})`, t.lineWidth = P, t.beginPath(), t.moveTo(0, e), t.lineTo(i, e), t.stroke();
  }
  for (let A = 0; A < 300; A++) {
    const e = Math.random() * i, P = Math.random() * i, r = 20 + Math.random() * 60, s = (Math.random() - 0.5) * 0.3, v = Math.floor(100 + Math.random() * 80), c = Math.floor(60 + Math.random() * 50), n = Math.floor(30 + Math.random() * 40);
    t.strokeStyle = `rgba(${v},${c},${n},0.35)`, t.lineWidth = 0.5 + Math.random() * 1, t.beginPath(), t.moveTo(e, P), t.quadraticCurveTo(
      e + r * 0.5 * Math.cos(s),
      P + r * 0.5 * Math.sin(s) * 0.1,
      e + r * Math.cos(s * 0.5),
      P + r * 0.15
    ), t.stroke();
  }
  return Nt = new l.CanvasTexture(Q), Nt;
}
function Je(i, Q) {
  const t = new l.Group();
  t.name = "gviewer:sim3d-slab";
  const B = i.xMax - i.xMin, A = i.yMax - i.yMin, e = i.zTop - i.zBot, P = (i.xMin + i.xMax) / 2, r = (i.yMin + i.yMax) / 2, s = (i.zBot + i.zTop) / 2, v = Ve();
  ...
  const c = new l.MeshStandardMaterial({
    ...
    roughness: 0.9,
    metalness: 0
  }), z = new l.PlaneGeometry(B, A, Q - 1, Q - 1), a = Q * Q, o = new Float32Array(a * 3), w = z.attributes.position;
  for (let G = 0; G < a; G++)
    o[G * 3] = w.getX(G), o[G * 3 + 1] = w.getY(G), o[G * 3 + 2] = 0;
  ...
  h.name = "gviewer:sim3d-top", h.position.set(P, r, i.zTop), t.add(h);
  const u = new l.PlaneGeometry(B, e);
  u.rotateX(Math.PI / 2);
  const U = new l.Mesh(u, n);
  U.position.set(P, i.yMax, s), t.add(U);
  const k = new l.PlaneGeometry(B, e);
  k.rotateX(Math.PI / 2);
  const O = new l.Mesh(k, n);
  O.position.set(P, i.yMin, s), t.add(O);
  const D = new l.PlaneGeometry(e, A);
  D.rotateY(-Math.PI / 2);
  const N = new l.Mesh(D, n);
  N.position.set(i.xMax, r, s), t.add(N);
  const d = new l.PlaneGeometry(e, A);
  d.rotateY(-Math.PI / 2);
  const M = new l.Mesh(d, n);
  M.position.set(i.xMin, r, s), t.add(M);
  const b = new l.Mesh(new l.PlaneGeometry(B, A), T);
  return b.position.set(P, r, i.zBot), t.add(b), { group: t, topMesh: h, topPositionAttr: E, slabBounds: i, resolution: Q };
}
function Rt(i, Q) {
  const { topPositionAttr: t, slabBounds: B, resolution: A } = i, e = A * A;
  for (let P = 0; P < e; P++) {
    const r = Math.floor(P / A), s = P % A, v = (A - 1 - r) * A + s;
    t.setZ(P, Q[v] - B.zTop);
  }
  t.needsUpdate = !0, i.topMesh.geometry.computeVertexNormals();
}
function Re(i) {
  i.group.traverse((Q) => {
    if (Q instanceof l.Mesh) {
      Q.geometry.dispose();
      const t = Q.material;
...
const Xe = ".gViewer-viewcube{position:absolute;left:60px;bottom:60px;width:84px;height:84px;z-index:9999;perspective:600px;pointer-events:auto}.gViewer-viewcube__cube{position:absolute;top:0;right:0;bottom:0;left:0;transform-style:preserve-3d;transition:transform 60ms linear}.gViewer-viewcube__face{position:absolute;width:84px;height:84px;display:flex;align-items:center;justify-content:center;font:600 11px system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;letter-spacing:.06em;text-transform:uppercase;color:#f0f5fce0;background:#0c1014eb;border:1px solid rgba(120,140,160,.35);border-radius:14px;cursor:pointer;-webkit-user-select:none;user-select:none;padding:0}.gViewer-viewcube__face:hover{background:#0c1014db;border-color:#a0bedc8c}.gViewer-viewcube__face.is-active{border-color:#60a5faf2;box-shadow:0 0 0 1px #60a5fa59,0 10px 30px #00000059;color:#60a5faf2}.gViewer-viewcube__face--front{transform:translateZ(42px) scaleY(-1)}.gViewer-viewcube__face--back{transform:rotateY(180deg) translateZ(42px) scaleY(-1)}.gViewer-viewcube__face--left{transform:rotateY(-90deg) translateZ(42px) scaleY(-1)}.gViewer-viewcube__face--right{transform:rotateY(90deg) translateZ(42px) scaleY(-1)}.gViewer-viewcube__face--top{transform:rotateX(-90deg) translateZ(42px) scaleY(-1)}.gViewer-viewcube__face--bottom{transform:rotateX(90deg) translateZ(42px) scaleY(-1)}@media (orientation: portrait){.gViewer-viewcube{left:60px;bottom:110px;width:64px;height:64px}.gViewer-viewcube__face{width:64px;height:64px}.gViewer-viewcube__face--front{transform:translateZ(32px) scaleY(-1)}.gViewer-viewcube__face--back{transform:rotateY(180deg) translateZ(32px) scaleY(-1)}.gViewer-viewcube__face--left{transform:rotateY(-90deg) translateZ(32px) scaleY(-1)}.gViewer-viewcube__face--right{transform:rotateY(90deg) translateZ(32px) scaleY(-1)}.gViewer-viewcube__face--top{transform:rotateX(-90deg) translateZ(32px) scaleY(-1)}.gViewer-viewcube__face--bottom{transform:rotateX(90deg) translateZ(32px) scaleY(-1)}}", qB = "gviewer-viewcube-style";
function Ke() {
  if (typeof document > "u" || document.getElementById(qB)) return;
  const i = document.createElement("style");
  i.id = qB, i.setAttribute("data-gviewer", "viewcube"), i.textContent = Xe, document.head.appendChild(i);
}
const yt = class yt {
  constructor(Q) {
    Ke(), this.onSelectView = Q.onSelectView, this.faceButtons = /* @__PURE__ */ new Map(), this.root = document.createElement("div"), this.root.className = "gViewer-viewcube", this.cube = document.createElement("div"), this.cube.className = "gViewer-viewcube__cube", this.root.appendChild(this.cube);
    const t = [
    ...
    for (const B of t) {
      ...
      A.type = "button", A.className = `gViewer-viewcube__face ${B.className}`, A.textContent = B.label, A.addEventListener("click", () => this.onSelectView(B.view)), this.cube.appendChild(A), this.faceButtons.set(B.view, A);
    }
    Q.container.appendChild(this.root);
  }
  setActiveFace(Q) {
    for (const [t, B] of this.faceButtons.entries())
      t === Q ? B.classList.add("is-active") : B.classList.remove("is-active");
  }
  setRotationMatrix3d(Q) {
    const t = Array.from({ length: 16 }, (B, A) => {
      const e = Q[A] ?? 0, P = yt.Y_ROW_INDICES.has(A) ? -e : e;
      return Number.isFinite(P) ? String(Math.abs(P) < 1e-12 ? 0 : Number(P.toFixed(8))) : "0";
    });
    this.cube.style.transform = `matrix3d(${t.join(",")})`;
  }
  dispose() {
    this.root.remove();
  }
};
yt.Y_ROW_INDICES = /* @__PURE__ */ new Set([1, 5, 9, 13]);
let cB = yt;
function MQ(i, Q, t = {}) {
  const { opacity: B = 1, size: A = 10 } = t, e = new l.Object3D(), P = 100, r = document.createElement("canvas"), s = r.getContext("2d");
  if (!s)
    throw new Error("Missing canvas 2D context.");
  s.font = `normal ${P}px Arial`;
  const c = s.measureText(i).width;
  r.width = c, r.height = P, s.font = `normal ${P}px Arial`, s.textAlign = "center", s.textBaseline = "middle", s.fillStyle = Q, s.fillText(i, c / 2, P / 2);
  ...
  e.textHeight = A, e.textWidth = c / P * e.textHeight, t.textAlign === "left" ? e.position.x = (t.x ?? 0) + (e.textWidth ?? 0) / 2 : t.textAlign === "right" ? e.position.x = (t.x ?? 0) - (e.textWidth ?? 0) / 2 : e.position.x = t.x ?? 0, t.textBaseline === "top" ? e.position.y = (t.y ?? 0) - (e.textHeight ?? 0) / 2 : t.textBaseline === "bottom" ? e.position.y = (t.y ?? 0) + (e.textHeight ?? 0) / 2 : e.position.y = t.y ?? 0, e.position.z = t.z ?? 0;
  const z = new l.Sprite(T);
  return z.scale.set(c / P * A, A, 1), e.add(z), e;
}
function Ze(i) {
  i.traverse((Q) => {
    var B;
    if (!(Q instanceof l.Sprite))
      return;
    const t = Q.material;
    (B = t.map) == null || B.dispose(), t.dispose();
  });
}
const We = 25.4;
function qe(i, Q) {
  const t = new l.Group(), B = new l.Box3Helper(
    i.clone(),
    new l.Color(Q.render.theme.colors.boundingBox)
  ), A = B.material;
  return A.transparent = !0, A.opacity = 0.65, A.depthWrite = !1, A.linewidth = 2, t.add(B), Q.boundingBox.labels && t.add(..._e(i.clone(), Q)), t;
}
function _e(i, Q) {
  const t = Q.render.theme.colors.boundingBox, B = 0.8, A = (u) => Math.abs(u) < 1e-9 ? "0" : u.toFixed(3), e = (u) => Q.units === "in" ? `${A(u / We)} in` : `${A(u)} mm`, P = i.min, r = i.max, s = (u) => MQ(u, t, { size: 4, opacity: B }), v = s(e(P.x)), c = s(e(r.x)), n = s(e(P.y)), T = s(e(r.y)), z = s(e(P.z)), a = s(e(r.z)), o = new l.Vector3();
  i.getSize(o);
  const w = Math.max(2, Math.max(o.x, o.y, o.z) * 0.02), E = (P.x + r.x) / 2, h = (P.y + r.y) / 2;
  return v.position.set(P.x - w, h, P.z - w), c.position.set(r.x + w, h, P.z - w), n.position.set(E, P.y - w, P.z - w), T.position.set(E, r.y + w, P.z - w), z.position.set(E, h, P.z - w), a.position.set(E, h, r.z + w), [v, c, n, T, z, a];
}
function $e(i) {
  i.traverse((Q) => {
    var t;
    if (Q instanceof l.LineSegments || Q instanceof l.Line) {
      Q.geometry.dispose();
      const B = Q.material;
    ...
    if (Q instanceof l.Sprite) {
      const B = Q.material;
      (t = B.map) == null || t.dispose(), B.dispose();
    }
  });
}
function _B(i, Q, t) {
...
function QP(i, Q, t, B = 10) {
  const A = [], e = i.x - Q.y, P = Q.x - i.y;
  for (let v = e; v <= P; v += B) {
    const c = Math.max(i.x, i.y + v), n = Math.min(Q.x, Q.y + v);
...
function tP(i, Q, t, B) {
  const A = new l.Group();
  if (A.add(_B(i, Q, t.render.theme.colors.machineBed)), B) {
    const e = t.render.theme.colors.machineBedKeepout;
    A.add(_B(B.min, B.max, e)), A.add(QP(B.min, B.max, e));
  }
  return A;
}
function BP(i) {
  i.traverse((Q) => {
    if (Q instanceof l.LineSegments || Q instanceof l.Line) {
      Q.geometry.dispose();
      const t = Q.material;
      Array.isArray(t) ? t.forEach((B) => B.dispose()) : t.dispose();
    }
  });
}
class AP extends ZB {
  _rotateUp(Q) {
    ZB.prototype._rotateUp.call(this, -Q);
  }
}
function eP(i) {
  const Q = window.getComputedStyle(i);
  (Q.position === "static" || !Q.position) && (i.style.position = "relative"), i.style.overflow || (i.style.overflow = "hidden");
}
function PP(i) {
  if (i < 0.5)
    return 4 * i * i * i;
...
function $B(i) {
  ...
  switch (i) {
    ...
    case "bottom":
      ...
      break;
    case "front-top-left":
      Q.copy(t.front).add(t.top).add(t.left);
      break;
    case "front-top-right":
      Q.copy(t.front).add(t.top).add(t.right);
      break;
    case "front-bottom-left":
      Q.copy(t.front).add(t.bottom).add(t.left);
      break;
    case "front-bottom-right":
      Q.copy(t.front).add(t.bottom).add(t.right);
      break;
    case "back-top-left":
      Q.copy(t.back).add(t.top).add(t.left);
      break;
    case "back-top-right":
      Q.copy(t.back).add(t.top).add(t.right);
      break;
    case "back-bottom-left":
      Q.copy(t.back).add(t.bottom).add(t.left);
      break;
    case "back-bottom-right":
      Q.copy(t.back).add(t.bottom).add(t.right);
      break;
    default:
      Q.copy(t.front);
      break;
...
function iP(i, Q) {
  const t = i.x - Q.x, B = i.y - Q.y, A = i.z - Q.z, e = Math.abs(t), P = Math.abs(B), r = Math.abs(A);
  return r >= e && r >= P ? A >= 0 ? "top" : "bottom" : e >= P ? t >= 0 ? "right" : "left" : B >= 0 ? "back" : "front";
}
function Ot(i, Q) {
  return 2 * i * Math.tan(l.MathUtils.degToRad(Q) / 2);
}
function MA(i, Q) {
  return i / 2 / Math.tan(l.MathUtils.degToRad(Q) / 2);
}
function QA(i, Q, t = 1.25) {
  const B = Math.max(i.x, i.y, 1) / 2;
  return MA(B * 2, Q) * t + i.z;
}
function Xt(i, Q) {
  const t = Math.max(1e-6, i) / 2, B = t * Math.max(1e-6, Q);
  return { left: -B, right: B, top: t, bottom: -t };
}
function tA(i) {
  const Q = Math.max(1, i);
  return { near: Q / 1e3, far: Q * 50 };
}
function BA(i, Q) {
  const t = Math.max(1, Math.max(i, Q)) * 4;
  return { near: -t, far: t };
}
function rP(i, Q, t, B) {
  const A = Q.z;
  if (Math.abs(A) < 1e-9)
    return null;
  const e = (t - i.z) / A;
  return e < 0 && !B ? null : i.clone().addScaledVector(Q, e);
}
const HA = 25.4;
function CB(i) {
  if (i.bounds)
  ...
  const Q = Math.max(1, i.sizeXMm) / 2, t = Math.max(1, i.sizeYMm) / 2;
  return { minX: -Q, maxX: Q, minY: -t, maxY: t };
}
function sP(i) {
  const { minX: Q, maxX: t, minY: B, maxY: A } = CB(i), e = i.units === "mm" ? 10 : HA, P = [], r = [], s = (a, o, w, E, h) => {
    h.push(a, o, 0, w, E, 0);
  }, v = Math.ceil(B / e) * e, c = Math.floor(A / e) * e;
  for (let a = v; a <= c + 1e-6; a += e)
    s(Q, a, t, a, Math.abs(a) < 1e-6 ? P : r);
  const n = Math.ceil(Q / e) * e, T = Math.floor(t / e) * e;
  for (let a = n; a <= T + 1e-6; a += e)
    s(a, B, a, A, Math.abs(a) < 1e-6 ? P : r);
  const z = new l.Group();
  return r.length > 0 && z.add(AA(r, i.theme.colors.grid.minor, 0.25)), P.length > 0 && z.add(AA(P, i.theme.colors.grid.major, 0.4)), z;
}
function AA(i, Q, t) {
  const B = new l.BufferGeometry();
  B.setAttribute("position", new l.Float32BufferAttribute(i, 3));
...
function vP(i) {
  i.traverse((Q) => {
    if (!(Q instanceof l.LineSegments))
      return;
    Q.geometry.dispose();
    const t = Q.material;
    Array.isArray(t) ? t.forEach((B) => B.dispose()) : t.dispose();
...
function cP(i) {
  const Q = new l.Group(), { minX: t, maxX: B, minY: A, maxY: e } = CB({
    sizeXMm: i.sizeXWorld,
    sizeYMm: i.sizeYWorld,
    bounds: i.bounds
  }), P = Math.max(1, i.depthWorld), r = Math.max(1, Math.min(Math.max(B - t, e - A) / 24, 30)), s = r * 0.6, v = (E) => new l.LineDashedMaterial({
    color: new l.Color(E),
    dashSize: r,
    gapSize: s,
    transparent: !0,
  ...
  const T = new l.Line(
    ...
    v(i.theme.colors.axes.z)
  );
  T.computeLineDistances(), Q.add(c, n, T);
  const z = Math.max(2, r), a = MQ("X", i.theme.colors.axes.x, { size: 5 });
  a.position.set(B + z, 0, 0);
  const o = MQ("Y", i.theme.colors.axes.y, { size: 5 });
  o.position.set(0, e + z, 0);
  const w = MQ("Z", i.theme.colors.axes.z, { size: 5 });
  return w.position.set(0, 0, P + z), Q.add(a, o, w), Q;
}
function nP(i) {
  i.traverse((Q) => {
    var t;
    if (Q instanceof l.Line) {
      Q.geometry.dispose();
      const B = Q.material;
      Array.isArray(B) ? B.forEach((A) => A.dispose()) : B.dispose();
      return;
...
function oP(i) {
  const Q = new l.Group(), { minX: t, maxX: B, minY: A, maxY: e } = CB(i), P = i.units === "mm" ? 10 : 1, r = P * 2, s = i.units === "mm" ? 1 : HA, v = 0.5, c = 4, n = 0.1, T = 5, z = i.theme.colors.axes.x, a = i.theme.colors.axes.y, o = { opacity: v, size: c };
  for (let w = P; ; w += r) {
    const E = w * s;
    if (E > B + 1e-6)
      break;
    const h = MQ(String(w), z, o);
    h.position.set(E, T, n), Q.add(h);
  }
  for (let w = P; ; w += r) {
    const E = w * s;
    if (-E < t - 1e-6)
  ...
  for (let w = P; ; w += r) {
    ...
    if (E > e + 1e-6)
      break;
    const h = MQ(String(w), a, o);
    h.position.set(-T, E, n), Q.add(h);
  }
  for (let w = P; ; w += r) {
    const E = w * s;
    if (-E < A - 1e-6)
...
function jA(i, Q) {
  i.prototype = Object.create(Q.prototype), i.prototype.constructor = i, i.__proto__ = Q;
}
/*!
 * GSAP 3.15.0
 * https://gsap.com
...
var zQ = {
  ...
  overwrite: !1,
  delay: 0
}, NB, q, y, hQ = 1e8, m = 1 / hQ, nB = Math.PI * 2, TP = nB / 4, zP = 0, FA = Math.sqrt, aP = Math.cos, wP = Math.sin, Z = function(Q) {
...
}, $ = Array.isArray, EP = /random\([^)]+\)/g, hP = /,\s*/g, eA = /(?:-?\.?\d|\.)+/gi, GA = /[-+=.]*\d+[.e\-+]*\d*[e\-+]*\d*/g, qQ = /[-+=.]*\d+[.e-]*\d*[a-z%]*/g, Kt = /[-+=.]*\d+\.?\d*(?:e-|e\+)?\d*/gi, bA = /[+-]=-?[.\d]+/, lP = /[^,'"\[\]\s]+/gi, kP = /^[+\-=e\s\d]*\d+[.\d]*([a-z]*|%)\s*$/i, S, kQ, oB, MB, aQ = {}, ft = {}, pA, gA = function(Q) {
  return (ft = Bt(Q, aQ)) && rQ;
}, HB = function(Q, t) {
  return console.warn("Invalid property", Q, "set to", t, "Missing plugin? gsap.registerPlugin()");
}, Et = function(Q, t) {
  return !t && console.warn(Q);
}, mA = function(Q, t) {
  return Q && (aQ[Q] = t) && ft && (ft[Q] = t) || aQ;
}, ht = function() {
  return 0;
}, uP = {
  suppressEvents: !0,
  ...
  suppressEvents: !0
}, jB = {}, fQ = [], TB = {}, xA, cQ = {}, Zt = {}, PA = 30, Ht = [], FB = "", fB = function(Q) {
  var t = Q[0], B, A;
  if (DQ(t) || J(t) || (Q = [Q]), !(B = (t._gsap || {}).harness)) {
    for (A = Ht.length; A-- && !Ht[A].targetTest(t); )
      ;
    B = Ht[A];
  }
  for (A = Q.length; A--; )
    Q[A] && (Q[A]._gsap || (Q[A]._gsap = new re(Q[A], B))) || Q.splice(A, 1);
  return Q;
}, SQ = function(Q) {
  return Q._gsap || fB(lQ(Q))[0]._gsap;
}, yA = function(Q, t, B) {
  return (B = Q[t]) && J(B) ? Q[t]() : OB(B) && Q.getAttribute && Q.getAttribute(t) || B;
}, eQ = function(Q, t) {
  return (Q = Q.split(",")).forEach(t) || Q;
}, X = function(Q) {
  return Math.round(Q * 1e5) / 1e5 || 0;
}, Y = function(Q) {
  return Math.round(Q * 1e7) / 1e7 || 0;
}, $Q = function(Q, t) {
  var B = t.charAt(0), A = parseFloat(t.substr(2));
  return Q = parseFloat(Q), B === "+" ? Q + A : B === "-" ? Q - A : B === "*" ? Q * A : Q / A;
}, DP = function(Q, t) {
  for (var B = t.length, A = 0; Q.indexOf(t[A]) < 0 && ++A < B; )
    ;
  return A < B;
}, Gt = function() {
  var Q = fQ.length, t = fQ.slice(0), B, A;
  for (TB = {}, fQ.length = 0, B = 0; B < Q; B++)
    A = t[B], A && A._lazy && (A.render(A._lazy[0], A._lazy[1], !0)._lazy = 0);
}, GB = function(Q) {
  return !!(Q._initted || Q._startAt || Q.add);
}, IA = function(Q, t, B, A) {
  fQ.length && !q && Gt(), Q.render(t, B, !!(q && t < 0 && GB(Q))), fQ.length && !q && Gt();
}, YA = function(Q) {
  var t = parseFloat(Q);
  return (t || t === 0) && (Q + "").match(lP).length < 2 ? t : Z(Q) ? Q.trim() : Q;
}, SA = function(Q) {
  return Q;
}, wQ = function(Q, t) {
  for (var B in t)
  ...
  return function(t, B) {
    for (var A in B)
      A in t || A === "duration" && Q || A === "ease" || (t[A] = B[A]);
  };
}, Bt = function(Q, t) {
  for (var B in t)
    Q[B] = t[B];
  ...
  for (var B in t)
    B !== "__proto__" && B !== "constructor" && B !== "prototype" && (Q[B] = DQ(t[B]) ? i(Q[B] || (Q[B] = {}), t[B]) : t[B]);
  return Q;
}, bt = function(Q, t) {
  var B = {}, A;
  for (A in Q)
  ...
  var t = Q.parent || S, B = Q.keyframes ? dP($(Q.keyframes)) : wQ;
  if (AQ(Q.inherit))
    for (; t; )
      B(Q, t.vars.defaults), t = t.parent || t._dp;
  return Q;
}, CP = function(Q, t) {
  for (var B = Q.length, A = B === t.length; A && B-- && Q[B] === t[B]; )
    ;
  return B < 0;
}, VA = function(Q, t, B, A, e) {
  var P = Q[A], r;
  if (e)
    for (r = t[e]; P && P[e] > r; )
      P = P._prev;
  return P ? (t._next = P._next, P._next = t) : (t._next = Q[B], Q[B] = t), t._next ? t._next._prev = t : Q[A] = t, t._prev = P, t.parent = t._dp = Q, t;
}, It = function(Q, t, B, A) {
  B === void 0 && (B = "_first"), A === void 0 && (A = "_last");
  var e = t._prev, P = t._next;
  e ? e._next = P : Q[B] === t && (Q[B] = P), P ? P._prev = e : Q[A] === t && (Q[A] = e), t._next = t._prev = t.parent = null;
}, bQ = function(Q, t) {
  Q.parent && (!t || Q.parent.autoRemoveChildren) && Q.parent.remove && Q.parent.remove(Q), Q._act = 0;
}, VQ = function(Q, t) {
  if (Q && (!t || t._end > Q._dur || t._start < 0))
    for (var B = Q; B; )
      B._dirty = 1, B = B.parent;
  return Q;
}, NP = function(Q) {
  for (var t = Q.parent; t && t.parent; )
    t._dirty = 1, t.totalDuration(), t = t.parent;
  return Q;
}, zB = function(Q, t, B, A) {
  return Q._startAt && (q ? Q._startAt.revert(Mt) : Q.vars.immediateRender && !Q.vars.autoRevert || Q._startAt.render(t, !0, A));
}, OP = function i(Q) {
  return !Q || Q._ts && i(Q.parent);
}, rA = function(Q) {
  return Q._repeat ? At(Q._tTime, Q = Q.duration() + Q._rDelay) * Q : 0;
}, At = function(Q, t) {
  var B = Math.floor(Q = Y(Q / t));
  return Q && B === Q ? B - 1 : B;
}, pt = function(Q, t) {
  return (Q - t._start) * t._ts + (t._ts >= 0 ? 0 : t._dirty ? t.totalDuration() : t._tDur);
}, Yt = function(Q) {
  return Q._end = Y(Q._start + (Q._tDur / Math.abs(Q._ts || Q._rts || m) || 0));
}, St = function(Q, t) {
  var B = Q._dp;
  return B && B.smoothChildTiming && Q._ts && (Q._start = Y(B._time - (Q._ts > 0 ? t / Q._ts : ((Q._dirty ? Q.totalDuration() : Q._tDur) - t) / -Q._ts)), Yt(Q), B._dirty || VQ(B, Q)), Q;
}, JA = function(Q, t) {
  var B;
  if ((t._time || !t._dur && t._initted || t._start < Q._time && (t._dur || !t.add)) && (B = pt(Q.rawTime(), t), (!t._dur || dt(0, t.totalDuration(), B) - t._tTime > m) && t.render(B, !0)), VQ(Q, t)._dp && Q._initted && Q._time >= Q._dur && Q._ts) {
  ...
  return t.parent && bQ(t), t._start = Y((NQ(B) ? B : B || Q !== S ? EQ(Q, B, t) : Q._time) + t._delay), t._end = Y(t._start + (t.totalDuration() / Math.abs(t.timeScale()) || 0)), VA(Q, t, "_first", "_last", Q._sort ? "_start" : 0), aB(t) || (Q._recent = t), A || JA(Q, t), Q._ts < 0 && St(Q, Q._tTime), Q;
}, RA = function(Q, t) {
  return (aQ.ScrollTrigger || HB("scrollTrigger", t)) && aQ.ScrollTrigger.create(t, Q);
}, XA = function(Q, t, B, A, e) {
  if (pB(Q, t, e), !Q._initted)
    return 1;
  if (!B && Q._pt && !q && (Q._dur && Q.vars.lazy !== !1 || !Q._dur && Q.vars.lazy) && xA !== nQ.frame)
    return fQ.push(Q), Q._lazy = [e, A], 1;
}, LP = function i(Q) {
  var t = Q.parent;
  ...
  var e = Q.ratio, P = t < 0 || !t && (!Q._start && LP(Q) && !(!Q._initted && aB(Q)) || (Q._ts < 0 || Q._dp._ts < 0) && !aB(Q)) ? 0 : 1, r = Q._rDelay, s = 0, v, c, n;
  if (r && Q._repeat && (s = dt(0, Q._tDur, t), c = At(s, r), Q._yoyo && c & 1 && (P = 1 - P), c !== At(Q._tTime, r) && (e = 1 - P, Q.vars.repeatRefresh && Q._initted && Q.invalidate())), P !== e || q || A || Q._zTime === m || !t && Q._zTime) {
    if (!Q._initted && XA(Q, t, A, B, s))
      return;
    for (n = Q._zTime, Q._zTime = t || (B ? m : 0), B || (B = t && !n), Q.ratio = P, Q._from && (P = 1 - P), Q._time = 0, Q._tTime = s, v = Q._pt; v; )
      v.r(P, v.d), v = v._next;
    t < 0 && zB(Q, t, B, !0), Q._onUpdate && !B && oQ(Q, "onUpdate"), s && Q._repeat && !B && Q.parent && oQ(Q, "onRepeat"), (t >= Q._tDur || t < 0) && Q.ratio === P && (P && bQ(Q, 1), !B && !q && (oQ(Q, P ? "onComplete" : "onReverseComplete", !0), Q._prom && Q._prom()));
  } else Q._zTime || (Q._zTime = t);
}, HP = function(Q, t, B) {
  var A;
  if (B > t)
    for (A = Q._first; A && A._start <= B; ) {
      if (A.data === "isPause" && A._start > t)
        return A;
      A = A._next;
    }
  else
    for (A = Q._last; A && A._start >= B; ) {
      if (A.data === "isPause" && A._start < t)
        return A;
      A = A._prev;
    }
}, et = function(Q, t, B, A) {
  var e = Q._repeat, P = Y(t) || 0, r = Q._tTime / Q._tDur;
  return r && !A && (Q._time *= P / Q._dur), Q._dur = P, Q._tDur = e ? e < 0 ? 1e10 : Y(P * (e + 1) + Q._rDelay * e) : P, r > 0 && !A && St(Q, Q._tTime = Q._tDur * r), Q.parent && Yt(Q), B || VQ(Q.parent, Q), Q;
}, sA = function(Q) {
  return Q instanceof BQ ? VQ(Q) : et(Q, Q._dur);
}, jP = {
  _start: 0,
  endTime: ht,
  totalDuration: ht
}, EQ = function i(Q, t, B) {
  var A = Q.labels, e = Q._recent || jP, P = Q.duration() >= hQ ? e.endTime(!1) : Q._dur, r, s, v;
  return Z(t) && (isNaN(t) || t in A) ? (s = t.charAt(0), v = t.substr(-1) === "%", r = t.indexOf("="), s === "<" || s === ">" ? (r >= 0 && (t = t.replace(/=/, "")), (s === "<" ? e._start : e.endTime(e._repeat >= 0)) + (parseFloat(t.substr(1)) || 0) * (v ? (r < 0 ? e : B).totalDuration() / 100 : 1)) : r < 0 ? (t in A || (A[t] = P), A[t]) : (s = parseFloat(t.charAt(r - 1) + t.substr(r + 1)), v && B && (s = s / 100 * ($(B) ? B[0] : B).totalDuration()), r > 1 ? i(Q, t.substr(0, r - 1), B) + s : P + s)) : t == null ? P : +t;
}, zt = function(Q, t, B) {
  var A = NQ(t[1]), e = (A ? 2 : 1) + (Q < 2 ? 0 : 1), P = t[e], r, s;
  if (A && (P.duration = t[1]), P.parent = B, Q) {
    for (r = P, s = B; s && !("immediateRender" in r); )
      r = s.vars.defaults || {}, s = AQ(s.vars.inherit) && s.parent;
    P.immediateRender = AQ(r.immediateRender), Q < 2 ? P.runBackwards = 1 : P.startAt = t[e - 1];
  }
  return new K(t[0], P, t[e + 1]);
}, mQ = function(Q, t) {
  return Q || Q === 0 ? t(Q) : t;
}, dt = function(Q, t, B) {
  return B < Q ? Q : B > t ? t : B;
}, _ = function(Q, t) {
  return !Z(Q) || !(t = kP.exec(Q)) ? "" : t[1];
}, FP = function(Q, t, B) {
  return mQ(B, function(A) {
    return dt(Q, t, A);
  });
}, wB = [].slice, KA = function(Q, t) {
  return Q && DQ(Q) && "length" in Q && (!t && !Q.length || Q.length - 1 in Q && DQ(Q[0])) && !Q.nodeType && Q !== kQ;
}, fP = function(Q, t, B) {
  return B === void 0 && (B = []), Q.forEach(function(A) {
    var e;
    return Z(A) && !t || KA(A, 1) ? (e = B).push.apply(e, lQ(A)) : B.push(A);
  }) || B;
}, lQ = function(Q, t, B) {
  return y && !t && y.selector ? y.selector(Q) : Z(Q) && !B && (oB || !Pt()) ? wB.call((t || MB).querySelectorAll(Q), 0) : $(Q) ? fP(Q, B) : KA(Q) ? wB.call(Q, 0) : Q ? [Q] : [];
}, EB = function(Q) {
  return Q = lQ(Q)[0] || Et("Invalid scope") || {}, function(t) {
    var B = Q.current || Q.nativeElement || Q;
    return lQ(t, B.querySelectorAll ? B : B === Q ? Et("Invalid scope") || MB.createElement("div") : Q);
  };
}, ZA = function(Q) {
  ...
  var t = DQ(Q) ? Q : {
    each: Q
  }, B = JQ(t.ease), A = t.from || 0, e = parseFloat(t.base) || 0, P = {}, r = A > 0 && A < 1, s = isNaN(A) || r, v = t.axis, c = A, n = A;
  return Z(A) ? c = n = {
    center: 0.5,
    edges: 0.5,
    end: 1
  }[A] || 0 : !r && s && (c = A[0], n = A[1]), function(T, z, a) {
    var o = (a || t).length, w = P[o], E, h, u, U, k, O, D, N, d;
    if (!w) {
      if (d = t.grid === "auto" ? 0 : (t.grid || [1, hQ])[1], !d) {
        for (D = -hQ; D < (D = a[d++].getBoundingClientRect().left) && d < o; )
          ;
        d < o && d--;
      }
      for (w = P[o] = [], E = s ? Math.min(d, o) * c - 0.5 : A % d, h = d === hQ ? 0 : s ? o * n / d - 0.5 : A / d | 0, D = 0, N = hQ, O = 0; O < o; O++)
        u = O % d - E, U = h - (O / d | 0), w[O] = k = v ? Math.abs(v === "y" ? U : u) : FA(u * u + U * U), k > D && (D = k), k < N && (N = k);
      A === "random" && ZA(w), w.max = D - N, w.min = N, w.v = o = (parseFloat(t.amount) || parseFloat(t.each) * (d > o ? o - 1 : v ? v === "y" ? o / d : d : Math.max(d, o / d)) || 0) * (A === "edges" ? -1 : 1), w.b = o < 0 ? e - o : e, w.u = _(t.amount || t.each) || 0, B = B && o < 0 ? RP(B) : B;
    }
    return o = (w[T] - w.min) / w.max || 0, Y(w.b + (B ? B(o) : o) * w.v) + w.u;
  };
}, hB = function(Q) {
  var t = Math.pow(10, ((Q + "").split(".")[1] || "").length);
  return function(B) {
    var A = Y(Math.round(parseFloat(B) / Q) * Q * t);
    return (A - A % 1) / t + (NQ(B) ? 0 : _(B));
  };
}, qA = function(Q, t) {
  var B = $(Q), A, e;
  return !B && DQ(Q) && (A = B = Q.radius || hQ, Q.values ? (Q = lQ(Q.values), (e = !NQ(Q[0])) && (A *= A)) : Q = hB(Q.increment)), mQ(t, B ? J(Q) ? function(P) {
    return e = Q(P), Math.abs(e - P) <= A ? e : P;
  } : function(P) {
    for (var r = parseFloat(e ? P.x : P), s = parseFloat(e ? P.y : 0), v = hQ, c = 0, n = Q.length, T, z; n--; )
      e ? (T = Q[n].x - r, z = Q[n].y - s, T = T * T + z * z) : T = Math.abs(Q[n] - r), T < v && (v = T, c = n);
    return c = !A || v <= A ? Q[c] : P, e || c === P || NQ(P) ? c : c + _(P);
  } : hB(Q));
}, _A = function(Q, t, B, A) {
  return mQ($(Q) ? !t : B === !0 ? !!(B = 0) : !A, function() {
    return $(Q) ? Q[~~(Math.random() * Q.length)] : (B = B || 1e-5) && (A = B < 1 ? Math.pow(10, (B + "").length - 2) : 1) && Math.floor(Math.round((Q - B / 2 + Math.random() * (t - Q + B * 0.99)) / B) * B * A) / A;
  });
}, GP = function() {
  for (var Q = arguments.length, t = new Array(Q), B = 0; B < Q; B++)
    t[B] = arguments[B];
  return function(A) {
    return t.reduce(function(e, P) {
      return P(e);
    }, A);
  };
}, bP = function(Q, t) {
  return function(B) {
    return Q(parseFloat(B)) + (t || _(B));
  };
}, pP = function(Q, t, B) {
  return Qe(Q, t, 0, 1, B);
}, $A = function(Q, t, B) {
  ...
  var A = t - Q;
  return $(Q) ? $A(Q, i(0, Q.length), t) : mQ(B, function(e) {
    return (A + (e - Q) % A) % A + Q;
  });
}, mP = function i(Q, t, B) {
  var A = t - Q, e = A * 2;
  return $(Q) ? $A(Q, i(0, Q.length - 1), t) : mQ(B, function(P) {
    return P = (e + (P - Q) % e) % e || 0, Q + (P > A ? e - P : P);
  });
}, lt = function(Q) {
  return Q.replace(EP, function(t) {
    var B = t.indexOf("[") + 1, A = t.substring(B || 7, B ? t.indexOf("]") : t.length - 1).split(hP);
    return _A(B ? A : +A[0], B ? 0 : +A[1], +A[2] || 1e-5);
  });
}, Qe = function(Q, t, B, A, e) {
  var P = t - Q, r = A - B;
  return mQ(e, function(s) {
    return B + ((s - Q) / P * r || 0);
  });
}, xP = function i(Q, t, B, A) {
  var e = isNaN(Q + t) ? 0 : function(z) {
    return (1 - z) * Q + z * t;
  };
  if (!e) {
    var P = Z(Q), r = {}, s, v, c, n, T;
    if (B === !0 && (A = 1) && (B = null), P)
      Q = {
        p: Q
      }, t = {
    ...
    if (!c) {
      for (s in t)
        bB.call(r, Q, s, "get", t[s]);
      e = function(a) {
        return xB(a, r) || (P ? Q.p : Q);
      };
    }
  }
  return mQ(B, e);
}, vA = function(Q, t, B) {
  var A = Q.labels, e = hQ, P, r, s;
  for (P in A)
    r = A[P] - t, r < 0 == !!B && r && e > (r = Math.abs(r)) && (s = P, e = r);
  return s;
}, oQ = function(Q, t, B) {
  var A = Q.vars, e = A[t], P = y, r = Q._ctx, s, v, c;
  if (e)
    return s = A[t + "Params"], v = A.callbackScope || Q, B && fQ.length && Gt(), r && (y = r), c = s ? e.apply(v, s) : e.call(v), y = P, c;
}, nt = function(Q) {
  return bQ(Q), Q.scrollTrigger && Q.scrollTrigger.kill(!!q), Q.progress() < 1 && oQ(Q, "onInterrupt"), Q;
}, _Q, te = [], Be = function(Q) {
  if (Q)
    if (Q = !Q.name && Q.default || Q, LB() || Q.headless) {
      var t = Q.name, B = J(Q), A = t && !B && Q.init ? function() {
        this._props = [];
      } : Q, e = {
        init: ht,
        render: xB,
      ...
      if (Pt(), Q !== A) {
        if (cQ[t])
          return;
        wQ(A, wQ(bt(Q, e), P)), Bt(A.prototype, Bt(e, bt(Q, P))), cQ[A.prop = t] = A, Q.targetTest && (Ht.push(A), jB[t] = 1), t = (t === "css" ? "CSS" : t.charAt(0).toUpperCase() + t.substr(1)) + "Plugin";
      }
      mA(t, A), Q.register && Q.register(rQ, A, PQ);
    } else
      te.push(Q);
}, g = 255, ot = {
  aqua: [0, g, g],
  ...
  return Q += Q < 0 ? 1 : Q > 1 ? -1 : 0, (Q * 6 < 1 ? t + (B - t) * Q * 6 : Q < 0.5 ? B : Q * 3 < 2 ? t + (B - t) * (2 / 3 - Q) * 6 : t) * g + 0.5 | 0;
}, Ae = function(Q, t, B) {
  var A = Q ? NQ(Q) ? [Q >> 16, Q >> 8 & g, Q & g] : 0 : ot.black, e, P, r, s, v, c, n, T, z, a;
  if (!A) {
    if (Q.substr(-1) === "," && (Q = Q.substr(0, Q.length - 1)), ot[Q])
      A = ot[Q];
    else if (Q.charAt(0) === "#") {
      if (Q.length < 6 && (e = Q.charAt(1), P = Q.charAt(2), r = Q.charAt(3), Q = "#" + e + e + P + P + r + r + (Q.length === 5 ? Q.charAt(4) + Q.charAt(4) : "")), Q.length === 9)
        return A = parseInt(Q.substr(1, 6), 16), [A >> 16, A >> 8 & g, A & g, parseInt(Q.substr(7), 16) / 255];
      Q = parseInt(Q.substr(1), 16), A = [Q >> 16, Q >> 8 & g, Q & g];
    } else if (Q.substr(0, 3) === "hsl") {
      if (A = a = Q.match(eA), !t)
        s = +A[0] % 360 / 360, v = +A[1] / 100, c = +A[2] / 100, P = c <= 0.5 ? c * (v + 1) : c + v - c * v, e = c * 2 - P, A.length > 3 && (A[3] *= 1), A[0] = Wt(s + 1 / 3, e, P), A[1] = Wt(s, e, P), A[2] = Wt(s - 1 / 3, e, P);
      else if (~Q.indexOf("="))
        return A = Q.match(GA), B && A.length < 4 && (A[3] = 1), A;
    } else
      A = Q.match(eA) || ot.transparent;
    A = A.map(Number);
  }
  return t && !a && (e = A[0] / g, P = A[1] / g, r = A[2] / g, n = Math.max(e, P, r), T = Math.min(e, P, r), c = (n + T) / 2, n === T ? s = v = 0 : (z = n - T, v = c > 0.5 ? z / (2 - n - T) : z / (n + T), s = n === e ? (P - r) / z + (P < r ? 6 : 0) : n === P ? (r - e) / z + 2 : (e - P) / z + 4, s *= 60), A[0] = ~~(s + 0.5), A[1] = ~~(v * 100 + 0.5), A[2] = ~~(c * 100 + 0.5)), B && A.length < 4 && (A[3] = 1), A;
}, ee = function(Q) {
  var t = [], B = [], A = -1;
  return Q.split(GQ).forEach(function(e) {
    var P = e.match(qQ) || [];
    t.push.apply(t, P), B.push(A += P.length + 1);
  }), t.c = B, t;
}, cA = function(Q, t, B) {
  var A = "", e = (Q + A).match(GQ), P = t ? "hsla(" : "rgba(", r = 0, s, v, c, n;
  if (!e)
    return Q;
  if (e = e.map(function(T) {
    return (T = Ae(T, t, 1)) && P + (t ? T[0] + "," + T[1] + "%," + T[2] + "%," + T[3] : T.join(",")) + ")";
  }), B && (c = ee(Q), s = B.c, s.join(A) !== c.c.join(A)))
    for (v = Q.replace(GQ, "1").split(qQ), n = v.length - 1; r < n; r++)
      A += v[r] + (~s.indexOf(r) ? e.shift() || P + "0,0,0,0)" : (c.length ? c : e.length ? e : B).shift());
  if (!v)
    for (v = Q.split(GQ), n = v.length - 1; r < n; r++)
      A += v[r] + e[r];
  return A + v[n];
}, GQ = function() {
  var i = "(?:\\b(?:(?:rgb|rgba|hsl|hsla)\\(.+?\\))|\\B#(?:[0-9a-f]{3,4}){1,2}\\b", Q;
  for (Q in ot)
    i += "|" + Q + "\\b";
  return new RegExp(i + ")", "gi");
}(), yP = /hsl[a]?\(/, Pe = function(Q) {
  var t = Q.join(" "), B;
  if (GQ.lastIndex = 0, GQ.test(t))
    return B = yP.test(t), Q[1] = cA(Q[1], B), Q[0] = cA(Q[0], B, ee(Q[1])), !0;
}, kt, nQ = function() {
  var i = Date.now, Q = 500, t = 33, B = i(), A = B, e = 1e3 / 240, P = e, r = [], s, v, c, n, T, z, a = function o(w) {
    var E = i() - A, h = w === !0, u, U, k, O;
    if ((E > Q || E < 0) && (B += E - t), A += E, k = A - B, u = k - P, (u > 0 || h) && (O = ++n.frame, T = k - n.time * 1e3, n.time = k = k / 1e3, P += u + (u >= e ? 4 : e - u), U = 1), h || (s = v(o)), U)
  ...
  return n = {
    ...
    wake: function() {
      pA && (!oB && LB() && (kQ = oB = window, MB = kQ.document || {}, aQ.gsap = rQ, (kQ.gsapVersions || (kQ.gsapVersions = [])).push(rQ.version), gA(ft || kQ.GreenSockGlobals || !kQ.gsap && kQ || {}), te.forEach(Be)), c = typeof requestAnimationFrame < "u" && requestAnimationFrame, s && n.sleep(), v = c || function(w) {
        return setTimeout(w, P - n.time * 1e3 + 1 | 0);
      }, kt = 1, a(2));
    },
    sleep: function() {
      (c ? cancelAnimationFrame : clearTimeout)(s), kt = 0, v = ht;
    },
    lagSmoothing: function(w, E) {
      Q = w || 1 / 0, t = Math.min(E || 33, Q);
    },
    fps: function(w) {
      e = 1e3 / (w || 240), P = n.time * 1e3 + e;
    },
    add: function(w, E, h) {
      var u = E ? function(U, k, O, D) {
        w(U, k, O, D), n.remove(u);
      } : w;
      return n.remove(w), r[h ? "unshift" : "push"](u), Pt(), u;
    },
    remove: function(w, E) {
  ...
  return !kt && nQ.wake();
}, p = {}, IP = /^[\d.\-M][\d.\-,\s]/, YP = /["']/g, SP = function(Q) {
  for (var t = {}, B = Q.substr(1, Q.length - 3).split(":"), A = B[0], e = 1, P = B.length, r, s, v; e < P; e++)
    s = B[e], r = e !== P - 1 ? s.lastIndexOf(",") : s.length, v = s.substr(0, r), t[A] = isNaN(v) ? v.replace(YP, "").trim() : +v, A = s.substr(r + 1).trim();
  return t;
}, VP = function(Q) {
  var t = Q.indexOf("(") + 1, B = Q.indexOf(")"), A = Q.indexOf("(", t);
  return Q.substring(t, ~A && A < B ? Q.indexOf(")", B + 1) : B);
}, JP = function(Q) {
  var t = (Q + "").split("("), B = p[t[0]];
  return B && t.length > 1 && B.config ? B.config.apply(null, ~Q.indexOf("{") ? [SP(t[1])] : VP(Q).split(",").map(YA)) : p._CE && IP.test(Q) ? p._CE("", Q) : B;
}, RP = function(Q) {
  return function(t) {
    return 1 - Q(1 - t);
  };
}, JQ = function(Q, t) {
  return Q && (J(Q) ? Q : p[Q] || JP(Q)) || t;
}, XQ = function(Q, t, B, A) {
  B === void 0 && (B = function(s) {
    return 1 - t(1 - s);
  }), A === void 0 && (A = function(s) {
    return s < 0.5 ? t(s * 2) / 2 : 1 - t((1 - s) * 2) / 2;
  });
  var e = {
    easeIn: t,
    easeOut: B,
  ...
  return eQ(Q, function(r) {
    p[r] = aQ[r] = e, p[P = r.toLowerCase()] = B;
    for (var s in e)
      p[P + (s === "easeIn" ? ".in" : s === "easeOut" ? ".out" : ".inOut")] = p[r + "." + s] = e[s];
  }), e;
}, ie = function(Q) {
  return function(t) {
    return t < 0.5 ? (1 - Q(1 - t * 2)) / 2 : 0.5 + Q((t - 0.5) * 2) / 2;
  };
}, qt = function i(Q, t, B) {
  var A = t >= 1 ? t : 1, e = (B || (Q ? 0.3 : 0.45)) / (t < 1 ? t : 1), P = e / nB * (Math.asin(1 / A) || 0), r = function(c) {
    return c === 1 ? 1 : A * Math.pow(2, -10 * c) * wP((c - P) * e) + 1;
  }, s = Q === "out" ? r : Q === "in" ? function(v) {
    return 1 - r(1 - v);
  } : ie(r);
  return e = nB / e, s.config = function(v, c) {
    return i(Q, v, c);
  }, s;
}, _t = function i(Q, t) {
  t === void 0 && (t = 1.70158);
  var B = function(P) {
    return P ? --P * P * ((t + 1) * P + t) + 1 : 0;
  }, A = Q === "out" ? B : Q === "in" ? function(e) {
    return 1 - B(1 - e);
  } : ie(B);
...
eQ("Linear,Quad,Cubic,Quart,Quint,Strong", function(i, Q) {
  var t = Q < 5 ? Q + 1 : Q;
  XQ(i + ",Power" + (t - 1), Q ? function(B) {
    return Math.pow(B, t);
  } : function(B) {
    return B;
  }, function(B) {
    return 1 - Math.pow(1 - B, t);
  }, function(B) {
    return B < 0.5 ? Math.pow(B * 2, t) / 2 : 1 - Math.pow((1 - B) * 2, t) / 2;
  });
});
p.Linear.easeNone = p.none = p.Linear.easeIn;
XQ("Elastic", qt("in"), qt("out"), qt());
(function(i, Q) {
  var t = 1 / Q, B = 2 * t, A = 2.5 * t, e = function(r) {
    return r < t ? i * r * r : r < B ? i * Math.pow(r - 1.5 / Q, 2) + 0.75 : r < A ? i * (r -= 2.25 / Q) * r + 0.9375 : i * Math.pow(r - 2.625 / Q, 2) + 0.984375;
  };
  XQ("Bounce", function(P) {
    return 1 - e(1 - P);
  }, e);
})(7.5625, 2.75);
XQ("Expo", function(i) {
...
XQ("Sine", function(i) {
  return i === 1 ? 1 : -aP(i * TP) + 1;
});
XQ("Back", _t("in"), _t("out"), _t());
p.SteppedEase = p.steps = aQ.SteppedEase = {
  config: function(Q, t) {
    Q === void 0 && (Q = 1);
    var B = 1 / Q, A = Q + (t ? 0 : 1), e = t ? 1 : 0, P = 1 - m;
    return function(r) {
      return ((A * dt(0, P, r) | 0) + e) * B;
    };
  }
};
wt.ease = p["quad.out"];
eQ("onComplete,onUpdate,onStart,onRepeat,onReverseComplete,onInterrupt", function(i) {
  return FB += i + "," + i + "Params,";
});
var re = function(Q, t) {
  this.id = zP++, Q._gsap = this, this.target = Q, this.harness = t, this.get = t ? t.get : yA, this.set = t ? t.getSetter : mB;
}, ut = /* @__PURE__ */ function() {
  function i(t) {
    this.vars = t, this._delay = +t.delay || 0, (this._repeat = t.repeat === 1 / 0 ? -2 : t.repeat || 0) && (this._rDelay = t.repeatDelay || 0, this._yoyo = !!t.yoyo || !!t.yoyoEase), this._ts = 1, et(this, +t.duration, 1, 1), this.data = t.data, y && (this._ctx = y, y.data.push(this)), kt || nQ.wake();
  }
  var Q = i.prototype;
  return Q.delay = function(B) {
  ...
  }, Q.duration = function(B) {
    return arguments.length ? this.totalDuration(this._repeat > 0 ? B + (B + this._rDelay) * this._repeat : B) : this.totalDuration() && this._dur;
  }, Q.totalDuration = function(B) {
    return arguments.length ? (this._dirty = 0, et(this, this._repeat < 0 ? B : (B - this._repeat * this._rDelay) / (this._repeat + 1))) : this._tDur;
  }, Q.totalTime = function(B, A) {
    if (Pt(), !arguments.length)
      return this._tTime;
    var e = this._dp;
    if (e && e.smoothChildTiming && this._ts) {
      for (St(this, B), !e._dp || e.parent || JA(e, this); e && e.parent; )
        e.parent._time !== e._start + (e._ts >= 0 ? e._tTime / e._ts : (e.totalDuration() - e._tTime) / -e._ts) && e.totalTime(e._tTime, !0), e = e.parent;
      !this.parent && this._dp.autoRemoveChildren && (this._ts > 0 && B < this._tDur || this._ts < 0 && B > 0 || !this._tDur && !B) && uQ(this._dp, this, this._start - this._delay);
    }
    return (this._tTime !== B || !this._dur && !A || this._initted && Math.abs(this._zTime) === m || !this._initted && this._dur && B || !B && !this._initted && (this.add || this._ptLookup)) && (this._ts || (this._pTime = B), IA(this, B, A)), this;
  }, Q.time = function(B, A) {
    return arguments.length ? this.totalTime(Math.min(this.totalDuration(), B + rA(this)) % (this._dur + this._rDelay) || (B ? this._dur : 0), A) : this._time;
  }, Q.totalProgress = function(B, A) {
  ...
  }, Q.progress = function(B, A) {
    return arguments.length ? this.totalTime(this.duration() * (this._yoyo && !(this.iteration() & 1) ? 1 - B : B) + rA(this), A) : this.duration() ? Math.min(1, this._time / this._dur) : this.rawTime() > 0 ? 1 : 0;
  }, Q.iteration = function(B, A) {
    var e = this.duration() + this._rDelay;
    return arguments.length ? this.totalTime(this._time + (B - 1) * e, A) : this._repeat ? At(this._tTime, e) + 1 : 1;
  }, Q.timeScale = function(B, A) {
    if (!arguments.length)
      return this._rts === -m ? 0 : this._rts;
    if (this._rts === B)
      return this;
    var e = this.parent && this._ts ? pt(this.parent._time, this) : this._tTime;
    return this._rts = +B || 0, this._ts = this._ps || B === -m ? 0 : this._rts, this.totalTime(dt(-Math.abs(this._delay), this.totalDuration(), e), A !== !1), Yt(this), NP(this);
  }, Q.paused = function(B) {
    return arguments.length ? (this._ps !== B && (this._ps = B, B ? (this._pTime = this._tTime || Math.max(-this._delay, this.rawTime()), this._ts = this._act = 0) : (Pt(), this._ts = this._rts, this.totalTime(this.parent && !this.parent.smoothChildTiming ? this.rawTime() : this._tTime || this._pTime, this.progress() === 1 && Math.abs(this._zTime) !== m && (this._tTime -= m)))), this) : this._ps;
  }, Q.startTime = function(B) {
    if (arguments.length) {
      this._start = Y(B);
      var A = this.parent || this._dp;
      return A && (A._sort || !this.parent) && uQ(A, this, this._start - this._delay), this;
    }
    return this._start;
  }, Q.endTime = function(B) {
    return this._start + (AQ(B) ? this.totalDuration() : this.duration()) / Math.abs(this._ts || 1);
  }, Q.rawTime = function(B) {
    var A = this.parent || this._dp;
    return A ? B && (!this._ts || this._repeat && this._time && this.totalProgress() < 1) ? this._tTime % (this._dur + this._rDelay) : this._ts ? pt(A.rawTime(B), this) : this._tTime : this._tTime;
  }, Q.revert = function(B) {
    B === void 0 && (B = UP);
    var A = q;
    return q = B, GB(this) && (this.timeline && this.timeline.revert(B), this.totalTime(-0.01, B.suppressEvents)), this.data !== "nested" && B.kill !== !1 && this.kill(), q = A, this;
  }, Q.globalTime = function(B) {
    ...
    return arguments.length ? (this._repeat = B === 1 / 0 ? -2 : B, sA(this)) : this._repeat === -2 ? 1 / 0 : this._repeat;
  }, Q.repeatDelay = function(B) {
    if (arguments.length) {
      var A = this._time;
      return this._rDelay = B, sA(this), A ? this.time(A) : this;
    }
    return this._rDelay;
  }, Q.yoyo = function(B) {
    return arguments.length ? (this._yoyo = B, this) : this._yoyo;
  }, Q.seek = function(B, A) {
    return this.totalTime(EQ(this, B), AQ(A));
  }, Q.restart = function(B, A) {
    return this.play().totalTime(B ? -this._delay : 0, AQ(A)), this._dur || (this._zTime = -m), this;
  }, Q.play = function(B, A) {
    return B != null && this.seek(B, A), this.reversed(!1).paused(!1);
  }, Q.reverse = function(B, A) {
    return B != null && this.seek(B || this.totalDuration(), A), this.reversed(!0).paused(!1);
  }, Q.pause = function(B, A) {
    return B != null && this.seek(B, A), this.paused(!0);
  }, Q.resume = function() {
    return this.paused(!1);
  }, Q.reversed = function(B) {
  ...
  }, Q.eventCallback = function(B, A, e) {
    var P = this.vars;
    return arguments.length > 1 ? (A ? (P[B] = A, e && (P[B + "Params"] = e), B === "onUpdate" && (this._onUpdate = A)) : delete P[B], this) : P[B];
  }, Q.then = function(B) {
    var A = this, e = A._prom;
    return new Promise(function(P) {
      var r = J(B) ? B : SA, s = function() {
        var c = A.then;
        A.then = null, e && e(), J(r) && (r = r(A)) && (r.then || r === A) && (A.then = c), P(r), A.then = c;
      };
...
var BQ = /* @__PURE__ */ function(i) {
  jA(Q, i);
  function Q(B, A) {
    var e;
    return B === void 0 && (B = {}), e = i.call(this, B) || this, e.labels = {}, e.smoothChildTiming = !!B.smoothChildTiming, e.autoRemoveChildren = !!B.autoRemoveChildren, e._sort = AQ(B.sortChildren), S && uQ(B.parent || S, dQ(e), A), B.reversed && e.reverse(), B.paused && e.paused(!0), B.scrollTrigger && RA(dQ(e), B.scrollTrigger), e;
  }
  var t = Q.prototype;
  return t.to = function(A, e, P) {
    return zt(0, arguments, this), this;
  }, t.from = function(A, e, P) {
    return zt(1, arguments, this), this;
  }, t.fromTo = function(A, e, P, r) {
    return zt(2, arguments, this), this;
  }, t.set = function(A, e, P) {
    return e.duration = 0, e.parent = this, Tt(e).repeatDelay || (e.repeat = 0), e.immediateRender = !!e.immediateRender, new K(A, e, EQ(this, P), 1), this;
  }, t.call = function(A, e, P) {
    return uQ(this, K.delayedCall(0, A, e), P);
  }, t.staggerTo = function(A, e, P, r, s, v, c) {
    return P.duration = e, P.stagger = P.stagger || r, P.onComplete = v, P.onCompleteParams = c, P.parent = this, new K(A, P, EQ(this, s)), this;
  }, t.staggerFrom = function(A, e, P, r, s, v, c) {
    return P.runBackwards = 1, Tt(P).immediateRender = AQ(P.immediateRender), this.staggerTo(A, e, P, r, s, v, c);
  }, t.staggerFromTo = function(A, e, P, r, s, v, c, n) {
    return r.startAt = P, Tt(r).immediateRender = AQ(r.immediateRender), this.staggerTo(A, e, r, s, v, c, n);
  }, t.render = function(A, e, P) {
    ...
    if (this !== S && c > s && A >= 0 && (c = s), c !== this._tTime || P || n) {
      if (r !== this._time && v && (c += this._time - r, A += this._time - r), T = c, U = this._start, u = this._ts, E = !u, n && (v || (r = this._zTime), (A || !e) && (this._zTime = A)), this._repeat) {
        if (O = this._yoyo, w = v + this._rDelay, this._repeat < -1 && A < 0)
          return this.totalTime(w * 100 + A, e, P);
        if (T = Y(c % w), c === s ? (o = this._repeat, T = v) : (k = Y(c / w), o = ~~k, o && o === k && (T = v, o--), T > v && (T = v)), k = At(this._tTime, w), !r && this._tTime && k !== o && this._tTime - k * w - this._dur <= 0 && (k = o), O && o & 1 && (T = v - T, D = 1), o !== k && !this._lock) {
          var N = O && k & 1, d = N === (O && o & 1);
          if (o < k && (N = !N), r = N ? 0 : c % v ? v : c, this._lock = 1, this.render(r || (D ? 0 : Y(o * w)), e, !v)._lock = 0, this._tTime = c, !e && this.parent && oQ(this, "onRepeat"), this.vars.repeatRefresh && !D && (this.invalidate()._lock = 1, k = o), r && r !== this._time || E !== !this._ts || this.vars.onRepeat && !this.parent && !this._act)
            return this;
      ...
      if (this._hasPause && !this._forcing && this._lock < 2 && (h = HP(this, Y(r), Y(T)), h && (c -= T - (T = h._start))), this._tTime = c, this._time = T, this._act = !!u, this._initted || (this._onUpdate = this.vars.onUpdate, this._initted = 1, this._zTime = A, r = 0), !r && c && v && !e && !k && (oQ(this, "onStart"), this._tTime !== c))
        return this;
      if (T >= r && A >= 0)
        for (z = this._first; z; ) {
        ...
        for (var M = A < 0 ? A : T; z; ) {
          if (a = z._prev, (z._act || M <= z._end) && z._ts && h !== z) {
            if (z.parent !== this)
              return this.render(A, e, P);
            if (z.render(z._ts > 0 ? (M - z._start) * z._ts : (z._dirty ? z.totalDuration() : z._tDur) + (M - z._start) * z._ts, e, P || q && GB(z)), T !== this._time || !this._ts && !E) {
      ...
      if (h && !e && (this.pause(), h.render(T >= r ? 0 : -m)._zTime = T >= r ? 1 : -1, this._ts))
        return this._start = U, Yt(this), this.render(A, e, P);
      this._onUpdate && !e && oQ(this, "onUpdate", !0), (c === s && this._tTime >= this.totalDuration() || !c && r) && (U === this._start || Math.abs(u) !== Math.abs(this._ts)) && (this._lock || ((A || !v) && (c === s && this._ts > 0 || !c && this._ts < 0) && bQ(this, 1), !e && !(A < 0 && !r) && (c || r || !s) && (oQ(this, c === s && A >= 0 ? "onComplete" : "onReverseComplete", !0), this._prom && !(c < s && this.timeScale() > 0) && this._prom())));
    }
    return this;
  }, t.add = function(A, e) {
    var P = this;
    if (NQ(e) || (e = EQ(this, e, A)), !(A instanceof ut)) {
      if ($(A))
    ...
    return this !== A ? uQ(this, A, e) : this;
  }, t.getChildren = function(A, e, P, r) {
    A === void 0 && (A = !0), e === void 0 && (e = !0), P === void 0 && (P = !0), r === void 0 && (r = -hQ);
    for (var s = [], v = this._first; v; )
      v._start >= r && (v instanceof K ? e && s.push(v) : (P && s.push(v), A && s.push.apply(s, v.getChildren(!0, e, P)))), v = v._next;
    return s;
  }, t.getById = function(A) {
    for (var e = this.getChildren(1, 1, 1), P = e.length; P--; )
      if (e[P].vars.id === A)
        return e[P];
  }, t.remove = function(A) {
    return Z(A) ? this.removeLabel(A) : J(A) ? this.killTweensOf(A) : (A.parent === this && It(this, A), A === this._recent && (this._recent = this._last), VQ(this));
  }, t.totalTime = function(A, e) {
    return arguments.length ? (this._forcing = 1, !this._dp && this._ts && (this._start = Y(nQ.time - (this._ts > 0 ? A / this._ts : (this.totalDuration() - A) / -this._ts))), i.prototype.totalTime.call(this, A, e), this._forcing = 0, this) : this._tTime;
  }, t.addLabel = function(A, e) {
    return this.labels[A] = EQ(this, e), this;
  }, t.removeLabel = function(A) {
    return delete this.labels[A], this;
  }, t.addPause = function(A, e, P) {
    var r = K.delayedCall(0, e || ht, P);
    return r.data = "isPause", this._hasPause = 1, uQ(this, r, EQ(this, A));
  }, t.removePause = function(A) {
    var e = this._first;
    for (A = EQ(this, A); e; )
      e._start === A && e.data === "isPause" && bQ(e), e = e._next;
  }, t.killTweensOf = function(A, e, P) {
    for (var r = this.getTweensOf(A, P), s = r.length; s--; )
      HQ !== r[s] && r[s].kill(A, e);
    return this;
  }, t.getTweensOf = function(A, e) {
    for (var P = [], r = lQ(A), s = this._first, v = NQ(e), c; s; )
      s instanceof K ? DP(s._targets, r) && (v ? (!HQ || s._initted && s._ts) && s.globalTime(0) <= e && s.globalTime(s.totalDuration()) > e : !e || s.isActive()) && P.push(s) : (c = s.getTweensOf(r, e)).length && P.push.apply(P, c), s = s._next;
    return P;
  }, t.tweenTo = function(A, e) {
    e = e || {};
    var P = this, r = EQ(P, A), s = e, v = s.startAt, c = s.onStart, n = s.onStartParams, T = s.immediateRender, z, a = K.to(P, wQ({
      ease: e.ease || "none",
      lazy: !1,
      immediateRender: !1,
      time: r,
      overwrite: "auto",
      duration: e.duration || Math.abs((r - (v && "time" in v ? v.time : P._time)) / P.timeScale()) || m,
      onStart: function() {
        if (P.pause(), !z) {
          var w = e.duration || Math.abs((r - (v && "time" in v ? v.time : P._time)) / P.timeScale());
          a._dur !== w && et(a, w, 0, 1).render(a._time, !0, !0), z = 1;
        }
        c && c.apply(a, n || []);
      }
    }, e));
    return T ? a.render(0) : a;
  }, t.tweenFromTo = function(A, e, P) {
    return this.tweenTo(e, wQ({
      startAt: {
        time: EQ(this, A)
      }
    }, P));
    ...
    return A === void 0 && (A = this._time), vA(this, EQ(this, A));
  }, t.previousLabel = function(A) {
    return A === void 0 && (A = this._time), vA(this, EQ(this, A), 1);
  }, t.currentLabel = function(A) {
    return arguments.length ? this.seek(A, !0) : this.previousLabel(this._time + m);
  }, t.shiftChildren = function(A, e, P) {
    P === void 0 && (P = 0);
    var r = this._first, s = this.labels, v;
    for (A = Y(A); r; )
      r._start >= P && (r._start += A, r._end += A), r = r._next;
    if (e)
      for (v in s)
        s[v] >= P && (s[v] += A);
    return VQ(this);
    ...
    A === void 0 && (A = !0);
    for (var e = this._first, P; e; )
      P = e._next, this.remove(e), e = P;
    return this._dp && (this._time = this._tTime = this._pTime = 0), A && (this.labels = {}), VQ(this);
  }, t.totalDuration = function(A) {
    var e = 0, P = this, r = P._last, s = hQ, v, c, n;
    if (arguments.length)
      return P.timeScale((P._repeat < 0 ? P.duration() : P.totalDuration()) / (P.reversed() ? -A : A));
    if (P._dirty) {
      for (n = P.parent; r; )
        v = r._prev, r._dirty && r.totalDuration(), c = r._start, c > s && P._sort && r._ts && !P._lock ? (P._lock = 1, uQ(P, r, c - r._delay, 1)._lock = 0) : s = c, c < 0 && r._ts && (e -= c, (!n && !P._dp || n && n.smoothChildTiming) && (P._start += Y(c / P._ts), P._time -= c, P._tTime -= c), P.shiftChildren(-c, !1, -1 / 0), s = 0), r._end > e && r._ts && (e = r._end), r = v;
      et(P, P === S && P._time > e ? P._time : e, 1, 1), P._dirty = 0;
    }
    return P._tDur;
  }, Q.updateRoot = function(A) {
    if (S._ts && (IA(S, pt(A, S)), xA = nQ.frame), nQ.frame >= PA) {
      PA += zQ.autoSleep || 120;
      var e = S._first;
      if ((!e || !e._ts) && zQ.autoSleep && nQ._listeners.length < 2) {
        for (; e && !e._ts; )
          e = e._next;
        e || nQ.sleep();
      }
...
var XP = function(Q, t, B, A, e, P, r) {
  var s = new PQ(this._pt, Q, t, 0, 1, Te, null, e), v = 0, c = 0, n, T, z, a, o, w, E, h;
  for (s.b = B, s.e = A, B += "", A += "", (E = ~A.indexOf("random(")) && (A = lt(A)), P && (h = [B, A], P(h, Q, t), B = h[0], A = h[1]), T = B.match(Kt) || []; n = Kt.exec(A); )
    a = n[0], o = A.substring(v, n.index), z ? z = (z + 1) % 5 : o.substr(-5) === "rgba(" && (z = 1), a !== T[c++] && (w = parseFloat(T[c - 1]) || 0, s._pt = {
      ...
      s: w,
      c: a.charAt(1) === "=" ? $Q(w, a) - w : parseFloat(a) - w,
      m: z && z < 4 ? Math.round : 0
    }, v = Kt.lastIndex);
  return s.c = v < A.length ? A.substring(v, A.length) : "", s.fp = r, (bA.test(A) || E) && (s.e = 0), this._pt = s, s;
}, bB = function(Q, t, B, A, e, P, r, s, v, c) {
  J(A) && (A = A(e || 0, Q, P));
  var n = Q[t], T = B !== "get" ? B : J(n) ? v ? Q[t.indexOf("set") || !J(Q["get" + t.substr(3)]) ? t : "get" + t.substr(3)](v) : Q[t]() : n, z = J(n) ? v ? _P : ne : gB, a;
  if (Z(A) && (~A.indexOf("random(") && (A = lt(A)), A.charAt(1) === "=" && (a = $Q(T, A) + (_(T) || 0), (a || a === 0) && (A = a))), !c || T !== A || lB)
    return !isNaN(T * A) && A !== "" ? (a = new PQ(this._pt, Q, t, +T || 0, A - (T || 0), typeof n == "boolean" ? Qi : oe, 0, z), v && (a.fp = v), r && a.modifier(r, this, Q), this._pt = a) : (!n && !(t in Q) && HB(t, A), XP.call(this, Q, t, T, A, z, s || zQ.stringFilter, v));
}, KP = function(Q, t, B, A, e) {
  if (J(Q) && (Q = at(Q, e, t, B, A)), !DQ(Q) || Q.style && Q.nodeType || $(Q) || fA(Q))
    return Z(Q) ? at(Q, e, t, B, A) : Q;
  var P = {}, r;
  for (r in Q)
    P[r] = at(Q[r], e, t, B, A);
  return P;
}, se = function(Q, t, B, A, e, P) {
  var r, s, v, c;
  if (cQ[Q] && (r = new cQ[Q]()).init(e, r.rawVars ? t[Q] : KP(t[Q], A, e, P, B), B, A, P) !== !1 && (B._pt = s = new PQ(B._pt, e, Q, 0, 1, r.render, r, 0, r.priority), B !== _Q))
    for (v = B._ptLookup[B._targets.indexOf(e)], c = r._props.length; c--; )
      v[r._props[c]] = s;
  return r;
}, HQ, lB, pB = function i(Q, t, B) {
  var A = Q.vars, e = A.ease, P = A.startAt, r = A.immediateRender, s = A.lazy, v = A.onUpdate, c = A.runBackwards, n = A.yoyoEase, T = A.keyframes, z = A.autoRevert, a = Q._dur, o = Q._startAt, w = Q._targets, E = Q.parent, h = E && E.data === "nested" ? E.vars.targets : w, u = Q._overwrite === "auto" && !NB, U = Q.timeline, k = A.easeReverse || n, O, D, N, d, M, b, G, C, j, F, H, f, I;
  if (U && (!T || !e) && (e = "none"), Q._ease = JQ(e, wt.ease), Q._rEase = k && (JQ(k) || Q._ease), Q._from = !U && !!A.runBackwards, Q._from && (Q.ratio = 1), !U || T && !A.stagger) {
    if (C = w[0] ? SQ(w[0]).harness : 0, f = C && A[C.prop], O = bt(A, jB), o && (o._zTime < 0 && o.progress(1), t < 0 && c && r && !z ? o.render(-1, !0) : o.revert(c && a ? Mt : uP), o._lazy = 0), P) {
      if (bQ(Q._startAt = K.set(w, wQ({
        data: "isStart",
        overwrite: !1,
        parent: E,
        immediateRender: !0,
        ...
        delay: 0,
        onUpdate: v && function() {
          return oQ(Q, "onUpdate");
        },
        stagger: 0
      }, P))), Q._startAt._dp = 0, Q._startAt._sat = Q, t < 0 && (q || !r && !z) && Q._startAt.revert(Mt), r && a && t <= 0 && B <= 0) {
        t && (Q._zTime = t);
        return;
      }
    } else if (c && a && !o) {
      ...
      }, O), f && (N[C.prop] = f), bQ(Q._startAt = K.set(w, N)), Q._startAt._dp = 0, Q._startAt._sat = Q, t < 0 && (q ? Q._startAt.revert(Mt) : Q._startAt.render(-1, !0)), Q._zTime = t, !r)
        i(Q._startAt, m, m);
      else if (!t)
        return;
    }
    for (Q._pt = Q._ptCache = 0, s = a && AQ(s) || s && !a, D = 0; D < w.length; D++) {
      if (M = w[D], G = M._gsap || fB(w)[D]._gsap, Q._ptLookup[D] = F = {}, TB[G.id] && fQ.length && Gt(), H = h === w ? D : h.indexOf(M), C && (j = new C()).init(M, f || O, Q, H, h) !== !1 && (Q._pt = d = new PQ(Q._pt, M, j.name, 0, 1, j.render, j, 0, j.priority), j._props.forEach(function(R) {
        F[R] = d;
      }), j.priority && (b = 1)), !C || f)
        for (N in O)
          cQ[N] && (j = se(N, O, Q, H, M, h)) ? j.priority && (b = 1) : F[N] = d = bB.call(Q, M, N, "get", O[N], H, h, 0, A.stringFilter);
      Q._op && Q._op[D] && Q.kill(M, Q._op[D]), u && Q._pt && (HQ = Q, S.killTweensOf(M, F, Q.globalTime(t)), I = !Q.parent, HQ = 0), Q._pt && s && (TB[G.id] = 1);
    }
    b && ze(Q), Q._onInit && Q._onInit(Q);
  }
  Q._onUpdate = v, Q._initted = (!Q._op || Q._pt) && !I, T && t <= 0 && U.render(hQ, !0, !0);
}, ZP = function(Q, t, B, A, e, P, r, s) {
  var v = (Q._pt && Q._ptCache || (Q._ptCache = {}))[t], c, n, T, z;
  if (!v)
    for (v = Q._ptCache[t] = [], T = Q._ptLookup, z = Q._targets.length; z--; ) {
      if (c = T[z][t], c && c.d && c.d._pt)
        for (c = c.d._pt; c && c.p !== t && c.fp !== t; )
          c = c._next;
      if (!c)
        return lB = 1, Q.vars[t] = "+=0", pB(Q, r), lB = 0, s ? Et(t + " not eligible for reset. Try splitting into individual properties") : 1;
      v.push(c);
    }
  for (z = v.length; z--; )
    n = v[z], c = n._pt || n, c.s = (A || A === 0) && !e ? A : c.s + (A || 0) + P * c.c, c.c = B - c.s, n.e && (n.e = X(B) + _(n.e)), n.b && (n.b = c.s + _(n.b));
}, WP = function(Q, t) {
  var B = Q[0] ? SQ(Q[0]).harness : 0, A = B && B.aliases, e, P, r, s;
  if (!A)
    return t;
  e = Bt({}, t);
  for (P in A)
    if (P in e)
      for (s = A[P].split(","), r = s.length; r--; )
        e[s[r]] = e[P];
  return e;
}, qP = function(Q, t, B, A) {
  var e = t.ease || A || "power1.inOut", P, r;
  if ($(t))
    r = B[Q] || (B[Q] = []), t.forEach(function(s, v) {
      return r.push({
        t: v / (t.length - 1) * 100,
        v: s,
        e
    ...
    for (P in t)
      r = B[P] || (B[P] = []), P === "ease" || r.push({
        t: parseFloat(Q),
        v: t[P],
        e
      });
}, at = function(Q, t, B, A, e) {
  return J(Q) ? Q.call(t, B, A, e) : Z(Q) && ~Q.indexOf("random(") ? lt(Q) : Q;
}, ve = FB + "repeat,repeatDelay,yoyo,repeatRefresh,yoyoEase,easeReverse,autoRevert", ce = {};
eQ(ve + ",id,stagger,delay,duration,paused,scrollTrigger", function(i) {
  return ce[i] = 1;
});
var K = /* @__PURE__ */ function(i) {
  ...
  function Q(B, A, e, P) {
    var r;
    typeof A == "number" && (e.duration = A, A = e, e = null), r = i.call(this, P ? A : Tt(A)) || this;
    var s = r.vars, v = s.duration, c = s.delay, n = s.immediateRender, T = s.stagger, z = s.overwrite, a = s.keyframes, o = s.defaults, w = s.scrollTrigger, E = A.parent || S, h = ($(B) || fA(B) ? NQ(B[0]) : "length" in A) ? [B] : lQ(B), u, U, k, O, D, N, d, M;
    if (r._targets = h.length ? fB(h) : Et("GSAP target " + B + " not found. https://gsap.com", !zQ.nullTargetWarn) || [], r._ptLookup = [], r._overwrite = z, a || T || Lt(v) || Lt(c)) {
      A = r.vars;
      var b = A.easeReverse || A.yoyoEase;
      if (u = r.timeline = new BQ({
        data: "nested",
        defaults: o || {},
        targets: E && E.data === "nested" ? E.vars.targets : h
      }), u.kill(), u.parent = u._dp = dQ(r), u._start = 0, T || Lt(v) || Lt(c)) {
        if (O = h.length, d = T && WA(T), DQ(T))
          for (D in T)
            ~ve.indexOf(D) && (M || (M = {}), M[D] = T[D]);
        for (U = 0; U < O; U++)
          k = bt(A, ce), k.stagger = 0, b && (k.easeReverse = b), M && Bt(k, M), N = h[U], k.duration = +at(v, dQ(r), U, N, h), k.delay = (+at(c, dQ(r), U, N, h) || 0) - r._delay, !T && O === 1 && k.delay && (r._delay = c = k.delay, r._start += c, k.delay = 0), u.to(N, k, d ? d(U, N, h) : 0), u._ease = p.none;
        u.duration() ? v = c = 0 : r.timeline = 0;
      } else if (a) {
        Tt(wQ(u.vars.defaults, {
          ease: "none"
        })), u._ease = JQ(a.ease || A.ease || "none");
        var G = 0, C, j, F;
        if ($(a))
          ...
          k = {};
          for (D in a)
            D === "ease" || D === "easeEach" || qP(D, a[D], k, a.easeEach);
          for (D in k)
            for (C = k[D].sort(function(H, f) {
              return H.t - f.t;
            }), G = 0, U = 0; U < C.length; U++)
              j = C[U], F = {
                ease: j.e,
                duration: (j.t - (U ? C[U - 1].t : 0)) / 100 * v
              }, F[D] = j.v, u.to(h, F, G), G += F.duration;
          u.duration() < v && u.to({}, {
      ...
      r.timeline = 0;
    return z === !0 && !NB && (HQ = dQ(r), S.killTweensOf(h), HQ = 0), uQ(E, dQ(r), e), A.reversed && r.reverse(), A.paused && r.paused(!0), (n || !v && !a && r._start === Y(E._time) && AQ(n) && OP(dQ(r)) && E.data !== "nested") && (r._tTime = -m, r.render(Math.max(0, -c) || 0)), w && RA(dQ(r), w), r;
  }
  var t = Q.prototype;
  return t.render = function(A, e, P) {
    ...
    if (!v)
      ...
      if (T = n, u = this.timeline, this._repeat) {
        if (o = v + this._rDelay, this._repeat < -1 && c)
          return this.totalTime(o * 100 + A, e, P);
        if (T = Y(n % o), n === s ? (a = this._repeat, T = v) : (w = Y(n / o), a = ~~w, a && a === w ? (T = v, a--) : T > v && (T = v)), E = this._yoyo && a & 1, E && (T = v - T), w = At(this._tTime, o), T === r && !P && this._initted && a === w)
          return this._tTime = n, this;
        a !== w && this.vars.repeatRefresh && !E && !this._lock && T !== o && this._initted && (this._lock = P = 1, this.render(Y(o * a), !0).invalidate()._lock = 0);
      }
      if (!this._initted) {
        if (XA(this, c ? A : T, P, e, n))
          return this._tTime = 0, this;
        if (r !== this._time && !(P && this.vars.repeatRefresh && a !== w))
          return this;
        if (v !== this._dur)
          return this.render(A, e, P);
      }
      ...
      if (this._from && (this.ratio = h = 1 - h), this._tTime = n, this._time = T, !this._act && this._ts && (this._act = 1, this._lazy = 0), !r && n && !e && !w && (oQ(this, "onStart"), this._tTime !== n))
        return this;
      for (z = this._pt; z; )
        z.r(h, z.d), z = z._next;
      u && u.render(A < 0 ? A : u._dur * u._ease(T / this._dur), e, P) || this._startAt && (this._zTime = A), this._onUpdate && !e && (c && zB(this, A, e, P), oQ(this, "onUpdate")), this._repeat && a !== w && this.vars.onRepeat && !e && this.parent && oQ(this, "onRepeat"), (n === this._tDur || !n) && this._tTime === n && (c && !this._onUpdate && zB(this, A, !0, !0), (A || !v) && (n === this._tDur && this._ts > 0 || !n && this._ts < 0) && bQ(this, 1), !e && !(c && !r) && (n || r || E) && (oQ(this, n === s ? "onComplete" : "onReverseComplete", !0), this._prom && !(n < s && this.timeScale() > 0) && this._prom()));
    }
    return this;
  }, t.targets = function() {
  ...
  }, t.resetTo = function(A, e, P, r, s) {
    kt || nQ.wake(), this._ts || this.play();
    var v = Math.min(this._dur, (this._dp._time - this._start) * this._ts), c;
    return this._initted || pB(this, v), c = this._ease(v / this._dur), ZP(this, A, e, P, r, c, v, s) ? this.resetTo(A, e, P, r, 1) : (St(this, 0), this.parent || VA(this._dp, this, "_first", "_last", this._dp._sort ? "_start" : 0), this.render(0));
  }, t.kill = function(A, e) {
    if (e === void 0 && (e = "all"), !A && (!e || e === "all"))
      return this._lazy = this._pt = 0, this.parent ? nt(this) : this.scrollTrigger && this.scrollTrigger.kill(!!q), this;
    if (this.timeline) {
      var P = this.timeline.totalDuration();
      return this.timeline.killTweensOf(A, e, HQ && HQ.vars.overwrite !== !0)._first || nt(this), this.parent && P !== this.timeline.totalDuration() && et(this, this._dur * this.timeline._tDur / P, 0, 1), this;
    }
    var r = this._targets, s = A ? lQ(A) : r, v = this._ptLookup, c = this._pt, n, T, z, a, o, w, E;
    if ((!e || e === "all") && CP(r, s))
      return e === "all" && (this._pt = 0), nt(this);
    for (n = this._op = this._op || [], e !== "all" && (Z(e) && (o = {}, eQ(e, function(h) {
      return o[h] = 1;
    }), e = o), e = WP(r, e)), E = r.length; E--; )
      if (~s.indexOf(r[E])) {
        T = v[E], e === "all" ? (n[E] = e, a = T, z = {}) : (z = n[E] = n[E] || {}, a = e);
        for (o in a)
          w = T && T[o], w && ((!("kill" in w.d) || w.d.kill(o) === !0) && It(this, w, "_pt"), delete T[o]), z !== "all" && (z[o] = 1);
      }
    return this._initted && !this._pt && c && nt(this), this;
  }, Q.to = function(A, e) {
    return new Q(A, e, arguments[2]);
  }, Q.from = function(A, e) {
    return zt(1, arguments);
  }, Q.delayedCall = function(A, e, P, r) {
    return new Q(e, 0, {
      immediateRender: !1,
      lazy: !1,
      overwrite: !1,
  ...
  }, Q.fromTo = function(A, e, P) {
    return zt(2, arguments);
  }, Q.set = function(A, e) {
    return e.duration = 0, e.repeatDelay || (e.repeat = 0), new Q(A, e);
  }, Q.killTweensOf = function(A, e, P) {
    return S.killTweensOf(A, e, P);
  }, Q;
}(ut);
wQ(K.prototype, {
  _targets: [],
...
eQ("staggerTo,staggerFrom,staggerFromTo", function(i) {
  K[i] = function() {
    var Q = new BQ(), t = wB.call(arguments, 0);
    return t.splice(i === "staggerFromTo" ? 5 : 4, 0, 0), Q[i].apply(Q, t);
  };
});
var gB = function(Q, t, B) {
  return Q[t] = B;
}, ne = function(Q, t, B) {
  return Q[t](B);
}, _P = function(Q, t, B, A) {
  ...
  return Q.setAttribute(t, B);
}, mB = function(Q, t) {
  return J(Q[t]) ? ne : OB(Q[t]) && Q.setAttribute ? $P : gB;
}, oe = function(Q, t) {
  return t.set(t.t, t.p, Math.round((t.s + t.c * Q) * 1e6) / 1e6, t);
}, Qi = function(Q, t) {
  return t.set(t.t, t.p, !!(t.s + t.c * Q), t);
}, Te = function(Q, t) {
  var B = t._pt, A = "";
  if (!Q && t.b)
    A = t.b;
    ...
    for (; B; )
      A = B.p + (B.m ? B.m(B.s + B.c * Q) : Math.round((B.s + B.c * Q) * 1e4) / 1e4) + A, B = B._next;
    A += t.c;
  }
  t.set(t.t, t.p, A, t);
}, xB = function(Q, t) {
  ...
  for (var e = this._pt, P; e; )
    P = e._next, e.p === A && e.modifier(Q, t, B), e = P;
}, Bi = function(Q) {
  for (var t = this._pt, B, A; t; )
    A = t._next, t.p === Q && !t.op || t.op === Q ? It(this, t, "_pt") : t.dep || (B = 1), t = A;
  return !B;
}, Ai = function(Q, t, B, A) {
  A.mSet(Q, t, A.m.call(A.tween, B, A.mt), A);
}, ze = function(Q) {
  for (var t = Q._pt, B, A, e, P; t; ) {
    for (B = t._next, A = e; A && A.pr > t.pr; )
      A = A._next;
    (t._prev = A ? A._prev : P) ? t._prev._next = t : e = t, (t._next = A) ? A._prev = t : P = t, t = B;
  }
  Q._pt = e;
}, PQ = /* @__PURE__ */ function() {
  function i(t, B, A, e, P, r, s, v, c) {
    this.t = B, this.s = e, this.c = P, this.p = A, this.r = r || oe, this.d = s || this, this.set = v || gB, this.pr = c || 0, this._next = t, t && (t._prev = this);
  }
  var Q = i.prototype;
  return Q.modifier = function(B, A, e) {
    this.mSet = this.mSet || this.set, this.set = Ai, this.m = B, this.mt = e, this.tween = A;
  }, i;
}();
eQ(FB + "parent,duration,ease,delay,overwrite,runBackwards,startAt,yoyo,immediateRender,repeat,repeatDelay,data,paused,reversed,lazy,callbackScope,stringFilter,id,yoyoEase,stagger,inherit,repeatRefresh,keyframes,autoRevert,scrollTrigger,easeReverse", function(i) {
  return jB[i] = 1;
});
aQ.TweenMax = aQ.TweenLite = K;
aQ.TimelineLite = aQ.TimelineMax = BQ;
S = new BQ({
  sortChildren: !1,
  defaults: wt,
...
zQ.stringFilter = Pe;
var RQ = [], jt = {}, ei = [], nA = 0, Pi = 0, $t = function(Q) {
  return (jt[Q] || ei).map(function(t) {
    return t();
  });
}, kB = function() {
  var Q = Date.now(), t = [];
  Q - nA > 2 && ($t("matchMediaInit"), RQ.forEach(function(B) {
    var A = B.queries, e = B.conditions, P, r, s, v;
    for (r in A)
      P = kQ.matchMedia(A[r]).matches, P && (s = 1), P !== e[r] && (e[r] = P, v = 1);
    v && (B.revert(), s && t.push(B));
  }), $t("matchMediaRevert"), t.forEach(function(B) {
  ...
  function i(t, B) {
    this.selector = B && EB(B), this.data = [], this._r = [], this.isReverted = !1, this.id = Pi++, t && this.add(t);
  }
  var Q = i.prototype;
  return Q.add = function(B, A, e) {
    J(B) && (e = A, A = B, B = J);
    var P = this, r = function() {
      var v = y, c = P.selector, n;
      return v && v !== P && v.data.push(P), e && (P.selector = EB(e)), y = P, n = A.apply(P, arguments), J(n) && P._r.push(n), y = v, P.selector = c, P.isReverted = !1, n;
    };
    return P.last = r, B === J ? r(P, function(s) {
      return P.add(null, s);
    }) : B ? P[B] = r : r;
  }, Q.ignore = function(B) {
    var A = y;
    ...
    return this.data.forEach(function(A) {
      return A instanceof i ? B.push.apply(B, A.getTweens()) : A instanceof K && !(A.parent && A.parent.data === "nested") && B.push(A);
    }), B;
  }, Q.clear = function() {
    ...
    if (B ? function() {
      ...
      for (r.map(function(c) {
        ...
        v = e.data[s], v instanceof BQ ? v.data !== "nested" && (v.scrollTrigger && v.scrollTrigger.revert(), v.kill()) : !(v instanceof K) && v.revert && v.revert(B);
      ...
      for (var P = RQ.length; P--; )
        RQ[P].id === this.id && RQ.splice(P, 1);
  }, Q.revert = function(B) {
    this.kill(B || {});
  }, i;
  ...
  return Q.add = function(B, A, e) {
    ...
    var P = new ae(0, e || this.scope), r = P.conditions = {}, s, v, c;
    y && !P.selector && (P.selector = y.selector), this.contexts.push(P), A = P.add("onMatch", A), P.queries = B;
    for (v in B)
      v === "all" ? c = 1 : (s = kQ.matchMedia(B[v]), s && (RQ.indexOf(P) < 0 && RQ.push(P), (r[v] = s.matches) && (c = 1), s.addListener ? s.addListener(kB) : s.addEventListener("change", kB)));
    return c && A(P, function(n) {
      return P.add(null, n);
    }), this;
  }, Q.revert = function(B) {
    this.kill(B || {});
  ...
  getTweensOf: function(Q, t) {
    return S.getTweensOf(Q, t);
  },
  getProperty: function(Q, t, B, A) {
    Z(Q) && (Q = lQ(Q)[0]);
    var e = SQ(Q || {}).get, P = B ? SA : YA;
    return B === "native" && (B = ""), Q && (t ? P((cQ[t] && cQ[t].get || e)(Q, t, B, A)) : function(r, s, v) {
      return P((cQ[r] && cQ[r].get || e)(Q, r, s, v));
    });
  },
  quickSetter: function(Q, t, B) {
    if (Q = lQ(Q), Q.length > 1) {
      var A = Q.map(function(c) {
        return rQ.quickSetter(c, t, B);
      }), e = A.length;
      return function(c) {
        for (var n = e; n--; )
          A[n](c);
    ...
    Q = Q[0] || {};
    var P = cQ[t], r = SQ(Q), s = r.harness && (r.harness.aliases || {})[t] || t, v = P ? function(c) {
      var n = new P();
      _Q._pt = 0, n.init(Q, B ? c + B : c, _Q, 0, [Q]), n.render(1, n), _Q._pt && xB(1, _Q);
    } : r.set(Q, s);
    return P ? v : function(c) {
      return v(Q, s, B ? c + B : c, r, 1);
    };
  },
  quickTo: function(Q, t, B) {
    var A, e = rQ.to(Q, wQ((A = {}, A[t] = "+=0.1", A.paused = !0, A.stagger = 0, A), B || {})), P = function(s, v, c) {
      return e.resetTo(t, s, v, c);
    };
    return P.tween = e, P;
  },
  isTweening: function(Q) {
    return S.getTweensOf(Q, !0).length > 0;
  },
  defaults: function(Q) {
    return Q && Q.ease && (Q.ease = JQ(Q.ease, wt.ease)), iA(wt, Q || {});
  },
  config: function(Q) {
    return iA(zQ, Q || {});
  },
  registerEffect: function(Q) {
    var t = Q.name, B = Q.effect, A = Q.plugins, e = Q.defaults, P = Q.extendTimeline;
    (A || "").split(",").forEach(function(r) {
      return r && !cQ[r] && !aQ[r] && Et(t + " effect requires " + r + " plugin.");
    }), Zt[t] = function(r, s, v) {
      return B(lQ(r), wQ(s || {}, e), v);
    }, P && (BQ.prototype[t] = function(r, s, v) {
      return this.add(Zt[t](r, DQ(s) ? s : (v = s) && {}, this), v);
    });
  },
  registerEase: function(Q, t) {
    p[Q] = JQ(t);
  },
  parseEase: function(Q, t) {
    return arguments.length ? JQ(Q, t) : p;
  },
  getById: function(Q) {
    return S.getById(Q);
  },
  exportRoot: function(Q, t) {
    Q === void 0 && (Q = {});
    var B = new BQ(Q), A, e;
    for (B.smoothChildTiming = AQ(Q.smoothChildTiming), S.remove(B), B._dp = 0, B._time = B._tTime = S._time, A = S._first; A; )
      e = A._next, (t || !(!A._dur && A instanceof K && A.vars.onComplete === A._targets[0])) && uQ(B, A, A._start - A._delay), A = e;
    return uQ(S, B, 0), B;
  },
  context: function(Q, t) {
    return Q ? new ae(Q, t) : y;
  },
  matchMedia: function(Q) {
    return new ii(Q);
  },
  matchMediaRefresh: function() {
    return RQ.forEach(function(Q) {
      var t = Q.conditions, B, A;
      for (A in t)
        t[A] && (t[A] = !1, B = 1);
      B && Q.revert();
    }) || kB();
  },
  addEventListener: function(Q, t) {
    var B = jt[Q] || (jt[Q] = []);
    ~B.indexOf(t) || B.push(t);
  },
  removeEventListener: function(Q, t) {
    var B = jt[Q], A = B && B.indexOf(t);
    A >= 0 && B.splice(A, 1);
  },
  utils: {
    wrap: gP,
  ...
  core: {
    ...
    context: function(Q) {
      return Q && y && (y.data.push(Q), Q._ctx = y), y;
    },
...
eQ("to,from,fromTo,delayedCall,set,killTweensOf", function(i) {
  return gt[i] = K[i];
});
nQ.add(BQ.updateRoot);
_Q = gt.to({}, {
  duration: 0
});
var ri = function(Q, t) {
  for (var B = Q._pt; B && B.p !== t && B.op !== t && B.fp !== t; )
    B = B._next;
  return B;
}, si = function(Q, t) {
  var B = Q._targets, A, e, P;
  for (A in t)
    for (e = B.length; e--; )
      P = Q._ptLookup[e][A], P && (P = P.d) && (P._pt && (P = ri(P, A)), P && P.modifier && P.modifier(t[A], Q, B[e], A));
}, QB = function(Q, t) {
  return {
    name: Q,
    headless: 1,
    ...
    init: function(A, e, P) {
      P._onInit = function(r) {
        var s, v;
        if (Z(e) && (s = {}, eQ(e, function(c) {
          return s[c] = 1;
        }), e = s), t) {
          s = {};
          for (v in e)
...
}, rQ = gt.registerPlugin({
  name: "attr",
  init: function(Q, t, B, A, e) {
    var P, r, s;
    this.tween = B;
    for (P in t)
      s = Q.getAttribute(P) || "", r = this.add(Q, "setAttribute", (s || 0) + "", t[P], A, e, 0, 0, P), r.op = P, r.b = s, this._props.push(P);
  },
  render: function(Q, t) {
    for (var B = t._pt; B; )
      q ? B.set(B.t, B.p, B.b, B) : B.r(Q, B.d), B = B._next;
  }
}, {
  name: "endArray",
  headless: 1,
  init: function(Q, t) {
    for (var B = t.length; B--; )
      this.add(Q, B, Q[B] || 0, t[B], 0, 0, 0, 0, 0, 1);
  }
}, QB("roundProps", hB), QB("modifiers"), QB("snap", qA)) || gt;
K.version = BQ.version = rQ.version = "3.15.0";
pA = 1;
LB() && Pt();
p.Power0;
p.Power1;
 ...
 * @author: Jack Doyle, jack@greensock.com
*/
var oA, jQ, Qt, yB, YQ, TA, IB, vi = function() {
  return typeof window < "u";
}, OQ = {}, IQ = 180 / Math.PI, tt = Math.PI / 180, KQ = Math.atan2, zA = 1e8, YB = /([A-Z])/g, ci = /(left|right|width|margin|padding|x)/i, ni = /[\s,\(]\S/, UQ = {
  autoAlpha: "opacity,visibility",
  scale: "scaleX,scaleY",
  alpha: "opacity"
}, uB = function(Q, t) {
  return t.set(t.t, t.p, Math.round((t.s + t.c * Q) * 1e4) / 1e4 + t.u, t);
}, oi = function(Q, t) {
  return t.set(t.t, t.p, Q === 1 ? t.e : Math.round((t.s + t.c * Q) * 1e4) / 1e4 + t.u, t);
}, Ti = function(Q, t) {
  return t.set(t.t, t.p, Q ? Math.round((t.s + t.c * Q) * 1e4) / 1e4 + t.u : t.b, t);
}, zi = function(Q, t) {
  return t.set(t.t, t.p, Q === 1 ? t.e : Q ? Math.round((t.s + t.c * Q) * 1e4) / 1e4 + t.u : t.b, t);
}, ai = function(Q, t) {
  var B = t.s + t.c * Q;
  t.set(t.t, t.p, ~~(B + (B < 0 ? -0.5 : 0.5)) + t.u, t);
}, we = function(Q, t) {
  return t.set(t.t, t.p, Q ? t.e : t.b, t);
}, Ee = function(Q, t) {
  return t.set(t.t, t.p, Q !== 1 ? t.b : t.e, t);
}, wi = function(Q, t, B) {
  return Q.style[t] = B;
}, Ei = function(Q, t, B) {
  return Q.style.setProperty(t, B);
}, hi = function(Q, t, B) {
  return Q._gsap[t] = B;
}, li = function(Q, t, B) {
  return Q._gsap.scaleX = Q._gsap.scaleY = B;
}, ki = function(Q, t, B, A, e) {
  var P = Q._gsap;
  P.scaleX = P.scaleY = B, P.renderTransform(e, P);
}, ui = function(Q, t, B, A, e) {
  var P = Q._gsap;
  P[t] = B, P.renderTransform(e, P);
}, V = "transform", iQ = V + "Origin", Ui = function i(Q, t) {
  var B = this, A = this.target, e = A.style, P = A._gsap;
  if (Q in OQ && e) {
    if (this.tfm = this.tfm || {}, Q !== "transform")
      Q = UQ[Q] || Q, ~Q.indexOf(",") ? Q.split(",").forEach(function(r) {
        return B.tfm[r] = CQ(A, r);
      }) : this.tfm[Q] = P.x ? P[Q] : CQ(A, Q), Q === iQ && (this.tfm.zOrigin = P.zOrigin);
    else
      return UQ.transform.split(",").forEach(function(r) {
        return i.call(B, r, t);
      });
    if (this.props.indexOf(V) >= 0)
      return;
    P.svg && (this.svgo = A.getAttribute("data-svg-origin"), this.props.push(iQ, t, "")), Q = V;
  }
  (e || t) && this.props.push(Q, t, e[Q]);
}, he = function(Q) {
  Q.translate && (Q.removeProperty("translate"), Q.removeProperty("scale"), Q.removeProperty("rotate"));
}, Di = function() {
  var Q = this.props, t = this.target, B = t.style, A = t._gsap, e, P;
  for (e = 0; e < Q.length; e += 3)
    Q[e + 1] ? Q[e + 1] === 2 ? t[Q[e]](Q[e + 2]) : t[Q[e]] = Q[e + 2] : Q[e + 2] ? B[Q[e]] = Q[e + 2] : B.removeProperty(Q[e].substr(0, 2) === "--" ? Q[e] : Q[e].replace(YB, "-$1").toLowerCase());
  if (this.tfm) {
    for (P in this.tfm)
      A[P] = this.tfm[P];
    A.svg && (A.renderTransform(), t.setAttribute("data-svg-origin", this.svgo || "")), e = IB(), (!e || !e.isStart) && !B[V] && (he(B), A.zOrigin && B[iQ] && (B[iQ] += " " + A.zOrigin + "px", A.zOrigin = 0, A.renderTransform()), A.uncache = 1);
  }
}, le = function(Q, t) {
  var B = {
    target: Q,
  ...
  return Q._gsap || rQ.core.getCache(Q), t && Q.style && Q.nodeType && t.split(",").forEach(function(A) {
    return B.save(A);
  }), B;
}, ke, UB = function(Q, t) {
  var B = jQ.createElementNS ? jQ.createElementNS((t || "http://www.w3.org/1999/xhtml").replace(/^https/, "http"), Q) : jQ.createElement(Q);
  return B && B.style ? B : jQ.createElement(Q);
}, TQ = function i(Q, t, B) {
  var A = getComputedStyle(Q);
  return A[t] || A.getPropertyValue(t.replace(YB, "-$1").toLowerCase()) || A.getPropertyValue(t) || !B && i(Q, it(t) || t, 1) || "";
}, aA = "O,Moz,ms,Ms,Webkit".split(","), it = function(Q, t, B) {
  var A = t || YQ, e = A.style, P = 5;
  if (Q in e && !B)
    return Q;
  for (Q = Q.charAt(0).toUpperCase() + Q.substr(1); P-- && !(aA[P] + Q in e); )
    ;
  return P < 0 ? null : (P === 3 ? "ms" : P >= 0 ? aA[P] : "") + Q;
}, DB = function() {
  vi() && window.document && (oA = window, jQ = oA.document, Qt = jQ.documentElement, YQ = UB("div") || {
    style: {}
  }, UB("div"), V = it(V), iQ = V + "Origin", YQ.style.cssText = "border-width:0;line-height:0;position:absolute;padding:0", ke = !!it("perspective"), IB = rQ.core.reverting, yB = 1);
}, wA = function(Q) {
  var t = Q.ownerSVGElement, B = UB("svg", t && t.getAttribute("xmlns") || "http://www.w3.org/2000/svg"), A = Q.cloneNode(!0), e;
  A.style.display = "block", B.appendChild(A), Qt.appendChild(B);
  try {
    e = A.getBBox();
  } catch {
  }
  return B.removeChild(A), Qt.removeChild(B), e;
}, EA = function(Q, t) {
  for (var B = t.length; B--; )
    if (Q.hasAttribute(t[B]))
      return Q.getAttribute(t[B]);
}, ue = function(Q) {
  var t, B;
  try {
    t = Q.getBBox();
  ...
  return t && (t.width || t.height) || B || (t = wA(Q)), t && !t.width && !t.x && !t.y ? {
    x: +EA(Q, ["x", "cx", "x1"]) || 0,
    y: +EA(Q, ["y", "cy", "y1"]) || 0,
    width: 0,
    height: 0
  ...
  return !!(Q.getCTM && (!Q.parentNode || Q.ownerSVGElement) && ue(Q));
}, pQ = function(Q, t) {
  if (t) {
    var B = Q.style, A;
    t in OQ && t !== iQ && (t = V), B.removeProperty ? (A = t.substr(0, 2), (A === "ms" || t.substr(0, 6) === "webkit") && (t = "-" + t), B.removeProperty(A === "--" ? t : t.replace(YB, "-$1").toLowerCase())) : B.removeAttribute(t);
  }
}, FQ = function(Q, t, B, A, e, P) {
  var r = new PQ(Q._pt, t, B, 0, 1, P ? Ee : we);
  return Q._pt = r, r.b = A, r.e = e, Q._props.push(B), r;
}, hA = {
  deg: 1,
  rad: 1,
  turn: 1
  ...
  grid: 1,
  flex: 1
}, gQ = function i(Q, t, B, A) {
  var e = parseFloat(B) || 0, P = (B + "").trim().substr((e + "").length) || "px", r = YQ.style, s = ci.test(t), v = Q.tagName.toLowerCase() === "svg", c = (v ? "client" : "offset") + (s ? "Width" : "Height"), n = 100, T = A === "px", z = A === "%", a, o, w, E;
  if (A === P || !e || hA[A] || hA[P])
    return e;
  if (P !== "px" && !T && (e = i(Q, t, B, "px")), E = Q.getCTM && Ue(Q), (z || P === "%") && (OQ[t] || ~t.indexOf("adius")))
    return a = E ? Q.getBBox()[s ? "width" : "height"] : Q[c], X(z ? e / a * n : e / 100 * a);
  if (r[s ? "width" : "height"] = n + (T ? P : A), o = A !== "rem" && ~t.indexOf("adius") || A === "em" && Q.appendChild && !v ? Q : Q.parentNode, E && (o = (Q.ownerSVGElement || {}).parentNode), (!o || o === jQ || !o.appendChild) && (o = jQ.body), w = o._gsap, w && z && w.width && s && w.time === nQ.time && !w.uncache)
    return X(e / w.width * n);
  if (z && (t === "height" || t === "width")) {
    var h = Q.style[t];
    Q.style[t] = n + A, a = Q[c], h ? Q.style[t] = h : pQ(Q, t);
  } else
    (z || P === "%") && !di[TQ(o, "display")] && (r.position = TQ(Q, "position")), o === Q && (r.position = "static"), o.appendChild(YQ), a = YQ[c], o.removeChild(YQ), r.position = "absolute";
  return s && z && (w = SQ(o), w.time = nQ.time, w.width = o[c]), X(T ? a * e / n : a && e ? n / a * e : 0);
}, CQ = function(Q, t, B, A) {
  var e;
  return yB || DB(), t in UQ && t !== "transform" && (t = UQ[t], ~t.indexOf(",") && (t = t.split(",")[0])), OQ[t] && t !== "transform" ? (e = Dt(Q, A), e = t !== "transformOrigin" ? e[t] : e.svg ? e.origin : xt(TQ(Q, iQ)) + " " + e.zOrigin + "px") : (e = Q.style[t], (!e || e === "auto" || A || ~(e + "").indexOf("calc(")) && (e = mt[t] && mt[t](Q, t, B) || TQ(Q, t) || yA(Q, t) || (t === "opacity" ? 1 : 0))), B && !~(e + "").trim().indexOf(" ") ? gQ(Q, t, e, B) + B : e;
}, Ci = function(Q, t, B, A) {
  if (!B || B === "none") {
    var e = it(t, Q, 1), P = e && TQ(Q, e, 1);
    P && P !== B ? (t = e, B = P) : t === "borderColor" && (B = TQ(Q, "borderTopColor"));
  }
  var r = new PQ(this._pt, Q.style, t, 0, 1, Te), s = 0, v = 0, c, n, T, z, a, o, w, E, h, u, U, k;
  if (r.b = B, r.e = A, B += "", A += "", A.substring(0, 6) === "var(--" && (A = TQ(Q, A.substring(4, A.indexOf(")")))), A === "auto" && (o = Q.style[t], Q.style[t] = A, A = TQ(Q, t) || A, o ? Q.style[t] = o : pQ(Q, t)), c = [B, A], Pe(c), B = c[0], A = c[1], T = B.match(qQ) || [], k = A.match(qQ) || [], k.length) {
    for (; n = qQ.exec(A); )
      w = n[0], h = A.substring(s, n.index), a ? a = (a + 1) % 5 : (h.substr(-5) === "rgba(" || h.substr(-5) === "hsla(") && (a = 1), w !== (o = T[v++] || "") && (z = parseFloat(o) || 0, U = o.substr((z + "").length), w.charAt(1) === "=" && (w = $Q(z, w) + U), E = parseFloat(w), u = w.substr((E + "").length), s = qQ.lastIndex - u.length, u || (u = u || zQ.units[t] || U, s === A.length && (A += u, r.e += u)), U !== u && (z = gQ(Q, t, o, u) || 0), r._pt = {
        _next: r._pt,
        p: h || v === 1 ? h : ",",
        //note: SVG spec allows omission of comma/space when a negative sign is wedged between two numbers, like 2.5-5.3 instead of 2.5,-5.3 but when tweening, the negative value may switch to positive, so we insert the comma just in case.
        s: z,
        c: E - z,
        m: a && a < 4 || t === "zIndex" ? Math.round : 0
      });
    r.c = s < A.length ? A.substring(s, A.length) : "";
  } else
    r.r = t === "display" && A === "none" ? Ee : we;
  return bA.test(A) && (r.e = 0), this._pt = r, r;
}, lA = {
  top: "0%",
  bottom: "100%",
  left: "0%",
  ...
  var t = Q.split(" "), B = t[0], A = t[1] || "50%";
  return (B === "top" || B === "bottom" || A === "left" || A === "right") && (Q = B, B = A, A = Q), t[0] = lA[B] || B, t[1] = lA[A] || A, t.join(" ");
}, Oi = function(Q, t) {
  if (t.tween && t.tween._time === t.tween._dur) {
    var B = t.t, A = B.style, e = t.u, P = B._gsap, r, s, v;
    if (e === "all" || e === !0)
      A.cssText = "", s = 1;
    else
      for (e = e.split(","), v = e.length; --v > -1; )
        r = e[v], OQ[r] && (s = 1, r = r === "transformOrigin" ? iQ : V), pQ(B, r);
    s && (pQ(B, V), P && (P.svg && B.removeAttribute("transform"), A.scale = A.rotate = A.translate = "none", Dt(B, 1), P.uncache = 1, he(A)));
  }
}, mt = {
  clearProps: function(Q, t, B, A, e) {
    if (e.data !== "isFromStart") {
      var P = Q._pt = new PQ(Q._pt, t, B, 0, 0, Oi);
      return P.u = A, P.pr = -10, P.tween = e, Q._props.push(B), 1;
    }
  }
  ...
  */
}, Ut = [1, 0, 0, 1, 0, 0], De = {}, de = function(Q) {
  return Q === "matrix(1, 0, 0, 1, 0, 0)" || Q === "none" || !Q;
}, kA = function(Q) {
  var t = TQ(Q, V);
  return de(t) ? Ut : t.substr(7).match(GA).map(X);
}, SB = function(Q, t) {
  var B = Q._gsap || SQ(Q), A = Q.style, e = kA(Q), P, r, s, v;
  return B.svg && Q.getAttribute("transform") ? (s = Q.transform.baseVal.consolidate().matrix, e = [s.a, s.b, s.c, s.d, s.e, s.f], e.join(",") === "1,0,0,1,0,0" ? Ut : e) : (e === Ut && !Q.offsetParent && Q !== Qt && !B.svg && (s = A.display, A.display = "block", P = Q.parentNode, (!P || !Q.offsetParent && !Q.getBoundingClientRect().width) && (v = 1, r = Q.nextElementSibling, Qt.appendChild(Q)), e = kA(Q), s ? A.display = s : pQ(Q, "display"), v && (r ? P.insertBefore(Q, r) : P ? P.appendChild(Q) : Qt.removeChild(Q))), t && e.length > 6 ? [e[0], e[1], e[4], e[5], e[12], e[13]] : e);
}, dB = function(Q, t, B, A, e, P) {
  var r = Q._gsap, s = e || SB(Q, !0), v = r.xOrigin || 0, c = r.yOrigin || 0, n = r.xOffset || 0, T = r.yOffset || 0, z = s[0], a = s[1], o = s[2], w = s[3], E = s[4], h = s[5], u = t.split(" "), U = parseFloat(u[0]) || 0, k = parseFloat(u[1]) || 0, O, D, N, d;
  B ? s !== Ut && (D = z * w - a * o) && (N = U * (w / D) + k * (-o / D) + (o * h - w * E) / D, d = U * (-a / D) + k * (z / D) - (z * h - a * E) / D, U = N, k = d) : (O = ue(Q), U = O.x + (~u[0].indexOf("%") ? U / 100 * O.width : U), k = O.y + (~(u[1] || u[0]).indexOf("%") ? k / 100 * O.height : k)), A || A !== !1 && r.smooth ? (E = U - v, h = k - c, r.xOffset = n + (E * z + h * o) - E, r.yOffset = T + (E * a + h * w) - h) : r.xOffset = r.yOffset = 0, r.xOrigin = U, r.yOrigin = k, r.smooth = !!A, r.origin = t, r.originIsAbsolute = !!B, Q.style[iQ] = "0px 0px", P && (FQ(P, r, "xOrigin", v, U), FQ(P, r, "yOrigin", c, k), FQ(P, r, "xOffset", n, r.xOffset), FQ(P, r, "yOffset", T, r.yOffset)), Q.setAttribute("data-svg-origin", U + " " + k);
}, Dt = function(Q, t) {
  var B = Q._gsap || new re(Q);
  if ("x" in B && !t && !B.uncache)
    return B;
  var A = Q.style, e = B.scaleX < 0, P = "px", r = "deg", s = getComputedStyle(Q), v = TQ(Q, iQ) || "0", c, n, T, z, a, o, w, E, h, u, U, k, O, D, N, d, M, b, G, C, j, F, H, f, I, R, QQ, sQ, W, x, tQ, vQ;
  return c = n = T = o = w = E = h = u = U = 0, z = a = 1, B.svg = !!(Q.getCTM && Ue(Q)), s.translate && ((s.translate !== "none" || s.scale !== "none" || s.rotate !== "none") && (A[V] = (s.translate !== "none" ? "translate3d(" + (s.translate + " 0 0").split(" ").slice(0, 3).join(", ") + ") " : "") + (s.rotate !== "none" ? "rotate(" + s.rotate + ") " : "") + (s.scale !== "none" ? "scale(" + s.scale.split(" ").join(",") + ") " : "") + (s[V] !== "none" ? s[V] : "")), A.scale = A.rotate = A.translate = "none"), D = SB(Q, B.svg), B.svg && (B.uncache ? (I = Q.getBBox(), v = B.xOrigin - I.x + "px " + (B.yOrigin - I.y) + "px", f = "") : f = !t && Q.getAttribute("data-svg-origin"), dB(Q, f || v, !!f || B.originIsAbsolute, B.smooth !== !1, D)), k = B.xOrigin || 0, O = B.yOrigin || 0, D !== Ut && (b = D[0], G = D[1], C = D[2], j = D[3], c = F = D[4], n = H = D[5], D.length === 6 ? (z = Math.sqrt(b * b + G * G), a = Math.sqrt(j * j + C * C), o = b || G ? KQ(G, b) * IQ : 0, h = C || j ? KQ(C, j) * IQ + o : 0, h && (a *= Math.abs(Math.cos(h * tt))), B.svg && (c -= k - (k * b + O * C), n -= O - (k * G + O * j))) : (vQ = D[6], x = D[7], QQ = D[8], sQ = D[9], W = D[10], tQ = D[11], c = D[12], n = D[13], T = D[14], N = KQ(vQ, W), w = N * IQ, N && (d = Math.cos(-N), M = Math.sin(-N), f = F * d + QQ * M, I = H * d + sQ * M, R = vQ * d + W * M, QQ = F * -M + QQ * d, sQ = H * -M + sQ * d, W = vQ * -M + W * d, tQ = x * -M + tQ * d, F = f, H = I, vQ = R), N = KQ(-C, W), E = N * IQ, N && (d = Math.cos(-N), M = Math.sin(-N), f = b * d - QQ * M, I = G * d - sQ * M, R = C * d - W * M, tQ = j * M + tQ * d, b = f, G = I, C = R), N = KQ(G, b), o = N * IQ, N && (d = Math.cos(N), M = Math.sin(N), f = b * d + G * M, I = F * d + H * M, G = G * d - b * M, H = H * d - F * M, b = f, F = I), w && Math.abs(w) + Math.abs(o) > 359.9 && (w = o = 0, E = 180 - E), z = X(Math.sqrt(b * b + G * G + C * C)), a = X(Math.sqrt(H * H + vQ * vQ)), N = KQ(F, H), h = Math.abs(N) > 2e-4 ? N * IQ : 0, U = tQ ? 1 / (tQ < 0 ? -tQ : tQ) : 0), B.svg && (f = Q.getAttribute("transform"), B.forceCSS = Q.setAttribute("transform", "") || !de(TQ(Q, V)), f && Q.setAttribute("transform", f))), Math.abs(h) > 90 && Math.abs(h) < 270 && (e ? (z *= -1, h += o <= 0 ? 180 : -180, o += o <= 0 ? 180 : -180) : (a *= -1, h += h <= 0 ? 180 : -180)), t = t || B.uncache, B.x = c - ((B.xPercent = c && (!t && B.xPercent || (Math.round(Q.offsetWidth / 2) === Math.round(-c) ? -50 : 0))) ? Q.offsetWidth * B.xPercent / 100 : 0) + P, B.y = n - ((B.yPercent = n && (!t && B.yPercent || (Math.round(Q.offsetHeight / 2) === Math.round(-n) ? -50 : 0))) ? Q.offsetHeight * B.yPercent / 100 : 0) + P, B.z = T + P, B.scaleX = X(z), B.scaleY = X(a), B.rotation = X(o) + r, B.rotationX = X(w) + r, B.rotationY = X(E) + r, B.skewX = h + r, B.skewY = u + r, B.transformPerspective = U + P, (B.zOrigin = parseFloat(v.split(" ")[2]) || !t && B.zOrigin || 0) && (A[iQ] = xt(v)), B.xOffset = B.yOffset = 0, B.force3D = zQ.force3D, B.renderTransform = B.svg ? Mi : ke ? Ce : Li, B.uncache = 0, B;
}, xt = function(Q) {
  return (Q = Q.split(" "))[0] + " " + Q[1];
}, tB = function(Q, t, B) {
  var A = _(t);
  return X(parseFloat(t) + parseFloat(gQ(Q, "x", B + "px", A))) + A;
}, Li = function(Q, t) {
  t.z = "0px", t.rotationY = t.rotationX = "0deg", t.force3D = 0, Ce(Q, t);
}, xQ = "0deg", st = "0px", yQ = ") ", Ce = function(Q, t) {
  var B = t || this, A = B.xPercent, e = B.yPercent, P = B.x, r = B.y, s = B.z, v = B.rotation, c = B.rotationY, n = B.rotationX, T = B.skewX, z = B.skewY, a = B.scaleX, o = B.scaleY, w = B.transformPerspective, E = B.force3D, h = B.target, u = B.zOrigin, U = "", k = E === "auto" && Q && Q !== 1 || E === !0;
  if (u && (n !== xQ || c !== xQ)) {
    var O = parseFloat(c) * tt, D = Math.sin(O), N = Math.cos(O), d;
    O = parseFloat(n) * tt, d = Math.cos(O), P = tB(h, P, D * d * -u), r = tB(h, r, -Math.sin(O) * -u), s = tB(h, s, N * d * -u + u);
  }
  w !== st && (U += "perspective(" + w + yQ), (A || e) && (U += "translate(" + A + "%, " + e + "%) "), (k || P !== st || r !== st || s !== st) && (U += s !== st || k ? "translate3d(" + P + ", " + r + ", " + s + ") " : "translate(" + P + ", " + r + yQ), v !== xQ && (U += "rotate(" + v + yQ), c !== xQ && (U += "rotateY(" + c + yQ), n !== xQ && (U += "rotateX(" + n + yQ), (T !== xQ || z !== xQ) && (U += "skew(" + T + ", " + z + yQ), (a !== 1 || o !== 1) && (U += "scale(" + a + ", " + o + yQ), h.style[V] = U || "translate(0, 0)";
}, Mi = function(Q, t) {
  var B = t || this, A = B.xPercent, e = B.yPercent, P = B.x, r = B.y, s = B.rotation, v = B.skewX, c = B.skewY, n = B.scaleX, T = B.scaleY, z = B.target, a = B.xOrigin, o = B.yOrigin, w = B.xOffset, E = B.yOffset, h = B.forceCSS, u = parseFloat(P), U = parseFloat(r), k, O, D, N, d;
  s = parseFloat(s), v = parseFloat(v), c = parseFloat(c), c && (c = parseFloat(c), v += c, s += c), s || v ? (s *= tt, v *= tt, k = Math.cos(s) * n, O = Math.sin(s) * n, D = Math.sin(s - v) * -T, N = Math.cos(s - v) * T, v && (c *= tt, d = Math.tan(v - c), d = Math.sqrt(1 + d * d), D *= d, N *= d, c && (d = Math.tan(c), d = Math.sqrt(1 + d * d), k *= d, O *= d)), k = X(k), O = X(O), D = X(D), N = X(N)) : (k = n, N = T, O = D = 0), (u && !~(P + "").indexOf("px") || U && !~(r + "").indexOf("px")) && (u = gQ(z, "x", P, "px"), U = gQ(z, "y", r, "px")), (a || o || w || E) && (u = X(u + a - (a * k + o * D) + w), U = X(U + o - (a * O + o * N) + E)), (A || e) && (d = z.getBBox(), u = X(u + A / 100 * d.width), U = X(U + e / 100 * d.height)), d = "matrix(" + k + "," + O + "," + D + "," + N + "," + u + "," + U + ")", z.setAttribute("transform", d), h && (z.style[V] = d);
}, Hi = function(Q, t, B, A, e) {
  var P = 360, r = Z(e), s = parseFloat(e) * (r && ~e.indexOf("rad") ? IQ : 1), v = s - A, c = A + v + "deg", n, T;
  return r && (n = e.split("_")[1], n === "short" && (v %= P, v !== v % (P / 2) && (v += v < 0 ? P : -P)), n === "cw" && v < 0 ? v = (v + P * zA) % P - ~~(v / P) * P : n === "ccw" && v > 0 && (v = (v - P * zA) % P - ~~(v / P) * P)), Q._pt = T = new PQ(Q._pt, t, B, A, v, oi), T.e = c, T.u = "deg", Q._props.push(B), T;
}, uA = function(Q, t) {
  for (var B in t)
    Q[B] = t[B];
  return Q;
}, ji = function(Q, t, B) {
  var A = uA({}, B._gsap), e = "perspective,force3D,transformOrigin,svgOrigin", P = B.style, r, s, v, c, n, T, z, a;
  A.svg ? (v = B.getAttribute("transform"), B.setAttribute("transform", ""), P[V] = t, r = Dt(B, 1), pQ(B, V), B.setAttribute("transform", v)) : (v = getComputedStyle(B)[V], P[V] = t, r = Dt(B, 1), P[V] = v);
  for (s in OQ)
    v = A[s], c = r[s], v !== c && e.indexOf(s) < 0 && (z = _(v), a = _(c), n = z !== a ? gQ(B, s, v, a) : parseFloat(v), T = parseFloat(c), Q._pt = new PQ(Q._pt, r, s, n, T - n, uB), Q._pt.u = a || 0, Q._props.push(s));
  uA(r, A);
};
eQ("padding,margin,Width,Radius", function(i, Q) {
  var t = "Top", B = "Right", A = "Bottom", e = "Left", P = (Q < 3 ? [t, B, A, e] : [t + e, t + B, A + B, A + e]).map(function(r) {
    return Q < 2 ? i + r : "border" + r + i;
  });
  mt[Q > 1 ? "border" + i : i] = function(r, s, v, c, n) {
    var T, z;
    if (arguments.length < 4)
      return T = P.map(function(a) {
        return CQ(r, a, v);
      }), z = T.join(" "), z.split(T[0]).length === 5 ? T[0] : z;
    T = (c + "").split(" "), z = {}, P.forEach(function(a, o) {
...
var Ne = {
  ...
  init: function(Q, t, B, A, e) {
    var P = this._props, r = Q.style, s = B.vars.startAt, v, c, n, T, z, a, o, w, E, h, u, U, k, O, D, N, d;
    yB || DB(), this.styles = this.styles || le(Q), N = this.styles.props, this.tween = B;
    for (o in t)
      if (o !== "autoRound" && (c = t[o], !(cQ[o] && se(o, t, B, A, Q, e)))) {
        if (z = typeof c, a = mt[o], z === "function" && (c = c.call(B, A, Q, e), z = typeof c), z === "string" && ~c.indexOf("random(") && (c = lt(c)), a)
          a(this, Q, o, c, B) && (D = 1);
        else if (o.substr(0, 2) === "--")
          v = (getComputedStyle(Q).getPropertyValue(o) + "").trim(), c += "", GQ.lastIndex = 0, GQ.test(v) || (w = _(v), E = _(c), E ? w !== E && (v = gQ(Q, o, v, E) + E) : w && (c += w)), this.add(r, "setProperty", v, c, A, e, 0, 0, o), P.push(o), N.push(o, 0, r[o]);
        else if (z !== "undefined") {
          if (s && o in s ? (v = typeof s[o] == "function" ? s[o].call(B, A, Q, e) : s[o], Z(v) && ~v.indexOf("random(") && (v = lt(v)), _(v + "") || v === "auto" || (v += zQ.units[o] || _(CQ(Q, o)) || ""), (v + "").charAt(1) === "=" && (v = CQ(Q, o))) : v = CQ(Q, o), T = parseFloat(v), h = z === "string" && c.charAt(1) === "=" && c.substr(0, 2), h && (c = c.substr(2)), n = parseFloat(c), o in UQ && (o === "autoAlpha" && (T === 1 && CQ(Q, "visibility") === "hidden" && n && (T = 0), N.push("visibility", 0, r.visibility), FQ(this, r, "visibility", T ? "inherit" : "hidden", n ? "inherit" : "hidden", !n)), o !== "scale" && o !== "transform" && (o = UQ[o], ~o.indexOf(",") && (o = o.split(",")[0]))), u = o in OQ, u) {
            if (this.styles.save(o), d = c, z === "string" && c.substring(0, 6) === "var(--") {
              if (c = TQ(Q, c.substring(4, c.indexOf(")"))), c.substring(0, 5) === "calc(") {
                var M = Q.style.perspective;
                Q.style.perspective = c, c = TQ(Q, "perspective"), M ? Q.style.perspective = M : pQ(Q, "perspective");
              }
              n = parseFloat(c);
            }
            if (U || (k = Q._gsap, k.renderTransform && !t.parseTransform || Dt(Q, t.parseTransform), O = t.smoothOrigin !== !1 && k.smooth, U = this._pt = new PQ(this._pt, r, V, 0, 1, k.renderTransform, k, 0, -1), U.dep = 1), o === "scale")
              this._pt = new PQ(this._pt, k, "scaleY", k.scaleY, (h ? $Q(k.scaleY, h + n) : n) - k.scaleY || 0, uB), this._pt.u = 0, P.push("scaleY", o), o += "X";
            else if (o === "transformOrigin") {
              N.push(iQ, 0, r[iQ]), c = Ni(c), k.svg ? dB(Q, c, 0, O, 0, this) : (E = parseFloat(c.split(" ")[2]) || 0, E !== k.zOrigin && FQ(this, k, "zOrigin", k.zOrigin, E), FQ(this, r, o, xt(v), xt(c)));
              continue;
            } else if (o === "svgOrigin") {
              dB(Q, c, 1, O, 0, this);
              continue;
            } else if (o in De) {
              Hi(this, k, o, T, h ? $Q(T, h + c) : c);
              continue;
            } else if (o === "smoothOrigin") {
              FQ(this, k, "smooth", k.smooth, c);
              continue;
            } else if (o === "force3D") {
              k[o] = c;
              continue;
          ...
          } else o in r || (o = it(o) || o);
          if (u || (n || n === 0) && (T || T === 0) && !ni.test(c) && o in r)
            w = (v + "").substr((T + "").length), n || (n = 0), E = _(c) || (o in zQ.units ? zQ.units[o] : w), w !== E && (T = gQ(Q, o, v, E)), this._pt = new PQ(this._pt, u ? k : r, o, T, (h ? $Q(T, h + n) : n) - T, !u && (E === "px" || o === "zIndex") && t.autoRound !== !1 ? ai : uB), this._pt.u = E || 0, u && d !== c ? (this._pt.b = v, this._pt.e = d, this._pt.r = zi) : w !== E && E !== "%" && (this._pt.b = v, this._pt.r = Ti);
          else if (o in r)
            Ci.call(this, Q, o, v, h ? h + c : c);
          else if (o in Q)
            this.add(Q, o, v || Q[o], h ? h + c : c, A, e);
          else if (o !== "parseTransform") {
            HB(o, c);
            continue;
          }
          u || (o in r ? N.push(o, 0, r[o]) : typeof Q[o] == "function" ? N.push(o, 2, Q[o]()) : N.push(o, 1, v || Q[o])), P.push(o);
        }
      }
    D && ze(this);
  },
  ...
  get: CQ,
  aliases: UQ,
  getSetter: function(Q, t, B) {
    var A = UQ[t];
    return A && A.indexOf(",") < 0 && (t = A), t in OQ && t !== iQ && (Q._gsap.x || CQ(Q, "x")) ? B && TA === B ? t === "scale" ? li : hi : (TA = B || {}) && (t === "scale" ? ki : ui) : Q.style && !OB(Q.style[t]) ? wi : ~t.indexOf("-") ? Ei : mB(Q, t);
  },
  core: {
    _removeProperty: pQ,
    _getMatrix: SB
...
function pi(i) {
  const Q = Math.max(1e-3, i), t = Q * 1.6, B = Q * 0.7, A = new l.ConeGeometry(B, t, 3, 1, !1);
  return A.rotateX(-Math.PI / 2), A.translate(0, 0, t / 2), A;
}
function gi(i) {
  const t = Math.max(1e-3, i) * 0.65, B = new l.SphereGeometry(t, 18, 12);
  return B.translate(0, 0, t), B;
}
function mi(i, Q) {
  i.computeBoundingBox();
  const t = i.boundingBox, B = Math.max(1e-6, t.max.z - t.min.z), A = i.getAttribute("position"), e = new Float32Array(A.count * 2);
  for (let P = 0; P < A.count; P++) {
    const r = A.getX(P), s = A.getY(P), v = A.getZ(P);
    e[P * 2] = Math.atan2(s, r) / (2 * Math.PI) + 0.5, e[P * 2 + 1] = (v - t.min.z) / B * Q;
  }
  i.setAttribute("uv", new l.BufferAttribute(e, 2));
}
function Oe(i) {
  const Q = atob(Fi), t = new Uint8Array(Q.length);
  for (let v = 0; v < Q.length; v++) t[v] = Q.charCodeAt(v);
  const A = new fe().parse(t.buffer);
  A.rotateX(-Math.PI / 2), A.computeBoundingBox();
  const e = A.boundingBox, P = new l.Vector3();
  e.getSize(P);
  const r = Math.max(1e-3, i) * 1.6 / Math.max(P.x, P.y, P.z, 1e-3);
  ...
  return A.translate(
    ...
    -(s.min.y + s.max.y) / 2,
    -s.min.z
  ), mi(A, 6), A.computeVertexNormals(), A;
}
let BB = null;
function xi() {
  if (BB) return BB;
  const i = 64, Q = 256, t = document.createElement("canvas");
  t.width = i, t.height = Q;
  const B = t.getContext("2d");
  B.fillStyle = "#808080", B.fillRect(0, 0, i, Q);
  const A = 10;
  for (let r = 0; r < A; r++) {
    const s = r / A * i, v = r % 2 === 0 ? 150 : 90;
    B.fillStyle = `rgb(${v},${v},${v})`, B.fillRect(s, 0, i / A / 2, Q);
  }
  const e = B.getImageData(0, 0, i, Q);
  ...
  const P = new l.CanvasTexture(t);
  return P.wrapS = l.RepeatWrapping, P.wrapT = l.RepeatWrapping, BB = P, P;
}
function yi(i, Q) {
  const t = Math.max(1e-3, i), B = t * 1.6, A = t * 0.08, e = t * 0.187, P = Ct(Q), r = new l.CylinderGeometry(0, A, B, 8, 1, !1);
...
function Ii(i) {
  const Q = Math.max(1e-3, i.bit.size);
  return i.bit.type === "triangle" ? pi(Q) : i.bit.type === "drill" ? Oe(Q) : gi(Q);
}
function VB(i) {
  return i.bit.colorSource === "custom" ? i.bit.color : i.bit.type === "drill" ? Gi : fi;
}
function Yi(i) {
  const Q = Ct(i.bit.opacity), t = Oe(Math.max(1e-3, i.bit.size)), B = new l.Color(VB(i)), A = xi(), e = new l.MeshStandardMaterial({
    color: B,
    metalness: 0.8,
    roughness: 0.35,
    roughnessMap: A,
  ...
  return P.renderOrder = 1e3, P;
}
function Si(i) {
  const Q = Ct(i.bit.opacity), t = Ii(i), B = new l.MeshBasicMaterial({
    color: new l.Color(VB(i)),
    transparent: Q < 1,
    opacity: Q,
    side: l.DoubleSide,
    depthTest: !1,
...
function dA(i) {
  return i.bit.type === "laser" ? yi(i.bit.size, i.bit.opacity) : i.bit.type === "drill" ? Yi(i) : Si(i);
}
function CA(i) {
  if (i instanceof l.Mesh)
    i.geometry.dispose(), i.material.dispose();
  else if (i instanceof l.Group)
    for (const Q of i.children)
      Q instanceof l.Mesh && (Q.geometry.dispose(), Q.material.dispose());
}
function Vi(i) {
  const Q = new l.Group();
  Q.name = "gviewer:bit-marker", Q.visible = !!i.bit.enabled;
  let t = i, B = dA(t);
  Q.add(B);
  let A = null, e = !1, P = null, r = null;
  const s = (n) => {
    const T = Math.max(1, n);
    return Ft.to(B.rotation, {
      z: `+=${Math.PI * 2}`,
      duration: 60 / T,
      repeat: -1,
      ease: "none",
  ...
  P = s(t.bit.spinRpm);
  const v = (n) => {
    const T = n.bit.type, z = t.bit.type, a = n.bit.size, o = t.bit.size, w = T !== z, E = a !== o, h = Ct(n.bit.opacity);
    if (w || E)
      Q.remove(B), CA(B), B = dA(n), Q.add(B), r == null || r.kill(), P == null || P.kill(), P = s(n.bit.spinRpm), e && P.timeScale(1).play();
    else if (T === "laser") {
      const U = B, [k, O] = U.children, D = k.material, N = O.material;
      D.opacity = h, D.needsUpdate = !0, N.opacity = h * 0.3, N.needsUpdate = !0;
    } else {
      const k = B.material, O = new l.Color(VB(n));
      k.color = O, T === "drill" && (k.emissive = O), k.opacity = h, k.transparent = h < 1, k.needsUpdate = !0;
    }
    t = n, Q.visible = !!n.bit.enabled;
  };
  return {
    object: Q,
    setVisible: (n) => {
    ...
    setTarget: (n, T) => {
      ...
      if (n.a, z || t.bit.tweenMs <= 0) {
        A = null, Q.position.set(a, o, w);
        return;
      }
      A = {
        startedAt: performance.now(),
        duration: Math.max(1, t.bit.tweenMs),
    ...
    update: (n) => {
      if (!A) return;
      const T = (n - A.startedAt) / A.duration, z = bi(T);
      Q.position.set(
        l.MathUtils.lerp(A.from.x, A.to.x, z),
        l.MathUtils.lerp(A.from.y, A.to.y, z),
        l.MathUtils.lerp(A.from.z, A.to.z, z)
      ), T >= 1 && (Q.position.set(A.to.x, A.to.y, A.to.z), A = null);
    },
    dispose: () => {
      r == null || r.kill(), P == null || P.kill(), Q.remove(B), CA(B);
    }
  };
}
const Ji = 25.4, Ri = 500, Xi = 300;
class Br {
  constructor(Q) {
    this.viewCube = null, this.resizeObserver = null, this.onWindowResize = null, this.animationFrameId = null, this.gridGroup = null, this.axesGroup = null, this.gridLabelsGroup = null, this.boundingBoxGroup = null, this.machineBedGroup = null, this.bitMarker = null, this.preLaserBitType = "drill", this.toolpathStreams = [], this.toolpathCutBucketCount = 1, this.toolpathRotationA = 0, this.lastBitPosition = { x: 0, y: 0, z: 0, a: 0 }, this.sim3dHandle = null, this.currentLines = [], this.linePositions = null, this.renderSequence = 0, this.currentBounds = null, this.cameraFocusTransition = null, this.cameraFollowRequested = !1, this.cameraFollowInterrupted = !1, this.cameraFollowOffset = null, this.id = Q.id, this.container = Q.container, this.callbacks = Q.callbacks ?? {}, this.options = OA(Ie, Q.options), this.canvas = document.createElement("canvas"), this.canvas.style.width = "100%", this.canvas.style.height = "100%", this.canvas.style.display = "block", this.container.appendChild(this.canvas), eP(this.container), this.renderer = new l.WebGLRenderer({
      canvas: this.canvas,
    ...
    const t = new l.DirectionalLight(16777215, 0.9);
    t.position.set(1, -1, 1.5), this.scene.add(t);
    ...
    this.perspectiveCamera = new l.PerspectiveCamera(this.options.camera.fov, 1, 0.1, 1e5), this.orthographicCamera = new l.OrthographicCamera(-1, 1, 1, -1, -1, 1), this.orthoViewHeight = Ot(e, this.options.camera.fov);
    for (const P of [this.perspectiveCamera, this.orthographicCamera])
      P.up.set(0, 0, 1), P.position.set(A.x, A.y, A.z);
    Object.assign(this.perspectiveCamera, tA(e)), this.perspectiveCamera.updateProjectionMatrix(), Object.assign(
      this.orthographicCamera,
      Xt(this.orthoViewHeight, 1),
      BA(e, e)
    ), this.orthographicCamera.updateProjectionMatrix(), this.camera = this.options.camera.projection === "orthographic" ? this.orthographicCamera : this.perspectiveCamera, this.controls = new AP(this.camera, this.renderer.domElement), this.controls.enableDamping = this.options.camera.orbit.enableDamping, this.controls.addEventListener("start", () => {
      this.cameraFollowRequested && (this.cameraFollowInterrupted = !0);
    }), this.controls.update(), this.viewCubeCorrection = new l.Matrix4().makeRotationX(l.MathUtils.degToRad(90)), this.viewCube = new cB({
      ...
      onSelectView: (P) => {
        ...
        if (this.currentBounds) {
          ...
          this.currentBounds.getSize(c), s = QA(c, this.options.camera.fov);
        } else
          s = this.camera.position.distanceTo(this.controls.target);
        this.startSnapToView(P, r, Math.max(1e-6, s), 400, v);
      }
  ...
  setBitPosition(Q, t) {
    var B;
    if (this.ensureBitMarker(), this.lastBitPosition = { ...this.lastBitPosition, ...Q }, (B = this.bitMarker) == null || B.setTarget(Q, t), this.cameraFollowRequested && !this.cameraFollowInterrupted && this.cameraFollowOffset) {
  ...
  setCameraFollowEnabled(Q) {
    ...
    const t = new l.Vector3(this.lastBitPosition.x, this.lastBitPosition.y, this.controls.target.z), B = t.clone().add(this.cameraFollowOffset);
    this.startCameraLerp(t, B, Ri);
  }
  setBitVisible(Q) {
    var t;
    this.ensureBitMarker(), (t = this.bitMarker) == null || t.setVisible(Q);
  }
  setBitSpinning(Q) {
    var t;
    this.ensureBitMarker(), (t = this.bitMarker) == null || t.setSpinning(Q);
  }
  setToolpathRotationA(Q) {
    const t = Number(Q) || 0;
    this.toolpathRotationA = t, this.toolpathRoot.rotation.x = l.MathUtils.degToRad(t);
  }
  setCallbacks(Q) {
    this.callbacks = Q;
  }
  hideUntilLine(Q, t) {
    const B = t ?? this.options.progress.mode, A = Math.floor(Q);
    for (const e of this.toolpathStreams) {
      const P = A < 0 ? 0 : e.prefixEndVertex[Math.min(A, e.prefixEndVertex.length - 1)];
      B === "grey" ? (e.line.geometry.setDrawRange(0, e.totalVertices), me({ stream: e, nextCursorVertex: P, options: this.options })) : e.line.geometry.setDrawRange(P, Math.max(0, e.totalVertices - P));
    }
  }
  seekToLine(Q, t) {
    if (this.hideUntilLine(Q, t), this.linePositions && this.currentLines.length > 0) {
      const B = Math.max(0, Math.min(Math.floor(Q), this.currentLines.length - 1)), A = {
    ...
    if (this.sim3dHandle) {
      const B = Math.max(0, Math.min(Math.floor(Q), this.currentLines.length - 1));
      if (B !== this.sim3dHandle.currentLine) {
        const A = Vt(B, this.sim3dHandle.data);
        Jt(A, this.sim3dHandle.data.resolution, this.options.sim3d.erosionPasses, this.sim3dHandle.data.slabBounds.zTop), Rt(this.sim3dHandle.slab, A), this.sim3dHandle.currentLine = B;
      }
    }
  }
  showAll() {
   ...
   */
  setLineGroupVisible(Q, t) {
    for (const B of this.toolpathStreams)
      B.lineGroupIndex === Q && (B.line.visible = t);
  }
  showAllLineGroups() {
    for (const Q of this.toolpathStreams)
      Q.line.visible = !0;
  ...
  resetColors() {
    for (const Q of this.toolpathStreams) {
      ...
      const t = Q.line.geometry.getAttribute("color");
      t.clearUpdateRanges(), t.addUpdateRange(0, Q.simColors.length), t.needsUpdate = !0;
    }
  }
  /**
   ...
   */
  setCameraProjection(Q) {
    this.setOptions({ camera: { ...this.options.camera, projection: Q } });
  }
  getCameraProjection() {
   ...
   */
  snapCameraToView(Q, t = {}) {
    const B = Math.max(0, Math.floor(t.durationMs ?? 240)), A = this.controls.target.clone(), e = this.camera.position.distanceTo(A), P = Math.max(1e-6, t.distance ?? e);
    this.startSnapToView(Q, A, P, B);
  }
  /**
  ...
  screenToWorld(Q, t, B) {
    ...
    const e = new l.Vector2(
      (Q - A.left) / A.width * 2 - 1,
      -((t - A.top) / A.height) * 2 + 1
    ), P = new l.Raycaster();
    P.setFromCamera(e, this.camera);
    const r = (B == null ? void 0 : B.planeZ) ?? this.lastBitPosition.z ?? 0, s = rP(
      P.ray.origin,
      P.ray.direction,
      r,
   ...
   */
  worldToScreen(Q, t, B = 0) {
    const A = this.canvas.getBoundingClientRect();
    if (A.width === 0 || A.height === 0)
      return null;
    const e = new l.Vector3(Q, t, B).project(this.camera);
    return {
  ...
  viewportAspect() {
    const Q = this.container.clientWidth || 1, t = this.container.clientHeight || 1;
    return Q / t;
  }
  /** Ortho zoom that frames `height` world units vertically. */
  orthoZoomForHeight(Q) {
    return this.orthoViewHeight / Math.max(1e-6, Q);
  }
  ...
  applyCameraDepthRange(Q) {
    const t = Math.max(1, Q);
    Object.assign(this.perspectiveCamera, tA(t)), Object.assign(this.orthographicCamera, BA(t, t)), this.perspectiveCamera.updateProjectionMatrix(), this.orthographicCamera.updateProjectionMatrix();
  }
  /**
   ...
   */
  setActiveProjection(Q) {
    const t = Q !== "orthographic";
    if (t === this.isPerspectiveActive())
      return;
    const B = this.controls.target, A = Math.max(1e-6, this.camera.position.distanceTo(B)), e = this.viewportAspect();
    if (t) {
      const P = this.orthoFramedHeight(), r = Math.max(1e-6, MA(P, this.options.camera.fov)), s = this.camera.position.clone().sub(B).normalize();
      ...
      const P = Ot(A, this.options.camera.fov);
      this.orthographicCamera.zoom = this.orthoZoomForHeight(P), this.orthographicCamera.position.copy(this.camera.position), this.orthographicCamera.quaternion.copy(this.camera.quaternion), Object.assign(this.orthographicCamera, Xt(this.orthoViewHeight, e)), this.camera = this.orthographicCamera, this.applyCameraDepthRange(A);
    }
  ...
  startSnapToView(Q, t, B, A, e = !1) {
    const P = $B(Q), r = t.clone().add(P.multiplyScalar(B));
    this.applyCameraDepthRange(B);
    const s = e && !this.isPerspectiveActive() ? this.orthoZoomForHeight(Ot(B, this.options.camera.fov)) : void 0;
    this.startCameraLerp(t, r, A, "easeInOutCubic", s);
  }
  ...
  startCameraLerp(Q, t, B, A = "easeInOutCubic", e) {
  ...
  async loadFromUrl(Q, t = {}) {
    this.emitProgress({ state: "indeterminate", label: "Loading file..." });
    const B = await fetch(Q, { signal: t.signal });
    if (!B.ok)
   ...
   */
  async loadFromWorkerData(Q, t) {
    this.currentLines = [];
    const { rapids: B, cuts: A } = Me(Q, t), e = (Q.toolchangeCount ?? 0) > 0;
  ...
  setOptions(Q) {
    var T, z, a, o, w, E, h, u, U, k, O, D, N, d, M, b, G, C, j, F, H, f, I, R, QQ, sQ, W, x, tQ, vQ, rt, JB, RB, XB;
    const t = this.options;
    this.options = OA(this.options, Q), (t.render.antialias !== this.options.render.antialias || t.render.theme.background !== this.options.render.theme.background) && this.renderer.setClearColor(this.options.render.theme.background, 1), (t.render.theme.opacity !== this.options.render.theme.opacity || t.render.theme.rapidOpacity !== this.options.render.theme.rapidOpacity) && this.refreshToolpathOpacities(), t.camera.fov !== this.options.camera.fov && (this.perspectiveCamera.fov = this.options.camera.fov, this.perspectiveCamera.updateProjectionMatrix()), t.camera.projection !== this.options.camera.projection && this.setActiveProjection(this.options.camera.projection), t.camera.orbit.enableDamping !== this.options.camera.orbit.enableDamping && (this.controls.enableDamping = this.options.camera.orbit.enableDamping);
    const B = ((T = t.grid.bounds) == null ? void 0 : T.min.x) !== ((z = this.options.grid.bounds) == null ? void 0 : z.min.x) || ((a = t.grid.bounds) == null ? void 0 : a.min.y) !== ((o = this.options.grid.bounds) == null ? void 0 : o.min.y) || ((w = t.grid.bounds) == null ? void 0 : w.max.x) !== ((E = this.options.grid.bounds) == null ? void 0 : E.max.x) || ((h = t.grid.bounds) == null ? void 0 : h.max.y) !== ((u = this.options.grid.bounds) == null ? void 0 : u.max.y), A = t.units !== this.options.units || t.grid.sizeX !== this.options.grid.sizeX || t.grid.sizeY !== this.options.grid.sizeY || t.grid.axisDepth !== this.options.grid.axisDepth || B, e = t.render.theme.colors.grid.major !== this.options.render.theme.colors.grid.major || t.render.theme.colors.grid.minor !== this.options.render.theme.colors.grid.minor || t.render.theme.colors.axes.x !== this.options.render.theme.colors.axes.x || t.render.theme.colors.axes.y !== this.options.render.theme.colors.axes.y || t.render.theme.colors.axes.z !== this.options.render.theme.colors.axes.z;
    (A || e) && this.renderGridAndAxes(), (t.grid.labels !== this.options.grid.labels || t.units !== this.options.units || t.grid.sizeX !== this.options.grid.sizeX || t.grid.sizeY !== this.options.grid.sizeY || B || e) && this.refreshGridLabels(), (t.boundingBox.visible !== this.options.boundingBox.visible || t.boundingBox.labels !== this.options.boundingBox.labels || t.units !== this.options.units || t.render.theme.colors.boundingBox !== this.options.render.theme.colors.boundingBox) && this.refreshBoundingBox(), (t.machineBed.visible !== this.options.machineBed.visible || ((U = t.machineBed.min) == null ? void 0 : U.x) !== ((k = this.options.machineBed.min) == null ? void 0 : k.x) || ((O = t.machineBed.min) == null ? void 0 : O.y) !== ((D = this.options.machineBed.min) == null ? void 0 : D.y) || ((N = t.machineBed.max) == null ? void 0 : N.x) !== ((d = this.options.machineBed.max) == null ? void 0 : d.x) || ((M = t.machineBed.max) == null ? void 0 : M.y) !== ((b = this.options.machineBed.max) == null ? void 0 : b.y) || ((C = (G = t.machineBed.keepout) == null ? void 0 : G.min) == null ? void 0 : C.x) !== ((F = (j = this.options.machineBed.keepout) == null ? void 0 : j.min) == null ? void 0 : F.x) || ((f = (H = t.machineBed.keepout) == null ? void 0 : H.min) == null ? void 0 : f.y) !== ((R = (I = this.options.machineBed.keepout) == null ? void 0 : I.min) == null ? void 0 : R.y) || ((sQ = (QQ = t.machineBed.keepout) == null ? void 0 : QQ.max) == null ? void 0 : sQ.x) !== ((x = (W = this.options.machineBed.keepout) == null ? void 0 : W.max) == null ? void 0 : x.x) || ((vQ = (tQ = t.machineBed.keepout) == null ? void 0 : tQ.max) == null ? void 0 : vQ.y) !== ((JB = (rt = this.options.machineBed.keepout) == null ? void 0 : rt.max) == null ? void 0 : JB.y) || t.render.theme.colors.machineBed !== this.options.render.theme.colors.machineBed || t.render.theme.colors.machineBedKeepout !== this.options.render.theme.colors.machineBedKeepout) && this.refreshMachineBed();
    const v = t.render.theme.colors.rapid !== this.options.render.theme.colors.rapid || t.render.theme.colors.cutting !== this.options.render.theme.colors.cutting || t.render.theme.colors.laser !== this.options.render.theme.colors.laser || t.render.theme.background !== this.options.render.theme.background || t.render.theme.colors.processed !== this.options.render.theme.colors.processed;
    if (v && this.refreshToolpathColors(), (t.bit.enabled !== this.options.bit.enabled || t.bit.type !== this.options.bit.type || t.bit.size !== this.options.bit.size || t.bit.opacity !== this.options.bit.opacity || t.bit.colorSource !== this.options.bit.colorSource || t.bit.color !== this.options.bit.color || v) && (this.ensureBitMarker(), (RB = this.bitMarker) == null || RB.setOptions(this.options)), t.mode.laser !== this.options.mode.laser && (this.options.mode.laser ? this.options.bit.type !== "laser" && (this.preLaserBitType = this.options.bit.type, this.options = { ...this.options, bit: { ...this.options.bit, type: "laser" } }) : this.options = { ...this.options, bit: { ...this.options.bit, type: this.preLaserBitType } }, this.ensureBitMarker(), (XB = this.bitMarker) == null || XB.setOptions(this.options), this.currentLines.length > 0 && this.renderScene()), t.mode.sim3d !== this.options.mode.sim3d)
    ...
    if ((t.sim3d.toolDiameter !== this.options.sim3d.toolDiameter || t.sim3d.resolution !== this.options.sim3d.resolution) && this.options.mode.sim3d) {
      this.renderScene();
      return;
    }
    if (t.sim3d.erosionPasses !== this.options.sim3d.erosionPasses && this.sim3dHandle) {
      const Le = this.sim3dHandle.currentLine, KB = Vt(Le, this.sim3dHandle.data);
      Jt(KB, this.sim3dHandle.data.resolution, this.options.sim3d.erosionPasses, this.sim3dHandle.data.slabBounds.zTop), Rt(this.sim3dHandle.slab, KB);
    }
    t.sim3d.showToolpath !== this.options.sim3d.showToolpath && this.options.mode.sim3d && this.setToolpathStreamsVisible(this.options.sim3d.showToolpath);
  }
  getOptions() {
    return this.options;
  }
  resize() {
    const Q = this.container.getBoundingClientRect(), t = Math.max(
      1,
    ...
    this.renderer.setSize(t, B, !1);
    const A = t / B;
    this.perspectiveCamera.aspect = A, this.perspectiveCamera.updateProjectionMatrix(), Object.assign(this.orthographicCamera, Xt(this.orthoViewHeight, A)), this.orthographicCamera.updateProjectionMatrix();
  }
  focusToModel() {
  ...
  startAnimationLoop() {
    const Q = () => {
      ...
      this.controls.update(), this.updateCameraFocusTransition(), this.updateViewCubeRotation(), (B = this.bitMarker) == null || B.update(t), this.renderer.render(this.scene, this.camera);
    };
    Q();
  }
  ensureBitMarker() {
    this.bitMarker || (this.bitMarker = Vi(this.options), this.scene.add(this.bitMarker.object));
  }
  worldSizes() {
    const Q = this.options.units === "in" ? Ji : 1;
    return {
      sizeXMm: Math.max(1, this.options.grid.sizeX * Q),
      sizeYMm: Math.max(1, this.options.grid.sizeY * Q),
  ...
  renderGridAndAxes() {
    const { sizeXMm: Q, sizeYMm: t, axisDepthMm: B, bounds: A } = this.worldSizes();
    this.setGridGroup(
      sP({
  ...
  refreshGridLabels() {
    ...
    const { sizeXMm: Q, sizeYMm: t, bounds: B } = this.worldSizes();
    this.setGridLabelsGroup(
      oP({ sizeXMm: Q, sizeYMm: t, bounds: B, units: this.options.units, theme: this.options.render.theme })
    );
  }
  refreshBoundingBox() {
  ...
  refreshMachineBed() {
    const { visible: Q, min: t, max: B, keepout: A } = this.options.machineBed;
    if (!Q || !t || !B) {
      this.setMachineBedGroup(null);
      return;
    }
    this.setMachineBedGroup(tP(t, B, this.options, A));
  }
  setBitMarker(Q) {
  ...
  setGridGroup(Q) {
    this.gridGroup && (this.scene.remove(this.gridGroup), vP(this.gridGroup), this.gridGroup = null), Q && (this.gridGroup = Q, this.scene.add(Q));
  }
  setAxesGroup(Q) {
    this.axesGroup && (this.scene.remove(this.axesGroup), nP(this.axesGroup), this.axesGroup = null), Q && (this.axesGroup = Q, this.scene.add(Q));
  }
  setGridLabelsGroup(Q) {
    this.gridLabelsGroup && (this.scene.remove(this.gridLabelsGroup), Ze(this.gridLabelsGroup), this.gridLabelsGroup = null), Q && (this.gridLabelsGroup = Q, this.scene.add(Q));
  }
  setBoundingBoxGroup(Q) {
    this.boundingBoxGroup && (this.scene.remove(this.boundingBoxGroup), $e(this.boundingBoxGroup), this.boundingBoxGroup = null), Q && (this.boundingBoxGroup = Q, this.scene.add(Q));
  }
  setMachineBedGroup(Q) {
    this.machineBedGroup && (this.scene.remove(this.machineBedGroup), BP(this.machineBedGroup), this.machineBedGroup = null), Q && (this.machineBedGroup = Q, this.scene.add(Q));
  }
  setGeometryEmpty() {
  ...
  setToolpathGeometry(Q) {
    ...
    const t = [
      ...Q.rapids.map((e) => ({
        ...
        colors: e.colors,
        opacity: NA(this.options.render.theme.rapidOpacity ?? 0.3)
      })),
      ...Q.cuts.map((e, P) => {
        const r = e.cutBucketIndex ?? P;
        return {
          kind: "cut",
          cutBucketIndex: r,
          lineGroupIndex: e.lineGroupIndex ?? null,
  ...
  refreshToolpathColors() {
    pe(this.toolpathStreams, this.options);
  }
  ...
  cutBucketOpacity(Q) {
    const t = this.options.render.theme.opacity, B = this.toolpathCutBucketCount;
    if (B <= 1)
      return t;
    const A = Q / (B - 1);
    return NA(t * A);
  ...
  buildLinePositions() {
    ...
    for (let r = 0; r < Q; r++) {
      ...
      s && P.processLine(s), this.linePositions[r * 4] = t, this.linePositions[r * 4 + 1] = B, this.linePositions[r * 4 + 2] = A, this.linePositions[r * 4 + 3] = e;
    }
  }
  async renderScene() {
    ...
    try {
      ...
      const t = await He(this.currentLines, {
        ...
        batch: {
          ...
          onProgress: (B, A) => {
            Q === this.renderSequence && this.emitProgress({ state: "determinate", label: "Building geometry...", processed: B, total: A });
          }
        }
      });
      ...
      this.setToolpathGeometry({
        rapids: [t.rapid],
        cuts: t.cuts,
        cutBucketCount: t.cutBucketCount
      ...
      if (t instanceof Error && t.message === "Aborted.")
        return;
      throw t;
    } finally {
  ...
  setSim3dHandle(Q) {
    this.sim3dHandle && (this.scene.remove(this.sim3dHandle.slab.group), Re(this.sim3dHandle.slab), this.sim3dHandle = null), Q && (this.sim3dHandle = Q, this.scene.add(Q.slab.group));
  }
  ...
  async buildAndApplySim3d(Q) {
    ...
    const t = this.options.sim3d.toolDiameter / 2, B = this.options.sim3d.resolution;
    let A;
    try {
      A = await Ye(this.currentLines, t, B, {
        arcSegments: this.options.geometry.arcSegments,
        batch: {
          shouldAbort: () => Q !== this.renderSequence,
          onProgress: (r, s) => {
    ...
    const e = Vt(0, A);
    Jt(e, B, this.options.sim3d.erosionPasses, A.slabBounds.zTop);
    const P = Je(A.slabBounds, B);
    Rt(P, e), this.setSim3dHandle({ data: A, slab: P, currentLine: 0 });
  }
  startCameraFocus(Q) {
    ...
    Q.getSize(B);
    const A = QA(B, this.options.camera.fov), e = $B("front-top-left"), P = t.clone().add(e.multiplyScalar(A));
    this.applyCameraDepthRange(Math.max(B.x, B.y, B.z, A, 1));
    const r = this.isPerspectiveActive() ? void 0 : this.orthoZoomForHeight(Ot(A, this.options.camera.fov));
  ...
  updateCameraFocusTransition() {
    if (!this.cameraFocusTransition)
      return;
    const t = performance.now() - this.cameraFocusTransition.startedAt, B = Math.min(1, Math.max(0, t / this.cameraFocusTransition.duration)), A = this.cameraFocusTransition.easing === "linear" ? B : PP(B);
    ...
    const { fromZoom: e, toZoom: P } = this.cameraFocusTransition;
    if (e !== null && P !== null && (this.orthographicCamera.zoom = l.MathUtils.lerp(e, P, A), this.orthographicCamera.updateProjectionMatrix()), B >= 1) {
      const r = this.cameraFocusTransition.dampingEnabled;
      this.camera.position.copy(this.cameraFocusTransition.toPosition), this.controls.target.copy(this.cameraFocusTransition.toTarget), P !== null && (this.orthographicCamera.zoom = P, this.orthographicCamera.updateProjectionMatrix()), this.cameraFocusTransition = null, this.controls.enableDamping = !1, this.controls.update(), this.controls.enableDamping = r;
    }
  }
  updateViewCubeRotation() {
    ...
    const t = new l.Matrix4();
    t.makeRotationFromQuaternion(Q), t.multiply(this.viewCubeCorrection), this.viewCube.setRotationMatrix3d(t.elements);
    const B = iP(this.camera.position, this.controls.target);
...
function OA(i, Q) {
  ...
  if (!Q)
    return { ...i };
  const t = Ki(i.render.theme, (B = Q.render) == null ? void 0 : B.theme);
  return {
    ...i,
    ...Q,
    mode: { ...i.mode, ...Q.mode },
    sim3d: { ...i.sim3d, ...Q.sim3d },
    bit: { ...i.bit, ...Q.bit },
    progress: { ...i.progress, ...Q.progress },
    grid: { ...i.grid, ...Q.grid },
    boundingBox: { ...i.boundingBox, ...Q.boundingBox },
    machineBed: { ...i.machineBed, ...Q.machineBed },
    geometry: {
      ...i.geometry,
      ...Q.geometry,
      batching: { ...i.geometry.batching, ...(A = Q.geometry) == null ? void 0 : A.batching }
    },
    render: {
      ...i.render,
      ...Q.render,
    ...
    camera: {
      ...i.camera,
      ...Q.camera,
      orbit: { ...i.camera.orbit, ...(e = Q.camera) == null ? void 0 : e.orbit },
      initialPosition: { ...i.camera.initialPosition, ...(P = Q.camera) == null ? void 0 : P.initialPosition }
    }
  };
}
function Ki(i, Q) {
  ...
  return Q ? {
    ...
    colors: {
      ...i.colors,
      ...Q.colors,
      grid: { ...i.colors.grid, ...(t = Q.colors) == null ? void 0 : t.grid },
      axes: { ...i.colors.axes, ...(B = Q.colors) == null ? void 0 : B.axes }
    }
  } : i;
}
const Zi = {
...
class Ar {
  constructor(Q, t) {
    this.crosshairPos = null, this.crosshairVisible = !1, this.pathEls = [], this.segmentGroups = [], this.workerMode = !1, this.viewBox = { x: 0, y: 0, w: 100, h: 100 }, this.rotX = vt, this.rotY = ct, this.centerX = 0, this.centerY = 0, this.centerZ = 0, this.focalLength = 500, this.cosRotX = 1, this.sinRotX = 0, this.cosRotY = 1, this.sinRotY = 0, this.dragMode = "none", this.activePointers = /* @__PURE__ */ new Map(), this.pinchLastDist = 0, this.pinchLastMid = { x: 0, y: 0 }, this.rafPending = !1, this.overlayRafPending = !1, this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.bounds = { minX: 0, minY: 0, maxX: 0, maxY: 0, minZ: 0, maxZ: 0, empty: !0 }, this.hasZInfo = !0, this.project = (B, A, e) => {
      ...
      B.preventDefault();
      const A = B.deltaY > 0 ? 1.1 : 1 / 1.1, e = this.svgPoint(B.clientX, B.clientY), { x: P, y: r, w: s, h: v } = this.viewBox;
      this.viewBox = {
        x: e.x - (e.x - P) * A,
        y: e.y - (e.y - r) * A,
        w: s * A,
      ...
      const e = B.clientX - A.x, P = B.clientY - A.y;
      if (this.dragMode === "orbit") {
        const r = this.svg.getBoundingClientRect(), s = Math.PI / Math.min(r.width, r.height);
        this.rotY += e * s, this.rotX += P * s, this.rotX = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, this.rotX)), this.updateTrig(), this.fitView(), this.scheduleDraw();
      } else {
        const r = this.svg.getBoundingClientRect();
        this.viewBox = {
          ...this.viewBox,
          x: this.viewBox.x - e / r.width * this.viewBox.w,
          y: this.viewBox.y - P / r.height * this.viewBox.h
        }, this.applyViewBox();
      }
    }, this.onPointerUp = (B) => {
      ...
      B.preventDefault();
    }, this.options = { ...Zi, ...t }, this.svg = document.createElementNS("http://www.w3.org/2000/svg", "svg"), this.svg.style.cssText = "width:100%;height:100%;display:block;cursor:grab;user-select:none;touch-action:none;", this.svg.setAttribute("preserveAspectRatio", "xMidYMid meet"), this.svg.setAttribute("xmlns", "http://www.w3.org/2000/svg"), this.bboxPath = AB(), this.pathLayer = document.createElementNS("http://www.w3.org/2000/svg", "g"), this.bboxLabelX = eB("middle", "hanging"), this.bboxLabelY = eB("start", "middle"), this.bboxLabelZ = eB("middle", "auto"), this.originMarker = document.createElementNS("http://www.w3.org/2000/svg", "circle"), this.originMarker.setAttribute("stroke-opacity", "0.4"), this.originMarker.setAttribute("visibility", "hidden"), this.crosshairEl = AB(), this.crosshairEl.setAttribute("fill", "none"), this.crosshairEl.setAttribute("visibility", "hidden"), this.svg.appendChild(this.bboxPath), this.svg.appendChild(this.pathLayer), this.svg.appendChild(this.bboxLabelX), this.svg.appendChild(this.bboxLabelY), this.svg.appendChild(this.bboxLabelZ), this.svg.appendChild(this.originMarker), this.svg.appendChild(this.crosshairEl), Q.appendChild(this.svg), this.applyOptions(), this.bindEvents();
  }
  // ── Public API ────────────────────────────────────────────────────────────
  loadFromLines(Q) {
    const { rapid: t, cutting: B } = je(Q, {
      arcSegments: this.options.arcSegments
    });
    if (this.rapidVerts = t, this.cutVerts = B, this.workerMode = !1, this.syncSegmentGroupsFromLines(), this.hasZInfo = !0, this.bounds = iB([
      { verts: t, stride: 6 },
      { verts: B, stride: 6 }
    ]), !this.bounds.empty) {
    ...
    this.rotX = vt, this.rotY = ct, this.updateTrig(), this.fitView(), this.rebuildAndRender();
  }
  loadFromFile(Q) {
    return new Promise((t, B) => {
      ...
      A.onload = (e) => {
        var r;
        const P = (r = e.target) == null ? void 0 : r.result;
        if (typeof P != "string") {
          B(new Error("Failed to read file"));
          return;
        }
        this.loadFromText(P), t();
      }, A.onerror = () => B(new Error("FileReader error")), A.readAsText(Q);
    });
  }
  loadFromText(Q) {
  ...
  loadFromWorkerData(Q) {
    const t = Fe(Q);
    if (this.workerMode = !0, this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.segmentGroups = t.map((B) => ({
      color: B.hexColor,
      opacity: B.opacity,
      verts: B.positions,
      stride: 6
    })), this.hasZInfo = !0, this.bounds = iB(t.map((B) => ({ verts: B.positions, stride: 6 }))), !this.bounds.empty) {
      this.centerX = (this.bounds.minX + this.bounds.maxX) / 2, this.centerY = (this.bounds.minY + this.bounds.maxY) / 2, this.centerZ = (this.bounds.minZ + this.bounds.maxZ) / 2;
      const B = Math.hypot(
        this.bounds.maxX - this.bounds.minX,
        this.bounds.maxY - this.bounds.minY,
  ...
  loadFromPrecomputedGroups(Q, t) {
    this.workerMode = !0, this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.segmentGroups = Q.map((A) => ({
      color: A.hexColor,
      opacity: A.opacity,
      verts: new Float32Array(A.positionsBuffer, 0, A.positionsLen),
      stride: A.stride ?? 6
    })), this.bounds = iB(this.segmentGroups);
    const B = (t == null ? void 0 : t.minZ) !== void 0 && (t == null ? void 0 : t.maxZ) !== void 0;
    if (B && !this.bounds.empty && (this.bounds.minZ = t.minZ, this.bounds.maxZ = t.maxZ), this.hasZInfo = B || this.segmentGroups.every((A) => A.stride === 6), !this.bounds.empty) {
      this.centerX = (this.bounds.minX + this.bounds.maxX) / 2, this.centerY = (this.bounds.minY + this.bounds.maxY) / 2, this.centerZ = (this.bounds.minZ + this.bounds.maxZ) / 2;
      const A = Math.hypot(
        this.bounds.maxX - this.bounds.minX,
        this.bounds.maxY - this.bounds.minY,
  ...
  setBitPosition(Q) {
    this.crosshairPos = Q, this.crosshairVisible = !0, this.scheduleOverlayDraw();
  }
  setBitVisible(Q) {
    this.crosshairVisible = Q, this.scheduleOverlayDraw();
  }
  dispose() {
  ...
  rebuildAndRender(Q = !1) {
    this.rebuildToolpaths(Q), this.renderOverlays();
  }
  ...
  rebuildToolpaths(Q = !1) {
    const t = Q ? Wi : L, B = String(this.options.strokeWidth), A = this.simplifyTolerance();
    ...
    for (let e = 0; e < this.segmentGroups.length; e++) {
      const { color: P, opacity: r, verts: s, stride: v } = this.segmentGroups[e], c = this.pathEls[e];
      c.setAttribute("stroke", P), c.setAttribute("stroke-opacity", String(r)), c.setAttribute("stroke-width", B), c.setAttribute("stroke-linecap", "round"), c.setAttribute("stroke-linejoin", "round"), c.setAttribute(
        "d",
        v === 4 ? qi(s, t, A) : _i(s, this.project, t, A)
      );
    }
  }
  ...
  fitView() {
    ...
    const { minX: Q, maxX: t, minY: B, maxY: A, minZ: e, maxZ: P } = this.bounds, r = this.project, s = r(Q, B, e), v = r(t, B, e), c = r(t, A, e), n = r(Q, A, e), T = r(Q, B, P), z = r(t, B, P), a = r(t, A, P), o = r(Q, A, P), w = Math.hypot(v.x - s.x, v.y - s.y), E = Math.hypot(n.x - s.x, n.y - s.y), h = Math.hypot(T.x - s.x, T.y - s.y), u = Math.max(w, E, h) * 0.07, U = u * 0.5, k = PB(LQ(s, v), LQ(c, n), U), O = [s, v, c, n, T, z, a, o], D = Math.min(...O.map((x) => x.y)), N = Math.max(...O.map((x) => x.y)), d = Math.min(...O.map((x) => x.x)), M = Math.max(...O.map((x) => x.x)), b = { x: M + U, y: (D + N) / 2 }, G = { x: b.x + u * 5, y: b.y }, C = { x: (d + M) / 2, y: D - u }, j = [s, v, c, n, T, z, a, o, k, b, G, C];
  ...
  applyViewBox() {
    const { x: Q, y: t, w: B, h: A } = this.viewBox;
    this.svg.setAttribute("viewBox", `${L(Q)} ${L(t)} ${L(B)} ${L(A)}`);
  }
  applyOptions() {
    const { boundingBoxColor: Q, strokeWidth: t, originColor: B } = this.options;
    this.workerMode || this.syncSegmentGroupsFromLines(), this.bboxPath.setAttribute("stroke", Q), this.bboxPath.setAttribute("stroke-width", String(t * 0.6)), this.bboxPath.setAttribute("fill", Q), this.bboxPath.setAttribute("fill-opacity", "0.05");
    for (const A of [this.bboxLabelX, this.bboxLabelY, this.bboxLabelZ])
      A.setAttribute("fill", Q);
  ...
  renderBbox() {
    ...
    const { minX: Q, maxX: t, minY: B, maxY: A, minZ: e, maxZ: P } = this.bounds, r = this.project;
    if (this.options.projectionMode === "top") {
      const C = r(Q, B, 0), j = r(t, B, 0), F = r(t, A, 0), H = r(Q, A, 0);
      ...
      ZQ(this.bboxLabelX, LQ(C, j), PB(LQ(C, j), LQ(F, H), QQ), R, `X: ${WQ(t - Q)}`);
      const sQ = Math.min(C.y, j.y, F.y, H.y), W = Math.max(C.y, j.y, F.y, H.y), x = Math.max(C.x, j.x, F.x, H.x), tQ = Math.min(C.x, j.x, F.x, H.x), vQ = { x: x + QQ, y: (sQ + W) / 2 };
      if (ZQ(this.bboxLabelY, vQ, vQ, R, `Y: ${WQ(A - B)}`), this.hasZInfo) {
        const rt = { x: (tQ + x) / 2, y: sQ - R };
        ZQ(this.bboxLabelZ, rt, rt, R, `Z: ${WQ(P - e)}`);
      } else
        this.bboxLabelZ.setAttribute("visibility", "hidden");
      return;
    }
    const s = r(Q, B, e), v = r(t, B, e), c = r(t, A, e), n = r(Q, A, e), T = r(Q, B, P), z = r(t, B, P), a = r(t, A, P), o = r(Q, A, P), w = [
    ...
    ZQ(this.bboxLabelX, LQ(s, v), PB(LQ(s, v), LQ(c, n), k), U, `X: ${WQ(t - Q)}`);
    const O = [s, v, c, n, T, z, a, o], D = Math.min(...O.map((C) => C.y)), N = Math.max(...O.map((C) => C.y)), d = Math.min(...O.map((C) => C.x)), M = Math.max(...O.map((C) => C.x)), b = { x: M + k, y: (D + N) / 2 };
    ZQ(this.bboxLabelY, b, b, U, `Y: ${WQ(A - B)}`);
    const G = { x: (d + M) / 2, y: D - U };
    ZQ(this.bboxLabelZ, G, G, U, `Z: ${WQ(P - e)}`);
  }
  renderOriginMarker() {
    ...
    const Q = this.project(0, 0, 0), t = Math.min(this.viewBox.w, this.viewBox.h) * 0.018;
    this.originMarker.setAttribute("cx", L(Q.x)), this.originMarker.setAttribute("cy", L(Q.y)), this.originMarker.setAttribute("r", L(t)), this.originMarker.setAttribute("stroke-width", L(t * 0.25)), this.originMarker.setAttribute("visibility", "visible");
  }
  renderCrosshairMarker() {
    ...
    const { x: Q, y: t, z: B } = this.crosshairPos, A = this.project(Q, t, B), e = Math.min(this.viewBox.w, this.viewBox.h) * 0.025, P = e * 0.25, r = A.x, s = A.y, v = [
      `M${L(r - e)} ${L(s)}L${L(r - P)} ${L(s)}`,
      `M${L(r + P)} ${L(s)}L${L(r + e)} ${L(s)}`,
      `M${L(r)} ${L(s - e)}L${L(r)} ${L(s - P)}`,
      `M${L(r)} ${L(s + P)}L${L(r)} ${L(s + e)}`
    ].join("");
  ...
  svgPoint(Q, t) {
    const B = this.svg.getBoundingClientRect(), { x: A, y: e, w: P, h: r } = this.viewBox;
    return {
      x: A + (Q - B.left) / B.width * P,
      y: e + (t - B.top) / B.height * r
    };
  }
}
function AB() {
  const i = document.createElementNS("http://www.w3.org/2000/svg", "path");
  return i.setAttribute("fill", "none"), i;
}
function eB(i, Q) {
  const t = document.createElementNS("http://www.w3.org/2000/svg", "text");
  return t.setAttribute("text-anchor", i), t.setAttribute("dominant-baseline", Q), t;
}
function ZQ(i, Q, t, B, A) {
  i.setAttribute("x", L(t.x)), i.setAttribute("y", L(t.y)), i.setAttribute("font-size", L(B)), i.textContent = A, i.setAttribute("visibility", "visible");
}
function LQ(i, Q) {
  return { x: (i.x + Q.x) / 2, y: (i.y + Q.y) / 2 };
}
function PB(i, Q, t) {
  const B = i.x - Q.x, A = i.y - Q.y, e = Math.hypot(B, A) || 1;
  return { x: i.x + B / e * t, y: i.y + A / e * t };
}
function L(i) {
  return i.toFixed(2);
}
...
function qi(i, Q, t = 0) {
  ...
  const B = [], A = 1e-6, e = t * t;
  let P = NaN, r = NaN, s = NaN, v = NaN, c = NaN, n = NaN, T = !1;
  for (let z = 0; z + 3 < i.length; z += 4) {
    const a = i[z], o = -i[z + 1], w = i[z + 2], E = -i[z + 3];
    if (Math.abs(a - P) < A && Math.abs(o - r) < A) {
      const h = w - s, u = E - v;
      e > 0 && h * h + u * u < e ? (c = w, n = E, T = !0) : (B.push(`L${Q(w)} ${Q(E)}`), s = w, v = E, T = !1);
    } else
      T && (B.push(`L${Q(c)} ${Q(n)}`), T = !1), B.push(`M${Q(a)} ${Q(o)}L${Q(w)} ${Q(E)}`), s = w, v = E;
    P = w, r = E;
  }
  return T && B.push(`L${Q(c)} ${Q(n)}`), B.join("");
}
function _i(i, Q, t, B = 0) {
  if (i.length === 0) return "";
  const A = [], e = 1e-6, P = B * B;
  let r = NaN, s = NaN, v = NaN, c = NaN, n = NaN, T = NaN, z = !1;
  for (let a = 0; a + 5 < i.length; a += 6) {
    const o = Q(i[a], i[a + 1], i[a + 2]), w = Q(i[a + 3], i[a + 4], i[a + 5]);
    if (Math.abs(o.x - r) < e && Math.abs(o.y - s) < e) {
      const E = w.x - v, h = w.y - c;
      P > 0 && E * E + h * h < P ? (n = w.x, T = w.y, z = !0) : (A.push(`L${t(w.x)} ${t(w.y)}`), v = w.x, c = w.y, z = !1);
    } else
      z && (A.push(`L${t(n)} ${t(T)}`), z = !1), A.push(`M${t(o.x)} ${t(o.y)}L${t(w.x)} ${t(w.y)}`), v = w.x, c = w.y;
    r = w.x, s = w.y;
  }
  return z && A.push(`L${t(n)} ${t(T)}`), A.join("");
}
function iB(i) {
  let Q = 1 / 0, t = 1 / 0, B = 1 / 0, A = -1 / 0, e = -1 / 0, P = -1 / 0, r = !0, s = !1;
  for (const { verts: v, stride: c } of i)
    if (c === 4)
      for (let n = 0; n + 3 < v.length; n += 4) {
        const T = v[n], z = v[n + 1], a = v[n + 2], o = v[n + 3];
        T < Q && (Q = T), z < t && (t = z), T > A && (A = T), z > e && (e = z), a < Q && (Q = a), o < t && (t = o), a > A && (A = a), o > e && (e = o), r = !1;
      }
    else
      for (let n = 0; n + 5 < v.length; n += 6) {
        const T = v[n], z = v[n + 1], a = v[n + 2], o = v[n + 3], w = v[n + 4], E = v[n + 5];
        T < Q && (Q = T), z < t && (t = z), a < B && (B = a), T > A && (A = T), z > e && (e = z), a > P && (P = a), o < Q && (Q = o), w < t && (t = w), E < B && (B = E), o > A && (A = o), w > e && (e = w), E > P && (P = E), r = !1, s = !0;
      }
  return !r && !s && (B = 0, P = 0), { minX: Q, minY: t, maxX: A, maxY: e, minZ: B, maxZ: P, empty: r };
}
export {
  Ar as G,
  cB as V,
