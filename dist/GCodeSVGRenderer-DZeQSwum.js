...
import { GCodeVirtualizer as tA, buildWorkerToolpathStreams as TP, buildToolpathGeometryFromLinesBatched as zP, buildMovementVerticesFromLines as wP, buildWorkerSegmentGroups as EP } from "./gviewer.js";
...
const mA = {
  ...
  machineBed: { visible: !1, min: null, max: null },
...
function MP(r, Q) {
  ...
  u.rotateY(-Math.PI / 2);
  const O = new a.Mesh(u, n);
  O.position.set(r.xMax, i, s), B.add(O);
...
function pP(r, Q, B) {
  const A = new a.Group(), t = new Float32Array([
    r.x,
    r.y,
    0,
    Q.x,
    ...
    r.y,
    0
  ]), P = new a.BufferGeometry();
  P.setAttribute("position", new a.BufferAttribute(t, 3));
  const e = new a.LineBasicMaterial({
    color: new a.Color(B.render.theme.colors.machineBed),
    transparent: !0,
    opacity: 0.35,
    depthWrite: !1
  });
  return A.add(new a.LineLoop(P, e)), A;
}
function mP(r) {
  r.traverse((Q) => {
    if (Q instanceof a.LineSegments || Q instanceof a.Line) {
...
function SP(r) {
  ...
  const v = new a.Group();
  return i.length > 0 && v.add(yA(i, r.theme.colors.grid.minor, 0.25)), e.length > 0 && v.add(yA(e, r.theme.colors.grid.major, 0.4)), v;
}
function yA(r, Q, B) {
...
function VP(r) {
  ...
  const n = Math.max(2, P), T = OQ("X", r.theme.colors.axes.x, { size: 5 });
  T.position.set(B + n, 0, 0);
  const z = OQ("Y", r.theme.colors.axes.y, { size: 5 });
  z.position.set(0, A + n, 0);
  const w = OQ("Z", r.theme.colors.axes.z, { size: 5 });
...
function RP(r) {
  ...
  for (let k = P; k <= s + 1e-6; k += e) {
    ...
    if (h > B + 1e-6)
      continue;
    const U = OQ(String(k), w, E);
    U.position.set(h, z, T);
    const D = OQ(String(-k), w, E);
  ...
  for (let k = P; k <= v + 1e-6; k += e) {
    ...
    if (h > A + 1e-6)
      continue;
    const U = OQ(String(k), o, E);
    U.position.set(-z, h, T);
    const D = OQ(String(-k), o, E);
...
var sQ = {
  ...
  overwrite: !1,
  delay: 0
}, lA, Z, x, zQ = 1e8, m = 1 / zQ, iA = Math.PI * 2, XP = iA / 4, KP = 0, zt = Math.sqrt, ZP = Math.cos, WP = Math.sin, K = function(Q) {
...
}, q = Array.isArray, qP = /random\([^)]+\)/g, _P = /,\s*/g, IA = /(?:-?\.?\d|\.)+/gi, Et = /[-+=.]*\d+[.e\-+]*\d*[e\-+]*\d*/g, KQ = /[-+=.]*\d+[.e-]*\d*[a-z%]*/g, YB = /[-+=.]*\d+\.?\d*(?:e-|e\+)?\d*/gi, at = /[+-]=-?[.\d]+/, $P = /[^,'"\[\]\s]+/gi, Qe = /^[+\-=e\s\d]*\d+[.\d]*([a-z]*|%)\s*$/i, I, EQ, rA, uA, vQ = {}, HB = {}, ht, kt = function(Q) {
  return (HB = $Q(Q, vQ)) && tQ;
}, CA = function(Q, B) {
  ...
  suppressEvents: !0
}, NA = {}, HQ = [], sA = {}, Ut, PQ = {}, VB = {}, SA = 30, dB = [], OA = "", dA = function(Q) {
  var B = Q[0], A, t;
  ...
  HQ.length && !Z && jB(), Q.render(B, A, !!(Z && B < 0 && LA(Q))), HQ.length && !Z && jB();
}, Ct = function(Q) {
  var B = parseFloat(Q);
  return (B || B === 0) && (Q + "").match($P).length < 2 ? B : K(Q) ? Q.trim() : Q;
}, Nt = function(Q) {
  return Q;
}, cQ = function(Q, B) {
  for (var A in B)
  ...
  var B = Q.parent || I, A = Q.keyframes ? Pe(q(Q.keyframes)) : cQ;
  if ($(Q.inherit))
    for (; B; )
  ...
  var P = Q.ratio, e = B < 0 || !B && (!Q._start && se(Q) && !(!Q._initted && cA(Q)) || (Q._ts < 0 || Q._dp._ts < 0) && !cA(Q)) ? 0 : 1, i = Q._rDelay, s = 0, v, c, n;
  ...
  var t = Q.labels, P = Q._recent || ne, e = Q.duration() >= zQ ? P.endTime(!1) : Q._dur, i, s, v;
  ...
  return x && !B && x.selector ? x.selector(Q) : K(Q) && !A && (rA || !AB()) ? nA.call((B || uA).querySelectorAll(Q), 0) : q(Q) ? Te(Q, A) : Ht(Q) ? nA.call(Q, 0) : Q ? [Q] : [];
}, oA = function(Q) {
  ...
  return K(t) ? c = n = {
    ...
    var o = (w || B).length, E = e[o], k, h, U, D, l, N, u, O, C;
    if (!E) {
      ...
      for (E = e[o] = [], k = s ? Math.min(C, o) * c - 0.5 : t % C, h = C === zQ ? 0 : s ? o * n / C - 0.5 : t / C | 0, u = 0, O = zQ, N = 0; N < o; N++)
        U = N % C - k, D = h - (N / C | 0), E[N] = l = v ? Math.abs(v === "y" ? D : U) : zt(U * U + D * D), l > u && (u = l), l < O && (O = l);
      t === "random" && jt(E), E.max = u - O, E.min = O, E.v = o = (parseFloat(B.amount) || parseFloat(B.each) * (C > o ? o - 1 : v ? v === "y" ? o / C : C : Math.max(C, o / C)) || 0) * (t === "edges" ? -1 : 1), E.b = o < 0 ? P - o : P, E.u = W(B.amount || B.each) || 0, A = A && o < 0 ? Oe(A) : A;
    }
  ...
  return Q.replace(qP, function(B) {
    var A = B.indexOf("[") + 1, t = B.substring(A || 7, A ? B.indexOf("]") : B.length - 1).split(_P);
  ...
  if (P)
    return s = t[B + "Params"], v = t.callbackScope || Q, A && HQ.length && jB(), i && (x = i), c = s ? P.apply(v, s) : P.call(v), x = e, c;
}, sB = function(Q) {
  ...
  if (Q)
    if (Q = !Q.name && Q.default || Q, DA() || Q.headless) {
      ...
      if (AB(), Q !== t) {
        if (PQ[B])
          return;
        cQ(t, cQ(FB(Q, P), e)), $Q(t.prototype, $Q(P, FB(Q, e))), PQ[t.prop = B] = t, Q.targetTest && (dB.push(t), NA[B] = 1), B = (B === "css" ? "CSS" : B.charAt(0).toUpperCase() + B.substr(1)) + "Plugin";
      }
  ...
  if (P = P.map(function(T) {
    return (T = xt(T, B, 1)) && e + (B ? T[0] + "," + T[1] + "%," + T[2] + "%," + T[3] : T.join(",")) + ")";
...
}(), le = /hsl[a]?\(/, It = function(Q) {
  var B = Q.join(" "), A;
  if (jQ.lastIndex = 0, jQ.test(B))
    return A = le.test(B), Q[1] = XA(Q[1], A), Q[0] = XA(Q[0], A, yt(Q[1])), !0;
}, aB, eQ = function() {
  ...
  return n = {
    ...
    wake: function() {
      ht && (!rA && DA() && (EQ = rA = window, uA = EQ.document || {}, vQ.gsap = tQ, (EQ.gsapVersions || (EQ.gsapVersions = [])).push(tQ.version), kt(HB || EQ.GreenSockGlobals || !EQ.gsap && EQ || {}), pt.forEach(mt)), c = typeof requestAnimationFrame < "u" && requestAnimationFrame, s && n.sleep(), v = c || function(E) {
  ...
  return !aB && eQ.wake();
}, g = {}, Ue = /^[\d.\-M][\d.\-,\s]/, De = /["']/g, ue = function(Q) {
  for (var B = {}, A = Q.substr(1, Q.length - 3).split(":"), t = A[0], P = 1, e = A.length, i, s, v; P < e; P++)
    s = A[P], i = P !== e - 1 ? s.lastIndexOf(",") : s.length, v = s.substr(0, i), B[t] = isNaN(v) ? v.replace(De, "").trim() : +v, t = s.substr(i + 1).trim();
  return B;
}, Ce = function(Q) {
  ...
  return A && B.length > 1 && A.config ? A.config.apply(null, ~Q.indexOf("{") ? [ue(B[1])] : Ce(Q).split(",").map(Ct)) : g._CE && Ue.test(Q) ? g._CE("", Q) : A;
}, Oe = function(Q) {
  return function(B) {
    return 1 - Q(1 - B);
  };
}, VQ = function(Q, B) {
  return Q && (Y(Q) ? Q : g[Q] || Ne(Q)) || B;
}, RQ = function(Q, B, A, t) {
  ...
  var t = B >= 1 ? B : 1, P = (A || (Q ? 0.3 : 0.45)) / (B < 1 ? B : 1), e = P / iA * (Math.asin(1 / t) || 0), i = function(c) {
    return c === 1 ? 1 : t * Math.pow(2, -10 * c) * WP((c - e) * P) + 1;
...
var Yt = function(Q, B) {
  this.id = KP++, Q._gsap = this, this.target = Q, this.harness = B, this.get = B ? B.get : Dt, this.set = B ? B.getSetter : FA;
}, hB = /* @__PURE__ */ function() {
  function r(B) {
    this.vars = B, this._delay = +B.delay || 0, (this._repeat = B.repeat === 1 / 0 ? -2 : B.repeat || 0) && (this._rDelay = B.repeatDelay || 0, this._yoyo = !!B.yoyo || !!B.yoyoEase), this._ts = 1, BB(this, +B.duration, 1, 1), this.data = B.data, x && (this._ctx = x, x.data.push(this)), aB || eQ.wake();
  }
  var Q = r.prototype;
  return Q.delay = function(A) {
    ...
    return (this._tTime !== A || !this._dur && !t || this._initted && Math.abs(this._zTime) === m || !this._initted && this._dur && A || !A && !this._initted && (this.add || this._ptLookup)) && (this._ts || (this._pTime = A), ut(this, A, t)), this;
  }, Q.time = function(A, t) {
    ...
    return this._rts = +A || 0, this._ts = this._ps || A === -m ? 0 : this._rts, this.totalTime(UB(-Math.abs(this._delay), this.totalDuration(), P), t !== !1), mB(this), ie(this);
  }, Q.paused = function(A) {
    ...
    return this.play().totalTime(A ? -this._delay : 0, $(t)), this._dur || (this._zTime = -m), this;
  }, Q.play = function(A, t) {
...
var _ = /* @__PURE__ */ function(r) {
  ...
  return B.to = function(t, P, e) {
    ...
    if (this !== I && c > s && t >= 0 && (c = s), c !== this._tTime || e || n) {
      if (i !== this._time && v && (c += this._time - i, t += this._time - i), T = c, D = this._start, U = this._ts, k = !U, n && (v || (i = this._zTime), (t || !P) && (this._zTime = t)), this._repeat) {
        ...
        if (T = y(c % E), c === s ? (o = this._repeat, T = v) : (l = y(c / E), o = ~~l, o && o === l && (T = v, o--), T > v && (T = v)), l = QB(this._tTime, E), !i && this._tTime && l !== o && this._tTime - l * E - this._dur <= 0 && (l = o), N && o & 1 && (T = v - T, u = 1), o !== l && !this._lock) {
          var O = N && l & 1, C = O === (N && o & 1);
          if (o < l && (O = !O), i = O ? 0 : c % v ? v : c, this._lock = 1, this.render(i || (u ? 0 : y(o * E)), P, !v)._lock = 0, this._tTime = c, !P && this.parent && iQ(this, "onRepeat"), this.vars.repeatRefresh && !u && (this.invalidate()._lock = 1, l = o), i && i !== this._time || k !== !this._ts || this.vars.onRepeat && !this.parent && !this._act)
            return this;
          if (v = this._dur, s = this._tDur, C && (this._lock = 2, i = O ? v : -1e-4, this.render(i, !0), this.vars.repeatRefresh && !u && this.invalidate()), this._lock = 0, !this._ts && !k)
            return this;
        }
      }
      if (this._hasPause && !this._forcing && this._lock < 2 && (h = ce(this, y(i), y(T)), h && (c -= T - (T = h._start))), this._tTime = c, this._time = T, this._act = !!U, this._initted || (this._onUpdate = this.vars.onUpdate, this._initted = 1, this._zTime = t, i = 0), !i && c && v && !P && !l && (iQ(this, "onStart"), this._tTime !== c))
        return this;
      if (T >= i && t >= 0)
        for (z = this._first; z; ) {
    ...
    for (var e = [], i = wQ(t), s = this._first, v = uQ(P), c; s; )
      s instanceof J ? te(s._targets, i) && (v ? (!dQ || s._initted && s._ts) && s.globalTime(0) <= P && s.globalTime(s.totalDuration()) > P : !P || s.isActive()) && e.push(s) : (c = s.getTweensOf(i, P)).length && e.push.apply(e, c), s = s._next;
    return e;
    ...
    if (I._ts && (ut(I, GB(t, I)), Ut = eQ.frame), eQ.frame >= SA) {
      SA += sQ.autoSleep || 120;
      var P = I._first;
...
var de = function(Q, B, A, t, P, e, i) {
  ...
  Y(t) && (t = t(P || 0, Q, e));
  var n = Q[B], T = A !== "get" ? A : Y(n) ? v ? Q[B.indexOf("set") || !Y(Q["get" + B.substr(3)]) ? B : "get" + B.substr(3)](v) : Q[B]() : n, z = Y(n) ? v ? Fe : Xt : jA, w;
  if (K(t) && (~t.indexOf("random(") && (t = EB(t)), t.charAt(1) === "=" && (w = WQ(T, t) + (W(T) || 0), (w || w === 0) && (t = w))), !c || T !== t || zA)
    return !isNaN(T * t) && t !== "" ? (w = new BQ(this._pt, Q, B, +T || 0, t - (T || 0), typeof n == "boolean" ? fe : Kt, 0, z), v && (w.fp = v), i && w.modifier(i, this, Q), this._pt = w) : (!n && !(B in Q) && CA(B, t), de.call(this, Q, B, T, t, z, s || sQ.stringFilter, v));
}, Le = function(Q, B, A, t, P) {
  ...
  var i, s, v, c;
  if (PQ[Q] && (i = new PQ[Q]()).init(P, i.rawVars ? B[Q] : Le(B[Q], t, P, e, A), A, t, e) !== !1 && (A._pt = s = new BQ(A._pt, P, Q, 0, 1, i.render, i, 0, i.priority), A !== ZQ))
    for (v = A._ptLookup[A._targets.indexOf(P)], c = i._props.length; c--; )
      v[i._props[c]] = s;
  return i;
}, dQ, zA, HA = function r(Q, B, A) {
  var t = Q.vars, P = t.ease, e = t.startAt, i = t.immediateRender, s = t.lazy, v = t.onUpdate, c = t.runBackwards, n = t.yoyoEase, T = t.keyframes, z = t.autoRevert, w = Q._dur, o = Q._startAt, E = Q._targets, k = Q.parent, h = k && k.data === "nested" ? k.vars.targets : E, U = Q._overwrite === "auto" && !lA, D = Q.timeline, l = t.easeReverse || n, N, u, O, C, M, b, j, d, G, f, H, F, X;
  if (D && (!T || !P) && (P = "none"), Q._ease = VQ(P, TB.ease), Q._rEase = l && (VQ(l) || Q._ease), Q._from = !D && !!t.runBackwards, Q._from && (Q.ratio = 1), !D || T && !t.stagger) {
    if (d = E[0] ? SQ(E[0]).harness : 0, F = d && t[d.prop], N = FB(t, NA), o && (o._zTime < 0 && o.progress(1), B < 0 && c && i && !z ? o.render(-1, !0) : o.revert(c && w ? OB : Be), o._lazy = 0), e) {
      ...
      }, N), F && (O[d.prop] = F), FQ(Q._startAt = J.set(E, O)), Q._startAt._dp = 0, Q._startAt._sat = Q, B < 0 && (Z ? Q._startAt.revert(OB) : Q._startAt.render(-1, !0)), Q._zTime = B, !i)
        r(Q._startAt, m, m);
      else if (!B)
        return;
    }
    for (Q._pt = Q._ptCache = 0, s = w && $(s) || s && !w, u = 0; u < E.length; u++) {
      if (M = E[u], j = M._gsap || dA(E)[u]._gsap, Q._ptLookup[u] = f = {}, sA[j.id] && HQ.length && jB(), H = h === E ? u : h.indexOf(M), d && (G = new d()).init(M, F || N, Q, H, h) !== !1 && (Q._pt = C = new BQ(Q._pt, M, G.name, 0, 1, G.render, G, 0, G.priority), G._props.forEach(function(nQ) {
        f[nQ] = C;
      }), G.priority && (b = 1)), !d || F)
        for (O in N)
          PQ[O] && (G = Vt(O, N, Q, H, M, h)) ? G.priority && (b = 1) : f[O] = C = MA.call(Q, M, O, "get", N[O], H, h, 0, t.stringFilter);
...
var J = /* @__PURE__ */ function(r) {
  ...
  function Q(A, t, P, e) {
    ...
    var s = i.vars, v = s.duration, c = s.delay, n = s.immediateRender, T = s.stagger, z = s.overwrite, w = s.keyframes, o = s.defaults, E = s.scrollTrigger, k = t.parent || I, h = (q(A) || wt(A) ? uQ(A[0]) : "length" in t) ? [A] : wQ(A), U, D, l, N, u, O, C, M;
    if (i._targets = h.length ? dA(h) : zB("GSAP target " + A + " not found. https://gsap.com", !sQ.nullTargetWarn) || [], i._ptLookup = [], i._overwrite = z, w || T || CB(v) || CB(c)) {
      ...
      if (U = i.timeline = new _({
        ...
        if (N = h.length, C = T && Ft(T), kQ(T))
          for (u in T)
            ~Jt.indexOf(u) && (M || (M = {}), M[u] = T[u]);
        for (D = 0; D < N; D++)
          l = FB(t, Rt), l.stagger = 0, b && (l.easeReverse = b), M && $Q(l, M), O = h[D], l.duration = +oB(v, UQ(i), D, O, h), l.delay = (+oB(c, UQ(i), D, O, h) || 0) - i._delay, !T && N === 1 && l.delay && (i._delay = c = l.delay, i._start += c, l.delay = 0), U.to(O, l, C ? C(D, O, h) : 0), U._ease = g.none;
        ...
        if (q(w))
          ...
          l = {};
          for (u in w)
            u === "ease" || u === "easeEach" || je(u, w[u], l, w.easeEach);
          for (u in l)
      ...
      i.timeline = 0;
    return z === !0 && !lA && (dQ = UQ(i), I.killTweensOf(h), dQ = 0), aQ(k, UQ(i), P), t.reversed && i.reverse(), t.paused && i.paused(!0), (n || !v && !w && i._start === y(k._time) && $(n) && re(UQ(i)) && k.data !== "nested") && (i._tTime = -m, i.render(Math.max(0, -c) || 0)), E && Lt(UQ(i), E), i;
  }
  var B = Q.prototype;
  return B.render = function(t, P, e) {
    ...
    return this._initted || HA(this, v), c = this._ease(v / this._dur), Me(this, t, P, e, i, c, v, s) ? this.resetTo(t, P, e, i, 1) : (xB(this, 0), this.parent || Ot(this._dp, this, "_first", "_last", this._dp._sort ? "_start" : 0), this.render(0));
  }, B.kill = function(t, P) {
...
var jA = function(Q, B, A) {
  ...
  return Y(Q[B]) ? Xt : UA(Q[B]) && Q.setAttribute ? Ge : jA;
}, Kt = function(Q, B) {
  ...
  return Q.modifier = function(A, t, P) {
    this.mSet = this.mSet || this.set, this.set = pe, this.m = A, this.mt = P, this.tween = t;
  }, r;
}();
...
sQ.stringFilter = It;
var JQ = [], LB = {}, me = [], KA = 0, xe = 0, KB = function(Q) {
  return (LB[Q] || me).map(function(B) {
    return B();
  });
}, wA = function() {
  var Q = Date.now(), B = [];
  Q - KA > 2 && (KB("matchMediaInit"), JQ.forEach(function(A) {
  ...
  function r(B, A) {
    this.selector = A && oA(A), this.data = [], this._r = [], this.isReverted = !1, this.id = xe++, B && this.add(B);
  }
  var Q = r.prototype;
  ...
  registerEffect: function(Q) {
    ...
    (t || "").split(",").forEach(function(i) {
      ...
      return this.add(VB[B](i, kQ(s) ? s : (v = s) && {}, this), v);
    });
  },
...
var Ie = function(Q, B) {
  ...
  for (t in B)
    for (P = A.length; P--; )
      e = Q._ptLookup[P][t], e && (e = e.d) && (e._pt && (e = Ie(e, t)), e && e.modifier && e.modifier(B[t], Q, A[P], t));
}, ZB = function(Q, B) {
  return {
    name: Q,
    headless: 1,
...
}, ZB("roundProps", TA), ZB("modifiers"), ZB("snap", Gt)) || fB;
 ...
 * @author: Jack Doyle, jack@greensock.com
*/
var ZA, LQ, qQ, fA, IQ, WA, bA, Ye = function() {
  return typeof window < "u";
}, CQ = {}, yQ = 180 / Math.PI, _Q = Math.PI / 180, XQ = Math.atan2, qA = 1e8, gA = /([A-Z])/g, Ve = /(left|right|width|margin|padding|x)/i, Je = /[\s,\(]\S/, hQ = {
...
}, Qi = function(Q, B, A, t, P) {
  var e = Q._gsap;
  e.scaleX = e.scaleY = A, e.renderTransform(P, e);
}, Bi = function(Q, B, A, t, P) {
  var e = Q._gsap;
  e[B] = A, e.renderTransform(P, e);
}, S = "transform", AQ = S + "Origin", Ai = function r(Q, B) {
  ...
  Ye() && window.document && (ZA = window, LQ = ZA.document, qQ = LQ.documentElement, IQ = aA("div") || {
    style: {}
  ...
  var P = parseFloat(A) || 0, e = (A + "").trim().substr((P + "").length) || "px", i = IQ.style, s = Ve.test(B), v = Q.tagName.toLowerCase() === "svg", c = (v ? "client" : "offset") + (s ? "Width" : "Height"), n = 100, T = t === "px", z = t === "%", w, o, E, k;
  if (t === e || !P || Bt[t] || Bt[e])
    return P;
  ...
  if (z && (B === "height" || B === "width")) {
    ...
    (z || e === "%") && !Pi[rQ(o, "display")] && (i.position = rQ(Q, "position")), o === Q && (i.position = "static"), o.appendChild(IQ), w = IQ[c], o.removeChild(IQ), i.position = "absolute";
  return s && z && (E = SQ(o), E.time = eQ.time, E.width = o[c]), V(T ? w * P / n : w && P ? n / w * P : 0);
}, DQ = function(Q, B, A, t) {
  var P;
  return fA || hA(), B in hQ && B !== "transform" && (B = hQ[B], ~B.indexOf(",") && (B = B.split(",")[0])), CQ[B] && B !== "transform" ? (P = lB(Q, t), P = B !== "transformOrigin" ? P[B] : P.svg ? P.origin : gB(rQ(Q, AQ)) + " " + P.zOrigin + "px") : (P = Q.style[B], (!P || P === "auto" || t || ~(P + "").indexOf("calc(")) && (P = bB[B] && bB[B](Q, B, A) || rQ(Q, B) || Dt(Q, B) || (B === "opacity" ? 1 : 0))), A && !~(P + "").trim().indexOf(" ") ? fQ(Q, B, P, A) + A : P;
}, ei = function(Q, B, A, t) {
  if (!A || A === "none") {
  ...
  clearProps: function(Q, B, A, t, P) {
    if (P.data !== "isFromStart") {
      var e = Q._pt = new BQ(Q._pt, B, A, 0, 0, ri);
  ...
  */
  ...
  var i = Q._gsap, s = P || pA(Q, !0), v = i.xOrigin || 0, c = i.yOrigin || 0, n = i.xOffset || 0, T = i.yOffset || 0, z = s[0], w = s[1], o = s[2], E = s[3], k = s[4], h = s[5], U = B.split(" "), D = parseFloat(U[0]) || 0, l = parseFloat(U[1]) || 0, N, u, O, C;
  A ? s !== kB && (u = z * E - w * o) && (O = D * (E / u) + l * (-o / u) + (o * h - E * k) / u, C = D * (-w / u) + l * (z / u) - (z * h - w * k) / u, D = O, l = C) : (N = tP(Q), D = N.x + (~U[0].indexOf("%") ? D / 100 * N.width : D), l = N.y + (~(U[1] || U[0]).indexOf("%") ? l / 100 * N.height : l)), t || t !== !1 && i.smooth ? (k = D - v, h = l - c, i.xOffset = n + (k * z + h * o) - k, i.yOffset = T + (k * w + h * E) - h) : i.xOffset = i.yOffset = 0, i.xOrigin = D, i.yOrigin = l, i.smooth = !!t, i.origin = B, i.originIsAbsolute = !!A, Q.style[AQ] = "0px 0px", e && (MQ(e, i, "xOrigin", v, D), MQ(e, i, "yOrigin", c, l), MQ(e, i, "xOffset", n, i.xOffset), MQ(e, i, "yOffset", T, i.yOffset)), Q.setAttribute("data-svg-origin", D + " " + l);
}, lB = function(Q, B) {
  var A = Q._gsap || new Yt(Q);
  if ("x" in A && !B && !A.uncache)
    return A;
  var t = Q.style, P = A.scaleX < 0, e = "px", i = "deg", s = getComputedStyle(Q), v = rQ(Q, AQ) || "0", c, n, T, z, w, o, E, k, h, U, D, l, N, u, O, C, M, b, j, d, G, f, H, F, X, nQ, NQ, gQ, oQ, R, lQ, pQ;
  return c = n = T = o = E = k = h = U = D = 0, z = w = 1, A.svg = !!(Q.getCTM && PP(Q)), s.translate && ((s.translate !== "none" || s.scale !== "none" || s.rotate !== "none") && (t[S] = (s.translate !== "none" ? "translate3d(" + (s.translate + " 0 0").split(" ").slice(0, 3).join(", ") + ") " : "") + (s.rotate !== "none" ? "rotate(" + s.rotate + ") " : "") + (s.scale !== "none" ? "scale(" + s.scale.split(" ").join(",") + ") " : "") + (s[S] !== "none" ? s[S] : "")), t.scale = t.rotate = t.translate = "none"), u = pA(Q, A.svg), A.svg && (A.uncache ? (X = Q.getBBox(), v = A.xOrigin - X.x + "px " + (A.yOrigin - X.y) + "px", F = "") : F = !B && Q.getAttribute("data-svg-origin"), kA(Q, F || v, !!F || A.originIsAbsolute, A.smooth !== !1, u)), l = A.xOrigin || 0, N = A.yOrigin || 0, u !== kB && (b = u[0], j = u[1], d = u[2], G = u[3], c = f = u[4], n = H = u[5], u.length === 6 ? (z = Math.sqrt(b * b + j * j), w = Math.sqrt(G * G + d * d), o = b || j ? XQ(j, b) * yQ : 0, h = d || G ? XQ(d, G) * yQ + o : 0, h && (w *= Math.abs(Math.cos(h * _Q))), A.svg && (c -= l - (l * b + N * d), n -= N - (l * j + N * G))) : (pQ = u[6], R = u[7], NQ = u[8], gQ = u[9], oQ = u[10], lQ = u[11], c = u[12], n = u[13], T = u[14], O = XQ(pQ, oQ), E = O * yQ, O && (C = Math.cos(-O), M = Math.sin(-O), F = f * C + NQ * M, X = H * C + gQ * M, nQ = pQ * C + oQ * M, NQ = f * -M + NQ * C, gQ = H * -M + gQ * C, oQ = pQ * -M + oQ * C, lQ = R * -M + lQ * C, f = F, H = X, pQ = nQ), O = XQ(-d, oQ), k = O * yQ, O && (C = Math.cos(-O), M = Math.sin(-O), F = b * C - NQ * M, X = j * C - gQ * M, nQ = d * C - oQ * M, lQ = G * M + lQ * C, b = F, j = X, d = nQ), O = XQ(j, b), o = O * yQ, O && (C = Math.cos(O), M = Math.sin(O), F = b * C + j * M, X = f * C + H * M, j = j * C - b * M, H = H * C - f * M, b = F, f = X), E && Math.abs(E) + Math.abs(o) > 359.9 && (E = o = 0, k = 180 - k), z = V(Math.sqrt(b * b + j * j + d * d)), w = V(Math.sqrt(H * H + pQ * pQ)), O = XQ(f, H), h = Math.abs(O) > 2e-4 ? O * yQ : 0, D = lQ ? 1 / (lQ < 0 ? -lQ : lQ) : 0), A.svg && (F = Q.getAttribute("transform"), A.forceCSS = Q.setAttribute("transform", "") || !iP(rQ(Q, S)), F && Q.setAttribute("transform", F))), Math.abs(h) > 90 && Math.abs(h) < 270 && (P ? (z *= -1, h += o <= 0 ? 180 : -180, o += o <= 0 ? 180 : -180) : (w *= -1, h += h <= 0 ? 180 : -180)), B = B || A.uncache, A.x = c - ((A.xPercent = c && (!B && A.xPercent || (Math.round(Q.offsetWidth / 2) === Math.round(-c) ? -50 : 0))) ? Q.offsetWidth * A.xPercent / 100 : 0) + e, A.y = n - ((A.yPercent = n && (!B && A.yPercent || (Math.round(Q.offsetHeight / 2) === Math.round(-n) ? -50 : 0))) ? Q.offsetHeight * A.yPercent / 100 : 0) + e, A.z = T + e, A.scaleX = V(z), A.scaleY = V(w), A.rotation = V(o) + i, A.rotationX = V(E) + i, A.rotationY = V(k) + i, A.skewX = h + i, A.skewY = U + i, A.transformPerspective = D + e, (A.zOrigin = parseFloat(v.split(" ")[2]) || !B && A.zOrigin || 0) && (t[AQ] = gB(v)), A.xOffset = A.yOffset = 0, A.force3D = sQ.force3D, A.renderTransform = A.svg ? vi : AP ? rP : si, A.uncache = 0, A;
}, gB = function(Q) {
...
}, mQ = "0deg", PB = "0px", xQ = ") ", rP = function(Q, B) {
  ...
  if (U && (n !== mQ || c !== mQ)) {
    var N = parseFloat(c) * _Q, u = Math.sin(N), O = Math.cos(N), C;
    N = parseFloat(n) * _Q, C = Math.cos(N), e = WB(h, e, u * C * -U), i = WB(h, i, -Math.sin(N) * -U), s = WB(h, s, O * C * -U + U);
  }
  ...
  var A = B || this, t = A.xPercent, P = A.yPercent, e = A.x, i = A.y, s = A.rotation, v = A.skewX, c = A.skewY, n = A.scaleX, T = A.scaleY, z = A.target, w = A.xOrigin, o = A.yOrigin, E = A.xOffset, k = A.yOffset, h = A.forceCSS, U = parseFloat(e), D = parseFloat(i), l, N, u, O, C;
  s = parseFloat(s), v = parseFloat(v), c = parseFloat(c), c && (c = parseFloat(c), v += c, s += c), s || v ? (s *= _Q, v *= _Q, l = Math.cos(s) * n, N = Math.sin(s) * n, u = Math.sin(s - v) * -T, O = Math.cos(s - v) * T, v && (c *= _Q, C = Math.tan(v - c), C = Math.sqrt(1 + C * C), u *= C, O *= C, c && (C = Math.tan(c), C = Math.sqrt(1 + C * C), l *= C, N *= C)), l = V(l), N = V(N), u = V(u), O = V(O)) : (l = n, O = T, N = u = 0), (U && !~(e + "").indexOf("px") || D && !~(i + "").indexOf("px")) && (U = fQ(z, "x", e, "px"), D = fQ(z, "y", i, "px")), (w || o || E || k) && (U = V(U + w - (w * l + o * u) + E), D = V(D + o - (w * N + o * O) + k)), (t || P) && (C = z.getBBox(), U = V(U + t / 100 * C.width), D = V(D + P / 100 * C.height)), C = "matrix(" + l + "," + N + "," + u + "," + O + "," + U + "," + D + ")", z.setAttribute("transform", C), h && (z.style[S] = C);
}, ci = function(Q, B, A, t, P) {
  var e = 360, i = K(P), s = parseFloat(P) * (i && ~P.indexOf("rad") ? yQ : 1), v = s - t, c = t + v + "deg", n, T;
  return i && (n = P.split("_")[1], n === "short" && (v %= e, v !== v % (e / 2) && (v += v < 0 ? e : -e)), n === "cw" && v < 0 ? v = (v + e * qA) % e - ~~(v / e) * e : n === "ccw" && v > 0 && (v = (v - e * qA) % e - ~~(v / e) * e)), Q._pt = T = new BQ(Q._pt, B, A, t, v, Re), T.e = c, T.u = "deg", Q._props.push(A), T;
}, Pt = function(Q, B) {
  for (var A in B)
    Q[A] = B[A];
  return Q;
...
var sP = {
  ...
  init: function(Q, B, A, t, P) {
    var e = this._props, i = Q.style, s = A.vars.startAt, v, c, n, T, z, w, o, E, k, h, U, D, l, N, u, O, C;
    fA || hA(), this.styles = this.styles || BP(Q), O = this.styles.props, this.tween = A;
    for (o in B)
      if (o !== "autoRound" && (c = B[o], !(PQ[o] && Vt(o, B, A, t, Q, P)))) {
        if (z = typeof c, w = bB[o], z === "function" && (c = c.call(A, t, Q, P), z = typeof c), z === "string" && ~c.indexOf("random(") && (c = EB(c)), w)
          ...
          v = (getComputedStyle(Q).getPropertyValue(o) + "").trim(), c += "", jQ.lastIndex = 0, jQ.test(v) || (E = W(v), k = W(c), k ? E !== k && (v = fQ(Q, o, v, k) + k) : E && (c += E)), this.add(i, "setProperty", v, c, t, P, 0, 0, o), e.push(o), O.push(o, 0, i[o]);
        else if (z !== "undefined") {
          if (s && o in s ? (v = typeof s[o] == "function" ? s[o].call(A, t, Q, P) : s[o], K(v) && ~v.indexOf("random(") && (v = EB(v)), W(v + "") || v === "auto" || (v += sQ.units[o] || W(DQ(Q, o)) || ""), (v + "").charAt(1) === "=" && (v = DQ(Q, o))) : v = DQ(Q, o), T = parseFloat(v), h = z === "string" && c.charAt(1) === "=" && c.substr(0, 2), h && (c = c.substr(2)), n = parseFloat(c), o in hQ && (o === "autoAlpha" && (T === 1 && DQ(Q, "visibility") === "hidden" && n && (T = 0), O.push("visibility", 0, i.visibility), MQ(this, i, "visibility", T ? "inherit" : "hidden", n ? "inherit" : "hidden", !n)), o !== "scale" && o !== "transform" && (o = hQ[o], ~o.indexOf(",") && (o = o.split(",")[0]))), U = o in CQ, U) {
            ...
            if (D || (l = Q._gsap, l.renderTransform && !B.parseTransform || lB(Q, B.parseTransform), N = B.smoothOrigin !== !1 && l.smooth, D = this._pt = new BQ(this._pt, i, S, 0, 1, l.renderTransform, l, 0, -1), D.dep = 1), o === "scale")
              ...
              O.push(AQ, 0, i[AQ]), c = ii(c), l.svg ? kA(Q, c, 0, N, 0, this) : (k = parseFloat(c.split(" ")[2]) || 0, k !== l.zOrigin && MQ(this, l, "zOrigin", l.zOrigin, k), MQ(this, i, o, gB(v), gB(c)));
              continue;
            } else if (o === "svgOrigin") {
              kA(Q, c, 1, N, 0, this);
              continue;
            } else if (o in eP) {
              ci(this, l, o, T, h ? WQ(T, h + c) : c);
              continue;
          ...
          if (U || (n || n === 0) && (T || T === 0) && !Je.test(c) && o in i)
            E = (v + "").substr((T + "").length), n || (n = 0), k = W(c) || (o in sQ.units ? sQ.units[o] : E), E !== k && (T = fQ(Q, o, v, k)), this._pt = new BQ(this._pt, U ? l : i, o, T, (h ? WQ(T, h + n) : n) - T, !U && (k === "px" || o === "zIndex") && B.autoRound !== !1 ? Ze : EA), this._pt.u = k || 0, U && C !== c ? (this._pt.b = v, this._pt.e = C, this._pt.r = Ke) : E !== k && k !== "%" && (this._pt.b = v, this._pt.r = Xe);
          else if (o in i)
            ei.call(this, Q, o, v, h ? h + c : c);
          else if (o in Q)
          ...
          U || (o in i ? O.push(o, 0, i[o]) : typeof Q[o] == "function" ? O.push(o, 2, Q[o]()) : O.push(o, 1, v || Q[o])), e.push(o);
        }
      }
    u && Wt(this);
  },
  ...
  getSetter: function(Q, B, A) {
    var t = hQ[B];
    return t && t.indexOf(",") < 0 && (B = t), B in CQ && B !== AQ && (Q._gsap.x || DQ(Q, "x")) ? A && WA === A ? B === "scale" ? $e : _e : (WA = A || {}) && (B === "scale" ? Qi : Bi) : Q.style && !UA(Q.style[B]) ? We : ~B.indexOf("-") ? qe : FA(Q, B);
  },
  core: {
    _removeProperty: GQ,
    _getMatrix: pA
...
function cP(r) {
  const Q = atob(oi), B = new Uint8Array(Q.length);
  ...
  return t.translate(
    ...
    -(s.min.y + s.max.y) / 2,
    -s.min.z
  ), ai(t, 6), t.computeVertexNormals(), t;
}
let qB = null;
function hi() {
  if (qB) return qB;
...
function li(r) {
  ...
  return r.bit.type === "triangle" ? wi(Q) : r.bit.type === "drill" ? cP(Q) : Ei(Q);
}
function nP(r) {
  return r.bit.colorSource === "custom" ? r.bit.color : Ti;
}
function Ui(r) {
  const Q = DB(r.bit.opacity), B = cP(Math.max(1e-3, r.bit.size)), A = new a.Color(nP(r)), t = hi(), P = new a.MeshStandardMaterial({
    color: A,
    metalness: 0.8,
    roughness: 0.35,
    roughnessMap: t,
...
function Di(r) {
  const Q = DB(r.bit.opacity), B = li(r), A = new a.MeshBasicMaterial({
...
function rt(r) {
  return r.bit.type === "laser" ? ki(r.bit.size, r.bit.opacity) : r.bit.type === "drill" ? Ui(r) : Di(r);
}
function st(r) {
...
function ui(r) {
  ...
  const v = (n) => {
    ...
    if (E || k)
      ...
      const D = A, [l, N] = D.children, u = l.material, O = N.material;
      u.opacity = h, u.needsUpdate = !0, O.opacity = h * 0.3, O.needsUpdate = !0;
    } else {
      const l = A.material, N = new a.Color(T === "drill" ? nP(n) : vP);
  ...
  return {
    ...
    update: (n) => {
      if (!t) return;
      const T = (n - t.startedAt) / t.duration, z = zi(T);
      Q.position.set(
...
class Fi {
  constructor(Q) {
    this.viewCube = null, this.resizeObserver = null, this.onWindowResize = null, this.animationFrameId = null, this.gridGroup = null, this.axesGroup = null, this.gridLabelsGroup = null, this.boundingBoxGroup = null, this.machineBedGroup = null, this.bitMarker = null, this.preLaserBitType = "drill", this.toolpathStreams = [], this.toolpathCutBucketCount = 1, this.toolpathRotationA = 0, this.lastBitPosition = { x: 0, y: 0, z: 0, a: 0 }, this.sim3dHandle = null, this.currentLines = [], this.linePositions = null, this.renderSequence = 0, this.currentBounds = null, this.cameraFocusTransition = null, this.id = Q.id, this.container = Q.container, this.callbacks = Q.callbacks ?? {}, this.options = ct(NP, Q.options), this.canvas = document.createElement("canvas"), this.canvas.style.width = "100%", this.canvas.style.height = "100%", this.canvas.style.display = "block", this.container.appendChild(this.canvas), xP(this.container), this.renderer = new a.WebGLRenderer({
      canvas: this.canvas,
  ...
  setOptions(Q) {
    ...
    (A || t) && this.renderGridAndAxes(), (B.grid.labels !== this.options.grid.labels || B.units !== this.options.units || B.grid.sizeX !== this.options.grid.sizeX || B.grid.sizeY !== this.options.grid.sizeY || t) && this.refreshGridLabels(), (B.boundingBox.visible !== this.options.boundingBox.visible || B.boundingBox.labels !== this.options.boundingBox.labels || B.units !== this.options.units || B.render.theme.colors.boundingBox !== this.options.render.theme.colors.boundingBox) && this.refreshBoundingBox(), (B.machineBed.visible !== this.options.machineBed.visible || ((n = B.machineBed.min) == null ? void 0 : n.x) !== ((T = this.options.machineBed.min) == null ? void 0 : T.x) || ((z = B.machineBed.min) == null ? void 0 : z.y) !== ((w = this.options.machineBed.min) == null ? void 0 : w.y) || ((o = B.machineBed.max) == null ? void 0 : o.x) !== ((E = this.options.machineBed.max) == null ? void 0 : E.x) || ((k = B.machineBed.max) == null ? void 0 : k.y) !== ((h = this.options.machineBed.max) == null ? void 0 : h.y) || B.render.theme.colors.machineBed !== this.options.render.theme.colors.machineBed) && this.refreshMachineBed();
    ...
    if (B.sim3d.erosionPasses !== this.options.sim3d.erosionPasses && this.sim3dHandle) {
      const l = this.sim3dHandle.currentLine, N = yB(l, this.sim3dHandle.data);
      IB(N, this.sim3dHandle.data.resolution, this.options.sim3d.erosionPasses, this.sim3dHandle.data.slabBounds.zTop), SB(this.sim3dHandle.slab, N);
    }
  ...
  worldSizes() {
    const Q = this.options.units === "in" ? Ci : 1;
    return {
  ...
  renderGridAndAxes() {
    ...
    this.setGridGroup(
      ...
      VP({ sizeXWorld: Q, sizeYWorld: B, depthWorld: A, theme: this.options.render.theme })
    );
  }
  refreshGridLabels() {
    ...
    this.setGridLabelsGroup(
      RP({ sizeXMm: Q, sizeYMm: B, units: this.options.units, theme: this.options.render.theme })
    );
  }
  refreshBoundingBox() {
  ...
  refreshMachineBed() {
    const { visible: Q, min: B, max: A } = this.options.machineBed;
    if (!Q || !B || !A) {
      this.setMachineBedGroup(null);
      return;
    }
    this.setMachineBedGroup(pP(B, A, this.options));
  }
  setBitMarker(Q) {
  ...
  setAxesGroup(Q) {
    this.axesGroup && (this.scene.remove(this.axesGroup), JP(this.axesGroup), this.axesGroup = null), Q && (this.axesGroup = Q, this.scene.add(Q));
  }
  ...
  setMachineBedGroup(Q) {
    this.machineBedGroup && (this.scene.remove(this.machineBedGroup), mP(this.machineBedGroup), this.machineBedGroup = null), Q && (this.machineBedGroup = Q, this.scene.add(Q));
  }
  setGeometryEmpty() {
  ...
  startCameraFocus(Q) {
    ...
    Q.getSize(A);
    const t = Math.max(A.x, A.y, 1) / 2, P = a.MathUtils.degToRad(this.camera.fov), e = t / Math.tan(P / 2) * 1.25 + A.z, i = xA("front-top-left"), s = B.clone().add(i.multiplyScalar(e)), v = Math.max(A.x, A.y, A.z, 1);
  ...
  updateCameraFocusTransition() {
    if (!this.cameraFocusTransition)
      return;
    const B = performance.now() - this.cameraFocusTransition.startedAt, A = Math.min(1, Math.max(0, B / this.cameraFocusTransition.duration)), t = yP(A);
  ...
  updateViewCubeRotation() {
    ...
    const A = IP(this.camera.position, this.controls.target);
...
function ct(r, Q) {
  ...
  if (!Q)
    return { ...r };
  const B = Ni(r.render.theme, (A = Q.render) == null ? void 0 : A.theme);
  return {
    ...r,
    ...Q,
    mode: { ...r.mode, ...Q.mode },
...
class Gi {
  ...
  rebuildAndRender(Q = !1) {
    const B = Q ? di : L, A = String(this.options.strokeWidth);
    ...
    for (let t = 0; t < this.segmentGroups.length; t++) {
      ...
      s.setAttribute("stroke", P), s.setAttribute("stroke-opacity", String(e)), s.setAttribute("stroke-width", A), s.setAttribute("stroke-linecap", "round"), s.setAttribute("stroke-linejoin", "round"), s.setAttribute("d", Li(i, this.project, B));
    }
  ...
  renderBbox() {
    ...
    QA(this.bboxLabelX, rB(s, v), nt(rB(s, v), rB(c, n), l), D, `X: ${BA(B - Q)}`);
    const N = [s, v, c, n, T, z, w, o], u = Math.min(...N.map((d) => d.y)), O = Math.max(...N.map((d) => d.y)), C = Math.min(...N.map((d) => d.x)), M = Math.max(...N.map((d) => d.x)), b = { x: M + l, y: (u + O) / 2 };
