import * as a from "three";
import { OrbitControls as kP } from "three/examples/jsm/controls/OrbitControls.js";
import { GCodeVirtualizer as PA, buildWorkerToolpathStreams as UP, buildToolpathGeometryFromLinesBatched as DP, buildMovementVerticesFromLines as uP, buildWorkerSegmentGroups as dP } from "./gviewer.js";
import { STLLoader as CP } from "three/examples/jsm/loaders/STLLoader.js";
function NP(r) {
  const Q = [];
  let B = null;
  for (const A of r.specs) {
    if (A.positions.length === 0 || A.opacity <= 0)
      continue;
    const t = A.positions.length / 3, P = A.colors ? A.colors.slice() : ht(A.kind, t, r.options), e = new Float32Array(P.length);
    e.set(P);
    const i = new a.BufferGeometry();
    i.setAttribute("position", new a.BufferAttribute(A.positions, 3));
    const s = new a.BufferAttribute(e, 3);
    s.setUsage(a.DynamicDrawUsage), i.setAttribute("color", s), i.computeBoundingBox();
    const v = new a.LineBasicMaterial({
      vertexColors: !0,
      transparent: !0,
      opacity: A.opacity
    }), c = new a.LineSegments(i, v);
    r.scene.add(c), Q.push({
      line: c,
      baseColors: P,
      simColors: e,
      prefixEndVertex: A.prefixEndVertex,
      totalVertices: t,
      greyCursorVertex: 0,
      kind: A.kind,
      cutBucketIndex: A.cutBucketIndex,
      workerColors: A.colors ? P : void 0
    }), i.boundingBox && (B = B ? B.union(i.boundingBox) : i.boundingBox.clone());
  }
  return { streams: Q, bounds: B };
}
function OP(r, Q) {
  for (const B of Q)
    r.remove(B.line), B.line.geometry.dispose(), B.line.material.dispose();
}
function LP(r, Q) {
  for (const B of r) {
    if (B.workerColors) continue;
    const A = ht(B.kind, B.totalVertices, Q);
    B.baseColors = A, B.simColors.set(A), B.greyCursorVertex = 0;
    const t = B.line.geometry.getAttribute("color");
    t.clearUpdateRanges(), t.addUpdateRange(0, B.simColors.length), t.needsUpdate = !0;
  }
}
function MP(r) {
  const Q = eA(r.options.render.theme.rapidOpacity ?? 0.3);
  for (const B of r.streams) {
    const A = B.line.material;
    if (B.kind === "rapid") {
      A.opacity = Q;
      continue;
    }
    A.opacity = FP({
      bucketIndex: B.cutBucketIndex ?? 0,
      bucketCount: r.cutBucketCount,
      baseOpacity: r.options.render.theme.opacity
    });
  }
}
function HP(r) {
  const Q = r.stream.totalVertices, B = Math.max(0, Math.min(Q, Math.floor(r.nextCursorVertex))), A = Math.max(0, Math.min(Q, Math.floor(r.stream.greyCursorVertex)));
  if (B === A)
    return;
  const t = r.stream.line.geometry.getAttribute("color"), P = jP(r.options), e = Math.min(A, B), i = Math.max(A, B);
  if (B > A)
    for (let s = A; s < B; s += 1) {
      const v = s * 3;
      r.stream.simColors[v] = P.r, r.stream.simColors[v + 1] = P.g, r.stream.simColors[v + 2] = P.b;
    }
  else {
    const s = B * 3, v = A * 3;
    r.stream.simColors.set(r.stream.baseColors.subarray(s, v), s);
  }
  t.clearUpdateRanges(), t.addUpdateRange(e * 3, (i - e) * 3), t.needsUpdate = !0, r.stream.greyCursorVertex = B;
}
function ht(r, Q, B) {
  const A = B.render.theme.colors, t = B.mode.laser ? A.laser ?? A.cutting : A.cutting, P = new a.Color(r === "rapid" ? A.rapid : t), e = new Float32Array(Q * 3);
  for (let i = 0; i < Q; i += 1) {
    const s = i * 3;
    e[s] = P.r, e[s + 1] = P.g, e[s + 2] = P.b;
  }
  return e;
}
function jP(r) {
  const Q = r.render.theme.colors, B = new a.Color(r.render.theme.background), A = new a.Color(Q.processed ?? Q.cutting), t = B.clone().lerp(A, 0.65);
  return { r: t.r, g: t.g, b: t.b };
}
function FP(r) {
  if (r.bucketCount <= 1)
    return eA(r.baseOpacity);
  const Q = r.bucketIndex / (r.bucketCount - 1);
  return eA(r.baseOpacity * Q);
}
function eA(r) {
  return Number.isFinite(r) ? Math.max(0, Math.min(1, r)) : 0;
}
const VA = {
  background: "#111827",
  opacity: 0.9,
  rapidOpacity: 0.3,
  colors: {
    rapid: "#0ef6ae",
    cutting: "#3e85c7",
    laser: "#a855f7",
    processed: "#6b7280",
    boundingBox: "#e2e8f0",
    machineBed: "#fbbf24",
    machineBedKeepout: "#df3b3b",
    grid: { major: "#2f3840", minor: "#1f252b" },
    axes: { x: "#df3b3b", y: "#06b881", z: "#295d8d" }
  }
}, GP = {
  units: "mm",
  mode: { laser: !1, sim3d: !1 },
  sim3d: { toolDiameter: 6.35, resolution: 256, showToolpath: !1, erosionPasses: 2 },
  bit: {
    enabled: !0,
    type: "drill",
    size: 4.05,
    opacity: 0.9,
    tweenMs: 140,
    colorSource: "cutting",
    color: VA.colors.cutting,
    spinRpm: 300
  },
  progress: { mode: "grey" },
  grid: { sizeX: 1e3, sizeY: 1e3, axisDepth: 200, labels: !0, bounds: null },
  boundingBox: { visible: !1, labels: !1 },
  machineBed: { visible: !1, min: null, max: null, keepout: null },
  geometry: { arcSegments: 30, batching: { progressEveryLines: 5e3, yieldEveryLines: 5e4 } },
  render: { antialias: !0, theme: VA },
  camera: {
    fov: 45,
    focusDurationMs: 900,
    orbit: { enableDamping: !0 },
    initialPosition: { x: 0, y: -200, z: 200 }
  }
};
async function fP(r, Q, B, A) {
  var M, b, f;
  const t = Math.max(2, Math.floor(B)), P = (A == null ? void 0 : A.arcSegments) ?? 30, e = (A == null ? void 0 : A.checkpointEveryLines) ?? 2e3, i = ((M = A == null ? void 0 : A.batch) == null ? void 0 : M.yieldEveryLines) ?? 1e4, s = ((b = A == null ? void 0 : A.batch) == null ? void 0 : b.shouldAbort) ?? (() => !1), v = (f = A == null ? void 0 : A.batch) == null ? void 0 : f.onProgress;
  if (r.length === 0) {
    const C = {
      xMin: -1,
      xMax: 1,
      yMin: -1,
      yMax: 1,
      zTop: 1,
      zBot: 0
    }, j = new Float32Array(t * t).fill(C.zTop), F = /* @__PURE__ */ new Map();
    return F.set(0, new Float32Array(j)), { moves: [], checkpoints: F, slabBounds: C, resolution: t, toolRadius: Q };
  }
  let c = 1 / 0, n = -1 / 0, T = 1 / 0, z = -1 / 0, w = 1 / 0, o = -1 / 0;
  const E = new PA({
    onLinearMove(C) {
      if (C.modals.motion === "G0") return;
      const j = C.transformedStart ?? C.start, F = C.transformedEnd ?? C.end;
      for (const H of [j, F])
        H.X < c && (c = H.X), H.X > n && (n = H.X), H.Y < T && (T = H.Y), H.Y > z && (z = H.Y), H.Z < w && (w = H.Z), H.Z > o && (o = H.Z);
    },
    onArcMove(C) {
      const j = C.transformedStart ?? C.start, F = C.transformedEnd ?? C.end, H = C.transformedMax ?? C.max;
      for (const G of [j, F, H])
        G.X < c && (c = G.X), G.X > n && (n = G.X), G.Y < T && (T = G.Y), G.Y > z && (z = G.Y), G.Z < w && (w = G.Z), G.Z > o && (o = G.Z);
    }
  });
  for (const C of r)
    E.processLine(C);
  isFinite(c) || (c = -1, n = 1, T = -1, z = 1, w = 0, o = 0), c === n && (c -= Q * 2, n += Q * 2), T === z && (T -= Q * 2, z += Q * 2), c -= Q, n += Q, T -= Q, z += Q;
  const h = o;
  let l = w - Q;
  l >= h && (l = h - 1);
  const U = { xMin: c, xMax: n, yMin: T, yMax: z, zTop: h, zBot: l }, D = new Float32Array(t * t).fill(h), k = /* @__PURE__ */ new Map();
  k.set(0, new Float32Array(D));
  const O = [];
  let u = 0, N = 0;
  const d = new PA({
    onLinearMove(C) {
      const j = C.transformedStart ?? C.start, F = C.transformedEnd ?? C.end, H = {
        lineIndex: N,
        x0: j.X,
        y0: j.Y,
        z0: j.Z,
        x1: F.X,
        y1: F.Y,
        z1: F.Z
      };
      O.push(H), iA(H, D, U, Q, t);
    },
    onArcMove(C) {
      const j = C.transformedStart ?? C.start, F = C.transformedEnd ?? C.end, H = C.transformedCenter ?? C.center, G = bP(j, F, H, C.plane, C.motion, P);
      for (const I of G) {
        const R = { lineIndex: N, ...I };
        O.push(R), iA(R, D, U, Q, t);
      }
    }
  });
  for (let C = 0; C < r.length && !s(); C++) {
    N = C;
    const j = r[C];
    j !== void 0 && d.processLine(j), C - u >= e && (k.set(C, new Float32Array(D)), u = C), C % i === 0 && C > 0 && (v == null || v(C, r.length), await new Promise((F) => setTimeout(F, 0)));
  }
  return v == null || v(r.length, r.length), { moves: O, checkpoints: k, slabBounds: U, resolution: t, toolRadius: Q };
}
function iA(r, Q, B, A, t) {
  const P = (B.xMax - B.xMin) / t, e = (B.yMax - B.yMin) / t, i = Math.hypot(r.x1 - r.x0, r.y1 - r.y0), s = Math.max(1e-3, Math.min(P, e) * 0.5), v = Math.max(1, Math.ceil(i / s)), c = Math.ceil(A / P), n = Math.ceil(A / e), T = A * A;
  for (let z = 0; z <= v; z++) {
    const w = z / v, o = r.x0 + (r.x1 - r.x0) * w, E = r.y0 + (r.y1 - r.y0) * w, h = r.z0 + (r.z1 - r.z0) * w, l = Math.floor((o - B.xMin) / P), U = Math.floor((E - B.yMin) / e);
    for (let D = -c; D <= c; D++) {
      const k = l + D;
      if (k < 0 || k >= t) continue;
      const O = B.xMin + (k + 0.5) * P, u = (O - o) * (O - o);
      if (!(u > T))
        for (let N = -n; N <= n; N++) {
          const d = U + N;
          if (d < 0 || d >= t) continue;
          const M = B.yMin + (d + 0.5) * e;
          if (u + (M - E) * (M - E) > T) continue;
          const f = d * t + k;
          Q[f] > h && (Q[f] = h);
        }
    }
  }
}
function SB(r, Q) {
  let B = 0;
  for (const P of Q.checkpoints.keys())
    P <= r && P > B && (B = P);
  const A = Q.checkpoints.get(B), t = A ? new Float32Array(A) : new Float32Array(Q.resolution * Q.resolution).fill(Q.slabBounds.zTop);
  for (const P of Q.moves)
    P.lineIndex > B && P.lineIndex <= r && iA(P, t, Q.slabBounds, Q.toolRadius, Q.resolution);
  return t;
}
function YB(r, Q, B, A) {
  if (B <= 0) return;
  const t = 1e-3, P = Q * Q, e = new Float32Array(P);
  for (let i = 0; i < B; i++) {
    e.set(r);
    for (let s = 0; s < Q; s++)
      for (let v = 0; v < Q; v++) {
        if (e[s * Q + v] < A - t) continue;
        let n = A;
        for (let T = -1; T <= 1; T++) {
          const z = s + T;
          if (!(z < 0 || z >= Q))
            for (let w = -1; w <= 1; w++) {
              if (w === 0 && T === 0) continue;
              const o = v + w;
              if (o < 0 || o >= Q) continue;
              const E = e[z * Q + o];
              E < n && (n = E);
            }
        }
        n < A - t && (r[s * Q + v] = n);
      }
  }
}
function bP(r, Q, B, A, t, P) {
  const e = Math.max(1, P);
  let i, s, v;
  A === "G18" ? (i = "Z", s = "X", v = "Y") : A === "G19" ? (i = "Y", s = "Z", v = "X") : (i = "X", s = "Y", v = "Z");
  const c = Math.hypot(r[i] - B[i], r[s] - B[s]), n = Math.atan2(r[s] - B[s], r[i] - B[i]), T = Math.atan2(Q[s] - B[s], Q[i] - B[i]);
  let z;
  t === "G2" ? (z = T - n, z > 0 && (z -= Math.PI * 2)) : (z = T - n, z < 0 && (z += Math.PI * 2));
  const w = [];
  let o = { ...r };
  for (let E = 1; E <= e; E++) {
    const h = E / e, l = n + z * h, U = h, D = B[i] + c * Math.cos(l), k = B[s] + c * Math.sin(l), O = r[v] + (Q[v] - r[v]) * U, u = { X: 0, Y: 0, Z: 0 };
    u[i] = D, u[s] = k, u[v] = O, w.push({
      x0: o.X,
      y0: o.Y,
      z0: o.Z,
      x1: u.X,
      y1: u.Y,
      z1: u.Z
    }), o = u;
  }
  return w;
}
let NB = null;
function gP() {
  if (NB) return NB;
  const r = 512, Q = document.createElement("canvas");
  Q.width = r, Q.height = r;
  const B = Q.getContext("2d");
  B.fillStyle = "#8B5E3C", B.fillRect(0, 0, r, r);
  const A = Math.floor(40 + Math.random() * 20);
  for (let t = 0; t < A; t++) {
    const P = Math.random() * r, e = 1 + Math.random() * 2, i = Math.floor(139 + (Math.random() - 0.5) * 40), s = Math.floor(94 + (Math.random() - 0.5) * 30), v = Math.floor(60 + (Math.random() - 0.5) * 20);
    B.strokeStyle = `rgb(${i},${s},${v})`, B.lineWidth = e, B.beginPath(), B.moveTo(0, P), B.lineTo(r, P), B.stroke();
  }
  for (let t = 0; t < 300; t++) {
    const P = Math.random() * r, e = Math.random() * r, i = 20 + Math.random() * 60, s = (Math.random() - 0.5) * 0.3, v = Math.floor(100 + Math.random() * 80), c = Math.floor(60 + Math.random() * 50), n = Math.floor(30 + Math.random() * 40);
    B.strokeStyle = `rgba(${v},${c},${n},0.35)`, B.lineWidth = 0.5 + Math.random() * 1, B.beginPath(), B.moveTo(P, e), B.quadraticCurveTo(
      P + i * 0.5 * Math.cos(s),
      e + i * 0.5 * Math.sin(s) * 0.1,
      P + i * Math.cos(s * 0.5),
      e + i * 0.15
    ), B.stroke();
  }
  return NB = new a.CanvasTexture(Q), NB;
}
function pP(r, Q) {
  const B = new a.Group();
  B.name = "gviewer:sim3d-slab";
  const A = r.xMax - r.xMin, t = r.yMax - r.yMin, P = r.zTop - r.zBot, e = (r.xMin + r.xMax) / 2, i = (r.yMin + r.yMax) / 2, s = (r.zBot + r.zTop) / 2, v = gP();
  v.wrapS = a.RepeatWrapping, v.wrapT = a.RepeatWrapping, v.repeat.set(A / 100, t / 100);
  const c = new a.MeshStandardMaterial({
    map: v,
    roughness: 0.85,
    metalness: 0,
    side: a.FrontSide
  }), n = new a.MeshStandardMaterial({
    color: 8016432,
    roughness: 0.9,
    metalness: 0,
    side: a.DoubleSide
  }), T = new a.MeshStandardMaterial({
    color: 7030822,
    roughness: 0.9,
    metalness: 0
  }), z = new a.PlaneGeometry(A, t, Q - 1, Q - 1), w = Q * Q, o = new Float32Array(w * 3), E = z.attributes.position;
  for (let f = 0; f < w; f++)
    o[f * 3] = E.getX(f), o[f * 3 + 1] = E.getY(f), o[f * 3 + 2] = 0;
  const h = new a.BufferAttribute(o, 3);
  h.usage = a.DynamicDrawUsage, z.setAttribute("position", h), z.computeVertexNormals();
  const l = new a.Mesh(z, c);
  l.name = "gviewer:sim3d-top", l.position.set(e, i, r.zTop), B.add(l);
  const U = new a.PlaneGeometry(A, P);
  U.rotateX(Math.PI / 2);
  const D = new a.Mesh(U, n);
  D.position.set(e, r.yMax, s), B.add(D);
  const k = new a.PlaneGeometry(A, P);
  k.rotateX(Math.PI / 2);
  const O = new a.Mesh(k, n);
  O.position.set(e, r.yMin, s), B.add(O);
  const u = new a.PlaneGeometry(P, t);
  u.rotateY(-Math.PI / 2);
  const N = new a.Mesh(u, n);
  N.position.set(r.xMax, i, s), B.add(N);
  const d = new a.PlaneGeometry(P, t);
  d.rotateY(-Math.PI / 2);
  const M = new a.Mesh(d, n);
  M.position.set(r.xMin, i, s), B.add(M);
  const b = new a.Mesh(new a.PlaneGeometry(A, t), T);
  return b.position.set(e, i, r.zBot), B.add(b), { group: B, topMesh: l, topPositionAttr: h, slabBounds: r, resolution: Q };
}
function VB(r, Q) {
  const { topPositionAttr: B, slabBounds: A, resolution: t } = r, P = t * t;
  for (let e = 0; e < P; e++) {
    const i = Math.floor(e / t), s = e % t, v = (t - 1 - i) * t + s;
    B.setZ(e, Q[v] - A.zTop);
  }
  B.needsUpdate = !0, r.topMesh.geometry.computeVertexNormals();
}
function mP(r) {
  r.group.traverse((Q) => {
    if (Q instanceof a.Mesh) {
      Q.geometry.dispose();
      const B = Q.material;
      Array.isArray(B) ? B.forEach((A) => {
        A.map && A.map.dispose(), A.dispose();
      }) : (B.map && B.map.dispose(), B.dispose());
    }
  });
}
class xP {
  constructor(Q) {
    this.onSelectView = Q.onSelectView, this.faceButtons = /* @__PURE__ */ new Map(), this.root = document.createElement("div"), this.root.className = "gViewer-viewcube", this.cube = document.createElement("div"), this.cube.className = "gViewer-viewcube__cube", this.root.appendChild(this.cube);
    const B = [
      { view: "front", className: "gViewer-viewcube__face--front", label: "Front" },
      { view: "back", className: "gViewer-viewcube__face--back", label: "Back" },
      { view: "left", className: "gViewer-viewcube__face--left", label: "Left" },
      { view: "right", className: "gViewer-viewcube__face--right", label: "Right" },
      { view: "top", className: "gViewer-viewcube__face--top", label: "Top" },
      { view: "bottom", className: "gViewer-viewcube__face--bottom", label: "Bottom" }
    ];
    for (const A of B) {
      const t = document.createElement("button");
      t.type = "button", t.className = `gViewer-viewcube__face ${A.className}`, t.textContent = A.label, t.addEventListener("click", () => this.onSelectView(A.view)), this.cube.appendChild(t), this.faceButtons.set(A.view, t);
    }
    Q.container.appendChild(this.root);
  }
  setActiveFace(Q) {
    for (const [B, A] of this.faceButtons.entries())
      B === Q ? A.classList.add("is-active") : A.classList.remove("is-active");
  }
  setRotationMatrix3d(Q) {
    const B = Array.from({ length: 16 }, (A, t) => {
      const P = Q[t] ?? 0;
      return Number.isFinite(P) ? String(Math.abs(P) < 1e-12 ? 0 : Number(P.toFixed(8))) : "0";
    });
    this.cube.style.transform = `matrix3d(${B.join(",")})`;
  }
  dispose() {
    this.root.remove();
  }
}
function MQ(r, Q, B = {}) {
  const { opacity: A = 1, size: t = 10 } = B, P = new a.Object3D(), e = 100, i = document.createElement("canvas"), s = i.getContext("2d");
  if (!s)
    throw new Error("Missing canvas 2D context.");
  s.font = `normal ${e}px Arial`;
  const c = s.measureText(r).width;
  i.width = c, i.height = e, s.font = `normal ${e}px Arial`, s.textAlign = "center", s.textBaseline = "middle", s.fillStyle = Q, s.fillText(r, c / 2, e / 2);
  const n = new a.Texture(i);
  n.needsUpdate = !0, n.minFilter = a.LinearFilter;
  const T = new a.SpriteMaterial({
    map: n,
    transparent: !0,
    opacity: A
  });
  P.textHeight = t, P.textWidth = c / e * P.textHeight, B.textAlign === "left" ? P.position.x = (B.x ?? 0) + (P.textWidth ?? 0) / 2 : B.textAlign === "right" ? P.position.x = (B.x ?? 0) - (P.textWidth ?? 0) / 2 : P.position.x = B.x ?? 0, B.textBaseline === "top" ? P.position.y = (B.y ?? 0) - (P.textHeight ?? 0) / 2 : B.textBaseline === "bottom" ? P.position.y = (B.y ?? 0) + (P.textHeight ?? 0) / 2 : P.position.y = B.y ?? 0, P.position.z = B.z ?? 0;
  const z = new a.Sprite(T);
  return z.scale.set(c / e * t, t, 1), P.add(z), P;
}
function yP(r) {
  r.traverse((Q) => {
    var A;
    if (!(Q instanceof a.Sprite))
      return;
    const B = Q.material;
    (A = B.map) == null || A.dispose(), B.dispose();
  });
}
const IP = 25.4;
function SP(r, Q) {
  const B = new a.Group(), A = new a.Box3Helper(
    r.clone(),
    new a.Color(Q.render.theme.colors.boundingBox)
  ), t = A.material;
  return t.transparent = !0, t.opacity = 0.65, t.depthWrite = !1, t.linewidth = 2, B.add(A), Q.boundingBox.labels && B.add(...YP(r.clone(), Q)), B;
}
function YP(r, Q) {
  const B = Q.render.theme.colors.boundingBox, A = 0.8, t = (U) => Math.abs(U) < 1e-9 ? "0" : U.toFixed(3), P = (U) => Q.units === "in" ? `${t(U / IP)} in` : `${t(U)} mm`, e = r.min, i = r.max, s = (U) => MQ(U, B, { size: 4, opacity: A }), v = s(P(e.x)), c = s(P(i.x)), n = s(P(e.y)), T = s(P(i.y)), z = s(P(e.z)), w = s(P(i.z)), o = new a.Vector3();
  r.getSize(o);
  const E = Math.max(2, Math.max(o.x, o.y, o.z) * 0.02), h = (e.x + i.x) / 2, l = (e.y + i.y) / 2;
  return v.position.set(e.x - E, l, e.z - E), c.position.set(i.x + E, l, e.z - E), n.position.set(h, e.y - E, e.z - E), T.position.set(h, i.y + E, e.z - E), z.position.set(h, l, e.z - E), w.position.set(h, l, i.z + E), [v, c, n, T, z, w];
}
function VP(r) {
  r.traverse((Q) => {
    var B;
    if (Q instanceof a.LineSegments || Q instanceof a.Line) {
      Q.geometry.dispose();
      const A = Q.material;
      Array.isArray(A) ? A.forEach((t) => t.dispose()) : A.dispose();
      return;
    }
    if (Q instanceof a.Sprite) {
      const A = Q.material;
      (B = A.map) == null || B.dispose(), A.dispose();
    }
  });
}
function JA(r, Q, B) {
  const A = new Float32Array([
    r.x,
    r.y,
    0,
    Q.x,
    r.y,
    0,
    Q.x,
    Q.y,
    0,
    r.x,
    Q.y,
    0,
    r.x,
    r.y,
    0
  ]), t = new a.BufferGeometry();
  t.setAttribute("position", new a.BufferAttribute(A, 3));
  const P = new a.LineBasicMaterial({
    color: new a.Color(B),
    transparent: !0,
    opacity: 0.35,
    depthWrite: !1
  });
  return new a.LineLoop(t, P);
}
function JP(r, Q, B, A = 10) {
  const t = [], P = r.x - Q.y, e = Q.x - r.y;
  for (let v = P; v <= e; v += A) {
    const c = Math.max(r.x, r.y + v), n = Math.min(Q.x, Q.y + v);
    n > c && t.push(c, c - v, 0, n, n - v, 0);
  }
  const i = new a.BufferGeometry();
  i.setAttribute("position", new a.BufferAttribute(new Float32Array(t), 3));
  const s = new a.LineBasicMaterial({
    color: new a.Color(B),
    transparent: !0,
    opacity: 0.35,
    depthWrite: !1
  });
  return new a.LineSegments(i, s);
}
function RP(r, Q, B, A) {
  const t = new a.Group();
  if (t.add(JA(r, Q, B.render.theme.colors.machineBed)), A) {
    const P = B.render.theme.colors.machineBedKeepout;
    t.add(JA(A.min, A.max, P)), t.add(JP(A.min, A.max, P));
  }
  return t;
}
function XP(r) {
  r.traverse((Q) => {
    if (Q instanceof a.LineSegments || Q instanceof a.Line) {
      Q.geometry.dispose();
      const B = Q.material;
      Array.isArray(B) ? B.forEach((A) => A.dispose()) : B.dispose();
    }
  });
}
function KP(r) {
  const Q = window.getComputedStyle(r);
  (Q.position === "static" || !Q.position) && (r.style.position = "relative"), r.style.overflow || (r.style.overflow = "hidden");
}
function ZP(r) {
  if (r < 0.5)
    return 4 * r * r * r;
  const Q = -2 * r + 2;
  return 1 - Q * Q * Q / 2;
}
function RA(r) {
  const Q = new a.Vector3(), B = {
    right: new a.Vector3(1, 0, 0),
    left: new a.Vector3(-1, 0, 0),
    front: new a.Vector3(0, -1, 0),
    back: new a.Vector3(0, 1, 0),
    top: new a.Vector3(0, 0, 1),
    bottom: new a.Vector3(0, 0, -1)
  };
  switch (r) {
    case "front":
      Q.copy(B.front);
      break;
    case "back":
      Q.copy(B.back);
      break;
    case "left":
      Q.copy(B.left);
      break;
    case "right":
      Q.copy(B.right);
      break;
    case "top":
      Q.copy(B.top);
      break;
    case "bottom":
      Q.copy(B.bottom);
      break;
    case "front-top-left":
      Q.copy(B.front).add(B.top).add(B.left);
      break;
    case "front-top-right":
      Q.copy(B.front).add(B.top).add(B.right);
      break;
    case "front-bottom-left":
      Q.copy(B.front).add(B.bottom).add(B.left);
      break;
    case "front-bottom-right":
      Q.copy(B.front).add(B.bottom).add(B.right);
      break;
    case "back-top-left":
      Q.copy(B.back).add(B.top).add(B.left);
      break;
    case "back-top-right":
      Q.copy(B.back).add(B.top).add(B.right);
      break;
    case "back-bottom-left":
      Q.copy(B.back).add(B.bottom).add(B.left);
      break;
    case "back-bottom-right":
      Q.copy(B.back).add(B.bottom).add(B.right);
      break;
    default:
      Q.copy(B.front);
      break;
  }
  return Q.normalize();
}
function WP(r, Q) {
  const B = r.x - Q.x, A = r.y - Q.y, t = r.z - Q.z, P = Math.abs(B), e = Math.abs(A), i = Math.abs(t);
  return i >= P && i >= e ? t >= 0 ? "top" : "bottom" : P >= e ? B >= 0 ? "right" : "left" : A >= 0 ? "back" : "front";
}
const lt = 25.4;
function UA(r) {
  if (r.bounds)
    return {
      minX: r.bounds.min.x,
      maxX: r.bounds.max.x,
      minY: r.bounds.min.y,
      maxY: r.bounds.max.y
    };
  const Q = Math.max(1, r.sizeXMm) / 2, B = Math.max(1, r.sizeYMm) / 2;
  return { minX: -Q, maxX: Q, minY: -B, maxY: B };
}
function qP(r) {
  const { minX: Q, maxX: B, minY: A, maxY: t } = UA(r), P = r.units === "mm" ? 10 : lt, e = [], i = [], s = (w, o, E, h, l) => {
    l.push(w, o, 0, E, h, 0);
  }, v = Math.ceil(A / P) * P, c = Math.floor(t / P) * P;
  for (let w = v; w <= c + 1e-6; w += P)
    s(Q, w, B, w, Math.abs(w) < 1e-6 ? e : i);
  const n = Math.ceil(Q / P) * P, T = Math.floor(B / P) * P;
  for (let w = n; w <= T + 1e-6; w += P)
    s(w, A, w, t, Math.abs(w) < 1e-6 ? e : i);
  const z = new a.Group();
  return i.length > 0 && z.add(XA(i, r.theme.colors.grid.minor, 0.25)), e.length > 0 && z.add(XA(e, r.theme.colors.grid.major, 0.4)), z;
}
function XA(r, Q, B) {
  const A = new a.BufferGeometry();
  A.setAttribute("position", new a.Float32BufferAttribute(r, 3));
  const t = new a.LineBasicMaterial({
    color: new a.Color(Q),
    transparent: !0,
    opacity: B,
    depthWrite: !1
  });
  return new a.LineSegments(A, t);
}
function _P(r) {
  r.traverse((Q) => {
    if (!(Q instanceof a.LineSegments))
      return;
    Q.geometry.dispose();
    const B = Q.material;
    Array.isArray(B) ? B.forEach((A) => A.dispose()) : B.dispose();
  });
}
function $P(r) {
  const Q = new a.Group(), { minX: B, maxX: A, minY: t, maxY: P } = UA({
    sizeXMm: r.sizeXWorld,
    sizeYMm: r.sizeYWorld,
    bounds: r.bounds
  }), e = Math.max(1, r.depthWorld), i = Math.max(1, Math.min(Math.max(A - B, P - t) / 24, 30)), s = i * 0.6, v = (h) => new a.LineDashedMaterial({
    color: new a.Color(h),
    dashSize: i,
    gapSize: s,
    transparent: !0,
    opacity: 0.75,
    depthWrite: !1
  }), c = new a.Line(
    new a.BufferGeometry().setFromPoints([
      new a.Vector3(B, 0, 0),
      new a.Vector3(A, 0, 0)
    ]),
    v(r.theme.colors.axes.x)
  );
  c.computeLineDistances();
  const n = new a.Line(
    new a.BufferGeometry().setFromPoints([
      new a.Vector3(0, t, 0),
      new a.Vector3(0, P, 0)
    ]),
    v(r.theme.colors.axes.y)
  );
  n.computeLineDistances();
  const T = new a.Line(
    new a.BufferGeometry().setFromPoints([
      new a.Vector3(0, 0, 0),
      new a.Vector3(0, 0, e)
    ]),
    v(r.theme.colors.axes.z)
  );
  T.computeLineDistances(), Q.add(c, n, T);
  const z = Math.max(2, i), w = MQ("X", r.theme.colors.axes.x, { size: 5 });
  w.position.set(A + z, 0, 0);
  const o = MQ("Y", r.theme.colors.axes.y, { size: 5 });
  o.position.set(0, P + z, 0);
  const E = MQ("Z", r.theme.colors.axes.z, { size: 5 });
  return E.position.set(0, 0, e + z), Q.add(w, o, E), Q;
}
function Qe(r) {
  r.traverse((Q) => {
    var B;
    if (Q instanceof a.Line) {
      Q.geometry.dispose();
      const A = Q.material;
      Array.isArray(A) ? A.forEach((t) => t.dispose()) : A.dispose();
      return;
    }
    if (Q instanceof a.Sprite) {
      const A = Q.material;
      (B = A.map) == null || B.dispose(), A.dispose();
    }
  });
}
function Be(r) {
  const Q = new a.Group(), { minX: B, maxX: A, minY: t, maxY: P } = UA(r), e = r.units === "mm" ? 10 : 1, i = e * 2, s = r.units === "mm" ? 1 : lt, v = 0.5, c = 4, n = 0.1, T = 5, z = r.theme.colors.axes.x, w = r.theme.colors.axes.y, o = { opacity: v, size: c };
  for (let E = e; ; E += i) {
    const h = E * s;
    if (h > A + 1e-6)
      break;
    const l = MQ(String(E), z, o);
    l.position.set(h, T, n), Q.add(l);
  }
  for (let E = e; ; E += i) {
    const h = E * s;
    if (-h < B - 1e-6)
      break;
    const l = MQ(String(-E), z, o);
    l.position.set(-h, T, n), Q.add(l);
  }
  for (let E = e; ; E += i) {
    const h = E * s;
    if (h > P + 1e-6)
      break;
    const l = MQ(String(E), w, o);
    l.position.set(-T, h, n), Q.add(l);
  }
  for (let E = e; ; E += i) {
    const h = E * s;
    if (-h < t - 1e-6)
      break;
    const l = MQ(String(-E), w, o);
    l.position.set(-T, -h, n), Q.add(l);
  }
  return Q;
}
function dQ(r) {
  if (r === void 0)
    throw new ReferenceError("this hasn't been initialised - super() hasn't been called");
  return r;
}
function kt(r, Q) {
  r.prototype = Object.create(Q.prototype), r.prototype.constructor = r, r.__proto__ = Q;
}
/*!
 * GSAP 3.15.0
 * https://gsap.com
 *
 * @license Copyright 2008-2026, GreenSock. All rights reserved.
 * Subject to the terms at https://gsap.com/standard-license
 * @author: Jack Doyle, jack@greensock.com
*/
var zQ = {
  autoSleep: 120,
  force3D: "auto",
  nullTargetWarn: 1,
  units: {
    lineHeight: ""
  }
}, EB = {
  duration: 0.5,
  overwrite: !1,
  delay: 0
}, DA, q, y, hQ = 1e8, m = 1 / hQ, rA = Math.PI * 2, Ae = rA / 4, te = 0, Ut = Math.sqrt, Pe = Math.cos, ee = Math.sin, Z = function(Q) {
  return typeof Q == "string";
}, J = function(Q) {
  return typeof Q == "function";
}, NQ = function(Q) {
  return typeof Q == "number";
}, uA = function(Q) {
  return typeof Q > "u";
}, uQ = function(Q) {
  return typeof Q == "object";
}, tQ = function(Q) {
  return Q !== !1;
}, dA = function() {
  return typeof window < "u";
}, OB = function(Q) {
  return J(Q) || Z(Q);
}, Dt = typeof ArrayBuffer == "function" && ArrayBuffer.isView || function() {
}, $ = Array.isArray, ie = /random\([^)]+\)/g, re = /,\s*/g, KA = /(?:-?\.?\d|\.)+/gi, ut = /[-+=.]*\d+[.e\-+]*\d*[e\-+]*\d*/g, qQ = /[-+=.]*\d+[.e-]*\d*[a-z%]*/g, JB = /[-+=.]*\d+\.?\d*(?:e-|e\+)?\d*/gi, dt = /[+-]=-?[.\d]+/, se = /[^,'"\[\]\s]+/gi, ve = /^[+\-=e\s\d]*\d+[.\d]*([a-z]*|%)\s*$/i, Y, kQ, sA, CA, wQ = {}, FB = {}, Ct, Nt = function(Q) {
  return (FB = AB(Q, wQ)) && rQ;
}, NA = function(Q, B) {
  return console.warn("Invalid property", Q, "set to", B, "Missing plugin? gsap.registerPlugin()");
}, aB = function(Q, B) {
  return !B && console.warn(Q);
}, Ot = function(Q, B) {
  return Q && (wQ[Q] = B) && FB && (FB[Q] = B) || wQ;
}, hB = function() {
  return 0;
}, ce = {
  suppressEvents: !0,
  isStart: !0,
  kill: !1
}, LB = {
  suppressEvents: !0,
  kill: !1
}, ne = {
  suppressEvents: !0
}, OA = {}, GQ = [], vA = {}, Lt, cQ = {}, RB = {}, ZA = 30, MB = [], LA = "", MA = function(Q) {
  var B = Q[0], A, t;
  if (uQ(B) || J(B) || (Q = [Q]), !(A = (B._gsap || {}).harness)) {
    for (t = MB.length; t-- && !MB[t].targetTest(B); )
      ;
    A = MB[t];
  }
  for (t = Q.length; t--; )
    Q[t] && (Q[t]._gsap || (Q[t]._gsap = new Wt(Q[t], A))) || Q.splice(t, 1);
  return Q;
}, YQ = function(Q) {
  return Q._gsap || MA(lQ(Q))[0]._gsap;
}, Mt = function(Q, B, A) {
  return (A = Q[B]) && J(A) ? Q[B]() : uA(A) && Q.getAttribute && Q.getAttribute(B) || A;
}, PQ = function(Q, B) {
  return (Q = Q.split(",")).forEach(B) || Q;
}, X = function(Q) {
  return Math.round(Q * 1e5) / 1e5 || 0;
}, S = function(Q) {
  return Math.round(Q * 1e7) / 1e7 || 0;
}, $Q = function(Q, B) {
  var A = B.charAt(0), t = parseFloat(B.substr(2));
  return Q = parseFloat(Q), A === "+" ? Q + t : A === "-" ? Q - t : A === "*" ? Q * t : Q / t;
}, oe = function(Q, B) {
  for (var A = B.length, t = 0; Q.indexOf(B[t]) < 0 && ++t < A; )
    ;
  return t < A;
}, GB = function() {
  var Q = GQ.length, B = GQ.slice(0), A, t;
  for (vA = {}, GQ.length = 0, A = 0; A < Q; A++)
    t = B[A], t && t._lazy && (t.render(t._lazy[0], t._lazy[1], !0)._lazy = 0);
}, HA = function(Q) {
  return !!(Q._initted || Q._startAt || Q.add);
}, Ht = function(Q, B, A, t) {
  GQ.length && !q && GB(), Q.render(B, A, !!(q && B < 0 && HA(Q))), GQ.length && !q && GB();
}, jt = function(Q) {
  var B = parseFloat(Q);
  return (B || B === 0) && (Q + "").match(se).length < 2 ? B : Z(Q) ? Q.trim() : Q;
}, Ft = function(Q) {
  return Q;
}, EQ = function(Q, B) {
  for (var A in B)
    A in Q || (Q[A] = B[A]);
  return Q;
}, Te = function(Q) {
  return function(B, A) {
    for (var t in A)
      t in B || t === "duration" && Q || t === "ease" || (B[t] = A[t]);
  };
}, AB = function(Q, B) {
  for (var A in B)
    Q[A] = B[A];
  return Q;
}, WA = function r(Q, B) {
  for (var A in B)
    A !== "__proto__" && A !== "constructor" && A !== "prototype" && (Q[A] = uQ(B[A]) ? r(Q[A] || (Q[A] = {}), B[A]) : B[A]);
  return Q;
}, fB = function(Q, B) {
  var A = {}, t;
  for (t in Q)
    t in B || (A[t] = Q[t]);
  return A;
}, TB = function(Q) {
  var B = Q.parent || Y, A = Q.keyframes ? Te($(Q.keyframes)) : EQ;
  if (tQ(Q.inherit))
    for (; B; )
      A(Q, B.vars.defaults), B = B.parent || B._dp;
  return Q;
}, ze = function(Q, B) {
  for (var A = Q.length, t = A === B.length; t && A-- && Q[A] === B[A]; )
    ;
  return A < 0;
}, Gt = function(Q, B, A, t, P) {
  var e = Q[t], i;
  if (P)
    for (i = B[P]; e && e[P] > i; )
      e = e._prev;
  return e ? (B._next = e._next, e._next = B) : (B._next = Q[A], Q[A] = B), B._next ? B._next._prev = B : Q[t] = B, B._prev = e, B.parent = B._dp = Q, B;
}, xB = function(Q, B, A, t) {
  A === void 0 && (A = "_first"), t === void 0 && (t = "_last");
  var P = B._prev, e = B._next;
  P ? P._next = e : Q[A] === B && (Q[A] = e), e ? e._prev = P : Q[t] === B && (Q[t] = P), B._next = B._prev = B.parent = null;
}, bQ = function(Q, B) {
  Q.parent && (!B || Q.parent.autoRemoveChildren) && Q.parent.remove && Q.parent.remove(Q), Q._act = 0;
}, VQ = function(Q, B) {
  if (Q && (!B || B._end > Q._dur || B._start < 0))
    for (var A = Q; A; )
      A._dirty = 1, A = A.parent;
  return Q;
}, we = function(Q) {
  for (var B = Q.parent; B && B.parent; )
    B._dirty = 1, B.totalDuration(), B = B.parent;
  return Q;
}, cA = function(Q, B, A, t) {
  return Q._startAt && (q ? Q._startAt.revert(LB) : Q.vars.immediateRender && !Q.vars.autoRevert || Q._startAt.render(B, !0, t));
}, Ee = function r(Q) {
  return !Q || Q._ts && r(Q.parent);
}, qA = function(Q) {
  return Q._repeat ? tB(Q._tTime, Q = Q.duration() + Q._rDelay) * Q : 0;
}, tB = function(Q, B) {
  var A = Math.floor(Q = S(Q / B));
  return Q && A === Q ? A - 1 : A;
}, bB = function(Q, B) {
  return (Q - B._start) * B._ts + (B._ts >= 0 ? 0 : B._dirty ? B.totalDuration() : B._tDur);
}, yB = function(Q) {
  return Q._end = S(Q._start + (Q._tDur / Math.abs(Q._ts || Q._rts || m) || 0));
}, IB = function(Q, B) {
  var A = Q._dp;
  return A && A.smoothChildTiming && Q._ts && (Q._start = S(A._time - (Q._ts > 0 ? B / Q._ts : ((Q._dirty ? Q.totalDuration() : Q._tDur) - B) / -Q._ts)), yB(Q), A._dirty || VQ(A, Q)), Q;
}, ft = function(Q, B) {
  var A;
  if ((B._time || !B._dur && B._initted || B._start < Q._time && (B._dur || !B.add)) && (A = bB(Q.rawTime(), B), (!B._dur || dB(0, B.totalDuration(), A) - B._tTime > m) && B.render(A, !0)), VQ(Q, B)._dp && Q._initted && Q._time >= Q._dur && Q._ts) {
    if (Q._dur < Q.duration())
      for (A = Q; A._dp; )
        A.rawTime() >= 0 && A.totalTime(A._tTime), A = A._dp;
    Q._zTime = -m;
  }
}, UQ = function(Q, B, A, t) {
  return B.parent && bQ(B), B._start = S((NQ(A) ? A : A || Q !== Y ? aQ(Q, A, B) : Q._time) + B._delay), B._end = S(B._start + (B.totalDuration() / Math.abs(B.timeScale()) || 0)), Gt(Q, B, "_first", "_last", Q._sort ? "_start" : 0), nA(B) || (Q._recent = B), t || ft(Q, B), Q._ts < 0 && IB(Q, Q._tTime), Q;
}, bt = function(Q, B) {
  return (wQ.ScrollTrigger || NA("scrollTrigger", B)) && wQ.ScrollTrigger.create(B, Q);
}, gt = function(Q, B, A, t, P) {
  if (FA(Q, B, P), !Q._initted)
    return 1;
  if (!A && Q._pt && !q && (Q._dur && Q.vars.lazy !== !1 || !Q._dur && Q.vars.lazy) && Lt !== nQ.frame)
    return GQ.push(Q), Q._lazy = [P, t], 1;
}, ae = function r(Q) {
  var B = Q.parent;
  return B && B._ts && B._initted && !B._lock && (B.rawTime() < 0 || r(B));
}, nA = function(Q) {
  var B = Q.data;
  return B === "isFromStart" || B === "isStart";
}, he = function(Q, B, A, t) {
  var P = Q.ratio, e = B < 0 || !B && (!Q._start && ae(Q) && !(!Q._initted && nA(Q)) || (Q._ts < 0 || Q._dp._ts < 0) && !nA(Q)) ? 0 : 1, i = Q._rDelay, s = 0, v, c, n;
  if (i && Q._repeat && (s = dB(0, Q._tDur, B), c = tB(s, i), Q._yoyo && c & 1 && (e = 1 - e), c !== tB(Q._tTime, i) && (P = 1 - e, Q.vars.repeatRefresh && Q._initted && Q.invalidate())), e !== P || q || t || Q._zTime === m || !B && Q._zTime) {
    if (!Q._initted && gt(Q, B, t, A, s))
      return;
    for (n = Q._zTime, Q._zTime = B || (A ? m : 0), A || (A = B && !n), Q.ratio = e, Q._from && (e = 1 - e), Q._time = 0, Q._tTime = s, v = Q._pt; v; )
      v.r(e, v.d), v = v._next;
    B < 0 && cA(Q, B, A, !0), Q._onUpdate && !A && oQ(Q, "onUpdate"), s && Q._repeat && !A && Q.parent && oQ(Q, "onRepeat"), (B >= Q._tDur || B < 0) && Q.ratio === e && (e && bQ(Q, 1), !A && !q && (oQ(Q, e ? "onComplete" : "onReverseComplete", !0), Q._prom && Q._prom()));
  } else Q._zTime || (Q._zTime = B);
}, le = function(Q, B, A) {
  var t;
  if (A > B)
    for (t = Q._first; t && t._start <= A; ) {
      if (t.data === "isPause" && t._start > B)
        return t;
      t = t._next;
    }
  else
    for (t = Q._last; t && t._start >= A; ) {
      if (t.data === "isPause" && t._start < B)
        return t;
      t = t._prev;
    }
}, PB = function(Q, B, A, t) {
  var P = Q._repeat, e = S(B) || 0, i = Q._tTime / Q._tDur;
  return i && !t && (Q._time *= e / Q._dur), Q._dur = e, Q._tDur = P ? P < 0 ? 1e10 : S(e * (P + 1) + Q._rDelay * P) : e, i > 0 && !t && IB(Q, Q._tTime = Q._tDur * i), Q.parent && yB(Q), A || VQ(Q.parent, Q), Q;
}, _A = function(Q) {
  return Q instanceof AQ ? VQ(Q) : PB(Q, Q._dur);
}, ke = {
  _start: 0,
  endTime: hB,
  totalDuration: hB
}, aQ = function r(Q, B, A) {
  var t = Q.labels, P = Q._recent || ke, e = Q.duration() >= hQ ? P.endTime(!1) : Q._dur, i, s, v;
  return Z(B) && (isNaN(B) || B in t) ? (s = B.charAt(0), v = B.substr(-1) === "%", i = B.indexOf("="), s === "<" || s === ">" ? (i >= 0 && (B = B.replace(/=/, "")), (s === "<" ? P._start : P.endTime(P._repeat >= 0)) + (parseFloat(B.substr(1)) || 0) * (v ? (i < 0 ? P : A).totalDuration() / 100 : 1)) : i < 0 ? (B in t || (t[B] = e), t[B]) : (s = parseFloat(B.charAt(i - 1) + B.substr(i + 1)), v && A && (s = s / 100 * ($(A) ? A[0] : A).totalDuration()), i > 1 ? r(Q, B.substr(0, i - 1), A) + s : e + s)) : B == null ? e : +B;
}, zB = function(Q, B, A) {
  var t = NQ(B[1]), P = (t ? 2 : 1) + (Q < 2 ? 0 : 1), e = B[P], i, s;
  if (t && (e.duration = B[1]), e.parent = A, Q) {
    for (i = e, s = A; s && !("immediateRender" in i); )
      i = s.vars.defaults || {}, s = tQ(s.vars.inherit) && s.parent;
    e.immediateRender = tQ(i.immediateRender), Q < 2 ? e.runBackwards = 1 : e.startAt = B[P - 1];
  }
  return new K(B[0], e, B[P + 1]);
}, mQ = function(Q, B) {
  return Q || Q === 0 ? B(Q) : B;
}, dB = function(Q, B, A) {
  return A < Q ? Q : A > B ? B : A;
}, _ = function(Q, B) {
  return !Z(Q) || !(B = ve.exec(Q)) ? "" : B[1];
}, Ue = function(Q, B, A) {
  return mQ(A, function(t) {
    return dB(Q, B, t);
  });
}, oA = [].slice, pt = function(Q, B) {
  return Q && uQ(Q) && "length" in Q && (!B && !Q.length || Q.length - 1 in Q && uQ(Q[0])) && !Q.nodeType && Q !== kQ;
}, De = function(Q, B, A) {
  return A === void 0 && (A = []), Q.forEach(function(t) {
    var P;
    return Z(t) && !B || pt(t, 1) ? (P = A).push.apply(P, lQ(t)) : A.push(t);
  }) || A;
}, lQ = function(Q, B, A) {
  return y && !B && y.selector ? y.selector(Q) : Z(Q) && !A && (sA || !eB()) ? oA.call((B || CA).querySelectorAll(Q), 0) : $(Q) ? De(Q, A) : pt(Q) ? oA.call(Q, 0) : Q ? [Q] : [];
}, TA = function(Q) {
  return Q = lQ(Q)[0] || aB("Invalid scope") || {}, function(B) {
    var A = Q.current || Q.nativeElement || Q;
    return lQ(B, A.querySelectorAll ? A : A === Q ? aB("Invalid scope") || CA.createElement("div") : Q);
  };
}, mt = function(Q) {
  return Q.sort(function() {
    return 0.5 - Math.random();
  });
}, xt = function(Q) {
  if (J(Q))
    return Q;
  var B = uQ(Q) ? Q : {
    each: Q
  }, A = JQ(B.ease), t = B.from || 0, P = parseFloat(B.base) || 0, e = {}, i = t > 0 && t < 1, s = isNaN(t) || i, v = B.axis, c = t, n = t;
  return Z(t) ? c = n = {
    center: 0.5,
    edges: 0.5,
    end: 1
  }[t] || 0 : !i && s && (c = t[0], n = t[1]), function(T, z, w) {
    var o = (w || B).length, E = e[o], h, l, U, D, k, O, u, N, d;
    if (!E) {
      if (d = B.grid === "auto" ? 0 : (B.grid || [1, hQ])[1], !d) {
        for (u = -hQ; u < (u = w[d++].getBoundingClientRect().left) && d < o; )
          ;
        d < o && d--;
      }
      for (E = e[o] = [], h = s ? Math.min(d, o) * c - 0.5 : t % d, l = d === hQ ? 0 : s ? o * n / d - 0.5 : t / d | 0, u = 0, N = hQ, O = 0; O < o; O++)
        U = O % d - h, D = l - (O / d | 0), E[O] = k = v ? Math.abs(v === "y" ? D : U) : Ut(U * U + D * D), k > u && (u = k), k < N && (N = k);
      t === "random" && mt(E), E.max = u - N, E.min = N, E.v = o = (parseFloat(B.amount) || parseFloat(B.each) * (d > o ? o - 1 : v ? v === "y" ? o / d : d : Math.max(d, o / d)) || 0) * (t === "edges" ? -1 : 1), E.b = o < 0 ? P - o : P, E.u = _(B.amount || B.each) || 0, A = A && o < 0 ? be(A) : A;
    }
    return o = (E[T] - E.min) / E.max || 0, S(E.b + (A ? A(o) : o) * E.v) + E.u;
  };
}, zA = function(Q) {
  var B = Math.pow(10, ((Q + "").split(".")[1] || "").length);
  return function(A) {
    var t = S(Math.round(parseFloat(A) / Q) * Q * B);
    return (t - t % 1) / B + (NQ(A) ? 0 : _(A));
  };
}, yt = function(Q, B) {
  var A = $(Q), t, P;
  return !A && uQ(Q) && (t = A = Q.radius || hQ, Q.values ? (Q = lQ(Q.values), (P = !NQ(Q[0])) && (t *= t)) : Q = zA(Q.increment)), mQ(B, A ? J(Q) ? function(e) {
    return P = Q(e), Math.abs(P - e) <= t ? P : e;
  } : function(e) {
    for (var i = parseFloat(P ? e.x : e), s = parseFloat(P ? e.y : 0), v = hQ, c = 0, n = Q.length, T, z; n--; )
      P ? (T = Q[n].x - i, z = Q[n].y - s, T = T * T + z * z) : T = Math.abs(Q[n] - i), T < v && (v = T, c = n);
    return c = !t || v <= t ? Q[c] : e, P || c === e || NQ(e) ? c : c + _(e);
  } : zA(Q));
}, It = function(Q, B, A, t) {
  return mQ($(Q) ? !B : A === !0 ? !!(A = 0) : !t, function() {
    return $(Q) ? Q[~~(Math.random() * Q.length)] : (A = A || 1e-5) && (t = A < 1 ? Math.pow(10, (A + "").length - 2) : 1) && Math.floor(Math.round((Q - A / 2 + Math.random() * (B - Q + A * 0.99)) / A) * A * t) / t;
  });
}, ue = function() {
  for (var Q = arguments.length, B = new Array(Q), A = 0; A < Q; A++)
    B[A] = arguments[A];
  return function(t) {
    return B.reduce(function(P, e) {
      return e(P);
    }, t);
  };
}, de = function(Q, B) {
  return function(A) {
    return Q(parseFloat(A)) + (B || _(A));
  };
}, Ce = function(Q, B, A) {
  return Yt(Q, B, 0, 1, A);
}, St = function(Q, B, A) {
  return mQ(A, function(t) {
    return Q[~~B(t)];
  });
}, Ne = function r(Q, B, A) {
  var t = B - Q;
  return $(Q) ? St(Q, r(0, Q.length), B) : mQ(A, function(P) {
    return (t + (P - Q) % t) % t + Q;
  });
}, Oe = function r(Q, B, A) {
  var t = B - Q, P = t * 2;
  return $(Q) ? St(Q, r(0, Q.length - 1), B) : mQ(A, function(e) {
    return e = (P + (e - Q) % P) % P || 0, Q + (e > t ? P - e : e);
  });
}, lB = function(Q) {
  return Q.replace(ie, function(B) {
    var A = B.indexOf("[") + 1, t = B.substring(A || 7, A ? B.indexOf("]") : B.length - 1).split(re);
    return It(A ? t : +t[0], A ? 0 : +t[1], +t[2] || 1e-5);
  });
}, Yt = function(Q, B, A, t, P) {
  var e = B - Q, i = t - A;
  return mQ(P, function(s) {
    return A + ((s - Q) / e * i || 0);
  });
}, Le = function r(Q, B, A, t) {
  var P = isNaN(Q + B) ? 0 : function(z) {
    return (1 - z) * Q + z * B;
  };
  if (!P) {
    var e = Z(Q), i = {}, s, v, c, n, T;
    if (A === !0 && (t = 1) && (A = null), e)
      Q = {
        p: Q
      }, B = {
        p: B
      };
    else if ($(Q) && !$(B)) {
      for (c = [], n = Q.length, T = n - 2, v = 1; v < n; v++)
        c.push(r(Q[v - 1], Q[v]));
      n--, P = function(w) {
        w *= n;
        var o = Math.min(T, ~~w);
        return c[o](w - o);
      }, A = B;
    } else t || (Q = AB($(Q) ? [] : {}, Q));
    if (!c) {
      for (s in B)
        jA.call(i, Q, s, "get", B[s]);
      P = function(w) {
        return bA(w, i) || (e ? Q.p : Q);
      };
    }
  }
  return mQ(A, P);
}, $A = function(Q, B, A) {
  var t = Q.labels, P = hQ, e, i, s;
  for (e in t)
    i = t[e] - B, i < 0 == !!A && i && P > (i = Math.abs(i)) && (s = e, P = i);
  return s;
}, oQ = function(Q, B, A) {
  var t = Q.vars, P = t[B], e = y, i = Q._ctx, s, v, c;
  if (P)
    return s = t[B + "Params"], v = t.callbackScope || Q, A && GQ.length && GB(), i && (y = i), c = s ? P.apply(v, s) : P.call(v), y = e, c;
}, nB = function(Q) {
  return bQ(Q), Q.scrollTrigger && Q.scrollTrigger.kill(!!q), Q.progress() < 1 && oQ(Q, "onInterrupt"), Q;
}, _Q, Vt = [], Jt = function(Q) {
  if (Q)
    if (Q = !Q.name && Q.default || Q, dA() || Q.headless) {
      var B = Q.name, A = J(Q), t = B && !A && Q.init ? function() {
        this._props = [];
      } : Q, P = {
        init: hB,
        render: bA,
        add: jA,
        kill: Je,
        modifier: Ve,
        rawVars: 0
      }, e = {
        targetTest: 0,
        get: 0,
        getSetter: fA,
        aliases: {},
        register: 0
      };
      if (eB(), Q !== t) {
        if (cQ[B])
          return;
        EQ(t, EQ(fB(Q, P), e)), AB(t.prototype, AB(P, fB(Q, e))), cQ[t.prop = B] = t, Q.targetTest && (MB.push(t), OA[B] = 1), B = (B === "css" ? "CSS" : B.charAt(0).toUpperCase() + B.substr(1)) + "Plugin";
      }
      Ot(B, t), Q.register && Q.register(rQ, t, eQ);
    } else
      Vt.push(Q);
}, p = 255, oB = {
  aqua: [0, p, p],
  lime: [0, p, 0],
  silver: [192, 192, 192],
  black: [0, 0, 0],
  maroon: [128, 0, 0],
  teal: [0, 128, 128],
  blue: [0, 0, p],
  navy: [0, 0, 128],
  white: [p, p, p],
  olive: [128, 128, 0],
  yellow: [p, p, 0],
  orange: [p, 165, 0],
  gray: [128, 128, 128],
  purple: [128, 0, 128],
  green: [0, 128, 0],
  red: [p, 0, 0],
  pink: [p, 192, 203],
  cyan: [0, p, p],
  transparent: [p, p, p, 0]
}, XB = function(Q, B, A) {
  return Q += Q < 0 ? 1 : Q > 1 ? -1 : 0, (Q * 6 < 1 ? B + (A - B) * Q * 6 : Q < 0.5 ? A : Q * 3 < 2 ? B + (A - B) * (2 / 3 - Q) * 6 : B) * p + 0.5 | 0;
}, Rt = function(Q, B, A) {
  var t = Q ? NQ(Q) ? [Q >> 16, Q >> 8 & p, Q & p] : 0 : oB.black, P, e, i, s, v, c, n, T, z, w;
  if (!t) {
    if (Q.substr(-1) === "," && (Q = Q.substr(0, Q.length - 1)), oB[Q])
      t = oB[Q];
    else if (Q.charAt(0) === "#") {
      if (Q.length < 6 && (P = Q.charAt(1), e = Q.charAt(2), i = Q.charAt(3), Q = "#" + P + P + e + e + i + i + (Q.length === 5 ? Q.charAt(4) + Q.charAt(4) : "")), Q.length === 9)
        return t = parseInt(Q.substr(1, 6), 16), [t >> 16, t >> 8 & p, t & p, parseInt(Q.substr(7), 16) / 255];
      Q = parseInt(Q.substr(1), 16), t = [Q >> 16, Q >> 8 & p, Q & p];
    } else if (Q.substr(0, 3) === "hsl") {
      if (t = w = Q.match(KA), !B)
        s = +t[0] % 360 / 360, v = +t[1] / 100, c = +t[2] / 100, e = c <= 0.5 ? c * (v + 1) : c + v - c * v, P = c * 2 - e, t.length > 3 && (t[3] *= 1), t[0] = XB(s + 1 / 3, P, e), t[1] = XB(s, P, e), t[2] = XB(s - 1 / 3, P, e);
      else if (~Q.indexOf("="))
        return t = Q.match(ut), A && t.length < 4 && (t[3] = 1), t;
    } else
      t = Q.match(KA) || oB.transparent;
    t = t.map(Number);
  }
  return B && !w && (P = t[0] / p, e = t[1] / p, i = t[2] / p, n = Math.max(P, e, i), T = Math.min(P, e, i), c = (n + T) / 2, n === T ? s = v = 0 : (z = n - T, v = c > 0.5 ? z / (2 - n - T) : z / (n + T), s = n === P ? (e - i) / z + (e < i ? 6 : 0) : n === e ? (i - P) / z + 2 : (P - e) / z + 4, s *= 60), t[0] = ~~(s + 0.5), t[1] = ~~(v * 100 + 0.5), t[2] = ~~(c * 100 + 0.5)), A && t.length < 4 && (t[3] = 1), t;
}, Xt = function(Q) {
  var B = [], A = [], t = -1;
  return Q.split(fQ).forEach(function(P) {
    var e = P.match(qQ) || [];
    B.push.apply(B, e), A.push(t += e.length + 1);
  }), B.c = A, B;
}, Qt = function(Q, B, A) {
  var t = "", P = (Q + t).match(fQ), e = B ? "hsla(" : "rgba(", i = 0, s, v, c, n;
  if (!P)
    return Q;
  if (P = P.map(function(T) {
    return (T = Rt(T, B, 1)) && e + (B ? T[0] + "," + T[1] + "%," + T[2] + "%," + T[3] : T.join(",")) + ")";
  }), A && (c = Xt(Q), s = A.c, s.join(t) !== c.c.join(t)))
    for (v = Q.replace(fQ, "1").split(qQ), n = v.length - 1; i < n; i++)
      t += v[i] + (~s.indexOf(i) ? P.shift() || e + "0,0,0,0)" : (c.length ? c : P.length ? P : A).shift());
  if (!v)
    for (v = Q.split(fQ), n = v.length - 1; i < n; i++)
      t += v[i] + P[i];
  return t + v[n];
}, fQ = function() {
  var r = "(?:\\b(?:(?:rgb|rgba|hsl|hsla)\\(.+?\\))|\\B#(?:[0-9a-f]{3,4}){1,2}\\b", Q;
  for (Q in oB)
    r += "|" + Q + "\\b";
  return new RegExp(r + ")", "gi");
}(), Me = /hsl[a]?\(/, Kt = function(Q) {
  var B = Q.join(" "), A;
  if (fQ.lastIndex = 0, fQ.test(B))
    return A = Me.test(B), Q[1] = Qt(Q[1], A), Q[0] = Qt(Q[0], A, Xt(Q[1])), !0;
}, kB, nQ = function() {
  var r = Date.now, Q = 500, B = 33, A = r(), t = A, P = 1e3 / 240, e = P, i = [], s, v, c, n, T, z, w = function o(E) {
    var h = r() - t, l = E === !0, U, D, k, O;
    if ((h > Q || h < 0) && (A += h - B), t += h, k = t - A, U = k - e, (U > 0 || l) && (O = ++n.frame, T = k - n.time * 1e3, n.time = k = k / 1e3, e += U + (U >= P ? 4 : P - U), D = 1), l || (s = v(o)), D)
      for (z = 0; z < i.length; z++)
        i[z](k, T, O, E);
  };
  return n = {
    time: 0,
    frame: 0,
    tick: function() {
      w(!0);
    },
    deltaRatio: function(E) {
      return T / (1e3 / (E || 60));
    },
    wake: function() {
      Ct && (!sA && dA() && (kQ = sA = window, CA = kQ.document || {}, wQ.gsap = rQ, (kQ.gsapVersions || (kQ.gsapVersions = [])).push(rQ.version), Nt(FB || kQ.GreenSockGlobals || !kQ.gsap && kQ || {}), Vt.forEach(Jt)), c = typeof requestAnimationFrame < "u" && requestAnimationFrame, s && n.sleep(), v = c || function(E) {
        return setTimeout(E, e - n.time * 1e3 + 1 | 0);
      }, kB = 1, w(2));
    },
    sleep: function() {
      (c ? cancelAnimationFrame : clearTimeout)(s), kB = 0, v = hB;
    },
    lagSmoothing: function(E, h) {
      Q = E || 1 / 0, B = Math.min(h || 33, Q);
    },
    fps: function(E) {
      P = 1e3 / (E || 240), e = n.time * 1e3 + P;
    },
    add: function(E, h, l) {
      var U = h ? function(D, k, O, u) {
        E(D, k, O, u), n.remove(U);
      } : E;
      return n.remove(E), i[l ? "unshift" : "push"](U), eB(), U;
    },
    remove: function(E, h) {
      ~(h = i.indexOf(E)) && i.splice(h, 1) && z >= h && z--;
    },
    _listeners: i
  }, n;
}(), eB = function() {
  return !kB && nQ.wake();
}, g = {}, He = /^[\d.\-M][\d.\-,\s]/, je = /["']/g, Fe = function(Q) {
  for (var B = {}, A = Q.substr(1, Q.length - 3).split(":"), t = A[0], P = 1, e = A.length, i, s, v; P < e; P++)
    s = A[P], i = P !== e - 1 ? s.lastIndexOf(",") : s.length, v = s.substr(0, i), B[t] = isNaN(v) ? v.replace(je, "").trim() : +v, t = s.substr(i + 1).trim();
  return B;
}, Ge = function(Q) {
  var B = Q.indexOf("(") + 1, A = Q.indexOf(")"), t = Q.indexOf("(", B);
  return Q.substring(B, ~t && t < A ? Q.indexOf(")", A + 1) : A);
}, fe = function(Q) {
  var B = (Q + "").split("("), A = g[B[0]];
  return A && B.length > 1 && A.config ? A.config.apply(null, ~Q.indexOf("{") ? [Fe(B[1])] : Ge(Q).split(",").map(jt)) : g._CE && He.test(Q) ? g._CE("", Q) : A;
}, be = function(Q) {
  return function(B) {
    return 1 - Q(1 - B);
  };
}, JQ = function(Q, B) {
  return Q && (J(Q) ? Q : g[Q] || fe(Q)) || B;
}, XQ = function(Q, B, A, t) {
  A === void 0 && (A = function(s) {
    return 1 - B(1 - s);
  }), t === void 0 && (t = function(s) {
    return s < 0.5 ? B(s * 2) / 2 : 1 - B((1 - s) * 2) / 2;
  });
  var P = {
    easeIn: B,
    easeOut: A,
    easeInOut: t
  }, e;
  return PQ(Q, function(i) {
    g[i] = wQ[i] = P, g[e = i.toLowerCase()] = A;
    for (var s in P)
      g[e + (s === "easeIn" ? ".in" : s === "easeOut" ? ".out" : ".inOut")] = g[i + "." + s] = P[s];
  }), P;
}, Zt = function(Q) {
  return function(B) {
    return B < 0.5 ? (1 - Q(1 - B * 2)) / 2 : 0.5 + Q((B - 0.5) * 2) / 2;
  };
}, KB = function r(Q, B, A) {
  var t = B >= 1 ? B : 1, P = (A || (Q ? 0.3 : 0.45)) / (B < 1 ? B : 1), e = P / rA * (Math.asin(1 / t) || 0), i = function(c) {
    return c === 1 ? 1 : t * Math.pow(2, -10 * c) * ee((c - e) * P) + 1;
  }, s = Q === "out" ? i : Q === "in" ? function(v) {
    return 1 - i(1 - v);
  } : Zt(i);
  return P = rA / P, s.config = function(v, c) {
    return r(Q, v, c);
  }, s;
}, ZB = function r(Q, B) {
  B === void 0 && (B = 1.70158);
  var A = function(e) {
    return e ? --e * e * ((B + 1) * e + B) + 1 : 0;
  }, t = Q === "out" ? A : Q === "in" ? function(P) {
    return 1 - A(1 - P);
  } : Zt(A);
  return t.config = function(P) {
    return r(Q, P);
  }, t;
};
PQ("Linear,Quad,Cubic,Quart,Quint,Strong", function(r, Q) {
  var B = Q < 5 ? Q + 1 : Q;
  XQ(r + ",Power" + (B - 1), Q ? function(A) {
    return Math.pow(A, B);
  } : function(A) {
    return A;
  }, function(A) {
    return 1 - Math.pow(1 - A, B);
  }, function(A) {
    return A < 0.5 ? Math.pow(A * 2, B) / 2 : 1 - Math.pow((1 - A) * 2, B) / 2;
  });
});
g.Linear.easeNone = g.none = g.Linear.easeIn;
XQ("Elastic", KB("in"), KB("out"), KB());
(function(r, Q) {
  var B = 1 / Q, A = 2 * B, t = 2.5 * B, P = function(i) {
    return i < B ? r * i * i : i < A ? r * Math.pow(i - 1.5 / Q, 2) + 0.75 : i < t ? r * (i -= 2.25 / Q) * i + 0.9375 : r * Math.pow(i - 2.625 / Q, 2) + 0.984375;
  };
  XQ("Bounce", function(e) {
    return 1 - P(1 - e);
  }, P);
})(7.5625, 2.75);
XQ("Expo", function(r) {
  return Math.pow(2, 10 * (r - 1)) * r + r * r * r * r * r * r * (1 - r);
});
XQ("Circ", function(r) {
  return -(Ut(1 - r * r) - 1);
});
XQ("Sine", function(r) {
  return r === 1 ? 1 : -Pe(r * Ae) + 1;
});
XQ("Back", ZB("in"), ZB("out"), ZB());
g.SteppedEase = g.steps = wQ.SteppedEase = {
  config: function(Q, B) {
    Q === void 0 && (Q = 1);
    var A = 1 / Q, t = Q + (B ? 0 : 1), P = B ? 1 : 0, e = 1 - m;
    return function(i) {
      return ((t * dB(0, e, i) | 0) + P) * A;
    };
  }
};
EB.ease = g["quad.out"];
PQ("onComplete,onUpdate,onStart,onRepeat,onReverseComplete,onInterrupt", function(r) {
  return LA += r + "," + r + "Params,";
});
var Wt = function(Q, B) {
  this.id = te++, Q._gsap = this, this.target = Q, this.harness = B, this.get = B ? B.get : Mt, this.set = B ? B.getSetter : fA;
}, UB = /* @__PURE__ */ function() {
  function r(B) {
    this.vars = B, this._delay = +B.delay || 0, (this._repeat = B.repeat === 1 / 0 ? -2 : B.repeat || 0) && (this._rDelay = B.repeatDelay || 0, this._yoyo = !!B.yoyo || !!B.yoyoEase), this._ts = 1, PB(this, +B.duration, 1, 1), this.data = B.data, y && (this._ctx = y, y.data.push(this)), kB || nQ.wake();
  }
  var Q = r.prototype;
  return Q.delay = function(A) {
    return A || A === 0 ? (this.parent && this.parent.smoothChildTiming && this.startTime(this._start + A - this._delay), this._delay = A, this) : this._delay;
  }, Q.duration = function(A) {
    return arguments.length ? this.totalDuration(this._repeat > 0 ? A + (A + this._rDelay) * this._repeat : A) : this.totalDuration() && this._dur;
  }, Q.totalDuration = function(A) {
    return arguments.length ? (this._dirty = 0, PB(this, this._repeat < 0 ? A : (A - this._repeat * this._rDelay) / (this._repeat + 1))) : this._tDur;
  }, Q.totalTime = function(A, t) {
    if (eB(), !arguments.length)
      return this._tTime;
    var P = this._dp;
    if (P && P.smoothChildTiming && this._ts) {
      for (IB(this, A), !P._dp || P.parent || ft(P, this); P && P.parent; )
        P.parent._time !== P._start + (P._ts >= 0 ? P._tTime / P._ts : (P.totalDuration() - P._tTime) / -P._ts) && P.totalTime(P._tTime, !0), P = P.parent;
      !this.parent && this._dp.autoRemoveChildren && (this._ts > 0 && A < this._tDur || this._ts < 0 && A > 0 || !this._tDur && !A) && UQ(this._dp, this, this._start - this._delay);
    }
    return (this._tTime !== A || !this._dur && !t || this._initted && Math.abs(this._zTime) === m || !this._initted && this._dur && A || !A && !this._initted && (this.add || this._ptLookup)) && (this._ts || (this._pTime = A), Ht(this, A, t)), this;
  }, Q.time = function(A, t) {
    return arguments.length ? this.totalTime(Math.min(this.totalDuration(), A + qA(this)) % (this._dur + this._rDelay) || (A ? this._dur : 0), t) : this._time;
  }, Q.totalProgress = function(A, t) {
    return arguments.length ? this.totalTime(this.totalDuration() * A, t) : this.totalDuration() ? Math.min(1, this._tTime / this._tDur) : this.rawTime() >= 0 && this._initted ? 1 : 0;
  }, Q.progress = function(A, t) {
    return arguments.length ? this.totalTime(this.duration() * (this._yoyo && !(this.iteration() & 1) ? 1 - A : A) + qA(this), t) : this.duration() ? Math.min(1, this._time / this._dur) : this.rawTime() > 0 ? 1 : 0;
  }, Q.iteration = function(A, t) {
    var P = this.duration() + this._rDelay;
    return arguments.length ? this.totalTime(this._time + (A - 1) * P, t) : this._repeat ? tB(this._tTime, P) + 1 : 1;
  }, Q.timeScale = function(A, t) {
    if (!arguments.length)
      return this._rts === -m ? 0 : this._rts;
    if (this._rts === A)
      return this;
    var P = this.parent && this._ts ? bB(this.parent._time, this) : this._tTime;
    return this._rts = +A || 0, this._ts = this._ps || A === -m ? 0 : this._rts, this.totalTime(dB(-Math.abs(this._delay), this.totalDuration(), P), t !== !1), yB(this), we(this);
  }, Q.paused = function(A) {
    return arguments.length ? (this._ps !== A && (this._ps = A, A ? (this._pTime = this._tTime || Math.max(-this._delay, this.rawTime()), this._ts = this._act = 0) : (eB(), this._ts = this._rts, this.totalTime(this.parent && !this.parent.smoothChildTiming ? this.rawTime() : this._tTime || this._pTime, this.progress() === 1 && Math.abs(this._zTime) !== m && (this._tTime -= m)))), this) : this._ps;
  }, Q.startTime = function(A) {
    if (arguments.length) {
      this._start = S(A);
      var t = this.parent || this._dp;
      return t && (t._sort || !this.parent) && UQ(t, this, this._start - this._delay), this;
    }
    return this._start;
  }, Q.endTime = function(A) {
    return this._start + (tQ(A) ? this.totalDuration() : this.duration()) / Math.abs(this._ts || 1);
  }, Q.rawTime = function(A) {
    var t = this.parent || this._dp;
    return t ? A && (!this._ts || this._repeat && this._time && this.totalProgress() < 1) ? this._tTime % (this._dur + this._rDelay) : this._ts ? bB(t.rawTime(A), this) : this._tTime : this._tTime;
  }, Q.revert = function(A) {
    A === void 0 && (A = ne);
    var t = q;
    return q = A, HA(this) && (this.timeline && this.timeline.revert(A), this.totalTime(-0.01, A.suppressEvents)), this.data !== "nested" && A.kill !== !1 && this.kill(), q = t, this;
  }, Q.globalTime = function(A) {
    for (var t = this, P = arguments.length ? A : t.rawTime(); t; )
      P = t._start + P / (Math.abs(t._ts) || 1), t = t._dp;
    return !this.parent && this._sat ? this._sat.globalTime(A) : P;
  }, Q.repeat = function(A) {
    return arguments.length ? (this._repeat = A === 1 / 0 ? -2 : A, _A(this)) : this._repeat === -2 ? 1 / 0 : this._repeat;
  }, Q.repeatDelay = function(A) {
    if (arguments.length) {
      var t = this._time;
      return this._rDelay = A, _A(this), t ? this.time(t) : this;
    }
    return this._rDelay;
  }, Q.yoyo = function(A) {
    return arguments.length ? (this._yoyo = A, this) : this._yoyo;
  }, Q.seek = function(A, t) {
    return this.totalTime(aQ(this, A), tQ(t));
  }, Q.restart = function(A, t) {
    return this.play().totalTime(A ? -this._delay : 0, tQ(t)), this._dur || (this._zTime = -m), this;
  }, Q.play = function(A, t) {
    return A != null && this.seek(A, t), this.reversed(!1).paused(!1);
  }, Q.reverse = function(A, t) {
    return A != null && this.seek(A || this.totalDuration(), t), this.reversed(!0).paused(!1);
  }, Q.pause = function(A, t) {
    return A != null && this.seek(A, t), this.paused(!0);
  }, Q.resume = function() {
    return this.paused(!1);
  }, Q.reversed = function(A) {
    return arguments.length ? (!!A !== this.reversed() && this.timeScale(-this._rts || (A ? -m : 0)), this) : this._rts < 0;
  }, Q.invalidate = function() {
    return this._initted = this._act = 0, this._zTime = -m, this;
  }, Q.isActive = function() {
    var A = this.parent || this._dp, t = this._start, P;
    return !!(!A || this._ts && this._initted && A.isActive() && (P = A.rawTime(!0)) >= t && P < this.endTime(!0) - m);
  }, Q.eventCallback = function(A, t, P) {
    var e = this.vars;
    return arguments.length > 1 ? (t ? (e[A] = t, P && (e[A + "Params"] = P), A === "onUpdate" && (this._onUpdate = t)) : delete e[A], this) : e[A];
  }, Q.then = function(A) {
    var t = this, P = t._prom;
    return new Promise(function(e) {
      var i = J(A) ? A : Ft, s = function() {
        var c = t.then;
        t.then = null, P && P(), J(i) && (i = i(t)) && (i.then || i === t) && (t.then = c), e(i), t.then = c;
      };
      t._initted && t.totalProgress() === 1 && t._ts >= 0 || !t._tTime && t._ts < 0 ? s() : t._prom = s;
    });
  }, Q.kill = function() {
    nB(this);
  }, r;
}();
EQ(UB.prototype, {
  _time: 0,
  _start: 0,
  _end: 0,
  _tTime: 0,
  _tDur: 0,
  _dirty: 0,
  _repeat: 0,
  _yoyo: !1,
  parent: null,
  _initted: !1,
  _rDelay: 0,
  _ts: 1,
  _dp: 0,
  ratio: 0,
  _zTime: -m,
  _prom: 0,
  _ps: !1,
  _rts: 1
});
var AQ = /* @__PURE__ */ function(r) {
  kt(Q, r);
  function Q(A, t) {
    var P;
    return A === void 0 && (A = {}), P = r.call(this, A) || this, P.labels = {}, P.smoothChildTiming = !!A.smoothChildTiming, P.autoRemoveChildren = !!A.autoRemoveChildren, P._sort = tQ(A.sortChildren), Y && UQ(A.parent || Y, dQ(P), t), A.reversed && P.reverse(), A.paused && P.paused(!0), A.scrollTrigger && bt(dQ(P), A.scrollTrigger), P;
  }
  var B = Q.prototype;
  return B.to = function(t, P, e) {
    return zB(0, arguments, this), this;
  }, B.from = function(t, P, e) {
    return zB(1, arguments, this), this;
  }, B.fromTo = function(t, P, e, i) {
    return zB(2, arguments, this), this;
  }, B.set = function(t, P, e) {
    return P.duration = 0, P.parent = this, TB(P).repeatDelay || (P.repeat = 0), P.immediateRender = !!P.immediateRender, new K(t, P, aQ(this, e), 1), this;
  }, B.call = function(t, P, e) {
    return UQ(this, K.delayedCall(0, t, P), e);
  }, B.staggerTo = function(t, P, e, i, s, v, c) {
    return e.duration = P, e.stagger = e.stagger || i, e.onComplete = v, e.onCompleteParams = c, e.parent = this, new K(t, e, aQ(this, s)), this;
  }, B.staggerFrom = function(t, P, e, i, s, v, c) {
    return e.runBackwards = 1, TB(e).immediateRender = tQ(e.immediateRender), this.staggerTo(t, P, e, i, s, v, c);
  }, B.staggerFromTo = function(t, P, e, i, s, v, c, n) {
    return i.startAt = e, TB(i).immediateRender = tQ(i.immediateRender), this.staggerTo(t, P, i, s, v, c, n);
  }, B.render = function(t, P, e) {
    var i = this._time, s = this._dirty ? this.totalDuration() : this._tDur, v = this._dur, c = t <= 0 ? 0 : S(t), n = this._zTime < 0 != t < 0 && (this._initted || !v), T, z, w, o, E, h, l, U, D, k, O, u;
    if (this !== Y && c > s && t >= 0 && (c = s), c !== this._tTime || e || n) {
      if (i !== this._time && v && (c += this._time - i, t += this._time - i), T = c, D = this._start, U = this._ts, h = !U, n && (v || (i = this._zTime), (t || !P) && (this._zTime = t)), this._repeat) {
        if (O = this._yoyo, E = v + this._rDelay, this._repeat < -1 && t < 0)
          return this.totalTime(E * 100 + t, P, e);
        if (T = S(c % E), c === s ? (o = this._repeat, T = v) : (k = S(c / E), o = ~~k, o && o === k && (T = v, o--), T > v && (T = v)), k = tB(this._tTime, E), !i && this._tTime && k !== o && this._tTime - k * E - this._dur <= 0 && (k = o), O && o & 1 && (T = v - T, u = 1), o !== k && !this._lock) {
          var N = O && k & 1, d = N === (O && o & 1);
          if (o < k && (N = !N), i = N ? 0 : c % v ? v : c, this._lock = 1, this.render(i || (u ? 0 : S(o * E)), P, !v)._lock = 0, this._tTime = c, !P && this.parent && oQ(this, "onRepeat"), this.vars.repeatRefresh && !u && (this.invalidate()._lock = 1, k = o), i && i !== this._time || h !== !this._ts || this.vars.onRepeat && !this.parent && !this._act)
            return this;
          if (v = this._dur, s = this._tDur, d && (this._lock = 2, i = N ? v : -1e-4, this.render(i, !0), this.vars.repeatRefresh && !u && this.invalidate()), this._lock = 0, !this._ts && !h)
            return this;
        }
      }
      if (this._hasPause && !this._forcing && this._lock < 2 && (l = le(this, S(i), S(T)), l && (c -= T - (T = l._start))), this._tTime = c, this._time = T, this._act = !!U, this._initted || (this._onUpdate = this.vars.onUpdate, this._initted = 1, this._zTime = t, i = 0), !i && c && v && !P && !k && (oQ(this, "onStart"), this._tTime !== c))
        return this;
      if (T >= i && t >= 0)
        for (z = this._first; z; ) {
          if (w = z._next, (z._act || T >= z._start) && z._ts && l !== z) {
            if (z.parent !== this)
              return this.render(t, P, e);
            if (z.render(z._ts > 0 ? (T - z._start) * z._ts : (z._dirty ? z.totalDuration() : z._tDur) + (T - z._start) * z._ts, P, e), T !== this._time || !this._ts && !h) {
              l = 0, w && (c += this._zTime = -m);
              break;
            }
          }
          z = w;
        }
      else {
        z = this._last;
        for (var M = t < 0 ? t : T; z; ) {
          if (w = z._prev, (z._act || M <= z._end) && z._ts && l !== z) {
            if (z.parent !== this)
              return this.render(t, P, e);
            if (z.render(z._ts > 0 ? (M - z._start) * z._ts : (z._dirty ? z.totalDuration() : z._tDur) + (M - z._start) * z._ts, P, e || q && HA(z)), T !== this._time || !this._ts && !h) {
              l = 0, w && (c += this._zTime = M ? -m : m);
              break;
            }
          }
          z = w;
        }
      }
      if (l && !P && (this.pause(), l.render(T >= i ? 0 : -m)._zTime = T >= i ? 1 : -1, this._ts))
        return this._start = D, yB(this), this.render(t, P, e);
      this._onUpdate && !P && oQ(this, "onUpdate", !0), (c === s && this._tTime >= this.totalDuration() || !c && i) && (D === this._start || Math.abs(U) !== Math.abs(this._ts)) && (this._lock || ((t || !v) && (c === s && this._ts > 0 || !c && this._ts < 0) && bQ(this, 1), !P && !(t < 0 && !i) && (c || i || !s) && (oQ(this, c === s && t >= 0 ? "onComplete" : "onReverseComplete", !0), this._prom && !(c < s && this.timeScale() > 0) && this._prom())));
    }
    return this;
  }, B.add = function(t, P) {
    var e = this;
    if (NQ(P) || (P = aQ(this, P, t)), !(t instanceof UB)) {
      if ($(t))
        return t.forEach(function(i) {
          return e.add(i, P);
        }), this;
      if (Z(t))
        return this.addLabel(t, P);
      if (J(t))
        t = K.delayedCall(0, t);
      else
        return this;
    }
    return this !== t ? UQ(this, t, P) : this;
  }, B.getChildren = function(t, P, e, i) {
    t === void 0 && (t = !0), P === void 0 && (P = !0), e === void 0 && (e = !0), i === void 0 && (i = -hQ);
    for (var s = [], v = this._first; v; )
      v._start >= i && (v instanceof K ? P && s.push(v) : (e && s.push(v), t && s.push.apply(s, v.getChildren(!0, P, e)))), v = v._next;
    return s;
  }, B.getById = function(t) {
    for (var P = this.getChildren(1, 1, 1), e = P.length; e--; )
      if (P[e].vars.id === t)
        return P[e];
  }, B.remove = function(t) {
    return Z(t) ? this.removeLabel(t) : J(t) ? this.killTweensOf(t) : (t.parent === this && xB(this, t), t === this._recent && (this._recent = this._last), VQ(this));
  }, B.totalTime = function(t, P) {
    return arguments.length ? (this._forcing = 1, !this._dp && this._ts && (this._start = S(nQ.time - (this._ts > 0 ? t / this._ts : (this.totalDuration() - t) / -this._ts))), r.prototype.totalTime.call(this, t, P), this._forcing = 0, this) : this._tTime;
  }, B.addLabel = function(t, P) {
    return this.labels[t] = aQ(this, P), this;
  }, B.removeLabel = function(t) {
    return delete this.labels[t], this;
  }, B.addPause = function(t, P, e) {
    var i = K.delayedCall(0, P || hB, e);
    return i.data = "isPause", this._hasPause = 1, UQ(this, i, aQ(this, t));
  }, B.removePause = function(t) {
    var P = this._first;
    for (t = aQ(this, t); P; )
      P._start === t && P.data === "isPause" && bQ(P), P = P._next;
  }, B.killTweensOf = function(t, P, e) {
    for (var i = this.getTweensOf(t, e), s = i.length; s--; )
      HQ !== i[s] && i[s].kill(t, P);
    return this;
  }, B.getTweensOf = function(t, P) {
    for (var e = [], i = lQ(t), s = this._first, v = NQ(P), c; s; )
      s instanceof K ? oe(s._targets, i) && (v ? (!HQ || s._initted && s._ts) && s.globalTime(0) <= P && s.globalTime(s.totalDuration()) > P : !P || s.isActive()) && e.push(s) : (c = s.getTweensOf(i, P)).length && e.push.apply(e, c), s = s._next;
    return e;
  }, B.tweenTo = function(t, P) {
    P = P || {};
    var e = this, i = aQ(e, t), s = P, v = s.startAt, c = s.onStart, n = s.onStartParams, T = s.immediateRender, z, w = K.to(e, EQ({
      ease: P.ease || "none",
      lazy: !1,
      immediateRender: !1,
      time: i,
      overwrite: "auto",
      duration: P.duration || Math.abs((i - (v && "time" in v ? v.time : e._time)) / e.timeScale()) || m,
      onStart: function() {
        if (e.pause(), !z) {
          var E = P.duration || Math.abs((i - (v && "time" in v ? v.time : e._time)) / e.timeScale());
          w._dur !== E && PB(w, E, 0, 1).render(w._time, !0, !0), z = 1;
        }
        c && c.apply(w, n || []);
      }
    }, P));
    return T ? w.render(0) : w;
  }, B.tweenFromTo = function(t, P, e) {
    return this.tweenTo(P, EQ({
      startAt: {
        time: aQ(this, t)
      }
    }, e));
  }, B.recent = function() {
    return this._recent;
  }, B.nextLabel = function(t) {
    return t === void 0 && (t = this._time), $A(this, aQ(this, t));
  }, B.previousLabel = function(t) {
    return t === void 0 && (t = this._time), $A(this, aQ(this, t), 1);
  }, B.currentLabel = function(t) {
    return arguments.length ? this.seek(t, !0) : this.previousLabel(this._time + m);
  }, B.shiftChildren = function(t, P, e) {
    e === void 0 && (e = 0);
    var i = this._first, s = this.labels, v;
    for (t = S(t); i; )
      i._start >= e && (i._start += t, i._end += t), i = i._next;
    if (P)
      for (v in s)
        s[v] >= e && (s[v] += t);
    return VQ(this);
  }, B.invalidate = function(t) {
    var P = this._first;
    for (this._lock = 0; P; )
      P.invalidate(t), P = P._next;
    return r.prototype.invalidate.call(this, t);
  }, B.clear = function(t) {
    t === void 0 && (t = !0);
    for (var P = this._first, e; P; )
      e = P._next, this.remove(P), P = e;
    return this._dp && (this._time = this._tTime = this._pTime = 0), t && (this.labels = {}), VQ(this);
  }, B.totalDuration = function(t) {
    var P = 0, e = this, i = e._last, s = hQ, v, c, n;
    if (arguments.length)
      return e.timeScale((e._repeat < 0 ? e.duration() : e.totalDuration()) / (e.reversed() ? -t : t));
    if (e._dirty) {
      for (n = e.parent; i; )
        v = i._prev, i._dirty && i.totalDuration(), c = i._start, c > s && e._sort && i._ts && !e._lock ? (e._lock = 1, UQ(e, i, c - i._delay, 1)._lock = 0) : s = c, c < 0 && i._ts && (P -= c, (!n && !e._dp || n && n.smoothChildTiming) && (e._start += S(c / e._ts), e._time -= c, e._tTime -= c), e.shiftChildren(-c, !1, -1 / 0), s = 0), i._end > P && i._ts && (P = i._end), i = v;
      PB(e, e === Y && e._time > P ? e._time : P, 1, 1), e._dirty = 0;
    }
    return e._tDur;
  }, Q.updateRoot = function(t) {
    if (Y._ts && (Ht(Y, bB(t, Y)), Lt = nQ.frame), nQ.frame >= ZA) {
      ZA += zQ.autoSleep || 120;
      var P = Y._first;
      if ((!P || !P._ts) && zQ.autoSleep && nQ._listeners.length < 2) {
        for (; P && !P._ts; )
          P = P._next;
        P || nQ.sleep();
      }
    }
  }, Q;
}(UB);
EQ(AQ.prototype, {
  _lock: 0,
  _hasPause: 0,
  _forcing: 0
});
var ge = function(Q, B, A, t, P, e, i) {
  var s = new eQ(this._pt, Q, B, 0, 1, AP, null, P), v = 0, c = 0, n, T, z, w, o, E, h, l;
  for (s.b = A, s.e = t, A += "", t += "", (h = ~t.indexOf("random(")) && (t = lB(t)), e && (l = [A, t], e(l, Q, B), A = l[0], t = l[1]), T = A.match(JB) || []; n = JB.exec(t); )
    w = n[0], o = t.substring(v, n.index), z ? z = (z + 1) % 5 : o.substr(-5) === "rgba(" && (z = 1), w !== T[c++] && (E = parseFloat(T[c - 1]) || 0, s._pt = {
      _next: s._pt,
      p: o || c === 1 ? o : ",",
      //note: SVG spec allows omission of comma/space when a negative sign is wedged between two numbers, like 2.5-5.3 instead of 2.5,-5.3 but when tweening, the negative value may switch to positive, so we insert the comma just in case.
      s: E,
      c: w.charAt(1) === "=" ? $Q(E, w) - E : parseFloat(w) - E,
      m: z && z < 4 ? Math.round : 0
    }, v = JB.lastIndex);
  return s.c = v < t.length ? t.substring(v, t.length) : "", s.fp = i, (dt.test(t) || h) && (s.e = 0), this._pt = s, s;
}, jA = function(Q, B, A, t, P, e, i, s, v, c) {
  J(t) && (t = t(P || 0, Q, e));
  var n = Q[B], T = A !== "get" ? A : J(n) ? v ? Q[B.indexOf("set") || !J(Q["get" + B.substr(3)]) ? B : "get" + B.substr(3)](v) : Q[B]() : n, z = J(n) ? v ? Ie : QP : GA, w;
  if (Z(t) && (~t.indexOf("random(") && (t = lB(t)), t.charAt(1) === "=" && (w = $Q(T, t) + (_(T) || 0), (w || w === 0) && (t = w))), !c || T !== t || wA)
    return !isNaN(T * t) && t !== "" ? (w = new eQ(this._pt, Q, B, +T || 0, t - (T || 0), typeof n == "boolean" ? Ye : BP, 0, z), v && (w.fp = v), i && w.modifier(i, this, Q), this._pt = w) : (!n && !(B in Q) && NA(B, t), ge.call(this, Q, B, T, t, z, s || zQ.stringFilter, v));
}, pe = function(Q, B, A, t, P) {
  if (J(Q) && (Q = wB(Q, P, B, A, t)), !uQ(Q) || Q.style && Q.nodeType || $(Q) || Dt(Q))
    return Z(Q) ? wB(Q, P, B, A, t) : Q;
  var e = {}, i;
  for (i in Q)
    e[i] = wB(Q[i], P, B, A, t);
  return e;
}, qt = function(Q, B, A, t, P, e) {
  var i, s, v, c;
  if (cQ[Q] && (i = new cQ[Q]()).init(P, i.rawVars ? B[Q] : pe(B[Q], t, P, e, A), A, t, e) !== !1 && (A._pt = s = new eQ(A._pt, P, Q, 0, 1, i.render, i, 0, i.priority), A !== _Q))
    for (v = A._ptLookup[A._targets.indexOf(P)], c = i._props.length; c--; )
      v[i._props[c]] = s;
  return i;
}, HQ, wA, FA = function r(Q, B, A) {
  var t = Q.vars, P = t.ease, e = t.startAt, i = t.immediateRender, s = t.lazy, v = t.onUpdate, c = t.runBackwards, n = t.yoyoEase, T = t.keyframes, z = t.autoRevert, w = Q._dur, o = Q._startAt, E = Q._targets, h = Q.parent, l = h && h.data === "nested" ? h.vars.targets : E, U = Q._overwrite === "auto" && !DA, D = Q.timeline, k = t.easeReverse || n, O, u, N, d, M, b, f, C, j, F, H, G, I;
  if (D && (!T || !P) && (P = "none"), Q._ease = JQ(P, EB.ease), Q._rEase = k && (JQ(k) || Q._ease), Q._from = !D && !!t.runBackwards, Q._from && (Q.ratio = 1), !D || T && !t.stagger) {
    if (C = E[0] ? YQ(E[0]).harness : 0, G = C && t[C.prop], O = fB(t, OA), o && (o._zTime < 0 && o.progress(1), B < 0 && c && i && !z ? o.render(-1, !0) : o.revert(c && w ? LB : ce), o._lazy = 0), e) {
      if (bQ(Q._startAt = K.set(E, EQ({
        data: "isStart",
        overwrite: !1,
        parent: h,
        immediateRender: !0,
        lazy: !o && tQ(s),
        startAt: null,
        delay: 0,
        onUpdate: v && function() {
          return oQ(Q, "onUpdate");
        },
        stagger: 0
      }, e))), Q._startAt._dp = 0, Q._startAt._sat = Q, B < 0 && (q || !i && !z) && Q._startAt.revert(LB), i && w && B <= 0 && A <= 0) {
        B && (Q._zTime = B);
        return;
      }
    } else if (c && w && !o) {
      if (B && (i = !1), N = EQ({
        overwrite: !1,
        data: "isFromStart",
        //we tag the tween with as "isFromStart" so that if [inside a plugin] we need to only do something at the very END of a tween, we have a way of identifying this tween as merely the one that's setting the beginning values for a "from()" tween. For example, clearProps in CSSPlugin should only get applied at the very END of a tween and without this tag, from(...{height:100, clearProps:"height", delay:1}) would wipe the height at the beginning of the tween and after 1 second, it'd kick back in.
        lazy: i && !o && tQ(s),
        immediateRender: i,
        //zero-duration tweens render immediately by default, but if we're not specifically instructed to render this tween immediately, we should skip this and merely _init() to record the starting values (rendering them immediately would push them to completion which is wasteful in that case - we'd have to render(-1) immediately after)
        stagger: 0,
        parent: h
        //ensures that nested tweens that had a stagger are handled properly, like gsap.from(".class", {y: gsap.utils.wrap([-100,100]), stagger: 0.5})
      }, O), G && (N[C.prop] = G), bQ(Q._startAt = K.set(E, N)), Q._startAt._dp = 0, Q._startAt._sat = Q, B < 0 && (q ? Q._startAt.revert(LB) : Q._startAt.render(-1, !0)), Q._zTime = B, !i)
        r(Q._startAt, m, m);
      else if (!B)
        return;
    }
    for (Q._pt = Q._ptCache = 0, s = w && tQ(s) || s && !w, u = 0; u < E.length; u++) {
      if (M = E[u], f = M._gsap || MA(E)[u]._gsap, Q._ptLookup[u] = F = {}, vA[f.id] && GQ.length && GB(), H = l === E ? u : l.indexOf(M), C && (j = new C()).init(M, G || O, Q, H, l) !== !1 && (Q._pt = d = new eQ(Q._pt, M, j.name, 0, 1, j.render, j, 0, j.priority), j._props.forEach(function(R) {
        F[R] = d;
      }), j.priority && (b = 1)), !C || G)
        for (N in O)
          cQ[N] && (j = qt(N, O, Q, H, M, l)) ? j.priority && (b = 1) : F[N] = d = jA.call(Q, M, N, "get", O[N], H, l, 0, t.stringFilter);
      Q._op && Q._op[u] && Q.kill(M, Q._op[u]), U && Q._pt && (HQ = Q, Y.killTweensOf(M, F, Q.globalTime(B)), I = !Q.parent, HQ = 0), Q._pt && s && (vA[f.id] = 1);
    }
    b && tP(Q), Q._onInit && Q._onInit(Q);
  }
  Q._onUpdate = v, Q._initted = (!Q._op || Q._pt) && !I, T && B <= 0 && D.render(hQ, !0, !0);
}, me = function(Q, B, A, t, P, e, i, s) {
  var v = (Q._pt && Q._ptCache || (Q._ptCache = {}))[B], c, n, T, z;
  if (!v)
    for (v = Q._ptCache[B] = [], T = Q._ptLookup, z = Q._targets.length; z--; ) {
      if (c = T[z][B], c && c.d && c.d._pt)
        for (c = c.d._pt; c && c.p !== B && c.fp !== B; )
          c = c._next;
      if (!c)
        return wA = 1, Q.vars[B] = "+=0", FA(Q, i), wA = 0, s ? aB(B + " not eligible for reset. Try splitting into individual properties") : 1;
      v.push(c);
    }
  for (z = v.length; z--; )
    n = v[z], c = n._pt || n, c.s = (t || t === 0) && !P ? t : c.s + (t || 0) + e * c.c, c.c = A - c.s, n.e && (n.e = X(A) + _(n.e)), n.b && (n.b = c.s + _(n.b));
}, xe = function(Q, B) {
  var A = Q[0] ? YQ(Q[0]).harness : 0, t = A && A.aliases, P, e, i, s;
  if (!t)
    return B;
  P = AB({}, B);
  for (e in t)
    if (e in P)
      for (s = t[e].split(","), i = s.length; i--; )
        P[s[i]] = P[e];
  return P;
}, ye = function(Q, B, A, t) {
  var P = B.ease || t || "power1.inOut", e, i;
  if ($(B))
    i = A[Q] || (A[Q] = []), B.forEach(function(s, v) {
      return i.push({
        t: v / (B.length - 1) * 100,
        v: s,
        e: P
      });
    });
  else
    for (e in B)
      i = A[e] || (A[e] = []), e === "ease" || i.push({
        t: parseFloat(Q),
        v: B[e],
        e: P
      });
}, wB = function(Q, B, A, t, P) {
  return J(Q) ? Q.call(B, A, t, P) : Z(Q) && ~Q.indexOf("random(") ? lB(Q) : Q;
}, _t = LA + "repeat,repeatDelay,yoyo,repeatRefresh,yoyoEase,easeReverse,autoRevert", $t = {};
PQ(_t + ",id,stagger,delay,duration,paused,scrollTrigger", function(r) {
  return $t[r] = 1;
});
var K = /* @__PURE__ */ function(r) {
  kt(Q, r);
  function Q(A, t, P, e) {
    var i;
    typeof t == "number" && (P.duration = t, t = P, P = null), i = r.call(this, e ? t : TB(t)) || this;
    var s = i.vars, v = s.duration, c = s.delay, n = s.immediateRender, T = s.stagger, z = s.overwrite, w = s.keyframes, o = s.defaults, E = s.scrollTrigger, h = t.parent || Y, l = ($(A) || Dt(A) ? NQ(A[0]) : "length" in t) ? [A] : lQ(A), U, D, k, O, u, N, d, M;
    if (i._targets = l.length ? MA(l) : aB("GSAP target " + A + " not found. https://gsap.com", !zQ.nullTargetWarn) || [], i._ptLookup = [], i._overwrite = z, w || T || OB(v) || OB(c)) {
      t = i.vars;
      var b = t.easeReverse || t.yoyoEase;
      if (U = i.timeline = new AQ({
        data: "nested",
        defaults: o || {},
        targets: h && h.data === "nested" ? h.vars.targets : l
      }), U.kill(), U.parent = U._dp = dQ(i), U._start = 0, T || OB(v) || OB(c)) {
        if (O = l.length, d = T && xt(T), uQ(T))
          for (u in T)
            ~_t.indexOf(u) && (M || (M = {}), M[u] = T[u]);
        for (D = 0; D < O; D++)
          k = fB(t, $t), k.stagger = 0, b && (k.easeReverse = b), M && AB(k, M), N = l[D], k.duration = +wB(v, dQ(i), D, N, l), k.delay = (+wB(c, dQ(i), D, N, l) || 0) - i._delay, !T && O === 1 && k.delay && (i._delay = c = k.delay, i._start += c, k.delay = 0), U.to(N, k, d ? d(D, N, l) : 0), U._ease = g.none;
        U.duration() ? v = c = 0 : i.timeline = 0;
      } else if (w) {
        TB(EQ(U.vars.defaults, {
          ease: "none"
        })), U._ease = JQ(w.ease || t.ease || "none");
        var f = 0, C, j, F;
        if ($(w))
          w.forEach(function(H) {
            return U.to(l, H, ">");
          }), U.duration();
        else {
          k = {};
          for (u in w)
            u === "ease" || u === "easeEach" || ye(u, w[u], k, w.easeEach);
          for (u in k)
            for (C = k[u].sort(function(H, G) {
              return H.t - G.t;
            }), f = 0, D = 0; D < C.length; D++)
              j = C[D], F = {
                ease: j.e,
                duration: (j.t - (D ? C[D - 1].t : 0)) / 100 * v
              }, F[u] = j.v, U.to(l, F, f), f += F.duration;
          U.duration() < v && U.to({}, {
            duration: v - U.duration()
          });
        }
      }
      v || i.duration(v = U.duration());
    } else
      i.timeline = 0;
    return z === !0 && !DA && (HQ = dQ(i), Y.killTweensOf(l), HQ = 0), UQ(h, dQ(i), P), t.reversed && i.reverse(), t.paused && i.paused(!0), (n || !v && !w && i._start === S(h._time) && tQ(n) && Ee(dQ(i)) && h.data !== "nested") && (i._tTime = -m, i.render(Math.max(0, -c) || 0)), E && bt(dQ(i), E), i;
  }
  var B = Q.prototype;
  return B.render = function(t, P, e) {
    var i = this._time, s = this._tDur, v = this._dur, c = t < 0, n = t > s - m && !c ? s : t < m ? 0 : t, T, z, w, o, E, h, l, U;
    if (!v)
      he(this, t, P, e);
    else if (n !== this._tTime || !t || e || !this._initted && this._tTime || this._startAt && this._zTime < 0 !== c || this._lazy) {
      if (T = n, U = this.timeline, this._repeat) {
        if (o = v + this._rDelay, this._repeat < -1 && c)
          return this.totalTime(o * 100 + t, P, e);
        if (T = S(n % o), n === s ? (w = this._repeat, T = v) : (E = S(n / o), w = ~~E, w && w === E ? (T = v, w--) : T > v && (T = v)), h = this._yoyo && w & 1, h && (T = v - T), E = tB(this._tTime, o), T === i && !e && this._initted && w === E)
          return this._tTime = n, this;
        w !== E && this.vars.repeatRefresh && !h && !this._lock && T !== o && this._initted && (this._lock = e = 1, this.render(S(o * w), !0).invalidate()._lock = 0);
      }
      if (!this._initted) {
        if (gt(this, c ? t : T, e, P, n))
          return this._tTime = 0, this;
        if (i !== this._time && !(e && this.vars.repeatRefresh && w !== E))
          return this;
        if (v !== this._dur)
          return this.render(t, P, e);
      }
      if (this._rEase) {
        var D = T < i;
        if (D !== this._inv) {
          var k = D ? i : v - i;
          this._inv = D, this._from && (this.ratio = 1 - this.ratio), this._invRatio = this.ratio, this._invTime = i, this._invRecip = k ? (D ? -1 : 1) / k : 0, this._invScale = D ? -this.ratio : 1 - this.ratio, this._invEase = D ? this._rEase : this._ease;
        }
        this.ratio = l = this._invRatio + this._invScale * this._invEase((T - this._invTime) * this._invRecip);
      } else
        this.ratio = l = this._ease(T / v);
      if (this._from && (this.ratio = l = 1 - l), this._tTime = n, this._time = T, !this._act && this._ts && (this._act = 1, this._lazy = 0), !i && n && !P && !E && (oQ(this, "onStart"), this._tTime !== n))
        return this;
      for (z = this._pt; z; )
        z.r(l, z.d), z = z._next;
      U && U.render(t < 0 ? t : U._dur * U._ease(T / this._dur), P, e) || this._startAt && (this._zTime = t), this._onUpdate && !P && (c && cA(this, t, P, e), oQ(this, "onUpdate")), this._repeat && w !== E && this.vars.onRepeat && !P && this.parent && oQ(this, "onRepeat"), (n === this._tDur || !n) && this._tTime === n && (c && !this._onUpdate && cA(this, t, !0, !0), (t || !v) && (n === this._tDur && this._ts > 0 || !n && this._ts < 0) && bQ(this, 1), !P && !(c && !i) && (n || i || h) && (oQ(this, n === s ? "onComplete" : "onReverseComplete", !0), this._prom && !(n < s && this.timeScale() > 0) && this._prom()));
    }
    return this;
  }, B.targets = function() {
    return this._targets;
  }, B.invalidate = function(t) {
    return (!t || !this.vars.runBackwards) && (this._startAt = 0), this._pt = this._op = this._onUpdate = this._lazy = this.ratio = 0, this._ptLookup = [], this.timeline && this.timeline.invalidate(t), r.prototype.invalidate.call(this, t);
  }, B.resetTo = function(t, P, e, i, s) {
    kB || nQ.wake(), this._ts || this.play();
    var v = Math.min(this._dur, (this._dp._time - this._start) * this._ts), c;
    return this._initted || FA(this, v), c = this._ease(v / this._dur), me(this, t, P, e, i, c, v, s) ? this.resetTo(t, P, e, i, 1) : (IB(this, 0), this.parent || Gt(this._dp, this, "_first", "_last", this._dp._sort ? "_start" : 0), this.render(0));
  }, B.kill = function(t, P) {
    if (P === void 0 && (P = "all"), !t && (!P || P === "all"))
      return this._lazy = this._pt = 0, this.parent ? nB(this) : this.scrollTrigger && this.scrollTrigger.kill(!!q), this;
    if (this.timeline) {
      var e = this.timeline.totalDuration();
      return this.timeline.killTweensOf(t, P, HQ && HQ.vars.overwrite !== !0)._first || nB(this), this.parent && e !== this.timeline.totalDuration() && PB(this, this._dur * this.timeline._tDur / e, 0, 1), this;
    }
    var i = this._targets, s = t ? lQ(t) : i, v = this._ptLookup, c = this._pt, n, T, z, w, o, E, h;
    if ((!P || P === "all") && ze(i, s))
      return P === "all" && (this._pt = 0), nB(this);
    for (n = this._op = this._op || [], P !== "all" && (Z(P) && (o = {}, PQ(P, function(l) {
      return o[l] = 1;
    }), P = o), P = xe(i, P)), h = i.length; h--; )
      if (~s.indexOf(i[h])) {
        T = v[h], P === "all" ? (n[h] = P, w = T, z = {}) : (z = n[h] = n[h] || {}, w = P);
        for (o in w)
          E = T && T[o], E && ((!("kill" in E.d) || E.d.kill(o) === !0) && xB(this, E, "_pt"), delete T[o]), z !== "all" && (z[o] = 1);
      }
    return this._initted && !this._pt && c && nB(this), this;
  }, Q.to = function(t, P) {
    return new Q(t, P, arguments[2]);
  }, Q.from = function(t, P) {
    return zB(1, arguments);
  }, Q.delayedCall = function(t, P, e, i) {
    return new Q(P, 0, {
      immediateRender: !1,
      lazy: !1,
      overwrite: !1,
      delay: t,
      onComplete: P,
      onReverseComplete: P,
      onCompleteParams: e,
      onReverseCompleteParams: e,
      callbackScope: i
    });
  }, Q.fromTo = function(t, P, e) {
    return zB(2, arguments);
  }, Q.set = function(t, P) {
    return P.duration = 0, P.repeatDelay || (P.repeat = 0), new Q(t, P);
  }, Q.killTweensOf = function(t, P, e) {
    return Y.killTweensOf(t, P, e);
  }, Q;
}(UB);
EQ(K.prototype, {
  _targets: [],
  _lazy: 0,
  _startAt: 0,
  _op: 0,
  _onInit: 0
});
PQ("staggerTo,staggerFrom,staggerFromTo", function(r) {
  K[r] = function() {
    var Q = new AQ(), B = oA.call(arguments, 0);
    return B.splice(r === "staggerFromTo" ? 5 : 4, 0, 0), Q[r].apply(Q, B);
  };
});
var GA = function(Q, B, A) {
  return Q[B] = A;
}, QP = function(Q, B, A) {
  return Q[B](A);
}, Ie = function(Q, B, A, t) {
  return Q[B](t.fp, A);
}, Se = function(Q, B, A) {
  return Q.setAttribute(B, A);
}, fA = function(Q, B) {
  return J(Q[B]) ? QP : uA(Q[B]) && Q.setAttribute ? Se : GA;
}, BP = function(Q, B) {
  return B.set(B.t, B.p, Math.round((B.s + B.c * Q) * 1e6) / 1e6, B);
}, Ye = function(Q, B) {
  return B.set(B.t, B.p, !!(B.s + B.c * Q), B);
}, AP = function(Q, B) {
  var A = B._pt, t = "";
  if (!Q && B.b)
    t = B.b;
  else if (Q === 1 && B.e)
    t = B.e;
  else {
    for (; A; )
      t = A.p + (A.m ? A.m(A.s + A.c * Q) : Math.round((A.s + A.c * Q) * 1e4) / 1e4) + t, A = A._next;
    t += B.c;
  }
  B.set(B.t, B.p, t, B);
}, bA = function(Q, B) {
  for (var A = B._pt; A; )
    A.r(Q, A.d), A = A._next;
}, Ve = function(Q, B, A, t) {
  for (var P = this._pt, e; P; )
    e = P._next, P.p === t && P.modifier(Q, B, A), P = e;
}, Je = function(Q) {
  for (var B = this._pt, A, t; B; )
    t = B._next, B.p === Q && !B.op || B.op === Q ? xB(this, B, "_pt") : B.dep || (A = 1), B = t;
  return !A;
}, Re = function(Q, B, A, t) {
  t.mSet(Q, B, t.m.call(t.tween, A, t.mt), t);
}, tP = function(Q) {
  for (var B = Q._pt, A, t, P, e; B; ) {
    for (A = B._next, t = P; t && t.pr > B.pr; )
      t = t._next;
    (B._prev = t ? t._prev : e) ? B._prev._next = B : P = B, (B._next = t) ? t._prev = B : e = B, B = A;
  }
  Q._pt = P;
}, eQ = /* @__PURE__ */ function() {
  function r(B, A, t, P, e, i, s, v, c) {
    this.t = A, this.s = P, this.c = e, this.p = t, this.r = i || BP, this.d = s || this, this.set = v || GA, this.pr = c || 0, this._next = B, B && (B._prev = this);
  }
  var Q = r.prototype;
  return Q.modifier = function(A, t, P) {
    this.mSet = this.mSet || this.set, this.set = Re, this.m = A, this.mt = P, this.tween = t;
  }, r;
}();
PQ(LA + "parent,duration,ease,delay,overwrite,runBackwards,startAt,yoyo,immediateRender,repeat,repeatDelay,data,paused,reversed,lazy,callbackScope,stringFilter,id,yoyoEase,stagger,inherit,repeatRefresh,keyframes,autoRevert,scrollTrigger,easeReverse", function(r) {
  return OA[r] = 1;
});
wQ.TweenMax = wQ.TweenLite = K;
wQ.TimelineLite = wQ.TimelineMax = AQ;
Y = new AQ({
  sortChildren: !1,
  defaults: EB,
  autoRemoveChildren: !0,
  id: "root",
  smoothChildTiming: !0
});
zQ.stringFilter = Kt;
var RQ = [], HB = {}, Xe = [], Bt = 0, Ke = 0, WB = function(Q) {
  return (HB[Q] || Xe).map(function(B) {
    return B();
  });
}, EA = function() {
  var Q = Date.now(), B = [];
  Q - Bt > 2 && (WB("matchMediaInit"), RQ.forEach(function(A) {
    var t = A.queries, P = A.conditions, e, i, s, v;
    for (i in t)
      e = kQ.matchMedia(t[i]).matches, e && (s = 1), e !== P[i] && (P[i] = e, v = 1);
    v && (A.revert(), s && B.push(A));
  }), WB("matchMediaRevert"), B.forEach(function(A) {
    return A.onMatch(A, function(t) {
      return A.add(null, t);
    });
  }), Bt = Q, WB("matchMedia"));
}, PP = /* @__PURE__ */ function() {
  function r(B, A) {
    this.selector = A && TA(A), this.data = [], this._r = [], this.isReverted = !1, this.id = Ke++, B && this.add(B);
  }
  var Q = r.prototype;
  return Q.add = function(A, t, P) {
    J(A) && (P = t, t = A, A = J);
    var e = this, i = function() {
      var v = y, c = e.selector, n;
      return v && v !== e && v.data.push(e), P && (e.selector = TA(P)), y = e, n = t.apply(e, arguments), J(n) && e._r.push(n), y = v, e.selector = c, e.isReverted = !1, n;
    };
    return e.last = i, A === J ? i(e, function(s) {
      return e.add(null, s);
    }) : A ? e[A] = i : i;
  }, Q.ignore = function(A) {
    var t = y;
    y = null, A(this), y = t;
  }, Q.getTweens = function() {
    var A = [];
    return this.data.forEach(function(t) {
      return t instanceof r ? A.push.apply(A, t.getTweens()) : t instanceof K && !(t.parent && t.parent.data === "nested") && A.push(t);
    }), A;
  }, Q.clear = function() {
    this._r.length = this.data.length = 0;
  }, Q.kill = function(A, t) {
    var P = this;
    if (A ? function() {
      for (var i = P.getTweens(), s = P.data.length, v; s--; )
        v = P.data[s], v.data === "isFlip" && (v.revert(), v.getChildren(!0, !0, !1).forEach(function(c) {
          return i.splice(i.indexOf(c), 1);
        }));
      for (i.map(function(c) {
        return {
          g: c._dur || c._delay || c._sat && !c._sat.vars.immediateRender ? c.globalTime(0) : -1 / 0,
          t: c
        };
      }).sort(function(c, n) {
        return n.g - c.g || -1 / 0;
      }).forEach(function(c) {
        return c.t.revert(A);
      }), s = P.data.length; s--; )
        v = P.data[s], v instanceof AQ ? v.data !== "nested" && (v.scrollTrigger && v.scrollTrigger.revert(), v.kill()) : !(v instanceof K) && v.revert && v.revert(A);
      P._r.forEach(function(c) {
        return c(A, P);
      }), P.isReverted = !0;
    }() : this.data.forEach(function(i) {
      return i.kill && i.kill();
    }), this.clear(), t)
      for (var e = RQ.length; e--; )
        RQ[e].id === this.id && RQ.splice(e, 1);
  }, Q.revert = function(A) {
    this.kill(A || {});
  }, r;
}(), Ze = /* @__PURE__ */ function() {
  function r(B) {
    this.contexts = [], this.scope = B, y && y.data.push(this);
  }
  var Q = r.prototype;
  return Q.add = function(A, t, P) {
    uQ(A) || (A = {
      matches: A
    });
    var e = new PP(0, P || this.scope), i = e.conditions = {}, s, v, c;
    y && !e.selector && (e.selector = y.selector), this.contexts.push(e), t = e.add("onMatch", t), e.queries = A;
    for (v in A)
      v === "all" ? c = 1 : (s = kQ.matchMedia(A[v]), s && (RQ.indexOf(e) < 0 && RQ.push(e), (i[v] = s.matches) && (c = 1), s.addListener ? s.addListener(EA) : s.addEventListener("change", EA)));
    return c && t(e, function(n) {
      return e.add(null, n);
    }), this;
  }, Q.revert = function(A) {
    this.kill(A || {});
  }, Q.kill = function(A) {
    this.contexts.forEach(function(t) {
      return t.kill(A, !0);
    });
  }, r;
}(), gB = {
  registerPlugin: function() {
    for (var Q = arguments.length, B = new Array(Q), A = 0; A < Q; A++)
      B[A] = arguments[A];
    B.forEach(function(t) {
      return Jt(t);
    });
  },
  timeline: function(Q) {
    return new AQ(Q);
  },
  getTweensOf: function(Q, B) {
    return Y.getTweensOf(Q, B);
  },
  getProperty: function(Q, B, A, t) {
    Z(Q) && (Q = lQ(Q)[0]);
    var P = YQ(Q || {}).get, e = A ? Ft : jt;
    return A === "native" && (A = ""), Q && (B ? e((cQ[B] && cQ[B].get || P)(Q, B, A, t)) : function(i, s, v) {
      return e((cQ[i] && cQ[i].get || P)(Q, i, s, v));
    });
  },
  quickSetter: function(Q, B, A) {
    if (Q = lQ(Q), Q.length > 1) {
      var t = Q.map(function(c) {
        return rQ.quickSetter(c, B, A);
      }), P = t.length;
      return function(c) {
        for (var n = P; n--; )
          t[n](c);
      };
    }
    Q = Q[0] || {};
    var e = cQ[B], i = YQ(Q), s = i.harness && (i.harness.aliases || {})[B] || B, v = e ? function(c) {
      var n = new e();
      _Q._pt = 0, n.init(Q, A ? c + A : c, _Q, 0, [Q]), n.render(1, n), _Q._pt && bA(1, _Q);
    } : i.set(Q, s);
    return e ? v : function(c) {
      return v(Q, s, A ? c + A : c, i, 1);
    };
  },
  quickTo: function(Q, B, A) {
    var t, P = rQ.to(Q, EQ((t = {}, t[B] = "+=0.1", t.paused = !0, t.stagger = 0, t), A || {})), e = function(s, v, c) {
      return P.resetTo(B, s, v, c);
    };
    return e.tween = P, e;
  },
  isTweening: function(Q) {
    return Y.getTweensOf(Q, !0).length > 0;
  },
  defaults: function(Q) {
    return Q && Q.ease && (Q.ease = JQ(Q.ease, EB.ease)), WA(EB, Q || {});
  },
  config: function(Q) {
    return WA(zQ, Q || {});
  },
  registerEffect: function(Q) {
    var B = Q.name, A = Q.effect, t = Q.plugins, P = Q.defaults, e = Q.extendTimeline;
    (t || "").split(",").forEach(function(i) {
      return i && !cQ[i] && !wQ[i] && aB(B + " effect requires " + i + " plugin.");
    }), RB[B] = function(i, s, v) {
      return A(lQ(i), EQ(s || {}, P), v);
    }, e && (AQ.prototype[B] = function(i, s, v) {
      return this.add(RB[B](i, uQ(s) ? s : (v = s) && {}, this), v);
    });
  },
  registerEase: function(Q, B) {
    g[Q] = JQ(B);
  },
  parseEase: function(Q, B) {
    return arguments.length ? JQ(Q, B) : g;
  },
  getById: function(Q) {
    return Y.getById(Q);
  },
  exportRoot: function(Q, B) {
    Q === void 0 && (Q = {});
    var A = new AQ(Q), t, P;
    for (A.smoothChildTiming = tQ(Q.smoothChildTiming), Y.remove(A), A._dp = 0, A._time = A._tTime = Y._time, t = Y._first; t; )
      P = t._next, (B || !(!t._dur && t instanceof K && t.vars.onComplete === t._targets[0])) && UQ(A, t, t._start - t._delay), t = P;
    return UQ(Y, A, 0), A;
  },
  context: function(Q, B) {
    return Q ? new PP(Q, B) : y;
  },
  matchMedia: function(Q) {
    return new Ze(Q);
  },
  matchMediaRefresh: function() {
    return RQ.forEach(function(Q) {
      var B = Q.conditions, A, t;
      for (t in B)
        B[t] && (B[t] = !1, A = 1);
      A && Q.revert();
    }) || EA();
  },
  addEventListener: function(Q, B) {
    var A = HB[Q] || (HB[Q] = []);
    ~A.indexOf(B) || A.push(B);
  },
  removeEventListener: function(Q, B) {
    var A = HB[Q], t = A && A.indexOf(B);
    t >= 0 && A.splice(t, 1);
  },
  utils: {
    wrap: Ne,
    wrapYoyo: Oe,
    distribute: xt,
    random: It,
    snap: yt,
    normalize: Ce,
    getUnit: _,
    clamp: Ue,
    splitColor: Rt,
    toArray: lQ,
    selector: TA,
    mapRange: Yt,
    pipe: ue,
    unitize: de,
    interpolate: Le,
    shuffle: mt
  },
  install: Nt,
  effects: RB,
  ticker: nQ,
  updateRoot: AQ.updateRoot,
  plugins: cQ,
  globalTimeline: Y,
  core: {
    PropTween: eQ,
    globals: Ot,
    Tween: K,
    Timeline: AQ,
    Animation: UB,
    getCache: YQ,
    _removeLinkedListItem: xB,
    reverting: function() {
      return q;
    },
    context: function(Q) {
      return Q && y && (y.data.push(Q), Q._ctx = y), y;
    },
    suppressOverwrites: function(Q) {
      return DA = Q;
    }
  }
};
PQ("to,from,fromTo,delayedCall,set,killTweensOf", function(r) {
  return gB[r] = K[r];
});
nQ.add(AQ.updateRoot);
_Q = gB.to({}, {
  duration: 0
});
var We = function(Q, B) {
  for (var A = Q._pt; A && A.p !== B && A.op !== B && A.fp !== B; )
    A = A._next;
  return A;
}, qe = function(Q, B) {
  var A = Q._targets, t, P, e;
  for (t in B)
    for (P = A.length; P--; )
      e = Q._ptLookup[P][t], e && (e = e.d) && (e._pt && (e = We(e, t)), e && e.modifier && e.modifier(B[t], Q, A[P], t));
}, qB = function(Q, B) {
  return {
    name: Q,
    headless: 1,
    rawVars: 1,
    //don't pre-process function-based values or "random()" strings.
    init: function(t, P, e) {
      e._onInit = function(i) {
        var s, v;
        if (Z(P) && (s = {}, PQ(P, function(c) {
          return s[c] = 1;
        }), P = s), B) {
          s = {};
          for (v in P)
            s[v] = B(P[v]);
          P = s;
        }
        qe(i, P);
      };
    }
  };
}, rQ = gB.registerPlugin({
  name: "attr",
  init: function(Q, B, A, t, P) {
    var e, i, s;
    this.tween = A;
    for (e in B)
      s = Q.getAttribute(e) || "", i = this.add(Q, "setAttribute", (s || 0) + "", B[e], t, P, 0, 0, e), i.op = e, i.b = s, this._props.push(e);
  },
  render: function(Q, B) {
    for (var A = B._pt; A; )
      q ? A.set(A.t, A.p, A.b, A) : A.r(Q, A.d), A = A._next;
  }
}, {
  name: "endArray",
  headless: 1,
  init: function(Q, B) {
    for (var A = B.length; A--; )
      this.add(Q, A, Q[A] || 0, B[A], 0, 0, 0, 0, 0, 1);
  }
}, qB("roundProps", zA), qB("modifiers"), qB("snap", yt)) || gB;
K.version = AQ.version = rQ.version = "3.15.0";
Ct = 1;
dA() && eB();
g.Power0;
g.Power1;
g.Power2;
g.Power3;
g.Power4;
g.Linear;
g.Quad;
g.Cubic;
g.Quart;
g.Quint;
g.Strong;
g.Elastic;
g.Back;
g.SteppedEase;
g.Bounce;
g.Sine;
g.Expo;
g.Circ;
/*!
 * CSSPlugin 3.15.0
 * https://gsap.com
 *
 * Copyright 2008-2026, GreenSock. All rights reserved.
 * Subject to the terms at https://gsap.com/standard-license
 * @author: Jack Doyle, jack@greensock.com
*/
var At, jQ, QB, gA, SQ, tt, pA, _e = function() {
  return typeof window < "u";
}, OQ = {}, IQ = 180 / Math.PI, BB = Math.PI / 180, KQ = Math.atan2, Pt = 1e8, mA = /([A-Z])/g, $e = /(left|right|width|margin|padding|x)/i, Qi = /[\s,\(]\S/, DQ = {
  autoAlpha: "opacity,visibility",
  scale: "scaleX,scaleY",
  alpha: "opacity"
}, aA = function(Q, B) {
  return B.set(B.t, B.p, Math.round((B.s + B.c * Q) * 1e4) / 1e4 + B.u, B);
}, Bi = function(Q, B) {
  return B.set(B.t, B.p, Q === 1 ? B.e : Math.round((B.s + B.c * Q) * 1e4) / 1e4 + B.u, B);
}, Ai = function(Q, B) {
  return B.set(B.t, B.p, Q ? Math.round((B.s + B.c * Q) * 1e4) / 1e4 + B.u : B.b, B);
}, ti = function(Q, B) {
  return B.set(B.t, B.p, Q === 1 ? B.e : Q ? Math.round((B.s + B.c * Q) * 1e4) / 1e4 + B.u : B.b, B);
}, Pi = function(Q, B) {
  var A = B.s + B.c * Q;
  B.set(B.t, B.p, ~~(A + (A < 0 ? -0.5 : 0.5)) + B.u, B);
}, eP = function(Q, B) {
  return B.set(B.t, B.p, Q ? B.e : B.b, B);
}, iP = function(Q, B) {
  return B.set(B.t, B.p, Q !== 1 ? B.b : B.e, B);
}, ei = function(Q, B, A) {
  return Q.style[B] = A;
}, ii = function(Q, B, A) {
  return Q.style.setProperty(B, A);
}, ri = function(Q, B, A) {
  return Q._gsap[B] = A;
}, si = function(Q, B, A) {
  return Q._gsap.scaleX = Q._gsap.scaleY = A;
}, vi = function(Q, B, A, t, P) {
  var e = Q._gsap;
  e.scaleX = e.scaleY = A, e.renderTransform(P, e);
}, ci = function(Q, B, A, t, P) {
  var e = Q._gsap;
  e[B] = A, e.renderTransform(P, e);
}, V = "transform", iQ = V + "Origin", ni = function r(Q, B) {
  var A = this, t = this.target, P = t.style, e = t._gsap;
  if (Q in OQ && P) {
    if (this.tfm = this.tfm || {}, Q !== "transform")
      Q = DQ[Q] || Q, ~Q.indexOf(",") ? Q.split(",").forEach(function(i) {
        return A.tfm[i] = CQ(t, i);
      }) : this.tfm[Q] = e.x ? e[Q] : CQ(t, Q), Q === iQ && (this.tfm.zOrigin = e.zOrigin);
    else
      return DQ.transform.split(",").forEach(function(i) {
        return r.call(A, i, B);
      });
    if (this.props.indexOf(V) >= 0)
      return;
    e.svg && (this.svgo = t.getAttribute("data-svg-origin"), this.props.push(iQ, B, "")), Q = V;
  }
  (P || B) && this.props.push(Q, B, P[Q]);
}, rP = function(Q) {
  Q.translate && (Q.removeProperty("translate"), Q.removeProperty("scale"), Q.removeProperty("rotate"));
}, oi = function() {
  var Q = this.props, B = this.target, A = B.style, t = B._gsap, P, e;
  for (P = 0; P < Q.length; P += 3)
    Q[P + 1] ? Q[P + 1] === 2 ? B[Q[P]](Q[P + 2]) : B[Q[P]] = Q[P + 2] : Q[P + 2] ? A[Q[P]] = Q[P + 2] : A.removeProperty(Q[P].substr(0, 2) === "--" ? Q[P] : Q[P].replace(mA, "-$1").toLowerCase());
  if (this.tfm) {
    for (e in this.tfm)
      t[e] = this.tfm[e];
    t.svg && (t.renderTransform(), B.setAttribute("data-svg-origin", this.svgo || "")), P = pA(), (!P || !P.isStart) && !A[V] && (rP(A), t.zOrigin && A[iQ] && (A[iQ] += " " + t.zOrigin + "px", t.zOrigin = 0, t.renderTransform()), t.uncache = 1);
  }
}, sP = function(Q, B) {
  var A = {
    target: Q,
    props: [],
    revert: oi,
    save: ni
  };
  return Q._gsap || rQ.core.getCache(Q), B && Q.style && Q.nodeType && B.split(",").forEach(function(t) {
    return A.save(t);
  }), A;
}, vP, hA = function(Q, B) {
  var A = jQ.createElementNS ? jQ.createElementNS((B || "http://www.w3.org/1999/xhtml").replace(/^https/, "http"), Q) : jQ.createElement(Q);
  return A && A.style ? A : jQ.createElement(Q);
}, TQ = function r(Q, B, A) {
  var t = getComputedStyle(Q);
  return t[B] || t.getPropertyValue(B.replace(mA, "-$1").toLowerCase()) || t.getPropertyValue(B) || !A && r(Q, iB(B) || B, 1) || "";
}, et = "O,Moz,ms,Ms,Webkit".split(","), iB = function(Q, B, A) {
  var t = B || SQ, P = t.style, e = 5;
  if (Q in P && !A)
    return Q;
  for (Q = Q.charAt(0).toUpperCase() + Q.substr(1); e-- && !(et[e] + Q in P); )
    ;
  return e < 0 ? null : (e === 3 ? "ms" : e >= 0 ? et[e] : "") + Q;
}, lA = function() {
  _e() && window.document && (At = window, jQ = At.document, QB = jQ.documentElement, SQ = hA("div") || {
    style: {}
  }, hA("div"), V = iB(V), iQ = V + "Origin", SQ.style.cssText = "border-width:0;line-height:0;position:absolute;padding:0", vP = !!iB("perspective"), pA = rQ.core.reverting, gA = 1);
}, it = function(Q) {
  var B = Q.ownerSVGElement, A = hA("svg", B && B.getAttribute("xmlns") || "http://www.w3.org/2000/svg"), t = Q.cloneNode(!0), P;
  t.style.display = "block", A.appendChild(t), QB.appendChild(A);
  try {
    P = t.getBBox();
  } catch {
  }
  return A.removeChild(t), QB.removeChild(A), P;
}, rt = function(Q, B) {
  for (var A = B.length; A--; )
    if (Q.hasAttribute(B[A]))
      return Q.getAttribute(B[A]);
}, cP = function(Q) {
  var B, A;
  try {
    B = Q.getBBox();
  } catch {
    B = it(Q), A = 1;
  }
  return B && (B.width || B.height) || A || (B = it(Q)), B && !B.width && !B.x && !B.y ? {
    x: +rt(Q, ["x", "cx", "x1"]) || 0,
    y: +rt(Q, ["y", "cy", "y1"]) || 0,
    width: 0,
    height: 0
  } : B;
}, nP = function(Q) {
  return !!(Q.getCTM && (!Q.parentNode || Q.ownerSVGElement) && cP(Q));
}, gQ = function(Q, B) {
  if (B) {
    var A = Q.style, t;
    B in OQ && B !== iQ && (B = V), A.removeProperty ? (t = B.substr(0, 2), (t === "ms" || B.substr(0, 6) === "webkit") && (B = "-" + B), A.removeProperty(t === "--" ? B : B.replace(mA, "-$1").toLowerCase())) : A.removeAttribute(B);
  }
}, FQ = function(Q, B, A, t, P, e) {
  var i = new eQ(Q._pt, B, A, 0, 1, e ? iP : eP);
  return Q._pt = i, i.b = t, i.e = P, Q._props.push(A), i;
}, st = {
  deg: 1,
  rad: 1,
  turn: 1
}, Ti = {
  grid: 1,
  flex: 1
}, pQ = function r(Q, B, A, t) {
  var P = parseFloat(A) || 0, e = (A + "").trim().substr((P + "").length) || "px", i = SQ.style, s = $e.test(B), v = Q.tagName.toLowerCase() === "svg", c = (v ? "client" : "offset") + (s ? "Width" : "Height"), n = 100, T = t === "px", z = t === "%", w, o, E, h;
  if (t === e || !P || st[t] || st[e])
    return P;
  if (e !== "px" && !T && (P = r(Q, B, A, "px")), h = Q.getCTM && nP(Q), (z || e === "%") && (OQ[B] || ~B.indexOf("adius")))
    return w = h ? Q.getBBox()[s ? "width" : "height"] : Q[c], X(z ? P / w * n : P / 100 * w);
  if (i[s ? "width" : "height"] = n + (T ? e : t), o = t !== "rem" && ~B.indexOf("adius") || t === "em" && Q.appendChild && !v ? Q : Q.parentNode, h && (o = (Q.ownerSVGElement || {}).parentNode), (!o || o === jQ || !o.appendChild) && (o = jQ.body), E = o._gsap, E && z && E.width && s && E.time === nQ.time && !E.uncache)
    return X(P / E.width * n);
  if (z && (B === "height" || B === "width")) {
    var l = Q.style[B];
    Q.style[B] = n + t, w = Q[c], l ? Q.style[B] = l : gQ(Q, B);
  } else
    (z || e === "%") && !Ti[TQ(o, "display")] && (i.position = TQ(Q, "position")), o === Q && (i.position = "static"), o.appendChild(SQ), w = SQ[c], o.removeChild(SQ), i.position = "absolute";
  return s && z && (E = YQ(o), E.time = nQ.time, E.width = o[c]), X(T ? w * P / n : w && P ? n / w * P : 0);
}, CQ = function(Q, B, A, t) {
  var P;
  return gA || lA(), B in DQ && B !== "transform" && (B = DQ[B], ~B.indexOf(",") && (B = B.split(",")[0])), OQ[B] && B !== "transform" ? (P = uB(Q, t), P = B !== "transformOrigin" ? P[B] : P.svg ? P.origin : mB(TQ(Q, iQ)) + " " + P.zOrigin + "px") : (P = Q.style[B], (!P || P === "auto" || t || ~(P + "").indexOf("calc(")) && (P = pB[B] && pB[B](Q, B, A) || TQ(Q, B) || Mt(Q, B) || (B === "opacity" ? 1 : 0))), A && !~(P + "").trim().indexOf(" ") ? pQ(Q, B, P, A) + A : P;
}, zi = function(Q, B, A, t) {
  if (!A || A === "none") {
    var P = iB(B, Q, 1), e = P && TQ(Q, P, 1);
    e && e !== A ? (B = P, A = e) : B === "borderColor" && (A = TQ(Q, "borderTopColor"));
  }
  var i = new eQ(this._pt, Q.style, B, 0, 1, AP), s = 0, v = 0, c, n, T, z, w, o, E, h, l, U, D, k;
  if (i.b = A, i.e = t, A += "", t += "", t.substring(0, 6) === "var(--" && (t = TQ(Q, t.substring(4, t.indexOf(")")))), t === "auto" && (o = Q.style[B], Q.style[B] = t, t = TQ(Q, B) || t, o ? Q.style[B] = o : gQ(Q, B)), c = [A, t], Kt(c), A = c[0], t = c[1], T = A.match(qQ) || [], k = t.match(qQ) || [], k.length) {
    for (; n = qQ.exec(t); )
      E = n[0], l = t.substring(s, n.index), w ? w = (w + 1) % 5 : (l.substr(-5) === "rgba(" || l.substr(-5) === "hsla(") && (w = 1), E !== (o = T[v++] || "") && (z = parseFloat(o) || 0, D = o.substr((z + "").length), E.charAt(1) === "=" && (E = $Q(z, E) + D), h = parseFloat(E), U = E.substr((h + "").length), s = qQ.lastIndex - U.length, U || (U = U || zQ.units[B] || D, s === t.length && (t += U, i.e += U)), D !== U && (z = pQ(Q, B, o, U) || 0), i._pt = {
        _next: i._pt,
        p: l || v === 1 ? l : ",",
        //note: SVG spec allows omission of comma/space when a negative sign is wedged between two numbers, like 2.5-5.3 instead of 2.5,-5.3 but when tweening, the negative value may switch to positive, so we insert the comma just in case.
        s: z,
        c: h - z,
        m: w && w < 4 || B === "zIndex" ? Math.round : 0
      });
    i.c = s < t.length ? t.substring(s, t.length) : "";
  } else
    i.r = B === "display" && t === "none" ? iP : eP;
  return dt.test(t) && (i.e = 0), this._pt = i, i;
}, vt = {
  top: "0%",
  bottom: "100%",
  left: "0%",
  right: "100%",
  center: "50%"
}, wi = function(Q) {
  var B = Q.split(" "), A = B[0], t = B[1] || "50%";
  return (A === "top" || A === "bottom" || t === "left" || t === "right") && (Q = A, A = t, t = Q), B[0] = vt[A] || A, B[1] = vt[t] || t, B.join(" ");
}, Ei = function(Q, B) {
  if (B.tween && B.tween._time === B.tween._dur) {
    var A = B.t, t = A.style, P = B.u, e = A._gsap, i, s, v;
    if (P === "all" || P === !0)
      t.cssText = "", s = 1;
    else
      for (P = P.split(","), v = P.length; --v > -1; )
        i = P[v], OQ[i] && (s = 1, i = i === "transformOrigin" ? iQ : V), gQ(A, i);
    s && (gQ(A, V), e && (e.svg && A.removeAttribute("transform"), t.scale = t.rotate = t.translate = "none", uB(A, 1), e.uncache = 1, rP(t)));
  }
}, pB = {
  clearProps: function(Q, B, A, t, P) {
    if (P.data !== "isFromStart") {
      var e = Q._pt = new eQ(Q._pt, B, A, 0, 0, Ei);
      return e.u = t, e.pr = -10, e.tween = P, Q._props.push(A), 1;
    }
  }
  /* className feature (about 0.4kb gzipped).
  , className(plugin, target, property, endValue, tween) {
  	let _renderClassName = (ratio, data) => {
  			data.css.render(ratio, data.css);
  			if (!ratio || ratio === 1) {
  				let inline = data.rmv,
  					target = data.t,
  					p;
  				target.setAttribute("class", ratio ? data.e : data.b);
  				for (p in inline) {
  					_removeProperty(target, p);
  				}
  			}
  		},
  		_getAllStyles = (target) => {
  			let styles = {},
  				computed = getComputedStyle(target),
  				p;
  			for (p in computed) {
  				if (isNaN(p) && p !== "cssText" && p !== "length") {
  					styles[p] = computed[p];
  				}
  			}
  			_setDefaults(styles, _parseTransform(target, 1));
  			return styles;
  		},
  		startClassList = target.getAttribute("class"),
  		style = target.style,
  		cssText = style.cssText,
  		cache = target._gsap,
  		classPT = cache.classPT,
  		inlineToRemoveAtEnd = {},
  		data = {t:target, plugin:plugin, rmv:inlineToRemoveAtEnd, b:startClassList, e:(endValue.charAt(1) !== "=") ? endValue : startClassList.replace(new RegExp("(?:\\s|^)" + endValue.substr(2) + "(?![\\w-])"), "") + ((endValue.charAt(0) === "+") ? " " + endValue.substr(2) : "")},
  		changingVars = {},
  		startVars = _getAllStyles(target),
  		transformRelated = /(transform|perspective)/i,
  		endVars, p;
  	if (classPT) {
  		classPT.r(1, classPT.d);
  		_removeLinkedListItem(classPT.d.plugin, classPT, "_pt");
  	}
  	target.setAttribute("class", data.e);
  	endVars = _getAllStyles(target, true);
  	target.setAttribute("class", startClassList);
  	for (p in endVars) {
  		if (endVars[p] !== startVars[p] && !transformRelated.test(p)) {
  			changingVars[p] = endVars[p];
  			if (!style[p] && style[p] !== "0") {
  				inlineToRemoveAtEnd[p] = 1;
  			}
  		}
  	}
  	cache.classPT = plugin._pt = new PropTween(plugin._pt, target, "className", 0, 0, _renderClassName, data, 0, -11);
  	if (style.cssText !== cssText) { //only apply if things change. Otherwise, in cases like a background-image that's pulled dynamically, it could cause a refresh. See https://gsap.com/forums/topic/20368-possible-gsap-bug-switching-classnames-in-chrome/.
  		style.cssText = cssText; //we recorded cssText before we swapped classes and ran _getAllStyles() because in cases when a className tween is overwritten, we remove all the related tweening properties from that class change (otherwise class-specific stuff can't override properties we've directly set on the target's style object due to specificity).
  	}
  	_parseTransform(target, true); //to clear the caching of transforms
  	data.css = new gsap.plugins.css();
  	data.css.init(target, changingVars, tween);
  	plugin._props.push(...data.css._props);
  	return 1;
  }
  */
}, DB = [1, 0, 0, 1, 0, 0], oP = {}, TP = function(Q) {
  return Q === "matrix(1, 0, 0, 1, 0, 0)" || Q === "none" || !Q;
}, ct = function(Q) {
  var B = TQ(Q, V);
  return TP(B) ? DB : B.substr(7).match(ut).map(X);
}, xA = function(Q, B) {
  var A = Q._gsap || YQ(Q), t = Q.style, P = ct(Q), e, i, s, v;
  return A.svg && Q.getAttribute("transform") ? (s = Q.transform.baseVal.consolidate().matrix, P = [s.a, s.b, s.c, s.d, s.e, s.f], P.join(",") === "1,0,0,1,0,0" ? DB : P) : (P === DB && !Q.offsetParent && Q !== QB && !A.svg && (s = t.display, t.display = "block", e = Q.parentNode, (!e || !Q.offsetParent && !Q.getBoundingClientRect().width) && (v = 1, i = Q.nextElementSibling, QB.appendChild(Q)), P = ct(Q), s ? t.display = s : gQ(Q, "display"), v && (i ? e.insertBefore(Q, i) : e ? e.appendChild(Q) : QB.removeChild(Q))), B && P.length > 6 ? [P[0], P[1], P[4], P[5], P[12], P[13]] : P);
}, kA = function(Q, B, A, t, P, e) {
  var i = Q._gsap, s = P || xA(Q, !0), v = i.xOrigin || 0, c = i.yOrigin || 0, n = i.xOffset || 0, T = i.yOffset || 0, z = s[0], w = s[1], o = s[2], E = s[3], h = s[4], l = s[5], U = B.split(" "), D = parseFloat(U[0]) || 0, k = parseFloat(U[1]) || 0, O, u, N, d;
  A ? s !== DB && (u = z * E - w * o) && (N = D * (E / u) + k * (-o / u) + (o * l - E * h) / u, d = D * (-w / u) + k * (z / u) - (z * l - w * h) / u, D = N, k = d) : (O = cP(Q), D = O.x + (~U[0].indexOf("%") ? D / 100 * O.width : D), k = O.y + (~(U[1] || U[0]).indexOf("%") ? k / 100 * O.height : k)), t || t !== !1 && i.smooth ? (h = D - v, l = k - c, i.xOffset = n + (h * z + l * o) - h, i.yOffset = T + (h * w + l * E) - l) : i.xOffset = i.yOffset = 0, i.xOrigin = D, i.yOrigin = k, i.smooth = !!t, i.origin = B, i.originIsAbsolute = !!A, Q.style[iQ] = "0px 0px", e && (FQ(e, i, "xOrigin", v, D), FQ(e, i, "yOrigin", c, k), FQ(e, i, "xOffset", n, i.xOffset), FQ(e, i, "yOffset", T, i.yOffset)), Q.setAttribute("data-svg-origin", D + " " + k);
}, uB = function(Q, B) {
  var A = Q._gsap || new Wt(Q);
  if ("x" in A && !B && !A.uncache)
    return A;
  var t = Q.style, P = A.scaleX < 0, e = "px", i = "deg", s = getComputedStyle(Q), v = TQ(Q, iQ) || "0", c, n, T, z, w, o, E, h, l, U, D, k, O, u, N, d, M, b, f, C, j, F, H, G, I, R, QQ, sQ, W, x, BQ, vQ;
  return c = n = T = o = E = h = l = U = D = 0, z = w = 1, A.svg = !!(Q.getCTM && nP(Q)), s.translate && ((s.translate !== "none" || s.scale !== "none" || s.rotate !== "none") && (t[V] = (s.translate !== "none" ? "translate3d(" + (s.translate + " 0 0").split(" ").slice(0, 3).join(", ") + ") " : "") + (s.rotate !== "none" ? "rotate(" + s.rotate + ") " : "") + (s.scale !== "none" ? "scale(" + s.scale.split(" ").join(",") + ") " : "") + (s[V] !== "none" ? s[V] : "")), t.scale = t.rotate = t.translate = "none"), u = xA(Q, A.svg), A.svg && (A.uncache ? (I = Q.getBBox(), v = A.xOrigin - I.x + "px " + (A.yOrigin - I.y) + "px", G = "") : G = !B && Q.getAttribute("data-svg-origin"), kA(Q, G || v, !!G || A.originIsAbsolute, A.smooth !== !1, u)), k = A.xOrigin || 0, O = A.yOrigin || 0, u !== DB && (b = u[0], f = u[1], C = u[2], j = u[3], c = F = u[4], n = H = u[5], u.length === 6 ? (z = Math.sqrt(b * b + f * f), w = Math.sqrt(j * j + C * C), o = b || f ? KQ(f, b) * IQ : 0, l = C || j ? KQ(C, j) * IQ + o : 0, l && (w *= Math.abs(Math.cos(l * BB))), A.svg && (c -= k - (k * b + O * C), n -= O - (k * f + O * j))) : (vQ = u[6], x = u[7], QQ = u[8], sQ = u[9], W = u[10], BQ = u[11], c = u[12], n = u[13], T = u[14], N = KQ(vQ, W), E = N * IQ, N && (d = Math.cos(-N), M = Math.sin(-N), G = F * d + QQ * M, I = H * d + sQ * M, R = vQ * d + W * M, QQ = F * -M + QQ * d, sQ = H * -M + sQ * d, W = vQ * -M + W * d, BQ = x * -M + BQ * d, F = G, H = I, vQ = R), N = KQ(-C, W), h = N * IQ, N && (d = Math.cos(-N), M = Math.sin(-N), G = b * d - QQ * M, I = f * d - sQ * M, R = C * d - W * M, BQ = j * M + BQ * d, b = G, f = I, C = R), N = KQ(f, b), o = N * IQ, N && (d = Math.cos(N), M = Math.sin(N), G = b * d + f * M, I = F * d + H * M, f = f * d - b * M, H = H * d - F * M, b = G, F = I), E && Math.abs(E) + Math.abs(o) > 359.9 && (E = o = 0, h = 180 - h), z = X(Math.sqrt(b * b + f * f + C * C)), w = X(Math.sqrt(H * H + vQ * vQ)), N = KQ(F, H), l = Math.abs(N) > 2e-4 ? N * IQ : 0, D = BQ ? 1 / (BQ < 0 ? -BQ : BQ) : 0), A.svg && (G = Q.getAttribute("transform"), A.forceCSS = Q.setAttribute("transform", "") || !TP(TQ(Q, V)), G && Q.setAttribute("transform", G))), Math.abs(l) > 90 && Math.abs(l) < 270 && (P ? (z *= -1, l += o <= 0 ? 180 : -180, o += o <= 0 ? 180 : -180) : (w *= -1, l += l <= 0 ? 180 : -180)), B = B || A.uncache, A.x = c - ((A.xPercent = c && (!B && A.xPercent || (Math.round(Q.offsetWidth / 2) === Math.round(-c) ? -50 : 0))) ? Q.offsetWidth * A.xPercent / 100 : 0) + e, A.y = n - ((A.yPercent = n && (!B && A.yPercent || (Math.round(Q.offsetHeight / 2) === Math.round(-n) ? -50 : 0))) ? Q.offsetHeight * A.yPercent / 100 : 0) + e, A.z = T + e, A.scaleX = X(z), A.scaleY = X(w), A.rotation = X(o) + i, A.rotationX = X(E) + i, A.rotationY = X(h) + i, A.skewX = l + i, A.skewY = U + i, A.transformPerspective = D + e, (A.zOrigin = parseFloat(v.split(" ")[2]) || !B && A.zOrigin || 0) && (t[iQ] = mB(v)), A.xOffset = A.yOffset = 0, A.force3D = zQ.force3D, A.renderTransform = A.svg ? hi : vP ? zP : ai, A.uncache = 0, A;
}, mB = function(Q) {
  return (Q = Q.split(" "))[0] + " " + Q[1];
}, _B = function(Q, B, A) {
  var t = _(B);
  return X(parseFloat(B) + parseFloat(pQ(Q, "x", A + "px", t))) + t;
}, ai = function(Q, B) {
  B.z = "0px", B.rotationY = B.rotationX = "0deg", B.force3D = 0, zP(Q, B);
}, xQ = "0deg", sB = "0px", yQ = ") ", zP = function(Q, B) {
  var A = B || this, t = A.xPercent, P = A.yPercent, e = A.x, i = A.y, s = A.z, v = A.rotation, c = A.rotationY, n = A.rotationX, T = A.skewX, z = A.skewY, w = A.scaleX, o = A.scaleY, E = A.transformPerspective, h = A.force3D, l = A.target, U = A.zOrigin, D = "", k = h === "auto" && Q && Q !== 1 || h === !0;
  if (U && (n !== xQ || c !== xQ)) {
    var O = parseFloat(c) * BB, u = Math.sin(O), N = Math.cos(O), d;
    O = parseFloat(n) * BB, d = Math.cos(O), e = _B(l, e, u * d * -U), i = _B(l, i, -Math.sin(O) * -U), s = _B(l, s, N * d * -U + U);
  }
  E !== sB && (D += "perspective(" + E + yQ), (t || P) && (D += "translate(" + t + "%, " + P + "%) "), (k || e !== sB || i !== sB || s !== sB) && (D += s !== sB || k ? "translate3d(" + e + ", " + i + ", " + s + ") " : "translate(" + e + ", " + i + yQ), v !== xQ && (D += "rotate(" + v + yQ), c !== xQ && (D += "rotateY(" + c + yQ), n !== xQ && (D += "rotateX(" + n + yQ), (T !== xQ || z !== xQ) && (D += "skew(" + T + ", " + z + yQ), (w !== 1 || o !== 1) && (D += "scale(" + w + ", " + o + yQ), l.style[V] = D || "translate(0, 0)";
}, hi = function(Q, B) {
  var A = B || this, t = A.xPercent, P = A.yPercent, e = A.x, i = A.y, s = A.rotation, v = A.skewX, c = A.skewY, n = A.scaleX, T = A.scaleY, z = A.target, w = A.xOrigin, o = A.yOrigin, E = A.xOffset, h = A.yOffset, l = A.forceCSS, U = parseFloat(e), D = parseFloat(i), k, O, u, N, d;
  s = parseFloat(s), v = parseFloat(v), c = parseFloat(c), c && (c = parseFloat(c), v += c, s += c), s || v ? (s *= BB, v *= BB, k = Math.cos(s) * n, O = Math.sin(s) * n, u = Math.sin(s - v) * -T, N = Math.cos(s - v) * T, v && (c *= BB, d = Math.tan(v - c), d = Math.sqrt(1 + d * d), u *= d, N *= d, c && (d = Math.tan(c), d = Math.sqrt(1 + d * d), k *= d, O *= d)), k = X(k), O = X(O), u = X(u), N = X(N)) : (k = n, N = T, O = u = 0), (U && !~(e + "").indexOf("px") || D && !~(i + "").indexOf("px")) && (U = pQ(z, "x", e, "px"), D = pQ(z, "y", i, "px")), (w || o || E || h) && (U = X(U + w - (w * k + o * u) + E), D = X(D + o - (w * O + o * N) + h)), (t || P) && (d = z.getBBox(), U = X(U + t / 100 * d.width), D = X(D + P / 100 * d.height)), d = "matrix(" + k + "," + O + "," + u + "," + N + "," + U + "," + D + ")", z.setAttribute("transform", d), l && (z.style[V] = d);
}, li = function(Q, B, A, t, P) {
  var e = 360, i = Z(P), s = parseFloat(P) * (i && ~P.indexOf("rad") ? IQ : 1), v = s - t, c = t + v + "deg", n, T;
  return i && (n = P.split("_")[1], n === "short" && (v %= e, v !== v % (e / 2) && (v += v < 0 ? e : -e)), n === "cw" && v < 0 ? v = (v + e * Pt) % e - ~~(v / e) * e : n === "ccw" && v > 0 && (v = (v - e * Pt) % e - ~~(v / e) * e)), Q._pt = T = new eQ(Q._pt, B, A, t, v, Bi), T.e = c, T.u = "deg", Q._props.push(A), T;
}, nt = function(Q, B) {
  for (var A in B)
    Q[A] = B[A];
  return Q;
}, ki = function(Q, B, A) {
  var t = nt({}, A._gsap), P = "perspective,force3D,transformOrigin,svgOrigin", e = A.style, i, s, v, c, n, T, z, w;
  t.svg ? (v = A.getAttribute("transform"), A.setAttribute("transform", ""), e[V] = B, i = uB(A, 1), gQ(A, V), A.setAttribute("transform", v)) : (v = getComputedStyle(A)[V], e[V] = B, i = uB(A, 1), e[V] = v);
  for (s in OQ)
    v = t[s], c = i[s], v !== c && P.indexOf(s) < 0 && (z = _(v), w = _(c), n = z !== w ? pQ(A, s, v, w) : parseFloat(v), T = parseFloat(c), Q._pt = new eQ(Q._pt, i, s, n, T - n, aA), Q._pt.u = w || 0, Q._props.push(s));
  nt(i, t);
};
PQ("padding,margin,Width,Radius", function(r, Q) {
  var B = "Top", A = "Right", t = "Bottom", P = "Left", e = (Q < 3 ? [B, A, t, P] : [B + P, B + A, t + A, t + P]).map(function(i) {
    return Q < 2 ? r + i : "border" + i + r;
  });
  pB[Q > 1 ? "border" + r : r] = function(i, s, v, c, n) {
    var T, z;
    if (arguments.length < 4)
      return T = e.map(function(w) {
        return CQ(i, w, v);
      }), z = T.join(" "), z.split(T[0]).length === 5 ? T[0] : z;
    T = (c + "").split(" "), z = {}, e.forEach(function(w, o) {
      return z[w] = T[o] = T[o] || T[(o - 1) / 2 | 0];
    }), i.init(s, z, n);
  };
});
var wP = {
  name: "css",
  register: lA,
  targetTest: function(Q) {
    return Q.style && Q.nodeType;
  },
  init: function(Q, B, A, t, P) {
    var e = this._props, i = Q.style, s = A.vars.startAt, v, c, n, T, z, w, o, E, h, l, U, D, k, O, u, N, d;
    gA || lA(), this.styles = this.styles || sP(Q), N = this.styles.props, this.tween = A;
    for (o in B)
      if (o !== "autoRound" && (c = B[o], !(cQ[o] && qt(o, B, A, t, Q, P)))) {
        if (z = typeof c, w = pB[o], z === "function" && (c = c.call(A, t, Q, P), z = typeof c), z === "string" && ~c.indexOf("random(") && (c = lB(c)), w)
          w(this, Q, o, c, A) && (u = 1);
        else if (o.substr(0, 2) === "--")
          v = (getComputedStyle(Q).getPropertyValue(o) + "").trim(), c += "", fQ.lastIndex = 0, fQ.test(v) || (E = _(v), h = _(c), h ? E !== h && (v = pQ(Q, o, v, h) + h) : E && (c += E)), this.add(i, "setProperty", v, c, t, P, 0, 0, o), e.push(o), N.push(o, 0, i[o]);
        else if (z !== "undefined") {
          if (s && o in s ? (v = typeof s[o] == "function" ? s[o].call(A, t, Q, P) : s[o], Z(v) && ~v.indexOf("random(") && (v = lB(v)), _(v + "") || v === "auto" || (v += zQ.units[o] || _(CQ(Q, o)) || ""), (v + "").charAt(1) === "=" && (v = CQ(Q, o))) : v = CQ(Q, o), T = parseFloat(v), l = z === "string" && c.charAt(1) === "=" && c.substr(0, 2), l && (c = c.substr(2)), n = parseFloat(c), o in DQ && (o === "autoAlpha" && (T === 1 && CQ(Q, "visibility") === "hidden" && n && (T = 0), N.push("visibility", 0, i.visibility), FQ(this, i, "visibility", T ? "inherit" : "hidden", n ? "inherit" : "hidden", !n)), o !== "scale" && o !== "transform" && (o = DQ[o], ~o.indexOf(",") && (o = o.split(",")[0]))), U = o in OQ, U) {
            if (this.styles.save(o), d = c, z === "string" && c.substring(0, 6) === "var(--") {
              if (c = TQ(Q, c.substring(4, c.indexOf(")"))), c.substring(0, 5) === "calc(") {
                var M = Q.style.perspective;
                Q.style.perspective = c, c = TQ(Q, "perspective"), M ? Q.style.perspective = M : gQ(Q, "perspective");
              }
              n = parseFloat(c);
            }
            if (D || (k = Q._gsap, k.renderTransform && !B.parseTransform || uB(Q, B.parseTransform), O = B.smoothOrigin !== !1 && k.smooth, D = this._pt = new eQ(this._pt, i, V, 0, 1, k.renderTransform, k, 0, -1), D.dep = 1), o === "scale")
              this._pt = new eQ(this._pt, k, "scaleY", k.scaleY, (l ? $Q(k.scaleY, l + n) : n) - k.scaleY || 0, aA), this._pt.u = 0, e.push("scaleY", o), o += "X";
            else if (o === "transformOrigin") {
              N.push(iQ, 0, i[iQ]), c = wi(c), k.svg ? kA(Q, c, 0, O, 0, this) : (h = parseFloat(c.split(" ")[2]) || 0, h !== k.zOrigin && FQ(this, k, "zOrigin", k.zOrigin, h), FQ(this, i, o, mB(v), mB(c)));
              continue;
            } else if (o === "svgOrigin") {
              kA(Q, c, 1, O, 0, this);
              continue;
            } else if (o in oP) {
              li(this, k, o, T, l ? $Q(T, l + c) : c);
              continue;
            } else if (o === "smoothOrigin") {
              FQ(this, k, "smooth", k.smooth, c);
              continue;
            } else if (o === "force3D") {
              k[o] = c;
              continue;
            } else if (o === "transform") {
              ki(this, c, Q);
              continue;
            }
          } else o in i || (o = iB(o) || o);
          if (U || (n || n === 0) && (T || T === 0) && !Qi.test(c) && o in i)
            E = (v + "").substr((T + "").length), n || (n = 0), h = _(c) || (o in zQ.units ? zQ.units[o] : E), E !== h && (T = pQ(Q, o, v, h)), this._pt = new eQ(this._pt, U ? k : i, o, T, (l ? $Q(T, l + n) : n) - T, !U && (h === "px" || o === "zIndex") && B.autoRound !== !1 ? Pi : aA), this._pt.u = h || 0, U && d !== c ? (this._pt.b = v, this._pt.e = d, this._pt.r = ti) : E !== h && h !== "%" && (this._pt.b = v, this._pt.r = Ai);
          else if (o in i)
            zi.call(this, Q, o, v, l ? l + c : c);
          else if (o in Q)
            this.add(Q, o, v || Q[o], l ? l + c : c, t, P);
          else if (o !== "parseTransform") {
            NA(o, c);
            continue;
          }
          U || (o in i ? N.push(o, 0, i[o]) : typeof Q[o] == "function" ? N.push(o, 2, Q[o]()) : N.push(o, 1, v || Q[o])), e.push(o);
        }
      }
    u && tP(this);
  },
  render: function(Q, B) {
    if (B.tween._time || !pA())
      for (var A = B._pt; A; )
        A.r(Q, A.d), A = A._next;
    else
      B.styles.revert();
  },
  get: CQ,
  aliases: DQ,
  getSetter: function(Q, B, A) {
    var t = DQ[B];
    return t && t.indexOf(",") < 0 && (B = t), B in OQ && B !== iQ && (Q._gsap.x || CQ(Q, "x")) ? A && tt === A ? B === "scale" ? si : ri : (tt = A || {}) && (B === "scale" ? vi : ci) : Q.style && !uA(Q.style[B]) ? ei : ~B.indexOf("-") ? ii : fA(Q, B);
  },
  core: {
    _removeProperty: gQ,
    _getMatrix: xA
  }
};
rQ.utils.checkPrefix = iB;
rQ.core.getStyleSaver = sP;
(function(r, Q, B, A) {
  var t = PQ(r + "," + Q + "," + B, function(P) {
    OQ[P] = 1;
  });
  PQ(Q, function(P) {
    zQ.units[P] = "deg", oP[P] = 1;
  }), DQ[t[13]] = r + "," + Q, PQ(A, function(P) {
    var e = P.split(":");
    DQ[e[1]] = t[e[0]];
  });
})("x,y,z,scale,scaleX,scaleY,xPercent,yPercent", "rotation,rotationX,rotationY,skewX,skewY", "transform,transformOrigin,svgOrigin,force3D,smoothOrigin,transformPerspective", "0:translateX,1:translateY,2:translateZ,8:rotate,8:rotationZ,8:rotateZ,9:rotateX,10:rotateY");
PQ("x,y,z,top,right,bottom,left,width,height,fontSize,padding,margin,perspective", function(r) {
  zQ.units[r] = "px";
});
rQ.registerPlugin(wP);
var jB = rQ.registerPlugin(wP) || rQ;
jB.core.Tween;
const Ui = "", ot = 0.3, EP = "#c9883d", Di = "#eef1f4", Tt = "#a855f7";
function CB(r) {
  return Number.isFinite(r) ? Math.max(0, Math.min(1, r)) : 0;
}
function ui(r) {
  const Q = CB(r);
  return 1 - Math.pow(1 - Q, 3);
}
function di(r) {
  const Q = Math.max(1e-3, r), B = Q * 1.6, A = Q * 0.7, t = new a.ConeGeometry(A, B, 3, 1, !1);
  return t.rotateX(-Math.PI / 2), t.translate(0, 0, B / 2), t;
}
function Ci(r) {
  const B = Math.max(1e-3, r) * 0.65, A = new a.SphereGeometry(B, 18, 12);
  return A.translate(0, 0, B), A;
}
function Ni(r, Q) {
  r.computeBoundingBox();
  const B = r.boundingBox, A = Math.max(1e-6, B.max.z - B.min.z), t = r.getAttribute("position"), P = new Float32Array(t.count * 2);
  for (let e = 0; e < t.count; e++) {
    const i = t.getX(e), s = t.getY(e), v = t.getZ(e);
    P[e * 2] = Math.atan2(s, i) / (2 * Math.PI) + 0.5, P[e * 2 + 1] = (v - B.min.z) / A * Q;
  }
  r.setAttribute("uv", new a.BufferAttribute(P, 2));
}
function aP(r) {
  const Q = atob(Ui), B = new Uint8Array(Q.length);
  for (let v = 0; v < Q.length; v++) B[v] = Q.charCodeAt(v);
  const t = new CP().parse(B.buffer);
  t.rotateX(-Math.PI / 2), t.computeBoundingBox();
  const P = t.boundingBox, e = new a.Vector3();
  P.getSize(e);
  const i = Math.max(1e-3, r) * 1.6 / Math.max(e.x, e.y, e.z, 1e-3);
  t.scale(i, i, i), t.computeBoundingBox();
  const s = t.boundingBox;
  return t.translate(
    -(s.min.x + s.max.x) / 2,
    -(s.min.y + s.max.y) / 2,
    -s.min.z
  ), Ni(t, 6), t.computeVertexNormals(), t;
}
let $B = null;
function Oi() {
  if ($B) return $B;
  const r = 64, Q = 256, B = document.createElement("canvas");
  B.width = r, B.height = Q;
  const A = B.getContext("2d");
  A.fillStyle = "#808080", A.fillRect(0, 0, r, Q);
  const t = 10;
  for (let i = 0; i < t; i++) {
    const s = i / t * r, v = i % 2 === 0 ? 150 : 90;
    A.fillStyle = `rgb(${v},${v},${v})`, A.fillRect(s, 0, r / t / 2, Q);
  }
  const P = A.getImageData(0, 0, r, Q);
  for (let i = 0; i < P.data.length; i += 4) {
    const s = (Math.random() - 0.5) * 40;
    P.data[i] = Math.min(255, Math.max(0, P.data[i] + s)), P.data[i + 1] = P.data[i], P.data[i + 2] = P.data[i];
  }
  A.putImageData(P, 0, 0);
  const e = new a.CanvasTexture(B);
  return e.wrapS = a.RepeatWrapping, e.wrapT = a.RepeatWrapping, $B = e, e;
}
function Li(r, Q) {
  const B = Math.max(1e-3, r), A = B * 1.6, t = B * 0.08, P = B * 0.187, e = CB(Q), i = new a.CylinderGeometry(0, t, A, 8, 1, !1);
  i.rotateX(-Math.PI / 2), i.translate(0, 0, A / 2);
  const s = new a.CylinderGeometry(0, P, A, 8, 1, !0);
  s.rotateX(-Math.PI / 2), s.translate(0, 0, A / 2);
  const v = new a.MeshBasicMaterial({
    color: new a.Color(Tt),
    transparent: !0,
    opacity: e,
    depthTest: !1,
    depthWrite: !1,
    blending: a.AdditiveBlending
  }), c = new a.MeshBasicMaterial({
    color: new a.Color(Tt),
    transparent: !0,
    opacity: e * 0.3,
    depthTest: !1,
    depthWrite: !1,
    blending: a.AdditiveBlending,
    side: a.DoubleSide
  }), n = new a.Mesh(i, v);
  n.renderOrder = 1e3;
  const T = new a.Mesh(s, c);
  T.renderOrder = 1e3;
  const z = new a.Group();
  return z.add(n), z.add(T), z;
}
function Mi(r) {
  const Q = Math.max(1e-3, r.bit.size);
  return r.bit.type === "triangle" ? di(Q) : r.bit.type === "drill" ? aP(Q) : Ci(Q);
}
function hP(r) {
  return r.bit.colorSource === "custom" ? r.bit.color : Di;
}
function Hi(r) {
  const Q = CB(r.bit.opacity), B = aP(Math.max(1e-3, r.bit.size)), A = new a.Color(hP(r)), t = Oi(), P = new a.MeshStandardMaterial({
    color: A,
    metalness: 0.8,
    roughness: 0.35,
    roughnessMap: t,
    bumpMap: t,
    bumpScale: 0.03,
    // Self-illuminate slightly so the bit stays legible as a UI marker
    // regardless of viewing angle/lighting, matching the old visualizer's
    // brighter, always-visible look.
    emissive: A,
    emissiveIntensity: 0.12,
    transparent: Q < 1,
    opacity: Q,
    depthTest: !1,
    depthWrite: !1
  }), e = new a.Mesh(B, P);
  return e.renderOrder = 1e3, e;
}
function ji(r) {
  const Q = CB(r.bit.opacity), B = Mi(r), A = new a.MeshBasicMaterial({
    color: new a.Color(EP),
    transparent: Q < 1,
    opacity: Q,
    side: a.DoubleSide,
    depthTest: !1,
    depthWrite: !1
  }), t = new a.Mesh(B, A);
  return t.renderOrder = 1e3, t;
}
function zt(r) {
  return r.bit.type === "laser" ? Li(r.bit.size, r.bit.opacity) : r.bit.type === "drill" ? Hi(r) : ji(r);
}
function wt(r) {
  if (r instanceof a.Mesh)
    r.geometry.dispose(), r.material.dispose();
  else if (r instanceof a.Group)
    for (const Q of r.children)
      Q instanceof a.Mesh && (Q.geometry.dispose(), Q.material.dispose());
}
function Fi(r) {
  const Q = new a.Group();
  Q.name = "gviewer:bit-marker", Q.visible = !!r.bit.enabled;
  let B = r, A = zt(B);
  Q.add(A);
  let t = null, P = !1, e = null, i = null;
  const s = (n) => {
    const T = Math.max(1, n);
    return jB.to(A.rotation, {
      z: `+=${Math.PI * 2}`,
      duration: 60 / T,
      repeat: -1,
      ease: "none",
      paused: !0
    });
  };
  e = s(B.bit.spinRpm);
  const v = (n) => {
    const T = n.bit.type, z = B.bit.type, w = n.bit.size, o = B.bit.size, E = T !== z, h = w !== o, l = CB(n.bit.opacity);
    if (E || h)
      Q.remove(A), wt(A), A = zt(n), Q.add(A), i == null || i.kill(), e == null || e.kill(), e = s(n.bit.spinRpm), P && e.timeScale(1).play();
    else if (T === "laser") {
      const D = A, [k, O] = D.children, u = k.material, N = O.material;
      u.opacity = l, u.needsUpdate = !0, N.opacity = l * 0.3, N.needsUpdate = !0;
    } else {
      const k = A.material, O = new a.Color(T === "drill" ? hP(n) : EP);
      k.color = O, T === "drill" && (k.emissive = O), k.opacity = l, k.transparent = l < 1, k.needsUpdate = !0;
    }
    B = n, Q.visible = !!n.bit.enabled;
  };
  return {
    object: Q,
    setVisible: (n) => {
      Q.visible = !!n;
    },
    setOptions: (n) => v(n),
    setTarget: (n, T) => {
      const z = !!(T != null && T.immediate), w = Number(n.x) || 0, o = Number(n.y) || 0, E = Number(n.z) || 0;
      if (n.a, z || B.bit.tweenMs <= 0) {
        t = null, Q.position.set(w, o, E);
        return;
      }
      t = {
        startedAt: performance.now(),
        duration: Math.max(1, B.bit.tweenMs),
        from: { x: Q.position.x, y: Q.position.y, z: Q.position.z },
        to: { x: w, y: o, z: E }
      };
    },
    setSpinning: (n) => {
      const T = !!n;
      if (T !== P)
        if (P = T, i == null || i.kill(), P)
          e == null || e.play(), i = jB.to(e, {
            timeScale: 1,
            duration: ot,
            ease: "power1.out"
          });
        else {
          const z = e;
          i = jB.to(e, {
            timeScale: 0,
            duration: ot,
            ease: "power1.in",
            onComplete: () => z == null ? void 0 : z.pause()
          });
        }
    },
    update: (n) => {
      if (!t) return;
      const T = (n - t.startedAt) / t.duration, z = ui(T);
      Q.position.set(
        a.MathUtils.lerp(t.from.x, t.to.x, z),
        a.MathUtils.lerp(t.from.y, t.to.y, z),
        a.MathUtils.lerp(t.from.z, t.to.z, z)
      ), T >= 1 && (Q.position.set(t.to.x, t.to.y, t.to.z), t = null);
    },
    dispose: () => {
      i == null || i.kill(), e == null || e.kill(), Q.remove(A), wt(A);
    }
  };
}
const Gi = 25.4, fi = 500, bi = 300;
class Vi {
  constructor(Q) {
    this.viewCube = null, this.resizeObserver = null, this.onWindowResize = null, this.animationFrameId = null, this.gridGroup = null, this.axesGroup = null, this.gridLabelsGroup = null, this.boundingBoxGroup = null, this.machineBedGroup = null, this.bitMarker = null, this.preLaserBitType = "drill", this.toolpathStreams = [], this.toolpathCutBucketCount = 1, this.toolpathRotationA = 0, this.lastBitPosition = { x: 0, y: 0, z: 0, a: 0 }, this.sim3dHandle = null, this.currentLines = [], this.linePositions = null, this.renderSequence = 0, this.currentBounds = null, this.cameraFocusTransition = null, this.cameraFollowRequested = !1, this.cameraFollowInterrupted = !1, this.cameraFollowOffset = null, this.id = Q.id, this.container = Q.container, this.callbacks = Q.callbacks ?? {}, this.options = at(GP, Q.options), this.canvas = document.createElement("canvas"), this.canvas.style.width = "100%", this.canvas.style.height = "100%", this.canvas.style.display = "block", this.container.appendChild(this.canvas), KP(this.container), this.renderer = new a.WebGLRenderer({
      canvas: this.canvas,
      antialias: this.options.render.antialias
    }), this.renderer.setPixelRatio(window.devicePixelRatio), this.renderer.setClearColor(this.options.render.theme.background, 1), this.scene = new a.Scene(), this.scene.add(new a.AmbientLight(16777215, 0.8));
    const B = new a.DirectionalLight(16777215, 0.9);
    B.position.set(1, -1, 1.5), this.scene.add(B);
    const A = new a.DirectionalLight(16777215, 0.5);
    A.position.set(-1, 1, 1), this.scene.add(A), this.toolpathRoot = new a.Group(), this.toolpathRoot.name = "gviewer:toolpath-root", this.scene.add(this.toolpathRoot), this.camera = new a.PerspectiveCamera(this.options.camera.fov, 1, 0.1, 1e5), this.camera.up.set(0, 0, 1), this.camera.position.set(
      this.options.camera.initialPosition.x,
      this.options.camera.initialPosition.y,
      this.options.camera.initialPosition.z
    ), this.controls = new kP(this.camera, this.renderer.domElement), this.controls.enableDamping = this.options.camera.orbit.enableDamping, this.controls.addEventListener("start", () => {
      this.cameraFollowRequested && (this.cameraFollowInterrupted = !0);
    }), this.controls.update(), this.viewCubeCorrection = new a.Matrix4().makeRotationX(a.MathUtils.degToRad(90)), this.viewCube = new xP({
      container: this.container,
      onSelectView: (t) => {
        const P = new a.Vector3();
        let e;
        if (this.currentBounds) {
          this.currentBounds.getCenter(P);
          const i = new a.Vector3();
          this.currentBounds.getSize(i);
          const s = Math.max(i.x, i.y, 1) / 2, v = a.MathUtils.degToRad(this.camera.fov);
          e = s / Math.tan(v / 2) * 1.25 + i.z;
        } else
          e = this.camera.position.distanceTo(this.controls.target);
        this.startSnapToView(t, P, Math.max(1e-6, e), 400);
      }
    }), typeof ResizeObserver < "u" ? (this.resizeObserver = new ResizeObserver(() => this.resize()), this.resizeObserver.observe(this.container)) : (this.onWindowResize = () => this.resize(), window.addEventListener("resize", this.onWindowResize)), this.renderGridAndAxes(), this.refreshGridLabels(), this.refreshBoundingBox(), this.refreshMachineBed(), this.ensureBitMarker(), this.resize(), this.startAnimationLoop();
  }
  setBitPosition(Q, B) {
    var A;
    if (this.ensureBitMarker(), this.lastBitPosition = { ...this.lastBitPosition, ...Q }, (A = this.bitMarker) == null || A.setTarget(Q, B), this.cameraFollowRequested && !this.cameraFollowInterrupted && this.cameraFollowOffset) {
      const t = new a.Vector3(this.lastBitPosition.x, this.lastBitPosition.y, this.controls.target.z), P = t.clone().add(this.cameraFollowOffset);
      this.startCameraLerp(t, P, bi, "linear");
    }
  }
  /**
   * Enable or disable "follow tool" camera tracking. While enabled, every
   * subsequent setBitPosition() call pans the camera+target in X/Y to keep
   * the tool centered, holding the camera→target offset captured at engage
   * time constant — which is what preserves viewing angle and height. A
   * user-initiated camera drag (OrbitControls "start") interrupts an active
   * session; it only re-arms on the next disabled→enabled edge.
   */
  setCameraFollowEnabled(Q) {
    if (Q === this.cameraFollowRequested)
      return;
    if (this.cameraFollowRequested = Q, this.cameraFollowInterrupted = !1, !Q) {
      this.cameraFollowOffset = null;
      return;
    }
    this.cameraFollowOffset = this.camera.position.clone().sub(this.controls.target);
    const B = new a.Vector3(this.lastBitPosition.x, this.lastBitPosition.y, this.controls.target.z), A = B.clone().add(this.cameraFollowOffset);
    this.startCameraLerp(B, A, fi);
  }
  setBitVisible(Q) {
    var B;
    this.ensureBitMarker(), (B = this.bitMarker) == null || B.setVisible(Q);
  }
  setBitSpinning(Q) {
    var B;
    this.ensureBitMarker(), (B = this.bitMarker) == null || B.setSpinning(Q);
  }
  setToolpathRotationA(Q) {
    const B = Number(Q) || 0;
    this.toolpathRotationA = B, this.toolpathRoot.rotation.x = a.MathUtils.degToRad(B);
  }
  setCallbacks(Q) {
    this.callbacks = Q;
  }
  hideUntilLine(Q, B) {
    const A = B ?? this.options.progress.mode, t = Math.floor(Q);
    for (const P of this.toolpathStreams) {
      const e = t < 0 ? 0 : P.prefixEndVertex[Math.min(t, P.prefixEndVertex.length - 1)];
      A === "grey" ? (P.line.geometry.setDrawRange(0, P.totalVertices), HP({ stream: P, nextCursorVertex: e, options: this.options })) : P.line.geometry.setDrawRange(e, Math.max(0, P.totalVertices - e));
    }
  }
  seekToLine(Q, B) {
    if (this.hideUntilLine(Q, B), this.linePositions && this.currentLines.length > 0) {
      const A = Math.max(0, Math.min(Math.floor(Q), this.currentLines.length - 1)), t = {
        x: this.linePositions[A * 4],
        y: this.linePositions[A * 4 + 1],
        z: this.linePositions[A * 4 + 2],
        a: this.linePositions[A * 4 + 3]
      };
      this.setBitPosition(t, { immediate: !0 });
    }
    if (this.sim3dHandle) {
      const A = Math.max(0, Math.min(Math.floor(Q), this.currentLines.length - 1));
      if (A !== this.sim3dHandle.currentLine) {
        const t = SB(A, this.sim3dHandle.data);
        YB(t, this.sim3dHandle.data.resolution, this.options.sim3d.erosionPasses, this.sim3dHandle.data.slabBounds.zTop), VB(this.sim3dHandle.slab, t), this.sim3dHandle.currentLine = A;
      }
    }
  }
  showAll() {
    for (const Q of this.toolpathStreams)
      Q.line.geometry.setDrawRange(0, Q.totalVertices);
  }
  resetColors() {
    for (const Q of this.toolpathStreams) {
      Q.greyCursorVertex = 0, Q.simColors.set(Q.baseColors);
      const B = Q.line.geometry.getAttribute("color");
      B.clearUpdateRanges(), B.addUpdateRange(0, Q.simColors.length), B.needsUpdate = !0;
    }
  }
  snapCameraToView(Q, B = {}) {
    const A = Math.max(0, Math.floor(B.durationMs ?? 240)), t = this.controls.target.clone(), P = this.camera.position.distanceTo(t), e = Math.max(1e-6, B.distance ?? P);
    this.startSnapToView(Q, t, e, A);
  }
  /**
   * Enable or disable orbit *rotation* while leaving pan and zoom untouched.
   *
   * Used to pin the camera to a fixed view (e.g. top-down) so a press-and-hold
   * gesture is not interpreted as a rotate-drag and the picked plane stays
   * stable under the cursor. Picking (screenToWorld) works from any camera
   * orientation, so this is a UX lock, not a correctness requirement.
   */
  setRotateEnabled(Q) {
    this.controls.enableRotate = Q;
  }
  /**
   * Convert a viewport pixel (clientX/clientY, e.g. from a PointerEvent) into a
   * point on a horizontal toolpath plane, in the scene's coordinate space.
   *
   * A ray is cast from the camera through the pixel and intersected with the
   * plane Z = `planeZ`, which defaults to the bit's current Z so the pick lands
   * on the plane the tool is sitting on. Returns null when the pixel is outside
   * a laid-out canvas or the ray is parallel to the plane (no intersection).
   *
   * The result is in world/scene space. The toolpath root sits at the origin,
   * so for a normal (non-rotary) file this equals the gcode work coordinate.
   * For a rotary file the toolpath root is rotated about X, so the returned XY
   * is not a meaningful work coordinate — callers supporting only XY moves
   * should gate on file type.
   */
  screenToWorld(Q, B, A) {
    const t = this.canvas.getBoundingClientRect();
    if (t.width === 0 || t.height === 0)
      return null;
    const P = new a.Vector2(
      (Q - t.left) / t.width * 2 - 1,
      -((B - t.top) / t.height) * 2 + 1
    ), e = new a.Raycaster();
    e.setFromCamera(P, this.camera);
    const i = (A == null ? void 0 : A.planeZ) ?? this.lastBitPosition.z ?? 0, s = new a.Plane(new a.Vector3(0, 0, 1), -i), v = new a.Vector3();
    return e.ray.intersectPlane(s, v) ? { x: v.x, y: v.y, z: v.z } : null;
  }
  /**
   * Project a point in world/scene space to a viewport pixel (clientX/clientY),
   * the inverse of screenToWorld and in the same coordinate space it consumes.
   *
   * The point is projected through the camera to normalized device coordinates
   * and mapped back onto the laid-out canvas rect, so overlays drawn in fixed
   * (viewport) coordinates line up with picked scene points and track pan/zoom.
   *
   * `z` is the point's height in scene space (default 0); it changes the
   * projected pixel under the perspective camera, so callers placing markers
   * off the Z=0 plane should pass it. Returns null when the canvas has no layout.
   */
  worldToScreen(Q, B, A = 0) {
    const t = this.canvas.getBoundingClientRect();
    if (t.width === 0 || t.height === 0)
      return null;
    const P = new a.Vector3(Q, B, A).project(this.camera);
    return {
      x: t.left + (P.x + 1) / 2 * t.width,
      y: t.top + (1 - P.y) / 2 * t.height
    };
  }
  startSnapToView(Q, B, A, t) {
    const P = RA(Q), e = B.clone().add(P.multiplyScalar(A)), i = Math.max(1, A);
    this.camera.near = i / 1e3, this.camera.far = i * 50, this.camera.updateProjectionMatrix(), this.startCameraLerp(B, e, t);
  }
  // Shared tween kickoff for any camera position+target move (view snapping,
  // model focus, tool-follow engage/track): saves/restores damping around the
  // transition and hands off to updateCameraFocusTransition() each frame.
  startCameraLerp(Q, B, A, t = "easeInOutCubic") {
    this.cameraFocusTransition && (this.controls.enableDamping = this.cameraFocusTransition.dampingEnabled), this.cameraFocusTransition = {
      startedAt: performance.now(),
      duration: A,
      fromPosition: this.camera.position.clone(),
      toPosition: B,
      fromTarget: this.controls.target.clone(),
      toTarget: Q,
      dampingEnabled: this.controls.enableDamping,
      easing: t
    }, this.controls.enableDamping = !1;
  }
  async loadFromUrl(Q, B = {}) {
    this.emitProgress({ state: "indeterminate", label: "Loading file..." });
    const A = await fetch(Q, { signal: B.signal });
    if (!A.ok)
      throw this.emitProgress({ state: "hidden" }), new Error(`Failed to load gcode: ${A.statusText}`);
    const t = await A.text();
    await this.loadFromText(t);
  }
  async loadFromFile(Q) {
    this.emitProgress({ state: "indeterminate", label: "Loading file..." });
    const B = await Q.text();
    await this.loadFromText(B);
  }
  async loadFromText(Q) {
    await this.loadFromLines(Q.split(/\r?\n/));
  }
  async loadFromLines(Q) {
    this.currentLines = Array.from(Q), await this.renderScene();
  }
  async loadFromWorkerData(Q) {
    this.currentLines = [];
    const { rapid: B, cut: A } = UP(Q), t = (Q.toolchangeCount ?? 0) > 0;
    this.setToolpathGeometry({
      rapid: {
        positions: B.positions,
        colors: void 0,
        prefixEndVertex: B.prefixEndVertex
      },
      cuts: [{
        positions: A.positions,
        colors: t ? A.colors : void 0,
        prefixEndVertex: A.prefixEndVertex
      }],
      cutBucketCount: 1
    });
  }
  unload() {
    this.currentLines = [], this.setGeometryEmpty(), this.emitProgress({ state: "hidden" });
  }
  setOptions(Q) {
    var T, z, w, o, E, h, l, U, D, k, O, u, N, d, M, b, f, C, j, F, H, G, I, R, QQ, sQ, W, x, BQ, vQ, rB, yA, IA, SA;
    const B = this.options;
    this.options = at(this.options, Q), (B.render.antialias !== this.options.render.antialias || B.render.theme.background !== this.options.render.theme.background) && this.renderer.setClearColor(this.options.render.theme.background, 1), (B.render.theme.opacity !== this.options.render.theme.opacity || B.render.theme.rapidOpacity !== this.options.render.theme.rapidOpacity) && this.refreshToolpathOpacities(), B.camera.fov !== this.options.camera.fov && (this.camera.fov = this.options.camera.fov, this.camera.updateProjectionMatrix()), B.camera.orbit.enableDamping !== this.options.camera.orbit.enableDamping && (this.controls.enableDamping = this.options.camera.orbit.enableDamping);
    const A = ((T = B.grid.bounds) == null ? void 0 : T.min.x) !== ((z = this.options.grid.bounds) == null ? void 0 : z.min.x) || ((w = B.grid.bounds) == null ? void 0 : w.min.y) !== ((o = this.options.grid.bounds) == null ? void 0 : o.min.y) || ((E = B.grid.bounds) == null ? void 0 : E.max.x) !== ((h = this.options.grid.bounds) == null ? void 0 : h.max.x) || ((l = B.grid.bounds) == null ? void 0 : l.max.y) !== ((U = this.options.grid.bounds) == null ? void 0 : U.max.y), t = B.units !== this.options.units || B.grid.sizeX !== this.options.grid.sizeX || B.grid.sizeY !== this.options.grid.sizeY || B.grid.axisDepth !== this.options.grid.axisDepth || A, P = B.render.theme.colors.grid.major !== this.options.render.theme.colors.grid.major || B.render.theme.colors.grid.minor !== this.options.render.theme.colors.grid.minor || B.render.theme.colors.axes.x !== this.options.render.theme.colors.axes.x || B.render.theme.colors.axes.y !== this.options.render.theme.colors.axes.y || B.render.theme.colors.axes.z !== this.options.render.theme.colors.axes.z;
    (t || P) && this.renderGridAndAxes(), (B.grid.labels !== this.options.grid.labels || B.units !== this.options.units || B.grid.sizeX !== this.options.grid.sizeX || B.grid.sizeY !== this.options.grid.sizeY || A || P) && this.refreshGridLabels(), (B.boundingBox.visible !== this.options.boundingBox.visible || B.boundingBox.labels !== this.options.boundingBox.labels || B.units !== this.options.units || B.render.theme.colors.boundingBox !== this.options.render.theme.colors.boundingBox) && this.refreshBoundingBox(), (B.machineBed.visible !== this.options.machineBed.visible || ((D = B.machineBed.min) == null ? void 0 : D.x) !== ((k = this.options.machineBed.min) == null ? void 0 : k.x) || ((O = B.machineBed.min) == null ? void 0 : O.y) !== ((u = this.options.machineBed.min) == null ? void 0 : u.y) || ((N = B.machineBed.max) == null ? void 0 : N.x) !== ((d = this.options.machineBed.max) == null ? void 0 : d.x) || ((M = B.machineBed.max) == null ? void 0 : M.y) !== ((b = this.options.machineBed.max) == null ? void 0 : b.y) || ((C = (f = B.machineBed.keepout) == null ? void 0 : f.min) == null ? void 0 : C.x) !== ((F = (j = this.options.machineBed.keepout) == null ? void 0 : j.min) == null ? void 0 : F.x) || ((G = (H = B.machineBed.keepout) == null ? void 0 : H.min) == null ? void 0 : G.y) !== ((R = (I = this.options.machineBed.keepout) == null ? void 0 : I.min) == null ? void 0 : R.y) || ((sQ = (QQ = B.machineBed.keepout) == null ? void 0 : QQ.max) == null ? void 0 : sQ.x) !== ((x = (W = this.options.machineBed.keepout) == null ? void 0 : W.max) == null ? void 0 : x.x) || ((vQ = (BQ = B.machineBed.keepout) == null ? void 0 : BQ.max) == null ? void 0 : vQ.y) !== ((yA = (rB = this.options.machineBed.keepout) == null ? void 0 : rB.max) == null ? void 0 : yA.y) || B.render.theme.colors.machineBed !== this.options.render.theme.colors.machineBed || B.render.theme.colors.machineBedKeepout !== this.options.render.theme.colors.machineBedKeepout) && this.refreshMachineBed();
    const v = B.render.theme.colors.rapid !== this.options.render.theme.colors.rapid || B.render.theme.colors.cutting !== this.options.render.theme.colors.cutting || B.render.theme.colors.laser !== this.options.render.theme.colors.laser || B.render.theme.background !== this.options.render.theme.background || B.render.theme.colors.processed !== this.options.render.theme.colors.processed;
    if (v && this.refreshToolpathColors(), (B.bit.enabled !== this.options.bit.enabled || B.bit.type !== this.options.bit.type || B.bit.size !== this.options.bit.size || B.bit.opacity !== this.options.bit.opacity || B.bit.colorSource !== this.options.bit.colorSource || B.bit.color !== this.options.bit.color || v) && (this.ensureBitMarker(), (IA = this.bitMarker) == null || IA.setOptions(this.options)), B.mode.laser !== this.options.mode.laser && (this.options.mode.laser ? this.options.bit.type !== "laser" && (this.preLaserBitType = this.options.bit.type, this.options = { ...this.options, bit: { ...this.options.bit, type: "laser" } }) : this.options = { ...this.options, bit: { ...this.options.bit, type: this.preLaserBitType } }, this.ensureBitMarker(), (SA = this.bitMarker) == null || SA.setOptions(this.options), this.currentLines.length > 0 && this.renderScene()), B.mode.sim3d !== this.options.mode.sim3d)
      if (!this.options.mode.sim3d)
        this.setSim3dHandle(null), this.setToolpathStreamsVisible(!0);
      else {
        this.renderScene();
        return;
      }
    if ((B.sim3d.toolDiameter !== this.options.sim3d.toolDiameter || B.sim3d.resolution !== this.options.sim3d.resolution) && this.options.mode.sim3d) {
      this.renderScene();
      return;
    }
    if (B.sim3d.erosionPasses !== this.options.sim3d.erosionPasses && this.sim3dHandle) {
      const lP = this.sim3dHandle.currentLine, YA = SB(lP, this.sim3dHandle.data);
      YB(YA, this.sim3dHandle.data.resolution, this.options.sim3d.erosionPasses, this.sim3dHandle.data.slabBounds.zTop), VB(this.sim3dHandle.slab, YA);
    }
    B.sim3d.showToolpath !== this.options.sim3d.showToolpath && this.options.mode.sim3d && this.setToolpathStreamsVisible(this.options.sim3d.showToolpath);
  }
  getOptions() {
    return this.options;
  }
  resize() {
    const Q = this.container.getBoundingClientRect(), B = Math.max(
      1,
      Math.floor(Q.width || this.container.clientWidth || window.innerWidth)
    ), A = Math.max(
      1,
      Math.floor(Q.height || this.container.clientHeight || window.innerHeight)
    );
    this.renderer.setSize(B, A, !1), this.camera.aspect = B / A, this.camera.updateProjectionMatrix();
  }
  focusToModel() {
    this.currentBounds && this.startCameraFocus(this.currentBounds);
  }
  resetCamera() {
    this.cameraFocusTransition = null, this.controls.enableDamping = this.options.camera.orbit.enableDamping, this.controls.target.set(0, 0, 0), this.camera.position.set(
      this.options.camera.initialPosition.x,
      this.options.camera.initialPosition.y,
      this.options.camera.initialPosition.z
    ), this.controls.update();
  }
  getBounds() {
    return this.currentBounds ? {
      min: {
        x: this.currentBounds.min.x,
        y: this.currentBounds.min.y,
        z: this.currentBounds.min.z
      },
      max: {
        x: this.currentBounds.max.x,
        y: this.currentBounds.max.y,
        z: this.currentBounds.max.z
      }
    } : null;
  }
  dispose() {
    var Q;
    this.renderSequence += 1, this.animationFrameId !== null && (cancelAnimationFrame(this.animationFrameId), this.animationFrameId = null), this.resizeObserver && (this.resizeObserver.disconnect(), this.resizeObserver = null), this.onWindowResize && (window.removeEventListener("resize", this.onWindowResize), this.onWindowResize = null), this.setGeometryEmpty(), this.setGridGroup(null), this.setAxesGroup(null), this.setGridLabelsGroup(null), this.setBoundingBoxGroup(null), this.setMachineBedGroup(null), this.setBitMarker(null), this.controls.dispose(), this.renderer.dispose(), (Q = this.viewCube) == null || Q.dispose(), this.viewCube = null, this.container.removeChild(this.canvas);
  }
  emitProgress(Q) {
    var B, A;
    (A = (B = this.callbacks).onProgress) == null || A.call(B, { id: this.id, ...Q });
  }
  emitBoundsChanged() {
    var Q, B;
    (B = (Q = this.callbacks).onBoundsChanged) == null || B.call(Q, { id: this.id, bounds: this.getBounds() });
  }
  startAnimationLoop() {
    const Q = () => {
      var A;
      this.animationFrameId = requestAnimationFrame(Q);
      const B = performance.now();
      this.controls.update(), this.updateCameraFocusTransition(), this.updateViewCubeRotation(), (A = this.bitMarker) == null || A.update(B), this.renderer.render(this.scene, this.camera);
    };
    Q();
  }
  ensureBitMarker() {
    this.bitMarker || (this.bitMarker = Fi(this.options), this.scene.add(this.bitMarker.object));
  }
  worldSizes() {
    const Q = this.options.units === "in" ? Gi : 1;
    return {
      sizeXMm: Math.max(1, this.options.grid.sizeX * Q),
      sizeYMm: Math.max(1, this.options.grid.sizeY * Q),
      axisDepthMm: Math.max(1, this.options.grid.axisDepth * Q),
      // Bounds are literal absolute mm (unlike sizeX/sizeY), so they don't
      // go through the units scale round-trip above.
      bounds: this.options.grid.bounds ?? null
    };
  }
  renderGridAndAxes() {
    const { sizeXMm: Q, sizeYMm: B, axisDepthMm: A, bounds: t } = this.worldSizes();
    this.setGridGroup(
      qP({
        units: this.options.units,
        sizeXMm: Q,
        sizeYMm: B,
        bounds: t,
        theme: this.options.render.theme
      })
    ), this.setAxesGroup(
      $P({
        sizeXWorld: Q,
        sizeYWorld: B,
        depthWorld: A,
        bounds: t,
        theme: this.options.render.theme
      })
    );
  }
  refreshGridLabels() {
    if (!this.options.grid.labels) {
      this.setGridLabelsGroup(null);
      return;
    }
    const { sizeXMm: Q, sizeYMm: B, bounds: A } = this.worldSizes();
    this.setGridLabelsGroup(
      Be({ sizeXMm: Q, sizeYMm: B, bounds: A, units: this.options.units, theme: this.options.render.theme })
    );
  }
  refreshBoundingBox() {
    if (!this.options.boundingBox.visible || !this.currentBounds) {
      this.setBoundingBoxGroup(null);
      return;
    }
    this.setBoundingBoxGroup(SP(this.currentBounds, this.options));
  }
  refreshMachineBed() {
    const { visible: Q, min: B, max: A, keepout: t } = this.options.machineBed;
    if (!Q || !B || !A) {
      this.setMachineBedGroup(null);
      return;
    }
    this.setMachineBedGroup(RP(B, A, this.options, t));
  }
  setBitMarker(Q) {
    this.bitMarker && (this.scene.remove(this.bitMarker.object), this.bitMarker.dispose()), this.bitMarker = Q, this.bitMarker && this.scene.add(this.bitMarker.object);
  }
  setGridGroup(Q) {
    this.gridGroup && (this.scene.remove(this.gridGroup), _P(this.gridGroup), this.gridGroup = null), Q && (this.gridGroup = Q, this.scene.add(Q));
  }
  setAxesGroup(Q) {
    this.axesGroup && (this.scene.remove(this.axesGroup), Qe(this.axesGroup), this.axesGroup = null), Q && (this.axesGroup = Q, this.scene.add(Q));
  }
  setGridLabelsGroup(Q) {
    this.gridLabelsGroup && (this.scene.remove(this.gridLabelsGroup), yP(this.gridLabelsGroup), this.gridLabelsGroup = null), Q && (this.gridLabelsGroup = Q, this.scene.add(Q));
  }
  setBoundingBoxGroup(Q) {
    this.boundingBoxGroup && (this.scene.remove(this.boundingBoxGroup), VP(this.boundingBoxGroup), this.boundingBoxGroup = null), Q && (this.boundingBoxGroup = Q, this.scene.add(Q));
  }
  setMachineBedGroup(Q) {
    this.machineBedGroup && (this.scene.remove(this.machineBedGroup), XP(this.machineBedGroup), this.machineBedGroup = null), Q && (this.machineBedGroup = Q, this.scene.add(Q));
  }
  setGeometryEmpty() {
    this.setSim3dHandle(null), OP(this.toolpathRoot, this.toolpathStreams), this.toolpathStreams = [], this.toolpathCutBucketCount = 1, this.linePositions = null, this.currentBounds = null, this.emitBoundsChanged(), this.refreshBoundingBox();
  }
  setToolpathGeometry(Q) {
    this.setGeometryEmpty(), this.toolpathCutBucketCount = Math.max(1, Math.floor(Q.cutBucketCount));
    const B = [
      {
        kind: "rapid",
        cutBucketIndex: null,
        positions: Q.rapid.positions,
        prefixEndVertex: Q.rapid.prefixEndVertex,
        colors: Q.rapid.colors,
        opacity: Et(this.options.render.theme.rapidOpacity ?? 0.3)
      },
      ...Q.cuts.map((P, e) => ({
        kind: "cut",
        cutBucketIndex: e,
        positions: P.positions,
        prefixEndVertex: P.prefixEndVertex,
        colors: P.colors,
        opacity: this.cutBucketOpacity(e)
      }))
    ], { streams: A, bounds: t } = NP({
      specs: B,
      options: this.options,
      scene: this.toolpathRoot
    });
    this.toolpathStreams = A, this.currentBounds = t ? t.clone() : null, this.emitBoundsChanged(), this.refreshBoundingBox(), this.setToolpathRotationA(this.toolpathRotationA);
  }
  refreshToolpathColors() {
    LP(this.toolpathStreams, this.options);
  }
  refreshToolpathOpacities() {
    MP({
      streams: this.toolpathStreams,
      options: this.options,
      cutBucketCount: this.toolpathCutBucketCount
    });
  }
  cutBucketOpacity(Q) {
    const B = this.options.render.theme.opacity, A = this.toolpathCutBucketCount;
    if (A <= 1)
      return B;
    const t = Q / (A - 1);
    return Et(B * t);
  }
  buildLinePositions() {
    const Q = this.currentLines.length;
    this.linePositions = new Float32Array(Q * 4);
    let B = 0, A = 0, t = 0, P = 0;
    const e = new PA({
      onLinearMove: (i) => {
        const s = i.transformedEnd ?? i.end;
        B = s.X, A = s.Y, t = s.Z, P = i.end.A;
      },
      onArcMove: (i) => {
        const s = i.transformedEnd ?? i.end;
        B = s.X, A = s.Y, t = s.Z, P = i.end.A;
      }
    });
    for (let i = 0; i < Q; i++) {
      const s = this.currentLines[i];
      s && e.processLine(s), this.linePositions[i * 4] = B, this.linePositions[i * 4 + 1] = A, this.linePositions[i * 4 + 2] = t, this.linePositions[i * 4 + 3] = P;
    }
  }
  async renderScene() {
    if (this.currentLines.length === 0) {
      this.setGeometryEmpty(), this.emitProgress({ state: "hidden" });
      return;
    }
    const Q = this.renderSequence += 1;
    try {
      this.options.mode.laser ? this.emitProgress({ state: "indeterminate", label: "Scanning power..." }) : this.emitProgress({
        state: "determinate",
        label: "Building geometry...",
        processed: 0,
        total: this.currentLines.length
      });
      const B = await DP(this.currentLines, {
        arcSegments: this.options.geometry.arcSegments,
        bucketCount: 16,
        laserMode: this.options.mode.laser,
        batch: {
          everyLines: this.options.geometry.batching.progressEveryLines,
          yieldEveryLines: this.options.geometry.batching.yieldEveryLines,
          shouldAbort: () => Q !== this.renderSequence,
          onProgress: (A, t) => {
            Q === this.renderSequence && this.emitProgress({ state: "determinate", label: "Building geometry...", processed: A, total: t });
          }
        }
      });
      if (Q !== this.renderSequence)
        return;
      this.setToolpathGeometry({
        rapid: B.rapid,
        cuts: B.cuts,
        cutBucketCount: B.cutBucketCount
      }), this.buildLinePositions(), this.options.mode.sim3d && (this.setToolpathStreamsVisible(this.options.sim3d.showToolpath), await this.buildAndApplySim3d(Q));
    } catch (B) {
      if (B instanceof Error && B.message === "Aborted.")
        return;
      throw B;
    } finally {
      Q === this.renderSequence && this.emitProgress({ state: "hidden" });
    }
  }
  setSim3dHandle(Q) {
    this.sim3dHandle && (this.scene.remove(this.sim3dHandle.slab.group), mP(this.sim3dHandle.slab), this.sim3dHandle = null), Q && (this.sim3dHandle = Q, this.scene.add(Q.slab.group));
  }
  setToolpathStreamsVisible(Q) {
    for (const B of this.toolpathStreams)
      B.line.visible = Q;
  }
  async buildAndApplySim3d(Q) {
    this.emitProgress({ state: "indeterminate", label: "Building 3D simulation..." });
    const B = this.options.sim3d.toolDiameter / 2, A = this.options.sim3d.resolution;
    let t;
    try {
      t = await fP(this.currentLines, B, A, {
        arcSegments: this.options.geometry.arcSegments,
        batch: {
          shouldAbort: () => Q !== this.renderSequence,
          onProgress: (i, s) => {
            Q === this.renderSequence && this.emitProgress({
              state: "determinate",
              label: "Building 3D simulation...",
              processed: i,
              total: s
            });
          },
          yieldEveryLines: this.options.geometry.batching.yieldEveryLines
        }
      });
    } catch {
      return;
    }
    if (Q !== this.renderSequence) return;
    const P = SB(0, t);
    YB(P, A, this.options.sim3d.erosionPasses, t.slabBounds.zTop);
    const e = pP(t.slabBounds, A);
    VB(e, P), this.setSim3dHandle({ data: t, slab: e, currentLine: 0 });
  }
  startCameraFocus(Q) {
    const B = new a.Vector3();
    Q.getCenter(B);
    const A = new a.Vector3();
    Q.getSize(A);
    const t = Math.max(A.x, A.y, 1) / 2, P = a.MathUtils.degToRad(this.camera.fov), e = t / Math.tan(P / 2) * 1.25 + A.z, i = RA("front-top-left"), s = B.clone().add(i.multiplyScalar(e)), v = Math.max(A.x, A.y, A.z, 1);
    this.camera.near = v / 1e3, this.camera.far = v * 50, this.camera.updateProjectionMatrix(), this.startCameraLerp(B, s, this.options.camera.focusDurationMs);
  }
  updateCameraFocusTransition() {
    if (!this.cameraFocusTransition)
      return;
    const B = performance.now() - this.cameraFocusTransition.startedAt, A = Math.min(1, Math.max(0, B / this.cameraFocusTransition.duration)), t = this.cameraFocusTransition.easing === "linear" ? A : ZP(A);
    if (this.camera.position.lerpVectors(this.cameraFocusTransition.fromPosition, this.cameraFocusTransition.toPosition, t), this.controls.target.lerpVectors(this.cameraFocusTransition.fromTarget, this.cameraFocusTransition.toTarget, t), A >= 1) {
      const P = this.cameraFocusTransition.dampingEnabled;
      this.camera.position.copy(this.cameraFocusTransition.toPosition), this.controls.target.copy(this.cameraFocusTransition.toTarget), this.cameraFocusTransition = null, this.controls.enableDamping = !1, this.controls.update(), this.controls.enableDamping = P;
    }
  }
  updateViewCubeRotation() {
    if (!this.viewCube)
      return;
    const Q = this.camera.quaternion.clone();
    typeof Q.invert == "function" ? Q.invert() : typeof Q.inverse == "function" && Q.inverse();
    const B = new a.Matrix4();
    B.makeRotationFromQuaternion(Q), B.multiply(this.viewCubeCorrection), this.viewCube.setRotationMatrix3d(B.elements);
    const A = WP(this.camera.position, this.controls.target);
    this.viewCube.setActiveFace(A);
  }
}
function Et(r) {
  return Number.isFinite(r) ? Math.max(0, Math.min(1, r)) : 0;
}
function at(r, Q) {
  var A, t, P, e;
  if (!Q)
    return { ...r };
  const B = gi(r.render.theme, (A = Q.render) == null ? void 0 : A.theme);
  return {
    ...r,
    ...Q,
    mode: { ...r.mode, ...Q.mode },
    sim3d: { ...r.sim3d, ...Q.sim3d },
    bit: { ...r.bit, ...Q.bit },
    progress: { ...r.progress, ...Q.progress },
    grid: { ...r.grid, ...Q.grid },
    boundingBox: { ...r.boundingBox, ...Q.boundingBox },
    machineBed: { ...r.machineBed, ...Q.machineBed },
    geometry: {
      ...r.geometry,
      ...Q.geometry,
      batching: { ...r.geometry.batching, ...(t = Q.geometry) == null ? void 0 : t.batching }
    },
    render: {
      ...r.render,
      ...Q.render,
      theme: B
    },
    camera: {
      ...r.camera,
      ...Q.camera,
      orbit: { ...r.camera.orbit, ...(P = Q.camera) == null ? void 0 : P.orbit },
      initialPosition: { ...r.camera.initialPosition, ...(e = Q.camera) == null ? void 0 : e.initialPosition }
    }
  };
}
function gi(r, Q) {
  var B, A;
  return Q ? {
    ...r,
    ...Q,
    colors: {
      ...r.colors,
      ...Q.colors,
      grid: { ...r.colors.grid, ...(B = Q.colors) == null ? void 0 : B.grid },
      axes: { ...r.colors.axes, ...(A = Q.colors) == null ? void 0 : A.axes }
    }
  } : r;
}
const pi = {
  rapidColor: "#0ef6ae",
  cutColor: "#3e85c7",
  boundingBoxColor: "#d0d0d0",
  strokeWidth: 0.5,
  arcSegments: 30,
  padding: 5,
  projectionMode: "isometric",
  showOrigin: !0,
  originColor: "#ffffff",
  crosshairColor: "#ffffff"
}, vB = 0, cB = 0;
class Ji {
  constructor(Q, B) {
    this.crosshairPos = null, this.crosshairVisible = !1, this.pathEls = [], this.segmentGroups = [], this.workerMode = !1, this.viewBox = { x: 0, y: 0, w: 100, h: 100 }, this.rotX = vB, this.rotY = cB, this.centerX = 0, this.centerY = 0, this.centerZ = 0, this.focalLength = 500, this.cosRotX = 1, this.sinRotX = 0, this.cosRotY = 1, this.sinRotY = 0, this.dragMode = "none", this.activePointers = /* @__PURE__ */ new Map(), this.pinchLastDist = 0, this.pinchLastMid = { x: 0, y: 0 }, this.rafPending = !1, this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.bounds = { minX: 0, minY: 0, maxX: 0, maxY: 0, minZ: 0, maxZ: 0, empty: !0 }, this.hasZInfo = !0, this.project = (A, t, P) => {
      if (this.options.projectionMode === "top")
        return { x: A, y: -t };
      const e = A - this.centerX, i = t - this.centerY, s = P - this.centerZ, v = e * this.cosRotY + s * this.sinRotY, c = i, n = -e * this.sinRotY + s * this.cosRotY, T = v, z = c * this.cosRotX - n * this.sinRotX, w = c * this.sinRotX + n * this.cosRotX;
      if (this.options.projectionMode === "perspective") {
        const o = this.focalLength / (this.focalLength + w);
        return { x: T * o, y: -z * o };
      }
      return { x: T, y: -z };
    }, this.onWheel = (A) => {
      A.preventDefault();
      const t = A.deltaY > 0 ? 1.1 : 1 / 1.1, P = this.svgPoint(A.clientX, A.clientY), { x: e, y: i, w: s, h: v } = this.viewBox;
      this.viewBox = {
        x: P.x - (P.x - e) * t,
        y: P.y - (P.y - i) * t,
        w: s * t,
        h: v * t
      }, this.applyViewBox();
    }, this.onPointerDown = (A) => {
      if (this.activePointers.set(A.pointerId, { x: A.clientX, y: A.clientY }), this.svg.setPointerCapture(A.pointerId), A.preventDefault(), this.activePointers.size === 1)
        this.dragMode = "pan", this.svg.style.cursor = "grabbing";
      else if (this.activePointers.size === 2) {
        this.dragMode = "pan", this.svg.style.cursor = "move";
        const [t, P] = Array.from(this.activePointers.values());
        this.pinchLastDist = Math.hypot(P.x - t.x, P.y - t.y), this.pinchLastMid = { x: (t.x + P.x) / 2, y: (t.y + P.y) / 2 };
      }
    }, this.onPointerMove = (A) => {
      if (!this.activePointers.has(A.pointerId)) return;
      const t = this.activePointers.get(A.pointerId);
      if (this.activePointers.set(A.pointerId, { x: A.clientX, y: A.clientY }), this.activePointers.size === 2) {
        const [i, s] = Array.from(this.activePointers.values()), v = Math.hypot(s.x - i.x, s.y - i.y), c = { x: (i.x + s.x) / 2, y: (i.y + s.y) / 2 };
        if (this.pinchLastDist > 0) {
          const w = this.pinchLastDist / v, o = this.svgPoint(c.x, c.y), { x: E, y: h, w: l, h: U } = this.viewBox;
          this.viewBox = {
            x: o.x - (o.x - E) * w,
            y: o.y - (o.y - h) * w,
            w: l * w,
            h: U * w
          };
        }
        const n = this.svg.getBoundingClientRect(), T = c.x - this.pinchLastMid.x, z = c.y - this.pinchLastMid.y;
        this.viewBox = {
          ...this.viewBox,
          x: this.viewBox.x - T / n.width * this.viewBox.w,
          y: this.viewBox.y - z / n.height * this.viewBox.h
        }, this.pinchLastDist = v, this.pinchLastMid = c, this.applyViewBox();
        return;
      }
      if (this.dragMode === "none") return;
      const P = A.clientX - t.x, e = A.clientY - t.y;
      if (this.dragMode === "orbit") {
        const i = this.svg.getBoundingClientRect(), s = Math.PI / Math.min(i.width, i.height);
        this.rotY += P * s, this.rotX += e * s, this.rotX = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, this.rotX)), this.updateTrig(), this.fitView(), this.scheduleDraw();
      } else {
        const i = this.svg.getBoundingClientRect();
        this.viewBox = {
          ...this.viewBox,
          x: this.viewBox.x - P / i.width * this.viewBox.w,
          y: this.viewBox.y - e / i.height * this.viewBox.h
        }, this.applyViewBox();
      }
    }, this.onPointerUp = (A) => {
      if (this.activePointers.delete(A.pointerId), this.svg.releasePointerCapture(A.pointerId), this.activePointers.size === 1) {
        const [t] = this.activePointers.values();
        this.pinchLastMid = { x: t.x, y: t.y }, this.dragMode = "pan", this.svg.style.cursor = "grabbing";
      } else this.activePointers.size === 0 && (this.dragMode = "none", this.svg.style.cursor = "grab");
    }, this.onContextMenu = (A) => {
      A.preventDefault();
    }, this.options = { ...pi, ...B }, this.svg = document.createElementNS("http://www.w3.org/2000/svg", "svg"), this.svg.style.cssText = "width:100%;height:100%;display:block;cursor:grab;user-select:none;touch-action:none;", this.svg.setAttribute("preserveAspectRatio", "xMidYMid meet"), this.svg.setAttribute("xmlns", "http://www.w3.org/2000/svg"), this.bboxPath = QA(), this.pathLayer = document.createElementNS("http://www.w3.org/2000/svg", "g"), this.bboxLabelX = BA("middle", "hanging"), this.bboxLabelY = BA("start", "middle"), this.bboxLabelZ = BA("middle", "auto"), this.originMarker = document.createElementNS("http://www.w3.org/2000/svg", "circle"), this.originMarker.setAttribute("stroke-opacity", "0.4"), this.originMarker.setAttribute("visibility", "hidden"), this.crosshairEl = QA(), this.crosshairEl.setAttribute("fill", "none"), this.crosshairEl.setAttribute("visibility", "hidden"), this.svg.appendChild(this.bboxPath), this.svg.appendChild(this.pathLayer), this.svg.appendChild(this.bboxLabelX), this.svg.appendChild(this.bboxLabelY), this.svg.appendChild(this.bboxLabelZ), this.svg.appendChild(this.originMarker), this.svg.appendChild(this.crosshairEl), Q.appendChild(this.svg), this.applyOptions(), this.bindEvents();
  }
  // ── Public API ────────────────────────────────────────────────────────────
  loadFromLines(Q) {
    const { rapid: B, cutting: A } = uP(Q, {
      arcSegments: this.options.arcSegments
    });
    if (this.rapidVerts = B, this.cutVerts = A, this.workerMode = !1, this.syncSegmentGroupsFromLines(), this.hasZInfo = !0, this.bounds = tA([
      { verts: B, stride: 6 },
      { verts: A, stride: 6 }
    ]), !this.bounds.empty) {
      this.centerX = (this.bounds.minX + this.bounds.maxX) / 2, this.centerY = (this.bounds.minY + this.bounds.maxY) / 2, this.centerZ = (this.bounds.minZ + this.bounds.maxZ) / 2;
      const t = Math.hypot(
        this.bounds.maxX - this.bounds.minX,
        this.bounds.maxY - this.bounds.minY,
        this.bounds.maxZ - this.bounds.minZ
      );
      this.focalLength = t * 2;
    }
    this.rotX = vB, this.rotY = cB, this.updateTrig(), this.fitView(), this.rebuildAndRender();
  }
  loadFromFile(Q) {
    return new Promise((B, A) => {
      const t = new FileReader();
      t.onload = (P) => {
        var i;
        const e = (i = P.target) == null ? void 0 : i.result;
        if (typeof e != "string") {
          A(new Error("Failed to read file"));
          return;
        }
        this.loadFromText(e), B();
      }, t.onerror = () => A(new Error("FileReader error")), t.readAsText(Q);
    });
  }
  loadFromText(Q) {
    this.loadFromLines(Q.split(/\r?\n/));
  }
  clear() {
    this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.workerMode = !1, this.segmentGroups = [], this.hasZInfo = !0, this.bounds = { minX: 0, minY: 0, maxX: 0, maxY: 0, minZ: 0, maxZ: 0, empty: !0 }, this.rebuildAndRender();
  }
  loadFromWorkerData(Q) {
    const B = dP(Q);
    if (this.workerMode = !0, this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.segmentGroups = B.map((A) => ({
      color: A.hexColor,
      opacity: A.opacity,
      verts: A.positions,
      stride: 6
    })), this.hasZInfo = !0, this.bounds = tA(B.map((A) => ({ verts: A.positions, stride: 6 }))), !this.bounds.empty) {
      this.centerX = (this.bounds.minX + this.bounds.maxX) / 2, this.centerY = (this.bounds.minY + this.bounds.maxY) / 2, this.centerZ = (this.bounds.minZ + this.bounds.maxZ) / 2;
      const A = Math.hypot(
        this.bounds.maxX - this.bounds.minX,
        this.bounds.maxY - this.bounds.minY,
        this.bounds.maxZ - this.bounds.minZ
      );
      this.focalLength = A * 2;
    }
    this.rotX = vB, this.rotY = cB, this.updateTrig(), this.fitView(), this.rebuildAndRender();
  }
  loadFromPrecomputedGroups(Q, B) {
    this.workerMode = !0, this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.segmentGroups = Q.map((t) => ({
      color: t.hexColor,
      opacity: t.opacity,
      verts: new Float32Array(t.positionsBuffer, 0, t.positionsLen),
      stride: t.stride ?? 6
    })), this.bounds = tA(this.segmentGroups);
    const A = (B == null ? void 0 : B.minZ) !== void 0 && (B == null ? void 0 : B.maxZ) !== void 0;
    if (A && !this.bounds.empty && (this.bounds.minZ = B.minZ, this.bounds.maxZ = B.maxZ), this.hasZInfo = A || this.segmentGroups.every((t) => t.stride === 6), !this.bounds.empty) {
      this.centerX = (this.bounds.minX + this.bounds.maxX) / 2, this.centerY = (this.bounds.minY + this.bounds.maxY) / 2, this.centerZ = (this.bounds.minZ + this.bounds.maxZ) / 2;
      const t = Math.hypot(
        this.bounds.maxX - this.bounds.minX,
        this.bounds.maxY - this.bounds.minY,
        this.bounds.maxZ - this.bounds.minZ
      );
      this.focalLength = t * 2;
    }
    this.rotX = vB, this.rotY = cB, this.updateTrig(), this.fitView(), this.rebuildAndRender();
  }
  syncSegmentGroupsFromLines() {
    this.segmentGroups = [
      { color: this.options.rapidColor, opacity: 0.5, verts: this.rapidVerts, stride: 6 },
      { color: this.options.cutColor, opacity: 1, verts: this.cutVerts, stride: 6 }
    ];
  }
  resetView() {
    this.rotX = vB, this.rotY = cB, this.updateTrig(), this.fitView(), this.rebuildAndRender();
  }
  setOptions(Q) {
    this.options = { ...this.options, ...Q }, this.applyOptions(), this.rebuildAndRender();
  }
  setProjectionMode(Q) {
    this.options = { ...this.options, projectionMode: Q }, this.rebuildAndRender();
  }
  getSVGElement() {
    return this.svg;
  }
  setBitPosition(Q) {
    this.crosshairPos = Q, this.crosshairVisible = !0, this.rebuildAndRender();
  }
  setBitVisible(Q) {
    this.crosshairVisible = Q, this.rebuildAndRender();
  }
  dispose() {
    this.svg.removeEventListener("wheel", this.onWheel), this.svg.removeEventListener("pointerdown", this.onPointerDown), this.svg.removeEventListener("pointermove", this.onPointerMove), this.svg.removeEventListener("pointerup", this.onPointerUp), this.svg.removeEventListener("pointercancel", this.onPointerUp), this.svg.removeEventListener("contextmenu", this.onContextMenu), this.svg.remove();
  }
  // ── Projection ────────────────────────────────────────────────────────────
  updateTrig() {
    this.cosRotX = Math.cos(this.rotX), this.sinRotX = Math.sin(this.rotX), this.cosRotY = Math.cos(this.rotY), this.sinRotY = Math.sin(this.rotY);
  }
  // ── Rendering ─────────────────────────────────────────────────────────────
  rebuildAndRender(Q = !1) {
    const B = Q ? mi : L, A = String(this.options.strokeWidth);
    for (; this.pathEls.length < this.segmentGroups.length; ) {
      const t = QA();
      this.pathLayer.appendChild(t), this.pathEls.push(t);
    }
    for (; this.pathEls.length > this.segmentGroups.length; )
      this.pathLayer.removeChild(this.pathEls.pop());
    for (let t = 0; t < this.segmentGroups.length; t++) {
      const { color: P, opacity: e, verts: i, stride: s } = this.segmentGroups[t], v = this.pathEls[t];
      v.setAttribute("stroke", P), v.setAttribute("stroke-opacity", String(e)), v.setAttribute("stroke-width", A), v.setAttribute("stroke-linecap", "round"), v.setAttribute("stroke-linejoin", "round"), v.setAttribute(
        "d",
        s === 4 ? xi(i, B) : yi(i, this.project, B)
      );
    }
    this.renderBbox(), this.renderOriginMarker(), this.renderCrosshairMarker(), this.applyViewBox();
  }
  scheduleDraw() {
    this.rafPending || (this.rafPending = !0, requestAnimationFrame(() => {
      this.rafPending = !1, this.rebuildAndRender(!0);
    }));
  }
  fitView() {
    if (this.bounds.empty) {
      this.viewBox = { x: -50, y: -50, w: 100, h: 100 };
      return;
    }
    const { minX: Q, maxX: B, minY: A, maxY: t, minZ: P, maxZ: e } = this.bounds, i = this.project, s = i(Q, A, P), v = i(B, A, P), c = i(B, t, P), n = i(Q, t, P), T = i(Q, A, e), z = i(B, A, e), w = i(B, t, e), o = i(Q, t, e), E = Math.hypot(v.x - s.x, v.y - s.y), h = Math.hypot(n.x - s.x, n.y - s.y), l = Math.hypot(T.x - s.x, T.y - s.y), U = Math.max(E, h, l) * 0.07, D = U * 0.5, k = AA(LQ(s, v), LQ(c, n), D), O = [s, v, c, n, T, z, w, o], u = Math.min(...O.map((x) => x.y)), N = Math.max(...O.map((x) => x.y)), d = Math.min(...O.map((x) => x.x)), M = Math.max(...O.map((x) => x.x)), b = { x: M + D, y: (u + N) / 2 }, f = { x: b.x + U * 5, y: b.y }, C = { x: (d + M) / 2, y: u - U }, j = [s, v, c, n, T, z, w, o, k, b, f, C];
    let F = 1 / 0, H = 1 / 0, G = -1 / 0, I = -1 / 0;
    for (const x of j)
      x.x < F && (F = x.x), x.y < H && (H = x.y), x.x > G && (G = x.x), x.y > I && (I = x.y);
    const R = G - F, QQ = I - H, sQ = Math.max(R, QQ) * 0.08, W = this.options.padding + sQ;
    this.viewBox = {
      x: F - W,
      y: H - W,
      w: R + W * 2,
      h: QQ + W * 2
    };
  }
  applyViewBox() {
    const { x: Q, y: B, w: A, h: t } = this.viewBox;
    this.svg.setAttribute("viewBox", `${L(Q)} ${L(B)} ${L(A)} ${L(t)}`);
  }
  applyOptions() {
    const { boundingBoxColor: Q, strokeWidth: B, originColor: A } = this.options;
    this.workerMode || this.syncSegmentGroupsFromLines(), this.bboxPath.setAttribute("stroke", Q), this.bboxPath.setAttribute("stroke-width", String(B * 0.6)), this.bboxPath.setAttribute("fill", Q), this.bboxPath.setAttribute("fill-opacity", "0.05");
    for (const t of [this.bboxLabelX, this.bboxLabelY, this.bboxLabelZ])
      t.setAttribute("fill", Q);
    this.originMarker.setAttribute("fill", A), this.originMarker.setAttribute("stroke", "#000000"), this.crosshairEl.setAttribute("stroke", this.options.crosshairColor);
  }
  renderBbox() {
    if (this.bounds.empty) {
      this.bboxPath.setAttribute("d", ""), this.bboxLabelX.setAttribute("visibility", "hidden"), this.bboxLabelY.setAttribute("visibility", "hidden"), this.bboxLabelZ.setAttribute("visibility", "hidden");
      return;
    }
    const { minX: Q, maxX: B, minY: A, maxY: t, minZ: P, maxZ: e } = this.bounds, i = this.project;
    if (this.options.projectionMode === "top") {
      const C = i(Q, A, 0), j = i(B, A, 0), F = i(B, t, 0), H = i(Q, t, 0);
      this.bboxPath.setAttribute(
        "d",
        `M${L(C.x)} ${L(C.y)}L${L(j.x)} ${L(j.y)}L${L(F.x)} ${L(F.y)}L${L(H.x)} ${L(H.y)}Z`
      );
      const G = Math.abs(j.x - C.x), I = Math.abs(H.y - C.y), R = Math.max(G, I) * 0.07, QQ = R * 0.5;
      ZQ(this.bboxLabelX, LQ(C, j), AA(LQ(C, j), LQ(F, H), QQ), R, `X: ${WQ(B - Q)}`);
      const sQ = Math.min(C.y, j.y, F.y, H.y), W = Math.max(C.y, j.y, F.y, H.y), x = Math.max(C.x, j.x, F.x, H.x), BQ = Math.min(C.x, j.x, F.x, H.x), vQ = { x: x + QQ, y: (sQ + W) / 2 };
      if (ZQ(this.bboxLabelY, vQ, vQ, R, `Y: ${WQ(t - A)}`), this.hasZInfo) {
        const rB = { x: (BQ + x) / 2, y: sQ - R };
        ZQ(this.bboxLabelZ, rB, rB, R, `Z: ${WQ(e - P)}`);
      } else
        this.bboxLabelZ.setAttribute("visibility", "hidden");
      return;
    }
    const s = i(Q, A, P), v = i(B, A, P), c = i(B, t, P), n = i(Q, t, P), T = i(Q, A, e), z = i(B, A, e), w = i(B, t, e), o = i(Q, t, e), E = [
      `M${L(s.x)} ${L(s.y)}L${L(v.x)} ${L(v.y)}L${L(c.x)} ${L(c.y)}L${L(n.x)} ${L(n.y)}Z`,
      `M${L(T.x)} ${L(T.y)}L${L(z.x)} ${L(z.y)}L${L(w.x)} ${L(w.y)}L${L(o.x)} ${L(o.y)}Z`,
      `M${L(s.x)} ${L(s.y)}L${L(T.x)} ${L(T.y)}`,
      `M${L(v.x)} ${L(v.y)}L${L(z.x)} ${L(z.y)}`,
      `M${L(c.x)} ${L(c.y)}L${L(w.x)} ${L(w.y)}`,
      `M${L(n.x)} ${L(n.y)}L${L(o.x)} ${L(o.y)}`
    ].join("");
    this.bboxPath.setAttribute("d", E);
    const h = Math.hypot(v.x - s.x, v.y - s.y), l = Math.hypot(n.x - s.x, n.y - s.y), U = Math.hypot(T.x - s.x, T.y - s.y), D = Math.max(h, l, U) * 0.07, k = D * 0.5;
    ZQ(this.bboxLabelX, LQ(s, v), AA(LQ(s, v), LQ(c, n), k), D, `X: ${WQ(B - Q)}`);
    const O = [s, v, c, n, T, z, w, o], u = Math.min(...O.map((C) => C.y)), N = Math.max(...O.map((C) => C.y)), d = Math.min(...O.map((C) => C.x)), M = Math.max(...O.map((C) => C.x)), b = { x: M + k, y: (u + N) / 2 };
    ZQ(this.bboxLabelY, b, b, D, `Y: ${WQ(t - A)}`);
    const f = { x: (d + M) / 2, y: u - D };
    ZQ(this.bboxLabelZ, f, f, D, `Z: ${WQ(e - P)}`);
  }
  renderOriginMarker() {
    if (!this.options.showOrigin || this.bounds.empty) {
      this.originMarker.setAttribute("visibility", "hidden");
      return;
    }
    const Q = this.project(0, 0, 0), B = Math.min(this.viewBox.w, this.viewBox.h) * 0.018;
    this.originMarker.setAttribute("cx", L(Q.x)), this.originMarker.setAttribute("cy", L(Q.y)), this.originMarker.setAttribute("r", L(B)), this.originMarker.setAttribute("stroke-width", L(B * 0.25)), this.originMarker.setAttribute("visibility", "visible");
  }
  renderCrosshairMarker() {
    if (!this.crosshairVisible || this.crosshairPos === null) {
      this.crosshairEl.setAttribute("visibility", "hidden");
      return;
    }
    const { x: Q, y: B, z: A } = this.crosshairPos, t = this.project(Q, B, A), P = Math.min(this.viewBox.w, this.viewBox.h) * 0.025, e = P * 0.25, i = t.x, s = t.y, v = [
      `M${L(i - P)} ${L(s)}L${L(i - e)} ${L(s)}`,
      `M${L(i + e)} ${L(s)}L${L(i + P)} ${L(s)}`,
      `M${L(i)} ${L(s - P)}L${L(i)} ${L(s - e)}`,
      `M${L(i)} ${L(s + e)}L${L(i)} ${L(s + P)}`
    ].join("");
    this.crosshairEl.setAttribute("d", v), this.crosshairEl.setAttribute("stroke-width", L(this.options.strokeWidth * 2)), this.crosshairEl.setAttribute("stroke-linecap", "round"), this.crosshairEl.setAttribute("visibility", "visible");
  }
  bindEvents() {
    this.svg.addEventListener("wheel", this.onWheel, { passive: !1 }), this.svg.addEventListener("pointerdown", this.onPointerDown), this.svg.addEventListener("pointermove", this.onPointerMove), this.svg.addEventListener("pointerup", this.onPointerUp), this.svg.addEventListener("pointercancel", this.onPointerUp), this.svg.addEventListener("contextmenu", this.onContextMenu);
  }
  svgPoint(Q, B) {
    const A = this.svg.getBoundingClientRect(), { x: t, y: P, w: e, h: i } = this.viewBox;
    return {
      x: t + (Q - A.left) / A.width * e,
      y: P + (B - A.top) / A.height * i
    };
  }
}
function QA() {
  const r = document.createElementNS("http://www.w3.org/2000/svg", "path");
  return r.setAttribute("fill", "none"), r;
}
function BA(r, Q) {
  const B = document.createElementNS("http://www.w3.org/2000/svg", "text");
  return B.setAttribute("text-anchor", r), B.setAttribute("dominant-baseline", Q), B;
}
function ZQ(r, Q, B, A, t) {
  r.setAttribute("x", L(B.x)), r.setAttribute("y", L(B.y)), r.setAttribute("font-size", L(A)), r.textContent = t, r.setAttribute("visibility", "visible");
}
function LQ(r, Q) {
  return { x: (r.x + Q.x) / 2, y: (r.y + Q.y) / 2 };
}
function AA(r, Q, B) {
  const A = r.x - Q.x, t = r.y - Q.y, P = Math.hypot(A, t) || 1;
  return { x: r.x + A / P * B, y: r.y + t / P * B };
}
function L(r) {
  return r.toFixed(2);
}
function mi(r) {
  return Math.round(r) + "";
}
function WQ(r) {
  return r.toFixed(2);
}
function xi(r, Q) {
  if (r.length === 0) return "";
  const B = [], A = 1e-6;
  let t = NaN, P = NaN;
  for (let e = 0; e + 3 < r.length; e += 4) {
    const i = r[e], s = -r[e + 1], v = r[e + 2], c = -r[e + 3];
    Math.abs(i - t) < A && Math.abs(s - P) < A ? B.push(`L${Q(v)} ${Q(c)}`) : B.push(`M${Q(i)} ${Q(s)}L${Q(v)} ${Q(c)}`), t = v, P = c;
  }
  return B.join("");
}
function yi(r, Q, B) {
  if (r.length === 0) return "";
  const A = [], t = 1e-6;
  let P = NaN, e = NaN;
  for (let i = 0; i + 5 < r.length; i += 6) {
    const s = Q(r[i], r[i + 1], r[i + 2]), v = Q(r[i + 3], r[i + 4], r[i + 5]);
    Math.abs(s.x - P) < t && Math.abs(s.y - e) < t ? A.push(`L${B(v.x)} ${B(v.y)}`) : A.push(`M${B(s.x)} ${B(s.y)}L${B(v.x)} ${B(v.y)}`), P = v.x, e = v.y;
  }
  return A.join("");
}
function tA(r) {
  let Q = 1 / 0, B = 1 / 0, A = 1 / 0, t = -1 / 0, P = -1 / 0, e = -1 / 0, i = !0, s = !1;
  for (const { verts: v, stride: c } of r)
    if (c === 4)
      for (let n = 0; n + 3 < v.length; n += 4) {
        const T = v[n], z = v[n + 1], w = v[n + 2], o = v[n + 3];
        T < Q && (Q = T), z < B && (B = z), T > t && (t = T), z > P && (P = z), w < Q && (Q = w), o < B && (B = o), w > t && (t = w), o > P && (P = o), i = !1;
      }
    else
      for (let n = 0; n + 5 < v.length; n += 6) {
        const T = v[n], z = v[n + 1], w = v[n + 2], o = v[n + 3], E = v[n + 4], h = v[n + 5];
        T < Q && (Q = T), z < B && (B = z), w < A && (A = w), T > t && (t = T), z > P && (P = z), w > e && (e = w), o < Q && (Q = o), E < B && (B = E), h < A && (A = h), o > t && (t = o), E > P && (P = E), h > e && (e = h), i = !1, s = !0;
      }
  return !i && !s && (A = 0, e = 0), { minX: Q, minY: B, maxX: t, maxY: P, minZ: A, maxZ: e, empty: i };
}
export {
  Ji as G,
  xP as V,
  Vi as a,
  VA as b,
  GP as d
};
