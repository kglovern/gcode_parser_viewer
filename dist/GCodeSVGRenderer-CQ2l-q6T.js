...
import { OrbitControls as ke } from "three/examples/jsm/controls/OrbitControls.js";
import { GCodeVirtualizer as et, buildWorkerToolpathStreams as ue, buildToolpathGeometryFromLinesBatched as Ue, buildMovementVerticesFromLines as De, buildWorkerSegmentGroups as de } from "./gviewer.js";
...
function Ne(r) {
  ...
  for (const t of r.specs) {
    if (t.positions.length === 0 || t.opacity <= 0)
      continue;
    const A = t.positions.length / 3, e = t.colors ? t.colors.slice() : kA(t.kind, A, r.options), P = new Float32Array(e.length);
    P.set(e);
...
function Le(r, Q) {
  for (const B of r) {
    ...
    const t = kA(B.kind, B.totalVertices, Q);
...
function Me(r) {
  const Q = Pt(r.options.render.theme.rapidOpacity ?? 0.3);
...
function He(r) {
  ...
  if (B === t)
    return;
  const A = r.stream.line.geometry.getAttribute("color"), e = je(r.options), P = Math.min(t, B), i = Math.max(t, B);
  if (B > t)
    for (let s = t; s < B; s += 1) {
      const v = s * 3;
  ...
  A.clearUpdateRanges(), A.addUpdateRange(P * 3, (i - P) * 3), A.needsUpdate = !0, r.stream.greyCursorVertex = B;
}
function kA(r, Q, B) {
  const t = B.render.theme.colors, A = B.mode.laser ? t.laser ?? t.cutting : t.cutting, e = new h.Color(r === "rapid" ? t.rapid : A), P = new Float32Array(Q * 3);
  for (let i = 0; i < Q; i += 1) {
    const s = i * 3;
...
function Fe(r) {
  if (r.bucketCount <= 1)
    return Pt(r.baseOpacity);
  const Q = r.bucketIndex / (r.bucketCount - 1);
  return Pt(r.baseOpacity * Q);
}
function Pt(r) {
...
async function Ge(r, Q, B, t) {
  var M, b, G;
  const A = Math.max(2, Math.floor(B)), e = (t == null ? void 0 : t.arcSegments) ?? 30, P = (t == null ? void 0 : t.checkpointEveryLines) ?? 2e3, i = ((M = t == null ? void 0 : t.batch) == null ? void 0 : M.yieldEveryLines) ?? 1e4, s = ((b = t == null ? void 0 : t.batch) == null ? void 0 : b.shouldAbort) ?? (() => !1), v = (G = t == null ? void 0 : t.batch) == null ? void 0 : G.onProgress;
  if (r.length === 0) {
    const C = {
      xMin: -1,
      xMax: 1,
  ...
  const d = new et({
    onLinearMove(C) {
      ...
      O.push(H), it(H, U, u, Q, A);
    },
    onArcMove(C) {
      const j = C.transformedStart ?? C.start, F = C.transformedEnd ?? C.end, H = C.transformedCenter ?? C.center, f = be(j, F, H, C.plane, C.motion, e);
      for (const I of f) {
        const R = { lineIndex: N, ...I };
        O.push(R), it(R, U, u, Q, A);
      }
    }
  });
...
function it(r, Q, B, t, A) {
  const e = (B.xMax - B.xMin) / A, P = (B.yMax - B.yMin) / A, i = Math.hypot(r.x1 - r.x0, r.y1 - r.y0), s = Math.max(1e-3, Math.min(e, P) * 0.5), v = Math.max(1, Math.ceil(i / s)), c = Math.ceil(t / e), n = Math.ceil(t / P), T = t * t;
  for (let z = 0; z <= v; z++) {
    const w = z / v, o = r.x0 + (r.x1 - r.x0) * w, a = r.y0 + (r.y1 - r.y0) * w, E = r.z0 + (r.z1 - r.z0) * w, l = Math.floor((o - B.xMin) / e), u = Math.floor((a - B.yMin) / P);
    for (let U = -c; U <= c; U++) {
      const k = l + U;
      if (k < 0 || k >= A) continue;
      const O = B.xMin + (k + 0.5) * e, D = (O - o) * (O - o);
      if (!(D > T))
        for (let N = -n; N <= n; N++) {
          const d = u + N;
          if (d < 0 || d >= A) continue;
          const M = B.yMin + (d + 0.5) * P;
          if (D + (M - a) * (M - a) > T) continue;
          const G = d * A + k;
          Q[G] > E && (Q[G] = E);
        }
    }
...
function SB(r, Q) {
  ...
  for (const e of Q.moves)
    e.lineIndex > B && e.lineIndex <= r && it(e, A, Q.slabBounds, Q.toolRadius, Q.resolution);
  return A;
}
function YB(r, Q, B, t) {
  if (B <= 0) return;
...
function be(r, Q, B, t, A, e) {
  ...
  let o = { ...r };
  for (let a = 1; a <= P; a++) {
    const E = a / P, l = n + z * E, u = E, U = B[i] + c * Math.cos(l), k = B[s] + c * Math.sin(l), O = r[v] + (Q[v] - r[v]) * u, D = { X: 0, Y: 0, Z: 0 };
    D[i] = U, D[s] = k, D[v] = O, w.push({
      x0: o.X,
      y0: o.Y,
      z0: o.Z,
      x1: D.X,
...
function ge() {
  ...
  for (let A = 0; A < t; A++) {
    ...
    B.strokeStyle = `rgb(${i},${s},${v})`, B.lineWidth = P, B.beginPath(), B.moveTo(0, e), B.lineTo(r, e), B.stroke();
  }
  for (let A = 0; A < 300; A++) {
    ...
    B.strokeStyle = `rgba(${v},${c},${n},0.35)`, B.lineWidth = 0.5 + Math.random() * 1, B.beginPath(), B.moveTo(e, P), B.quadraticCurveTo(
      e + i * 0.5 * Math.cos(s),
...
function pe(r, Q) {
  ...
  const t = r.xMax - r.xMin, A = r.yMax - r.yMin, e = r.zTop - r.zBot, P = (r.xMin + r.xMax) / 2, i = (r.yMin + r.yMax) / 2, s = (r.zBot + r.zTop) / 2, v = ge();
  ...
  l.name = "gviewer:sim3d-top", l.position.set(P, i, r.zTop), B.add(l);
  const u = new h.PlaneGeometry(t, e);
  u.rotateX(Math.PI / 2);
  const U = new h.Mesh(u, n);
  U.position.set(P, r.yMax, s), B.add(U);
  const k = new h.PlaneGeometry(t, e);
  k.rotateX(Math.PI / 2);
  const O = new h.Mesh(k, n);
  O.position.set(P, r.yMin, s), B.add(O);
  ...
  return b.position.set(P, i, r.zBot), B.add(b), { group: B, topMesh: l, topPositionAttr: E, slabBounds: r, resolution: Q };
}
function VB(r, Q) {
  const { topPositionAttr: B, slabBounds: t, resolution: A } = r, e = A * A;
  for (let P = 0; P < e; P++) {
...
const me = ".gViewer-viewcube{position:absolute;left:60px;bottom:60px;width:84px;height:84px;z-index:9999;perspective:600px;pointer-events:auto}.gViewer-viewcube__cube{position:absolute;top:0;right:0;bottom:0;left:0;transform-style:preserve-3d;transition:transform 60ms linear}.gViewer-viewcube__face{position:absolute;width:84px;height:84px;display:flex;align-items:center;justify-content:center;font:600 11px system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;letter-spacing:.06em;text-transform:uppercase;color:#f0f5fce0;background:#0c1014b3;border:1px solid rgba(120,140,160,.35);border-radius:14px;cursor:pointer;-webkit-user-select:none;user-select:none;padding:0}.gViewer-viewcube__face:hover{background:#0c1014db;border-color:#a0bedc8c}.gViewer-viewcube__face.is-active{border-color:#60a5faf2;box-shadow:0 0 0 1px #60a5fa59,0 10px 30px #00000059;color:#60a5faf2}.gViewer-viewcube__face--front{transform:translateZ(42px)}.gViewer-viewcube__face--back{transform:rotateY(180deg) translateZ(42px)}.gViewer-viewcube__face--left{transform:rotateY(-90deg) translateZ(42px)}.gViewer-viewcube__face--right{transform:rotateY(90deg) translateZ(42px)}.gViewer-viewcube__face--top{transform:rotateX(-90deg) translateZ(42px)}.gViewer-viewcube__face--bottom{transform:rotateX(90deg) translateZ(42px)}@media (orientation: portrait){.gViewer-viewcube{left:60px;bottom:110px;width:64px;height:64px}.gViewer-viewcube__face{width:64px;height:64px}.gViewer-viewcube__face--front{transform:translateZ(32px)}.gViewer-viewcube__face--back{transform:rotateY(180deg) translateZ(32px)}.gViewer-viewcube__face--left{transform:rotateY(-90deg) translateZ(32px)}.gViewer-viewcube__face--right{transform:rotateY(90deg) translateZ(32px)}.gViewer-viewcube__face--top{transform:rotateX(-90deg) translateZ(32px)}.gViewer-viewcube__face--bottom{transform:rotateX(90deg) translateZ(32px)}}", Rt = "gviewer-viewcube-style";
function ye() {
  if (typeof document > "u" || document.getElementById(Rt)) return;
  const r = document.createElement("style");
  r.id = Rt, r.setAttribute("data-gviewer", "viewcube"), r.textContent = me, document.head.appendChild(r);
}
class Ie {
  constructor(Q) {
    ye(), this.onSelectView = Q.onSelectView, this.faceButtons = /* @__PURE__ */ new Map(), this.root = document.createElement("div"), this.root.className = "gViewer-viewcube", this.cube = document.createElement("div"), this.cube.className = "gViewer-viewcube__cube", this.root.appendChild(this.cube);
    const B = [
...
function MQ(r, Q, B = {}) {
  const { opacity: t = 1, size: A = 10 } = B, e = new h.Object3D(), P = 100, i = document.createElement("canvas"), s = i.getContext("2d");
  if (!s)
  ...
  e.textHeight = A, e.textWidth = c / P * e.textHeight, B.textAlign === "left" ? e.position.x = (B.x ?? 0) + (e.textWidth ?? 0) / 2 : B.textAlign === "right" ? e.position.x = (B.x ?? 0) - (e.textWidth ?? 0) / 2 : e.position.x = B.x ?? 0, B.textBaseline === "top" ? e.position.y = (B.y ?? 0) - (e.textHeight ?? 0) / 2 : B.textBaseline === "bottom" ? e.position.y = (B.y ?? 0) + (e.textHeight ?? 0) / 2 : e.position.y = B.y ?? 0, e.position.z = B.z ?? 0;
...
function Ve(r, Q) {
  ...
  return A.transparent = !0, A.opacity = 0.65, A.depthWrite = !1, A.linewidth = 2, B.add(t), Q.boundingBox.labels && B.add(...Je(r.clone(), Q)), B;
}
function Je(r, Q) {
  const B = Q.render.theme.colors.boundingBox, t = 0.8, A = (u) => Math.abs(u) < 1e-9 ? "0" : u.toFixed(3), e = (u) => Q.units === "in" ? `${A(u / Ye)} in` : `${A(u)} mm`, P = r.min, i = r.max, s = (u) => MQ(u, B, { size: 4, opacity: t }), v = s(e(P.x)), c = s(e(i.x)), n = s(e(P.y)), T = s(e(i.y)), z = s(e(P.z)), w = s(e(i.z)), o = new h.Vector3();
  r.getSize(o);
  const a = Math.max(2, Math.max(o.x, o.y, o.z) * 0.02), E = (P.x + i.x) / 2, l = (P.y + i.y) / 2;
  return v.position.set(P.x - a, l, P.z - a), c.position.set(i.x + a, l, P.z - a), n.position.set(E, P.y - a, P.z - a), T.position.set(E, i.y + a, P.z - a), z.position.set(E, l, P.z - a), w.position.set(E, l, i.z + a), [v, c, n, T, z, w];
}
function Re(r) {
  r.traverse((Q) => {
    var B;
...
function Ke(r, Q, B, t) {
  const A = new h.Group();
  if (A.add(Xt(r, Q, B.render.theme.colors.machineBed)), t) {
    const e = B.render.theme.colors.machineBedKeepout;
    A.add(Xt(t.min, t.max, e)), A.add(Xe(t.min, t.max, e));
  }
  return A;
}
function Ze(r) {
...
function _e(r, Q) {
  const B = r.x - Q.x, t = r.y - Q.y, A = r.z - Q.z, e = Math.abs(B), P = Math.abs(t), i = Math.abs(A);
  return i >= e && i >= P ? A >= 0 ? "top" : "bottom" : e >= P ? B >= 0 ? "right" : "left" : t >= 0 ? "back" : "front";
}
const uA = 25.4;
function ut(r) {
  if (r.bounds)
...
function $e(r) {
  const { minX: Q, maxX: B, minY: t, maxY: A } = ut(r), e = r.units === "mm" ? 10 : uA, P = [], i = [], s = (w, o, a, E, l) => {
    l.push(w, o, 0, a, E, 0);
  }, v = Math.ceil(t / e) * e, c = Math.floor(A / e) * e;
  for (let w = v; w <= c + 1e-6; w += e)
    s(Q, w, B, w, Math.abs(w) < 1e-6 ? P : i);
  const n = Math.ceil(Q / e) * e, T = Math.floor(B / e) * e;
  for (let w = n; w <= T + 1e-6; w += e)
    s(w, t, w, A, Math.abs(w) < 1e-6 ? P : i);
  const z = new h.Group();
  return i.length > 0 && z.add(Zt(i, r.theme.colors.grid.minor, 0.25)), P.length > 0 && z.add(Zt(P, r.theme.colors.grid.major, 0.4)), z;
}
function Zt(r, Q, B) {
...
function BP(r) {
  const Q = new h.Group(), { minX: B, maxX: t, minY: A, maxY: e } = ut({
    sizeXMm: r.sizeXWorld,
    sizeYMm: r.sizeYWorld,
    bounds: r.bounds
  }), P = Math.max(1, r.depthWorld), i = Math.max(1, Math.min(Math.max(t - B, e - A) / 24, 30)), s = i * 0.6, v = (E) => new h.LineDashedMaterial({
    color: new h.Color(E),
    dashSize: i,
    gapSize: s,
    transparent: !0,
  ...
  return a.position.set(0, 0, P + z), Q.add(w, o, a), Q;
}
function tP(r) {
  r.traverse((Q) => {
    var B;
...
function AP(r) {
  const Q = new h.Group(), { minX: B, maxX: t, minY: A, maxY: e } = ut(r), P = r.units === "mm" ? 10 : 1, i = P * 2, s = r.units === "mm" ? 1 : uA, v = 0.5, c = 4, n = 0.1, T = 5, z = r.theme.colors.axes.x, w = r.theme.colors.axes.y, o = { opacity: v, size: c };
  for (let a = P; ; a += i) {
    const E = a * s;
    if (E > t + 1e-6)
      break;
...
var zQ = {
  ...
  overwrite: !1,
  delay: 0
}, Ut, q, y, hQ = 1e8, x = 1 / hQ, rt = Math.PI * 2, eP = rt / 4, PP = 0, DA = Math.sqrt, iP = Math.cos, rP = Math.sin, Z = function(Q) {
...
}, $ = Array.isArray, sP = /random\([^)]+\)/g, vP = /,\s*/g, Wt = /(?:-?\.?\d|\.)+/gi, CA = /[-+=.]*\d+[.e\-+]*\d*[e\-+]*\d*/g, qQ = /[-+=.]*\d+[.e-]*\d*[a-z%]*/g, JB = /[-+=.]*\d+\.?\d*(?:e-|e\+)?\d*/gi, NA = /[+-]=-?[.\d]+/, cP = /[^,'"\[\]\s]+/gi, nP = /^[+\-=e\s\d]*\d+[.\d]*([a-z]*|%)\s*$/i, Y, kQ, st, Ct, wQ = {}, FB = {}, OA, LA = function(Q) {
  ...
  suppressEvents: !0
}, Ot = {}, fQ = [], vt = {}, HA, cQ = {}, RB = {}, qt = 30, MB = [], Lt = "", Mt = function(Q) {
  var B = Q[0], t, A;
  if (DQ(B) || J(B) || (Q = [Q]), !(t = (B._gsap || {}).harness)) {
    for (A = MB.length; A-- && !MB[A].targetTest(B); )
      ;
    t = MB[A];
  }
  for (A = Q.length; A--; )
    Q[A] && (Q[A]._gsap || (Q[A]._gsap = new _A(Q[A], t))) || Q.splice(A, 1);
  return Q;
}, YQ = function(Q) {
  return Q._gsap || Mt(lQ(Q))[0]._gsap;
}, jA = function(Q, B, t) {
  return (t = Q[B]) && J(t) ? Q[B]() : Dt(t) && Q.getAttribute && Q.getAttribute(B) || t;
}, eQ = function(Q, B) {
  ...
  var B = parseFloat(Q);
  return (B || B === 0) && (Q + "").match(cP).length < 2 ? B : Z(Q) ? Q.trim() : Q;
}, GA = function(Q) {
  return Q;
}, aQ = function(Q, B) {
  for (var t in B)
  ...
  for (var t in B)
    t !== "__proto__" && t !== "constructor" && t !== "prototype" && (Q[t] = DQ(B[t]) ? r(Q[t] || (Q[t] = {}), B[t]) : B[t]);
  return Q;
}, GB = function(Q, B) {
  var t = {}, A;
  for (A in Q)
  ...
  var B = Q.parent || Y, t = Q.keyframes ? wP($(Q.keyframes)) : aQ;
  if (AQ(Q.inherit))
    for (; B; )
  ...
  return B.parent && bQ(B), B._start = S((NQ(t) ? t : t || Q !== Y ? EQ(Q, t, B) : Q._time) + B._delay), B._end = S(B._start + (B.totalDuration() / Math.abs(B.timeScale()) || 0)), bA(Q, B, "_first", "_last", Q._sort ? "_start" : 0), nt(B) || (Q._recent = B), A || gA(Q, B), Q._ts < 0 && IB(Q, Q._tTime), Q;
}, pA = function(Q, B) {
  return (wQ.ScrollTrigger || Nt("scrollTrigger", B)) && wQ.ScrollTrigger.create(B, Q);
}, xA = function(Q, B, t, A, e) {
  if (Ft(Q, B, e), !Q._initted)
    return 1;
  if (!t && Q._pt && !q && (Q._dur && Q.vars.lazy !== !1 || !Q._dur && Q.vars.lazy) && HA !== nQ.frame)
  ...
  var e = Q.ratio, P = B < 0 || !B && (!Q._start && lP(Q) && !(!Q._initted && nt(Q)) || (Q._ts < 0 || Q._dp._ts < 0) && !nt(Q)) ? 0 : 1, i = Q._rDelay, s = 0, v, c, n;
  ...
  var e = Q._repeat, P = S(B) || 0, i = Q._tTime / Q._tDur;
  return i && !A && (Q._time *= P / Q._dur), Q._dur = P, Q._tDur = e ? e < 0 ? 1e10 : S(P * (e + 1) + Q._rDelay * e) : P, i > 0 && !A && IB(Q, Q._tTime = Q._tDur * i), Q.parent && yB(Q), t || VQ(Q.parent, Q), Q;
}, QA = function(Q) {
  return Q instanceof tQ ? VQ(Q) : eB(Q, Q._dur);
}, UP = {
  _start: 0,
  endTime: hB,
  totalDuration: hB
}, EQ = function r(Q, B, t) {
  var A = Q.labels, e = Q._recent || UP, P = Q.duration() >= hQ ? e.endTime(!1) : Q._dur, i, s, v;
  return Z(B) && (isNaN(B) || B in A) ? (s = B.charAt(0), v = B.substr(-1) === "%", i = B.indexOf("="), s === "<" || s === ">" ? (i >= 0 && (B = B.replace(/=/, "")), (s === "<" ? e._start : e.endTime(e._repeat >= 0)) + (parseFloat(B.substr(1)) || 0) * (v ? (i < 0 ? e : t).totalDuration() / 100 : 1)) : i < 0 ? (B in A || (A[B] = P), A[B]) : (s = parseFloat(B.charAt(i - 1) + B.substr(i + 1)), v && t && (s = s / 100 * ($(t) ? t[0] : t).totalDuration()), i > 1 ? r(Q, B.substr(0, i - 1), t) + s : P + s)) : B == null ? P : +B;
}, zB = function(Q, B, t) {
  var A = NQ(B[1]), e = (A ? 2 : 1) + (Q < 2 ? 0 : 1), P = B[e], i, s;
  if (A && (P.duration = B[1]), P.parent = t, Q) {
    for (i = P, s = t; s && !("immediateRender" in i); )
  ...
  return !Z(Q) || !(B = nP.exec(Q)) ? "" : B[1];
}, DP = function(Q, B, t) {
  return xQ(t, function(A) {
    return dB(Q, B, A);
  });
}, ot = [].slice, mA = function(Q, B) {
  return Q && DQ(Q) && "length" in Q && (!B && !Q.length || Q.length - 1 in Q && DQ(Q[0])) && !Q.nodeType && Q !== kQ;
}, dP = function(Q, B, t) {
  return t === void 0 && (t = []), Q.forEach(function(A) {
    var e;
    return Z(A) && !B || mA(A, 1) ? (e = t).push.apply(e, lQ(A)) : t.push(A);
  }) || t;
}, lQ = function(Q, B, t) {
  return y && !B && y.selector ? y.selector(Q) : Z(Q) && !t && (st || !PB()) ? ot.call((B || Ct).querySelectorAll(Q), 0) : $(Q) ? dP(Q, t) : mA(Q) ? ot.call(Q, 0) : Q ? [Q] : [];
}, Tt = function(Q) {
  ...
  var B = DQ(Q) ? Q : {
    each: Q
  }, t = JQ(B.ease), A = B.from || 0, e = parseFloat(B.base) || 0, P = {}, i = A > 0 && A < 1, s = isNaN(A) || i, v = B.axis, c = A, n = A;
  return Z(A) ? c = n = {
    center: 0.5,
    edges: 0.5,
    end: 1
  }[A] || 0 : !i && s && (c = A[0], n = A[1]), function(T, z, w) {
    var o = (w || B).length, a = P[o], E, l, u, U, k, O, D, N, d;
    if (!a) {
      if (d = B.grid === "auto" ? 0 : (B.grid || [1, hQ])[1], !d) {
        for (D = -hQ; D < (D = w[d++].getBoundingClientRect().left) && d < o; )
          ;
        d < o && d--;
      }
      for (a = P[o] = [], E = s ? Math.min(d, o) * c - 0.5 : A % d, l = d === hQ ? 0 : s ? o * n / d - 0.5 : A / d | 0, D = 0, N = hQ, O = 0; O < o; O++)
        u = O % d - E, U = l - (O / d | 0), a[O] = k = v ? Math.abs(v === "y" ? U : u) : DA(u * u + U * U), k > D && (D = k), k < N && (N = k);
      A === "random" && yA(a), a.max = D - N, a.min = N, a.v = o = (parseFloat(B.amount) || parseFloat(B.each) * (d > o ? o - 1 : v ? v === "y" ? o / d : d : Math.max(d, o / d)) || 0) * (A === "edges" ? -1 : 1), a.b = o < 0 ? e - o : e, a.u = _(B.amount || B.each) || 0, t = t && o < 0 ? pP(t) : t;
    }
  ...
  var A = B - Q;
  return $(Q) ? VA(Q, r(0, Q.length), B) : xQ(t, function(e) {
    return (A + (e - Q) % A) % A + Q;
  });
}, MP = function r(Q, B, t) {
  var A = B - Q, e = A * 2;
  return $(Q) ? VA(Q, r(0, Q.length - 1), B) : xQ(t, function(P) {
    return P = (e + (P - Q) % e) % e || 0, Q + (P > A ? e - P : P);
  });
}, lB = function(Q) {
  return Q.replace(sP, function(B) {
    var t = B.indexOf("[") + 1, A = B.substring(t || 7, t ? B.indexOf("]") : B.length - 1).split(vP);
    return YA(t ? A : +A[0], t ? 0 : +A[1], +A[2] || 1e-5);
  });
}, JA = function(Q, B, t, A, e) {
  var P = B - Q, i = A - t;
  return xQ(e, function(s) {
    return t + ((s - Q) / P * i || 0);
  });
}, HP = function r(Q, B, t, A) {
  var e = isNaN(Q + B) ? 0 : function(z) {
    return (1 - z) * Q + z * B;
  };
  if (!e) {
  ...
  for (P in A)
    i = A[P] - B, i < 0 == !!t && i && e > (i = Math.abs(i)) && (s = P, e = i);
  return s;
}, oQ = function(Q, B, t) {
  var A = Q.vars, e = A[B], P = y, i = Q._ctx, s, v, c;
  if (e)
    return s = A[B + "Params"], v = A.callbackScope || Q, t && fQ.length && fB(), i && (y = i), c = s ? e.apply(v, s) : e.call(v), y = P, c;
}, nB = function(Q) {
  ...
  if (Q)
    if (Q = !Q.name && Q.default || Q, dt() || Q.headless) {
      ...
      if (PB(), Q !== A) {
        if (cQ[B])
          return;
        aQ(A, aQ(GB(Q, e), P)), tB(A.prototype, tB(e, GB(Q, P))), cQ[A.prop = B] = A, Q.targetTest && (MB.push(A), Ot[B] = 1), B = (B === "css" ? "CSS" : B.charAt(0).toUpperCase() + B.substr(1)) + "Plugin";
      }
      MA(B, A), Q.register && Q.register(rQ, A, PQ);
    } else
      RA.push(Q);
}, p = 255, oB = {
  aqua: [0, p, p],
  ...
  var A = Q ? NQ(Q) ? [Q >> 16, Q >> 8 & p, Q & p] : 0 : oB.black, e, P, i, s, v, c, n, T, z, w;
  if (!A) {
    if (Q.substr(-1) === "," && (Q = Q.substr(0, Q.length - 1)), oB[Q])
      ...
      if (A = w = Q.match(Wt), !B)
        s = +A[0] % 360 / 360, v = +A[1] / 100, c = +A[2] / 100, P = c <= 0.5 ? c * (v + 1) : c + v - c * v, e = c * 2 - P, A.length > 3 && (A[3] *= 1), A[0] = XB(s + 1 / 3, e, P), A[1] = XB(s, e, P), A[2] = XB(s - 1 / 3, e, P);
      else if (~Q.indexOf("="))
        return A = Q.match(CA), t && A.length < 4 && (A[3] = 1), A;
    } else
      A = Q.match(Wt) || oB.transparent;
    A = A.map(Number);
  }
  return B && !w && (e = A[0] / p, P = A[1] / p, i = A[2] / p, n = Math.max(e, P, i), T = Math.min(e, P, i), c = (n + T) / 2, n === T ? s = v = 0 : (z = n - T, v = c > 0.5 ? z / (2 - n - T) : z / (n + T), s = n === e ? (P - i) / z + (P < i ? 6 : 0) : n === P ? (i - e) / z + 2 : (e - P) / z + 4, s *= 60), A[0] = ~~(s + 0.5), A[1] = ~~(v * 100 + 0.5), A[2] = ~~(c * 100 + 0.5)), t && A.length < 4 && (A[3] = 1), A;
}, ZA = function(Q) {
  var B = [], t = [], A = -1;
  return Q.split(GQ).forEach(function(e) {
    var P = e.match(qQ) || [];
    B.push.apply(B, P), t.push(A += P.length + 1);
  }), B.c = t, B;
}, tA = function(Q, B, t) {
  var A = "", e = (Q + A).match(GQ), P = B ? "hsla(" : "rgba(", i = 0, s, v, c, n;
  if (!e)
    return Q;
  if (e = e.map(function(T) {
    return (T = KA(T, B, 1)) && P + (B ? T[0] + "," + T[1] + "%," + T[2] + "%," + T[3] : T.join(",")) + ")";
  }), t && (c = ZA(Q), s = t.c, s.join(A) !== c.c.join(A)))
    for (v = Q.replace(GQ, "1").split(qQ), n = v.length - 1; i < n; i++)
      A += v[i] + (~s.indexOf(i) ? e.shift() || P + "0,0,0,0)" : (c.length ? c : e.length ? e : t).shift());
  if (!v)
...
}(), jP = /hsl[a]?\(/, WA = function(Q) {
  var B = Q.join(" "), t;
  if (GQ.lastIndex = 0, GQ.test(B))
    return t = jP.test(B), Q[1] = tA(Q[1], t), Q[0] = tA(Q[0], t, ZA(Q[1])), !0;
}, kB, nQ = function() {
  var r = Date.now, Q = 500, B = 33, t = r(), A = t, e = 1e3 / 240, P = e, i = [], s, v, c, n, T, z, w = function o(a) {
    var E = r() - A, l = a === !0, u, U, k, O;
    if ((E > Q || E < 0) && (t += E - B), A += E, k = A - t, u = k - P, (u > 0 || l) && (O = ++n.frame, T = k - n.time * 1e3, n.time = k = k / 1e3, P += u + (u >= e ? 4 : e - u), U = 1), l || (s = v(o)), U)
  ...
  return n = {
    ...
    wake: function() {
      OA && (!st && dt() && (kQ = st = window, Ct = kQ.document || {}, wQ.gsap = rQ, (kQ.gsapVersions || (kQ.gsapVersions = [])).push(rQ.version), LA(FB || kQ.GreenSockGlobals || !kQ.gsap && kQ || {}), RA.forEach(XA)), c = typeof requestAnimationFrame < "u" && requestAnimationFrame, s && n.sleep(), v = c || function(a) {
        return setTimeout(a, P - n.time * 1e3 + 1 | 0);
      }, kB = 1, w(2));
    },
    sleep: function() {
    ...
    fps: function(a) {
      e = 1e3 / (a || 240), P = n.time * 1e3 + e;
    },
    add: function(a, E, l) {
      var u = E ? function(U, k, O, D) {
        a(U, k, O, D), n.remove(u);
      } : a;
      return n.remove(a), i[l ? "unshift" : "push"](u), PB(), u;
    },
    remove: function(a, E) {
  ...
  return !kB && nQ.wake();
}, g = {}, FP = /^[\d.\-M][\d.\-,\s]/, fP = /["']/g, GP = function(Q) {
  for (var B = {}, t = Q.substr(1, Q.length - 3).split(":"), A = t[0], e = 1, P = t.length, i, s, v; e < P; e++)
    s = t[e], i = e !== P - 1 ? s.lastIndexOf(",") : s.length, v = s.substr(0, i), B[A] = isNaN(v) ? v.replace(fP, "").trim() : +v, A = s.substr(i + 1).trim();
  return B;
}, bP = function(Q) {
  ...
  return t && B.length > 1 && t.config ? t.config.apply(null, ~Q.indexOf("{") ? [GP(B[1])] : bP(Q).split(",").map(fA)) : g._CE && FP.test(Q) ? g._CE("", Q) : t;
}, pP = function(Q) {
  return function(B) {
    return 1 - Q(1 - B);
  };
}, JQ = function(Q, B) {
  return Q && (J(Q) ? Q : g[Q] || gP(Q)) || B;
}, XQ = function(Q, B, t, A) {
  ...
  return eQ(Q, function(i) {
    g[i] = wQ[i] = e, g[P = i.toLowerCase()] = t;
    for (var s in e)
  ...
  var A = B >= 1 ? B : 1, e = (t || (Q ? 0.3 : 0.45)) / (B < 1 ? B : 1), P = e / rt * (Math.asin(1 / A) || 0), i = function(c) {
    return c === 1 ? 1 : A * Math.pow(2, -10 * c) * rP((c - P) * e) + 1;
  }, s = Q === "out" ? i : Q === "in" ? function(v) {
    return 1 - i(1 - v);
  } : qA(i);
  return e = rt / e, s.config = function(v, c) {
    return r(Q, v, c);
  }, s;
}, ZB = function r(Q, B) {
  B === void 0 && (B = 1.70158);
  var t = function(P) {
    return P ? --P * P * ((B + 1) * P + B) + 1 : 0;
  }, A = Q === "out" ? t : Q === "in" ? function(e) {
    return 1 - t(1 - e);
  } : qA(t);
...
(function(r, Q) {
  var B = 1 / Q, t = 2 * B, A = 2.5 * B, e = function(i) {
...
var _A = function(Q, B) {
  this.id = PP++, Q._gsap = this, this.target = Q, this.harness = B, this.get = B ? B.get : jA, this.set = B ? B.getSetter : Gt;
}, uB = /* @__PURE__ */ function() {
  function r(B) {
    this.vars = B, this._delay = +B.delay || 0, (this._repeat = B.repeat === 1 / 0 ? -2 : B.repeat || 0) && (this._rDelay = B.repeatDelay || 0, this._yoyo = !!B.yoyo || !!B.yoyoEase), this._ts = 1, eB(this, +B.duration, 1, 1), this.data = B.data, y && (this._ctx = y, y.data.push(this)), kB || nQ.wake();
  }
  var Q = r.prototype;
  return Q.delay = function(t) {
    ...
    return arguments.length ? (this._dirty = 0, eB(this, this._repeat < 0 ? t : (t - this._repeat * this._rDelay) / (this._repeat + 1))) : this._tDur;
  }, Q.totalTime = function(t, A) {
    if (PB(), !arguments.length)
      return this._tTime;
    var e = this._dp;
    if (e && e.smoothChildTiming && this._ts) {
      for (IB(this, t), !e._dp || e.parent || gA(e, this); e && e.parent; )
    ...
    return (this._tTime !== t || !this._dur && !A || this._initted && Math.abs(this._zTime) === x || !this._initted && this._dur && t || !t && !this._initted && (this.add || this._ptLookup)) && (this._ts || (this._pTime = t), FA(this, t, A)), this;
  }, Q.time = function(t, A) {
    return arguments.length ? this.totalTime(Math.min(this.totalDuration(), t + $t(this)) % (this._dur + this._rDelay) || (t ? this._dur : 0), A) : this._time;
    ...
    return arguments.length ? this.totalTime(this.duration() * (this._yoyo && !(this.iteration() & 1) ? 1 - t : t) + $t(this), A) : this.duration() ? Math.min(1, this._time / this._dur) : this.rawTime() > 0 ? 1 : 0;
    ...
    return this._rts = +t || 0, this._ts = this._ps || t === -x ? 0 : this._rts, this.totalTime(dB(-Math.abs(this._delay), this.totalDuration(), e), A !== !1), yB(this), EP(this);
  }, Q.paused = function(t) {
    ...
    for (var A = this, e = arguments.length ? t : A.rawTime(); A; )
      e = A._start + e / (Math.abs(A._ts) || 1), A = A._dp;
    return !this.parent && this._sat ? this._sat.globalTime(t) : e;
  }, Q.repeat = function(t) {
    return arguments.length ? (this._repeat = t === 1 / 0 ? -2 : t, QA(this)) : this._repeat === -2 ? 1 / 0 : this._repeat;
  }, Q.repeatDelay = function(t) {
    if (arguments.length) {
      var A = this._time;
      return this._rDelay = t, QA(this), A ? this.time(A) : this;
    }
    return this._rDelay;
  }, Q.yoyo = function(t) {
...
var tQ = /* @__PURE__ */ function(r) {
  ...
  function Q(t, A) {
    var e;
    return t === void 0 && (t = {}), e = r.call(this, t) || this, e.labels = {}, e.smoothChildTiming = !!t.smoothChildTiming, e.autoRemoveChildren = !!t.autoRemoveChildren, e._sort = AQ(t.sortChildren), Y && uQ(t.parent || Y, dQ(e), A), t.reversed && e.reverse(), t.paused && e.paused(!0), t.scrollTrigger && pA(dQ(e), t.scrollTrigger), e;
  }
  var B = Q.prototype;
  return B.to = function(A, e, P) {
    return zB(0, arguments, this), this;
  }, B.from = function(A, e, P) {
    return zB(1, arguments, this), this;
  }, B.fromTo = function(A, e, P, i) {
    return zB(2, arguments, this), this;
  }, B.set = function(A, e, P) {
    return e.duration = 0, e.parent = this, TB(e).repeatDelay || (e.repeat = 0), e.immediateRender = !!e.immediateRender, new K(A, e, EQ(this, P), 1), this;
  }, B.call = function(A, e, P) {
    return uQ(this, K.delayedCall(0, A, e), P);
  }, B.staggerTo = function(A, e, P, i, s, v, c) {
    return P.duration = e, P.stagger = P.stagger || i, P.onComplete = v, P.onCompleteParams = c, P.parent = this, new K(A, P, EQ(this, s)), this;
  }, B.staggerFrom = function(A, e, P, i, s, v, c) {
    return P.runBackwards = 1, TB(P).immediateRender = AQ(P.immediateRender), this.staggerTo(A, e, P, i, s, v, c);
  }, B.staggerFromTo = function(A, e, P, i, s, v, c, n) {
    return i.startAt = P, TB(i).immediateRender = AQ(i.immediateRender), this.staggerTo(A, e, i, s, v, c, n);
  }, B.render = function(A, e, P) {
    var i = this._time, s = this._dirty ? this.totalDuration() : this._tDur, v = this._dur, c = A <= 0 ? 0 : S(A), n = this._zTime < 0 != A < 0 && (this._initted || !v), T, z, w, o, a, E, l, u, U, k, O, D;
    if (this !== Y && c > s && A >= 0 && (c = s), c !== this._tTime || P || n) {
      if (i !== this._time && v && (c += this._time - i, A += this._time - i), T = c, U = this._start, u = this._ts, E = !u, n && (v || (i = this._zTime), (A || !e) && (this._zTime = A)), this._repeat) {
        if (O = this._yoyo, a = v + this._rDelay, this._repeat < -1 && A < 0)
          return this.totalTime(a * 100 + A, e, P);
        if (T = S(c % a), c === s ? (o = this._repeat, T = v) : (k = S(c / a), o = ~~k, o && o === k && (T = v, o--), T > v && (T = v)), k = AB(this._tTime, a), !i && this._tTime && k !== o && this._tTime - k * a - this._dur <= 0 && (k = o), O && o & 1 && (T = v - T, D = 1), o !== k && !this._lock) {
          var N = O && k & 1, d = N === (O && o & 1);
          if (o < k && (N = !N), i = N ? 0 : c % v ? v : c, this._lock = 1, this.render(i || (D ? 0 : S(o * a)), e, !v)._lock = 0, this._tTime = c, !e && this.parent && oQ(this, "onRepeat"), this.vars.repeatRefresh && !D && (this.invalidate()._lock = 1, k = o), i && i !== this._time || E !== !this._ts || this.vars.onRepeat && !this.parent && !this._act)
            return this;
      ...
      if (this._hasPause && !this._forcing && this._lock < 2 && (l = uP(this, S(i), S(T)), l && (c -= T - (T = l._start))), this._tTime = c, this._time = T, this._act = !!u, this._initted || (this._onUpdate = this.vars.onUpdate, this._initted = 1, this._zTime = A, i = 0), !i && c && v && !e && !k && (oQ(this, "onStart"), this._tTime !== c))
        return this;
      if (T >= i && A >= 0)
        for (z = this._first; z; ) {
    ...
    var P = this;
    if (NQ(e) || (e = EQ(this, e, A)), !(A instanceof uB)) {
      if ($(A))
  ...
  }, B.getChildren = function(A, e, P, i) {
  ...
  }, B.addPause = function(A, e, P) {
    var i = K.delayedCall(0, e || hB, P);
  ...
  }, B.killTweensOf = function(A, e, P) {
    for (var i = this.getTweensOf(A, P), s = i.length; s--; )
      HQ !== i[s] && i[s].kill(A, e);
    return this;
  }, B.getTweensOf = function(A, e) {
    for (var P = [], i = lQ(A), s = this._first, v = NQ(e), c; s; )
      s instanceof K ? zP(s._targets, i) && (v ? (!HQ || s._initted && s._ts) && s.globalTime(0) <= e && s.globalTime(s.totalDuration()) > e : !e || s.isActive()) && P.push(s) : (c = s.getTweensOf(i, e)).length && P.push.apply(P, c), s = s._next;
    return P;
  }, B.tweenTo = function(A, e) {
    e = e || {};
    var P = this, i = EQ(P, A), s = e, v = s.startAt, c = s.onStart, n = s.onStartParams, T = s.immediateRender, z, w = K.to(P, aQ({
      ease: e.ease || "none",
      lazy: !1,
      immediateRender: !1,
      time: i,
      ...
      onStart: function() {
        if (P.pause(), !z) {
          ...
          w._dur !== a && eB(w, a, 0, 1).render(w._time, !0, !0), z = 1;
        }
        c && c.apply(w, n || []);
      }
    }, e));
    return T ? w.render(0) : w;
  }, B.tweenFromTo = function(A, e, P) {
    return this.tweenTo(e, aQ({
      startAt: {
        time: EQ(this, A)
      }
    }, P));
    ...
    return A === void 0 && (A = this._time), BA(this, EQ(this, A));
  }, B.previousLabel = function(A) {
    return A === void 0 && (A = this._time), BA(this, EQ(this, A), 1);
    ...
    if (P._dirty) {
      for (n = P.parent; i; )
        v = i._prev, i._dirty && i.totalDuration(), c = i._start, c > s && P._sort && i._ts && !P._lock ? (P._lock = 1, uQ(P, i, c - i._delay, 1)._lock = 0) : s = c, c < 0 && i._ts && (e -= c, (!n && !P._dp || n && n.smoothChildTiming) && (P._start += S(c / P._ts), P._time -= c, P._tTime -= c), P.shiftChildren(-c, !1, -1 / 0), s = 0), i._end > e && i._ts && (e = i._end), i = v;
      eB(P, P === Y && P._time > e ? P._time : e, 1, 1), P._dirty = 0;
    }
    return P._tDur;
  }, Q.updateRoot = function(A) {
    if (Y._ts && (FA(Y, bB(A, Y)), HA = nQ.frame), nQ.frame >= qt) {
      qt += zQ.autoSleep || 120;
      var e = Y._first;
...
var xP = function(Q, B, t, A, e, P, i) {
  var s = new PQ(this._pt, Q, B, 0, 1, ee, null, e), v = 0, c = 0, n, T, z, w, o, a, E, l;
...
}, jt = function(Q, B, t, A, e, P, i, s, v, c) {
  J(A) && (A = A(e || 0, Q, P));
  var n = Q[B], T = t !== "get" ? t : J(n) ? v ? Q[B.indexOf("set") || !J(Q["get" + B.substr(3)]) ? B : "get" + B.substr(3)](v) : Q[B]() : n, z = J(n) ? v ? YP : te : ft, w;
  if (Z(A) && (~A.indexOf("random(") && (A = lB(A)), A.charAt(1) === "=" && (w = $Q(T, A) + (_(T) || 0), (w || w === 0) && (A = w))), !c || T !== A || wt)
    return !isNaN(T * A) && A !== "" ? (w = new PQ(this._pt, Q, B, +T || 0, A - (T || 0), typeof n == "boolean" ? JP : Ae, 0, z), v && (w.fp = v), i && w.modifier(i, this, Q), this._pt = w) : (!n && !(B in Q) && Nt(B, A), xP.call(this, Q, B, T, A, z, s || zQ.stringFilter, v));
}, mP = function(Q, B, t, A, e) {
  ...
  for (i in Q)
    P[i] = wB(Q[i], e, B, t, A);
  return P;
}, $A = function(Q, B, t, A, e, P) {
  var i, s, v, c;
  if (cQ[Q] && (i = new cQ[Q]()).init(e, i.rawVars ? B[Q] : mP(B[Q], A, e, P, t), t, A, P) !== !1 && (t._pt = s = new PQ(t._pt, e, Q, 0, 1, i.render, i, 0, i.priority), t !== _Q))
    for (v = t._ptLookup[t._targets.indexOf(e)], c = i._props.length; c--; )
      v[i._props[c]] = s;
  return i;
}, HQ, wt, Ft = function r(Q, B, t) {
  var A = Q.vars, e = A.ease, P = A.startAt, i = A.immediateRender, s = A.lazy, v = A.onUpdate, c = A.runBackwards, n = A.yoyoEase, T = A.keyframes, z = A.autoRevert, w = Q._dur, o = Q._startAt, a = Q._targets, E = Q.parent, l = E && E.data === "nested" ? E.vars.targets : a, u = Q._overwrite === "auto" && !Ut, U = Q.timeline, k = A.easeReverse || n, O, D, N, d, M, b, G, C, j, F, H, f, I;
  if (U && (!T || !e) && (e = "none"), Q._ease = JQ(e, aB.ease), Q._rEase = k && (JQ(k) || Q._ease), Q._from = !U && !!A.runBackwards, Q._from && (Q.ratio = 1), !U || T && !A.stagger) {
    if (C = a[0] ? YQ(a[0]).harness : 0, f = C && A[C.prop], O = GB(A, Ot), o && (o._zTime < 0 && o.progress(1), B < 0 && c && i && !z ? o.render(-1, !0) : o.revert(c && w ? LB : oP), o._lazy = 0), P) {
      if (bQ(Q._startAt = K.set(a, aQ({
        ...
        stagger: 0
      }, P))), Q._startAt._dp = 0, Q._startAt._sat = Q, B < 0 && (q || !i && !z) && Q._startAt.revert(LB), i && w && B <= 0 && t <= 0) {
        B && (Q._zTime = B);
        return;
      }
    } else if (c && w && !o) {
    ...
    for (Q._pt = Q._ptCache = 0, s = w && AQ(s) || s && !w, D = 0; D < a.length; D++) {
      if (M = a[D], G = M._gsap || Mt(a)[D]._gsap, Q._ptLookup[D] = F = {}, vt[G.id] && fQ.length && fB(), H = l === a ? D : l.indexOf(M), C && (j = new C()).init(M, f || O, Q, H, l) !== !1 && (Q._pt = d = new PQ(Q._pt, M, j.name, 0, 1, j.render, j, 0, j.priority), j._props.forEach(function(R) {
        F[R] = d;
      }), j.priority && (b = 1)), !C || f)
        for (N in O)
          cQ[N] && (j = $A(N, O, Q, H, M, l)) ? j.priority && (b = 1) : F[N] = d = jt.call(Q, M, N, "get", O[N], H, l, 0, A.stringFilter);
      Q._op && Q._op[D] && Q.kill(M, Q._op[D]), u && Q._pt && (HQ = Q, Y.killTweensOf(M, F, Q.globalTime(B)), I = !Q.parent, HQ = 0), Q._pt && s && (vt[G.id] = 1);
    }
    b && Pe(Q), Q._onInit && Q._onInit(Q);
  }
  Q._onUpdate = v, Q._initted = (!Q._op || Q._pt) && !I, T && B <= 0 && U.render(hQ, !0, !0);
}, yP = function(Q, B, t, A, e, P, i, s) {
  ...
  for (z = v.length; z--; )
    n = v[z], c = n._pt || n, c.s = (A || A === 0) && !e ? A : c.s + (A || 0) + P * c.c, c.c = t - c.s, n.e && (n.e = X(t) + _(n.e)), n.b && (n.b = c.s + _(n.b));
}, IP = function(Q, B) {
  var t = Q[0] ? YQ(Q[0]).harness : 0, A = t && t.aliases, e, P, i, s;
  if (!A)
    return B;
  e = tB({}, B);
  for (P in A)
  ...
  var e = B.ease || A || "power1.inOut", P, i;
  if ($(B))
    ...
    for (P in B)
      i = t[P] || (t[P] = []), P === "ease" || i.push({
        t: parseFloat(Q),
        v: B[P],
        e
      });
}, wB = function(Q, B, t, A, e) {
  return J(Q) ? Q.call(B, t, A, e) : Z(Q) && ~Q.indexOf("random(") ? lB(Q) : Q;
}, Qe = Lt + "repeat,repeatDelay,yoyo,repeatRefresh,yoyoEase,easeReverse,autoRevert", Be = {};
...
var K = /* @__PURE__ */ function(r) {
  ...
  function Q(t, A, e, P) {
    var i;
    typeof A == "number" && (e.duration = A, A = e, e = null), i = r.call(this, P ? A : TB(A)) || this;
    var s = i.vars, v = s.duration, c = s.delay, n = s.immediateRender, T = s.stagger, z = s.overwrite, w = s.keyframes, o = s.defaults, a = s.scrollTrigger, E = A.parent || Y, l = ($(t) || dA(t) ? NQ(t[0]) : "length" in A) ? [t] : lQ(t), u, U, k, O, D, N, d, M;
    if (i._targets = l.length ? Mt(l) : EB("GSAP target " + t + " not found. https://gsap.com", !zQ.nullTargetWarn) || [], i._ptLookup = [], i._overwrite = z, w || T || OB(v) || OB(c)) {
      ...
      if (u = i.timeline = new tQ({
        ...
        for (U = 0; U < O; U++)
          k = GB(A, Be), k.stagger = 0, b && (k.easeReverse = b), M && tB(k, M), N = l[U], k.duration = +wB(v, dQ(i), U, N, l), k.delay = (+wB(c, dQ(i), U, N, l) || 0) - i._delay, !T && O === 1 && k.delay && (i._delay = c = k.delay, i._start += c, k.delay = 0), u.to(N, k, d ? d(U, N, l) : 0), u._ease = g.none;
        ...
        if ($(w))
          ...
          k = {};
          for (D in w)
            D === "ease" || D === "easeEach" || SP(D, w[D], k, w.easeEach);
          for (D in k)
            for (C = k[D].sort(function(H, f) {
              ...
              }, F[D] = j.v, u.to(l, F, G), G += F.duration;
          u.duration() < v && u.to({}, {
      ...
      i.timeline = 0;
    return z === !0 && !Ut && (HQ = dQ(i), Y.killTweensOf(l), HQ = 0), uQ(E, dQ(i), e), A.reversed && i.reverse(), A.paused && i.paused(!0), (n || !v && !w && i._start === S(E._time) && AQ(n) && hP(dQ(i)) && E.data !== "nested") && (i._tTime = -x, i.render(Math.max(0, -c) || 0)), a && pA(dQ(i), a), i;
  }
  var B = Q.prototype;
  return B.render = function(A, e, P) {
    ...
    if (!v)
      ...
      if (T = n, u = this.timeline, this._repeat) {
        ...
        if (T = S(n % o), n === s ? (w = this._repeat, T = v) : (a = S(n / o), w = ~~a, w && w === a ? (T = v, w--) : T > v && (T = v)), E = this._yoyo && w & 1, E && (T = v - T), a = AB(this._tTime, o), T === i && !P && this._initted && w === a)
          return this._tTime = n, this;
        w !== a && this.vars.repeatRefresh && !E && !this._lock && T !== o && this._initted && (this._lock = P = 1, this.render(S(o * w), !0).invalidate()._lock = 0);
      }
      if (!this._initted) {
        if (xA(this, c ? A : T, P, e, n))
          return this._tTime = 0, this;
        if (i !== this._time && !(P && this.vars.repeatRefresh && w !== a))
          return this;
        if (v !== this._dur)
          return this.render(A, e, P);
      }
      ...
      if (this._from && (this.ratio = l = 1 - l), this._tTime = n, this._time = T, !this._act && this._ts && (this._act = 1, this._lazy = 0), !i && n && !e && !a && (oQ(this, "onStart"), this._tTime !== n))
        return this;
      for (z = this._pt; z; )
        z.r(l, z.d), z = z._next;
      u && u.render(A < 0 ? A : u._dur * u._ease(T / this._dur), e, P) || this._startAt && (this._zTime = A), this._onUpdate && !e && (c && ct(this, A, e, P), oQ(this, "onUpdate")), this._repeat && w !== a && this.vars.onRepeat && !e && this.parent && oQ(this, "onRepeat"), (n === this._tDur || !n) && this._tTime === n && (c && !this._onUpdate && ct(this, A, !0, !0), (A || !v) && (n === this._tDur && this._ts > 0 || !n && this._ts < 0) && bQ(this, 1), !e && !(c && !i) && (n || i || E) && (oQ(this, n === s ? "onComplete" : "onReverseComplete", !0), this._prom && !(n < s && this.timeScale() > 0) && this._prom()));
    }
    return this;
  }, B.targets = function() {
  ...
  }, B.resetTo = function(A, e, P, i, s) {
    kB || nQ.wake(), this._ts || this.play();
    var v = Math.min(this._dur, (this._dp._time - this._start) * this._ts), c;
    return this._initted || Ft(this, v), c = this._ease(v / this._dur), yP(this, A, e, P, i, c, v, s) ? this.resetTo(A, e, P, i, 1) : (IB(this, 0), this.parent || bA(this._dp, this, "_first", "_last", this._dp._sort ? "_start" : 0), this.render(0));
  }, B.kill = function(A, e) {
    ...
    if (this.timeline) {
      ...
      return this.timeline.killTweensOf(A, e, HQ && HQ.vars.overwrite !== !0)._first || nB(this), this.parent && P !== this.timeline.totalDuration() && eB(this, this._dur * this.timeline._tDur / P, 0, 1), this;
    }
    var i = this._targets, s = A ? lQ(A) : i, v = this._ptLookup, c = this._pt, n, T, z, w, o, a, E;
    if ((!e || e === "all") && aP(i, s))
      return e === "all" && (this._pt = 0), nB(this);
    for (n = this._op = this._op || [], e !== "all" && (Z(e) && (o = {}, eQ(e, function(l) {
      return o[l] = 1;
    }), e = o), e = IP(i, e)), E = i.length; E--; )
      if (~s.indexOf(i[E])) {
...
var ft = function(Q, B, t) {
  ...
  return J(Q[B]) ? te : Dt(Q[B]) && Q.setAttribute ? VP : ft;
}, Ae = function(Q, B) {
  ...
  for (var B = Q._pt, t, A, e, P; B; ) {
    for (t = B._next, A = e; A && A.pr > B.pr; )
      A = A._next;
    (B._prev = A ? A._prev : P) ? B._prev._next = B : e = B, (B._next = A) ? A._prev = B : P = B, B = t;
  }
  Q._pt = e;
}, PQ = /* @__PURE__ */ function() {
  function r(B, t, A, e, P, i, s, v, c) {
    this.t = t, this.s = e, this.c = P, this.p = A, this.r = i || Ae, this.d = s || this, this.set = v || ft, this.pr = c || 0, this._next = B, B && (B._prev = this);
  }
  var Q = r.prototype;
  return Q.modifier = function(t, A, e) {
    this.mSet = this.mSet || this.set, this.set = KP, this.m = t, this.mt = e, this.tween = A;
  }, r;
}();
eQ(Lt + "parent,duration,ease,delay,overwrite,runBackwards,startAt,yoyo,immediateRender,repeat,repeatDelay,data,paused,reversed,lazy,callbackScope,stringFilter,id,yoyoEase,stagger,inherit,repeatRefresh,keyframes,autoRevert,scrollTrigger,easeReverse", function(r) {
  return Ot[r] = 1;
});
...
zQ.stringFilter = WA;
var RQ = [], HB = {}, ZP = [], AA = 0, WP = 0, WB = function(Q) {
  return (HB[Q] || ZP).map(function(B) {
    return B();
  });
}, at = function() {
  var Q = Date.now(), B = [];
  Q - AA > 2 && (WB("matchMediaInit"), RQ.forEach(function(t) {
    var A = t.queries, e = t.conditions, P, i, s, v;
    for (i in A)
  ...
  function r(B, t) {
    this.selector = t && Tt(t), this.data = [], this._r = [], this.isReverted = !1, this.id = WP++, B && this.add(B);
  }
  var Q = r.prototype;
  return Q.add = function(t, A, e) {
    J(t) && (e = A, A = t, t = J);
    var P = this, i = function() {
      var v = y, c = P.selector, n;
      return v && v !== P && v.data.push(P), e && (P.selector = Tt(e)), y = P, n = A.apply(P, arguments), J(n) && P._r.push(n), y = v, P.selector = c, P.isReverted = !1, n;
    };
    return P.last = i, t === J ? i(P, function(s) {
      return P.add(null, s);
    }) : t ? P[t] = i : i;
  }, Q.ignore = function(t) {
    var A = y;
    ...
    if (t ? function() {
      ...
      for (i.map(function(c) {
        ...
        v = e.data[s], v instanceof tQ ? v.data !== "nested" && (v.scrollTrigger && v.scrollTrigger.revert(), v.kill()) : !(v instanceof K) && v.revert && v.revert(t);
  ...
  return Q.add = function(t, A, e) {
    ...
    y && !P.selector && (P.selector = y.selector), this.contexts.push(P), A = P.add("onMatch", A), P.queries = t;
    for (v in t)
      v === "all" ? c = 1 : (s = kQ.matchMedia(t[v]), s && (RQ.indexOf(P) < 0 && RQ.push(P), (i[v] = s.matches) && (c = 1), s.addListener ? s.addListener(at) : s.addEventListener("change", at)));
  ...
  getProperty: function(Q, B, t, A) {
    Z(Q) && (Q = lQ(Q)[0]);
    var e = YQ(Q || {}).get, P = t ? GA : fA;
    return t === "native" && (t = ""), Q && (B ? P((cQ[B] && cQ[B].get || e)(Q, B, t, A)) : function(i, s, v) {
  ...
  quickSetter: function(Q, B, t) {
    ...
    Q = Q[0] || {};
    var P = cQ[B], i = YQ(Q), s = i.harness && (i.harness.aliases || {})[B] || B, v = P ? function(c) {
      var n = new P();
  ...
  quickTo: function(Q, B, t) {
    var A, e = rQ.to(Q, aQ((A = {}, A[B] = "+=0.1", A.paused = !0, A.stagger = 0, A), t || {})), P = function(s, v, c) {
      return e.resetTo(B, s, v, c);
    };
    return P.tween = e, P;
  },
  ...
  defaults: function(Q) {
    return Q && Q.ease && (Q.ease = JQ(Q.ease, aB.ease)), _t(aB, Q || {});
  },
  config: function(Q) {
    return _t(zQ, Q || {});
  },
  registerEffect: function(Q) {
    var B = Q.name, t = Q.effect, A = Q.plugins, e = Q.defaults, P = Q.extendTimeline;
    (A || "").split(",").forEach(function(i) {
      return i && !cQ[i] && !wQ[i] && EB(B + " effect requires " + i + " plugin.");
    }), RB[B] = function(i, s, v) {
      return t(lQ(i), aQ(s || {}, e), v);
    }, P && (tQ.prototype[B] = function(i, s, v) {
      return this.add(RB[B](i, DQ(s) ? s : (v = s) && {}, this), v);
    });
  },
  ...
  exportRoot: function(Q, B) {
    ...
    for (t.smoothChildTiming = AQ(Q.smoothChildTiming), Y.remove(t), t._dp = 0, t._time = t._tTime = Y._time, A = Y._first; A; )
      e = A._next, (B || !(!A._dur && A instanceof K && A.vars.onComplete === A._targets[0])) && uQ(t, A, A._start - A._delay), A = e;
    return uQ(Y, t, 0), t;
  },
  context: function(Q, B) {
    return Q ? new ie(Q, B) : y;
  },
...
var _P = function(Q, B) {
  ...
  var t = Q._targets, A, e, P;
  for (A in B)
    for (e = t.length; e--; )
      P = Q._ptLookup[e][A], P && (P = P.d) && (P._pt && (P = _P(P, A)), P && P.modifier && P.modifier(B[A], Q, t[e], A));
}, qB = function(Q, B) {
  return {
    name: Q,
    headless: 1,
  ...
  name: "attr",
  init: function(Q, B, t, A, e) {
    var P, i, s;
    this.tween = t;
    for (P in B)
      s = Q.getAttribute(P) || "", i = this.add(Q, "setAttribute", (s || 0) + "", B[P], A, e, 0, 0, P), i.op = P, i.b = s, this._props.push(P);
  },
  render: function(Q, B) {
    for (var t = B._pt; t; )
...
}, qB("roundProps", zt), qB("modifiers"), qB("snap", SA)) || gB;
 ...
 * @author: Jack Doyle, jack@greensock.com
*/
var eA, jQ, QB, gt, SQ, PA, pt, Qi = function() {
  return typeof window < "u";
}, OQ = {}, IQ = 180 / Math.PI, BB = Math.PI / 180, KQ = Math.atan2, iA = 1e8, xt = /([A-Z])/g, Bi = /(left|right|width|margin|padding|x)/i, ti = /[\s,\(]\S/, UQ = {
...
}, ni = function(Q, B, t, A, e) {
  var P = Q._gsap;
  P.scaleX = P.scaleY = t, P.renderTransform(e, P);
}, oi = function(Q, B, t, A, e) {
  var P = Q._gsap;
  P[B] = t, P.renderTransform(e, P);
}, V = "transform", iQ = V + "Origin", Ti = function r(Q, B) {
  var t = this, A = this.target, e = A.style, P = A._gsap;
  if (Q in OQ && e) {
    ...
    if (this.props.indexOf(V) >= 0)
      return;
    P.svg && (this.svgo = A.getAttribute("data-svg-origin"), this.props.push(iQ, B, "")), Q = V;
  }
  (e || B) && this.props.push(Q, B, e[Q]);
}, ve = function(Q) {
  ...
  if (this.tfm) {
    for (P in this.tfm)
      A[P] = this.tfm[P];
    A.svg && (A.renderTransform(), B.setAttribute("data-svg-origin", this.svgo || "")), e = pt(), (!e || !e.isStart) && !t[V] && (ve(t), A.zOrigin && t[iQ] && (t[iQ] += " " + A.zOrigin + "px", A.zOrigin = 0, A.renderTransform()), A.uncache = 1);
  }
}, ce = function(Q, B) {
  var t = {
    target: Q,
...
}, rA = "O,Moz,ms,Ms,Webkit".split(","), iB = function(Q, B, t) {
  ...
  Qi() && window.document && (eA = window, jQ = eA.document, QB = jQ.documentElement, SQ = ht("div") || {
    style: {}
  }, ht("div"), V = iB(V), iQ = V + "Origin", SQ.style.cssText = "border-width:0;line-height:0;position:absolute;padding:0", ne = !!iB("perspective"), pt = rQ.core.reverting, gt = 1);
}, sA = function(Q) {
  var B = Q.ownerSVGElement, t = ht("svg", B && B.getAttribute("xmlns") || "http://www.w3.org/2000/svg"), A = Q.cloneNode(!0), e;
  A.style.display = "block", t.appendChild(A), QB.appendChild(t);
  try {
    e = A.getBBox();
  } catch {
  }
  return t.removeChild(A), QB.removeChild(t), e;
}, vA = function(Q, B) {
  for (var t = B.length; t--; )
  ...
  return B && (B.width || B.height) || t || (B = sA(Q)), B && !B.width && !B.x && !B.y ? {
    x: +vA(Q, ["x", "cx", "x1"]) || 0,
    y: +vA(Q, ["y", "cy", "y1"]) || 0,
    width: 0,
    height: 0
  ...
  var i = new PQ(Q._pt, B, t, 0, 1, P ? se : re);
  return Q._pt = i, i.b = A, i.e = e, Q._props.push(t), i;
}, cA = {
  deg: 1,
  rad: 1,
  turn: 1
  ...
  var e = parseFloat(t) || 0, P = (t + "").trim().substr((e + "").length) || "px", i = SQ.style, s = Bi.test(B), v = Q.tagName.toLowerCase() === "svg", c = (v ? "client" : "offset") + (s ? "Width" : "Height"), n = 100, T = A === "px", z = A === "%", w, o, a, E;
  if (A === P || !e || cA[A] || cA[P])
    return e;
  if (P !== "px" && !T && (e = r(Q, B, t, "px")), E = Q.getCTM && Te(Q), (z || P === "%") && (OQ[B] || ~B.indexOf("adius")))
    return w = E ? Q.getBBox()[s ? "width" : "height"] : Q[c], X(z ? e / w * n : e / 100 * w);
  if (i[s ? "width" : "height"] = n + (T ? P : A), o = A !== "rem" && ~B.indexOf("adius") || A === "em" && Q.appendChild && !v ? Q : Q.parentNode, E && (o = (Q.ownerSVGElement || {}).parentNode), (!o || o === jQ || !o.appendChild) && (o = jQ.body), a = o._gsap, a && z && a.width && s && a.time === nQ.time && !a.uncache)
    return X(e / a.width * n);
  if (z && (B === "height" || B === "width")) {
    var l = Q.style[B];
    Q.style[B] = n + A, w = Q[c], l ? Q.style[B] = l : gQ(Q, B);
  } else
    (z || P === "%") && !wi[TQ(o, "display")] && (i.position = TQ(Q, "position")), o === Q && (i.position = "static"), o.appendChild(SQ), w = SQ[c], o.removeChild(SQ), i.position = "absolute";
  return s && z && (a = YQ(o), a.time = nQ.time, a.width = o[c]), X(T ? w * e / n : w && e ? n / w * e : 0);
}, CQ = function(Q, B, t, A) {
  var e;
  return gt || lt(), B in UQ && B !== "transform" && (B = UQ[B], ~B.indexOf(",") && (B = B.split(",")[0])), OQ[B] && B !== "transform" ? (e = DB(Q, A), e = B !== "transformOrigin" ? e[B] : e.svg ? e.origin : xB(TQ(Q, iQ)) + " " + e.zOrigin + "px") : (e = Q.style[B], (!e || e === "auto" || A || ~(e + "").indexOf("calc(")) && (e = pB[B] && pB[B](Q, B, t) || TQ(Q, B) || jA(Q, B) || (B === "opacity" ? 1 : 0))), t && !~(e + "").trim().indexOf(" ") ? pQ(Q, B, e, t) + t : e;
}, ai = function(Q, B, t, A) {
  if (!t || t === "none") {
    var e = iB(B, Q, 1), P = e && TQ(Q, e, 1);
    P && P !== t ? (B = e, t = P) : B === "borderColor" && (t = TQ(Q, "borderTopColor"));
  }
  var i = new PQ(this._pt, Q.style, B, 0, 1, ee), s = 0, v = 0, c, n, T, z, w, o, a, E, l, u, U, k;
  if (i.b = t, i.e = A, t += "", A += "", A.substring(0, 6) === "var(--" && (A = TQ(Q, A.substring(4, A.indexOf(")")))), A === "auto" && (o = Q.style[B], Q.style[B] = A, A = TQ(Q, B) || A, o ? Q.style[B] = o : gQ(Q, B)), c = [t, A], WA(c), t = c[0], A = c[1], T = t.match(qQ) || [], k = A.match(qQ) || [], k.length) {
    for (; n = qQ.exec(A); )
      a = n[0], l = A.substring(s, n.index), w ? w = (w + 1) % 5 : (l.substr(-5) === "rgba(" || l.substr(-5) === "hsla(") && (w = 1), a !== (o = T[v++] || "") && (z = parseFloat(o) || 0, U = o.substr((z + "").length), a.charAt(1) === "=" && (a = $Q(z, a) + U), E = parseFloat(a), u = a.substr((E + "").length), s = qQ.lastIndex - u.length, u || (u = u || zQ.units[B] || U, s === A.length && (A += u, i.e += u)), U !== u && (z = pQ(Q, B, o, u) || 0), i._pt = {
        _next: i._pt,
        p: l || v === 1 ? l : ",",
        //note: SVG spec allows omission of comma/space when a negative sign is wedged between two numbers, like 2.5-5.3 instead of 2.5,-5.3 but when tweening, the negative value may switch to positive, so we insert the comma just in case.
        s: z,
  ...
  return (t === "top" || t === "bottom" || A === "left" || A === "right") && (Q = t, t = A, A = Q), B[0] = nA[t] || t, B[1] = nA[A] || A, B.join(" ");
}, hi = function(Q, B) {
  if (B.tween && B.tween._time === B.tween._dur) {
    var t = B.t, A = t.style, e = B.u, P = t._gsap, i, s, v;
    if (e === "all" || e === !0)
      A.cssText = "", s = 1;
    else
      for (e = e.split(","), v = e.length; --v > -1; )
        i = e[v], OQ[i] && (s = 1, i = i === "transformOrigin" ? iQ : V), gQ(t, i);
    s && (gQ(t, V), P && (P.svg && t.removeAttribute("transform"), A.scale = A.rotate = A.translate = "none", DB(t, 1), P.uncache = 1, ve(A)));
  }
}, pB = {
  clearProps: function(Q, B, t, A, e) {
    if (e.data !== "isFromStart") {
      var P = Q._pt = new PQ(Q._pt, B, t, 0, 0, hi);
      return P.u = A, P.pr = -10, P.tween = e, Q._props.push(t), 1;
    }
  }
  ...
  */
}, UB = [1, 0, 0, 1, 0, 0], ze = {}, we = function(Q) {
  return Q === "matrix(1, 0, 0, 1, 0, 0)" || Q === "none" || !Q;
}, oA = function(Q) {
  var B = TQ(Q, V);
  return we(B) ? UB : B.substr(7).match(CA).map(X);
}, mt = function(Q, B) {
  var t = Q._gsap || YQ(Q), A = Q.style, e = oA(Q), P, i, s, v;
  return t.svg && Q.getAttribute("transform") ? (s = Q.transform.baseVal.consolidate().matrix, e = [s.a, s.b, s.c, s.d, s.e, s.f], e.join(",") === "1,0,0,1,0,0" ? UB : e) : (e === UB && !Q.offsetParent && Q !== QB && !t.svg && (s = A.display, A.display = "block", P = Q.parentNode, (!P || !Q.offsetParent && !Q.getBoundingClientRect().width) && (v = 1, i = Q.nextElementSibling, QB.appendChild(Q)), e = oA(Q), s ? A.display = s : gQ(Q, "display"), v && (i ? P.insertBefore(Q, i) : P ? P.appendChild(Q) : QB.removeChild(Q))), B && e.length > 6 ? [e[0], e[1], e[4], e[5], e[12], e[13]] : e);
  ...
  t ? s !== UB && (D = z * a - w * o) && (N = U * (a / D) + k * (-o / D) + (o * l - a * E) / D, d = U * (-w / D) + k * (z / D) - (z * l - w * E) / D, U = N, k = d) : (O = oe(Q), U = O.x + (~u[0].indexOf("%") ? U / 100 * O.width : U), k = O.y + (~(u[1] || u[0]).indexOf("%") ? k / 100 * O.height : k)), A || A !== !1 && i.smooth ? (E = U - v, l = k - c, i.xOffset = n + (E * z + l * o) - E, i.yOffset = T + (E * w + l * a) - l) : i.xOffset = i.yOffset = 0, i.xOrigin = U, i.yOrigin = k, i.smooth = !!A, i.origin = B, i.originIsAbsolute = !!t, Q.style[iQ] = "0px 0px", P && (FQ(P, i, "xOrigin", v, U), FQ(P, i, "yOrigin", c, k), FQ(P, i, "xOffset", n, i.xOffset), FQ(P, i, "yOffset", T, i.yOffset)), Q.setAttribute("data-svg-origin", U + " " + k);
}, DB = function(Q, B) {
  var t = Q._gsap || new _A(Q);
  ...
  B.z = "0px", B.rotationY = B.rotationX = "0deg", B.force3D = 0, ae(Q, B);
}, mQ = "0deg", sB = "0px", yQ = ") ", ae = function(Q, B) {
...
}, ui = function(Q, B, t, A, e) {
  var P = 360, i = Z(e), s = parseFloat(e) * (i && ~e.indexOf("rad") ? IQ : 1), v = s - A, c = A + v + "deg", n, T;
  return i && (n = e.split("_")[1], n === "short" && (v %= P, v !== v % (P / 2) && (v += v < 0 ? P : -P)), n === "cw" && v < 0 ? v = (v + P * iA) % P - ~~(v / P) * P : n === "ccw" && v > 0 && (v = (v - P * iA) % P - ~~(v / P) * P)), Q._pt = T = new PQ(Q._pt, B, t, A, v, Ai), T.e = c, T.u = "deg", Q._props.push(t), T;
}, TA = function(Q, B) {
  for (var t in B)
    Q[t] = B[t];
  return Q;
}, Ui = function(Q, B, t) {
  var A = TA({}, t._gsap), e = "perspective,force3D,transformOrigin,svgOrigin", P = t.style, i, s, v, c, n, T, z, w;
  A.svg ? (v = t.getAttribute("transform"), t.setAttribute("transform", ""), P[V] = B, i = DB(t, 1), gQ(t, V), t.setAttribute("transform", v)) : (v = getComputedStyle(t)[V], P[V] = B, i = DB(t, 1), P[V] = v);
  for (s in OQ)
    v = A[s], c = i[s], v !== c && e.indexOf(s) < 0 && (z = _(v), w = _(c), n = z !== w ? pQ(t, s, v, w) : parseFloat(v), T = parseFloat(c), Q._pt = new PQ(Q._pt, i, s, n, T - n, Et), Q._pt.u = w || 0, Q._props.push(s));
  TA(i, A);
};
eQ("padding,margin,Width,Radius", function(r, Q) {
  var B = "Top", t = "Right", A = "Bottom", e = "Left", P = (Q < 3 ? [B, t, A, e] : [B + e, B + t, A + t, A + e]).map(function(i) {
    return Q < 2 ? r + i : "border" + i + r;
  });
  pB[Q > 1 ? "border" + r : r] = function(i, s, v, c, n) {
    var T, z;
    if (arguments.length < 4)
      return T = P.map(function(w) {
        return CQ(i, w, v);
      }), z = T.join(" "), z.split(T[0]).length === 5 ? T[0] : z;
    T = (c + "").split(" "), z = {}, P.forEach(function(w, o) {
...
var Ee = {
  ...
  init: function(Q, B, t, A, e) {
    var P = this._props, i = Q.style, s = t.vars.startAt, v, c, n, T, z, w, o, a, E, l, u, U, k, O, D, N, d;
    gt || lt(), this.styles = this.styles || ce(Q), N = this.styles.props, this.tween = t;
    for (o in B)
      if (o !== "autoRound" && (c = B[o], !(cQ[o] && $A(o, B, t, A, Q, e)))) {
        if (z = typeof c, w = pB[o], z === "function" && (c = c.call(t, A, Q, e), z = typeof c), z === "string" && ~c.indexOf("random(") && (c = lB(c)), w)
          w(this, Q, o, c, t) && (D = 1);
        else if (o.substr(0, 2) === "--")
          v = (getComputedStyle(Q).getPropertyValue(o) + "").trim(), c += "", GQ.lastIndex = 0, GQ.test(v) || (a = _(v), E = _(c), E ? a !== E && (v = pQ(Q, o, v, E) + E) : a && (c += a)), this.add(i, "setProperty", v, c, A, e, 0, 0, o), P.push(o), N.push(o, 0, i[o]);
        else if (z !== "undefined") {
          if (s && o in s ? (v = typeof s[o] == "function" ? s[o].call(t, A, Q, e) : s[o], Z(v) && ~v.indexOf("random(") && (v = lB(v)), _(v + "") || v === "auto" || (v += zQ.units[o] || _(CQ(Q, o)) || ""), (v + "").charAt(1) === "=" && (v = CQ(Q, o))) : v = CQ(Q, o), T = parseFloat(v), l = z === "string" && c.charAt(1) === "=" && c.substr(0, 2), l && (c = c.substr(2)), n = parseFloat(c), o in UQ && (o === "autoAlpha" && (T === 1 && CQ(Q, "visibility") === "hidden" && n && (T = 0), N.push("visibility", 0, i.visibility), FQ(this, i, "visibility", T ? "inherit" : "hidden", n ? "inherit" : "hidden", !n)), o !== "scale" && o !== "transform" && (o = UQ[o], ~o.indexOf(",") && (o = o.split(",")[0]))), u = o in OQ, u) {
            ...
            if (U || (k = Q._gsap, k.renderTransform && !B.parseTransform || DB(Q, B.parseTransform), O = B.smoothOrigin !== !1 && k.smooth, U = this._pt = new PQ(this._pt, i, V, 0, 1, k.renderTransform, k, 0, -1), U.dep = 1), o === "scale")
              ...
              N.push(iQ, 0, i[iQ]), c = Ei(c), k.svg ? kt(Q, c, 0, O, 0, this) : (E = parseFloat(c.split(" ")[2]) || 0, E !== k.zOrigin && FQ(this, k, "zOrigin", k.zOrigin, E), FQ(this, i, o, xB(v), xB(c)));
              continue;
            } else if (o === "svgOrigin") {
              kt(Q, c, 1, O, 0, this);
              continue;
            } else if (o in ze) {
              ui(this, k, o, T, l ? $Q(T, l + c) : c);
              continue;
          ...
          if (u || (n || n === 0) && (T || T === 0) && !ti.test(c) && o in i)
            a = (v + "").substr((T + "").length), n || (n = 0), E = _(c) || (o in zQ.units ? zQ.units[o] : a), a !== E && (T = pQ(Q, o, v, E)), this._pt = new PQ(this._pt, u ? k : i, o, T, (l ? $Q(T, l + n) : n) - T, !u && (E === "px" || o === "zIndex") && B.autoRound !== !1 ? ii : Et), this._pt.u = E || 0, u && d !== c ? (this._pt.b = v, this._pt.e = d, this._pt.r = Pi) : a !== E && E !== "%" && (this._pt.b = v, this._pt.r = ei);
          else if (o in i)
            ai.call(this, Q, o, v, l ? l + c : c);
          else if (o in Q)
  ...
  getSetter: function(Q, B, t) {
    var A = UQ[B];
    return A && A.indexOf(",") < 0 && (B = A), B in OQ && B !== iQ && (Q._gsap.x || CQ(Q, "x")) ? t && PA === t ? B === "scale" ? ci : vi : (PA = t || {}) && (B === "scale" ? ni : oi) : Q.style && !Dt(Q.style[B]) ? ri : ~B.indexOf("-") ? si : Gt(Q, B);
  },
  core: {
    _removeProperty: gQ,
    _getMatrix: mt
...
(function(r, Q, B, t) {
  var A = eQ(r + "," + Q + "," + B, function(e) {
    OQ[e] = 1;
  });
  eQ(Q, function(e) {
    zQ.units[e] = "deg", ze[e] = 1;
  }), UQ[A[13]] = r + "," + Q, eQ(t, function(e) {
    var P = e.split(":");
    UQ[P[1]] = A[P[0]];
  });
...
function Mi(r, Q) {
  ...
  const B = r.boundingBox, t = Math.max(1e-6, B.max.z - B.min.z), A = r.getAttribute("position"), e = new Float32Array(A.count * 2);
  for (let P = 0; P < A.count; P++) {
    const i = A.getX(P), s = A.getY(P), v = A.getZ(P);
    e[P * 2] = Math.atan2(s, i) / (2 * Math.PI) + 0.5, e[P * 2 + 1] = (v - B.min.z) / t * Q;
  }
  r.setAttribute("uv", new h.BufferAttribute(e, 2));
}
function he(r) {
  const Q = atob(Di), B = new Uint8Array(Q.length);
  for (let v = 0; v < Q.length; v++) B[v] = Q.charCodeAt(v);
  const A = new Ce().parse(B.buffer);
...
function Hi() {
  ...
  const e = t.getImageData(0, 0, r, Q);
  ...
  const P = new h.CanvasTexture(B);
  return P.wrapS = h.RepeatWrapping, P.wrapT = h.RepeatWrapping, $B = P, P;
}
function ji(r, Q) {
  const B = Math.max(1e-3, r), t = B * 1.6, A = B * 0.08, e = B * 0.187, P = CB(Q), i = new h.CylinderGeometry(0, A, t, 8, 1, !1);
  i.rotateX(-Math.PI / 2), i.translate(0, 0, t / 2);
  const s = new h.CylinderGeometry(0, e, t, 8, 1, !0);
  s.rotateX(-Math.PI / 2), s.translate(0, 0, t / 2);
  const v = new h.MeshBasicMaterial({
    color: new h.Color(wA),
    transparent: !0,
    opacity: P,
    depthTest: !1,
    depthWrite: !1,
...
function Fi(r) {
  ...
  return r.bit.type === "triangle" ? Oi(Q) : r.bit.type === "drill" ? he(Q) : Li(Q);
}
function yt(r) {
  return r.bit.colorSource === "custom" ? r.bit.color : r.bit.type === "drill" ? Ci : di;
}
function fi(r) {
  const Q = CB(r.bit.opacity), B = he(Math.max(1e-3, r.bit.size)), t = new h.Color(yt(r)), A = Hi(), e = new h.MeshStandardMaterial({
    color: t,
    metalness: 0.8,
    roughness: 0.35,
    roughnessMap: A,
...
function Gi(r) {
  const Q = CB(r.bit.opacity), B = Fi(r), t = new h.MeshBasicMaterial({
    color: new h.Color(yt(r)),
    transparent: Q < 1,
    opacity: Q,
    side: h.DoubleSide,
    depthTest: !1,
...
function aA(r) {
  return r.bit.type === "laser" ? ji(r.bit.size, r.bit.opacity) : r.bit.type === "drill" ? fi(r) : Gi(r);
}
function EA(r) {
...
function bi(r) {
  ...
  const v = (n) => {
    ...
    if (a || E)
      Q.remove(t), EA(t), t = aA(n), Q.add(t), i == null || i.kill(), P == null || P.kill(), P = s(n.bit.spinRpm), e && P.timeScale(1).play();
    else if (T === "laser") {
      const U = t, [k, O] = U.children, D = k.material, N = O.material;
      D.opacity = l, D.needsUpdate = !0, N.opacity = l * 0.3, N.needsUpdate = !0;
    } else {
      const k = t.material, O = new h.Color(yt(n));
  ...
  return {
    ...
    update: (n) => {
      if (!A) return;
      const T = (n - A.startedAt) / A.duration, z = Ni(T);
      Q.position.set(
    ...
    dispose: () => {
      i == null || i.kill(), P == null || P.kill(), Q.remove(t), EA(t);
    }
  };
}
...
class Xi {
  constructor(Q) {
    this.viewCube = null, this.resizeObserver = null, this.onWindowResize = null, this.animationFrameId = null, this.gridGroup = null, this.axesGroup = null, this.gridLabelsGroup = null, this.boundingBoxGroup = null, this.machineBedGroup = null, this.bitMarker = null, this.preLaserBitType = "drill", this.toolpathStreams = [], this.toolpathCutBucketCount = 1, this.toolpathRotationA = 0, this.lastBitPosition = { x: 0, y: 0, z: 0, a: 0 }, this.sim3dHandle = null, this.currentLines = [], this.linePositions = null, this.renderSequence = 0, this.currentBounds = null, this.cameraFocusTransition = null, this.cameraFollowRequested = !1, this.cameraFollowInterrupted = !1, this.cameraFollowOffset = null, this.id = Q.id, this.container = Q.container, this.callbacks = Q.callbacks ?? {}, this.options = lA(fe, Q.options), this.canvas = document.createElement("canvas"), this.canvas.style.width = "100%", this.canvas.style.height = "100%", this.canvas.style.display = "block", this.container.appendChild(this.canvas), We(this.container), this.renderer = new h.WebGLRenderer({
      canvas: this.canvas,
    ...
    ), this.controls = new ke(this.camera, this.renderer.domElement), this.controls.enableDamping = this.options.camera.orbit.enableDamping, this.controls.addEventListener("start", () => {
      this.cameraFollowRequested && (this.cameraFollowInterrupted = !0);
    }), this.controls.update(), this.viewCubeCorrection = new h.Matrix4().makeRotationX(h.MathUtils.degToRad(90)), this.viewCube = new Ie({
  ...
  hideUntilLine(Q, B) {
    ...
    for (const e of this.toolpathStreams) {
      ...
      t === "grey" ? (e.line.geometry.setDrawRange(0, e.totalVertices), He({ stream: e, nextCursorVertex: P, options: this.options })) : e.line.geometry.setDrawRange(P, Math.max(0, e.totalVertices - P));
    }
  }
  seekToLine(Q, B) {
  ...
  snapCameraToView(Q, B = {}) {
    const t = Math.max(0, Math.floor(B.durationMs ?? 240)), A = this.controls.target.clone(), e = this.camera.position.distanceTo(A), P = Math.max(1e-6, B.distance ?? e);
    this.startSnapToView(Q, A, P, t);
  }
  /**
  ...
  worldToScreen(Q, B, t = 0) {
    ...
    if (A.width === 0 || A.height === 0)
      return null;
    const e = new h.Vector3(Q, B, t).project(this.camera);
    return {
  ...
  startSnapToView(Q, B, t, A) {
    const e = Kt(Q), P = B.clone().add(e.multiplyScalar(t)), i = Math.max(1, t);
    this.camera.near = i / 1e3, this.camera.far = i * 50, this.camera.updateProjectionMatrix(), this.startCameraLerp(B, P, A);
  }
  ...
  setOptions(Q) {
    var T, z, w, o, a, E, l, u, U, k, O, D, N, d, M, b, G, C, j, F, H, f, I, R, QQ, sQ, W, m, BQ, vQ, rB, It, St, Yt;
    const B = this.options;
    this.options = lA(this.options, Q), (B.render.antialias !== this.options.render.antialias || B.render.theme.background !== this.options.render.theme.background) && this.renderer.setClearColor(this.options.render.theme.background, 1), (B.render.theme.opacity !== this.options.render.theme.opacity || B.render.theme.rapidOpacity !== this.options.render.theme.rapidOpacity) && this.refreshToolpathOpacities(), B.camera.fov !== this.options.camera.fov && (this.camera.fov = this.options.camera.fov, this.camera.updateProjectionMatrix()), B.camera.orbit.enableDamping !== this.options.camera.orbit.enableDamping && (this.controls.enableDamping = this.options.camera.orbit.enableDamping);
    ...
    if (B.sim3d.erosionPasses !== this.options.sim3d.erosionPasses && this.sim3dHandle) {
      const le = this.sim3dHandle.currentLine, Vt = SB(le, this.sim3dHandle.data);
      YB(Vt, this.sim3dHandle.data.resolution, this.options.sim3d.erosionPasses, this.sim3dHandle.data.slabBounds.zTop), VB(this.sim3dHandle.slab, Vt);
    }
  ...
  ensureBitMarker() {
    this.bitMarker || (this.bitMarker = bi(this.options), this.scene.add(this.bitMarker.object));
  }
  worldSizes() {
    const Q = this.options.units === "in" ? gi : 1;
    return {
  ...
  refreshGridLabels() {
    ...
    this.setGridLabelsGroup(
      AP({ sizeXMm: Q, sizeYMm: B, bounds: t, units: this.options.units, theme: this.options.render.theme })
    );
  }
  refreshBoundingBox() {
    ...
    this.setBoundingBoxGroup(Ve(this.currentBounds, this.options));
  }
  refreshMachineBed() {
    ...
    this.setMachineBedGroup(Ke(B, t, this.options, A));
  }
  setBitMarker(Q) {
  ...
  setGridGroup(Q) {
    this.gridGroup && (this.scene.remove(this.gridGroup), QP(this.gridGroup), this.gridGroup = null), Q && (this.gridGroup = Q, this.scene.add(Q));
  }
  setAxesGroup(Q) {
    this.axesGroup && (this.scene.remove(this.axesGroup), tP(this.axesGroup), this.axesGroup = null), Q && (this.axesGroup = Q, this.scene.add(Q));
  }
  ...
  setBoundingBoxGroup(Q) {
    this.boundingBoxGroup && (this.scene.remove(this.boundingBoxGroup), Re(this.boundingBoxGroup), this.boundingBoxGroup = null), Q && (this.boundingBoxGroup = Q, this.scene.add(Q));
  }
  setMachineBedGroup(Q) {
    this.machineBedGroup && (this.scene.remove(this.machineBedGroup), Ze(this.machineBedGroup), this.machineBedGroup = null), Q && (this.machineBedGroup = Q, this.scene.add(Q));
  }
  setGeometryEmpty() {
    this.setSim3dHandle(null), Oe(this.toolpathRoot, this.toolpathStreams), this.toolpathStreams = [], this.toolpathCutBucketCount = 1, this.linePositions = null, this.currentBounds = null, this.emitBoundsChanged(), this.refreshBoundingBox();
  }
  ...
  refreshToolpathColors() {
    Le(this.toolpathStreams, this.options);
  }
  ...
  buildLinePositions() {
    ...
    for (let i = 0; i < Q; i++) {
      ...
      s && P.processLine(s), this.linePositions[i * 4] = B, this.linePositions[i * 4 + 1] = t, this.linePositions[i * 4 + 2] = A, this.linePositions[i * 4 + 3] = e;
    }
  }
  async renderScene() {
    ...
    try {
      ...
      const B = await Ue(this.currentLines, {
  ...
  async buildAndApplySim3d(Q) {
    ...
    let A;
    try {
      A = await Ge(this.currentLines, B, t, {
        arcSegments: this.options.geometry.arcSegments,
        batch: {
          shouldAbort: () => Q !== this.renderSequence,
          onProgress: (i, s) => {
  ...
  startCameraFocus(Q) {
    ...
    Q.getSize(t);
    const A = Math.max(t.x, t.y, 1) / 2, e = h.MathUtils.degToRad(this.camera.fov), P = A / Math.tan(e / 2) * 1.25 + t.z, i = Kt("front-top-left"), s = B.clone().add(i.multiplyScalar(P)), v = Math.max(t.x, t.y, t.z, 1);
  ...
  updateCameraFocusTransition() {
    if (!this.cameraFocusTransition)
      return;
    const B = performance.now() - this.cameraFocusTransition.startedAt, t = Math.min(1, Math.max(0, B / this.cameraFocusTransition.duration)), A = this.cameraFocusTransition.easing === "linear" ? t : qe(t);
  ...
  updateViewCubeRotation() {
    ...
    const t = _e(this.camera.position, this.controls.target);
...
class Ki {
  constructor(Q, B) {
    this.crosshairPos = null, this.crosshairVisible = !1, this.pathEls = [], this.segmentGroups = [], this.workerMode = !1, this.viewBox = { x: 0, y: 0, w: 100, h: 100 }, this.rotX = vB, this.rotY = cB, this.centerX = 0, this.centerY = 0, this.centerZ = 0, this.focalLength = 500, this.cosRotX = 1, this.sinRotX = 0, this.cosRotY = 1, this.sinRotY = 0, this.dragMode = "none", this.activePointers = /* @__PURE__ */ new Map(), this.pinchLastDist = 0, this.pinchLastMid = { x: 0, y: 0 }, this.rafPending = !1, this.overlayRafPending = !1, this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.bounds = { minX: 0, minY: 0, maxX: 0, maxY: 0, minZ: 0, maxZ: 0, empty: !0 }, this.hasZInfo = !0, this.project = (t, A, e) => {
      ...
      t.preventDefault();
      const A = t.deltaY > 0 ? 1.1 : 1 / 1.1, e = this.svgPoint(t.clientX, t.clientY), { x: P, y: i, w: s, h: v } = this.viewBox;
      this.viewBox = {
        x: e.x - (e.x - P) * A,
        y: e.y - (e.y - i) * A,
        w: s * A,
      ...
      const e = t.clientX - A.x, P = t.clientY - A.y;
  ...
  rebuildToolpaths(Q = !1) {
    const B = Q ? Ii : L, t = String(this.options.strokeWidth), A = this.simplifyTolerance();
    ...
    for (let e = 0; e < this.segmentGroups.length; e++) {
      ...
      c.setAttribute("stroke", P), c.setAttribute("stroke-opacity", String(i)), c.setAttribute("stroke-width", t), c.setAttribute("stroke-linecap", "round"), c.setAttribute("stroke-linejoin", "round"), c.setAttribute(
        "d",
        v === 4 ? Si(s, B, A) : Yi(s, this.project, B, A)
      );
    }
  }
  ...
  renderBbox() {
    ...
    const { minX: Q, maxX: B, minY: t, maxY: A, minZ: e, maxZ: P } = this.bounds, i = this.project;
    if (this.options.projectionMode === "top") {
    ...
    const s = i(Q, t, e), v = i(B, t, e), c = i(B, A, e), n = i(Q, A, e), T = i(Q, t, P), z = i(B, t, P), w = i(B, A, P), o = i(Q, A, P), a = [
    ...
    ZQ(this.bboxLabelX, LQ(s, v), tt(LQ(s, v), LQ(c, n), k), U, `X: ${WQ(B - Q)}`);
    const O = [s, v, c, n, T, z, w, o], D = Math.min(...O.map((C) => C.y)), N = Math.max(...O.map((C) => C.y)), d = Math.min(...O.map((C) => C.x)), M = Math.max(...O.map((C) => C.x)), b = { x: M + k, y: (D + N) / 2 };
    ZQ(this.bboxLabelY, b, b, U, `Y: ${WQ(A - t)}`);
  ...
  renderCrosshairMarker() {
    ...
    const { x: Q, y: B, z: t } = this.crosshairPos, A = this.project(Q, B, t), e = Math.min(this.viewBox.w, this.viewBox.h) * 0.025, P = e * 0.25, i = A.x, s = A.y, v = [
  ...
  svgPoint(Q, B) {
    const t = this.svg.getBoundingClientRect(), { x: A, y: e, w: P, h: i } = this.viewBox;
    return {
      x: A + (Q - t.left) / t.width * P,
      y: e + (B - t.top) / t.height * i
    };
  }
}
function Qt() {
...
function tt(r, Q, B) {
  const t = r.x - Q.x, A = r.y - Q.y, e = Math.hypot(t, A) || 1;
  return { x: r.x + t / e * B, y: r.y + A / e * B };
}
function L(r) {
  return r.toFixed(2);
}
...
function Si(r, Q, B = 0) {
  ...
  const t = [], A = 1e-6, e = B * B;
  let P = NaN, i = NaN, s = NaN, v = NaN, c = NaN, n = NaN, T = !1;
  for (let z = 0; z + 3 < r.length; z += 4) {
    const w = r[z], o = -r[z + 1], a = r[z + 2], E = -r[z + 3];
    if (Math.abs(w - P) < A && Math.abs(o - i) < A) {
      const l = a - s, u = E - v;
      e > 0 && l * l + u * u < e ? (c = a, n = E, T = !0) : (t.push(`L${Q(a)} ${Q(E)}`), s = a, v = E, T = !1);
    } else
...
function Yi(r, Q, B, t = 0) {
  ...
  for (let w = 0; w + 5 < r.length; w += 6) {
    ...
    if (Math.abs(o.x - i) < e && Math.abs(o.y - s) < e) {
      ...
      P > 0 && E * E + l * l < P ? (n = a.x, T = a.y, z = !0) : (A.push(`L${B(a.x)} ${B(a.y)}`), v = a.x, c = a.y, z = !1);
    } else
...
function At(r) {
  let Q = 1 / 0, B = 1 / 0, t = 1 / 0, A = -1 / 0, e = -1 / 0, P = -1 / 0, i = !0, s = !1;
  for (const { verts: v, stride: c } of r)
    if (c === 4)
      for (let n = 0; n + 3 < v.length; n += 4) {
        const T = v[n], z = v[n + 1], w = v[n + 2], o = v[n + 3];
        T < Q && (Q = T), z < B && (B = z), T > A && (A = T), z > e && (e = z), w < Q && (Q = w), o < B && (B = o), w > A && (A = w), o > e && (e = o), i = !1;
      }
    else
      for (let n = 0; n + 5 < v.length; n += 6) {
        const T = v[n], z = v[n + 1], w = v[n + 2], o = v[n + 3], a = v[n + 4], E = v[n + 5];
        T < Q && (Q = T), z < B && (B = z), w < t && (t = w), T > A && (A = T), z > e && (e = z), w > P && (P = w), o < Q && (Q = o), a < B && (B = a), E < t && (t = E), o > A && (A = o), a > e && (e = a), E > P && (P = E), i = !1, s = !0;
      }
  return !i && !s && (t = 0, P = 0), { minX: Q, minY: B, maxX: A, maxY: e, minZ: t, maxZ: P, empty: i };
}
export {
  Ki as G,
  Ie as V,
