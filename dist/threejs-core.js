...
exports.buildVerticesFromLines = buildVerticesFromLines;
exports.buildMovementVerticesFromLines = buildMovementVerticesFromLines;
exports.buildMovementVerticesFromLinesBatched = buildMovementVerticesFromLinesBatched;
exports.buildMovementGeometryFromLinesBatched = buildMovementGeometryFromLinesBatched;
exports.buildLaserVerticesFromLines = buildLaserVerticesFromLines;
exports.buildLaserVerticesFromLinesBatched = buildLaserVerticesFromLinesBatched;
exports.buildToolpathGeometryFromLinesBatched = buildToolpathGeometryFromLinesBatched;
exports.buildLaserGeometryFromLinesBatched = buildLaserGeometryFromLinesBatched;
exports.pushXYZ = pushXYZ;
const parser_1 = require("./parser");
const virtualizer_1 = require("./virtualizer");
function buildVerticesFromLines(lines, options = {}) {
    const vertices = [];
...
async function buildMovementVerticesFromLinesBatched(lines, options = {}) {
    const rapidVertices = [];
    const cuttingVertices = [];
    const arcSegments = options.arcSegments ?? 30;
...
async function buildMovementGeometryFromLinesBatched(lines, options = {}) {
    const vertices = [];
    const arcSegments = options.arcSegments ?? 30;
    const batch = options.batch;
    const everyLines = Math.max(1, Math.floor(batch?.everyLines ?? 5000));
    ...
    let activeLineKind = 0;
    const virtualizer = new virtualizer_1.GCodeVirtualizer({
        onLinearMove: (args) => {
            const kind = args.modals.motion === "G0" ? 1 : 2;
...
function buildLaserVerticesFromLines(lines, options = {}) {
    const arcSegments = options.arcSegments ?? 30;
    ...
    const rapidVertices = [];
    const bucketVertices = Array.from({ length: bucketCount }, () => []);
    const virtualizer = new virtualizer_1.GCodeVirtualizer({
        onLinearMove: (args) => {
            const start = args.transformedStart ?? args.start;
            const end = args.transformedEnd ?? args.end;
    ...
    const buckets = bucketVertices.map((vertices, index) => ({
...
async function buildLaserVerticesFromLinesBatched(lines, options = {}) {
...
async function buildToolpathGeometryFromLinesBatched(lines, options = {}) {
    ...
    if (laserMode) {
        const parser = new parser_1.GCodeParser();
        for (const line of lines) {
            if (!line) {
                continue;
            }
    ...
    const isLaserOn = (modals) => {
        ...
        const spindleEnabled = modals.spindle === "M3" ||
            modals.spindle === "M4" ||
    ...
    if (laserMode) {
        const rangeVirtualizer = new virtualizer_1.GCodeVirtualizer({
            onLinearMove: (args) => {
    ...
    const rapidVertices = [];
    const cutVertices = laserMode
        ? Array.from({ length: bucketCount }, () => [])
        : [new Array()];
    const rapidPrefixEndVertex = new Int32Array(lines.length);
    const cutPrefixEndVertex = Array.from({ length: laserMode ? bucketCount : 1 }, () => new Int32Array(lines.length));
    const virtualizer = new virtualizer_1.GCodeVirtualizer({
        onLinearMove: (args) => {
            const start = args.transformedStart ?? args.start;
            const end = args.transformedEnd ?? args.end;
...
async function buildLaserGeometryFromLinesBatched(lines, options = {}) {
    const bucketCount = Math.max(1, Math.floor(options.bucketCount ?? 16));
    const baseOpacity = options.baseOpacity ?? 0.9;
    ...
    const buckets = result.cuts.map((stream, index) => ({
        opacity: bucketOpacity(index, bucketCount, baseOpacity),
...
function pushXYZ(vertices, position) {
...
function bucketIndex(spindleSpeed, minPower, maxPower, bucketCount) {
    if (bucketCount <= 1) {
        return 0;
    }
...
function bucketOpacity(index, bucketCount, baseOpacity) {
    if (bucketCount <= 1) {
        return baseOpacity;
    }
