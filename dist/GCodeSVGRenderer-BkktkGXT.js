...
import { GCodeVirtualizer as iB, buildWorkerToolpathStreams as Le, buildToolpathGeometryFromLinesBatched as Me, buildMovementVerticesFromLines as He, buildWorkerSegmentGroups as je } from "./gviewer.js";
...
function fe(i) {
  ...
  for (const B of i.specs) {
    if (B.positions.length === 0 || B.opacity <= 0)
      continue;
    const A = B.positions.length / 3, e = B.colors ? B.colors.slice() : CA(B.kind, A, i.options), P = new Float32Array(e.length);
    P.set(e);
...
function be(i, Q) {
  for (const t of i) {
    ...
    const B = CA(t.kind, t.totalVertices, Q);
...
function ge(i) {
  ...
  if (t === B)
    return;
  const A = i.stream.line.geometry.getAttribute("color"), e = me(i.options), P = Math.min(B, t), r = Math.max(B, t);
  if (t > B)
    for (let s = B; s < t; s += 1) {
      const v = s * 3;
...
function Ve(i, Q) {
  ...
  const B = i.xMax - i.xMin, A = i.yMax - i.yMin, e = i.zTop - i.zBot, P = (i.xMin + i.xMax) / 2, r = (i.yMin + i.yMax) / 2, s = (i.zBot + i.zTop) / 2, v = Ye();
...
function tP(i, Q, t, B) {
  const A = new l.Group();
  if (A.add(ZB(i, Q, t.render.theme.colors.machineBed)), B) {
    const e = t.render.theme.colors.machineBedKeepout;
    A.add(ZB(B.min, B.max, e)), A.add(QP(B.min, B.max, e));
  }
  return A;
}
function BP(i) {
...
class AP extends RB {
  _rotateUp(Q) {
    RB.prototype._rotateUp.call(this, -Q);
  }
}
function eP(i) {
...
function qB(i, Q, t = 1.25) {
  const B = Math.max(i.x, i.y, 1) / 2;
  return NA(B * 2, Q) * t + i.z;
}
function Rt(i, Q) {
...
function sP(i) {
  const { minX: Q, maxX: t, minY: B, maxY: A } = DB(i), e = i.units === "mm" ? 10 : OA, P = [], r = [], s = (a, o, w, E, h) => {
    h.push(a, o, 0, w, E, 0);
  ...
  const z = new l.Group();
  return r.length > 0 && z.add(QA(r, i.theme.colors.grid.minor, 0.25)), P.length > 0 && z.add(QA(P, i.theme.colors.grid.major, 0.4)), z;
}
function QA(i, Q, t) {
...
function oP(i) {
  const Q = new l.Group(), { minX: t, maxX: B, minY: A, maxY: e } = DB(i), P = i.units === "mm" ? 10 : 1, r = P * 2, s = i.units === "mm" ? 1 : OA, v = 0.5, c = 4, n = 0.1, T = 5, z = i.theme.colors.axes.x, a = i.theme.colors.axes.y, o = { opacity: v, size: c };
  for (let w = P; ; w += r) {
    const E = w * s;
    if (E > B + 1e-6)
      break;
...
var zQ = {
  ...
  overwrite: !1,
  delay: 0
}, dB, q, y, hQ = 1e8, m = 1 / hQ, vB = Math.PI * 2, TP = vB / 4, zP = 0, MA = Math.sqrt, aP = Math.cos, wP = Math.sin, Z = function(Q) {
...
}, $ = Array.isArray, EP = /random\([^)]+\)/g, hP = /,\s*/g, tA = /(?:-?\.?\d|\.)+/gi, jA = /[-+=.]*\d+[.e\-+]*\d*[e\-+]*\d*/g, qQ = /[-+=.]*\d+[.e-]*\d*[a-z%]*/g, Xt = /[-+=.]*\d+\.?\d*(?:e-|e\+)?\d*/gi, FA = /[+-]=-?[.\d]+/, lP = /[^,'"\[\]\s]+/gi, kP = /^[+\-=e\s\d]*\d+[.\d]*([a-z]*|%)\s*$/i, Y, kQ, cB, OB, aQ = {}, ft = {}, fA, GA = function(Q) {
  ...
  suppressEvents: !0
}, MB = {}, fQ = [], nB = {}, pA, cQ = {}, Kt = {}, BA = 30, Ht = [], HB = "", jB = function(Q) {
  var t = Q[0], B, A;
  ...
  for (A = Q.length; A--; )
    Q[A] && (Q[A]._gsap || (Q[A]._gsap = new ee(Q[A], B))) || Q.splice(A, 1);
  return Q;
}, YQ = function(Q) {
  ...
  return t.parent && bQ(t), t._start = S((NQ(B) ? B : B || Q !== Y ? EQ(Q, B, t) : Q._time) + t._delay), t._end = S(t._start + (t.totalDuration() / Math.abs(t.timeScale()) || 0)), IA(Q, t, "_first", "_last", Q._sort ? "_start" : 0), TB(t) || (Q._recent = t), A || SA(Q, t), Q._ts < 0 && St(Q, Q._tTime), Q;
}, YA = function(Q, t) {
  return (aQ.ScrollTrigger || LB("scrollTrigger", t)) && aQ.ScrollTrigger.create(t, Q);
}, VA = function(Q, t, B, A, e) {
  if (GB(Q, t, e), !Q._initted)
    return 1;
  if (!B && Q._pt && !q && (Q._dur && Q.vars.lazy !== !1 || !Q._dur && Q.vars.lazy) && pA !== nQ.frame)
  ...
  var e = Q.ratio, P = t < 0 || !t && (!Q._start && LP(Q) && !(!Q._initted && TB(Q)) || (Q._ts < 0 || Q._dp._ts < 0) && !TB(Q)) ? 0 : 1, r = Q._rDelay, s = 0, v, c, n;
...
}, zB = [].slice, JA = function(Q, t) {
  return Q && DQ(Q) && "length" in Q && (!t && !Q.length || Q.length - 1 in Q && DQ(Q[0])) && !Q.nodeType && Q !== kQ;
}, fP = function(Q, t, B) {
  return B === void 0 && (B = []), Q.forEach(function(A) {
    var e;
    return Z(A) && !t || JA(A, 1) ? (e = B).push.apply(e, lQ(A)) : B.push(A);
  }) || B;
}, lQ = function(Q, t, B) {
  return y && !t && y.selector ? y.selector(Q) : Z(Q) && !B && (cB || !Pt()) ? zB.call((t || OB).querySelectorAll(Q), 0) : $(Q) ? fP(Q, B) : JA(Q) ? zB.call(Q, 0) : Q ? [Q] : [];
}, aB = function(Q) {
  ...
  return Z(A) ? c = n = {
    ...
    if (!w) {
      ...
      A === "random" && RA(w), w.max = D - N, w.min = N, w.v = o = (parseFloat(t.amount) || parseFloat(t.each) * (d > o ? o - 1 : v ? v === "y" ? o / d : d : Math.max(d, o / d)) || 0) * (A === "edges" ? -1 : 1), w.b = o < 0 ? e - o : e, w.u = _(t.amount || t.each) || 0, B = B && o < 0 ? RP(B) : B;
    }
  ...
  var A = t - Q;
  return $(Q) ? WA(Q, i(0, Q.length), t) : mQ(B, function(e) {
    return (A + (e - Q) % A) % A + Q;
  });
}, mP = function i(Q, t, B) {
  var A = t - Q, e = A * 2;
  return $(Q) ? WA(Q, i(0, Q.length - 1), t) : mQ(B, function(P) {
  ...
  if (Q)
    if (Q = !Q.name && Q.default || Q, NB() || Q.headless) {
      ...
      bA(t, A), Q.register && Q.register(rQ, A, PQ);
    } else
      _A.push(Q);
}, g = 255, ot = {
  aqua: [0, g, g],
  ...
  if (!A) {
    if (Q.substr(-1) === "," && (Q = Q.substr(0, Q.length - 1)), ot[Q])
      ...
      if (A = a = Q.match(tA), !t)
        ...
        return A = Q.match(jA), B && A.length < 4 && (A[3] = 1), A;
    } else
      A = Q.match(tA) || ot.transparent;
    A = A.map(Number);
  }
  ...
  if (e = e.map(function(T) {
    return (T = Qe(T, t, 1)) && P + (t ? T[0] + "," + T[1] + "%," + T[2] + "%," + T[3] : T.join(",")) + ")";
  }), B && (c = te(Q), s = B.c, s.join(A) !== c.c.join(A)))
...
}(), yP = /hsl[a]?\(/, Be = function(Q) {
  var t = Q.join(" "), B;
  if (GQ.lastIndex = 0, GQ.test(t))
    return B = yP.test(t), Q[1] = rA(Q[1], B), Q[0] = rA(Q[0], B, te(Q[1])), !0;
}, kt, nQ = function() {
  ...
  return n = {
    ...
    wake: function() {
      fA && (!cB && NB() && (kQ = cB = window, OB = kQ.document || {}, aQ.gsap = rQ, (kQ.gsapVersions || (kQ.gsapVersions = [])).push(rQ.version), GA(ft || kQ.GreenSockGlobals || !kQ.gsap && kQ || {}), _A.forEach($A)), c = typeof requestAnimationFrame < "u" && requestAnimationFrame, s && n.sleep(), v = c || function(w) {
  ...
  return !kt && nQ.wake();
}, p = {}, IP = /^[\d.\-M][\d.\-,\s]/, SP = /["']/g, YP = function(Q) {
  for (var t = {}, B = Q.substr(1, Q.length - 3).split(":"), A = B[0], e = 1, P = B.length, r, s, v; e < P; e++)
    s = B[e], r = e !== P - 1 ? s.lastIndexOf(",") : s.length, v = s.substr(0, r), t[A] = isNaN(v) ? v.replace(SP, "").trim() : +v, A = s.substr(r + 1).trim();
  return t;
}, VP = function(Q) {
  ...
  return B && t.length > 1 && B.config ? B.config.apply(null, ~Q.indexOf("{") ? [YP(t[1])] : VP(Q).split(",").map(xA)) : p._CE && IP.test(Q) ? p._CE("", Q) : B;
}, RP = function(Q) {
  return function(t) {
    return 1 - Q(1 - t);
  };
...
var ee = function(Q, t) {
  this.id = zP++, Q._gsap = this, this.target = Q, this.harness = t, this.get = t ? t.get : gA, this.set = t ? t.getSetter : pB;
  ...
  return Q.delay = function(B) {
    ...
    if (e && e.smoothChildTiming && this._ts) {
      for (St(this, B), !e._dp || e.parent || SA(e, this); e && e.parent; )
    ...
    return this._rts = +B || 0, this._ts = this._ps || B === -m ? 0 : this._rts, this.totalTime(dt(-Math.abs(this._delay), this.totalDuration(), e), A !== !1), It(this), NP(this);
  }, Q.paused = function(B) {
...
var BQ = /* @__PURE__ */ function(i) {
  ...
  function Q(B, A) {
    var e;
    return B === void 0 && (B = {}), e = i.call(this, B) || this, e.labels = {}, e.smoothChildTiming = !!B.smoothChildTiming, e.autoRemoveChildren = !!B.autoRemoveChildren, e._sort = AQ(B.sortChildren), Y && uQ(B.parent || Y, dQ(e), A), B.reversed && e.reverse(), B.paused && e.paused(!0), B.scrollTrigger && YA(dQ(e), B.scrollTrigger), e;
  }
  var t = Q.prototype;
  return t.to = function(A, e, P) {
    ...
    if (this !== Y && c > s && A >= 0 && (c = s), c !== this._tTime || P || n) {
      ...
      if (this._hasPause && !this._forcing && this._lock < 2 && (h = HP(this, S(r), S(T)), h && (c -= T - (T = h._start))), this._tTime = c, this._time = T, this._act = !!u, this._initted || (this._onUpdate = this.vars.onUpdate, this._initted = 1, this._zTime = A, r = 0), !r && c && v && !e && !k && (oQ(this, "onStart"), this._tTime !== c))
        return this;
      if (T >= r && A >= 0)
        for (z = this._first; z; ) {
    ...
    return A === void 0 && (A = this._time), iA(this, EQ(this, A));
  }, t.previousLabel = function(A) {
    return A === void 0 && (A = this._time), iA(this, EQ(this, A), 1);
    ...
    if (Y._ts && (mA(Y, pt(A, Y)), pA = nQ.frame), nQ.frame >= BA) {
      BA += zQ.autoSleep || 120;
      var e = Y._first;
...
var XP = function(Q, t, B, A, e, P, r) {
  var s = new PQ(this._pt, Q, t, 0, 1, ce, null, e), v = 0, c = 0, n, T, z, a, o, w, E, h;
  ...
  J(A) && (A = A(e || 0, Q, P));
  var n = Q[t], T = B !== "get" ? B : J(n) ? v ? Q[t.indexOf("set") || !J(Q["get" + t.substr(3)]) ? t : "get" + t.substr(3)](v) : Q[t]() : n, z = J(n) ? v ? _P : se : bB, a;
  if (Z(A) && (~A.indexOf("random(") && (A = lt(A)), A.charAt(1) === "=" && (a = $Q(T, A) + (_(T) || 0), (a || a === 0) && (A = a))), !c || T !== A || EB)
    return !isNaN(T * A) && A !== "" ? (a = new PQ(this._pt, Q, t, +T || 0, A - (T || 0), typeof n == "boolean" ? Qi : ve, 0, z), v && (a.fp = v), r && a.modifier(r, this, Q), this._pt = a) : (!n && !(t in Q) && LB(t, A), XP.call(this, Q, t, T, A, z, s || zQ.stringFilter, v));
}, KP = function(Q, t, B, A, e) {
  if (J(Q) && (Q = at(Q, e, t, B, A)), !DQ(Q) || Q.style && Q.nodeType || $(Q) || HA(Q))
    return Z(Q) ? at(Q, e, t, B, A) : Q;
  var P = {}, r;
  for (r in Q)
    P[r] = at(Q[r], e, t, B, A);
  return P;
}, Pe = function(Q, t, B, A, e, P) {
  var r, s, v, c;
  ...
  if (U && (!T || !e) && (e = "none"), Q._ease = JQ(e, wt.ease), Q._rEase = k && (JQ(k) || Q._ease), Q._from = !U && !!A.runBackwards, Q._from && (Q.ratio = 1), !U || T && !A.stagger) {
    if (C = w[0] ? YQ(w[0]).harness : 0, f = C && A[C.prop], O = bt(A, MB), o && (o._zTime < 0 && o.progress(1), t < 0 && c && r && !z ? o.render(-1, !0) : o.revert(c && a ? Mt : uP), o._lazy = 0), P) {
    ...
    for (Q._pt = Q._ptCache = 0, s = a && AQ(s) || s && !a, D = 0; D < w.length; D++) {
      if (M = w[D], G = M._gsap || jB(w)[D]._gsap, Q._ptLookup[D] = F = {}, nB[G.id] && fQ.length && Gt(), H = h === w ? D : h.indexOf(M), C && (j = new C()).init(M, f || O, Q, H, h) !== !1 && (Q._pt = d = new PQ(Q._pt, M, j.name, 0, 1, j.render, j, 0, j.priority), j._props.forEach(function(R) {
        ...
        for (N in O)
          cQ[N] && (j = Pe(N, O, Q, H, M, h)) ? j.priority && (b = 1) : F[N] = d = fB.call(Q, M, N, "get", O[N], H, h, 0, A.stringFilter);
      Q._op && Q._op[D] && Q.kill(M, Q._op[D]), u && Q._pt && (HQ = Q, Y.killTweensOf(M, F, Q.globalTime(t)), I = !Q.parent, HQ = 0), Q._pt && s && (nB[G.id] = 1);
    }
    b && ne(Q), Q._onInit && Q._onInit(Q);
  }
...
}, ie = HB + "repeat,repeatDelay,yoyo,repeatRefresh,yoyoEase,easeReverse,autoRevert", re = {};
eQ(ie + ",id,stagger,delay,duration,paused,scrollTrigger", function(i) {
  return re[i] = 1;
});
var K = /* @__PURE__ */ function(i) {
  ...
  function Q(B, A, e, P) {
    ...
    var s = r.vars, v = s.duration, c = s.delay, n = s.immediateRender, T = s.stagger, z = s.overwrite, a = s.keyframes, o = s.defaults, w = s.scrollTrigger, E = A.parent || Y, h = ($(B) || HA(B) ? NQ(B[0]) : "length" in A) ? [B] : lQ(B), u, U, k, O, D, N, d, M;
    if (r._targets = h.length ? jB(h) : Et("GSAP target " + B + " not found. https://gsap.com", !zQ.nullTargetWarn) || [], r._ptLookup = [], r._overwrite = z, a || T || Lt(v) || Lt(c)) {
      ...
      if (u = r.timeline = new BQ({
        ...
        if (O = h.length, d = T && XA(T), DQ(T))
          for (D in T)
            ~ie.indexOf(D) && (M || (M = {}), M[D] = T[D]);
        for (U = 0; U < O; U++)
          k = bt(A, re), k.stagger = 0, b && (k.easeReverse = b), M && Bt(k, M), N = h[U], k.duration = +at(v, dQ(r), U, N, h), k.delay = (+at(c, dQ(r), U, N, h) || 0) - r._delay, !T && O === 1 && k.delay && (r._delay = c = k.delay, r._start += c, k.delay = 0), u.to(N, k, d ? d(U, N, h) : 0), u._ease = p.none;
      ...
      r.timeline = 0;
    return z === !0 && !dB && (HQ = dQ(r), Y.killTweensOf(h), HQ = 0), uQ(E, dQ(r), e), A.reversed && r.reverse(), A.paused && r.paused(!0), (n || !v && !a && r._start === S(E._time) && AQ(n) && OP(dQ(r)) && E.data !== "nested") && (r._tTime = -m, r.render(Math.max(0, -c) || 0)), w && YA(dQ(r), w), r;
  }
  var t = Q.prototype;
  return t.render = function(A, e, P) {
    ...
    return this._initted || GB(this, v), c = this._ease(v / this._dur), ZP(this, A, e, P, r, c, v, s) ? this.resetTo(A, e, P, r, 1) : (St(this, 0), this.parent || IA(this._dp, this, "_first", "_last", this._dp._sort ? "_start" : 0), this.render(0));
  }, t.kill = function(A, e) {
...
var bB = function(Q, t, B) {
  ...
  return J(Q[t]) ? se : CB(Q[t]) && Q.setAttribute ? $P : bB;
}, ve = function(Q, t) {
  ...
  function i(t, B, A, e, P, r, s, v, c) {
    this.t = B, this.s = e, this.c = P, this.p = A, this.r = r || ve, this.d = s || this, this.set = v || bB, this.pr = c || 0, this._next = t, t && (t._prev = this);
  }
  var Q = i.prototype;
...
zQ.stringFilter = Be;
var RQ = [], jt = {}, ei = [], sA = 0, Pi = 0, _t = function(Q) {
  return (jt[Q] || ei).map(function(t) {
    return t();
  });
}, hB = function() {
  var Q = Date.now(), t = [];
  Q - sA > 2 && (_t("matchMediaInit"), RQ.forEach(function(B) {
  ...
  function i(t, B) {
    this.selector = B && aB(B), this.data = [], this._r = [], this.isReverted = !1, this.id = Pi++, t && this.add(t);
  }
  var Q = i.prototype;
  ...
  return Q.add = function(B, A, e) {
    ...
    var P = new oe(0, e || this.scope), r = P.conditions = {}, s, v, c;
  ...
  defaults: function(Q) {
    return Q && Q.ease && (Q.ease = JQ(Q.ease, wt.ease)), AA(wt, Q || {});
  },
  config: function(Q) {
    return AA(zQ, Q || {});
  },
...
}, $t("roundProps", wB), $t("modifiers"), $t("snap", KA)) || gt;
 ...
 * @author: Jack Doyle, jack@greensock.com
*/
var vA, jQ, Qt, mB, SQ, cA, xB, vi = function() {
  return typeof window < "u";
}, OQ = {}, IQ = 180 / Math.PI, tt = Math.PI / 180, KQ = Math.atan2, nA = 1e8, yB = /([A-Z])/g, ci = /(left|right|width|margin|padding|x)/i, ni = /[\s,\(]\S/, UQ = {
  ...
  if (this.tfm) {
    for (P in this.tfm)
      A[P] = this.tfm[P];
    A.svg && (A.renderTransform(), t.setAttribute("data-svg-origin", this.svgo || "")), e = xB(), (!e || !e.isStart) && !B[V] && (ae(B), A.zOrigin && B[iQ] && (B[iQ] += " " + A.zOrigin + "px", A.zOrigin = 0, A.renderTransform()), A.uncache = 1);
  }
}, we = function(Q, t) {
  var B = {
    target: Q,
...
}, oA = "O,Moz,ms,Ms,Webkit".split(","), it = function(Q, t, B) {
  var A = t || SQ, e = A.style, P = 5;
  if (Q in e && !B)
    return Q;
  for (Q = Q.charAt(0).toUpperCase() + Q.substr(1); P-- && !(oA[P] + Q in e); )
    ;
  return P < 0 ? null : (P === 3 ? "ms" : P >= 0 ? oA[P] : "") + Q;
}, uB = function() {
  vi() && window.document && (vA = window, jQ = vA.document, Qt = jQ.documentElement, SQ = kB("div") || {
    style: {}
  }, kB("div"), V = it(V), iQ = V + "Origin", SQ.style.cssText = "border-width:0;line-height:0;position:absolute;padding:0", Ee = !!it("perspective"), xB = rQ.core.reverting, mB = 1);
}, TA = function(Q) {
  ...
  return t && (t.width || t.height) || B || (t = TA(Q)), t && !t.width && !t.x && !t.y ? {
    x: +zA(Q, ["x", "cx", "x1"]) || 0,
    y: +zA(Q, ["y", "cy", "y1"]) || 0,
    width: 0,
    height: 0
  ...
  return !!(Q.getCTM && (!Q.parentNode || Q.ownerSVGElement) && he(Q));
}, pQ = function(Q, t) {
  if (t) {
    var B = Q.style, A;
  ...
  var r = new PQ(Q._pt, t, B, 0, 1, P ? ze : Te);
  ...
  var e = parseFloat(B) || 0, P = (B + "").trim().substr((e + "").length) || "px", r = SQ.style, s = ci.test(t), v = Q.tagName.toLowerCase() === "svg", c = (v ? "client" : "offset") + (s ? "Width" : "Height"), n = 100, T = A === "px", z = A === "%", a, o, w, E;
  if (A === P || !e || aA[A] || aA[P])
    return e;
  if (P !== "px" && !T && (e = i(Q, t, B, "px")), E = Q.getCTM && le(Q), (z || P === "%") && (OQ[t] || ~t.indexOf("adius")))
  ...
  var e;
  return mB || uB(), t in UQ && t !== "transform" && (t = UQ[t], ~t.indexOf(",") && (t = t.split(",")[0])), OQ[t] && t !== "transform" ? (e = Dt(Q, A), e = t !== "transformOrigin" ? e[t] : e.svg ? e.origin : xt(TQ(Q, iQ)) + " " + e.zOrigin + "px") : (e = Q.style[t], (!e || e === "auto" || A || ~(e + "").indexOf("calc(")) && (e = mt[t] && mt[t](Q, t, B) || TQ(Q, t) || gA(Q, t) || (t === "opacity" ? 1 : 0))), B && !~(e + "").trim().indexOf(" ") ? gQ(Q, t, e, B) + B : e;
}, Ci = function(Q, t, B, A) {
  if (!B || B === "none") {
    var e = it(t, Q, 1), P = e && TQ(Q, e, 1);
    P && P !== B ? (t = e, B = P) : t === "borderColor" && (B = TQ(Q, "borderTopColor"));
  }
  var r = new PQ(this._pt, Q.style, t, 0, 1, ce), s = 0, v = 0, c, n, T, z, a, o, w, E, h, u, U, k;
  if (r.b = B, r.e = A, B += "", A += "", A.substring(0, 6) === "var(--" && (A = TQ(Q, A.substring(4, A.indexOf(")")))), A === "auto" && (o = Q.style[t], Q.style[t] = A, A = TQ(Q, t) || A, o ? Q.style[t] = o : pQ(Q, t)), c = [B, A], Be(c), B = c[0], A = c[1], T = B.match(qQ) || [], k = A.match(qQ) || [], k.length) {
    for (; n = qQ.exec(A); )
  ...
  return (B === "top" || B === "bottom" || A === "left" || A === "right") && (Q = B, B = A, A = Q), t[0] = wA[B] || B, t[1] = wA[A] || A, t.join(" ");
}, Oi = function(Q, t) {
  if (t.tween && t.tween._time === t.tween._dur) {
    ...
    s && (pQ(B, V), P && (P.svg && B.removeAttribute("transform"), A.scale = A.rotate = A.translate = "none", Dt(B, 1), P.uncache = 1, ae(A)));
  }
}, mt = {
  ...
  */
  ...
  t.z = "0px", t.rotationY = t.rotationX = "0deg", t.force3D = 0, Ue(Q, t);
}, xQ = "0deg", st = "0px", yQ = ") ", Ue = function(Q, t) {
  ...
  return r && (n = e.split("_")[1], n === "short" && (v %= P, v !== v % (P / 2) && (v += v < 0 ? P : -P)), n === "cw" && v < 0 ? v = (v + P * nA) % P - ~~(v / P) * P : n === "ccw" && v > 0 && (v = (v - P * nA) % P - ~~(v / P) * P)), Q._pt = T = new PQ(Q._pt, t, B, A, v, oi), T.e = c, T.u = "deg", Q._props.push(B), T;
}, hA = function(Q, t) {
  for (var B in t)
    Q[B] = t[B];
  return Q;
}, ji = function(Q, t, B) {
  var A = hA({}, B._gsap), e = "perspective,force3D,transformOrigin,svgOrigin", P = B.style, r, s, v, c, n, T, z, a;
...
var De = {
  ...
  init: function(Q, t, B, A, e) {
    ...
    mB || uB(), this.styles = this.styles || we(Q), N = this.styles.props, this.tween = B;
    for (o in t)
      if (o !== "autoRound" && (c = t[o], !(cQ[o] && Pe(o, t, B, A, Q, e)))) {
        if (z = typeof c, a = mt[o], z === "function" && (c = c.call(B, A, Q, e), z = typeof c), z === "string" && ~c.indexOf("random(") && (c = lt(c)), a)
          ...
          if (s && o in s ? (v = typeof s[o] == "function" ? s[o].call(B, A, Q, e) : s[o], Z(v) && ~v.indexOf("random(") && (v = lt(v)), _(v + "") || v === "auto" || (v += zQ.units[o] || _(CQ(Q, o)) || ""), (v + "").charAt(1) === "=" && (v = CQ(Q, o))) : v = CQ(Q, o), T = parseFloat(v), h = z === "string" && c.charAt(1) === "=" && c.substr(0, 2), h && (c = c.substr(2)), n = parseFloat(c), o in UQ && (o === "autoAlpha" && (T === 1 && CQ(Q, "visibility") === "hidden" && n && (T = 0), N.push("visibility", 0, r.visibility), FQ(this, r, "visibility", T ? "inherit" : "hidden", n ? "inherit" : "hidden", !n)), o !== "scale" && o !== "transform" && (o = UQ[o], ~o.indexOf(",") && (o = o.split(",")[0]))), u = o in OQ, u) {
            ...
            if (U || (k = Q._gsap, k.renderTransform && !t.parseTransform || Dt(Q, t.parseTransform), O = t.smoothOrigin !== !1 && k.smooth, U = this._pt = new PQ(this._pt, r, V, 0, 1, k.renderTransform, k, 0, -1), U.dep = 1), o === "scale")
              ...
              N.push(iQ, 0, r[iQ]), c = Ni(c), k.svg ? UB(Q, c, 0, O, 0, this) : (E = parseFloat(c.split(" ")[2]) || 0, E !== k.zOrigin && FQ(this, k, "zOrigin", k.zOrigin, E), FQ(this, r, o, xt(v), xt(c)));
              continue;
            } else if (o === "svgOrigin") {
              UB(Q, c, 1, O, 0, this);
              continue;
          ...
          if (u || (n || n === 0) && (T || T === 0) && !ni.test(c) && o in r)
            w = (v + "").substr((T + "").length), n || (n = 0), E = _(c) || (o in zQ.units ? zQ.units[o] : w), w !== E && (T = gQ(Q, o, v, E)), this._pt = new PQ(this._pt, u ? k : r, o, T, (h ? $Q(T, h + n) : n) - T, !u && (E === "px" || o === "zIndex") && t.autoRound !== !1 ? ai : lB), this._pt.u = E || 0, u && d !== c ? (this._pt.b = v, this._pt.e = d, this._pt.r = zi) : w !== E && E !== "%" && (this._pt.b = v, this._pt.r = Ti);
          else if (o in r)
  ...
  getSetter: function(Q, t, B) {
    var A = UQ[t];
    return A && A.indexOf(",") < 0 && (t = A), t in OQ && t !== iQ && (Q._gsap.x || CQ(Q, "x")) ? B && cA === B ? t === "scale" ? li : hi : (cA = B || {}) && (t === "scale" ? ki : ui) : Q.style && !CB(Q.style[t]) ? wi : ~t.indexOf("-") ? Ei : pB(Q, t);
  },
  core: {
    _removeProperty: pQ,
    _getMatrix: IB
...
rQ.core.getStyleSaver = we;
(function(i, Q, t, B) {
  var A = eQ(i + "," + Q + "," + t, function(e) {
    OQ[e] = 1;
  });
  eQ(Q, function(e) {
    zQ.units[e] = "deg", ke[e] = 1;
...
function Ne(i) {
  return i.bit.colorSource === "custom" ? i.bit.color : fi;
}
function Ii(i) {
  const Q = Ct(i.bit.opacity), t = Ce(Math.max(1e-3, i.bit.size)), B = new l.Color(Ne(i)), A = mi(), e = new l.MeshStandardMaterial({
    color: B,
    metalness: 0.8,
    roughness: 0.35,
    roughnessMap: A,
...
function Yi(i) {
  ...
  const v = (n) => {
    ...
    if (w || E)
      Q.remove(B), UA(B), B = uA(n), Q.add(B), r == null || r.kill(), P == null || P.kill(), P = s(n.bit.spinRpm), e && P.timeScale(1).play();
    else if (T === "laser") {
  ...
  return {
    ...
    dispose: () => {
      r == null || r.kill(), P == null || P.kill(), Q.remove(B), UA(B);
    }
  };
}
...
class tr {
  constructor(Q) {
    this.viewCube = null, this.resizeObserver = null, this.onWindowResize = null, this.animationFrameId = null, this.gridGroup = null, this.axesGroup = null, this.gridLabelsGroup = null, this.boundingBoxGroup = null, this.machineBedGroup = null, this.bitMarker = null, this.preLaserBitType = "drill", this.toolpathStreams = [], this.toolpathCutBucketCount = 1, this.toolpathRotationA = 0, this.lastBitPosition = { x: 0, y: 0, z: 0, a: 0 }, this.sim3dHandle = null, this.currentLines = [], this.linePositions = null, this.renderSequence = 0, this.currentBounds = null, this.cameraFocusTransition = null, this.cameraFollowRequested = !1, this.cameraFollowInterrupted = !1, this.cameraFollowOffset = null, this.id = Q.id, this.container = Q.container, this.callbacks = Q.callbacks ?? {}, this.options = dA(ye, Q.options), this.canvas = document.createElement("canvas"), this.canvas.style.width = "100%", this.canvas.style.height = "100%", this.canvas.style.display = "block", this.container.appendChild(this.canvas), eP(this.container), this.renderer = new l.WebGLRenderer({
      canvas: this.canvas,
  ...
  hideUntilLine(Q, t) {
    ...
    for (const e of this.toolpathStreams) {
      ...
      B === "grey" ? (e.line.geometry.setDrawRange(0, e.totalVertices), ge({ stream: e, nextCursorVertex: P, options: this.options })) : e.line.geometry.setDrawRange(P, Math.max(0, e.totalVertices - P));
    }
  }
  seekToLine(Q, t) {
  ...
  applyCameraDepthRange(Q) {
    ...
    Object.assign(this.perspectiveCamera, _B(t)), Object.assign(this.orthographicCamera, $B(t, t)), this.perspectiveCamera.updateProjectionMatrix(), this.orthographicCamera.updateProjectionMatrix();
  }
  /**
  ...
  setActiveProjection(Q) {
    ...
    if (t) {
      const P = this.orthoFramedHeight(), r = Math.max(1e-6, NA(P, this.options.camera.fov)), s = this.camera.position.clone().sub(B).normalize();
  ...
  startSnapToView(Q, t, B, A, e = !1) {
    const P = WB(Q), r = t.clone().add(P.multiplyScalar(B));
  ...
  async loadFromWorkerData(Q, t) {
    this.currentLines = [];
    const { rapids: B, cuts: A } = Le(Q, t), e = (Q.toolchangeCount ?? 0) > 0;
  ...
  setOptions(Q) {
    var T, z, a, o, w, E, h, u, U, k, O, D, N, d, M, b, G, C, j, F, H, f, I, R, QQ, sQ, W, x, tQ, vQ, rt, SB, YB, VB;
    const t = this.options;
    this.options = dA(this.options, Q), (t.render.antialias !== this.options.render.antialias || t.render.theme.background !== this.options.render.theme.background) && this.renderer.setClearColor(this.options.render.theme.background, 1), (t.render.theme.opacity !== this.options.render.theme.opacity || t.render.theme.rapidOpacity !== this.options.render.theme.rapidOpacity) && this.refreshToolpathOpacities(), t.camera.fov !== this.options.camera.fov && (this.perspectiveCamera.fov = this.options.camera.fov, this.perspectiveCamera.updateProjectionMatrix()), t.camera.projection !== this.options.camera.projection && this.setActiveProjection(this.options.camera.projection), t.camera.orbit.enableDamping !== this.options.camera.orbit.enableDamping && (this.controls.enableDamping = this.options.camera.orbit.enableDamping);
    ...
    if (t.sim3d.erosionPasses !== this.options.sim3d.erosionPasses && this.sim3dHandle) {
      const Oe = this.sim3dHandle.currentLine, JB = Yt(Oe, this.sim3dHandle.data);
      Vt(JB, this.sim3dHandle.data.resolution, this.options.sim3d.erosionPasses, this.sim3dHandle.data.slabBounds.zTop), Jt(this.sim3dHandle.slab, JB);
    }
  ...
  setGeometryEmpty() {
    this.setSim3dHandle(null), Ge(this.toolpathRoot, this.toolpathStreams), this.toolpathStreams = [], this.toolpathCutBucketCount = 1, this.linePositions = null, this.currentBounds = null, this.emitBoundsChanged(), this.refreshBoundingBox();
  }
  ...
  async renderScene() {
    ...
    try {
      ...
      const t = await Me(this.currentLines, {
  ...
  setSim3dHandle(Q) {
    this.sim3dHandle && (this.scene.remove(this.sim3dHandle.slab.group), Je(this.sim3dHandle.slab), this.sim3dHandle = null), Q && (this.sim3dHandle = Q, this.scene.add(Q.slab.group));
  }
  ...
  async buildAndApplySim3d(Q) {
    ...
    let A;
    try {
      A = await Ie(this.currentLines, t, B, {
        arcSegments: this.options.geometry.arcSegments,
        batch: {
          shouldAbort: () => Q !== this.renderSequence,
          onProgress: (r, s) => {
  ...
  startCameraFocus(Q) {
    ...
    Q.getSize(B);
    const A = qB(B, this.options.camera.fov), e = WB("front-top-left"), P = t.clone().add(e.multiplyScalar(A));
