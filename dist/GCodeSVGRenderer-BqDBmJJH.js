import * as h from "three";
import { GCodeVirtualizer as eB, buildWorkerToolpathStreams as Ue, buildToolpathGeometryFromLinesBatched as De, buildMovementVerticesFromLines as de, buildWorkerSegmentGroups as Ce } from "./gviewer.js";
import { OrbitControls as VB } from "three/examples/jsm/controls/OrbitControls.js";
import { STLLoader as Ne } from "three/examples/jsm/loaders/STLLoader.js";
function Oe(r) {
  const Q = [];
  let t = null;
  for (const B of r.specs) {
    if (B.positions.length === 0 || B.opacity <= 0)
      continue;
    const A = B.positions.length / 3, e = B.colors ? B.colors.slice() : kA(B.kind, A, r.options), P = new Float32Array(e.length);
    P.set(e);
    const i = new h.BufferGeometry();
    i.setAttribute("position", new h.BufferAttribute(B.positions, 3));
    const s = new h.BufferAttribute(P, 3);
    s.setUsage(h.DynamicDrawUsage), i.setAttribute("color", s), i.computeBoundingBox();
    const v = new h.LineBasicMaterial({
      vertexColors: !0,
      transparent: !0,
      opacity: B.opacity
    }), c = new h.LineSegments(i, v);
    r.scene.add(c), Q.push({
      line: c,
      baseColors: e,
      simColors: P,
      prefixEndVertex: B.prefixEndVertex,
      totalVertices: A,
      greyCursorVertex: 0,
      kind: B.kind,
      cutBucketIndex: B.cutBucketIndex,
      lineGroupIndex: B.lineGroupIndex ?? null,
      workerColors: B.colors ? e : void 0
    }), i.boundingBox && (t = t ? t.union(i.boundingBox) : i.boundingBox.clone());
  }
  return { streams: Q, bounds: t };
}
function Le(r, Q) {
  for (const t of Q)
    r.remove(t.line), t.line.geometry.dispose(), t.line.material.dispose();
}
function Me(r, Q) {
  for (const t of r) {
    if (t.workerColors) continue;
    const B = kA(t.kind, t.totalVertices, Q);
    t.baseColors = B, t.simColors.set(B), t.greyCursorVertex = 0;
    const A = t.line.geometry.getAttribute("color");
    A.clearUpdateRanges(), A.addUpdateRange(0, t.simColors.length), A.needsUpdate = !0;
  }
}
function He(r) {
  const Q = PB(r.options.render.theme.rapidOpacity ?? 0.3);
  for (const t of r.streams) {
    const B = t.line.material;
    if (t.kind === "rapid") {
      B.opacity = Q;
      continue;
    }
    B.opacity = Fe({
      bucketIndex: t.cutBucketIndex ?? 0,
      bucketCount: r.cutBucketCount,
      baseOpacity: r.options.render.theme.opacity
    });
  }
}
function je(r) {
  const Q = r.stream.totalVertices, t = Math.max(0, Math.min(Q, Math.floor(r.nextCursorVertex))), B = Math.max(0, Math.min(Q, Math.floor(r.stream.greyCursorVertex)));
  if (t === B)
    return;
  const A = r.stream.line.geometry.getAttribute("color"), e = fe(r.options), P = Math.min(B, t), i = Math.max(B, t);
  if (t > B)
    for (let s = B; s < t; s += 1) {
      const v = s * 3;
      r.stream.simColors[v] = e.r, r.stream.simColors[v + 1] = e.g, r.stream.simColors[v + 2] = e.b;
    }
  else {
    const s = t * 3, v = B * 3;
    r.stream.simColors.set(r.stream.baseColors.subarray(s, v), s);
  }
  A.clearUpdateRanges(), A.addUpdateRange(P * 3, (i - P) * 3), A.needsUpdate = !0, r.stream.greyCursorVertex = t;
}
function kA(r, Q, t) {
  const B = t.render.theme.colors, A = t.mode.laser ? B.laser ?? B.cutting : B.cutting, e = new h.Color(r === "rapid" ? B.rapid : A), P = new Float32Array(Q * 3);
  for (let i = 0; i < Q; i += 1) {
    const s = i * 3;
    P[s] = e.r, P[s + 1] = e.g, P[s + 2] = e.b;
  }
  return P;
}
function fe(r) {
  const Q = r.render.theme.colors, t = new h.Color(r.render.theme.background), B = new h.Color(Q.processed ?? Q.cutting), A = t.clone().lerp(B, 0.65);
  return { r: A.r, g: A.g, b: A.b };
}
function Fe(r) {
  if (r.bucketCount <= 1)
    return PB(r.baseOpacity);
  const Q = r.bucketIndex / (r.bucketCount - 1);
  return PB(r.baseOpacity * Q);
}
function PB(r) {
  return Number.isFinite(r) ? Math.max(0, Math.min(1, r)) : 0;
}
const JB = {
  background: "#111827",
  opacity: 0.9,
  rapidOpacity: 0.3,
  colors: {
    rapid: "#0ef6ae",
    cutting: "#3e85c7",
    laser: "#a855f7",
    processed: "#6b7280",
    boundingBox: "#e2e8f0",
    machineBed: "#fbbf24",
    machineBedKeepout: "#df3b3b",
    grid: { major: "#2f3840", minor: "#1f252b" },
    axes: { x: "#df3b3b", y: "#06b881", z: "#295d8d" }
  }
}, be = {
  units: "mm",
  mode: { laser: !1, sim3d: !1 },
  sim3d: { toolDiameter: 6.35, resolution: 256, showToolpath: !1, erosionPasses: 2 },
  bit: {
    enabled: !0,
    type: "drill",
    size: 4.05,
    opacity: 0.9,
    tweenMs: 140,
    colorSource: "cutting",
    color: JB.colors.cutting,
    spinRpm: 300
  },
  progress: { mode: "grey" },
  grid: { sizeX: 1e3, sizeY: 1e3, axisDepth: 200, labels: !0, bounds: null },
  boundingBox: { visible: !1, labels: !1 },
  machineBed: { visible: !1, min: null, max: null, keepout: null },
  geometry: { arcSegments: 30, batching: { progressEveryLines: 5e3, yieldEveryLines: 5e4 } },
  render: { antialias: !0, theme: JB },
  camera: {
    fov: 45,
    focusDurationMs: 900,
    orbit: { enableDamping: !0 },
    initialPosition: { x: 0, y: -200, z: 200 }
  }
};
async function Ge(r, Q, t, B) {
  var M, G, b;
  const A = Math.max(2, Math.floor(t)), e = (B == null ? void 0 : B.arcSegments) ?? 30, P = (B == null ? void 0 : B.checkpointEveryLines) ?? 2e3, i = ((M = B == null ? void 0 : B.batch) == null ? void 0 : M.yieldEveryLines) ?? 1e4, s = ((G = B == null ? void 0 : B.batch) == null ? void 0 : G.shouldAbort) ?? (() => !1), v = (b = B == null ? void 0 : B.batch) == null ? void 0 : b.onProgress;
  if (r.length === 0) {
    const C = {
      xMin: -1,
      xMax: 1,
      yMin: -1,
      yMax: 1,
      zTop: 1,
      zBot: 0
    }, j = new Float32Array(A * A).fill(C.zTop), f = /* @__PURE__ */ new Map();
    return f.set(0, new Float32Array(j)), { moves: [], checkpoints: f, slabBounds: C, resolution: A, toolRadius: Q };
  }
  let c = 1 / 0, n = -1 / 0, T = 1 / 0, z = -1 / 0, a = 1 / 0, o = -1 / 0;
  const w = new eB({
    onLinearMove(C) {
      if (C.modals.motion === "G0") return;
      const j = C.transformedStart ?? C.start, f = C.transformedEnd ?? C.end;
      for (const H of [j, f])
        H.X < c && (c = H.X), H.X > n && (n = H.X), H.Y < T && (T = H.Y), H.Y > z && (z = H.Y), H.Z < a && (a = H.Z), H.Z > o && (o = H.Z);
    },
    onArcMove(C) {
      const j = C.transformedStart ?? C.start, f = C.transformedEnd ?? C.end, H = C.transformedMax ?? C.max;
      for (const F of [j, f, H])
        F.X < c && (c = F.X), F.X > n && (n = F.X), F.Y < T && (T = F.Y), F.Y > z && (z = F.Y), F.Z < a && (a = F.Z), F.Z > o && (o = F.Z);
    }
  });
  for (const C of r)
    w.processLine(C);
  isFinite(c) || (c = -1, n = 1, T = -1, z = 1, a = 0, o = 0), c === n && (c -= Q * 2, n += Q * 2), T === z && (T -= Q * 2, z += Q * 2), c -= Q, n += Q, T -= Q, z += Q;
  const E = o;
  let l = a - Q;
  l >= E && (l = E - 1);
  const u = { xMin: c, xMax: n, yMin: T, yMax: z, zTop: E, zBot: l }, U = new Float32Array(A * A).fill(E), k = /* @__PURE__ */ new Map();
  k.set(0, new Float32Array(U));
  const O = [];
  let D = 0, N = 0;
  const d = new eB({
    onLinearMove(C) {
      const j = C.transformedStart ?? C.start, f = C.transformedEnd ?? C.end, H = {
        lineIndex: N,
        x0: j.X,
        y0: j.Y,
        z0: j.Z,
        x1: f.X,
        y1: f.Y,
        z1: f.Z
      };
      O.push(H), iB(H, U, u, Q, A);
    },
    onArcMove(C) {
      const j = C.transformedStart ?? C.start, f = C.transformedEnd ?? C.end, H = C.transformedCenter ?? C.center, F = ge(j, f, H, C.plane, C.motion, e);
      for (const I of F) {
        const R = { lineIndex: N, ...I };
        O.push(R), iB(R, U, u, Q, A);
      }
    }
  });
  for (let C = 0; C < r.length && !s(); C++) {
    N = C;
    const j = r[C];
    j !== void 0 && d.processLine(j), C - D >= P && (k.set(C, new Float32Array(U)), D = C), C % i === 0 && C > 0 && (v == null || v(C, r.length), await new Promise((f) => setTimeout(f, 0)));
  }
  return v == null || v(r.length, r.length), { moves: O, checkpoints: k, slabBounds: u, resolution: A, toolRadius: Q };
}
function iB(r, Q, t, B, A) {
  const e = (t.xMax - t.xMin) / A, P = (t.yMax - t.yMin) / A, i = Math.hypot(r.x1 - r.x0, r.y1 - r.y0), s = Math.max(1e-3, Math.min(e, P) * 0.5), v = Math.max(1, Math.ceil(i / s)), c = Math.ceil(B / e), n = Math.ceil(B / P), T = B * B;
  for (let z = 0; z <= v; z++) {
    const a = z / v, o = r.x0 + (r.x1 - r.x0) * a, w = r.y0 + (r.y1 - r.y0) * a, E = r.z0 + (r.z1 - r.z0) * a, l = Math.floor((o - t.xMin) / e), u = Math.floor((w - t.yMin) / P);
    for (let U = -c; U <= c; U++) {
      const k = l + U;
      if (k < 0 || k >= A) continue;
      const O = t.xMin + (k + 0.5) * e, D = (O - o) * (O - o);
      if (!(D > T))
        for (let N = -n; N <= n; N++) {
          const d = u + N;
          if (d < 0 || d >= A) continue;
          const M = t.yMin + (d + 0.5) * P;
          if (D + (M - w) * (M - w) > T) continue;
          const b = d * A + k;
          Q[b] > E && (Q[b] = E);
        }
    }
  }
}
function Yt(r, Q) {
  let t = 0;
  for (const e of Q.checkpoints.keys())
    e <= r && e > t && (t = e);
  const B = Q.checkpoints.get(t), A = B ? new Float32Array(B) : new Float32Array(Q.resolution * Q.resolution).fill(Q.slabBounds.zTop);
  for (const e of Q.moves)
    e.lineIndex > t && e.lineIndex <= r && iB(e, A, Q.slabBounds, Q.toolRadius, Q.resolution);
  return A;
}
function St(r, Q, t, B) {
  if (t <= 0) return;
  const A = 1e-3, e = Q * Q, P = new Float32Array(e);
  for (let i = 0; i < t; i++) {
    P.set(r);
    for (let s = 0; s < Q; s++)
      for (let v = 0; v < Q; v++) {
        if (P[s * Q + v] < B - A) continue;
        let n = B;
        for (let T = -1; T <= 1; T++) {
          const z = s + T;
          if (!(z < 0 || z >= Q))
            for (let a = -1; a <= 1; a++) {
              if (a === 0 && T === 0) continue;
              const o = v + a;
              if (o < 0 || o >= Q) continue;
              const w = P[z * Q + o];
              w < n && (n = w);
            }
        }
        n < B - A && (r[s * Q + v] = n);
      }
  }
}
function ge(r, Q, t, B, A, e) {
  const P = Math.max(1, e);
  let i, s, v;
  B === "G18" ? (i = "Z", s = "X", v = "Y") : B === "G19" ? (i = "Y", s = "Z", v = "X") : (i = "X", s = "Y", v = "Z");
  const c = Math.hypot(r[i] - t[i], r[s] - t[s]), n = Math.atan2(r[s] - t[s], r[i] - t[i]), T = Math.atan2(Q[s] - t[s], Q[i] - t[i]);
  let z;
  A === "G2" ? (z = T - n, z > 0 && (z -= Math.PI * 2)) : (z = T - n, z < 0 && (z += Math.PI * 2));
  const a = [];
  let o = { ...r };
  for (let w = 1; w <= P; w++) {
    const E = w / P, l = n + z * E, u = E, U = t[i] + c * Math.cos(l), k = t[s] + c * Math.sin(l), O = r[v] + (Q[v] - r[v]) * u, D = { X: 0, Y: 0, Z: 0 };
    D[i] = U, D[s] = k, D[v] = O, a.push({
      x0: o.X,
      y0: o.Y,
      z0: o.Z,
      x1: D.X,
      y1: D.Y,
      z1: D.Z
    }), o = D;
  }
  return a;
}
let Nt = null;
function pe() {
  if (Nt) return Nt;
  const r = 512, Q = document.createElement("canvas");
  Q.width = r, Q.height = r;
  const t = Q.getContext("2d");
  t.fillStyle = "#8B5E3C", t.fillRect(0, 0, r, r);
  const B = Math.floor(40 + Math.random() * 20);
  for (let A = 0; A < B; A++) {
    const e = Math.random() * r, P = 1 + Math.random() * 2, i = Math.floor(139 + (Math.random() - 0.5) * 40), s = Math.floor(94 + (Math.random() - 0.5) * 30), v = Math.floor(60 + (Math.random() - 0.5) * 20);
    t.strokeStyle = `rgb(${i},${s},${v})`, t.lineWidth = P, t.beginPath(), t.moveTo(0, e), t.lineTo(r, e), t.stroke();
  }
  for (let A = 0; A < 300; A++) {
    const e = Math.random() * r, P = Math.random() * r, i = 20 + Math.random() * 60, s = (Math.random() - 0.5) * 0.3, v = Math.floor(100 + Math.random() * 80), c = Math.floor(60 + Math.random() * 50), n = Math.floor(30 + Math.random() * 40);
    t.strokeStyle = `rgba(${v},${c},${n},0.35)`, t.lineWidth = 0.5 + Math.random() * 1, t.beginPath(), t.moveTo(e, P), t.quadraticCurveTo(
      e + i * 0.5 * Math.cos(s),
      P + i * 0.5 * Math.sin(s) * 0.1,
      e + i * Math.cos(s * 0.5),
      P + i * 0.15
    ), t.stroke();
  }
  return Nt = new h.CanvasTexture(Q), Nt;
}
function me(r, Q) {
  const t = new h.Group();
  t.name = "gviewer:sim3d-slab";
  const B = r.xMax - r.xMin, A = r.yMax - r.yMin, e = r.zTop - r.zBot, P = (r.xMin + r.xMax) / 2, i = (r.yMin + r.yMax) / 2, s = (r.zBot + r.zTop) / 2, v = pe();
  v.wrapS = h.RepeatWrapping, v.wrapT = h.RepeatWrapping, v.repeat.set(B / 100, A / 100);
  const c = new h.MeshStandardMaterial({
    map: v,
    roughness: 0.85,
    metalness: 0,
    side: h.FrontSide
  }), n = new h.MeshStandardMaterial({
    color: 8016432,
    roughness: 0.9,
    metalness: 0,
    side: h.DoubleSide
  }), T = new h.MeshStandardMaterial({
    color: 7030822,
    roughness: 0.9,
    metalness: 0
  }), z = new h.PlaneGeometry(B, A, Q - 1, Q - 1), a = Q * Q, o = new Float32Array(a * 3), w = z.attributes.position;
  for (let b = 0; b < a; b++)
    o[b * 3] = w.getX(b), o[b * 3 + 1] = w.getY(b), o[b * 3 + 2] = 0;
  const E = new h.BufferAttribute(o, 3);
  E.usage = h.DynamicDrawUsage, z.setAttribute("position", E), z.computeVertexNormals();
  const l = new h.Mesh(z, c);
  l.name = "gviewer:sim3d-top", l.position.set(P, i, r.zTop), t.add(l);
  const u = new h.PlaneGeometry(B, e);
  u.rotateX(Math.PI / 2);
  const U = new h.Mesh(u, n);
  U.position.set(P, r.yMax, s), t.add(U);
  const k = new h.PlaneGeometry(B, e);
  k.rotateX(Math.PI / 2);
  const O = new h.Mesh(k, n);
  O.position.set(P, r.yMin, s), t.add(O);
  const D = new h.PlaneGeometry(e, A);
  D.rotateY(-Math.PI / 2);
  const N = new h.Mesh(D, n);
  N.position.set(r.xMax, i, s), t.add(N);
  const d = new h.PlaneGeometry(e, A);
  d.rotateY(-Math.PI / 2);
  const M = new h.Mesh(d, n);
  M.position.set(r.xMin, i, s), t.add(M);
  const G = new h.Mesh(new h.PlaneGeometry(B, A), T);
  return G.position.set(P, i, r.zBot), t.add(G), { group: t, topMesh: l, topPositionAttr: E, slabBounds: r, resolution: Q };
}
function Vt(r, Q) {
  const { topPositionAttr: t, slabBounds: B, resolution: A } = r, e = A * A;
  for (let P = 0; P < e; P++) {
    const i = Math.floor(P / A), s = P % A, v = (A - 1 - i) * A + s;
    t.setZ(P, Q[v] - B.zTop);
  }
  t.needsUpdate = !0, r.topMesh.geometry.computeVertexNormals();
}
function xe(r) {
  r.group.traverse((Q) => {
    if (Q instanceof h.Mesh) {
      Q.geometry.dispose();
      const t = Q.material;
      Array.isArray(t) ? t.forEach((B) => {
        B.map && B.map.dispose(), B.dispose();
      }) : (t.map && t.map.dispose(), t.dispose());
    }
  });
}
const ye = ".gViewer-viewcube{position:absolute;left:60px;bottom:60px;width:84px;height:84px;z-index:9999;perspective:600px;pointer-events:auto}.gViewer-viewcube__cube{position:absolute;top:0;right:0;bottom:0;left:0;transform-style:preserve-3d;transition:transform 60ms linear}.gViewer-viewcube__face{position:absolute;width:84px;height:84px;display:flex;align-items:center;justify-content:center;font:600 11px system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;letter-spacing:.06em;text-transform:uppercase;color:#f0f5fce0;background:#0c1014b3;border:1px solid rgba(120,140,160,.35);border-radius:14px;cursor:pointer;-webkit-user-select:none;user-select:none;padding:0}.gViewer-viewcube__face:hover{background:#0c1014db;border-color:#a0bedc8c}.gViewer-viewcube__face.is-active{border-color:#60a5faf2;box-shadow:0 0 0 1px #60a5fa59,0 10px 30px #00000059;color:#60a5faf2}.gViewer-viewcube__face--front{transform:translateZ(42px)}.gViewer-viewcube__face--back{transform:rotateY(180deg) translateZ(42px)}.gViewer-viewcube__face--left{transform:rotateY(-90deg) translateZ(42px)}.gViewer-viewcube__face--right{transform:rotateY(90deg) translateZ(42px)}.gViewer-viewcube__face--top{transform:rotateX(-90deg) translateZ(42px)}.gViewer-viewcube__face--bottom{transform:rotateX(90deg) translateZ(42px)}.gViewer-viewcube__corner{position:absolute;width:20px;height:20px;top:32px;left:32px;border-radius:50%;background:#0c101473;border:1px solid rgba(120,140,160,.3);cursor:pointer;padding:0;-webkit-user-select:none;user-select:none}.gViewer-viewcube__corner:hover{background:#0c1014bf;border-color:#a0bedca6}.gViewer-viewcube__corner.is-active{border-color:#60a5faf2;box-shadow:0 0 0 1px #60a5fa59}.gViewer-viewcube__corner--front-top-right{transform:rotateY(45deg) rotateX(-35.264deg) translateZ(72.746px)}.gViewer-viewcube__corner--front-top-left{transform:rotateY(-45deg) rotateX(-35.264deg) translateZ(72.746px)}.gViewer-viewcube__corner--front-bottom-right{transform:rotateY(45deg) rotateX(35.264deg) translateZ(72.746px)}.gViewer-viewcube__corner--front-bottom-left{transform:rotateY(-45deg) rotateX(35.264deg) translateZ(72.746px)}.gViewer-viewcube__corner--back-top-right{transform:rotateY(135deg) rotateX(-35.264deg) translateZ(72.746px)}.gViewer-viewcube__corner--back-top-left{transform:rotateY(-135deg) rotateX(-35.264deg) translateZ(72.746px)}.gViewer-viewcube__corner--back-bottom-right{transform:rotateY(135deg) rotateX(35.264deg) translateZ(72.746px)}.gViewer-viewcube__corner--back-bottom-left{transform:rotateY(-135deg) rotateX(35.264deg) translateZ(72.746px)}@media (orientation: portrait){.gViewer-viewcube{left:60px;bottom:110px;width:64px;height:64px}.gViewer-viewcube__face{width:64px;height:64px}.gViewer-viewcube__face--front{transform:translateZ(32px)}.gViewer-viewcube__face--back{transform:rotateY(180deg) translateZ(32px)}.gViewer-viewcube__face--left{transform:rotateY(-90deg) translateZ(32px)}.gViewer-viewcube__face--right{transform:rotateY(90deg) translateZ(32px)}.gViewer-viewcube__face--top{transform:rotateX(-90deg) translateZ(32px)}.gViewer-viewcube__face--bottom{transform:rotateX(90deg) translateZ(32px)}.gViewer-viewcube__corner{width:15px;height:15px;top:24.5px;left:24.5px}.gViewer-viewcube__corner--front-top-right{transform:rotateY(45deg) rotateX(-35.264deg) translateZ(55.426px)}.gViewer-viewcube__corner--front-top-left{transform:rotateY(-45deg) rotateX(-35.264deg) translateZ(55.426px)}.gViewer-viewcube__corner--front-bottom-right{transform:rotateY(45deg) rotateX(35.264deg) translateZ(55.426px)}.gViewer-viewcube__corner--front-bottom-left{transform:rotateY(-45deg) rotateX(35.264deg) translateZ(55.426px)}.gViewer-viewcube__corner--back-top-right{transform:rotateY(135deg) rotateX(-35.264deg) translateZ(55.426px)}.gViewer-viewcube__corner--back-top-left{transform:rotateY(-135deg) rotateX(-35.264deg) translateZ(55.426px)}.gViewer-viewcube__corner--back-bottom-right{transform:rotateY(135deg) rotateX(35.264deg) translateZ(55.426px)}.gViewer-viewcube__corner--back-bottom-left{transform:rotateY(-135deg) rotateX(35.264deg) translateZ(55.426px)}}", RB = "gviewer-viewcube-style";
function Ie() {
  if (typeof document > "u" || document.getElementById(RB)) return;
  const r = document.createElement("style");
  r.id = RB, r.setAttribute("data-gviewer", "viewcube"), r.textContent = ye, document.head.appendChild(r);
}
class Ye {
  constructor(Q) {
    Ie(), this.onSelectView = Q.onSelectView, this.faceButtons = /* @__PURE__ */ new Map(), this.root = document.createElement("div"), this.root.className = "gViewer-viewcube", this.cube = document.createElement("div"), this.cube.className = "gViewer-viewcube__cube", this.root.appendChild(this.cube);
    const t = [
      { view: "front", className: "gViewer-viewcube__face--front", label: "Front" },
      { view: "back", className: "gViewer-viewcube__face--back", label: "Back" },
      { view: "left", className: "gViewer-viewcube__face--left", label: "Left" },
      { view: "right", className: "gViewer-viewcube__face--right", label: "Right" },
      { view: "top", className: "gViewer-viewcube__face--top", label: "Top" },
      { view: "bottom", className: "gViewer-viewcube__face--bottom", label: "Bottom" }
    ];
    for (const A of t) {
      const e = document.createElement("button");
      e.type = "button", e.className = `gViewer-viewcube__face ${A.className}`, e.textContent = A.label, e.addEventListener("click", () => this.onSelectView(A.view)), this.cube.appendChild(e), this.faceButtons.set(A.view, e);
    }
    const B = [
      { view: "front-top-left", className: "gViewer-viewcube__corner--front-top-left", title: "Front Top Left" },
      { view: "front-top-right", className: "gViewer-viewcube__corner--front-top-right", title: "Front Top Right" },
      {
        view: "front-bottom-left",
        className: "gViewer-viewcube__corner--front-bottom-left",
        title: "Front Bottom Left"
      },
      {
        view: "front-bottom-right",
        className: "gViewer-viewcube__corner--front-bottom-right",
        title: "Front Bottom Right"
      },
      { view: "back-top-left", className: "gViewer-viewcube__corner--back-top-left", title: "Back Top Left" },
      { view: "back-top-right", className: "gViewer-viewcube__corner--back-top-right", title: "Back Top Right" },
      {
        view: "back-bottom-left",
        className: "gViewer-viewcube__corner--back-bottom-left",
        title: "Back Bottom Left"
      },
      {
        view: "back-bottom-right",
        className: "gViewer-viewcube__corner--back-bottom-right",
        title: "Back Bottom Right"
      }
    ];
    for (const A of B) {
      const e = document.createElement("button");
      e.type = "button", e.className = `gViewer-viewcube__corner ${A.className}`, e.title = A.title, e.setAttribute("aria-label", A.title), e.addEventListener("click", () => this.onSelectView(A.view)), this.cube.appendChild(e);
    }
    Q.container.appendChild(this.root);
  }
  setActiveFace(Q) {
    for (const [t, B] of this.faceButtons.entries())
      t === Q ? B.classList.add("is-active") : B.classList.remove("is-active");
  }
  setRotationMatrix3d(Q) {
    const t = Array.from({ length: 16 }, (B, A) => {
      const e = Q[A] ?? 0;
      return Number.isFinite(e) ? String(Math.abs(e) < 1e-12 ? 0 : Number(e.toFixed(8))) : "0";
    });
    this.cube.style.transform = `matrix3d(${t.join(",")})`;
  }
  dispose() {
    this.root.remove();
  }
}
function MQ(r, Q, t = {}) {
  const { opacity: B = 1, size: A = 10 } = t, e = new h.Object3D(), P = 100, i = document.createElement("canvas"), s = i.getContext("2d");
  if (!s)
    throw new Error("Missing canvas 2D context.");
  s.font = `normal ${P}px Arial`;
  const c = s.measureText(r).width;
  i.width = c, i.height = P, s.font = `normal ${P}px Arial`, s.textAlign = "center", s.textBaseline = "middle", s.fillStyle = Q, s.fillText(r, c / 2, P / 2);
  const n = new h.Texture(i);
  n.needsUpdate = !0, n.minFilter = h.LinearFilter;
  const T = new h.SpriteMaterial({
    map: n,
    transparent: !0,
    opacity: B
  });
  e.textHeight = A, e.textWidth = c / P * e.textHeight, t.textAlign === "left" ? e.position.x = (t.x ?? 0) + (e.textWidth ?? 0) / 2 : t.textAlign === "right" ? e.position.x = (t.x ?? 0) - (e.textWidth ?? 0) / 2 : e.position.x = t.x ?? 0, t.textBaseline === "top" ? e.position.y = (t.y ?? 0) - (e.textHeight ?? 0) / 2 : t.textBaseline === "bottom" ? e.position.y = (t.y ?? 0) + (e.textHeight ?? 0) / 2 : e.position.y = t.y ?? 0, e.position.z = t.z ?? 0;
  const z = new h.Sprite(T);
  return z.scale.set(c / P * A, A, 1), e.add(z), e;
}
function Se(r) {
  r.traverse((Q) => {
    var B;
    if (!(Q instanceof h.Sprite))
      return;
    const t = Q.material;
    (B = t.map) == null || B.dispose(), t.dispose();
  });
}
const Ve = 25.4;
function Je(r, Q) {
  const t = new h.Group(), B = new h.Box3Helper(
    r.clone(),
    new h.Color(Q.render.theme.colors.boundingBox)
  ), A = B.material;
  return A.transparent = !0, A.opacity = 0.65, A.depthWrite = !1, A.linewidth = 2, t.add(B), Q.boundingBox.labels && t.add(...Re(r.clone(), Q)), t;
}
function Re(r, Q) {
  const t = Q.render.theme.colors.boundingBox, B = 0.8, A = (u) => Math.abs(u) < 1e-9 ? "0" : u.toFixed(3), e = (u) => Q.units === "in" ? `${A(u / Ve)} in` : `${A(u)} mm`, P = r.min, i = r.max, s = (u) => MQ(u, t, { size: 4, opacity: B }), v = s(e(P.x)), c = s(e(i.x)), n = s(e(P.y)), T = s(e(i.y)), z = s(e(P.z)), a = s(e(i.z)), o = new h.Vector3();
  r.getSize(o);
  const w = Math.max(2, Math.max(o.x, o.y, o.z) * 0.02), E = (P.x + i.x) / 2, l = (P.y + i.y) / 2;
  return v.position.set(P.x - w, l, P.z - w), c.position.set(i.x + w, l, P.z - w), n.position.set(E, P.y - w, P.z - w), T.position.set(E, i.y + w, P.z - w), z.position.set(E, l, P.z - w), a.position.set(E, l, i.z + w), [v, c, n, T, z, a];
}
function Xe(r) {
  r.traverse((Q) => {
    var t;
    if (Q instanceof h.LineSegments || Q instanceof h.Line) {
      Q.geometry.dispose();
      const B = Q.material;
      Array.isArray(B) ? B.forEach((A) => A.dispose()) : B.dispose();
      return;
    }
    if (Q instanceof h.Sprite) {
      const B = Q.material;
      (t = B.map) == null || t.dispose(), B.dispose();
    }
  });
}
function XB(r, Q, t) {
  const B = new Float32Array([
    r.x,
    r.y,
    0,
    Q.x,
    r.y,
    0,
    Q.x,
    Q.y,
    0,
    r.x,
    Q.y,
    0,
    r.x,
    r.y,
    0
  ]), A = new h.BufferGeometry();
  A.setAttribute("position", new h.BufferAttribute(B, 3));
  const e = new h.LineBasicMaterial({
    color: new h.Color(t),
    transparent: !0,
    opacity: 0.35,
    depthWrite: !1
  });
  return new h.LineLoop(A, e);
}
function Ke(r, Q, t, B = 10) {
  const A = [], e = r.x - Q.y, P = Q.x - r.y;
  for (let v = e; v <= P; v += B) {
    const c = Math.max(r.x, r.y + v), n = Math.min(Q.x, Q.y + v);
    n > c && A.push(c, c - v, 0, n, n - v, 0);
  }
  const i = new h.BufferGeometry();
  i.setAttribute("position", new h.BufferAttribute(new Float32Array(A), 3));
  const s = new h.LineBasicMaterial({
    color: new h.Color(t),
    transparent: !0,
    opacity: 0.35,
    depthWrite: !1
  });
  return new h.LineSegments(i, s);
}
function Ze(r, Q, t, B) {
  const A = new h.Group();
  if (A.add(XB(r, Q, t.render.theme.colors.machineBed)), B) {
    const e = t.render.theme.colors.machineBedKeepout;
    A.add(XB(B.min, B.max, e)), A.add(Ke(B.min, B.max, e));
  }
  return A;
}
function We(r) {
  r.traverse((Q) => {
    if (Q instanceof h.LineSegments || Q instanceof h.Line) {
      Q.geometry.dispose();
      const t = Q.material;
      Array.isArray(t) ? t.forEach((B) => B.dispose()) : t.dispose();
    }
  });
}
class qe extends VB {
  _rotateUp(Q) {
    VB.prototype._rotateUp.call(this, -Q);
  }
}
function _e(r) {
  const Q = window.getComputedStyle(r);
  (Q.position === "static" || !Q.position) && (r.style.position = "relative"), r.style.overflow || (r.style.overflow = "hidden");
}
function $e(r) {
  if (r < 0.5)
    return 4 * r * r * r;
  const Q = -2 * r + 2;
  return 1 - Q * Q * Q / 2;
}
function KB(r) {
  const Q = new h.Vector3(), t = {
    right: new h.Vector3(1, 0, 0),
    left: new h.Vector3(-1, 0, 0),
    front: new h.Vector3(0, -1, 0),
    back: new h.Vector3(0, 1, 0),
    top: new h.Vector3(0, 0, 1),
    bottom: new h.Vector3(0, 0, -1)
  };
  switch (r) {
    case "front":
      Q.copy(t.front);
      break;
    case "back":
      Q.copy(t.back);
      break;
    case "left":
      Q.copy(t.left);
      break;
    case "right":
      Q.copy(t.right);
      break;
    case "top":
      Q.copy(t.top);
      break;
    case "bottom":
      Q.copy(t.bottom);
      break;
    case "front-top-left":
      Q.copy(t.front).add(t.top).add(t.left);
      break;
    case "front-top-right":
      Q.copy(t.front).add(t.top).add(t.right);
      break;
    case "front-bottom-left":
      Q.copy(t.front).add(t.bottom).add(t.left);
      break;
    case "front-bottom-right":
      Q.copy(t.front).add(t.bottom).add(t.right);
      break;
    case "back-top-left":
      Q.copy(t.back).add(t.top).add(t.left);
      break;
    case "back-top-right":
      Q.copy(t.back).add(t.top).add(t.right);
      break;
    case "back-bottom-left":
      Q.copy(t.back).add(t.bottom).add(t.left);
      break;
    case "back-bottom-right":
      Q.copy(t.back).add(t.bottom).add(t.right);
      break;
    default:
      Q.copy(t.front);
      break;
  }
  return Q.normalize();
}
function QP(r, Q) {
  const t = r.x - Q.x, B = r.y - Q.y, A = r.z - Q.z, e = Math.abs(t), P = Math.abs(B), i = Math.abs(A);
  return i >= e && i >= P ? A >= 0 ? "top" : "bottom" : e >= P ? t >= 0 ? "right" : "left" : B >= 0 ? "back" : "front";
}
const uA = 25.4;
function uB(r) {
  if (r.bounds)
    return {
      minX: r.bounds.min.x,
      maxX: r.bounds.max.x,
      minY: r.bounds.min.y,
      maxY: r.bounds.max.y
    };
  const Q = Math.max(1, r.sizeXMm) / 2, t = Math.max(1, r.sizeYMm) / 2;
  return { minX: -Q, maxX: Q, minY: -t, maxY: t };
}
function tP(r) {
  const { minX: Q, maxX: t, minY: B, maxY: A } = uB(r), e = r.units === "mm" ? 10 : uA, P = [], i = [], s = (a, o, w, E, l) => {
    l.push(a, o, 0, w, E, 0);
  }, v = Math.ceil(B / e) * e, c = Math.floor(A / e) * e;
  for (let a = v; a <= c + 1e-6; a += e)
    s(Q, a, t, a, Math.abs(a) < 1e-6 ? P : i);
  const n = Math.ceil(Q / e) * e, T = Math.floor(t / e) * e;
  for (let a = n; a <= T + 1e-6; a += e)
    s(a, B, a, A, Math.abs(a) < 1e-6 ? P : i);
  const z = new h.Group();
  return i.length > 0 && z.add(ZB(i, r.theme.colors.grid.minor, 0.25)), P.length > 0 && z.add(ZB(P, r.theme.colors.grid.major, 0.4)), z;
}
function ZB(r, Q, t) {
  const B = new h.BufferGeometry();
  B.setAttribute("position", new h.Float32BufferAttribute(r, 3));
  const A = new h.LineBasicMaterial({
    color: new h.Color(Q),
    transparent: !0,
    opacity: t,
    depthWrite: !1
  });
  return new h.LineSegments(B, A);
}
function BP(r) {
  r.traverse((Q) => {
    if (!(Q instanceof h.LineSegments))
      return;
    Q.geometry.dispose();
    const t = Q.material;
    Array.isArray(t) ? t.forEach((B) => B.dispose()) : t.dispose();
  });
}
function AP(r) {
  const Q = new h.Group(), { minX: t, maxX: B, minY: A, maxY: e } = uB({
    sizeXMm: r.sizeXWorld,
    sizeYMm: r.sizeYWorld,
    bounds: r.bounds
  }), P = Math.max(1, r.depthWorld), i = Math.max(1, Math.min(Math.max(B - t, e - A) / 24, 30)), s = i * 0.6, v = (E) => new h.LineDashedMaterial({
    color: new h.Color(E),
    dashSize: i,
    gapSize: s,
    transparent: !0,
    opacity: 0.75,
    depthWrite: !1
  }), c = new h.Line(
    new h.BufferGeometry().setFromPoints([
      new h.Vector3(t, 0, 0),
      new h.Vector3(B, 0, 0)
    ]),
    v(r.theme.colors.axes.x)
  );
  c.computeLineDistances();
  const n = new h.Line(
    new h.BufferGeometry().setFromPoints([
      new h.Vector3(0, A, 0),
      new h.Vector3(0, e, 0)
    ]),
    v(r.theme.colors.axes.y)
  );
  n.computeLineDistances();
  const T = new h.Line(
    new h.BufferGeometry().setFromPoints([
      new h.Vector3(0, 0, 0),
      new h.Vector3(0, 0, P)
    ]),
    v(r.theme.colors.axes.z)
  );
  T.computeLineDistances(), Q.add(c, n, T);
  const z = Math.max(2, i), a = MQ("X", r.theme.colors.axes.x, { size: 5 });
  a.position.set(B + z, 0, 0);
  const o = MQ("Y", r.theme.colors.axes.y, { size: 5 });
  o.position.set(0, e + z, 0);
  const w = MQ("Z", r.theme.colors.axes.z, { size: 5 });
  return w.position.set(0, 0, P + z), Q.add(a, o, w), Q;
}
function eP(r) {
  r.traverse((Q) => {
    var t;
    if (Q instanceof h.Line) {
      Q.geometry.dispose();
      const B = Q.material;
      Array.isArray(B) ? B.forEach((A) => A.dispose()) : B.dispose();
      return;
    }
    if (Q instanceof h.Sprite) {
      const B = Q.material;
      (t = B.map) == null || t.dispose(), B.dispose();
    }
  });
}
function PP(r) {
  const Q = new h.Group(), { minX: t, maxX: B, minY: A, maxY: e } = uB(r), P = r.units === "mm" ? 10 : 1, i = P * 2, s = r.units === "mm" ? 1 : uA, v = 0.5, c = 4, n = 0.1, T = 5, z = r.theme.colors.axes.x, a = r.theme.colors.axes.y, o = { opacity: v, size: c };
  for (let w = P; ; w += i) {
    const E = w * s;
    if (E > B + 1e-6)
      break;
    const l = MQ(String(w), z, o);
    l.position.set(E, T, n), Q.add(l);
  }
  for (let w = P; ; w += i) {
    const E = w * s;
    if (-E < t - 1e-6)
      break;
    const l = MQ(String(-w), z, o);
    l.position.set(-E, T, n), Q.add(l);
  }
  for (let w = P; ; w += i) {
    const E = w * s;
    if (E > e + 1e-6)
      break;
    const l = MQ(String(w), a, o);
    l.position.set(-T, E, n), Q.add(l);
  }
  for (let w = P; ; w += i) {
    const E = w * s;
    if (-E < A - 1e-6)
      break;
    const l = MQ(String(-w), a, o);
    l.position.set(-T, -E, n), Q.add(l);
  }
  return Q;
}
function dQ(r) {
  if (r === void 0)
    throw new ReferenceError("this hasn't been initialised - super() hasn't been called");
  return r;
}
function UA(r, Q) {
  r.prototype = Object.create(Q.prototype), r.prototype.constructor = r, r.__proto__ = Q;
}
/*!
 * GSAP 3.15.0
 * https://gsap.com
 *
 * @license Copyright 2008-2026, GreenSock. All rights reserved.
 * Subject to the terms at https://gsap.com/standard-license
 * @author: Jack Doyle, jack@greensock.com
*/
var zQ = {
  autoSleep: 120,
  force3D: "auto",
  nullTargetWarn: 1,
  units: {
    lineHeight: ""
  }
}, wt = {
  duration: 0.5,
  overwrite: !1,
  delay: 0
}, UB, q, y, hQ = 1e8, m = 1 / hQ, rB = Math.PI * 2, iP = rB / 4, rP = 0, DA = Math.sqrt, sP = Math.cos, vP = Math.sin, Z = function(Q) {
  return typeof Q == "string";
}, J = function(Q) {
  return typeof Q == "function";
}, NQ = function(Q) {
  return typeof Q == "number";
}, DB = function(Q) {
  return typeof Q > "u";
}, DQ = function(Q) {
  return typeof Q == "object";
}, AQ = function(Q) {
  return Q !== !1;
}, dB = function() {
  return typeof window < "u";
}, Ot = function(Q) {
  return J(Q) || Z(Q);
}, dA = typeof ArrayBuffer == "function" && ArrayBuffer.isView || function() {
}, $ = Array.isArray, cP = /random\([^)]+\)/g, nP = /,\s*/g, WB = /(?:-?\.?\d|\.)+/gi, CA = /[-+=.]*\d+[.e\-+]*\d*[e\-+]*\d*/g, qQ = /[-+=.]*\d+[.e-]*\d*[a-z%]*/g, Jt = /[-+=.]*\d+\.?\d*(?:e-|e\+)?\d*/gi, NA = /[+-]=-?[.\d]+/, oP = /[^,'"\[\]\s]+/gi, TP = /^[+\-=e\s\d]*\d+[.\d]*([a-z]*|%)\s*$/i, S, kQ, sB, CB, aQ = {}, ft = {}, OA, LA = function(Q) {
  return (ft = Bt(Q, aQ)) && rQ;
}, NB = function(Q, t) {
  return console.warn("Invalid property", Q, "set to", t, "Missing plugin? gsap.registerPlugin()");
}, Et = function(Q, t) {
  return !t && console.warn(Q);
}, MA = function(Q, t) {
  return Q && (aQ[Q] = t) && ft && (ft[Q] = t) || aQ;
}, ht = function() {
  return 0;
}, zP = {
  suppressEvents: !0,
  isStart: !0,
  kill: !1
}, Lt = {
  suppressEvents: !0,
  kill: !1
}, aP = {
  suppressEvents: !0
}, OB = {}, FQ = [], vB = {}, HA, cQ = {}, Rt = {}, qB = 30, Mt = [], LB = "", MB = function(Q) {
  var t = Q[0], B, A;
  if (DQ(t) || J(t) || (Q = [Q]), !(B = (t._gsap || {}).harness)) {
    for (A = Mt.length; A-- && !Mt[A].targetTest(t); )
      ;
    B = Mt[A];
  }
  for (A = Q.length; A--; )
    Q[A] && (Q[A]._gsap || (Q[A]._gsap = new _A(Q[A], B))) || Q.splice(A, 1);
  return Q;
}, SQ = function(Q) {
  return Q._gsap || MB(lQ(Q))[0]._gsap;
}, jA = function(Q, t, B) {
  return (B = Q[t]) && J(B) ? Q[t]() : DB(B) && Q.getAttribute && Q.getAttribute(t) || B;
}, eQ = function(Q, t) {
  return (Q = Q.split(",")).forEach(t) || Q;
}, X = function(Q) {
  return Math.round(Q * 1e5) / 1e5 || 0;
}, Y = function(Q) {
  return Math.round(Q * 1e7) / 1e7 || 0;
}, $Q = function(Q, t) {
  var B = t.charAt(0), A = parseFloat(t.substr(2));
  return Q = parseFloat(Q), B === "+" ? Q + A : B === "-" ? Q - A : B === "*" ? Q * A : Q / A;
}, wP = function(Q, t) {
  for (var B = t.length, A = 0; Q.indexOf(t[A]) < 0 && ++A < B; )
    ;
  return A < B;
}, Ft = function() {
  var Q = FQ.length, t = FQ.slice(0), B, A;
  for (vB = {}, FQ.length = 0, B = 0; B < Q; B++)
    A = t[B], A && A._lazy && (A.render(A._lazy[0], A._lazy[1], !0)._lazy = 0);
}, HB = function(Q) {
  return !!(Q._initted || Q._startAt || Q.add);
}, fA = function(Q, t, B, A) {
  FQ.length && !q && Ft(), Q.render(t, B, !!(q && t < 0 && HB(Q))), FQ.length && !q && Ft();
}, FA = function(Q) {
  var t = parseFloat(Q);
  return (t || t === 0) && (Q + "").match(oP).length < 2 ? t : Z(Q) ? Q.trim() : Q;
}, bA = function(Q) {
  return Q;
}, wQ = function(Q, t) {
  for (var B in t)
    B in Q || (Q[B] = t[B]);
  return Q;
}, EP = function(Q) {
  return function(t, B) {
    for (var A in B)
      A in t || A === "duration" && Q || A === "ease" || (t[A] = B[A]);
  };
}, Bt = function(Q, t) {
  for (var B in t)
    Q[B] = t[B];
  return Q;
}, _B = function r(Q, t) {
  for (var B in t)
    B !== "__proto__" && B !== "constructor" && B !== "prototype" && (Q[B] = DQ(t[B]) ? r(Q[B] || (Q[B] = {}), t[B]) : t[B]);
  return Q;
}, bt = function(Q, t) {
  var B = {}, A;
  for (A in Q)
    A in t || (B[A] = Q[A]);
  return B;
}, Tt = function(Q) {
  var t = Q.parent || S, B = Q.keyframes ? EP($(Q.keyframes)) : wQ;
  if (AQ(Q.inherit))
    for (; t; )
      B(Q, t.vars.defaults), t = t.parent || t._dp;
  return Q;
}, hP = function(Q, t) {
  for (var B = Q.length, A = B === t.length; A && B-- && Q[B] === t[B]; )
    ;
  return B < 0;
}, GA = function(Q, t, B, A, e) {
  var P = Q[A], i;
  if (e)
    for (i = t[e]; P && P[e] > i; )
      P = P._prev;
  return P ? (t._next = P._next, P._next = t) : (t._next = Q[B], Q[B] = t), t._next ? t._next._prev = t : Q[A] = t, t._prev = P, t.parent = t._dp = Q, t;
}, xt = function(Q, t, B, A) {
  B === void 0 && (B = "_first"), A === void 0 && (A = "_last");
  var e = t._prev, P = t._next;
  e ? e._next = P : Q[B] === t && (Q[B] = P), P ? P._prev = e : Q[A] === t && (Q[A] = e), t._next = t._prev = t.parent = null;
}, GQ = function(Q, t) {
  Q.parent && (!t || Q.parent.autoRemoveChildren) && Q.parent.remove && Q.parent.remove(Q), Q._act = 0;
}, VQ = function(Q, t) {
  if (Q && (!t || t._end > Q._dur || t._start < 0))
    for (var B = Q; B; )
      B._dirty = 1, B = B.parent;
  return Q;
}, lP = function(Q) {
  for (var t = Q.parent; t && t.parent; )
    t._dirty = 1, t.totalDuration(), t = t.parent;
  return Q;
}, cB = function(Q, t, B, A) {
  return Q._startAt && (q ? Q._startAt.revert(Lt) : Q.vars.immediateRender && !Q.vars.autoRevert || Q._startAt.render(t, !0, A));
}, kP = function r(Q) {
  return !Q || Q._ts && r(Q.parent);
}, $B = function(Q) {
  return Q._repeat ? At(Q._tTime, Q = Q.duration() + Q._rDelay) * Q : 0;
}, At = function(Q, t) {
  var B = Math.floor(Q = Y(Q / t));
  return Q && B === Q ? B - 1 : B;
}, Gt = function(Q, t) {
  return (Q - t._start) * t._ts + (t._ts >= 0 ? 0 : t._dirty ? t.totalDuration() : t._tDur);
}, yt = function(Q) {
  return Q._end = Y(Q._start + (Q._tDur / Math.abs(Q._ts || Q._rts || m) || 0));
}, It = function(Q, t) {
  var B = Q._dp;
  return B && B.smoothChildTiming && Q._ts && (Q._start = Y(B._time - (Q._ts > 0 ? t / Q._ts : ((Q._dirty ? Q.totalDuration() : Q._tDur) - t) / -Q._ts)), yt(Q), B._dirty || VQ(B, Q)), Q;
}, gA = function(Q, t) {
  var B;
  if ((t._time || !t._dur && t._initted || t._start < Q._time && (t._dur || !t.add)) && (B = Gt(Q.rawTime(), t), (!t._dur || dt(0, t.totalDuration(), B) - t._tTime > m) && t.render(B, !0)), VQ(Q, t)._dp && Q._initted && Q._time >= Q._dur && Q._ts) {
    if (Q._dur < Q.duration())
      for (B = Q; B._dp; )
        B.rawTime() >= 0 && B.totalTime(B._tTime), B = B._dp;
    Q._zTime = -m;
  }
}, uQ = function(Q, t, B, A) {
  return t.parent && GQ(t), t._start = Y((NQ(B) ? B : B || Q !== S ? EQ(Q, B, t) : Q._time) + t._delay), t._end = Y(t._start + (t.totalDuration() / Math.abs(t.timeScale()) || 0)), GA(Q, t, "_first", "_last", Q._sort ? "_start" : 0), nB(t) || (Q._recent = t), A || gA(Q, t), Q._ts < 0 && It(Q, Q._tTime), Q;
}, pA = function(Q, t) {
  return (aQ.ScrollTrigger || NB("scrollTrigger", t)) && aQ.ScrollTrigger.create(t, Q);
}, mA = function(Q, t, B, A, e) {
  if (fB(Q, t, e), !Q._initted)
    return 1;
  if (!B && Q._pt && !q && (Q._dur && Q.vars.lazy !== !1 || !Q._dur && Q.vars.lazy) && HA !== nQ.frame)
    return FQ.push(Q), Q._lazy = [e, A], 1;
}, uP = function r(Q) {
  var t = Q.parent;
  return t && t._ts && t._initted && !t._lock && (t.rawTime() < 0 || r(t));
}, nB = function(Q) {
  var t = Q.data;
  return t === "isFromStart" || t === "isStart";
}, UP = function(Q, t, B, A) {
  var e = Q.ratio, P = t < 0 || !t && (!Q._start && uP(Q) && !(!Q._initted && nB(Q)) || (Q._ts < 0 || Q._dp._ts < 0) && !nB(Q)) ? 0 : 1, i = Q._rDelay, s = 0, v, c, n;
  if (i && Q._repeat && (s = dt(0, Q._tDur, t), c = At(s, i), Q._yoyo && c & 1 && (P = 1 - P), c !== At(Q._tTime, i) && (e = 1 - P, Q.vars.repeatRefresh && Q._initted && Q.invalidate())), P !== e || q || A || Q._zTime === m || !t && Q._zTime) {
    if (!Q._initted && mA(Q, t, A, B, s))
      return;
    for (n = Q._zTime, Q._zTime = t || (B ? m : 0), B || (B = t && !n), Q.ratio = P, Q._from && (P = 1 - P), Q._time = 0, Q._tTime = s, v = Q._pt; v; )
      v.r(P, v.d), v = v._next;
    t < 0 && cB(Q, t, B, !0), Q._onUpdate && !B && oQ(Q, "onUpdate"), s && Q._repeat && !B && Q.parent && oQ(Q, "onRepeat"), (t >= Q._tDur || t < 0) && Q.ratio === P && (P && GQ(Q, 1), !B && !q && (oQ(Q, P ? "onComplete" : "onReverseComplete", !0), Q._prom && Q._prom()));
  } else Q._zTime || (Q._zTime = t);
}, DP = function(Q, t, B) {
  var A;
  if (B > t)
    for (A = Q._first; A && A._start <= B; ) {
      if (A.data === "isPause" && A._start > t)
        return A;
      A = A._next;
    }
  else
    for (A = Q._last; A && A._start >= B; ) {
      if (A.data === "isPause" && A._start < t)
        return A;
      A = A._prev;
    }
}, et = function(Q, t, B, A) {
  var e = Q._repeat, P = Y(t) || 0, i = Q._tTime / Q._tDur;
  return i && !A && (Q._time *= P / Q._dur), Q._dur = P, Q._tDur = e ? e < 0 ? 1e10 : Y(P * (e + 1) + Q._rDelay * e) : P, i > 0 && !A && It(Q, Q._tTime = Q._tDur * i), Q.parent && yt(Q), B || VQ(Q.parent, Q), Q;
}, QA = function(Q) {
  return Q instanceof BQ ? VQ(Q) : et(Q, Q._dur);
}, dP = {
  _start: 0,
  endTime: ht,
  totalDuration: ht
}, EQ = function r(Q, t, B) {
  var A = Q.labels, e = Q._recent || dP, P = Q.duration() >= hQ ? e.endTime(!1) : Q._dur, i, s, v;
  return Z(t) && (isNaN(t) || t in A) ? (s = t.charAt(0), v = t.substr(-1) === "%", i = t.indexOf("="), s === "<" || s === ">" ? (i >= 0 && (t = t.replace(/=/, "")), (s === "<" ? e._start : e.endTime(e._repeat >= 0)) + (parseFloat(t.substr(1)) || 0) * (v ? (i < 0 ? e : B).totalDuration() / 100 : 1)) : i < 0 ? (t in A || (A[t] = P), A[t]) : (s = parseFloat(t.charAt(i - 1) + t.substr(i + 1)), v && B && (s = s / 100 * ($(B) ? B[0] : B).totalDuration()), i > 1 ? r(Q, t.substr(0, i - 1), B) + s : P + s)) : t == null ? P : +t;
}, zt = function(Q, t, B) {
  var A = NQ(t[1]), e = (A ? 2 : 1) + (Q < 2 ? 0 : 1), P = t[e], i, s;
  if (A && (P.duration = t[1]), P.parent = B, Q) {
    for (i = P, s = B; s && !("immediateRender" in i); )
      i = s.vars.defaults || {}, s = AQ(s.vars.inherit) && s.parent;
    P.immediateRender = AQ(i.immediateRender), Q < 2 ? P.runBackwards = 1 : P.startAt = t[e - 1];
  }
  return new K(t[0], P, t[e + 1]);
}, mQ = function(Q, t) {
  return Q || Q === 0 ? t(Q) : t;
}, dt = function(Q, t, B) {
  return B < Q ? Q : B > t ? t : B;
}, _ = function(Q, t) {
  return !Z(Q) || !(t = TP.exec(Q)) ? "" : t[1];
}, CP = function(Q, t, B) {
  return mQ(B, function(A) {
    return dt(Q, t, A);
  });
}, oB = [].slice, xA = function(Q, t) {
  return Q && DQ(Q) && "length" in Q && (!t && !Q.length || Q.length - 1 in Q && DQ(Q[0])) && !Q.nodeType && Q !== kQ;
}, NP = function(Q, t, B) {
  return B === void 0 && (B = []), Q.forEach(function(A) {
    var e;
    return Z(A) && !t || xA(A, 1) ? (e = B).push.apply(e, lQ(A)) : B.push(A);
  }) || B;
}, lQ = function(Q, t, B) {
  return y && !t && y.selector ? y.selector(Q) : Z(Q) && !B && (sB || !Pt()) ? oB.call((t || CB).querySelectorAll(Q), 0) : $(Q) ? NP(Q, B) : xA(Q) ? oB.call(Q, 0) : Q ? [Q] : [];
}, TB = function(Q) {
  return Q = lQ(Q)[0] || Et("Invalid scope") || {}, function(t) {
    var B = Q.current || Q.nativeElement || Q;
    return lQ(t, B.querySelectorAll ? B : B === Q ? Et("Invalid scope") || CB.createElement("div") : Q);
  };
}, yA = function(Q) {
  return Q.sort(function() {
    return 0.5 - Math.random();
  });
}, IA = function(Q) {
  if (J(Q))
    return Q;
  var t = DQ(Q) ? Q : {
    each: Q
  }, B = JQ(t.ease), A = t.from || 0, e = parseFloat(t.base) || 0, P = {}, i = A > 0 && A < 1, s = isNaN(A) || i, v = t.axis, c = A, n = A;
  return Z(A) ? c = n = {
    center: 0.5,
    edges: 0.5,
    end: 1
  }[A] || 0 : !i && s && (c = A[0], n = A[1]), function(T, z, a) {
    var o = (a || t).length, w = P[o], E, l, u, U, k, O, D, N, d;
    if (!w) {
      if (d = t.grid === "auto" ? 0 : (t.grid || [1, hQ])[1], !d) {
        for (D = -hQ; D < (D = a[d++].getBoundingClientRect().left) && d < o; )
          ;
        d < o && d--;
      }
      for (w = P[o] = [], E = s ? Math.min(d, o) * c - 0.5 : A % d, l = d === hQ ? 0 : s ? o * n / d - 0.5 : A / d | 0, D = 0, N = hQ, O = 0; O < o; O++)
        u = O % d - E, U = l - (O / d | 0), w[O] = k = v ? Math.abs(v === "y" ? U : u) : DA(u * u + U * U), k > D && (D = k), k < N && (N = k);
      A === "random" && yA(w), w.max = D - N, w.min = N, w.v = o = (parseFloat(t.amount) || parseFloat(t.each) * (d > o ? o - 1 : v ? v === "y" ? o / d : d : Math.max(d, o / d)) || 0) * (A === "edges" ? -1 : 1), w.b = o < 0 ? e - o : e, w.u = _(t.amount || t.each) || 0, B = B && o < 0 ? xP(B) : B;
    }
    return o = (w[T] - w.min) / w.max || 0, Y(w.b + (B ? B(o) : o) * w.v) + w.u;
  };
}, zB = function(Q) {
  var t = Math.pow(10, ((Q + "").split(".")[1] || "").length);
  return function(B) {
    var A = Y(Math.round(parseFloat(B) / Q) * Q * t);
    return (A - A % 1) / t + (NQ(B) ? 0 : _(B));
  };
}, YA = function(Q, t) {
  var B = $(Q), A, e;
  return !B && DQ(Q) && (A = B = Q.radius || hQ, Q.values ? (Q = lQ(Q.values), (e = !NQ(Q[0])) && (A *= A)) : Q = zB(Q.increment)), mQ(t, B ? J(Q) ? function(P) {
    return e = Q(P), Math.abs(e - P) <= A ? e : P;
  } : function(P) {
    for (var i = parseFloat(e ? P.x : P), s = parseFloat(e ? P.y : 0), v = hQ, c = 0, n = Q.length, T, z; n--; )
      e ? (T = Q[n].x - i, z = Q[n].y - s, T = T * T + z * z) : T = Math.abs(Q[n] - i), T < v && (v = T, c = n);
    return c = !A || v <= A ? Q[c] : P, e || c === P || NQ(P) ? c : c + _(P);
  } : zB(Q));
}, SA = function(Q, t, B, A) {
  return mQ($(Q) ? !t : B === !0 ? !!(B = 0) : !A, function() {
    return $(Q) ? Q[~~(Math.random() * Q.length)] : (B = B || 1e-5) && (A = B < 1 ? Math.pow(10, (B + "").length - 2) : 1) && Math.floor(Math.round((Q - B / 2 + Math.random() * (t - Q + B * 0.99)) / B) * B * A) / A;
  });
}, OP = function() {
  for (var Q = arguments.length, t = new Array(Q), B = 0; B < Q; B++)
    t[B] = arguments[B];
  return function(A) {
    return t.reduce(function(e, P) {
      return P(e);
    }, A);
  };
}, LP = function(Q, t) {
  return function(B) {
    return Q(parseFloat(B)) + (t || _(B));
  };
}, MP = function(Q, t, B) {
  return JA(Q, t, 0, 1, B);
}, VA = function(Q, t, B) {
  return mQ(B, function(A) {
    return Q[~~t(A)];
  });
}, HP = function r(Q, t, B) {
  var A = t - Q;
  return $(Q) ? VA(Q, r(0, Q.length), t) : mQ(B, function(e) {
    return (A + (e - Q) % A) % A + Q;
  });
}, jP = function r(Q, t, B) {
  var A = t - Q, e = A * 2;
  return $(Q) ? VA(Q, r(0, Q.length - 1), t) : mQ(B, function(P) {
    return P = (e + (P - Q) % e) % e || 0, Q + (P > A ? e - P : P);
  });
}, lt = function(Q) {
  return Q.replace(cP, function(t) {
    var B = t.indexOf("[") + 1, A = t.substring(B || 7, B ? t.indexOf("]") : t.length - 1).split(nP);
    return SA(B ? A : +A[0], B ? 0 : +A[1], +A[2] || 1e-5);
  });
}, JA = function(Q, t, B, A, e) {
  var P = t - Q, i = A - B;
  return mQ(e, function(s) {
    return B + ((s - Q) / P * i || 0);
  });
}, fP = function r(Q, t, B, A) {
  var e = isNaN(Q + t) ? 0 : function(z) {
    return (1 - z) * Q + z * t;
  };
  if (!e) {
    var P = Z(Q), i = {}, s, v, c, n, T;
    if (B === !0 && (A = 1) && (B = null), P)
      Q = {
        p: Q
      }, t = {
        p: t
      };
    else if ($(Q) && !$(t)) {
      for (c = [], n = Q.length, T = n - 2, v = 1; v < n; v++)
        c.push(r(Q[v - 1], Q[v]));
      n--, e = function(a) {
        a *= n;
        var o = Math.min(T, ~~a);
        return c[o](a - o);
      }, B = t;
    } else A || (Q = Bt($(Q) ? [] : {}, Q));
    if (!c) {
      for (s in t)
        jB.call(i, Q, s, "get", t[s]);
      e = function(a) {
        return GB(a, i) || (P ? Q.p : Q);
      };
    }
  }
  return mQ(B, e);
}, tA = function(Q, t, B) {
  var A = Q.labels, e = hQ, P, i, s;
  for (P in A)
    i = A[P] - t, i < 0 == !!B && i && e > (i = Math.abs(i)) && (s = P, e = i);
  return s;
}, oQ = function(Q, t, B) {
  var A = Q.vars, e = A[t], P = y, i = Q._ctx, s, v, c;
  if (e)
    return s = A[t + "Params"], v = A.callbackScope || Q, B && FQ.length && Ft(), i && (y = i), c = s ? e.apply(v, s) : e.call(v), y = P, c;
}, nt = function(Q) {
  return GQ(Q), Q.scrollTrigger && Q.scrollTrigger.kill(!!q), Q.progress() < 1 && oQ(Q, "onInterrupt"), Q;
}, _Q, RA = [], XA = function(Q) {
  if (Q)
    if (Q = !Q.name && Q.default || Q, dB() || Q.headless) {
      var t = Q.name, B = J(Q), A = t && !B && Q.init ? function() {
        this._props = [];
      } : Q, e = {
        init: ht,
        render: GB,
        add: jB,
        kill: ZP,
        modifier: KP,
        rawVars: 0
      }, P = {
        targetTest: 0,
        get: 0,
        getSetter: bB,
        aliases: {},
        register: 0
      };
      if (Pt(), Q !== A) {
        if (cQ[t])
          return;
        wQ(A, wQ(bt(Q, e), P)), Bt(A.prototype, Bt(e, bt(Q, P))), cQ[A.prop = t] = A, Q.targetTest && (Mt.push(A), OB[t] = 1), t = (t === "css" ? "CSS" : t.charAt(0).toUpperCase() + t.substr(1)) + "Plugin";
      }
      MA(t, A), Q.register && Q.register(rQ, A, PQ);
    } else
      RA.push(Q);
}, p = 255, ot = {
  aqua: [0, p, p],
  lime: [0, p, 0],
  silver: [192, 192, 192],
  black: [0, 0, 0],
  maroon: [128, 0, 0],
  teal: [0, 128, 128],
  blue: [0, 0, p],
  navy: [0, 0, 128],
  white: [p, p, p],
  olive: [128, 128, 0],
  yellow: [p, p, 0],
  orange: [p, 165, 0],
  gray: [128, 128, 128],
  purple: [128, 0, 128],
  green: [0, 128, 0],
  red: [p, 0, 0],
  pink: [p, 192, 203],
  cyan: [0, p, p],
  transparent: [p, p, p, 0]
}, Xt = function(Q, t, B) {
  return Q += Q < 0 ? 1 : Q > 1 ? -1 : 0, (Q * 6 < 1 ? t + (B - t) * Q * 6 : Q < 0.5 ? B : Q * 3 < 2 ? t + (B - t) * (2 / 3 - Q) * 6 : t) * p + 0.5 | 0;
}, KA = function(Q, t, B) {
  var A = Q ? NQ(Q) ? [Q >> 16, Q >> 8 & p, Q & p] : 0 : ot.black, e, P, i, s, v, c, n, T, z, a;
  if (!A) {
    if (Q.substr(-1) === "," && (Q = Q.substr(0, Q.length - 1)), ot[Q])
      A = ot[Q];
    else if (Q.charAt(0) === "#") {
      if (Q.length < 6 && (e = Q.charAt(1), P = Q.charAt(2), i = Q.charAt(3), Q = "#" + e + e + P + P + i + i + (Q.length === 5 ? Q.charAt(4) + Q.charAt(4) : "")), Q.length === 9)
        return A = parseInt(Q.substr(1, 6), 16), [A >> 16, A >> 8 & p, A & p, parseInt(Q.substr(7), 16) / 255];
      Q = parseInt(Q.substr(1), 16), A = [Q >> 16, Q >> 8 & p, Q & p];
    } else if (Q.substr(0, 3) === "hsl") {
      if (A = a = Q.match(WB), !t)
        s = +A[0] % 360 / 360, v = +A[1] / 100, c = +A[2] / 100, P = c <= 0.5 ? c * (v + 1) : c + v - c * v, e = c * 2 - P, A.length > 3 && (A[3] *= 1), A[0] = Xt(s + 1 / 3, e, P), A[1] = Xt(s, e, P), A[2] = Xt(s - 1 / 3, e, P);
      else if (~Q.indexOf("="))
        return A = Q.match(CA), B && A.length < 4 && (A[3] = 1), A;
    } else
      A = Q.match(WB) || ot.transparent;
    A = A.map(Number);
  }
  return t && !a && (e = A[0] / p, P = A[1] / p, i = A[2] / p, n = Math.max(e, P, i), T = Math.min(e, P, i), c = (n + T) / 2, n === T ? s = v = 0 : (z = n - T, v = c > 0.5 ? z / (2 - n - T) : z / (n + T), s = n === e ? (P - i) / z + (P < i ? 6 : 0) : n === P ? (i - e) / z + 2 : (e - P) / z + 4, s *= 60), A[0] = ~~(s + 0.5), A[1] = ~~(v * 100 + 0.5), A[2] = ~~(c * 100 + 0.5)), B && A.length < 4 && (A[3] = 1), A;
}, ZA = function(Q) {
  var t = [], B = [], A = -1;
  return Q.split(bQ).forEach(function(e) {
    var P = e.match(qQ) || [];
    t.push.apply(t, P), B.push(A += P.length + 1);
  }), t.c = B, t;
}, BA = function(Q, t, B) {
  var A = "", e = (Q + A).match(bQ), P = t ? "hsla(" : "rgba(", i = 0, s, v, c, n;
  if (!e)
    return Q;
  if (e = e.map(function(T) {
    return (T = KA(T, t, 1)) && P + (t ? T[0] + "," + T[1] + "%," + T[2] + "%," + T[3] : T.join(",")) + ")";
  }), B && (c = ZA(Q), s = B.c, s.join(A) !== c.c.join(A)))
    for (v = Q.replace(bQ, "1").split(qQ), n = v.length - 1; i < n; i++)
      A += v[i] + (~s.indexOf(i) ? e.shift() || P + "0,0,0,0)" : (c.length ? c : e.length ? e : B).shift());
  if (!v)
    for (v = Q.split(bQ), n = v.length - 1; i < n; i++)
      A += v[i] + e[i];
  return A + v[n];
}, bQ = function() {
  var r = "(?:\\b(?:(?:rgb|rgba|hsl|hsla)\\(.+?\\))|\\B#(?:[0-9a-f]{3,4}){1,2}\\b", Q;
  for (Q in ot)
    r += "|" + Q + "\\b";
  return new RegExp(r + ")", "gi");
}(), FP = /hsl[a]?\(/, WA = function(Q) {
  var t = Q.join(" "), B;
  if (bQ.lastIndex = 0, bQ.test(t))
    return B = FP.test(t), Q[1] = BA(Q[1], B), Q[0] = BA(Q[0], B, ZA(Q[1])), !0;
}, kt, nQ = function() {
  var r = Date.now, Q = 500, t = 33, B = r(), A = B, e = 1e3 / 240, P = e, i = [], s, v, c, n, T, z, a = function o(w) {
    var E = r() - A, l = w === !0, u, U, k, O;
    if ((E > Q || E < 0) && (B += E - t), A += E, k = A - B, u = k - P, (u > 0 || l) && (O = ++n.frame, T = k - n.time * 1e3, n.time = k = k / 1e3, P += u + (u >= e ? 4 : e - u), U = 1), l || (s = v(o)), U)
      for (z = 0; z < i.length; z++)
        i[z](k, T, O, w);
  };
  return n = {
    time: 0,
    frame: 0,
    tick: function() {
      a(!0);
    },
    deltaRatio: function(w) {
      return T / (1e3 / (w || 60));
    },
    wake: function() {
      OA && (!sB && dB() && (kQ = sB = window, CB = kQ.document || {}, aQ.gsap = rQ, (kQ.gsapVersions || (kQ.gsapVersions = [])).push(rQ.version), LA(ft || kQ.GreenSockGlobals || !kQ.gsap && kQ || {}), RA.forEach(XA)), c = typeof requestAnimationFrame < "u" && requestAnimationFrame, s && n.sleep(), v = c || function(w) {
        return setTimeout(w, P - n.time * 1e3 + 1 | 0);
      }, kt = 1, a(2));
    },
    sleep: function() {
      (c ? cancelAnimationFrame : clearTimeout)(s), kt = 0, v = ht;
    },
    lagSmoothing: function(w, E) {
      Q = w || 1 / 0, t = Math.min(E || 33, Q);
    },
    fps: function(w) {
      e = 1e3 / (w || 240), P = n.time * 1e3 + e;
    },
    add: function(w, E, l) {
      var u = E ? function(U, k, O, D) {
        w(U, k, O, D), n.remove(u);
      } : w;
      return n.remove(w), i[l ? "unshift" : "push"](u), Pt(), u;
    },
    remove: function(w, E) {
      ~(E = i.indexOf(w)) && i.splice(E, 1) && z >= E && z--;
    },
    _listeners: i
  }, n;
}(), Pt = function() {
  return !kt && nQ.wake();
}, g = {}, bP = /^[\d.\-M][\d.\-,\s]/, GP = /["']/g, gP = function(Q) {
  for (var t = {}, B = Q.substr(1, Q.length - 3).split(":"), A = B[0], e = 1, P = B.length, i, s, v; e < P; e++)
    s = B[e], i = e !== P - 1 ? s.lastIndexOf(",") : s.length, v = s.substr(0, i), t[A] = isNaN(v) ? v.replace(GP, "").trim() : +v, A = s.substr(i + 1).trim();
  return t;
}, pP = function(Q) {
  var t = Q.indexOf("(") + 1, B = Q.indexOf(")"), A = Q.indexOf("(", t);
  return Q.substring(t, ~A && A < B ? Q.indexOf(")", B + 1) : B);
}, mP = function(Q) {
  var t = (Q + "").split("("), B = g[t[0]];
  return B && t.length > 1 && B.config ? B.config.apply(null, ~Q.indexOf("{") ? [gP(t[1])] : pP(Q).split(",").map(FA)) : g._CE && bP.test(Q) ? g._CE("", Q) : B;
}, xP = function(Q) {
  return function(t) {
    return 1 - Q(1 - t);
  };
}, JQ = function(Q, t) {
  return Q && (J(Q) ? Q : g[Q] || mP(Q)) || t;
}, XQ = function(Q, t, B, A) {
  B === void 0 && (B = function(s) {
    return 1 - t(1 - s);
  }), A === void 0 && (A = function(s) {
    return s < 0.5 ? t(s * 2) / 2 : 1 - t((1 - s) * 2) / 2;
  });
  var e = {
    easeIn: t,
    easeOut: B,
    easeInOut: A
  }, P;
  return eQ(Q, function(i) {
    g[i] = aQ[i] = e, g[P = i.toLowerCase()] = B;
    for (var s in e)
      g[P + (s === "easeIn" ? ".in" : s === "easeOut" ? ".out" : ".inOut")] = g[i + "." + s] = e[s];
  }), e;
}, qA = function(Q) {
  return function(t) {
    return t < 0.5 ? (1 - Q(1 - t * 2)) / 2 : 0.5 + Q((t - 0.5) * 2) / 2;
  };
}, Kt = function r(Q, t, B) {
  var A = t >= 1 ? t : 1, e = (B || (Q ? 0.3 : 0.45)) / (t < 1 ? t : 1), P = e / rB * (Math.asin(1 / A) || 0), i = function(c) {
    return c === 1 ? 1 : A * Math.pow(2, -10 * c) * vP((c - P) * e) + 1;
  }, s = Q === "out" ? i : Q === "in" ? function(v) {
    return 1 - i(1 - v);
  } : qA(i);
  return e = rB / e, s.config = function(v, c) {
    return r(Q, v, c);
  }, s;
}, Zt = function r(Q, t) {
  t === void 0 && (t = 1.70158);
  var B = function(P) {
    return P ? --P * P * ((t + 1) * P + t) + 1 : 0;
  }, A = Q === "out" ? B : Q === "in" ? function(e) {
    return 1 - B(1 - e);
  } : qA(B);
  return A.config = function(e) {
    return r(Q, e);
  }, A;
};
eQ("Linear,Quad,Cubic,Quart,Quint,Strong", function(r, Q) {
  var t = Q < 5 ? Q + 1 : Q;
  XQ(r + ",Power" + (t - 1), Q ? function(B) {
    return Math.pow(B, t);
  } : function(B) {
    return B;
  }, function(B) {
    return 1 - Math.pow(1 - B, t);
  }, function(B) {
    return B < 0.5 ? Math.pow(B * 2, t) / 2 : 1 - Math.pow((1 - B) * 2, t) / 2;
  });
});
g.Linear.easeNone = g.none = g.Linear.easeIn;
XQ("Elastic", Kt("in"), Kt("out"), Kt());
(function(r, Q) {
  var t = 1 / Q, B = 2 * t, A = 2.5 * t, e = function(i) {
    return i < t ? r * i * i : i < B ? r * Math.pow(i - 1.5 / Q, 2) + 0.75 : i < A ? r * (i -= 2.25 / Q) * i + 0.9375 : r * Math.pow(i - 2.625 / Q, 2) + 0.984375;
  };
  XQ("Bounce", function(P) {
    return 1 - e(1 - P);
  }, e);
})(7.5625, 2.75);
XQ("Expo", function(r) {
  return Math.pow(2, 10 * (r - 1)) * r + r * r * r * r * r * r * (1 - r);
});
XQ("Circ", function(r) {
  return -(DA(1 - r * r) - 1);
});
XQ("Sine", function(r) {
  return r === 1 ? 1 : -sP(r * iP) + 1;
});
XQ("Back", Zt("in"), Zt("out"), Zt());
g.SteppedEase = g.steps = aQ.SteppedEase = {
  config: function(Q, t) {
    Q === void 0 && (Q = 1);
    var B = 1 / Q, A = Q + (t ? 0 : 1), e = t ? 1 : 0, P = 1 - m;
    return function(i) {
      return ((A * dt(0, P, i) | 0) + e) * B;
    };
  }
};
wt.ease = g["quad.out"];
eQ("onComplete,onUpdate,onStart,onRepeat,onReverseComplete,onInterrupt", function(r) {
  return LB += r + "," + r + "Params,";
});
var _A = function(Q, t) {
  this.id = rP++, Q._gsap = this, this.target = Q, this.harness = t, this.get = t ? t.get : jA, this.set = t ? t.getSetter : bB;
}, ut = /* @__PURE__ */ function() {
  function r(t) {
    this.vars = t, this._delay = +t.delay || 0, (this._repeat = t.repeat === 1 / 0 ? -2 : t.repeat || 0) && (this._rDelay = t.repeatDelay || 0, this._yoyo = !!t.yoyo || !!t.yoyoEase), this._ts = 1, et(this, +t.duration, 1, 1), this.data = t.data, y && (this._ctx = y, y.data.push(this)), kt || nQ.wake();
  }
  var Q = r.prototype;
  return Q.delay = function(B) {
    return B || B === 0 ? (this.parent && this.parent.smoothChildTiming && this.startTime(this._start + B - this._delay), this._delay = B, this) : this._delay;
  }, Q.duration = function(B) {
    return arguments.length ? this.totalDuration(this._repeat > 0 ? B + (B + this._rDelay) * this._repeat : B) : this.totalDuration() && this._dur;
  }, Q.totalDuration = function(B) {
    return arguments.length ? (this._dirty = 0, et(this, this._repeat < 0 ? B : (B - this._repeat * this._rDelay) / (this._repeat + 1))) : this._tDur;
  }, Q.totalTime = function(B, A) {
    if (Pt(), !arguments.length)
      return this._tTime;
    var e = this._dp;
    if (e && e.smoothChildTiming && this._ts) {
      for (It(this, B), !e._dp || e.parent || gA(e, this); e && e.parent; )
        e.parent._time !== e._start + (e._ts >= 0 ? e._tTime / e._ts : (e.totalDuration() - e._tTime) / -e._ts) && e.totalTime(e._tTime, !0), e = e.parent;
      !this.parent && this._dp.autoRemoveChildren && (this._ts > 0 && B < this._tDur || this._ts < 0 && B > 0 || !this._tDur && !B) && uQ(this._dp, this, this._start - this._delay);
    }
    return (this._tTime !== B || !this._dur && !A || this._initted && Math.abs(this._zTime) === m || !this._initted && this._dur && B || !B && !this._initted && (this.add || this._ptLookup)) && (this._ts || (this._pTime = B), fA(this, B, A)), this;
  }, Q.time = function(B, A) {
    return arguments.length ? this.totalTime(Math.min(this.totalDuration(), B + $B(this)) % (this._dur + this._rDelay) || (B ? this._dur : 0), A) : this._time;
  }, Q.totalProgress = function(B, A) {
    return arguments.length ? this.totalTime(this.totalDuration() * B, A) : this.totalDuration() ? Math.min(1, this._tTime / this._tDur) : this.rawTime() >= 0 && this._initted ? 1 : 0;
  }, Q.progress = function(B, A) {
    return arguments.length ? this.totalTime(this.duration() * (this._yoyo && !(this.iteration() & 1) ? 1 - B : B) + $B(this), A) : this.duration() ? Math.min(1, this._time / this._dur) : this.rawTime() > 0 ? 1 : 0;
  }, Q.iteration = function(B, A) {
    var e = this.duration() + this._rDelay;
    return arguments.length ? this.totalTime(this._time + (B - 1) * e, A) : this._repeat ? At(this._tTime, e) + 1 : 1;
  }, Q.timeScale = function(B, A) {
    if (!arguments.length)
      return this._rts === -m ? 0 : this._rts;
    if (this._rts === B)
      return this;
    var e = this.parent && this._ts ? Gt(this.parent._time, this) : this._tTime;
    return this._rts = +B || 0, this._ts = this._ps || B === -m ? 0 : this._rts, this.totalTime(dt(-Math.abs(this._delay), this.totalDuration(), e), A !== !1), yt(this), lP(this);
  }, Q.paused = function(B) {
    return arguments.length ? (this._ps !== B && (this._ps = B, B ? (this._pTime = this._tTime || Math.max(-this._delay, this.rawTime()), this._ts = this._act = 0) : (Pt(), this._ts = this._rts, this.totalTime(this.parent && !this.parent.smoothChildTiming ? this.rawTime() : this._tTime || this._pTime, this.progress() === 1 && Math.abs(this._zTime) !== m && (this._tTime -= m)))), this) : this._ps;
  }, Q.startTime = function(B) {
    if (arguments.length) {
      this._start = Y(B);
      var A = this.parent || this._dp;
      return A && (A._sort || !this.parent) && uQ(A, this, this._start - this._delay), this;
    }
    return this._start;
  }, Q.endTime = function(B) {
    return this._start + (AQ(B) ? this.totalDuration() : this.duration()) / Math.abs(this._ts || 1);
  }, Q.rawTime = function(B) {
    var A = this.parent || this._dp;
    return A ? B && (!this._ts || this._repeat && this._time && this.totalProgress() < 1) ? this._tTime % (this._dur + this._rDelay) : this._ts ? Gt(A.rawTime(B), this) : this._tTime : this._tTime;
  }, Q.revert = function(B) {
    B === void 0 && (B = aP);
    var A = q;
    return q = B, HB(this) && (this.timeline && this.timeline.revert(B), this.totalTime(-0.01, B.suppressEvents)), this.data !== "nested" && B.kill !== !1 && this.kill(), q = A, this;
  }, Q.globalTime = function(B) {
    for (var A = this, e = arguments.length ? B : A.rawTime(); A; )
      e = A._start + e / (Math.abs(A._ts) || 1), A = A._dp;
    return !this.parent && this._sat ? this._sat.globalTime(B) : e;
  }, Q.repeat = function(B) {
    return arguments.length ? (this._repeat = B === 1 / 0 ? -2 : B, QA(this)) : this._repeat === -2 ? 1 / 0 : this._repeat;
  }, Q.repeatDelay = function(B) {
    if (arguments.length) {
      var A = this._time;
      return this._rDelay = B, QA(this), A ? this.time(A) : this;
    }
    return this._rDelay;
  }, Q.yoyo = function(B) {
    return arguments.length ? (this._yoyo = B, this) : this._yoyo;
  }, Q.seek = function(B, A) {
    return this.totalTime(EQ(this, B), AQ(A));
  }, Q.restart = function(B, A) {
    return this.play().totalTime(B ? -this._delay : 0, AQ(A)), this._dur || (this._zTime = -m), this;
  }, Q.play = function(B, A) {
    return B != null && this.seek(B, A), this.reversed(!1).paused(!1);
  }, Q.reverse = function(B, A) {
    return B != null && this.seek(B || this.totalDuration(), A), this.reversed(!0).paused(!1);
  }, Q.pause = function(B, A) {
    return B != null && this.seek(B, A), this.paused(!0);
  }, Q.resume = function() {
    return this.paused(!1);
  }, Q.reversed = function(B) {
    return arguments.length ? (!!B !== this.reversed() && this.timeScale(-this._rts || (B ? -m : 0)), this) : this._rts < 0;
  }, Q.invalidate = function() {
    return this._initted = this._act = 0, this._zTime = -m, this;
  }, Q.isActive = function() {
    var B = this.parent || this._dp, A = this._start, e;
    return !!(!B || this._ts && this._initted && B.isActive() && (e = B.rawTime(!0)) >= A && e < this.endTime(!0) - m);
  }, Q.eventCallback = function(B, A, e) {
    var P = this.vars;
    return arguments.length > 1 ? (A ? (P[B] = A, e && (P[B + "Params"] = e), B === "onUpdate" && (this._onUpdate = A)) : delete P[B], this) : P[B];
  }, Q.then = function(B) {
    var A = this, e = A._prom;
    return new Promise(function(P) {
      var i = J(B) ? B : bA, s = function() {
        var c = A.then;
        A.then = null, e && e(), J(i) && (i = i(A)) && (i.then || i === A) && (A.then = c), P(i), A.then = c;
      };
      A._initted && A.totalProgress() === 1 && A._ts >= 0 || !A._tTime && A._ts < 0 ? s() : A._prom = s;
    });
  }, Q.kill = function() {
    nt(this);
  }, r;
}();
wQ(ut.prototype, {
  _time: 0,
  _start: 0,
  _end: 0,
  _tTime: 0,
  _tDur: 0,
  _dirty: 0,
  _repeat: 0,
  _yoyo: !1,
  parent: null,
  _initted: !1,
  _rDelay: 0,
  _ts: 1,
  _dp: 0,
  ratio: 0,
  _zTime: -m,
  _prom: 0,
  _ps: !1,
  _rts: 1
});
var BQ = /* @__PURE__ */ function(r) {
  UA(Q, r);
  function Q(B, A) {
    var e;
    return B === void 0 && (B = {}), e = r.call(this, B) || this, e.labels = {}, e.smoothChildTiming = !!B.smoothChildTiming, e.autoRemoveChildren = !!B.autoRemoveChildren, e._sort = AQ(B.sortChildren), S && uQ(B.parent || S, dQ(e), A), B.reversed && e.reverse(), B.paused && e.paused(!0), B.scrollTrigger && pA(dQ(e), B.scrollTrigger), e;
  }
  var t = Q.prototype;
  return t.to = function(A, e, P) {
    return zt(0, arguments, this), this;
  }, t.from = function(A, e, P) {
    return zt(1, arguments, this), this;
  }, t.fromTo = function(A, e, P, i) {
    return zt(2, arguments, this), this;
  }, t.set = function(A, e, P) {
    return e.duration = 0, e.parent = this, Tt(e).repeatDelay || (e.repeat = 0), e.immediateRender = !!e.immediateRender, new K(A, e, EQ(this, P), 1), this;
  }, t.call = function(A, e, P) {
    return uQ(this, K.delayedCall(0, A, e), P);
  }, t.staggerTo = function(A, e, P, i, s, v, c) {
    return P.duration = e, P.stagger = P.stagger || i, P.onComplete = v, P.onCompleteParams = c, P.parent = this, new K(A, P, EQ(this, s)), this;
  }, t.staggerFrom = function(A, e, P, i, s, v, c) {
    return P.runBackwards = 1, Tt(P).immediateRender = AQ(P.immediateRender), this.staggerTo(A, e, P, i, s, v, c);
  }, t.staggerFromTo = function(A, e, P, i, s, v, c, n) {
    return i.startAt = P, Tt(i).immediateRender = AQ(i.immediateRender), this.staggerTo(A, e, i, s, v, c, n);
  }, t.render = function(A, e, P) {
    var i = this._time, s = this._dirty ? this.totalDuration() : this._tDur, v = this._dur, c = A <= 0 ? 0 : Y(A), n = this._zTime < 0 != A < 0 && (this._initted || !v), T, z, a, o, w, E, l, u, U, k, O, D;
    if (this !== S && c > s && A >= 0 && (c = s), c !== this._tTime || P || n) {
      if (i !== this._time && v && (c += this._time - i, A += this._time - i), T = c, U = this._start, u = this._ts, E = !u, n && (v || (i = this._zTime), (A || !e) && (this._zTime = A)), this._repeat) {
        if (O = this._yoyo, w = v + this._rDelay, this._repeat < -1 && A < 0)
          return this.totalTime(w * 100 + A, e, P);
        if (T = Y(c % w), c === s ? (o = this._repeat, T = v) : (k = Y(c / w), o = ~~k, o && o === k && (T = v, o--), T > v && (T = v)), k = At(this._tTime, w), !i && this._tTime && k !== o && this._tTime - k * w - this._dur <= 0 && (k = o), O && o & 1 && (T = v - T, D = 1), o !== k && !this._lock) {
          var N = O && k & 1, d = N === (O && o & 1);
          if (o < k && (N = !N), i = N ? 0 : c % v ? v : c, this._lock = 1, this.render(i || (D ? 0 : Y(o * w)), e, !v)._lock = 0, this._tTime = c, !e && this.parent && oQ(this, "onRepeat"), this.vars.repeatRefresh && !D && (this.invalidate()._lock = 1, k = o), i && i !== this._time || E !== !this._ts || this.vars.onRepeat && !this.parent && !this._act)
            return this;
          if (v = this._dur, s = this._tDur, d && (this._lock = 2, i = N ? v : -1e-4, this.render(i, !0), this.vars.repeatRefresh && !D && this.invalidate()), this._lock = 0, !this._ts && !E)
            return this;
        }
      }
      if (this._hasPause && !this._forcing && this._lock < 2 && (l = DP(this, Y(i), Y(T)), l && (c -= T - (T = l._start))), this._tTime = c, this._time = T, this._act = !!u, this._initted || (this._onUpdate = this.vars.onUpdate, this._initted = 1, this._zTime = A, i = 0), !i && c && v && !e && !k && (oQ(this, "onStart"), this._tTime !== c))
        return this;
      if (T >= i && A >= 0)
        for (z = this._first; z; ) {
          if (a = z._next, (z._act || T >= z._start) && z._ts && l !== z) {
            if (z.parent !== this)
              return this.render(A, e, P);
            if (z.render(z._ts > 0 ? (T - z._start) * z._ts : (z._dirty ? z.totalDuration() : z._tDur) + (T - z._start) * z._ts, e, P), T !== this._time || !this._ts && !E) {
              l = 0, a && (c += this._zTime = -m);
              break;
            }
          }
          z = a;
        }
      else {
        z = this._last;
        for (var M = A < 0 ? A : T; z; ) {
          if (a = z._prev, (z._act || M <= z._end) && z._ts && l !== z) {
            if (z.parent !== this)
              return this.render(A, e, P);
            if (z.render(z._ts > 0 ? (M - z._start) * z._ts : (z._dirty ? z.totalDuration() : z._tDur) + (M - z._start) * z._ts, e, P || q && HB(z)), T !== this._time || !this._ts && !E) {
              l = 0, a && (c += this._zTime = M ? -m : m);
              break;
            }
          }
          z = a;
        }
      }
      if (l && !e && (this.pause(), l.render(T >= i ? 0 : -m)._zTime = T >= i ? 1 : -1, this._ts))
        return this._start = U, yt(this), this.render(A, e, P);
      this._onUpdate && !e && oQ(this, "onUpdate", !0), (c === s && this._tTime >= this.totalDuration() || !c && i) && (U === this._start || Math.abs(u) !== Math.abs(this._ts)) && (this._lock || ((A || !v) && (c === s && this._ts > 0 || !c && this._ts < 0) && GQ(this, 1), !e && !(A < 0 && !i) && (c || i || !s) && (oQ(this, c === s && A >= 0 ? "onComplete" : "onReverseComplete", !0), this._prom && !(c < s && this.timeScale() > 0) && this._prom())));
    }
    return this;
  }, t.add = function(A, e) {
    var P = this;
    if (NQ(e) || (e = EQ(this, e, A)), !(A instanceof ut)) {
      if ($(A))
        return A.forEach(function(i) {
          return P.add(i, e);
        }), this;
      if (Z(A))
        return this.addLabel(A, e);
      if (J(A))
        A = K.delayedCall(0, A);
      else
        return this;
    }
    return this !== A ? uQ(this, A, e) : this;
  }, t.getChildren = function(A, e, P, i) {
    A === void 0 && (A = !0), e === void 0 && (e = !0), P === void 0 && (P = !0), i === void 0 && (i = -hQ);
    for (var s = [], v = this._first; v; )
      v._start >= i && (v instanceof K ? e && s.push(v) : (P && s.push(v), A && s.push.apply(s, v.getChildren(!0, e, P)))), v = v._next;
    return s;
  }, t.getById = function(A) {
    for (var e = this.getChildren(1, 1, 1), P = e.length; P--; )
      if (e[P].vars.id === A)
        return e[P];
  }, t.remove = function(A) {
    return Z(A) ? this.removeLabel(A) : J(A) ? this.killTweensOf(A) : (A.parent === this && xt(this, A), A === this._recent && (this._recent = this._last), VQ(this));
  }, t.totalTime = function(A, e) {
    return arguments.length ? (this._forcing = 1, !this._dp && this._ts && (this._start = Y(nQ.time - (this._ts > 0 ? A / this._ts : (this.totalDuration() - A) / -this._ts))), r.prototype.totalTime.call(this, A, e), this._forcing = 0, this) : this._tTime;
  }, t.addLabel = function(A, e) {
    return this.labels[A] = EQ(this, e), this;
  }, t.removeLabel = function(A) {
    return delete this.labels[A], this;
  }, t.addPause = function(A, e, P) {
    var i = K.delayedCall(0, e || ht, P);
    return i.data = "isPause", this._hasPause = 1, uQ(this, i, EQ(this, A));
  }, t.removePause = function(A) {
    var e = this._first;
    for (A = EQ(this, A); e; )
      e._start === A && e.data === "isPause" && GQ(e), e = e._next;
  }, t.killTweensOf = function(A, e, P) {
    for (var i = this.getTweensOf(A, P), s = i.length; s--; )
      HQ !== i[s] && i[s].kill(A, e);
    return this;
  }, t.getTweensOf = function(A, e) {
    for (var P = [], i = lQ(A), s = this._first, v = NQ(e), c; s; )
      s instanceof K ? wP(s._targets, i) && (v ? (!HQ || s._initted && s._ts) && s.globalTime(0) <= e && s.globalTime(s.totalDuration()) > e : !e || s.isActive()) && P.push(s) : (c = s.getTweensOf(i, e)).length && P.push.apply(P, c), s = s._next;
    return P;
  }, t.tweenTo = function(A, e) {
    e = e || {};
    var P = this, i = EQ(P, A), s = e, v = s.startAt, c = s.onStart, n = s.onStartParams, T = s.immediateRender, z, a = K.to(P, wQ({
      ease: e.ease || "none",
      lazy: !1,
      immediateRender: !1,
      time: i,
      overwrite: "auto",
      duration: e.duration || Math.abs((i - (v && "time" in v ? v.time : P._time)) / P.timeScale()) || m,
      onStart: function() {
        if (P.pause(), !z) {
          var w = e.duration || Math.abs((i - (v && "time" in v ? v.time : P._time)) / P.timeScale());
          a._dur !== w && et(a, w, 0, 1).render(a._time, !0, !0), z = 1;
        }
        c && c.apply(a, n || []);
      }
    }, e));
    return T ? a.render(0) : a;
  }, t.tweenFromTo = function(A, e, P) {
    return this.tweenTo(e, wQ({
      startAt: {
        time: EQ(this, A)
      }
    }, P));
  }, t.recent = function() {
    return this._recent;
  }, t.nextLabel = function(A) {
    return A === void 0 && (A = this._time), tA(this, EQ(this, A));
  }, t.previousLabel = function(A) {
    return A === void 0 && (A = this._time), tA(this, EQ(this, A), 1);
  }, t.currentLabel = function(A) {
    return arguments.length ? this.seek(A, !0) : this.previousLabel(this._time + m);
  }, t.shiftChildren = function(A, e, P) {
    P === void 0 && (P = 0);
    var i = this._first, s = this.labels, v;
    for (A = Y(A); i; )
      i._start >= P && (i._start += A, i._end += A), i = i._next;
    if (e)
      for (v in s)
        s[v] >= P && (s[v] += A);
    return VQ(this);
  }, t.invalidate = function(A) {
    var e = this._first;
    for (this._lock = 0; e; )
      e.invalidate(A), e = e._next;
    return r.prototype.invalidate.call(this, A);
  }, t.clear = function(A) {
    A === void 0 && (A = !0);
    for (var e = this._first, P; e; )
      P = e._next, this.remove(e), e = P;
    return this._dp && (this._time = this._tTime = this._pTime = 0), A && (this.labels = {}), VQ(this);
  }, t.totalDuration = function(A) {
    var e = 0, P = this, i = P._last, s = hQ, v, c, n;
    if (arguments.length)
      return P.timeScale((P._repeat < 0 ? P.duration() : P.totalDuration()) / (P.reversed() ? -A : A));
    if (P._dirty) {
      for (n = P.parent; i; )
        v = i._prev, i._dirty && i.totalDuration(), c = i._start, c > s && P._sort && i._ts && !P._lock ? (P._lock = 1, uQ(P, i, c - i._delay, 1)._lock = 0) : s = c, c < 0 && i._ts && (e -= c, (!n && !P._dp || n && n.smoothChildTiming) && (P._start += Y(c / P._ts), P._time -= c, P._tTime -= c), P.shiftChildren(-c, !1, -1 / 0), s = 0), i._end > e && i._ts && (e = i._end), i = v;
      et(P, P === S && P._time > e ? P._time : e, 1, 1), P._dirty = 0;
    }
    return P._tDur;
  }, Q.updateRoot = function(A) {
    if (S._ts && (fA(S, Gt(A, S)), HA = nQ.frame), nQ.frame >= qB) {
      qB += zQ.autoSleep || 120;
      var e = S._first;
      if ((!e || !e._ts) && zQ.autoSleep && nQ._listeners.length < 2) {
        for (; e && !e._ts; )
          e = e._next;
        e || nQ.sleep();
      }
    }
  }, Q;
}(ut);
wQ(BQ.prototype, {
  _lock: 0,
  _hasPause: 0,
  _forcing: 0
});
var yP = function(Q, t, B, A, e, P, i) {
  var s = new PQ(this._pt, Q, t, 0, 1, ee, null, e), v = 0, c = 0, n, T, z, a, o, w, E, l;
  for (s.b = B, s.e = A, B += "", A += "", (E = ~A.indexOf("random(")) && (A = lt(A)), P && (l = [B, A], P(l, Q, t), B = l[0], A = l[1]), T = B.match(Jt) || []; n = Jt.exec(A); )
    a = n[0], o = A.substring(v, n.index), z ? z = (z + 1) % 5 : o.substr(-5) === "rgba(" && (z = 1), a !== T[c++] && (w = parseFloat(T[c - 1]) || 0, s._pt = {
      _next: s._pt,
      p: o || c === 1 ? o : ",",
      //note: SVG spec allows omission of comma/space when a negative sign is wedged between two numbers, like 2.5-5.3 instead of 2.5,-5.3 but when tweening, the negative value may switch to positive, so we insert the comma just in case.
      s: w,
      c: a.charAt(1) === "=" ? $Q(w, a) - w : parseFloat(a) - w,
      m: z && z < 4 ? Math.round : 0
    }, v = Jt.lastIndex);
  return s.c = v < A.length ? A.substring(v, A.length) : "", s.fp = i, (NA.test(A) || E) && (s.e = 0), this._pt = s, s;
}, jB = function(Q, t, B, A, e, P, i, s, v, c) {
  J(A) && (A = A(e || 0, Q, P));
  var n = Q[t], T = B !== "get" ? B : J(n) ? v ? Q[t.indexOf("set") || !J(Q["get" + t.substr(3)]) ? t : "get" + t.substr(3)](v) : Q[t]() : n, z = J(n) ? v ? JP : Be : FB, a;
  if (Z(A) && (~A.indexOf("random(") && (A = lt(A)), A.charAt(1) === "=" && (a = $Q(T, A) + (_(T) || 0), (a || a === 0) && (A = a))), !c || T !== A || aB)
    return !isNaN(T * A) && A !== "" ? (a = new PQ(this._pt, Q, t, +T || 0, A - (T || 0), typeof n == "boolean" ? XP : Ae, 0, z), v && (a.fp = v), i && a.modifier(i, this, Q), this._pt = a) : (!n && !(t in Q) && NB(t, A), yP.call(this, Q, t, T, A, z, s || zQ.stringFilter, v));
}, IP = function(Q, t, B, A, e) {
  if (J(Q) && (Q = at(Q, e, t, B, A)), !DQ(Q) || Q.style && Q.nodeType || $(Q) || dA(Q))
    return Z(Q) ? at(Q, e, t, B, A) : Q;
  var P = {}, i;
  for (i in Q)
    P[i] = at(Q[i], e, t, B, A);
  return P;
}, $A = function(Q, t, B, A, e, P) {
  var i, s, v, c;
  if (cQ[Q] && (i = new cQ[Q]()).init(e, i.rawVars ? t[Q] : IP(t[Q], A, e, P, B), B, A, P) !== !1 && (B._pt = s = new PQ(B._pt, e, Q, 0, 1, i.render, i, 0, i.priority), B !== _Q))
    for (v = B._ptLookup[B._targets.indexOf(e)], c = i._props.length; c--; )
      v[i._props[c]] = s;
  return i;
}, HQ, aB, fB = function r(Q, t, B) {
  var A = Q.vars, e = A.ease, P = A.startAt, i = A.immediateRender, s = A.lazy, v = A.onUpdate, c = A.runBackwards, n = A.yoyoEase, T = A.keyframes, z = A.autoRevert, a = Q._dur, o = Q._startAt, w = Q._targets, E = Q.parent, l = E && E.data === "nested" ? E.vars.targets : w, u = Q._overwrite === "auto" && !UB, U = Q.timeline, k = A.easeReverse || n, O, D, N, d, M, G, b, C, j, f, H, F, I;
  if (U && (!T || !e) && (e = "none"), Q._ease = JQ(e, wt.ease), Q._rEase = k && (JQ(k) || Q._ease), Q._from = !U && !!A.runBackwards, Q._from && (Q.ratio = 1), !U || T && !A.stagger) {
    if (C = w[0] ? SQ(w[0]).harness : 0, F = C && A[C.prop], O = bt(A, OB), o && (o._zTime < 0 && o.progress(1), t < 0 && c && i && !z ? o.render(-1, !0) : o.revert(c && a ? Lt : zP), o._lazy = 0), P) {
      if (GQ(Q._startAt = K.set(w, wQ({
        data: "isStart",
        overwrite: !1,
        parent: E,
        immediateRender: !0,
        lazy: !o && AQ(s),
        startAt: null,
        delay: 0,
        onUpdate: v && function() {
          return oQ(Q, "onUpdate");
        },
        stagger: 0
      }, P))), Q._startAt._dp = 0, Q._startAt._sat = Q, t < 0 && (q || !i && !z) && Q._startAt.revert(Lt), i && a && t <= 0 && B <= 0) {
        t && (Q._zTime = t);
        return;
      }
    } else if (c && a && !o) {
      if (t && (i = !1), N = wQ({
        overwrite: !1,
        data: "isFromStart",
        //we tag the tween with as "isFromStart" so that if [inside a plugin] we need to only do something at the very END of a tween, we have a way of identifying this tween as merely the one that's setting the beginning values for a "from()" tween. For example, clearProps in CSSPlugin should only get applied at the very END of a tween and without this tag, from(...{height:100, clearProps:"height", delay:1}) would wipe the height at the beginning of the tween and after 1 second, it'd kick back in.
        lazy: i && !o && AQ(s),
        immediateRender: i,
        //zero-duration tweens render immediately by default, but if we're not specifically instructed to render this tween immediately, we should skip this and merely _init() to record the starting values (rendering them immediately would push them to completion which is wasteful in that case - we'd have to render(-1) immediately after)
        stagger: 0,
        parent: E
        //ensures that nested tweens that had a stagger are handled properly, like gsap.from(".class", {y: gsap.utils.wrap([-100,100]), stagger: 0.5})
      }, O), F && (N[C.prop] = F), GQ(Q._startAt = K.set(w, N)), Q._startAt._dp = 0, Q._startAt._sat = Q, t < 0 && (q ? Q._startAt.revert(Lt) : Q._startAt.render(-1, !0)), Q._zTime = t, !i)
        r(Q._startAt, m, m);
      else if (!t)
        return;
    }
    for (Q._pt = Q._ptCache = 0, s = a && AQ(s) || s && !a, D = 0; D < w.length; D++) {
      if (M = w[D], b = M._gsap || MB(w)[D]._gsap, Q._ptLookup[D] = f = {}, vB[b.id] && FQ.length && Ft(), H = l === w ? D : l.indexOf(M), C && (j = new C()).init(M, F || O, Q, H, l) !== !1 && (Q._pt = d = new PQ(Q._pt, M, j.name, 0, 1, j.render, j, 0, j.priority), j._props.forEach(function(R) {
        f[R] = d;
      }), j.priority && (G = 1)), !C || F)
        for (N in O)
          cQ[N] && (j = $A(N, O, Q, H, M, l)) ? j.priority && (G = 1) : f[N] = d = jB.call(Q, M, N, "get", O[N], H, l, 0, A.stringFilter);
      Q._op && Q._op[D] && Q.kill(M, Q._op[D]), u && Q._pt && (HQ = Q, S.killTweensOf(M, f, Q.globalTime(t)), I = !Q.parent, HQ = 0), Q._pt && s && (vB[b.id] = 1);
    }
    G && Pe(Q), Q._onInit && Q._onInit(Q);
  }
  Q._onUpdate = v, Q._initted = (!Q._op || Q._pt) && !I, T && t <= 0 && U.render(hQ, !0, !0);
}, YP = function(Q, t, B, A, e, P, i, s) {
  var v = (Q._pt && Q._ptCache || (Q._ptCache = {}))[t], c, n, T, z;
  if (!v)
    for (v = Q._ptCache[t] = [], T = Q._ptLookup, z = Q._targets.length; z--; ) {
      if (c = T[z][t], c && c.d && c.d._pt)
        for (c = c.d._pt; c && c.p !== t && c.fp !== t; )
          c = c._next;
      if (!c)
        return aB = 1, Q.vars[t] = "+=0", fB(Q, i), aB = 0, s ? Et(t + " not eligible for reset. Try splitting into individual properties") : 1;
      v.push(c);
    }
  for (z = v.length; z--; )
    n = v[z], c = n._pt || n, c.s = (A || A === 0) && !e ? A : c.s + (A || 0) + P * c.c, c.c = B - c.s, n.e && (n.e = X(B) + _(n.e)), n.b && (n.b = c.s + _(n.b));
}, SP = function(Q, t) {
  var B = Q[0] ? SQ(Q[0]).harness : 0, A = B && B.aliases, e, P, i, s;
  if (!A)
    return t;
  e = Bt({}, t);
  for (P in A)
    if (P in e)
      for (s = A[P].split(","), i = s.length; i--; )
        e[s[i]] = e[P];
  return e;
}, VP = function(Q, t, B, A) {
  var e = t.ease || A || "power1.inOut", P, i;
  if ($(t))
    i = B[Q] || (B[Q] = []), t.forEach(function(s, v) {
      return i.push({
        t: v / (t.length - 1) * 100,
        v: s,
        e
      });
    });
  else
    for (P in t)
      i = B[P] || (B[P] = []), P === "ease" || i.push({
        t: parseFloat(Q),
        v: t[P],
        e
      });
}, at = function(Q, t, B, A, e) {
  return J(Q) ? Q.call(t, B, A, e) : Z(Q) && ~Q.indexOf("random(") ? lt(Q) : Q;
}, Qe = LB + "repeat,repeatDelay,yoyo,repeatRefresh,yoyoEase,easeReverse,autoRevert", te = {};
eQ(Qe + ",id,stagger,delay,duration,paused,scrollTrigger", function(r) {
  return te[r] = 1;
});
var K = /* @__PURE__ */ function(r) {
  UA(Q, r);
  function Q(B, A, e, P) {
    var i;
    typeof A == "number" && (e.duration = A, A = e, e = null), i = r.call(this, P ? A : Tt(A)) || this;
    var s = i.vars, v = s.duration, c = s.delay, n = s.immediateRender, T = s.stagger, z = s.overwrite, a = s.keyframes, o = s.defaults, w = s.scrollTrigger, E = A.parent || S, l = ($(B) || dA(B) ? NQ(B[0]) : "length" in A) ? [B] : lQ(B), u, U, k, O, D, N, d, M;
    if (i._targets = l.length ? MB(l) : Et("GSAP target " + B + " not found. https://gsap.com", !zQ.nullTargetWarn) || [], i._ptLookup = [], i._overwrite = z, a || T || Ot(v) || Ot(c)) {
      A = i.vars;
      var G = A.easeReverse || A.yoyoEase;
      if (u = i.timeline = new BQ({
        data: "nested",
        defaults: o || {},
        targets: E && E.data === "nested" ? E.vars.targets : l
      }), u.kill(), u.parent = u._dp = dQ(i), u._start = 0, T || Ot(v) || Ot(c)) {
        if (O = l.length, d = T && IA(T), DQ(T))
          for (D in T)
            ~Qe.indexOf(D) && (M || (M = {}), M[D] = T[D]);
        for (U = 0; U < O; U++)
          k = bt(A, te), k.stagger = 0, G && (k.easeReverse = G), M && Bt(k, M), N = l[U], k.duration = +at(v, dQ(i), U, N, l), k.delay = (+at(c, dQ(i), U, N, l) || 0) - i._delay, !T && O === 1 && k.delay && (i._delay = c = k.delay, i._start += c, k.delay = 0), u.to(N, k, d ? d(U, N, l) : 0), u._ease = g.none;
        u.duration() ? v = c = 0 : i.timeline = 0;
      } else if (a) {
        Tt(wQ(u.vars.defaults, {
          ease: "none"
        })), u._ease = JQ(a.ease || A.ease || "none");
        var b = 0, C, j, f;
        if ($(a))
          a.forEach(function(H) {
            return u.to(l, H, ">");
          }), u.duration();
        else {
          k = {};
          for (D in a)
            D === "ease" || D === "easeEach" || VP(D, a[D], k, a.easeEach);
          for (D in k)
            for (C = k[D].sort(function(H, F) {
              return H.t - F.t;
            }), b = 0, U = 0; U < C.length; U++)
              j = C[U], f = {
                ease: j.e,
                duration: (j.t - (U ? C[U - 1].t : 0)) / 100 * v
              }, f[D] = j.v, u.to(l, f, b), b += f.duration;
          u.duration() < v && u.to({}, {
            duration: v - u.duration()
          });
        }
      }
      v || i.duration(v = u.duration());
    } else
      i.timeline = 0;
    return z === !0 && !UB && (HQ = dQ(i), S.killTweensOf(l), HQ = 0), uQ(E, dQ(i), e), A.reversed && i.reverse(), A.paused && i.paused(!0), (n || !v && !a && i._start === Y(E._time) && AQ(n) && kP(dQ(i)) && E.data !== "nested") && (i._tTime = -m, i.render(Math.max(0, -c) || 0)), w && pA(dQ(i), w), i;
  }
  var t = Q.prototype;
  return t.render = function(A, e, P) {
    var i = this._time, s = this._tDur, v = this._dur, c = A < 0, n = A > s - m && !c ? s : A < m ? 0 : A, T, z, a, o, w, E, l, u;
    if (!v)
      UP(this, A, e, P);
    else if (n !== this._tTime || !A || P || !this._initted && this._tTime || this._startAt && this._zTime < 0 !== c || this._lazy) {
      if (T = n, u = this.timeline, this._repeat) {
        if (o = v + this._rDelay, this._repeat < -1 && c)
          return this.totalTime(o * 100 + A, e, P);
        if (T = Y(n % o), n === s ? (a = this._repeat, T = v) : (w = Y(n / o), a = ~~w, a && a === w ? (T = v, a--) : T > v && (T = v)), E = this._yoyo && a & 1, E && (T = v - T), w = At(this._tTime, o), T === i && !P && this._initted && a === w)
          return this._tTime = n, this;
        a !== w && this.vars.repeatRefresh && !E && !this._lock && T !== o && this._initted && (this._lock = P = 1, this.render(Y(o * a), !0).invalidate()._lock = 0);
      }
      if (!this._initted) {
        if (mA(this, c ? A : T, P, e, n))
          return this._tTime = 0, this;
        if (i !== this._time && !(P && this.vars.repeatRefresh && a !== w))
          return this;
        if (v !== this._dur)
          return this.render(A, e, P);
      }
      if (this._rEase) {
        var U = T < i;
        if (U !== this._inv) {
          var k = U ? i : v - i;
          this._inv = U, this._from && (this.ratio = 1 - this.ratio), this._invRatio = this.ratio, this._invTime = i, this._invRecip = k ? (U ? -1 : 1) / k : 0, this._invScale = U ? -this.ratio : 1 - this.ratio, this._invEase = U ? this._rEase : this._ease;
        }
        this.ratio = l = this._invRatio + this._invScale * this._invEase((T - this._invTime) * this._invRecip);
      } else
        this.ratio = l = this._ease(T / v);
      if (this._from && (this.ratio = l = 1 - l), this._tTime = n, this._time = T, !this._act && this._ts && (this._act = 1, this._lazy = 0), !i && n && !e && !w && (oQ(this, "onStart"), this._tTime !== n))
        return this;
      for (z = this._pt; z; )
        z.r(l, z.d), z = z._next;
      u && u.render(A < 0 ? A : u._dur * u._ease(T / this._dur), e, P) || this._startAt && (this._zTime = A), this._onUpdate && !e && (c && cB(this, A, e, P), oQ(this, "onUpdate")), this._repeat && a !== w && this.vars.onRepeat && !e && this.parent && oQ(this, "onRepeat"), (n === this._tDur || !n) && this._tTime === n && (c && !this._onUpdate && cB(this, A, !0, !0), (A || !v) && (n === this._tDur && this._ts > 0 || !n && this._ts < 0) && GQ(this, 1), !e && !(c && !i) && (n || i || E) && (oQ(this, n === s ? "onComplete" : "onReverseComplete", !0), this._prom && !(n < s && this.timeScale() > 0) && this._prom()));
    }
    return this;
  }, t.targets = function() {
    return this._targets;
  }, t.invalidate = function(A) {
    return (!A || !this.vars.runBackwards) && (this._startAt = 0), this._pt = this._op = this._onUpdate = this._lazy = this.ratio = 0, this._ptLookup = [], this.timeline && this.timeline.invalidate(A), r.prototype.invalidate.call(this, A);
  }, t.resetTo = function(A, e, P, i, s) {
    kt || nQ.wake(), this._ts || this.play();
    var v = Math.min(this._dur, (this._dp._time - this._start) * this._ts), c;
    return this._initted || fB(this, v), c = this._ease(v / this._dur), YP(this, A, e, P, i, c, v, s) ? this.resetTo(A, e, P, i, 1) : (It(this, 0), this.parent || GA(this._dp, this, "_first", "_last", this._dp._sort ? "_start" : 0), this.render(0));
  }, t.kill = function(A, e) {
    if (e === void 0 && (e = "all"), !A && (!e || e === "all"))
      return this._lazy = this._pt = 0, this.parent ? nt(this) : this.scrollTrigger && this.scrollTrigger.kill(!!q), this;
    if (this.timeline) {
      var P = this.timeline.totalDuration();
      return this.timeline.killTweensOf(A, e, HQ && HQ.vars.overwrite !== !0)._first || nt(this), this.parent && P !== this.timeline.totalDuration() && et(this, this._dur * this.timeline._tDur / P, 0, 1), this;
    }
    var i = this._targets, s = A ? lQ(A) : i, v = this._ptLookup, c = this._pt, n, T, z, a, o, w, E;
    if ((!e || e === "all") && hP(i, s))
      return e === "all" && (this._pt = 0), nt(this);
    for (n = this._op = this._op || [], e !== "all" && (Z(e) && (o = {}, eQ(e, function(l) {
      return o[l] = 1;
    }), e = o), e = SP(i, e)), E = i.length; E--; )
      if (~s.indexOf(i[E])) {
        T = v[E], e === "all" ? (n[E] = e, a = T, z = {}) : (z = n[E] = n[E] || {}, a = e);
        for (o in a)
          w = T && T[o], w && ((!("kill" in w.d) || w.d.kill(o) === !0) && xt(this, w, "_pt"), delete T[o]), z !== "all" && (z[o] = 1);
      }
    return this._initted && !this._pt && c && nt(this), this;
  }, Q.to = function(A, e) {
    return new Q(A, e, arguments[2]);
  }, Q.from = function(A, e) {
    return zt(1, arguments);
  }, Q.delayedCall = function(A, e, P, i) {
    return new Q(e, 0, {
      immediateRender: !1,
      lazy: !1,
      overwrite: !1,
      delay: A,
      onComplete: e,
      onReverseComplete: e,
      onCompleteParams: P,
      onReverseCompleteParams: P,
      callbackScope: i
    });
  }, Q.fromTo = function(A, e, P) {
    return zt(2, arguments);
  }, Q.set = function(A, e) {
    return e.duration = 0, e.repeatDelay || (e.repeat = 0), new Q(A, e);
  }, Q.killTweensOf = function(A, e, P) {
    return S.killTweensOf(A, e, P);
  }, Q;
}(ut);
wQ(K.prototype, {
  _targets: [],
  _lazy: 0,
  _startAt: 0,
  _op: 0,
  _onInit: 0
});
eQ("staggerTo,staggerFrom,staggerFromTo", function(r) {
  K[r] = function() {
    var Q = new BQ(), t = oB.call(arguments, 0);
    return t.splice(r === "staggerFromTo" ? 5 : 4, 0, 0), Q[r].apply(Q, t);
  };
});
var FB = function(Q, t, B) {
  return Q[t] = B;
}, Be = function(Q, t, B) {
  return Q[t](B);
}, JP = function(Q, t, B, A) {
  return Q[t](A.fp, B);
}, RP = function(Q, t, B) {
  return Q.setAttribute(t, B);
}, bB = function(Q, t) {
  return J(Q[t]) ? Be : DB(Q[t]) && Q.setAttribute ? RP : FB;
}, Ae = function(Q, t) {
  return t.set(t.t, t.p, Math.round((t.s + t.c * Q) * 1e6) / 1e6, t);
}, XP = function(Q, t) {
  return t.set(t.t, t.p, !!(t.s + t.c * Q), t);
}, ee = function(Q, t) {
  var B = t._pt, A = "";
  if (!Q && t.b)
    A = t.b;
  else if (Q === 1 && t.e)
    A = t.e;
  else {
    for (; B; )
      A = B.p + (B.m ? B.m(B.s + B.c * Q) : Math.round((B.s + B.c * Q) * 1e4) / 1e4) + A, B = B._next;
    A += t.c;
  }
  t.set(t.t, t.p, A, t);
}, GB = function(Q, t) {
  for (var B = t._pt; B; )
    B.r(Q, B.d), B = B._next;
}, KP = function(Q, t, B, A) {
  for (var e = this._pt, P; e; )
    P = e._next, e.p === A && e.modifier(Q, t, B), e = P;
}, ZP = function(Q) {
  for (var t = this._pt, B, A; t; )
    A = t._next, t.p === Q && !t.op || t.op === Q ? xt(this, t, "_pt") : t.dep || (B = 1), t = A;
  return !B;
}, WP = function(Q, t, B, A) {
  A.mSet(Q, t, A.m.call(A.tween, B, A.mt), A);
}, Pe = function(Q) {
  for (var t = Q._pt, B, A, e, P; t; ) {
    for (B = t._next, A = e; A && A.pr > t.pr; )
      A = A._next;
    (t._prev = A ? A._prev : P) ? t._prev._next = t : e = t, (t._next = A) ? A._prev = t : P = t, t = B;
  }
  Q._pt = e;
}, PQ = /* @__PURE__ */ function() {
  function r(t, B, A, e, P, i, s, v, c) {
    this.t = B, this.s = e, this.c = P, this.p = A, this.r = i || Ae, this.d = s || this, this.set = v || FB, this.pr = c || 0, this._next = t, t && (t._prev = this);
  }
  var Q = r.prototype;
  return Q.modifier = function(B, A, e) {
    this.mSet = this.mSet || this.set, this.set = WP, this.m = B, this.mt = e, this.tween = A;
  }, r;
}();
eQ(LB + "parent,duration,ease,delay,overwrite,runBackwards,startAt,yoyo,immediateRender,repeat,repeatDelay,data,paused,reversed,lazy,callbackScope,stringFilter,id,yoyoEase,stagger,inherit,repeatRefresh,keyframes,autoRevert,scrollTrigger,easeReverse", function(r) {
  return OB[r] = 1;
});
aQ.TweenMax = aQ.TweenLite = K;
aQ.TimelineLite = aQ.TimelineMax = BQ;
S = new BQ({
  sortChildren: !1,
  defaults: wt,
  autoRemoveChildren: !0,
  id: "root",
  smoothChildTiming: !0
});
zQ.stringFilter = WA;
var RQ = [], Ht = {}, qP = [], AA = 0, _P = 0, Wt = function(Q) {
  return (Ht[Q] || qP).map(function(t) {
    return t();
  });
}, wB = function() {
  var Q = Date.now(), t = [];
  Q - AA > 2 && (Wt("matchMediaInit"), RQ.forEach(function(B) {
    var A = B.queries, e = B.conditions, P, i, s, v;
    for (i in A)
      P = kQ.matchMedia(A[i]).matches, P && (s = 1), P !== e[i] && (e[i] = P, v = 1);
    v && (B.revert(), s && t.push(B));
  }), Wt("matchMediaRevert"), t.forEach(function(B) {
    return B.onMatch(B, function(A) {
      return B.add(null, A);
    });
  }), AA = Q, Wt("matchMedia"));
}, ie = /* @__PURE__ */ function() {
  function r(t, B) {
    this.selector = B && TB(B), this.data = [], this._r = [], this.isReverted = !1, this.id = _P++, t && this.add(t);
  }
  var Q = r.prototype;
  return Q.add = function(B, A, e) {
    J(B) && (e = A, A = B, B = J);
    var P = this, i = function() {
      var v = y, c = P.selector, n;
      return v && v !== P && v.data.push(P), e && (P.selector = TB(e)), y = P, n = A.apply(P, arguments), J(n) && P._r.push(n), y = v, P.selector = c, P.isReverted = !1, n;
    };
    return P.last = i, B === J ? i(P, function(s) {
      return P.add(null, s);
    }) : B ? P[B] = i : i;
  }, Q.ignore = function(B) {
    var A = y;
    y = null, B(this), y = A;
  }, Q.getTweens = function() {
    var B = [];
    return this.data.forEach(function(A) {
      return A instanceof r ? B.push.apply(B, A.getTweens()) : A instanceof K && !(A.parent && A.parent.data === "nested") && B.push(A);
    }), B;
  }, Q.clear = function() {
    this._r.length = this.data.length = 0;
  }, Q.kill = function(B, A) {
    var e = this;
    if (B ? function() {
      for (var i = e.getTweens(), s = e.data.length, v; s--; )
        v = e.data[s], v.data === "isFlip" && (v.revert(), v.getChildren(!0, !0, !1).forEach(function(c) {
          return i.splice(i.indexOf(c), 1);
        }));
      for (i.map(function(c) {
        return {
          g: c._dur || c._delay || c._sat && !c._sat.vars.immediateRender ? c.globalTime(0) : -1 / 0,
          t: c
        };
      }).sort(function(c, n) {
        return n.g - c.g || -1 / 0;
      }).forEach(function(c) {
        return c.t.revert(B);
      }), s = e.data.length; s--; )
        v = e.data[s], v instanceof BQ ? v.data !== "nested" && (v.scrollTrigger && v.scrollTrigger.revert(), v.kill()) : !(v instanceof K) && v.revert && v.revert(B);
      e._r.forEach(function(c) {
        return c(B, e);
      }), e.isReverted = !0;
    }() : this.data.forEach(function(i) {
      return i.kill && i.kill();
    }), this.clear(), A)
      for (var P = RQ.length; P--; )
        RQ[P].id === this.id && RQ.splice(P, 1);
  }, Q.revert = function(B) {
    this.kill(B || {});
  }, r;
}(), $P = /* @__PURE__ */ function() {
  function r(t) {
    this.contexts = [], this.scope = t, y && y.data.push(this);
  }
  var Q = r.prototype;
  return Q.add = function(B, A, e) {
    DQ(B) || (B = {
      matches: B
    });
    var P = new ie(0, e || this.scope), i = P.conditions = {}, s, v, c;
    y && !P.selector && (P.selector = y.selector), this.contexts.push(P), A = P.add("onMatch", A), P.queries = B;
    for (v in B)
      v === "all" ? c = 1 : (s = kQ.matchMedia(B[v]), s && (RQ.indexOf(P) < 0 && RQ.push(P), (i[v] = s.matches) && (c = 1), s.addListener ? s.addListener(wB) : s.addEventListener("change", wB)));
    return c && A(P, function(n) {
      return P.add(null, n);
    }), this;
  }, Q.revert = function(B) {
    this.kill(B || {});
  }, Q.kill = function(B) {
    this.contexts.forEach(function(A) {
      return A.kill(B, !0);
    });
  }, r;
}(), gt = {
  registerPlugin: function() {
    for (var Q = arguments.length, t = new Array(Q), B = 0; B < Q; B++)
      t[B] = arguments[B];
    t.forEach(function(A) {
      return XA(A);
    });
  },
  timeline: function(Q) {
    return new BQ(Q);
  },
  getTweensOf: function(Q, t) {
    return S.getTweensOf(Q, t);
  },
  getProperty: function(Q, t, B, A) {
    Z(Q) && (Q = lQ(Q)[0]);
    var e = SQ(Q || {}).get, P = B ? bA : FA;
    return B === "native" && (B = ""), Q && (t ? P((cQ[t] && cQ[t].get || e)(Q, t, B, A)) : function(i, s, v) {
      return P((cQ[i] && cQ[i].get || e)(Q, i, s, v));
    });
  },
  quickSetter: function(Q, t, B) {
    if (Q = lQ(Q), Q.length > 1) {
      var A = Q.map(function(c) {
        return rQ.quickSetter(c, t, B);
      }), e = A.length;
      return function(c) {
        for (var n = e; n--; )
          A[n](c);
      };
    }
    Q = Q[0] || {};
    var P = cQ[t], i = SQ(Q), s = i.harness && (i.harness.aliases || {})[t] || t, v = P ? function(c) {
      var n = new P();
      _Q._pt = 0, n.init(Q, B ? c + B : c, _Q, 0, [Q]), n.render(1, n), _Q._pt && GB(1, _Q);
    } : i.set(Q, s);
    return P ? v : function(c) {
      return v(Q, s, B ? c + B : c, i, 1);
    };
  },
  quickTo: function(Q, t, B) {
    var A, e = rQ.to(Q, wQ((A = {}, A[t] = "+=0.1", A.paused = !0, A.stagger = 0, A), B || {})), P = function(s, v, c) {
      return e.resetTo(t, s, v, c);
    };
    return P.tween = e, P;
  },
  isTweening: function(Q) {
    return S.getTweensOf(Q, !0).length > 0;
  },
  defaults: function(Q) {
    return Q && Q.ease && (Q.ease = JQ(Q.ease, wt.ease)), _B(wt, Q || {});
  },
  config: function(Q) {
    return _B(zQ, Q || {});
  },
  registerEffect: function(Q) {
    var t = Q.name, B = Q.effect, A = Q.plugins, e = Q.defaults, P = Q.extendTimeline;
    (A || "").split(",").forEach(function(i) {
      return i && !cQ[i] && !aQ[i] && Et(t + " effect requires " + i + " plugin.");
    }), Rt[t] = function(i, s, v) {
      return B(lQ(i), wQ(s || {}, e), v);
    }, P && (BQ.prototype[t] = function(i, s, v) {
      return this.add(Rt[t](i, DQ(s) ? s : (v = s) && {}, this), v);
    });
  },
  registerEase: function(Q, t) {
    g[Q] = JQ(t);
  },
  parseEase: function(Q, t) {
    return arguments.length ? JQ(Q, t) : g;
  },
  getById: function(Q) {
    return S.getById(Q);
  },
  exportRoot: function(Q, t) {
    Q === void 0 && (Q = {});
    var B = new BQ(Q), A, e;
    for (B.smoothChildTiming = AQ(Q.smoothChildTiming), S.remove(B), B._dp = 0, B._time = B._tTime = S._time, A = S._first; A; )
      e = A._next, (t || !(!A._dur && A instanceof K && A.vars.onComplete === A._targets[0])) && uQ(B, A, A._start - A._delay), A = e;
    return uQ(S, B, 0), B;
  },
  context: function(Q, t) {
    return Q ? new ie(Q, t) : y;
  },
  matchMedia: function(Q) {
    return new $P(Q);
  },
  matchMediaRefresh: function() {
    return RQ.forEach(function(Q) {
      var t = Q.conditions, B, A;
      for (A in t)
        t[A] && (t[A] = !1, B = 1);
      B && Q.revert();
    }) || wB();
  },
  addEventListener: function(Q, t) {
    var B = Ht[Q] || (Ht[Q] = []);
    ~B.indexOf(t) || B.push(t);
  },
  removeEventListener: function(Q, t) {
    var B = Ht[Q], A = B && B.indexOf(t);
    A >= 0 && B.splice(A, 1);
  },
  utils: {
    wrap: HP,
    wrapYoyo: jP,
    distribute: IA,
    random: SA,
    snap: YA,
    normalize: MP,
    getUnit: _,
    clamp: CP,
    splitColor: KA,
    toArray: lQ,
    selector: TB,
    mapRange: JA,
    pipe: OP,
    unitize: LP,
    interpolate: fP,
    shuffle: yA
  },
  install: LA,
  effects: Rt,
  ticker: nQ,
  updateRoot: BQ.updateRoot,
  plugins: cQ,
  globalTimeline: S,
  core: {
    PropTween: PQ,
    globals: MA,
    Tween: K,
    Timeline: BQ,
    Animation: ut,
    getCache: SQ,
    _removeLinkedListItem: xt,
    reverting: function() {
      return q;
    },
    context: function(Q) {
      return Q && y && (y.data.push(Q), Q._ctx = y), y;
    },
    suppressOverwrites: function(Q) {
      return UB = Q;
    }
  }
};
eQ("to,from,fromTo,delayedCall,set,killTweensOf", function(r) {
  return gt[r] = K[r];
});
nQ.add(BQ.updateRoot);
_Q = gt.to({}, {
  duration: 0
});
var Qi = function(Q, t) {
  for (var B = Q._pt; B && B.p !== t && B.op !== t && B.fp !== t; )
    B = B._next;
  return B;
}, ti = function(Q, t) {
  var B = Q._targets, A, e, P;
  for (A in t)
    for (e = B.length; e--; )
      P = Q._ptLookup[e][A], P && (P = P.d) && (P._pt && (P = Qi(P, A)), P && P.modifier && P.modifier(t[A], Q, B[e], A));
}, qt = function(Q, t) {
  return {
    name: Q,
    headless: 1,
    rawVars: 1,
    //don't pre-process function-based values or "random()" strings.
    init: function(A, e, P) {
      P._onInit = function(i) {
        var s, v;
        if (Z(e) && (s = {}, eQ(e, function(c) {
          return s[c] = 1;
        }), e = s), t) {
          s = {};
          for (v in e)
            s[v] = t(e[v]);
          e = s;
        }
        ti(i, e);
      };
    }
  };
}, rQ = gt.registerPlugin({
  name: "attr",
  init: function(Q, t, B, A, e) {
    var P, i, s;
    this.tween = B;
    for (P in t)
      s = Q.getAttribute(P) || "", i = this.add(Q, "setAttribute", (s || 0) + "", t[P], A, e, 0, 0, P), i.op = P, i.b = s, this._props.push(P);
  },
  render: function(Q, t) {
    for (var B = t._pt; B; )
      q ? B.set(B.t, B.p, B.b, B) : B.r(Q, B.d), B = B._next;
  }
}, {
  name: "endArray",
  headless: 1,
  init: function(Q, t) {
    for (var B = t.length; B--; )
      this.add(Q, B, Q[B] || 0, t[B], 0, 0, 0, 0, 0, 1);
  }
}, qt("roundProps", zB), qt("modifiers"), qt("snap", YA)) || gt;
K.version = BQ.version = rQ.version = "3.15.0";
OA = 1;
dB() && Pt();
g.Power0;
g.Power1;
g.Power2;
g.Power3;
g.Power4;
g.Linear;
g.Quad;
g.Cubic;
g.Quart;
g.Quint;
g.Strong;
g.Elastic;
g.Back;
g.SteppedEase;
g.Bounce;
g.Sine;
g.Expo;
g.Circ;
/*!
 * CSSPlugin 3.15.0
 * https://gsap.com
 *
 * Copyright 2008-2026, GreenSock. All rights reserved.
 * Subject to the terms at https://gsap.com/standard-license
 * @author: Jack Doyle, jack@greensock.com
*/
var eA, jQ, Qt, gB, YQ, PA, pB, Bi = function() {
  return typeof window < "u";
}, OQ = {}, IQ = 180 / Math.PI, tt = Math.PI / 180, KQ = Math.atan2, iA = 1e8, mB = /([A-Z])/g, Ai = /(left|right|width|margin|padding|x)/i, ei = /[\s,\(]\S/, UQ = {
  autoAlpha: "opacity,visibility",
  scale: "scaleX,scaleY",
  alpha: "opacity"
}, EB = function(Q, t) {
  return t.set(t.t, t.p, Math.round((t.s + t.c * Q) * 1e4) / 1e4 + t.u, t);
}, Pi = function(Q, t) {
  return t.set(t.t, t.p, Q === 1 ? t.e : Math.round((t.s + t.c * Q) * 1e4) / 1e4 + t.u, t);
}, ii = function(Q, t) {
  return t.set(t.t, t.p, Q ? Math.round((t.s + t.c * Q) * 1e4) / 1e4 + t.u : t.b, t);
}, ri = function(Q, t) {
  return t.set(t.t, t.p, Q === 1 ? t.e : Q ? Math.round((t.s + t.c * Q) * 1e4) / 1e4 + t.u : t.b, t);
}, si = function(Q, t) {
  var B = t.s + t.c * Q;
  t.set(t.t, t.p, ~~(B + (B < 0 ? -0.5 : 0.5)) + t.u, t);
}, re = function(Q, t) {
  return t.set(t.t, t.p, Q ? t.e : t.b, t);
}, se = function(Q, t) {
  return t.set(t.t, t.p, Q !== 1 ? t.b : t.e, t);
}, vi = function(Q, t, B) {
  return Q.style[t] = B;
}, ci = function(Q, t, B) {
  return Q.style.setProperty(t, B);
}, ni = function(Q, t, B) {
  return Q._gsap[t] = B;
}, oi = function(Q, t, B) {
  return Q._gsap.scaleX = Q._gsap.scaleY = B;
}, Ti = function(Q, t, B, A, e) {
  var P = Q._gsap;
  P.scaleX = P.scaleY = B, P.renderTransform(e, P);
}, zi = function(Q, t, B, A, e) {
  var P = Q._gsap;
  P[t] = B, P.renderTransform(e, P);
}, V = "transform", iQ = V + "Origin", ai = function r(Q, t) {
  var B = this, A = this.target, e = A.style, P = A._gsap;
  if (Q in OQ && e) {
    if (this.tfm = this.tfm || {}, Q !== "transform")
      Q = UQ[Q] || Q, ~Q.indexOf(",") ? Q.split(",").forEach(function(i) {
        return B.tfm[i] = CQ(A, i);
      }) : this.tfm[Q] = P.x ? P[Q] : CQ(A, Q), Q === iQ && (this.tfm.zOrigin = P.zOrigin);
    else
      return UQ.transform.split(",").forEach(function(i) {
        return r.call(B, i, t);
      });
    if (this.props.indexOf(V) >= 0)
      return;
    P.svg && (this.svgo = A.getAttribute("data-svg-origin"), this.props.push(iQ, t, "")), Q = V;
  }
  (e || t) && this.props.push(Q, t, e[Q]);
}, ve = function(Q) {
  Q.translate && (Q.removeProperty("translate"), Q.removeProperty("scale"), Q.removeProperty("rotate"));
}, wi = function() {
  var Q = this.props, t = this.target, B = t.style, A = t._gsap, e, P;
  for (e = 0; e < Q.length; e += 3)
    Q[e + 1] ? Q[e + 1] === 2 ? t[Q[e]](Q[e + 2]) : t[Q[e]] = Q[e + 2] : Q[e + 2] ? B[Q[e]] = Q[e + 2] : B.removeProperty(Q[e].substr(0, 2) === "--" ? Q[e] : Q[e].replace(mB, "-$1").toLowerCase());
  if (this.tfm) {
    for (P in this.tfm)
      A[P] = this.tfm[P];
    A.svg && (A.renderTransform(), t.setAttribute("data-svg-origin", this.svgo || "")), e = pB(), (!e || !e.isStart) && !B[V] && (ve(B), A.zOrigin && B[iQ] && (B[iQ] += " " + A.zOrigin + "px", A.zOrigin = 0, A.renderTransform()), A.uncache = 1);
  }
}, ce = function(Q, t) {
  var B = {
    target: Q,
    props: [],
    revert: wi,
    save: ai
  };
  return Q._gsap || rQ.core.getCache(Q), t && Q.style && Q.nodeType && t.split(",").forEach(function(A) {
    return B.save(A);
  }), B;
}, ne, hB = function(Q, t) {
  var B = jQ.createElementNS ? jQ.createElementNS((t || "http://www.w3.org/1999/xhtml").replace(/^https/, "http"), Q) : jQ.createElement(Q);
  return B && B.style ? B : jQ.createElement(Q);
}, TQ = function r(Q, t, B) {
  var A = getComputedStyle(Q);
  return A[t] || A.getPropertyValue(t.replace(mB, "-$1").toLowerCase()) || A.getPropertyValue(t) || !B && r(Q, it(t) || t, 1) || "";
}, rA = "O,Moz,ms,Ms,Webkit".split(","), it = function(Q, t, B) {
  var A = t || YQ, e = A.style, P = 5;
  if (Q in e && !B)
    return Q;
  for (Q = Q.charAt(0).toUpperCase() + Q.substr(1); P-- && !(rA[P] + Q in e); )
    ;
  return P < 0 ? null : (P === 3 ? "ms" : P >= 0 ? rA[P] : "") + Q;
}, lB = function() {
  Bi() && window.document && (eA = window, jQ = eA.document, Qt = jQ.documentElement, YQ = hB("div") || {
    style: {}
  }, hB("div"), V = it(V), iQ = V + "Origin", YQ.style.cssText = "border-width:0;line-height:0;position:absolute;padding:0", ne = !!it("perspective"), pB = rQ.core.reverting, gB = 1);
}, sA = function(Q) {
  var t = Q.ownerSVGElement, B = hB("svg", t && t.getAttribute("xmlns") || "http://www.w3.org/2000/svg"), A = Q.cloneNode(!0), e;
  A.style.display = "block", B.appendChild(A), Qt.appendChild(B);
  try {
    e = A.getBBox();
  } catch {
  }
  return B.removeChild(A), Qt.removeChild(B), e;
}, vA = function(Q, t) {
  for (var B = t.length; B--; )
    if (Q.hasAttribute(t[B]))
      return Q.getAttribute(t[B]);
}, oe = function(Q) {
  var t, B;
  try {
    t = Q.getBBox();
  } catch {
    t = sA(Q), B = 1;
  }
  return t && (t.width || t.height) || B || (t = sA(Q)), t && !t.width && !t.x && !t.y ? {
    x: +vA(Q, ["x", "cx", "x1"]) || 0,
    y: +vA(Q, ["y", "cy", "y1"]) || 0,
    width: 0,
    height: 0
  } : t;
}, Te = function(Q) {
  return !!(Q.getCTM && (!Q.parentNode || Q.ownerSVGElement) && oe(Q));
}, gQ = function(Q, t) {
  if (t) {
    var B = Q.style, A;
    t in OQ && t !== iQ && (t = V), B.removeProperty ? (A = t.substr(0, 2), (A === "ms" || t.substr(0, 6) === "webkit") && (t = "-" + t), B.removeProperty(A === "--" ? t : t.replace(mB, "-$1").toLowerCase())) : B.removeAttribute(t);
  }
}, fQ = function(Q, t, B, A, e, P) {
  var i = new PQ(Q._pt, t, B, 0, 1, P ? se : re);
  return Q._pt = i, i.b = A, i.e = e, Q._props.push(B), i;
}, cA = {
  deg: 1,
  rad: 1,
  turn: 1
}, Ei = {
  grid: 1,
  flex: 1
}, pQ = function r(Q, t, B, A) {
  var e = parseFloat(B) || 0, P = (B + "").trim().substr((e + "").length) || "px", i = YQ.style, s = Ai.test(t), v = Q.tagName.toLowerCase() === "svg", c = (v ? "client" : "offset") + (s ? "Width" : "Height"), n = 100, T = A === "px", z = A === "%", a, o, w, E;
  if (A === P || !e || cA[A] || cA[P])
    return e;
  if (P !== "px" && !T && (e = r(Q, t, B, "px")), E = Q.getCTM && Te(Q), (z || P === "%") && (OQ[t] || ~t.indexOf("adius")))
    return a = E ? Q.getBBox()[s ? "width" : "height"] : Q[c], X(z ? e / a * n : e / 100 * a);
  if (i[s ? "width" : "height"] = n + (T ? P : A), o = A !== "rem" && ~t.indexOf("adius") || A === "em" && Q.appendChild && !v ? Q : Q.parentNode, E && (o = (Q.ownerSVGElement || {}).parentNode), (!o || o === jQ || !o.appendChild) && (o = jQ.body), w = o._gsap, w && z && w.width && s && w.time === nQ.time && !w.uncache)
    return X(e / w.width * n);
  if (z && (t === "height" || t === "width")) {
    var l = Q.style[t];
    Q.style[t] = n + A, a = Q[c], l ? Q.style[t] = l : gQ(Q, t);
  } else
    (z || P === "%") && !Ei[TQ(o, "display")] && (i.position = TQ(Q, "position")), o === Q && (i.position = "static"), o.appendChild(YQ), a = YQ[c], o.removeChild(YQ), i.position = "absolute";
  return s && z && (w = SQ(o), w.time = nQ.time, w.width = o[c]), X(T ? a * e / n : a && e ? n / a * e : 0);
}, CQ = function(Q, t, B, A) {
  var e;
  return gB || lB(), t in UQ && t !== "transform" && (t = UQ[t], ~t.indexOf(",") && (t = t.split(",")[0])), OQ[t] && t !== "transform" ? (e = Dt(Q, A), e = t !== "transformOrigin" ? e[t] : e.svg ? e.origin : mt(TQ(Q, iQ)) + " " + e.zOrigin + "px") : (e = Q.style[t], (!e || e === "auto" || A || ~(e + "").indexOf("calc(")) && (e = pt[t] && pt[t](Q, t, B) || TQ(Q, t) || jA(Q, t) || (t === "opacity" ? 1 : 0))), B && !~(e + "").trim().indexOf(" ") ? pQ(Q, t, e, B) + B : e;
}, hi = function(Q, t, B, A) {
  if (!B || B === "none") {
    var e = it(t, Q, 1), P = e && TQ(Q, e, 1);
    P && P !== B ? (t = e, B = P) : t === "borderColor" && (B = TQ(Q, "borderTopColor"));
  }
  var i = new PQ(this._pt, Q.style, t, 0, 1, ee), s = 0, v = 0, c, n, T, z, a, o, w, E, l, u, U, k;
  if (i.b = B, i.e = A, B += "", A += "", A.substring(0, 6) === "var(--" && (A = TQ(Q, A.substring(4, A.indexOf(")")))), A === "auto" && (o = Q.style[t], Q.style[t] = A, A = TQ(Q, t) || A, o ? Q.style[t] = o : gQ(Q, t)), c = [B, A], WA(c), B = c[0], A = c[1], T = B.match(qQ) || [], k = A.match(qQ) || [], k.length) {
    for (; n = qQ.exec(A); )
      w = n[0], l = A.substring(s, n.index), a ? a = (a + 1) % 5 : (l.substr(-5) === "rgba(" || l.substr(-5) === "hsla(") && (a = 1), w !== (o = T[v++] || "") && (z = parseFloat(o) || 0, U = o.substr((z + "").length), w.charAt(1) === "=" && (w = $Q(z, w) + U), E = parseFloat(w), u = w.substr((E + "").length), s = qQ.lastIndex - u.length, u || (u = u || zQ.units[t] || U, s === A.length && (A += u, i.e += u)), U !== u && (z = pQ(Q, t, o, u) || 0), i._pt = {
        _next: i._pt,
        p: l || v === 1 ? l : ",",
        //note: SVG spec allows omission of comma/space when a negative sign is wedged between two numbers, like 2.5-5.3 instead of 2.5,-5.3 but when tweening, the negative value may switch to positive, so we insert the comma just in case.
        s: z,
        c: E - z,
        m: a && a < 4 || t === "zIndex" ? Math.round : 0
      });
    i.c = s < A.length ? A.substring(s, A.length) : "";
  } else
    i.r = t === "display" && A === "none" ? se : re;
  return NA.test(A) && (i.e = 0), this._pt = i, i;
}, nA = {
  top: "0%",
  bottom: "100%",
  left: "0%",
  right: "100%",
  center: "50%"
}, li = function(Q) {
  var t = Q.split(" "), B = t[0], A = t[1] || "50%";
  return (B === "top" || B === "bottom" || A === "left" || A === "right") && (Q = B, B = A, A = Q), t[0] = nA[B] || B, t[1] = nA[A] || A, t.join(" ");
}, ki = function(Q, t) {
  if (t.tween && t.tween._time === t.tween._dur) {
    var B = t.t, A = B.style, e = t.u, P = B._gsap, i, s, v;
    if (e === "all" || e === !0)
      A.cssText = "", s = 1;
    else
      for (e = e.split(","), v = e.length; --v > -1; )
        i = e[v], OQ[i] && (s = 1, i = i === "transformOrigin" ? iQ : V), gQ(B, i);
    s && (gQ(B, V), P && (P.svg && B.removeAttribute("transform"), A.scale = A.rotate = A.translate = "none", Dt(B, 1), P.uncache = 1, ve(A)));
  }
}, pt = {
  clearProps: function(Q, t, B, A, e) {
    if (e.data !== "isFromStart") {
      var P = Q._pt = new PQ(Q._pt, t, B, 0, 0, ki);
      return P.u = A, P.pr = -10, P.tween = e, Q._props.push(B), 1;
    }
  }
  /* className feature (about 0.4kb gzipped).
  , className(plugin, target, property, endValue, tween) {
  	let _renderClassName = (ratio, data) => {
  			data.css.render(ratio, data.css);
  			if (!ratio || ratio === 1) {
  				let inline = data.rmv,
  					target = data.t,
  					p;
  				target.setAttribute("class", ratio ? data.e : data.b);
  				for (p in inline) {
  					_removeProperty(target, p);
  				}
  			}
  		},
  		_getAllStyles = (target) => {
  			let styles = {},
  				computed = getComputedStyle(target),
  				p;
  			for (p in computed) {
  				if (isNaN(p) && p !== "cssText" && p !== "length") {
  					styles[p] = computed[p];
  				}
  			}
  			_setDefaults(styles, _parseTransform(target, 1));
  			return styles;
  		},
  		startClassList = target.getAttribute("class"),
  		style = target.style,
  		cssText = style.cssText,
  		cache = target._gsap,
  		classPT = cache.classPT,
  		inlineToRemoveAtEnd = {},
  		data = {t:target, plugin:plugin, rmv:inlineToRemoveAtEnd, b:startClassList, e:(endValue.charAt(1) !== "=") ? endValue : startClassList.replace(new RegExp("(?:\\s|^)" + endValue.substr(2) + "(?![\\w-])"), "") + ((endValue.charAt(0) === "+") ? " " + endValue.substr(2) : "")},
  		changingVars = {},
  		startVars = _getAllStyles(target),
  		transformRelated = /(transform|perspective)/i,
  		endVars, p;
  	if (classPT) {
  		classPT.r(1, classPT.d);
  		_removeLinkedListItem(classPT.d.plugin, classPT, "_pt");
  	}
  	target.setAttribute("class", data.e);
  	endVars = _getAllStyles(target, true);
  	target.setAttribute("class", startClassList);
  	for (p in endVars) {
  		if (endVars[p] !== startVars[p] && !transformRelated.test(p)) {
  			changingVars[p] = endVars[p];
  			if (!style[p] && style[p] !== "0") {
  				inlineToRemoveAtEnd[p] = 1;
  			}
  		}
  	}
  	cache.classPT = plugin._pt = new PropTween(plugin._pt, target, "className", 0, 0, _renderClassName, data, 0, -11);
  	if (style.cssText !== cssText) { //only apply if things change. Otherwise, in cases like a background-image that's pulled dynamically, it could cause a refresh. See https://gsap.com/forums/topic/20368-possible-gsap-bug-switching-classnames-in-chrome/.
  		style.cssText = cssText; //we recorded cssText before we swapped classes and ran _getAllStyles() because in cases when a className tween is overwritten, we remove all the related tweening properties from that class change (otherwise class-specific stuff can't override properties we've directly set on the target's style object due to specificity).
  	}
  	_parseTransform(target, true); //to clear the caching of transforms
  	data.css = new gsap.plugins.css();
  	data.css.init(target, changingVars, tween);
  	plugin._props.push(...data.css._props);
  	return 1;
  }
  */
}, Ut = [1, 0, 0, 1, 0, 0], ze = {}, ae = function(Q) {
  return Q === "matrix(1, 0, 0, 1, 0, 0)" || Q === "none" || !Q;
}, oA = function(Q) {
  var t = TQ(Q, V);
  return ae(t) ? Ut : t.substr(7).match(CA).map(X);
}, xB = function(Q, t) {
  var B = Q._gsap || SQ(Q), A = Q.style, e = oA(Q), P, i, s, v;
  return B.svg && Q.getAttribute("transform") ? (s = Q.transform.baseVal.consolidate().matrix, e = [s.a, s.b, s.c, s.d, s.e, s.f], e.join(",") === "1,0,0,1,0,0" ? Ut : e) : (e === Ut && !Q.offsetParent && Q !== Qt && !B.svg && (s = A.display, A.display = "block", P = Q.parentNode, (!P || !Q.offsetParent && !Q.getBoundingClientRect().width) && (v = 1, i = Q.nextElementSibling, Qt.appendChild(Q)), e = oA(Q), s ? A.display = s : gQ(Q, "display"), v && (i ? P.insertBefore(Q, i) : P ? P.appendChild(Q) : Qt.removeChild(Q))), t && e.length > 6 ? [e[0], e[1], e[4], e[5], e[12], e[13]] : e);
}, kB = function(Q, t, B, A, e, P) {
  var i = Q._gsap, s = e || xB(Q, !0), v = i.xOrigin || 0, c = i.yOrigin || 0, n = i.xOffset || 0, T = i.yOffset || 0, z = s[0], a = s[1], o = s[2], w = s[3], E = s[4], l = s[5], u = t.split(" "), U = parseFloat(u[0]) || 0, k = parseFloat(u[1]) || 0, O, D, N, d;
  B ? s !== Ut && (D = z * w - a * o) && (N = U * (w / D) + k * (-o / D) + (o * l - w * E) / D, d = U * (-a / D) + k * (z / D) - (z * l - a * E) / D, U = N, k = d) : (O = oe(Q), U = O.x + (~u[0].indexOf("%") ? U / 100 * O.width : U), k = O.y + (~(u[1] || u[0]).indexOf("%") ? k / 100 * O.height : k)), A || A !== !1 && i.smooth ? (E = U - v, l = k - c, i.xOffset = n + (E * z + l * o) - E, i.yOffset = T + (E * a + l * w) - l) : i.xOffset = i.yOffset = 0, i.xOrigin = U, i.yOrigin = k, i.smooth = !!A, i.origin = t, i.originIsAbsolute = !!B, Q.style[iQ] = "0px 0px", P && (fQ(P, i, "xOrigin", v, U), fQ(P, i, "yOrigin", c, k), fQ(P, i, "xOffset", n, i.xOffset), fQ(P, i, "yOffset", T, i.yOffset)), Q.setAttribute("data-svg-origin", U + " " + k);
}, Dt = function(Q, t) {
  var B = Q._gsap || new _A(Q);
  if ("x" in B && !t && !B.uncache)
    return B;
  var A = Q.style, e = B.scaleX < 0, P = "px", i = "deg", s = getComputedStyle(Q), v = TQ(Q, iQ) || "0", c, n, T, z, a, o, w, E, l, u, U, k, O, D, N, d, M, G, b, C, j, f, H, F, I, R, QQ, sQ, W, x, tQ, vQ;
  return c = n = T = o = w = E = l = u = U = 0, z = a = 1, B.svg = !!(Q.getCTM && Te(Q)), s.translate && ((s.translate !== "none" || s.scale !== "none" || s.rotate !== "none") && (A[V] = (s.translate !== "none" ? "translate3d(" + (s.translate + " 0 0").split(" ").slice(0, 3).join(", ") + ") " : "") + (s.rotate !== "none" ? "rotate(" + s.rotate + ") " : "") + (s.scale !== "none" ? "scale(" + s.scale.split(" ").join(",") + ") " : "") + (s[V] !== "none" ? s[V] : "")), A.scale = A.rotate = A.translate = "none"), D = xB(Q, B.svg), B.svg && (B.uncache ? (I = Q.getBBox(), v = B.xOrigin - I.x + "px " + (B.yOrigin - I.y) + "px", F = "") : F = !t && Q.getAttribute("data-svg-origin"), kB(Q, F || v, !!F || B.originIsAbsolute, B.smooth !== !1, D)), k = B.xOrigin || 0, O = B.yOrigin || 0, D !== Ut && (G = D[0], b = D[1], C = D[2], j = D[3], c = f = D[4], n = H = D[5], D.length === 6 ? (z = Math.sqrt(G * G + b * b), a = Math.sqrt(j * j + C * C), o = G || b ? KQ(b, G) * IQ : 0, l = C || j ? KQ(C, j) * IQ + o : 0, l && (a *= Math.abs(Math.cos(l * tt))), B.svg && (c -= k - (k * G + O * C), n -= O - (k * b + O * j))) : (vQ = D[6], x = D[7], QQ = D[8], sQ = D[9], W = D[10], tQ = D[11], c = D[12], n = D[13], T = D[14], N = KQ(vQ, W), w = N * IQ, N && (d = Math.cos(-N), M = Math.sin(-N), F = f * d + QQ * M, I = H * d + sQ * M, R = vQ * d + W * M, QQ = f * -M + QQ * d, sQ = H * -M + sQ * d, W = vQ * -M + W * d, tQ = x * -M + tQ * d, f = F, H = I, vQ = R), N = KQ(-C, W), E = N * IQ, N && (d = Math.cos(-N), M = Math.sin(-N), F = G * d - QQ * M, I = b * d - sQ * M, R = C * d - W * M, tQ = j * M + tQ * d, G = F, b = I, C = R), N = KQ(b, G), o = N * IQ, N && (d = Math.cos(N), M = Math.sin(N), F = G * d + b * M, I = f * d + H * M, b = b * d - G * M, H = H * d - f * M, G = F, f = I), w && Math.abs(w) + Math.abs(o) > 359.9 && (w = o = 0, E = 180 - E), z = X(Math.sqrt(G * G + b * b + C * C)), a = X(Math.sqrt(H * H + vQ * vQ)), N = KQ(f, H), l = Math.abs(N) > 2e-4 ? N * IQ : 0, U = tQ ? 1 / (tQ < 0 ? -tQ : tQ) : 0), B.svg && (F = Q.getAttribute("transform"), B.forceCSS = Q.setAttribute("transform", "") || !ae(TQ(Q, V)), F && Q.setAttribute("transform", F))), Math.abs(l) > 90 && Math.abs(l) < 270 && (e ? (z *= -1, l += o <= 0 ? 180 : -180, o += o <= 0 ? 180 : -180) : (a *= -1, l += l <= 0 ? 180 : -180)), t = t || B.uncache, B.x = c - ((B.xPercent = c && (!t && B.xPercent || (Math.round(Q.offsetWidth / 2) === Math.round(-c) ? -50 : 0))) ? Q.offsetWidth * B.xPercent / 100 : 0) + P, B.y = n - ((B.yPercent = n && (!t && B.yPercent || (Math.round(Q.offsetHeight / 2) === Math.round(-n) ? -50 : 0))) ? Q.offsetHeight * B.yPercent / 100 : 0) + P, B.z = T + P, B.scaleX = X(z), B.scaleY = X(a), B.rotation = X(o) + i, B.rotationX = X(w) + i, B.rotationY = X(E) + i, B.skewX = l + i, B.skewY = u + i, B.transformPerspective = U + P, (B.zOrigin = parseFloat(v.split(" ")[2]) || !t && B.zOrigin || 0) && (A[iQ] = mt(v)), B.xOffset = B.yOffset = 0, B.force3D = zQ.force3D, B.renderTransform = B.svg ? Ui : ne ? we : ui, B.uncache = 0, B;
}, mt = function(Q) {
  return (Q = Q.split(" "))[0] + " " + Q[1];
}, _t = function(Q, t, B) {
  var A = _(t);
  return X(parseFloat(t) + parseFloat(pQ(Q, "x", B + "px", A))) + A;
}, ui = function(Q, t) {
  t.z = "0px", t.rotationY = t.rotationX = "0deg", t.force3D = 0, we(Q, t);
}, xQ = "0deg", st = "0px", yQ = ") ", we = function(Q, t) {
  var B = t || this, A = B.xPercent, e = B.yPercent, P = B.x, i = B.y, s = B.z, v = B.rotation, c = B.rotationY, n = B.rotationX, T = B.skewX, z = B.skewY, a = B.scaleX, o = B.scaleY, w = B.transformPerspective, E = B.force3D, l = B.target, u = B.zOrigin, U = "", k = E === "auto" && Q && Q !== 1 || E === !0;
  if (u && (n !== xQ || c !== xQ)) {
    var O = parseFloat(c) * tt, D = Math.sin(O), N = Math.cos(O), d;
    O = parseFloat(n) * tt, d = Math.cos(O), P = _t(l, P, D * d * -u), i = _t(l, i, -Math.sin(O) * -u), s = _t(l, s, N * d * -u + u);
  }
  w !== st && (U += "perspective(" + w + yQ), (A || e) && (U += "translate(" + A + "%, " + e + "%) "), (k || P !== st || i !== st || s !== st) && (U += s !== st || k ? "translate3d(" + P + ", " + i + ", " + s + ") " : "translate(" + P + ", " + i + yQ), v !== xQ && (U += "rotate(" + v + yQ), c !== xQ && (U += "rotateY(" + c + yQ), n !== xQ && (U += "rotateX(" + n + yQ), (T !== xQ || z !== xQ) && (U += "skew(" + T + ", " + z + yQ), (a !== 1 || o !== 1) && (U += "scale(" + a + ", " + o + yQ), l.style[V] = U || "translate(0, 0)";
}, Ui = function(Q, t) {
  var B = t || this, A = B.xPercent, e = B.yPercent, P = B.x, i = B.y, s = B.rotation, v = B.skewX, c = B.skewY, n = B.scaleX, T = B.scaleY, z = B.target, a = B.xOrigin, o = B.yOrigin, w = B.xOffset, E = B.yOffset, l = B.forceCSS, u = parseFloat(P), U = parseFloat(i), k, O, D, N, d;
  s = parseFloat(s), v = parseFloat(v), c = parseFloat(c), c && (c = parseFloat(c), v += c, s += c), s || v ? (s *= tt, v *= tt, k = Math.cos(s) * n, O = Math.sin(s) * n, D = Math.sin(s - v) * -T, N = Math.cos(s - v) * T, v && (c *= tt, d = Math.tan(v - c), d = Math.sqrt(1 + d * d), D *= d, N *= d, c && (d = Math.tan(c), d = Math.sqrt(1 + d * d), k *= d, O *= d)), k = X(k), O = X(O), D = X(D), N = X(N)) : (k = n, N = T, O = D = 0), (u && !~(P + "").indexOf("px") || U && !~(i + "").indexOf("px")) && (u = pQ(z, "x", P, "px"), U = pQ(z, "y", i, "px")), (a || o || w || E) && (u = X(u + a - (a * k + o * D) + w), U = X(U + o - (a * O + o * N) + E)), (A || e) && (d = z.getBBox(), u = X(u + A / 100 * d.width), U = X(U + e / 100 * d.height)), d = "matrix(" + k + "," + O + "," + D + "," + N + "," + u + "," + U + ")", z.setAttribute("transform", d), l && (z.style[V] = d);
}, Di = function(Q, t, B, A, e) {
  var P = 360, i = Z(e), s = parseFloat(e) * (i && ~e.indexOf("rad") ? IQ : 1), v = s - A, c = A + v + "deg", n, T;
  return i && (n = e.split("_")[1], n === "short" && (v %= P, v !== v % (P / 2) && (v += v < 0 ? P : -P)), n === "cw" && v < 0 ? v = (v + P * iA) % P - ~~(v / P) * P : n === "ccw" && v > 0 && (v = (v - P * iA) % P - ~~(v / P) * P)), Q._pt = T = new PQ(Q._pt, t, B, A, v, Pi), T.e = c, T.u = "deg", Q._props.push(B), T;
}, TA = function(Q, t) {
  for (var B in t)
    Q[B] = t[B];
  return Q;
}, di = function(Q, t, B) {
  var A = TA({}, B._gsap), e = "perspective,force3D,transformOrigin,svgOrigin", P = B.style, i, s, v, c, n, T, z, a;
  A.svg ? (v = B.getAttribute("transform"), B.setAttribute("transform", ""), P[V] = t, i = Dt(B, 1), gQ(B, V), B.setAttribute("transform", v)) : (v = getComputedStyle(B)[V], P[V] = t, i = Dt(B, 1), P[V] = v);
  for (s in OQ)
    v = A[s], c = i[s], v !== c && e.indexOf(s) < 0 && (z = _(v), a = _(c), n = z !== a ? pQ(B, s, v, a) : parseFloat(v), T = parseFloat(c), Q._pt = new PQ(Q._pt, i, s, n, T - n, EB), Q._pt.u = a || 0, Q._props.push(s));
  TA(i, A);
};
eQ("padding,margin,Width,Radius", function(r, Q) {
  var t = "Top", B = "Right", A = "Bottom", e = "Left", P = (Q < 3 ? [t, B, A, e] : [t + e, t + B, A + B, A + e]).map(function(i) {
    return Q < 2 ? r + i : "border" + i + r;
  });
  pt[Q > 1 ? "border" + r : r] = function(i, s, v, c, n) {
    var T, z;
    if (arguments.length < 4)
      return T = P.map(function(a) {
        return CQ(i, a, v);
      }), z = T.join(" "), z.split(T[0]).length === 5 ? T[0] : z;
    T = (c + "").split(" "), z = {}, P.forEach(function(a, o) {
      return z[a] = T[o] = T[o] || T[(o - 1) / 2 | 0];
    }), i.init(s, z, n);
  };
});
var Ee = {
  name: "css",
  register: lB,
  targetTest: function(Q) {
    return Q.style && Q.nodeType;
  },
  init: function(Q, t, B, A, e) {
    var P = this._props, i = Q.style, s = B.vars.startAt, v, c, n, T, z, a, o, w, E, l, u, U, k, O, D, N, d;
    gB || lB(), this.styles = this.styles || ce(Q), N = this.styles.props, this.tween = B;
    for (o in t)
      if (o !== "autoRound" && (c = t[o], !(cQ[o] && $A(o, t, B, A, Q, e)))) {
        if (z = typeof c, a = pt[o], z === "function" && (c = c.call(B, A, Q, e), z = typeof c), z === "string" && ~c.indexOf("random(") && (c = lt(c)), a)
          a(this, Q, o, c, B) && (D = 1);
        else if (o.substr(0, 2) === "--")
          v = (getComputedStyle(Q).getPropertyValue(o) + "").trim(), c += "", bQ.lastIndex = 0, bQ.test(v) || (w = _(v), E = _(c), E ? w !== E && (v = pQ(Q, o, v, E) + E) : w && (c += w)), this.add(i, "setProperty", v, c, A, e, 0, 0, o), P.push(o), N.push(o, 0, i[o]);
        else if (z !== "undefined") {
          if (s && o in s ? (v = typeof s[o] == "function" ? s[o].call(B, A, Q, e) : s[o], Z(v) && ~v.indexOf("random(") && (v = lt(v)), _(v + "") || v === "auto" || (v += zQ.units[o] || _(CQ(Q, o)) || ""), (v + "").charAt(1) === "=" && (v = CQ(Q, o))) : v = CQ(Q, o), T = parseFloat(v), l = z === "string" && c.charAt(1) === "=" && c.substr(0, 2), l && (c = c.substr(2)), n = parseFloat(c), o in UQ && (o === "autoAlpha" && (T === 1 && CQ(Q, "visibility") === "hidden" && n && (T = 0), N.push("visibility", 0, i.visibility), fQ(this, i, "visibility", T ? "inherit" : "hidden", n ? "inherit" : "hidden", !n)), o !== "scale" && o !== "transform" && (o = UQ[o], ~o.indexOf(",") && (o = o.split(",")[0]))), u = o in OQ, u) {
            if (this.styles.save(o), d = c, z === "string" && c.substring(0, 6) === "var(--") {
              if (c = TQ(Q, c.substring(4, c.indexOf(")"))), c.substring(0, 5) === "calc(") {
                var M = Q.style.perspective;
                Q.style.perspective = c, c = TQ(Q, "perspective"), M ? Q.style.perspective = M : gQ(Q, "perspective");
              }
              n = parseFloat(c);
            }
            if (U || (k = Q._gsap, k.renderTransform && !t.parseTransform || Dt(Q, t.parseTransform), O = t.smoothOrigin !== !1 && k.smooth, U = this._pt = new PQ(this._pt, i, V, 0, 1, k.renderTransform, k, 0, -1), U.dep = 1), o === "scale")
              this._pt = new PQ(this._pt, k, "scaleY", k.scaleY, (l ? $Q(k.scaleY, l + n) : n) - k.scaleY || 0, EB), this._pt.u = 0, P.push("scaleY", o), o += "X";
            else if (o === "transformOrigin") {
              N.push(iQ, 0, i[iQ]), c = li(c), k.svg ? kB(Q, c, 0, O, 0, this) : (E = parseFloat(c.split(" ")[2]) || 0, E !== k.zOrigin && fQ(this, k, "zOrigin", k.zOrigin, E), fQ(this, i, o, mt(v), mt(c)));
              continue;
            } else if (o === "svgOrigin") {
              kB(Q, c, 1, O, 0, this);
              continue;
            } else if (o in ze) {
              Di(this, k, o, T, l ? $Q(T, l + c) : c);
              continue;
            } else if (o === "smoothOrigin") {
              fQ(this, k, "smooth", k.smooth, c);
              continue;
            } else if (o === "force3D") {
              k[o] = c;
              continue;
            } else if (o === "transform") {
              di(this, c, Q);
              continue;
            }
          } else o in i || (o = it(o) || o);
          if (u || (n || n === 0) && (T || T === 0) && !ei.test(c) && o in i)
            w = (v + "").substr((T + "").length), n || (n = 0), E = _(c) || (o in zQ.units ? zQ.units[o] : w), w !== E && (T = pQ(Q, o, v, E)), this._pt = new PQ(this._pt, u ? k : i, o, T, (l ? $Q(T, l + n) : n) - T, !u && (E === "px" || o === "zIndex") && t.autoRound !== !1 ? si : EB), this._pt.u = E || 0, u && d !== c ? (this._pt.b = v, this._pt.e = d, this._pt.r = ri) : w !== E && E !== "%" && (this._pt.b = v, this._pt.r = ii);
          else if (o in i)
            hi.call(this, Q, o, v, l ? l + c : c);
          else if (o in Q)
            this.add(Q, o, v || Q[o], l ? l + c : c, A, e);
          else if (o !== "parseTransform") {
            NB(o, c);
            continue;
          }
          u || (o in i ? N.push(o, 0, i[o]) : typeof Q[o] == "function" ? N.push(o, 2, Q[o]()) : N.push(o, 1, v || Q[o])), P.push(o);
        }
      }
    D && Pe(this);
  },
  render: function(Q, t) {
    if (t.tween._time || !pB())
      for (var B = t._pt; B; )
        B.r(Q, B.d), B = B._next;
    else
      t.styles.revert();
  },
  get: CQ,
  aliases: UQ,
  getSetter: function(Q, t, B) {
    var A = UQ[t];
    return A && A.indexOf(",") < 0 && (t = A), t in OQ && t !== iQ && (Q._gsap.x || CQ(Q, "x")) ? B && PA === B ? t === "scale" ? oi : ni : (PA = B || {}) && (t === "scale" ? Ti : zi) : Q.style && !DB(Q.style[t]) ? vi : ~t.indexOf("-") ? ci : bB(Q, t);
  },
  core: {
    _removeProperty: gQ,
    _getMatrix: xB
  }
};
rQ.utils.checkPrefix = it;
rQ.core.getStyleSaver = ce;
(function(r, Q, t, B) {
  var A = eQ(r + "," + Q + "," + t, function(e) {
    OQ[e] = 1;
  });
  eQ(Q, function(e) {
    zQ.units[e] = "deg", ze[e] = 1;
  }), UQ[A[13]] = r + "," + Q, eQ(B, function(e) {
    var P = e.split(":");
    UQ[P[1]] = A[P[0]];
  });
})("x,y,z,scale,scaleX,scaleY,xPercent,yPercent", "rotation,rotationX,rotationY,skewX,skewY", "transform,transformOrigin,svgOrigin,force3D,smoothOrigin,transformPerspective", "0:translateX,1:translateY,2:translateZ,8:rotate,8:rotationZ,8:rotateZ,9:rotateX,10:rotateY");
eQ("x,y,z,top,right,bottom,left,width,height,fontSize,padding,margin,perspective", function(r) {
  zQ.units[r] = "px";
});
rQ.registerPlugin(Ee);
var jt = rQ.registerPlugin(Ee) || rQ;
jt.core.Tween;
const Ci = "", zA = 0.3, he = "#c9883d", Ni = "#eef1f4", aA = "#a855f7";
function Ct(r) {
  return Number.isFinite(r) ? Math.max(0, Math.min(1, r)) : 0;
}
function Oi(r) {
  const Q = Ct(r);
  return 1 - Math.pow(1 - Q, 3);
}
function Li(r) {
  const Q = Math.max(1e-3, r), t = Q * 1.6, B = Q * 0.7, A = new h.ConeGeometry(B, t, 3, 1, !1);
  return A.rotateX(-Math.PI / 2), A.translate(0, 0, t / 2), A;
}
function Mi(r) {
  const t = Math.max(1e-3, r) * 0.65, B = new h.SphereGeometry(t, 18, 12);
  return B.translate(0, 0, t), B;
}
function Hi(r, Q) {
  r.computeBoundingBox();
  const t = r.boundingBox, B = Math.max(1e-6, t.max.z - t.min.z), A = r.getAttribute("position"), e = new Float32Array(A.count * 2);
  for (let P = 0; P < A.count; P++) {
    const i = A.getX(P), s = A.getY(P), v = A.getZ(P);
    e[P * 2] = Math.atan2(s, i) / (2 * Math.PI) + 0.5, e[P * 2 + 1] = (v - t.min.z) / B * Q;
  }
  r.setAttribute("uv", new h.BufferAttribute(e, 2));
}
function le(r) {
  const Q = atob(Ci), t = new Uint8Array(Q.length);
  for (let v = 0; v < Q.length; v++) t[v] = Q.charCodeAt(v);
  const A = new Ne().parse(t.buffer);
  A.rotateX(-Math.PI / 2), A.computeBoundingBox();
  const e = A.boundingBox, P = new h.Vector3();
  e.getSize(P);
  const i = Math.max(1e-3, r) * 1.6 / Math.max(P.x, P.y, P.z, 1e-3);
  A.scale(i, i, i), A.computeBoundingBox();
  const s = A.boundingBox;
  return A.translate(
    -(s.min.x + s.max.x) / 2,
    -(s.min.y + s.max.y) / 2,
    -s.min.z
  ), Hi(A, 6), A.computeVertexNormals(), A;
}
let $t = null;
function ji() {
  if ($t) return $t;
  const r = 64, Q = 256, t = document.createElement("canvas");
  t.width = r, t.height = Q;
  const B = t.getContext("2d");
  B.fillStyle = "#808080", B.fillRect(0, 0, r, Q);
  const A = 10;
  for (let i = 0; i < A; i++) {
    const s = i / A * r, v = i % 2 === 0 ? 150 : 90;
    B.fillStyle = `rgb(${v},${v},${v})`, B.fillRect(s, 0, r / A / 2, Q);
  }
  const e = B.getImageData(0, 0, r, Q);
  for (let i = 0; i < e.data.length; i += 4) {
    const s = (Math.random() - 0.5) * 40;
    e.data[i] = Math.min(255, Math.max(0, e.data[i] + s)), e.data[i + 1] = e.data[i], e.data[i + 2] = e.data[i];
  }
  B.putImageData(e, 0, 0);
  const P = new h.CanvasTexture(t);
  return P.wrapS = h.RepeatWrapping, P.wrapT = h.RepeatWrapping, $t = P, P;
}
function fi(r, Q) {
  const t = Math.max(1e-3, r), B = t * 1.6, A = t * 0.08, e = t * 0.187, P = Ct(Q), i = new h.CylinderGeometry(0, A, B, 8, 1, !1);
  i.rotateX(-Math.PI / 2), i.translate(0, 0, B / 2);
  const s = new h.CylinderGeometry(0, e, B, 8, 1, !0);
  s.rotateX(-Math.PI / 2), s.translate(0, 0, B / 2);
  const v = new h.MeshBasicMaterial({
    color: new h.Color(aA),
    transparent: !0,
    opacity: P,
    depthTest: !1,
    depthWrite: !1,
    blending: h.AdditiveBlending
  }), c = new h.MeshBasicMaterial({
    color: new h.Color(aA),
    transparent: !0,
    opacity: P * 0.3,
    depthTest: !1,
    depthWrite: !1,
    blending: h.AdditiveBlending,
    side: h.DoubleSide
  }), n = new h.Mesh(i, v);
  n.renderOrder = 1e3;
  const T = new h.Mesh(s, c);
  T.renderOrder = 1e3;
  const z = new h.Group();
  return z.add(n), z.add(T), z;
}
function Fi(r) {
  const Q = Math.max(1e-3, r.bit.size);
  return r.bit.type === "triangle" ? Li(Q) : r.bit.type === "drill" ? le(Q) : Mi(Q);
}
function ke(r) {
  return r.bit.colorSource === "custom" ? r.bit.color : Ni;
}
function bi(r) {
  const Q = Ct(r.bit.opacity), t = le(Math.max(1e-3, r.bit.size)), B = new h.Color(ke(r)), A = ji(), e = new h.MeshStandardMaterial({
    color: B,
    metalness: 0.8,
    roughness: 0.35,
    roughnessMap: A,
    bumpMap: A,
    bumpScale: 0.03,
    // Self-illuminate slightly so the bit stays legible as a UI marker
    // regardless of viewing angle/lighting, matching the old visualizer's
    // brighter, always-visible look.
    emissive: B,
    emissiveIntensity: 0.12,
    transparent: Q < 1,
    opacity: Q,
    depthTest: !1,
    depthWrite: !1
  }), P = new h.Mesh(t, e);
  return P.renderOrder = 1e3, P;
}
function Gi(r) {
  const Q = Ct(r.bit.opacity), t = Fi(r), B = new h.MeshBasicMaterial({
    color: new h.Color(he),
    transparent: Q < 1,
    opacity: Q,
    side: h.DoubleSide,
    depthTest: !1,
    depthWrite: !1
  }), A = new h.Mesh(t, B);
  return A.renderOrder = 1e3, A;
}
function wA(r) {
  return r.bit.type === "laser" ? fi(r.bit.size, r.bit.opacity) : r.bit.type === "drill" ? bi(r) : Gi(r);
}
function EA(r) {
  if (r instanceof h.Mesh)
    r.geometry.dispose(), r.material.dispose();
  else if (r instanceof h.Group)
    for (const Q of r.children)
      Q instanceof h.Mesh && (Q.geometry.dispose(), Q.material.dispose());
}
function gi(r) {
  const Q = new h.Group();
  Q.name = "gviewer:bit-marker", Q.visible = !!r.bit.enabled;
  let t = r, B = wA(t);
  Q.add(B);
  let A = null, e = !1, P = null, i = null;
  const s = (n) => {
    const T = Math.max(1, n);
    return jt.to(B.rotation, {
      z: `+=${Math.PI * 2}`,
      duration: 60 / T,
      repeat: -1,
      ease: "none",
      paused: !0
    });
  };
  P = s(t.bit.spinRpm);
  const v = (n) => {
    const T = n.bit.type, z = t.bit.type, a = n.bit.size, o = t.bit.size, w = T !== z, E = a !== o, l = Ct(n.bit.opacity);
    if (w || E)
      Q.remove(B), EA(B), B = wA(n), Q.add(B), i == null || i.kill(), P == null || P.kill(), P = s(n.bit.spinRpm), e && P.timeScale(1).play();
    else if (T === "laser") {
      const U = B, [k, O] = U.children, D = k.material, N = O.material;
      D.opacity = l, D.needsUpdate = !0, N.opacity = l * 0.3, N.needsUpdate = !0;
    } else {
      const k = B.material, O = new h.Color(T === "drill" ? ke(n) : he);
      k.color = O, T === "drill" && (k.emissive = O), k.opacity = l, k.transparent = l < 1, k.needsUpdate = !0;
    }
    t = n, Q.visible = !!n.bit.enabled;
  };
  return {
    object: Q,
    setVisible: (n) => {
      Q.visible = !!n;
    },
    setOptions: (n) => v(n),
    setTarget: (n, T) => {
      const z = !!(T != null && T.immediate), a = Number(n.x) || 0, o = Number(n.y) || 0, w = Number(n.z) || 0;
      if (n.a, z || t.bit.tweenMs <= 0) {
        A = null, Q.position.set(a, o, w);
        return;
      }
      A = {
        startedAt: performance.now(),
        duration: Math.max(1, t.bit.tweenMs),
        from: { x: Q.position.x, y: Q.position.y, z: Q.position.z },
        to: { x: a, y: o, z: w }
      };
    },
    setSpinning: (n) => {
      const T = !!n;
      if (T !== e)
        if (e = T, i == null || i.kill(), e)
          P == null || P.play(), i = jt.to(P, {
            timeScale: 1,
            duration: zA,
            ease: "power1.out"
          });
        else {
          const z = P;
          i = jt.to(P, {
            timeScale: 0,
            duration: zA,
            ease: "power1.in",
            onComplete: () => z == null ? void 0 : z.pause()
          });
        }
    },
    update: (n) => {
      if (!A) return;
      const T = (n - A.startedAt) / A.duration, z = Oi(T);
      Q.position.set(
        h.MathUtils.lerp(A.from.x, A.to.x, z),
        h.MathUtils.lerp(A.from.y, A.to.y, z),
        h.MathUtils.lerp(A.from.z, A.to.z, z)
      ), T >= 1 && (Q.position.set(A.to.x, A.to.y, A.to.z), A = null);
    },
    dispose: () => {
      i == null || i.kill(), P == null || P.kill(), Q.remove(B), EA(B);
    }
  };
}
const pi = 25.4, mi = 500, xi = 300;
class Ki {
  constructor(Q) {
    this.viewCube = null, this.resizeObserver = null, this.onWindowResize = null, this.animationFrameId = null, this.gridGroup = null, this.axesGroup = null, this.gridLabelsGroup = null, this.boundingBoxGroup = null, this.machineBedGroup = null, this.bitMarker = null, this.preLaserBitType = "drill", this.toolpathStreams = [], this.toolpathCutBucketCount = 1, this.toolpathRotationA = 0, this.lastBitPosition = { x: 0, y: 0, z: 0, a: 0 }, this.sim3dHandle = null, this.currentLines = [], this.linePositions = null, this.renderSequence = 0, this.currentBounds = null, this.cameraFocusTransition = null, this.cameraFollowRequested = !1, this.cameraFollowInterrupted = !1, this.cameraFollowOffset = null, this.id = Q.id, this.container = Q.container, this.callbacks = Q.callbacks ?? {}, this.options = lA(be, Q.options), this.canvas = document.createElement("canvas"), this.canvas.style.width = "100%", this.canvas.style.height = "100%", this.canvas.style.display = "block", this.container.appendChild(this.canvas), _e(this.container), this.renderer = new h.WebGLRenderer({
      canvas: this.canvas,
      antialias: this.options.render.antialias
    }), this.renderer.setPixelRatio(window.devicePixelRatio), this.renderer.setClearColor(this.options.render.theme.background, 1), this.scene = new h.Scene(), this.scene.add(new h.AmbientLight(16777215, 0.8));
    const t = new h.DirectionalLight(16777215, 0.9);
    t.position.set(1, -1, 1.5), this.scene.add(t);
    const B = new h.DirectionalLight(16777215, 0.5);
    B.position.set(-1, 1, 1), this.scene.add(B), this.toolpathRoot = new h.Group(), this.toolpathRoot.name = "gviewer:toolpath-root", this.scene.add(this.toolpathRoot), this.camera = new h.PerspectiveCamera(this.options.camera.fov, 1, 0.1, 1e5), this.camera.up.set(0, 0, 1), this.camera.position.set(
      this.options.camera.initialPosition.x,
      this.options.camera.initialPosition.y,
      this.options.camera.initialPosition.z
    ), this.controls = new qe(this.camera, this.renderer.domElement), this.controls.enableDamping = this.options.camera.orbit.enableDamping, this.controls.addEventListener("start", () => {
      this.cameraFollowRequested && (this.cameraFollowInterrupted = !0);
    }), this.controls.update(), this.viewCubeCorrection = new h.Matrix4().makeRotationX(h.MathUtils.degToRad(90)), this.viewCube = new Ye({
      container: this.container,
      onSelectView: (A) => {
        const e = new h.Vector3();
        let P;
        if (this.currentBounds) {
          this.currentBounds.getCenter(e);
          const i = new h.Vector3();
          this.currentBounds.getSize(i);
          const s = Math.max(i.x, i.y, 1) / 2, v = h.MathUtils.degToRad(this.camera.fov);
          P = s / Math.tan(v / 2) * 1.25 + i.z;
        } else
          P = this.camera.position.distanceTo(this.controls.target);
        this.startSnapToView(A, e, Math.max(1e-6, P), 400);
      }
    }), typeof ResizeObserver < "u" ? (this.resizeObserver = new ResizeObserver(() => this.resize()), this.resizeObserver.observe(this.container)) : (this.onWindowResize = () => this.resize(), window.addEventListener("resize", this.onWindowResize)), this.renderGridAndAxes(), this.refreshGridLabels(), this.refreshBoundingBox(), this.refreshMachineBed(), this.ensureBitMarker(), this.resize(), this.startAnimationLoop();
  }
  setBitPosition(Q, t) {
    var B;
    if (this.ensureBitMarker(), this.lastBitPosition = { ...this.lastBitPosition, ...Q }, (B = this.bitMarker) == null || B.setTarget(Q, t), this.cameraFollowRequested && !this.cameraFollowInterrupted && this.cameraFollowOffset) {
      const A = new h.Vector3(this.lastBitPosition.x, this.lastBitPosition.y, this.controls.target.z), e = A.clone().add(this.cameraFollowOffset);
      this.startCameraLerp(A, e, xi, "linear");
    }
  }
  /**
   * Enable or disable "follow tool" camera tracking. While enabled, every
   * subsequent setBitPosition() call pans the camera+target in X/Y to keep
   * the tool centered, holding the camera→target offset captured at engage
   * time constant — which is what preserves viewing angle and height. A
   * user-initiated camera drag (OrbitControls "start") interrupts an active
   * session; it only re-arms on the next disabled→enabled edge.
   */
  setCameraFollowEnabled(Q) {
    if (Q === this.cameraFollowRequested)
      return;
    if (this.cameraFollowRequested = Q, this.cameraFollowInterrupted = !1, !Q) {
      this.cameraFollowOffset = null;
      return;
    }
    this.cameraFollowOffset = this.camera.position.clone().sub(this.controls.target);
    const t = new h.Vector3(this.lastBitPosition.x, this.lastBitPosition.y, this.controls.target.z), B = t.clone().add(this.cameraFollowOffset);
    this.startCameraLerp(t, B, mi);
  }
  setBitVisible(Q) {
    var t;
    this.ensureBitMarker(), (t = this.bitMarker) == null || t.setVisible(Q);
  }
  setBitSpinning(Q) {
    var t;
    this.ensureBitMarker(), (t = this.bitMarker) == null || t.setSpinning(Q);
  }
  setToolpathRotationA(Q) {
    const t = Number(Q) || 0;
    this.toolpathRotationA = t, this.toolpathRoot.rotation.x = h.MathUtils.degToRad(t);
  }
  setCallbacks(Q) {
    this.callbacks = Q;
  }
  hideUntilLine(Q, t) {
    const B = t ?? this.options.progress.mode, A = Math.floor(Q);
    for (const e of this.toolpathStreams) {
      const P = A < 0 ? 0 : e.prefixEndVertex[Math.min(A, e.prefixEndVertex.length - 1)];
      B === "grey" ? (e.line.geometry.setDrawRange(0, e.totalVertices), je({ stream: e, nextCursorVertex: P, options: this.options })) : e.line.geometry.setDrawRange(P, Math.max(0, e.totalVertices - P));
    }
  }
  seekToLine(Q, t) {
    if (this.hideUntilLine(Q, t), this.linePositions && this.currentLines.length > 0) {
      const B = Math.max(0, Math.min(Math.floor(Q), this.currentLines.length - 1)), A = {
        x: this.linePositions[B * 4],
        y: this.linePositions[B * 4 + 1],
        z: this.linePositions[B * 4 + 2],
        a: this.linePositions[B * 4 + 3]
      };
      this.setBitPosition(A, { immediate: !0 });
    }
    if (this.sim3dHandle) {
      const B = Math.max(0, Math.min(Math.floor(Q), this.currentLines.length - 1));
      if (B !== this.sim3dHandle.currentLine) {
        const A = Yt(B, this.sim3dHandle.data);
        St(A, this.sim3dHandle.data.resolution, this.options.sim3d.erosionPasses, this.sim3dHandle.data.slabBounds.zTop), Vt(this.sim3dHandle.slab, A), this.sim3dHandle.currentLine = B;
      }
    }
  }
  showAll() {
    for (const Q of this.toolpathStreams)
      Q.line.geometry.setDrawRange(0, Q.totalVertices);
  }
  /**
   * Show or hide one of the `lineGroups` the toolpath was loaded with.
   *
   * This is deliberately independent of `showAll()`, `hideUntilLine()` and
   * `resetColors()`, which are all about *progress*: they move draw ranges and
   * colors, while this flips object visibility. So the two compose in either
   * order - but note a hidden group therefore stays hidden across a `showAll()`,
   * which looks surprising if you expect "show all" to mean everything. Call
   * `showAllLineGroups()` for that.
   *
   * A no-op when the load passed no `lineGroups`, or for the catch-all stream of
   * lines outside every group, which has no index and is always visible.
   */
  setLineGroupVisible(Q, t) {
    for (const B of this.toolpathStreams)
      B.lineGroupIndex === Q && (B.line.visible = t);
  }
  showAllLineGroups() {
    for (const Q of this.toolpathStreams)
      Q.line.visible = !0;
  }
  resetColors() {
    for (const Q of this.toolpathStreams) {
      Q.greyCursorVertex = 0, Q.simColors.set(Q.baseColors);
      const t = Q.line.geometry.getAttribute("color");
      t.clearUpdateRanges(), t.addUpdateRange(0, Q.simColors.length), t.needsUpdate = !0;
    }
  }
  snapCameraToView(Q, t = {}) {
    const B = Math.max(0, Math.floor(t.durationMs ?? 240)), A = this.controls.target.clone(), e = this.camera.position.distanceTo(A), P = Math.max(1e-6, t.distance ?? e);
    this.startSnapToView(Q, A, P, B);
  }
  /**
   * Enable or disable orbit *rotation* while leaving pan and zoom untouched.
   *
   * Used to pin the camera to a fixed view (e.g. top-down) so a press-and-hold
   * gesture is not interpreted as a rotate-drag and the picked plane stays
   * stable under the cursor. Picking (screenToWorld) works from any camera
   * orientation, so this is a UX lock, not a correctness requirement.
   */
  setRotateEnabled(Q) {
    this.controls.enableRotate = Q;
  }
  /**
   * Convert a viewport pixel (clientX/clientY, e.g. from a PointerEvent) into a
   * point on a horizontal toolpath plane, in the scene's coordinate space.
   *
   * A ray is cast from the camera through the pixel and intersected with the
   * plane Z = `planeZ`, which defaults to the bit's current Z so the pick lands
   * on the plane the tool is sitting on. Returns null when the pixel is outside
   * a laid-out canvas or the ray is parallel to the plane (no intersection).
   *
   * The result is in world/scene space. The toolpath root sits at the origin,
   * so for a normal (non-rotary) file this equals the gcode work coordinate.
   * For a rotary file the toolpath root is rotated about X, so the returned XY
   * is not a meaningful work coordinate — callers supporting only XY moves
   * should gate on file type.
   */
  screenToWorld(Q, t, B) {
    const A = this.canvas.getBoundingClientRect();
    if (A.width === 0 || A.height === 0)
      return null;
    const e = new h.Vector2(
      (Q - A.left) / A.width * 2 - 1,
      -((t - A.top) / A.height) * 2 + 1
    ), P = new h.Raycaster();
    P.setFromCamera(e, this.camera);
    const i = (B == null ? void 0 : B.planeZ) ?? this.lastBitPosition.z ?? 0, s = new h.Plane(new h.Vector3(0, 0, 1), -i), v = new h.Vector3();
    return P.ray.intersectPlane(s, v) ? { x: v.x, y: v.y, z: v.z } : null;
  }
  /**
   * Project a point in world/scene space to a viewport pixel (clientX/clientY),
   * the inverse of screenToWorld and in the same coordinate space it consumes.
   *
   * The point is projected through the camera to normalized device coordinates
   * and mapped back onto the laid-out canvas rect, so overlays drawn in fixed
   * (viewport) coordinates line up with picked scene points and track pan/zoom.
   *
   * `z` is the point's height in scene space (default 0); it changes the
   * projected pixel under the perspective camera, so callers placing markers
   * off the Z=0 plane should pass it. Returns null when the canvas has no layout.
   */
  worldToScreen(Q, t, B = 0) {
    const A = this.canvas.getBoundingClientRect();
    if (A.width === 0 || A.height === 0)
      return null;
    const e = new h.Vector3(Q, t, B).project(this.camera);
    return {
      x: A.left + (e.x + 1) / 2 * A.width,
      y: A.top + (1 - e.y) / 2 * A.height
    };
  }
  startSnapToView(Q, t, B, A) {
    const e = KB(Q), P = t.clone().add(e.multiplyScalar(B)), i = Math.max(1, B);
    this.camera.near = i / 1e3, this.camera.far = i * 50, this.camera.updateProjectionMatrix(), this.startCameraLerp(t, P, A);
  }
  // Shared tween kickoff for any camera position+target move (view snapping,
  // model focus, tool-follow engage/track): saves/restores damping around the
  // transition and hands off to updateCameraFocusTransition() each frame.
  startCameraLerp(Q, t, B, A = "easeInOutCubic") {
    this.cameraFocusTransition && (this.controls.enableDamping = this.cameraFocusTransition.dampingEnabled), this.cameraFocusTransition = {
      startedAt: performance.now(),
      duration: B,
      fromPosition: this.camera.position.clone(),
      toPosition: t,
      fromTarget: this.controls.target.clone(),
      toTarget: Q,
      dampingEnabled: this.controls.enableDamping,
      easing: A
    }, this.controls.enableDamping = !1;
  }
  async loadFromUrl(Q, t = {}) {
    this.emitProgress({ state: "indeterminate", label: "Loading file..." });
    const B = await fetch(Q, { signal: t.signal });
    if (!B.ok)
      throw this.emitProgress({ state: "hidden" }), new Error(`Failed to load gcode: ${B.statusText}`);
    const A = await B.text();
    await this.loadFromText(A);
  }
  async loadFromFile(Q) {
    this.emitProgress({ state: "indeterminate", label: "Loading file..." });
    const t = await Q.text();
    await this.loadFromText(t);
  }
  async loadFromText(Q) {
    await this.loadFromLines(Q.split(/\r?\n/));
  }
  async loadFromLines(Q) {
    this.currentLines = Array.from(Q), await this.renderScene();
  }
  /**
   * Pass `options.lineGroups` to split the toolpath into separately hideable
   * streams by source-line range; `setLineGroupVisible` then shows or hides a
   * group. Without it the toolpath is one rapid + one cut stream, exactly as
   * before, and only a prefix can be hidden (`hideUntilLine`).
   */
  async loadFromWorkerData(Q, t) {
    this.currentLines = [];
    const { rapids: B, cuts: A } = Ue(Q, t), e = (Q.toolchangeCount ?? 0) > 0;
    this.setToolpathGeometry({
      rapids: B.map((P) => ({
        positions: P.positions,
        colors: void 0,
        prefixEndVertex: P.prefixEndVertex,
        lineGroupIndex: P.lineGroupIndex
      })),
      cuts: A.map((P) => ({
        positions: P.positions,
        colors: e ? P.colors : void 0,
        prefixEndVertex: P.prefixEndVertex,
        // Line groups are not laser-power buckets: every cut stream here sits in
        // bucket 0, so splitting by group leaves opacity untouched.
        cutBucketIndex: 0,
        lineGroupIndex: P.lineGroupIndex
      })),
      cutBucketCount: 1
    });
  }
  unload() {
    this.currentLines = [], this.setGeometryEmpty(), this.emitProgress({ state: "hidden" });
  }
  setOptions(Q) {
    var T, z, a, o, w, E, l, u, U, k, O, D, N, d, M, G, b, C, j, f, H, F, I, R, QQ, sQ, W, x, tQ, vQ, rt, yB, IB, YB;
    const t = this.options;
    this.options = lA(this.options, Q), (t.render.antialias !== this.options.render.antialias || t.render.theme.background !== this.options.render.theme.background) && this.renderer.setClearColor(this.options.render.theme.background, 1), (t.render.theme.opacity !== this.options.render.theme.opacity || t.render.theme.rapidOpacity !== this.options.render.theme.rapidOpacity) && this.refreshToolpathOpacities(), t.camera.fov !== this.options.camera.fov && (this.camera.fov = this.options.camera.fov, this.camera.updateProjectionMatrix()), t.camera.orbit.enableDamping !== this.options.camera.orbit.enableDamping && (this.controls.enableDamping = this.options.camera.orbit.enableDamping);
    const B = ((T = t.grid.bounds) == null ? void 0 : T.min.x) !== ((z = this.options.grid.bounds) == null ? void 0 : z.min.x) || ((a = t.grid.bounds) == null ? void 0 : a.min.y) !== ((o = this.options.grid.bounds) == null ? void 0 : o.min.y) || ((w = t.grid.bounds) == null ? void 0 : w.max.x) !== ((E = this.options.grid.bounds) == null ? void 0 : E.max.x) || ((l = t.grid.bounds) == null ? void 0 : l.max.y) !== ((u = this.options.grid.bounds) == null ? void 0 : u.max.y), A = t.units !== this.options.units || t.grid.sizeX !== this.options.grid.sizeX || t.grid.sizeY !== this.options.grid.sizeY || t.grid.axisDepth !== this.options.grid.axisDepth || B, e = t.render.theme.colors.grid.major !== this.options.render.theme.colors.grid.major || t.render.theme.colors.grid.minor !== this.options.render.theme.colors.grid.minor || t.render.theme.colors.axes.x !== this.options.render.theme.colors.axes.x || t.render.theme.colors.axes.y !== this.options.render.theme.colors.axes.y || t.render.theme.colors.axes.z !== this.options.render.theme.colors.axes.z;
    (A || e) && this.renderGridAndAxes(), (t.grid.labels !== this.options.grid.labels || t.units !== this.options.units || t.grid.sizeX !== this.options.grid.sizeX || t.grid.sizeY !== this.options.grid.sizeY || B || e) && this.refreshGridLabels(), (t.boundingBox.visible !== this.options.boundingBox.visible || t.boundingBox.labels !== this.options.boundingBox.labels || t.units !== this.options.units || t.render.theme.colors.boundingBox !== this.options.render.theme.colors.boundingBox) && this.refreshBoundingBox(), (t.machineBed.visible !== this.options.machineBed.visible || ((U = t.machineBed.min) == null ? void 0 : U.x) !== ((k = this.options.machineBed.min) == null ? void 0 : k.x) || ((O = t.machineBed.min) == null ? void 0 : O.y) !== ((D = this.options.machineBed.min) == null ? void 0 : D.y) || ((N = t.machineBed.max) == null ? void 0 : N.x) !== ((d = this.options.machineBed.max) == null ? void 0 : d.x) || ((M = t.machineBed.max) == null ? void 0 : M.y) !== ((G = this.options.machineBed.max) == null ? void 0 : G.y) || ((C = (b = t.machineBed.keepout) == null ? void 0 : b.min) == null ? void 0 : C.x) !== ((f = (j = this.options.machineBed.keepout) == null ? void 0 : j.min) == null ? void 0 : f.x) || ((F = (H = t.machineBed.keepout) == null ? void 0 : H.min) == null ? void 0 : F.y) !== ((R = (I = this.options.machineBed.keepout) == null ? void 0 : I.min) == null ? void 0 : R.y) || ((sQ = (QQ = t.machineBed.keepout) == null ? void 0 : QQ.max) == null ? void 0 : sQ.x) !== ((x = (W = this.options.machineBed.keepout) == null ? void 0 : W.max) == null ? void 0 : x.x) || ((vQ = (tQ = t.machineBed.keepout) == null ? void 0 : tQ.max) == null ? void 0 : vQ.y) !== ((yB = (rt = this.options.machineBed.keepout) == null ? void 0 : rt.max) == null ? void 0 : yB.y) || t.render.theme.colors.machineBed !== this.options.render.theme.colors.machineBed || t.render.theme.colors.machineBedKeepout !== this.options.render.theme.colors.machineBedKeepout) && this.refreshMachineBed();
    const v = t.render.theme.colors.rapid !== this.options.render.theme.colors.rapid || t.render.theme.colors.cutting !== this.options.render.theme.colors.cutting || t.render.theme.colors.laser !== this.options.render.theme.colors.laser || t.render.theme.background !== this.options.render.theme.background || t.render.theme.colors.processed !== this.options.render.theme.colors.processed;
    if (v && this.refreshToolpathColors(), (t.bit.enabled !== this.options.bit.enabled || t.bit.type !== this.options.bit.type || t.bit.size !== this.options.bit.size || t.bit.opacity !== this.options.bit.opacity || t.bit.colorSource !== this.options.bit.colorSource || t.bit.color !== this.options.bit.color || v) && (this.ensureBitMarker(), (IB = this.bitMarker) == null || IB.setOptions(this.options)), t.mode.laser !== this.options.mode.laser && (this.options.mode.laser ? this.options.bit.type !== "laser" && (this.preLaserBitType = this.options.bit.type, this.options = { ...this.options, bit: { ...this.options.bit, type: "laser" } }) : this.options = { ...this.options, bit: { ...this.options.bit, type: this.preLaserBitType } }, this.ensureBitMarker(), (YB = this.bitMarker) == null || YB.setOptions(this.options), this.currentLines.length > 0 && this.renderScene()), t.mode.sim3d !== this.options.mode.sim3d)
      if (!this.options.mode.sim3d)
        this.setSim3dHandle(null), this.setToolpathStreamsVisible(!0);
      else {
        this.renderScene();
        return;
      }
    if ((t.sim3d.toolDiameter !== this.options.sim3d.toolDiameter || t.sim3d.resolution !== this.options.sim3d.resolution) && this.options.mode.sim3d) {
      this.renderScene();
      return;
    }
    if (t.sim3d.erosionPasses !== this.options.sim3d.erosionPasses && this.sim3dHandle) {
      const ue = this.sim3dHandle.currentLine, SB = Yt(ue, this.sim3dHandle.data);
      St(SB, this.sim3dHandle.data.resolution, this.options.sim3d.erosionPasses, this.sim3dHandle.data.slabBounds.zTop), Vt(this.sim3dHandle.slab, SB);
    }
    t.sim3d.showToolpath !== this.options.sim3d.showToolpath && this.options.mode.sim3d && this.setToolpathStreamsVisible(this.options.sim3d.showToolpath);
  }
  getOptions() {
    return this.options;
  }
  resize() {
    const Q = this.container.getBoundingClientRect(), t = Math.max(
      1,
      Math.floor(Q.width || this.container.clientWidth || window.innerWidth)
    ), B = Math.max(
      1,
      Math.floor(Q.height || this.container.clientHeight || window.innerHeight)
    );
    this.renderer.setSize(t, B, !1), this.camera.aspect = t / B, this.camera.updateProjectionMatrix();
  }
  focusToModel() {
    this.currentBounds && this.startCameraFocus(this.currentBounds);
  }
  resetCamera() {
    this.cameraFocusTransition = null, this.controls.enableDamping = this.options.camera.orbit.enableDamping, this.controls.target.set(0, 0, 0), this.camera.position.set(
      this.options.camera.initialPosition.x,
      this.options.camera.initialPosition.y,
      this.options.camera.initialPosition.z
    ), this.controls.update();
  }
  getBounds() {
    return this.currentBounds ? {
      min: {
        x: this.currentBounds.min.x,
        y: this.currentBounds.min.y,
        z: this.currentBounds.min.z
      },
      max: {
        x: this.currentBounds.max.x,
        y: this.currentBounds.max.y,
        z: this.currentBounds.max.z
      }
    } : null;
  }
  dispose() {
    var Q;
    this.renderSequence += 1, this.animationFrameId !== null && (cancelAnimationFrame(this.animationFrameId), this.animationFrameId = null), this.resizeObserver && (this.resizeObserver.disconnect(), this.resizeObserver = null), this.onWindowResize && (window.removeEventListener("resize", this.onWindowResize), this.onWindowResize = null), this.setGeometryEmpty(), this.setGridGroup(null), this.setAxesGroup(null), this.setGridLabelsGroup(null), this.setBoundingBoxGroup(null), this.setMachineBedGroup(null), this.setBitMarker(null), this.controls.dispose(), this.renderer.dispose(), (Q = this.viewCube) == null || Q.dispose(), this.viewCube = null, this.container.removeChild(this.canvas);
  }
  emitProgress(Q) {
    var t, B;
    (B = (t = this.callbacks).onProgress) == null || B.call(t, { id: this.id, ...Q });
  }
  emitBoundsChanged() {
    var Q, t;
    (t = (Q = this.callbacks).onBoundsChanged) == null || t.call(Q, { id: this.id, bounds: this.getBounds() });
  }
  startAnimationLoop() {
    const Q = () => {
      var B;
      this.animationFrameId = requestAnimationFrame(Q);
      const t = performance.now();
      this.controls.update(), this.updateCameraFocusTransition(), this.updateViewCubeRotation(), (B = this.bitMarker) == null || B.update(t), this.renderer.render(this.scene, this.camera);
    };
    Q();
  }
  ensureBitMarker() {
    this.bitMarker || (this.bitMarker = gi(this.options), this.scene.add(this.bitMarker.object));
  }
  worldSizes() {
    const Q = this.options.units === "in" ? pi : 1;
    return {
      sizeXMm: Math.max(1, this.options.grid.sizeX * Q),
      sizeYMm: Math.max(1, this.options.grid.sizeY * Q),
      axisDepthMm: Math.max(1, this.options.grid.axisDepth * Q),
      // Bounds are literal absolute mm (unlike sizeX/sizeY), so they don't
      // go through the units scale round-trip above.
      bounds: this.options.grid.bounds ?? null
    };
  }
  renderGridAndAxes() {
    const { sizeXMm: Q, sizeYMm: t, axisDepthMm: B, bounds: A } = this.worldSizes();
    this.setGridGroup(
      tP({
        units: this.options.units,
        sizeXMm: Q,
        sizeYMm: t,
        bounds: A,
        theme: this.options.render.theme
      })
    ), this.setAxesGroup(
      AP({
        sizeXWorld: Q,
        sizeYWorld: t,
        depthWorld: B,
        bounds: A,
        theme: this.options.render.theme
      })
    );
  }
  refreshGridLabels() {
    if (!this.options.grid.labels) {
      this.setGridLabelsGroup(null);
      return;
    }
    const { sizeXMm: Q, sizeYMm: t, bounds: B } = this.worldSizes();
    this.setGridLabelsGroup(
      PP({ sizeXMm: Q, sizeYMm: t, bounds: B, units: this.options.units, theme: this.options.render.theme })
    );
  }
  refreshBoundingBox() {
    if (!this.options.boundingBox.visible || !this.currentBounds) {
      this.setBoundingBoxGroup(null);
      return;
    }
    this.setBoundingBoxGroup(Je(this.currentBounds, this.options));
  }
  refreshMachineBed() {
    const { visible: Q, min: t, max: B, keepout: A } = this.options.machineBed;
    if (!Q || !t || !B) {
      this.setMachineBedGroup(null);
      return;
    }
    this.setMachineBedGroup(Ze(t, B, this.options, A));
  }
  setBitMarker(Q) {
    this.bitMarker && (this.scene.remove(this.bitMarker.object), this.bitMarker.dispose()), this.bitMarker = Q, this.bitMarker && this.scene.add(this.bitMarker.object);
  }
  setGridGroup(Q) {
    this.gridGroup && (this.scene.remove(this.gridGroup), BP(this.gridGroup), this.gridGroup = null), Q && (this.gridGroup = Q, this.scene.add(Q));
  }
  setAxesGroup(Q) {
    this.axesGroup && (this.scene.remove(this.axesGroup), eP(this.axesGroup), this.axesGroup = null), Q && (this.axesGroup = Q, this.scene.add(Q));
  }
  setGridLabelsGroup(Q) {
    this.gridLabelsGroup && (this.scene.remove(this.gridLabelsGroup), Se(this.gridLabelsGroup), this.gridLabelsGroup = null), Q && (this.gridLabelsGroup = Q, this.scene.add(Q));
  }
  setBoundingBoxGroup(Q) {
    this.boundingBoxGroup && (this.scene.remove(this.boundingBoxGroup), Xe(this.boundingBoxGroup), this.boundingBoxGroup = null), Q && (this.boundingBoxGroup = Q, this.scene.add(Q));
  }
  setMachineBedGroup(Q) {
    this.machineBedGroup && (this.scene.remove(this.machineBedGroup), We(this.machineBedGroup), this.machineBedGroup = null), Q && (this.machineBedGroup = Q, this.scene.add(Q));
  }
  setGeometryEmpty() {
    this.setSim3dHandle(null), Le(this.toolpathRoot, this.toolpathStreams), this.toolpathStreams = [], this.toolpathCutBucketCount = 1, this.linePositions = null, this.currentBounds = null, this.emitBoundsChanged(), this.refreshBoundingBox();
  }
  setToolpathGeometry(Q) {
    this.setGeometryEmpty(), this.toolpathCutBucketCount = Math.max(1, Math.floor(Q.cutBucketCount));
    const t = [
      ...Q.rapids.map((e) => ({
        kind: "rapid",
        cutBucketIndex: null,
        lineGroupIndex: e.lineGroupIndex ?? null,
        positions: e.positions,
        prefixEndVertex: e.prefixEndVertex,
        colors: e.colors,
        opacity: hA(this.options.render.theme.rapidOpacity ?? 0.3)
      })),
      ...Q.cuts.map((e, P) => {
        const i = e.cutBucketIndex ?? P;
        return {
          kind: "cut",
          cutBucketIndex: i,
          lineGroupIndex: e.lineGroupIndex ?? null,
          positions: e.positions,
          prefixEndVertex: e.prefixEndVertex,
          colors: e.colors,
          opacity: this.cutBucketOpacity(i)
        };
      })
    ], { streams: B, bounds: A } = Oe({
      specs: t,
      options: this.options,
      scene: this.toolpathRoot
    });
    this.toolpathStreams = B, this.currentBounds = A ? A.clone() : null, this.emitBoundsChanged(), this.refreshBoundingBox(), this.setToolpathRotationA(this.toolpathRotationA);
  }
  refreshToolpathColors() {
    Me(this.toolpathStreams, this.options);
  }
  refreshToolpathOpacities() {
    He({
      streams: this.toolpathStreams,
      options: this.options,
      cutBucketCount: this.toolpathCutBucketCount
    });
  }
  cutBucketOpacity(Q) {
    const t = this.options.render.theme.opacity, B = this.toolpathCutBucketCount;
    if (B <= 1)
      return t;
    const A = Q / (B - 1);
    return hA(t * A);
  }
  buildLinePositions() {
    const Q = this.currentLines.length;
    this.linePositions = new Float32Array(Q * 4);
    let t = 0, B = 0, A = 0, e = 0;
    const P = new eB({
      onLinearMove: (i) => {
        const s = i.transformedEnd ?? i.end;
        t = s.X, B = s.Y, A = s.Z, e = i.end.A;
      },
      onArcMove: (i) => {
        const s = i.transformedEnd ?? i.end;
        t = s.X, B = s.Y, A = s.Z, e = i.end.A;
      }
    });
    for (let i = 0; i < Q; i++) {
      const s = this.currentLines[i];
      s && P.processLine(s), this.linePositions[i * 4] = t, this.linePositions[i * 4 + 1] = B, this.linePositions[i * 4 + 2] = A, this.linePositions[i * 4 + 3] = e;
    }
  }
  async renderScene() {
    if (this.currentLines.length === 0) {
      this.setGeometryEmpty(), this.emitProgress({ state: "hidden" });
      return;
    }
    const Q = this.renderSequence += 1;
    try {
      this.options.mode.laser ? this.emitProgress({ state: "indeterminate", label: "Scanning power..." }) : this.emitProgress({
        state: "determinate",
        label: "Building geometry...",
        processed: 0,
        total: this.currentLines.length
      });
      const t = await De(this.currentLines, {
        arcSegments: this.options.geometry.arcSegments,
        bucketCount: 16,
        laserMode: this.options.mode.laser,
        batch: {
          everyLines: this.options.geometry.batching.progressEveryLines,
          yieldEveryLines: this.options.geometry.batching.yieldEveryLines,
          shouldAbort: () => Q !== this.renderSequence,
          onProgress: (B, A) => {
            Q === this.renderSequence && this.emitProgress({ state: "determinate", label: "Building geometry...", processed: B, total: A });
          }
        }
      });
      if (Q !== this.renderSequence)
        return;
      this.setToolpathGeometry({
        rapids: [t.rapid],
        cuts: t.cuts,
        cutBucketCount: t.cutBucketCount
      }), this.buildLinePositions(), this.options.mode.sim3d && (this.setToolpathStreamsVisible(this.options.sim3d.showToolpath), await this.buildAndApplySim3d(Q));
    } catch (t) {
      if (t instanceof Error && t.message === "Aborted.")
        return;
      throw t;
    } finally {
      Q === this.renderSequence && this.emitProgress({ state: "hidden" });
    }
  }
  setSim3dHandle(Q) {
    this.sim3dHandle && (this.scene.remove(this.sim3dHandle.slab.group), xe(this.sim3dHandle.slab), this.sim3dHandle = null), Q && (this.sim3dHandle = Q, this.scene.add(Q.slab.group));
  }
  // Whole-toolpath visibility for sim3d. Shares `line.visible` with
  // setLineGroupVisible, so turning the toolpath back on here also un-hides any
  // hidden line groups; sim3d and grouped loads are not used together today.
  setToolpathStreamsVisible(Q) {
    for (const t of this.toolpathStreams)
      t.line.visible = Q;
  }
  async buildAndApplySim3d(Q) {
    this.emitProgress({ state: "indeterminate", label: "Building 3D simulation..." });
    const t = this.options.sim3d.toolDiameter / 2, B = this.options.sim3d.resolution;
    let A;
    try {
      A = await Ge(this.currentLines, t, B, {
        arcSegments: this.options.geometry.arcSegments,
        batch: {
          shouldAbort: () => Q !== this.renderSequence,
          onProgress: (i, s) => {
            Q === this.renderSequence && this.emitProgress({
              state: "determinate",
              label: "Building 3D simulation...",
              processed: i,
              total: s
            });
          },
          yieldEveryLines: this.options.geometry.batching.yieldEveryLines
        }
      });
    } catch {
      return;
    }
    if (Q !== this.renderSequence) return;
    const e = Yt(0, A);
    St(e, B, this.options.sim3d.erosionPasses, A.slabBounds.zTop);
    const P = me(A.slabBounds, B);
    Vt(P, e), this.setSim3dHandle({ data: A, slab: P, currentLine: 0 });
  }
  startCameraFocus(Q) {
    const t = new h.Vector3();
    Q.getCenter(t);
    const B = new h.Vector3();
    Q.getSize(B);
    const A = Math.max(B.x, B.y, 1) / 2, e = h.MathUtils.degToRad(this.camera.fov), P = A / Math.tan(e / 2) * 1.25 + B.z, i = KB("front-top-left"), s = t.clone().add(i.multiplyScalar(P)), v = Math.max(B.x, B.y, B.z, 1);
    this.camera.near = v / 1e3, this.camera.far = v * 50, this.camera.updateProjectionMatrix(), this.startCameraLerp(t, s, this.options.camera.focusDurationMs);
  }
  updateCameraFocusTransition() {
    if (!this.cameraFocusTransition)
      return;
    const t = performance.now() - this.cameraFocusTransition.startedAt, B = Math.min(1, Math.max(0, t / this.cameraFocusTransition.duration)), A = this.cameraFocusTransition.easing === "linear" ? B : $e(B);
    if (this.camera.position.lerpVectors(this.cameraFocusTransition.fromPosition, this.cameraFocusTransition.toPosition, A), this.controls.target.lerpVectors(this.cameraFocusTransition.fromTarget, this.cameraFocusTransition.toTarget, A), B >= 1) {
      const e = this.cameraFocusTransition.dampingEnabled;
      this.camera.position.copy(this.cameraFocusTransition.toPosition), this.controls.target.copy(this.cameraFocusTransition.toTarget), this.cameraFocusTransition = null, this.controls.enableDamping = !1, this.controls.update(), this.controls.enableDamping = e;
    }
  }
  updateViewCubeRotation() {
    if (!this.viewCube)
      return;
    const Q = this.camera.quaternion.clone();
    typeof Q.invert == "function" ? Q.invert() : typeof Q.inverse == "function" && Q.inverse();
    const t = new h.Matrix4();
    t.makeRotationFromQuaternion(Q), t.multiply(this.viewCubeCorrection), this.viewCube.setRotationMatrix3d(t.elements);
    const B = QP(this.camera.position, this.controls.target);
    this.viewCube.setActiveFace(B);
  }
}
function hA(r) {
  return Number.isFinite(r) ? Math.max(0, Math.min(1, r)) : 0;
}
function lA(r, Q) {
  var B, A, e, P;
  if (!Q)
    return { ...r };
  const t = yi(r.render.theme, (B = Q.render) == null ? void 0 : B.theme);
  return {
    ...r,
    ...Q,
    mode: { ...r.mode, ...Q.mode },
    sim3d: { ...r.sim3d, ...Q.sim3d },
    bit: { ...r.bit, ...Q.bit },
    progress: { ...r.progress, ...Q.progress },
    grid: { ...r.grid, ...Q.grid },
    boundingBox: { ...r.boundingBox, ...Q.boundingBox },
    machineBed: { ...r.machineBed, ...Q.machineBed },
    geometry: {
      ...r.geometry,
      ...Q.geometry,
      batching: { ...r.geometry.batching, ...(A = Q.geometry) == null ? void 0 : A.batching }
    },
    render: {
      ...r.render,
      ...Q.render,
      theme: t
    },
    camera: {
      ...r.camera,
      ...Q.camera,
      orbit: { ...r.camera.orbit, ...(e = Q.camera) == null ? void 0 : e.orbit },
      initialPosition: { ...r.camera.initialPosition, ...(P = Q.camera) == null ? void 0 : P.initialPosition }
    }
  };
}
function yi(r, Q) {
  var t, B;
  return Q ? {
    ...r,
    ...Q,
    colors: {
      ...r.colors,
      ...Q.colors,
      grid: { ...r.colors.grid, ...(t = Q.colors) == null ? void 0 : t.grid },
      axes: { ...r.colors.axes, ...(B = Q.colors) == null ? void 0 : B.axes }
    }
  } : r;
}
const Ii = {
  rapidColor: "#0ef6ae",
  cutColor: "#3e85c7",
  boundingBoxColor: "#d0d0d0",
  strokeWidth: 0.5,
  arcSegments: 30,
  padding: 5,
  projectionMode: "isometric",
  showOrigin: !0,
  originColor: "#ffffff",
  crosshairColor: "#ffffff"
}, vt = 0, ct = 0;
class Zi {
  constructor(Q, t) {
    this.crosshairPos = null, this.crosshairVisible = !1, this.pathEls = [], this.segmentGroups = [], this.workerMode = !1, this.viewBox = { x: 0, y: 0, w: 100, h: 100 }, this.rotX = vt, this.rotY = ct, this.centerX = 0, this.centerY = 0, this.centerZ = 0, this.focalLength = 500, this.cosRotX = 1, this.sinRotX = 0, this.cosRotY = 1, this.sinRotY = 0, this.dragMode = "none", this.activePointers = /* @__PURE__ */ new Map(), this.pinchLastDist = 0, this.pinchLastMid = { x: 0, y: 0 }, this.rafPending = !1, this.overlayRafPending = !1, this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.bounds = { minX: 0, minY: 0, maxX: 0, maxY: 0, minZ: 0, maxZ: 0, empty: !0 }, this.hasZInfo = !0, this.project = (B, A, e) => {
      if (this.options.projectionMode === "top")
        return { x: B, y: -A };
      const P = B - this.centerX, i = A - this.centerY, s = e - this.centerZ, v = P * this.cosRotY + s * this.sinRotY, c = i, n = -P * this.sinRotY + s * this.cosRotY, T = v, z = c * this.cosRotX - n * this.sinRotX, a = c * this.sinRotX + n * this.cosRotX;
      if (this.options.projectionMode === "perspective") {
        const o = this.focalLength / (this.focalLength + a);
        return { x: T * o, y: -z * o };
      }
      return { x: T, y: -z };
    }, this.onWheel = (B) => {
      B.preventDefault();
      const A = B.deltaY > 0 ? 1.1 : 1 / 1.1, e = this.svgPoint(B.clientX, B.clientY), { x: P, y: i, w: s, h: v } = this.viewBox;
      this.viewBox = {
        x: e.x - (e.x - P) * A,
        y: e.y - (e.y - i) * A,
        w: s * A,
        h: v * A
      }, this.applyViewBox();
    }, this.onPointerDown = (B) => {
      if (this.activePointers.set(B.pointerId, { x: B.clientX, y: B.clientY }), this.svg.setPointerCapture(B.pointerId), B.preventDefault(), this.activePointers.size === 1)
        this.dragMode = "pan", this.svg.style.cursor = "grabbing";
      else if (this.activePointers.size === 2) {
        this.dragMode = "pan", this.svg.style.cursor = "move";
        const [A, e] = Array.from(this.activePointers.values());
        this.pinchLastDist = Math.hypot(e.x - A.x, e.y - A.y), this.pinchLastMid = { x: (A.x + e.x) / 2, y: (A.y + e.y) / 2 };
      }
    }, this.onPointerMove = (B) => {
      if (!this.activePointers.has(B.pointerId)) return;
      const A = this.activePointers.get(B.pointerId);
      if (this.activePointers.set(B.pointerId, { x: B.clientX, y: B.clientY }), this.activePointers.size === 2) {
        const [i, s] = Array.from(this.activePointers.values()), v = Math.hypot(s.x - i.x, s.y - i.y), c = { x: (i.x + s.x) / 2, y: (i.y + s.y) / 2 };
        if (this.pinchLastDist > 0) {
          const a = this.pinchLastDist / v, o = this.svgPoint(c.x, c.y), { x: w, y: E, w: l, h: u } = this.viewBox;
          this.viewBox = {
            x: o.x - (o.x - w) * a,
            y: o.y - (o.y - E) * a,
            w: l * a,
            h: u * a
          };
        }
        const n = this.svg.getBoundingClientRect(), T = c.x - this.pinchLastMid.x, z = c.y - this.pinchLastMid.y;
        this.viewBox = {
          ...this.viewBox,
          x: this.viewBox.x - T / n.width * this.viewBox.w,
          y: this.viewBox.y - z / n.height * this.viewBox.h
        }, this.pinchLastDist = v, this.pinchLastMid = c, this.applyViewBox();
        return;
      }
      if (this.dragMode === "none") return;
      const e = B.clientX - A.x, P = B.clientY - A.y;
      if (this.dragMode === "orbit") {
        const i = this.svg.getBoundingClientRect(), s = Math.PI / Math.min(i.width, i.height);
        this.rotY += e * s, this.rotX += P * s, this.rotX = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, this.rotX)), this.updateTrig(), this.fitView(), this.scheduleDraw();
      } else {
        const i = this.svg.getBoundingClientRect();
        this.viewBox = {
          ...this.viewBox,
          x: this.viewBox.x - e / i.width * this.viewBox.w,
          y: this.viewBox.y - P / i.height * this.viewBox.h
        }, this.applyViewBox();
      }
    }, this.onPointerUp = (B) => {
      if (this.activePointers.delete(B.pointerId), this.svg.releasePointerCapture(B.pointerId), this.activePointers.size === 1) {
        const [A] = this.activePointers.values();
        this.pinchLastMid = { x: A.x, y: A.y }, this.dragMode = "pan", this.svg.style.cursor = "grabbing";
      } else this.activePointers.size === 0 && (this.dragMode = "none", this.svg.style.cursor = "grab");
    }, this.onContextMenu = (B) => {
      B.preventDefault();
    }, this.options = { ...Ii, ...t }, this.svg = document.createElementNS("http://www.w3.org/2000/svg", "svg"), this.svg.style.cssText = "width:100%;height:100%;display:block;cursor:grab;user-select:none;touch-action:none;", this.svg.setAttribute("preserveAspectRatio", "xMidYMid meet"), this.svg.setAttribute("xmlns", "http://www.w3.org/2000/svg"), this.bboxPath = QB(), this.pathLayer = document.createElementNS("http://www.w3.org/2000/svg", "g"), this.bboxLabelX = tB("middle", "hanging"), this.bboxLabelY = tB("start", "middle"), this.bboxLabelZ = tB("middle", "auto"), this.originMarker = document.createElementNS("http://www.w3.org/2000/svg", "circle"), this.originMarker.setAttribute("stroke-opacity", "0.4"), this.originMarker.setAttribute("visibility", "hidden"), this.crosshairEl = QB(), this.crosshairEl.setAttribute("fill", "none"), this.crosshairEl.setAttribute("visibility", "hidden"), this.svg.appendChild(this.bboxPath), this.svg.appendChild(this.pathLayer), this.svg.appendChild(this.bboxLabelX), this.svg.appendChild(this.bboxLabelY), this.svg.appendChild(this.bboxLabelZ), this.svg.appendChild(this.originMarker), this.svg.appendChild(this.crosshairEl), Q.appendChild(this.svg), this.applyOptions(), this.bindEvents();
  }
  // ── Public API ────────────────────────────────────────────────────────────
  loadFromLines(Q) {
    const { rapid: t, cutting: B } = de(Q, {
      arcSegments: this.options.arcSegments
    });
    if (this.rapidVerts = t, this.cutVerts = B, this.workerMode = !1, this.syncSegmentGroupsFromLines(), this.hasZInfo = !0, this.bounds = AB([
      { verts: t, stride: 6 },
      { verts: B, stride: 6 }
    ]), !this.bounds.empty) {
      this.centerX = (this.bounds.minX + this.bounds.maxX) / 2, this.centerY = (this.bounds.minY + this.bounds.maxY) / 2, this.centerZ = (this.bounds.minZ + this.bounds.maxZ) / 2;
      const A = Math.hypot(
        this.bounds.maxX - this.bounds.minX,
        this.bounds.maxY - this.bounds.minY,
        this.bounds.maxZ - this.bounds.minZ
      );
      this.focalLength = A * 2;
    }
    this.rotX = vt, this.rotY = ct, this.updateTrig(), this.fitView(), this.rebuildAndRender();
  }
  loadFromFile(Q) {
    return new Promise((t, B) => {
      const A = new FileReader();
      A.onload = (e) => {
        var i;
        const P = (i = e.target) == null ? void 0 : i.result;
        if (typeof P != "string") {
          B(new Error("Failed to read file"));
          return;
        }
        this.loadFromText(P), t();
      }, A.onerror = () => B(new Error("FileReader error")), A.readAsText(Q);
    });
  }
  loadFromText(Q) {
    this.loadFromLines(Q.split(/\r?\n/));
  }
  clear() {
    this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.workerMode = !1, this.segmentGroups = [], this.hasZInfo = !0, this.bounds = { minX: 0, minY: 0, maxX: 0, maxY: 0, minZ: 0, maxZ: 0, empty: !0 }, this.rebuildAndRender();
  }
  loadFromWorkerData(Q) {
    const t = Ce(Q);
    if (this.workerMode = !0, this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.segmentGroups = t.map((B) => ({
      color: B.hexColor,
      opacity: B.opacity,
      verts: B.positions,
      stride: 6
    })), this.hasZInfo = !0, this.bounds = AB(t.map((B) => ({ verts: B.positions, stride: 6 }))), !this.bounds.empty) {
      this.centerX = (this.bounds.minX + this.bounds.maxX) / 2, this.centerY = (this.bounds.minY + this.bounds.maxY) / 2, this.centerZ = (this.bounds.minZ + this.bounds.maxZ) / 2;
      const B = Math.hypot(
        this.bounds.maxX - this.bounds.minX,
        this.bounds.maxY - this.bounds.minY,
        this.bounds.maxZ - this.bounds.minZ
      );
      this.focalLength = B * 2;
    }
    this.rotX = vt, this.rotY = ct, this.updateTrig(), this.fitView(), this.rebuildAndRender();
  }
  loadFromPrecomputedGroups(Q, t) {
    this.workerMode = !0, this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.segmentGroups = Q.map((A) => ({
      color: A.hexColor,
      opacity: A.opacity,
      verts: new Float32Array(A.positionsBuffer, 0, A.positionsLen),
      stride: A.stride ?? 6
    })), this.bounds = AB(this.segmentGroups);
    const B = (t == null ? void 0 : t.minZ) !== void 0 && (t == null ? void 0 : t.maxZ) !== void 0;
    if (B && !this.bounds.empty && (this.bounds.minZ = t.minZ, this.bounds.maxZ = t.maxZ), this.hasZInfo = B || this.segmentGroups.every((A) => A.stride === 6), !this.bounds.empty) {
      this.centerX = (this.bounds.minX + this.bounds.maxX) / 2, this.centerY = (this.bounds.minY + this.bounds.maxY) / 2, this.centerZ = (this.bounds.minZ + this.bounds.maxZ) / 2;
      const A = Math.hypot(
        this.bounds.maxX - this.bounds.minX,
        this.bounds.maxY - this.bounds.minY,
        this.bounds.maxZ - this.bounds.minZ
      );
      this.focalLength = A * 2;
    }
    this.rotX = vt, this.rotY = ct, this.updateTrig(), this.fitView(), this.rebuildAndRender();
  }
  syncSegmentGroupsFromLines() {
    this.segmentGroups = [
      { color: this.options.rapidColor, opacity: 0.5, verts: this.rapidVerts, stride: 6 },
      { color: this.options.cutColor, opacity: 1, verts: this.cutVerts, stride: 6 }
    ];
  }
  resetView() {
    this.rotX = vt, this.rotY = ct, this.updateTrig(), this.fitView(), this.rebuildAndRender();
  }
  setOptions(Q) {
    this.options = { ...this.options, ...Q }, this.applyOptions(), this.rebuildAndRender();
  }
  setProjectionMode(Q) {
    this.options = { ...this.options, projectionMode: Q }, this.rebuildAndRender();
  }
  getSVGElement() {
    return this.svg;
  }
  setBitPosition(Q) {
    this.crosshairPos = Q, this.crosshairVisible = !0, this.scheduleOverlayDraw();
  }
  setBitVisible(Q) {
    this.crosshairVisible = Q, this.scheduleOverlayDraw();
  }
  dispose() {
    this.svg.removeEventListener("wheel", this.onWheel), this.svg.removeEventListener("pointerdown", this.onPointerDown), this.svg.removeEventListener("pointermove", this.onPointerMove), this.svg.removeEventListener("pointerup", this.onPointerUp), this.svg.removeEventListener("pointercancel", this.onPointerUp), this.svg.removeEventListener("contextmenu", this.onContextMenu), this.svg.remove();
  }
  // ── Projection ────────────────────────────────────────────────────────────
  updateTrig() {
    this.cosRotX = Math.cos(this.rotX), this.sinRotX = Math.sin(this.rotX), this.cosRotY = Math.cos(this.rotY), this.sinRotY = Math.sin(this.rotY);
  }
  // ── Rendering ─────────────────────────────────────────────────────────────
  rebuildAndRender(Q = !1) {
    this.rebuildToolpaths(Q), this.renderOverlays();
  }
  // Expensive: re-projects and re-serialises every vertex into SVG path `d`
  // strings. Only call when geometry, projection, or stroke width changes —
  // never for bit-position/crosshair updates (use scheduleOverlayDraw instead).
  rebuildToolpaths(Q = !1) {
    const t = Q ? Yi : L, B = String(this.options.strokeWidth), A = this.simplifyTolerance();
    for (; this.pathEls.length < this.segmentGroups.length; ) {
      const e = QB();
      this.pathLayer.appendChild(e), this.pathEls.push(e);
    }
    for (; this.pathEls.length > this.segmentGroups.length; )
      this.pathLayer.removeChild(this.pathEls.pop());
    for (let e = 0; e < this.segmentGroups.length; e++) {
      const { color: P, opacity: i, verts: s, stride: v } = this.segmentGroups[e], c = this.pathEls[e];
      c.setAttribute("stroke", P), c.setAttribute("stroke-opacity", String(i)), c.setAttribute("stroke-width", B), c.setAttribute("stroke-linecap", "round"), c.setAttribute("stroke-linejoin", "round"), c.setAttribute(
        "d",
        v === 4 ? Si(s, t, A) : Vi(s, this.project, t, A)
      );
    }
  }
  // Cheap O(1) overlays that don't depend on toolpath geometry.
  renderOverlays() {
    this.renderBbox(), this.renderOriginMarker(), this.renderCrosshairMarker(), this.applyViewBox();
  }
  // World-space distance below which consecutive projected points are merged.
  // Derived from the bounds diagonal so it scales with model size and stays
  // deterministic (independent of zoom / layout timing).
  simplifyTolerance() {
    return this.bounds.empty ? 0 : Math.hypot(
      this.bounds.maxX - this.bounds.minX,
      this.bounds.maxY - this.bounds.minY,
      this.bounds.maxZ - this.bounds.minZ
    ) / 1500;
  }
  scheduleDraw() {
    this.rafPending || (this.rafPending = !0, requestAnimationFrame(() => {
      this.rafPending = !1, this.rebuildAndRender(!0);
    }));
  }
  // rAF-coalesced redraw of just the overlays (bbox/origin/crosshair/viewBox).
  // Used for bit-position updates so a running job never rebuilds the toolpath.
  scheduleOverlayDraw() {
    this.overlayRafPending || (this.overlayRafPending = !0, requestAnimationFrame(() => {
      this.overlayRafPending = !1, this.renderOverlays();
    }));
  }
  fitView() {
    if (this.bounds.empty) {
      this.viewBox = { x: -50, y: -50, w: 100, h: 100 };
      return;
    }
    const { minX: Q, maxX: t, minY: B, maxY: A, minZ: e, maxZ: P } = this.bounds, i = this.project, s = i(Q, B, e), v = i(t, B, e), c = i(t, A, e), n = i(Q, A, e), T = i(Q, B, P), z = i(t, B, P), a = i(t, A, P), o = i(Q, A, P), w = Math.hypot(v.x - s.x, v.y - s.y), E = Math.hypot(n.x - s.x, n.y - s.y), l = Math.hypot(T.x - s.x, T.y - s.y), u = Math.max(w, E, l) * 0.07, U = u * 0.5, k = BB(LQ(s, v), LQ(c, n), U), O = [s, v, c, n, T, z, a, o], D = Math.min(...O.map((x) => x.y)), N = Math.max(...O.map((x) => x.y)), d = Math.min(...O.map((x) => x.x)), M = Math.max(...O.map((x) => x.x)), G = { x: M + U, y: (D + N) / 2 }, b = { x: G.x + u * 5, y: G.y }, C = { x: (d + M) / 2, y: D - u }, j = [s, v, c, n, T, z, a, o, k, G, b, C];
    let f = 1 / 0, H = 1 / 0, F = -1 / 0, I = -1 / 0;
    for (const x of j)
      x.x < f && (f = x.x), x.y < H && (H = x.y), x.x > F && (F = x.x), x.y > I && (I = x.y);
    const R = F - f, QQ = I - H, sQ = Math.max(R, QQ) * 0.08, W = this.options.padding + sQ;
    this.viewBox = {
      x: f - W,
      y: H - W,
      w: R + W * 2,
      h: QQ + W * 2
    };
  }
  applyViewBox() {
    const { x: Q, y: t, w: B, h: A } = this.viewBox;
    this.svg.setAttribute("viewBox", `${L(Q)} ${L(t)} ${L(B)} ${L(A)}`);
  }
  applyOptions() {
    const { boundingBoxColor: Q, strokeWidth: t, originColor: B } = this.options;
    this.workerMode || this.syncSegmentGroupsFromLines(), this.bboxPath.setAttribute("stroke", Q), this.bboxPath.setAttribute("stroke-width", String(t * 0.6)), this.bboxPath.setAttribute("fill", Q), this.bboxPath.setAttribute("fill-opacity", "0.05");
    for (const A of [this.bboxLabelX, this.bboxLabelY, this.bboxLabelZ])
      A.setAttribute("fill", Q);
    this.originMarker.setAttribute("fill", B), this.originMarker.setAttribute("stroke", "#000000"), this.crosshairEl.setAttribute("stroke", this.options.crosshairColor);
  }
  renderBbox() {
    if (this.bounds.empty) {
      this.bboxPath.setAttribute("d", ""), this.bboxLabelX.setAttribute("visibility", "hidden"), this.bboxLabelY.setAttribute("visibility", "hidden"), this.bboxLabelZ.setAttribute("visibility", "hidden");
      return;
    }
    const { minX: Q, maxX: t, minY: B, maxY: A, minZ: e, maxZ: P } = this.bounds, i = this.project;
    if (this.options.projectionMode === "top") {
      const C = i(Q, B, 0), j = i(t, B, 0), f = i(t, A, 0), H = i(Q, A, 0);
      this.bboxPath.setAttribute(
        "d",
        `M${L(C.x)} ${L(C.y)}L${L(j.x)} ${L(j.y)}L${L(f.x)} ${L(f.y)}L${L(H.x)} ${L(H.y)}Z`
      );
      const F = Math.abs(j.x - C.x), I = Math.abs(H.y - C.y), R = Math.max(F, I) * 0.07, QQ = R * 0.5;
      ZQ(this.bboxLabelX, LQ(C, j), BB(LQ(C, j), LQ(f, H), QQ), R, `X: ${WQ(t - Q)}`);
      const sQ = Math.min(C.y, j.y, f.y, H.y), W = Math.max(C.y, j.y, f.y, H.y), x = Math.max(C.x, j.x, f.x, H.x), tQ = Math.min(C.x, j.x, f.x, H.x), vQ = { x: x + QQ, y: (sQ + W) / 2 };
      if (ZQ(this.bboxLabelY, vQ, vQ, R, `Y: ${WQ(A - B)}`), this.hasZInfo) {
        const rt = { x: (tQ + x) / 2, y: sQ - R };
        ZQ(this.bboxLabelZ, rt, rt, R, `Z: ${WQ(P - e)}`);
      } else
        this.bboxLabelZ.setAttribute("visibility", "hidden");
      return;
    }
    const s = i(Q, B, e), v = i(t, B, e), c = i(t, A, e), n = i(Q, A, e), T = i(Q, B, P), z = i(t, B, P), a = i(t, A, P), o = i(Q, A, P), w = [
      `M${L(s.x)} ${L(s.y)}L${L(v.x)} ${L(v.y)}L${L(c.x)} ${L(c.y)}L${L(n.x)} ${L(n.y)}Z`,
      `M${L(T.x)} ${L(T.y)}L${L(z.x)} ${L(z.y)}L${L(a.x)} ${L(a.y)}L${L(o.x)} ${L(o.y)}Z`,
      `M${L(s.x)} ${L(s.y)}L${L(T.x)} ${L(T.y)}`,
      `M${L(v.x)} ${L(v.y)}L${L(z.x)} ${L(z.y)}`,
      `M${L(c.x)} ${L(c.y)}L${L(a.x)} ${L(a.y)}`,
      `M${L(n.x)} ${L(n.y)}L${L(o.x)} ${L(o.y)}`
    ].join("");
    this.bboxPath.setAttribute("d", w);
    const E = Math.hypot(v.x - s.x, v.y - s.y), l = Math.hypot(n.x - s.x, n.y - s.y), u = Math.hypot(T.x - s.x, T.y - s.y), U = Math.max(E, l, u) * 0.07, k = U * 0.5;
    ZQ(this.bboxLabelX, LQ(s, v), BB(LQ(s, v), LQ(c, n), k), U, `X: ${WQ(t - Q)}`);
    const O = [s, v, c, n, T, z, a, o], D = Math.min(...O.map((C) => C.y)), N = Math.max(...O.map((C) => C.y)), d = Math.min(...O.map((C) => C.x)), M = Math.max(...O.map((C) => C.x)), G = { x: M + k, y: (D + N) / 2 };
    ZQ(this.bboxLabelY, G, G, U, `Y: ${WQ(A - B)}`);
    const b = { x: (d + M) / 2, y: D - U };
    ZQ(this.bboxLabelZ, b, b, U, `Z: ${WQ(P - e)}`);
  }
  renderOriginMarker() {
    if (!this.options.showOrigin || this.bounds.empty) {
      this.originMarker.setAttribute("visibility", "hidden");
      return;
    }
    const Q = this.project(0, 0, 0), t = Math.min(this.viewBox.w, this.viewBox.h) * 0.018;
    this.originMarker.setAttribute("cx", L(Q.x)), this.originMarker.setAttribute("cy", L(Q.y)), this.originMarker.setAttribute("r", L(t)), this.originMarker.setAttribute("stroke-width", L(t * 0.25)), this.originMarker.setAttribute("visibility", "visible");
  }
  renderCrosshairMarker() {
    if (!this.crosshairVisible || this.crosshairPos === null) {
      this.crosshairEl.setAttribute("visibility", "hidden");
      return;
    }
    const { x: Q, y: t, z: B } = this.crosshairPos, A = this.project(Q, t, B), e = Math.min(this.viewBox.w, this.viewBox.h) * 0.025, P = e * 0.25, i = A.x, s = A.y, v = [
      `M${L(i - e)} ${L(s)}L${L(i - P)} ${L(s)}`,
      `M${L(i + P)} ${L(s)}L${L(i + e)} ${L(s)}`,
      `M${L(i)} ${L(s - e)}L${L(i)} ${L(s - P)}`,
      `M${L(i)} ${L(s + P)}L${L(i)} ${L(s + e)}`
    ].join("");
    this.crosshairEl.setAttribute("d", v), this.crosshairEl.setAttribute("stroke-width", L(this.options.strokeWidth * 2)), this.crosshairEl.setAttribute("stroke-linecap", "round"), this.crosshairEl.setAttribute("visibility", "visible");
  }
  bindEvents() {
    this.svg.addEventListener("wheel", this.onWheel, { passive: !1 }), this.svg.addEventListener("pointerdown", this.onPointerDown), this.svg.addEventListener("pointermove", this.onPointerMove), this.svg.addEventListener("pointerup", this.onPointerUp), this.svg.addEventListener("pointercancel", this.onPointerUp), this.svg.addEventListener("contextmenu", this.onContextMenu);
  }
  svgPoint(Q, t) {
    const B = this.svg.getBoundingClientRect(), { x: A, y: e, w: P, h: i } = this.viewBox;
    return {
      x: A + (Q - B.left) / B.width * P,
      y: e + (t - B.top) / B.height * i
    };
  }
}
function QB() {
  const r = document.createElementNS("http://www.w3.org/2000/svg", "path");
  return r.setAttribute("fill", "none"), r;
}
function tB(r, Q) {
  const t = document.createElementNS("http://www.w3.org/2000/svg", "text");
  return t.setAttribute("text-anchor", r), t.setAttribute("dominant-baseline", Q), t;
}
function ZQ(r, Q, t, B, A) {
  r.setAttribute("x", L(t.x)), r.setAttribute("y", L(t.y)), r.setAttribute("font-size", L(B)), r.textContent = A, r.setAttribute("visibility", "visible");
}
function LQ(r, Q) {
  return { x: (r.x + Q.x) / 2, y: (r.y + Q.y) / 2 };
}
function BB(r, Q, t) {
  const B = r.x - Q.x, A = r.y - Q.y, e = Math.hypot(B, A) || 1;
  return { x: r.x + B / e * t, y: r.y + A / e * t };
}
function L(r) {
  return r.toFixed(2);
}
function Yi(r) {
  return Math.round(r) + "";
}
function WQ(r) {
  return r.toFixed(2);
}
function Si(r, Q, t = 0) {
  if (r.length === 0) return "";
  const B = [], A = 1e-6, e = t * t;
  let P = NaN, i = NaN, s = NaN, v = NaN, c = NaN, n = NaN, T = !1;
  for (let z = 0; z + 3 < r.length; z += 4) {
    const a = r[z], o = -r[z + 1], w = r[z + 2], E = -r[z + 3];
    if (Math.abs(a - P) < A && Math.abs(o - i) < A) {
      const l = w - s, u = E - v;
      e > 0 && l * l + u * u < e ? (c = w, n = E, T = !0) : (B.push(`L${Q(w)} ${Q(E)}`), s = w, v = E, T = !1);
    } else
      T && (B.push(`L${Q(c)} ${Q(n)}`), T = !1), B.push(`M${Q(a)} ${Q(o)}L${Q(w)} ${Q(E)}`), s = w, v = E;
    P = w, i = E;
  }
  return T && B.push(`L${Q(c)} ${Q(n)}`), B.join("");
}
function Vi(r, Q, t, B = 0) {
  if (r.length === 0) return "";
  const A = [], e = 1e-6, P = B * B;
  let i = NaN, s = NaN, v = NaN, c = NaN, n = NaN, T = NaN, z = !1;
  for (let a = 0; a + 5 < r.length; a += 6) {
    const o = Q(r[a], r[a + 1], r[a + 2]), w = Q(r[a + 3], r[a + 4], r[a + 5]);
    if (Math.abs(o.x - i) < e && Math.abs(o.y - s) < e) {
      const E = w.x - v, l = w.y - c;
      P > 0 && E * E + l * l < P ? (n = w.x, T = w.y, z = !0) : (A.push(`L${t(w.x)} ${t(w.y)}`), v = w.x, c = w.y, z = !1);
    } else
      z && (A.push(`L${t(n)} ${t(T)}`), z = !1), A.push(`M${t(o.x)} ${t(o.y)}L${t(w.x)} ${t(w.y)}`), v = w.x, c = w.y;
    i = w.x, s = w.y;
  }
  return z && A.push(`L${t(n)} ${t(T)}`), A.join("");
}
function AB(r) {
  let Q = 1 / 0, t = 1 / 0, B = 1 / 0, A = -1 / 0, e = -1 / 0, P = -1 / 0, i = !0, s = !1;
  for (const { verts: v, stride: c } of r)
    if (c === 4)
      for (let n = 0; n + 3 < v.length; n += 4) {
        const T = v[n], z = v[n + 1], a = v[n + 2], o = v[n + 3];
        T < Q && (Q = T), z < t && (t = z), T > A && (A = T), z > e && (e = z), a < Q && (Q = a), o < t && (t = o), a > A && (A = a), o > e && (e = o), i = !1;
      }
    else
      for (let n = 0; n + 5 < v.length; n += 6) {
        const T = v[n], z = v[n + 1], a = v[n + 2], o = v[n + 3], w = v[n + 4], E = v[n + 5];
        T < Q && (Q = T), z < t && (t = z), a < B && (B = a), T > A && (A = T), z > e && (e = z), a > P && (P = a), o < Q && (Q = o), w < t && (t = w), E < B && (B = E), o > A && (A = o), w > e && (e = w), E > P && (P = E), i = !1, s = !0;
      }
  return !i && !s && (B = 0, P = 0), { minX: Q, minY: t, maxX: A, maxY: e, minZ: B, maxZ: P, empty: i };
}
export {
  Zi as G,
  Ye as V,
  Ki as a,
  JB as b,
  be as d
};
