...
import { OrbitControls as kP } from "three/examples/jsm/controls/OrbitControls.js";
import { GCodeVirtualizer as Pt, buildWorkerToolpathStreams as UP, buildToolpathGeometryFromLinesBatched as DP, buildMovementVerticesFromLines as uP, buildWorkerSegmentGroups as dP } from "./gviewer.js";
import { STLLoader as CP } from "three/examples/jsm/loaders/STLLoader.js";
function NP(r) {
  const Q = [];
  let B = null;
  for (const t of r.specs) {
    if (t.positions.length === 0 || t.opacity <= 0)
      continue;
    const A = t.positions.length / 3, P = t.colors ? t.colors.slice() : hA(t.kind, A, r.options), e = new Float32Array(P.length);
    e.set(P);
    const i = new h.BufferGeometry();
    i.setAttribute("position", new h.BufferAttribute(t.positions, 3));
    ...
    r.scene.add(c), Q.push({
      ...
      baseColors: P,
      simColors: e,
      prefixEndVertex: t.prefixEndVertex,
      totalVertices: A,
      greyCursorVertex: 0,
      kind: t.kind,
      cutBucketIndex: t.cutBucketIndex,
      workerColors: t.colors ? P : void 0
...
function LP(r, Q) {
  for (const B of r) {
    ...
    const t = hA(B.kind, B.totalVertices, Q);
    B.baseColors = t, B.simColors.set(t), B.greyCursorVertex = 0;
    const A = B.line.geometry.getAttribute("color");
    A.clearUpdateRanges(), A.addUpdateRange(0, B.simColors.length), A.needsUpdate = !0;
  }
}
function MP(r) {
  const Q = et(r.options.render.theme.rapidOpacity ?? 0.3);
  for (const B of r.streams) {
    const t = B.line.material;
    if (B.kind === "rapid") {
      t.opacity = Q;
      continue;
    }
...
function HP(r) {
  const Q = r.stream.totalVertices, B = Math.max(0, Math.min(Q, Math.floor(r.nextCursorVertex))), t = Math.max(0, Math.min(Q, Math.floor(r.stream.greyCursorVertex)));
  if (B === t)
    return;
  const A = r.stream.line.geometry.getAttribute("color"), P = jP(r.options), e = Math.min(t, B), i = Math.max(t, B);
  if (B > t)
    for (let s = t; s < B; s += 1) {
      const v = s * 3;
  ...
  A.clearUpdateRanges(), A.addUpdateRange(e * 3, (i - e) * 3), A.needsUpdate = !0, r.stream.greyCursorVertex = B;
}
function hA(r, Q, B) {
  const t = B.render.theme.colors, A = B.mode.laser ? t.laser ?? t.cutting : t.cutting, P = new h.Color(r === "rapid" ? t.rapid : A), e = new Float32Array(Q * 3);
  for (let i = 0; i < Q; i += 1) {
    const s = i * 3;
...
function jP(r) {
  const Q = r.render.theme.colors, B = new h.Color(r.render.theme.background), t = new h.Color(Q.processed ?? Q.cutting), A = B.clone().lerp(t, 0.65);
  return { r: A.r, g: A.g, b: A.b };
}
function FP(r) {
  if (r.bucketCount <= 1)
    return et(r.baseOpacity);
  const Q = r.bucketIndex / (r.bucketCount - 1);
  return et(r.baseOpacity * Q);
}
function et(r) {
...
async function GP(r, Q, B, t) {
  var M, b, G;
  const A = Math.max(2, Math.floor(B)), P = (t == null ? void 0 : t.arcSegments) ?? 30, e = (t == null ? void 0 : t.checkpointEveryLines) ?? 2e3, i = ((M = t == null ? void 0 : t.batch) == null ? void 0 : M.yieldEveryLines) ?? 1e4, s = ((b = t == null ? void 0 : t.batch) == null ? void 0 : b.shouldAbort) ?? (() => !1), v = (G = t == null ? void 0 : t.batch) == null ? void 0 : G.onProgress;
  if (r.length === 0) {
    const C = {
      xMin: -1,
      xMax: 1,
  ...
  let l = w - Q;
  l >= E && (l = E - 1);
  const U = { xMin: c, xMax: n, yMin: T, yMax: z, zTop: E, zBot: l }, D = new Float32Array(A * A).fill(E), k = /* @__PURE__ */ new Map();
  k.set(0, new Float32Array(D));
  const O = [];
  let u = 0, N = 0;
  const d = new Pt({
    onLinearMove(C) {
      ...
      O.push(H), it(H, D, U, Q, A);
    },
    onArcMove(C) {
      const j = C.transformedStart ?? C.start, F = C.transformedEnd ?? C.end, H = C.transformedCenter ?? C.center, f = bP(j, F, H, C.plane, C.motion, P);
      for (const I of f) {
        const R = { lineIndex: N, ...I };
        O.push(R), it(R, D, U, Q, A);
      }
    }
  });
  for (let C = 0; C < r.length && !s(); C++) {
    N = C;
    const j = r[C];
    j !== void 0 && d.processLine(j), C - u >= e && (k.set(C, new Float32Array(D)), u = C), C % i === 0 && C > 0 && (v == null || v(C, r.length), await new Promise((F) => setTimeout(F, 0)));
  }
  return v == null || v(r.length, r.length), { moves: O, checkpoints: k, slabBounds: U, resolution: A, toolRadius: Q };
}
function it(r, Q, B, t, A) {
  const P = (B.xMax - B.xMin) / A, e = (B.yMax - B.yMin) / A, i = Math.hypot(r.x1 - r.x0, r.y1 - r.y0), s = Math.max(1e-3, Math.min(P, e) * 0.5), v = Math.max(1, Math.ceil(i / s)), c = Math.ceil(t / P), n = Math.ceil(t / e), T = t * t;
  for (let z = 0; z <= v; z++) {
    const w = z / v, o = r.x0 + (r.x1 - r.x0) * w, a = r.y0 + (r.y1 - r.y0) * w, E = r.z0 + (r.z1 - r.z0) * w, l = Math.floor((o - B.xMin) / P), U = Math.floor((a - B.yMin) / e);
    for (let D = -c; D <= c; D++) {
      const k = l + D;
      if (k < 0 || k >= A) continue;
      const O = B.xMin + (k + 0.5) * P, u = (O - o) * (O - o);
      if (!(u > T))
        for (let N = -n; N <= n; N++) {
          const d = U + N;
          if (d < 0 || d >= A) continue;
          const M = B.yMin + (d + 0.5) * e;
          if (u + (M - a) * (M - a) > T) continue;
          const G = d * A + k;
          Q[G] > E && (Q[G] = E);
        }
    }
...
function SB(r, Q) {
  ...
  for (const P of Q.checkpoints.keys())
    P <= r && P > B && (B = P);
  const t = Q.checkpoints.get(B), A = t ? new Float32Array(t) : new Float32Array(Q.resolution * Q.resolution).fill(Q.slabBounds.zTop);
  for (const P of Q.moves)
    P.lineIndex > B && P.lineIndex <= r && it(P, A, Q.slabBounds, Q.toolRadius, Q.resolution);
  return A;
}
function YB(r, Q, B, t) {
  if (B <= 0) return;
  const A = 1e-3, P = Q * Q, e = new Float32Array(P);
  for (let i = 0; i < B; i++) {
    e.set(r);
    for (let s = 0; s < Q; s++)
      for (let v = 0; v < Q; v++) {
        if (e[s * Q + v] < t - A) continue;
        let n = t;
        for (let T = -1; T <= 1; T++) {
          const z = s + T;
          if (!(z < 0 || z >= Q))
...
function bP(r, Q, B, t, A, P) {
  const e = Math.max(1, P);
  let i, s, v;
  t === "G18" ? (i = "Z", s = "X", v = "Y") : t === "G19" ? (i = "Y", s = "Z", v = "X") : (i = "X", s = "Y", v = "Z");
  ...
  let o = { ...r };
  for (let a = 1; a <= e; a++) {
    const E = a / e, l = n + z * E, U = E, D = B[i] + c * Math.cos(l), k = B[s] + c * Math.sin(l), O = r[v] + (Q[v] - r[v]) * U, u = { X: 0, Y: 0, Z: 0 };
    u[i] = D, u[s] = k, u[v] = O, w.push({
      x0: o.X,
      y0: o.Y,
      z0: o.Z,
      x1: u.X,
...
function gP() {
  ...
  const t = Math.floor(40 + Math.random() * 20);
  for (let A = 0; A < t; A++) {
...
function pP(r, Q) {
  ...
  const t = r.xMax - r.xMin, A = r.yMax - r.yMin, P = r.zTop - r.zBot, e = (r.xMin + r.xMax) / 2, i = (r.yMin + r.yMax) / 2, s = (r.zBot + r.zTop) / 2, v = gP();
  v.wrapS = h.RepeatWrapping, v.wrapT = h.RepeatWrapping, v.repeat.set(t / 100, A / 100);
  const c = new h.MeshStandardMaterial({
    ...
    roughness: 0.9,
    metalness: 0
  }), z = new h.PlaneGeometry(t, A, Q - 1, Q - 1), w = Q * Q, o = new Float32Array(w * 3), a = z.attributes.position;
  for (let G = 0; G < w; G++)
    o[G * 3] = a.getX(G), o[G * 3 + 1] = a.getY(G), o[G * 3 + 2] = 0;
  ...
  l.name = "gviewer:sim3d-top", l.position.set(e, i, r.zTop), B.add(l);
  const U = new h.PlaneGeometry(t, P);
  U.rotateX(Math.PI / 2);
  const D = new h.Mesh(U, n);
  D.position.set(e, r.yMax, s), B.add(D);
  const k = new h.PlaneGeometry(t, P);
  k.rotateX(Math.PI / 2);
  const O = new h.Mesh(k, n);
  O.position.set(e, r.yMin, s), B.add(O);
  const u = new h.PlaneGeometry(P, A);
  u.rotateY(-Math.PI / 2);
  const N = new h.Mesh(u, n);
  N.position.set(r.xMax, i, s), B.add(N);
  const d = new h.PlaneGeometry(P, A);
  d.rotateY(-Math.PI / 2);
  const M = new h.Mesh(d, n);
  M.position.set(r.xMin, i, s), B.add(M);
  const b = new h.Mesh(new h.PlaneGeometry(t, A), T);
  return b.position.set(e, i, r.zBot), B.add(b), { group: B, topMesh: l, topPositionAttr: E, slabBounds: r, resolution: Q };
}
function VB(r, Q) {
  const { topPositionAttr: B, slabBounds: t, resolution: A } = r, P = A * A;
  for (let e = 0; e < P; e++) {
    const i = Math.floor(e / A), s = e % A, v = (A - 1 - i) * A + s;
    B.setZ(e, Q[v] - t.zTop);
  }
...
class xP {
  constructor(Q) {
    ...
    for (const t of B) {
      ...
      A.type = "button", A.className = `gViewer-viewcube__face ${t.className}`, A.textContent = t.label, A.addEventListener("click", () => this.onSelectView(t.view)), this.cube.appendChild(A), this.faceButtons.set(t.view, A);
    }
    Q.container.appendChild(this.root);
  }
  setActiveFace(Q) {
    for (const [B, t] of this.faceButtons.entries())
      B === Q ? t.classList.add("is-active") : t.classList.remove("is-active");
  }
  setRotationMatrix3d(Q) {
    const B = Array.from({ length: 16 }, (t, A) => {
      const P = Q[A] ?? 0;
...
function MQ(r, Q, B = {}) {
  const { opacity: t = 1, size: A = 10 } = B, P = new h.Object3D(), e = 100, i = document.createElement("canvas"), s = i.getContext("2d");
  if (!s)
...
const IP = 25.4;
function SP(r, Q) {
  const B = new h.Group(), t = new h.Box3Helper(
    r.clone(),
    new h.Color(Q.render.theme.colors.boundingBox)
  ), A = t.material;
  return A.transparent = !0, A.opacity = 0.65, A.depthWrite = !1, A.linewidth = 2, B.add(t), Q.boundingBox.labels && B.add(...YP(r.clone(), Q)), B;
}
function YP(r, Q) {
  const B = Q.render.theme.colors.boundingBox, t = 0.8, A = (U) => Math.abs(U) < 1e-9 ? "0" : U.toFixed(3), P = (U) => Q.units === "in" ? `${A(U / IP)} in` : `${A(U)} mm`, e = r.min, i = r.max, s = (U) => MQ(U, B, { size: 4, opacity: t }), v = s(P(e.x)), c = s(P(i.x)), n = s(P(e.y)), T = s(P(i.y)), z = s(P(e.z)), w = s(P(i.z)), o = new h.Vector3();
  r.getSize(o);
  const a = Math.max(2, Math.max(o.x, o.y, o.z) * 0.02), E = (e.x + i.x) / 2, l = (e.y + i.y) / 2;
  return v.position.set(e.x - a, l, e.z - a), c.position.set(i.x + a, l, e.z - a), n.position.set(E, e.y - a, e.z - a), T.position.set(E, i.y + a, e.z - a), z.position.set(E, l, e.z - a), w.position.set(E, l, i.z + a), [v, c, n, T, z, w];
}
function VP(r) {
  r.traverse((Q) => {
    var B;
    if (Q instanceof h.LineSegments || Q instanceof h.Line) {
      Q.geometry.dispose();
      const t = Q.material;
      Array.isArray(t) ? t.forEach((A) => A.dispose()) : t.dispose();
      return;
    }
    if (Q instanceof h.Sprite) {
      const t = Q.material;
...
function Jt(r, Q, B) {
  ...
  A.setAttribute("position", new h.BufferAttribute(t, 3));
  const P = new h.LineBasicMaterial({
    color: new h.Color(B),
    transparent: !0,
    opacity: 0.35,
    depthWrite: !1
  });
  return new h.LineLoop(A, P);
}
function JP(r, Q, B, t = 10) {
  const A = [], P = r.x - Q.y, e = Q.x - r.y;
  for (let v = P; v <= e; v += t) {
...
function RP(r, Q, B, t) {
  const A = new h.Group();
  if (A.add(Jt(r, Q, B.render.theme.colors.machineBed)), t) {
    const P = B.render.theme.colors.machineBedKeepout;
    A.add(Jt(t.min, t.max, P)), A.add(JP(t.min, t.max, P));
  }
  return A;
}
function XP(r) {
  r.traverse((Q) => {
    if (Q instanceof h.LineSegments || Q instanceof h.Line) {
      Q.geometry.dispose();
      const B = Q.material;
      Array.isArray(B) ? B.forEach((t) => t.dispose()) : B.dispose();
    }
  });
}
function KP(r) {
...
function WP(r, Q) {
  const B = r.x - Q.x, t = r.y - Q.y, A = r.z - Q.z, P = Math.abs(B), e = Math.abs(t), i = Math.abs(A);
  return i >= P && i >= e ? A >= 0 ? "top" : "bottom" : P >= e ? B >= 0 ? "right" : "left" : t >= 0 ? "back" : "front";
}
const lA = 25.4;
function Ut(r) {
  if (r.bounds)
  ...
  const Q = Math.max(1, r.sizeXMm) / 2, B = Math.max(1, r.sizeYMm) / 2;
  return { minX: -Q, maxX: Q, minY: -B, maxY: B };
}
function qP(r) {
  const { minX: Q, maxX: B, minY: t, maxY: A } = Ut(r), P = r.units === "mm" ? 10 : lA, e = [], i = [], s = (w, o, a, E, l) => {
    l.push(w, o, 0, a, E, 0);
  }, v = Math.ceil(t / P) * P, c = Math.floor(A / P) * P;
  for (let w = v; w <= c + 1e-6; w += P)
    s(Q, w, B, w, Math.abs(w) < 1e-6 ? e : i);
  const n = Math.ceil(Q / P) * P, T = Math.floor(B / P) * P;
  for (let w = n; w <= T + 1e-6; w += P)
    s(w, t, w, A, Math.abs(w) < 1e-6 ? e : i);
  const z = new h.Group();
  return i.length > 0 && z.add(Xt(i, r.theme.colors.grid.minor, 0.25)), e.length > 0 && z.add(Xt(e, r.theme.colors.grid.major, 0.4)), z;
}
function Xt(r, Q, B) {
  const t = new h.BufferGeometry();
  t.setAttribute("position", new h.Float32BufferAttribute(r, 3));
...
function $P(r) {
  const Q = new h.Group(), { minX: B, maxX: t, minY: A, maxY: P } = Ut({
    sizeXMm: r.sizeXWorld,
    sizeYMm: r.sizeYWorld,
    bounds: r.bounds
  }), e = Math.max(1, r.depthWorld), i = Math.max(1, Math.min(Math.max(t - B, P - A) / 24, 30)), s = i * 0.6, v = (E) => new h.LineDashedMaterial({
    color: new h.Color(E),
    dashSize: i,
    gapSize: s,
    transparent: !0,
  ...
  const z = Math.max(2, i), w = MQ("X", r.theme.colors.axes.x, { size: 5 });
  w.position.set(t + z, 0, 0);
  const o = MQ("Y", r.theme.colors.axes.y, { size: 5 });
  o.position.set(0, P + z, 0);
  const a = MQ("Z", r.theme.colors.axes.z, { size: 5 });
  return a.position.set(0, 0, e + z), Q.add(w, o, a), Q;
}
function Qe(r) {
  r.traverse((Q) => {
    var B;
...
function Be(r) {
  const Q = new h.Group(), { minX: B, maxX: t, minY: A, maxY: P } = Ut(r), e = r.units === "mm" ? 10 : 1, i = e * 2, s = r.units === "mm" ? 1 : lA, v = 0.5, c = 4, n = 0.1, T = 5, z = r.theme.colors.axes.x, w = r.theme.colors.axes.y, o = { opacity: v, size: c };
  for (let a = e; ; a += i) {
    const E = a * s;
    if (E > t + 1e-6)
      break;
    const l = MQ(String(a), z, o);
...
var zQ = {
  ...
  overwrite: !1,
  delay: 0
}, Dt, q, y, hQ = 1e8, m = 1 / hQ, rt = Math.PI * 2, te = rt / 4, Ae = 0, UA = Math.sqrt, Pe = Math.cos, ee = Math.sin, Z = function(Q) {
...
}, $ = Array.isArray, ie = /random\([^)]+\)/g, re = /,\s*/g, Kt = /(?:-?\.?\d|\.)+/gi, uA = /[-+=.]*\d+[.e\-+]*\d*[e\-+]*\d*/g, qQ = /[-+=.]*\d+[.e-]*\d*[a-z%]*/g, JB = /[-+=.]*\d+\.?\d*(?:e-|e\+)?\d*/gi, dA = /[+-]=-?[.\d]+/, se = /[^,'"\[\]\s]+/gi, ve = /^[+\-=e\s\d]*\d+[.\d]*([a-z]*|%)\s*$/i, Y, kQ, st, Ct, wQ = {}, FB = {}, CA, NA = function(Q) {
  return (FB = tB(Q, wQ)) && rQ;
}, Nt = function(Q, B) {
  return console.warn("Invalid property", Q, "set to", B, "Missing plugin? gsap.registerPlugin()");
}, EB = function(Q, B) {
  return !B && console.warn(Q);
}, OA = function(Q, B) {
  return Q && (wQ[Q] = B) && FB && (FB[Q] = B) || wQ;
}, hB = function() {
  return 0;
}, ce = {
  suppressEvents: !0,
  ...
  suppressEvents: !0
}, Ot = {}, fQ = [], vt = {}, LA, cQ = {}, RB = {}, Zt = 30, MB = [], Lt = "", Mt = function(Q) {
  var B = Q[0], t, A;
  if (uQ(B) || J(B) || (Q = [Q]), !(t = (B._gsap || {}).harness)) {
    for (A = MB.length; A-- && !MB[A].targetTest(B); )
      ;
    t = MB[A];
  }
  for (A = Q.length; A--; )
    Q[A] && (Q[A]._gsap || (Q[A]._gsap = new WA(Q[A], t))) || Q.splice(A, 1);
  return Q;
}, YQ = function(Q) {
  return Q._gsap || Mt(lQ(Q))[0]._gsap;
}, MA = function(Q, B, t) {
  return (t = Q[B]) && J(t) ? Q[B]() : ut(t) && Q.getAttribute && Q.getAttribute(B) || t;
}, PQ = function(Q, B) {
  ...
  var t = B.charAt(0), A = parseFloat(B.substr(2));
  return Q = parseFloat(Q), t === "+" ? Q + A : t === "-" ? Q - A : t === "*" ? Q * A : Q / A;
}, oe = function(Q, B) {
  for (var t = B.length, A = 0; Q.indexOf(B[A]) < 0 && ++A < t; )
    ;
  return A < t;
}, fB = function() {
  var Q = fQ.length, B = fQ.slice(0), t, A;
  for (vt = {}, fQ.length = 0, t = 0; t < Q; t++)
    A = B[t], A && A._lazy && (A.render(A._lazy[0], A._lazy[1], !0)._lazy = 0);
}, Ht = function(Q) {
  return !!(Q._initted || Q._startAt || Q.add);
}, HA = function(Q, B, t, A) {
  fQ.length && !q && fB(), Q.render(B, t, !!(q && B < 0 && Ht(Q))), fQ.length && !q && fB();
}, jA = function(Q) {
  var B = parseFloat(Q);
  return (B || B === 0) && (Q + "").match(se).length < 2 ? B : Z(Q) ? Q.trim() : Q;
}, FA = function(Q) {
  return Q;
}, aQ = function(Q, B) {
  for (var t in B)
  ...
  return function(B, t) {
    for (var A in t)
      A in B || A === "duration" && Q || A === "ease" || (B[A] = t[A]);
  };
}, tB = function(Q, B) {
  for (var t in B)
    Q[t] = B[t];
  ...
  for (var t in B)
    t !== "__proto__" && t !== "constructor" && t !== "prototype" && (Q[t] = uQ(B[t]) ? r(Q[t] || (Q[t] = {}), B[t]) : B[t]);
  return Q;
}, GB = function(Q, B) {
  var t = {}, A;
  for (A in Q)
  ...
  var B = Q.parent || Y, t = Q.keyframes ? Te($(Q.keyframes)) : aQ;
  if (AQ(Q.inherit))
    for (; B; )
      t(Q, B.vars.defaults), B = B.parent || B._dp;
  return Q;
}, ze = function(Q, B) {
  for (var t = Q.length, A = t === B.length; A && t-- && Q[t] === B[t]; )
    ;
  return t < 0;
}, fA = function(Q, B, t, A, P) {
  var e = Q[A], i;
  if (P)
    for (i = B[P]; e && e[P] > i; )
      e = e._prev;
  return e ? (B._next = e._next, e._next = B) : (B._next = Q[t], Q[t] = B), B._next ? B._next._prev = B : Q[A] = B, B._prev = e, B.parent = B._dp = Q, B;
}, xB = function(Q, B, t, A) {
  t === void 0 && (t = "_first"), A === void 0 && (A = "_last");
  var P = B._prev, e = B._next;
  P ? P._next = e : Q[t] === B && (Q[t] = e), e ? e._prev = P : Q[A] === B && (Q[A] = P), B._next = B._prev = B.parent = null;
}, bQ = function(Q, B) {
  ...
  return Q._startAt && (q ? Q._startAt.revert(LB) : Q.vars.immediateRender && !Q.vars.autoRevert || Q._startAt.render(B, !0, A));
}, ae = function r(Q) {
  return !Q || Q._ts && r(Q.parent);
}, qt = function(Q) {
  return Q._repeat ? AB(Q._tTime, Q = Q.duration() + Q._rDelay) * Q : 0;
}, AB = function(Q, B) {
  var t = Math.floor(Q = S(Q / B));
  return Q && t === Q ? t - 1 : t;
}, bB = function(Q, B) {
  ...
  var t;
  if ((B._time || !B._dur && B._initted || B._start < Q._time && (B._dur || !B.add)) && (t = bB(Q.rawTime(), B), (!B._dur || dB(0, B.totalDuration(), t) - B._tTime > m) && B.render(t, !0)), VQ(Q, B)._dp && Q._initted && Q._time >= Q._dur && Q._ts) {
  ...
  return B.parent && bQ(B), B._start = S((NQ(t) ? t : t || Q !== Y ? EQ(Q, t, B) : Q._time) + B._delay), B._end = S(B._start + (B.totalDuration() / Math.abs(B.timeScale()) || 0)), fA(Q, B, "_first", "_last", Q._sort ? "_start" : 0), nt(B) || (Q._recent = B), A || GA(Q, B), Q._ts < 0 && IB(Q, Q._tTime), Q;
}, bA = function(Q, B) {
  return (wQ.ScrollTrigger || Nt("scrollTrigger", B)) && wQ.ScrollTrigger.create(B, Q);
}, gA = function(Q, B, t, A, P) {
  if (Ft(Q, B, P), !Q._initted)
    return 1;
  if (!t && Q._pt && !q && (Q._dur && Q.vars.lazy !== !1 || !Q._dur && Q.vars.lazy) && LA !== nQ.frame)
    return fQ.push(Q), Q._lazy = [P, A], 1;
}, Ee = function r(Q) {
  var B = Q.parent;
  ...
  var P = Q.ratio, e = B < 0 || !B && (!Q._start && Ee(Q) && !(!Q._initted && nt(Q)) || (Q._ts < 0 || Q._dp._ts < 0) && !nt(Q)) ? 0 : 1, i = Q._rDelay, s = 0, v, c, n;
  if (i && Q._repeat && (s = dB(0, Q._tDur, B), c = AB(s, i), Q._yoyo && c & 1 && (e = 1 - e), c !== AB(Q._tTime, i) && (P = 1 - e, Q.vars.repeatRefresh && Q._initted && Q.invalidate())), e !== P || q || A || Q._zTime === m || !B && Q._zTime) {
    if (!Q._initted && gA(Q, B, A, t, s))
      return;
    for (n = Q._zTime, Q._zTime = B || (t ? m : 0), t || (t = B && !n), Q.ratio = e, Q._from && (e = 1 - e), Q._time = 0, Q._tTime = s, v = Q._pt; v; )
      v.r(e, v.d), v = v._next;
    B < 0 && ct(Q, B, t, !0), Q._onUpdate && !t && oQ(Q, "onUpdate"), s && Q._repeat && !t && Q.parent && oQ(Q, "onRepeat"), (B >= Q._tDur || B < 0) && Q.ratio === e && (e && bQ(Q, 1), !t && !q && (oQ(Q, e ? "onComplete" : "onReverseComplete", !0), Q._prom && Q._prom()));
  } else Q._zTime || (Q._zTime = B);
}, le = function(Q, B, t) {
  var A;
  if (t > B)
    for (A = Q._first; A && A._start <= t; ) {
      if (A.data === "isPause" && A._start > B)
        return A;
      A = A._next;
    }
  else
    for (A = Q._last; A && A._start >= t; ) {
      if (A.data === "isPause" && A._start < B)
        return A;
      A = A._prev;
    }
}, PB = function(Q, B, t, A) {
  var P = Q._repeat, e = S(B) || 0, i = Q._tTime / Q._tDur;
  return i && !A && (Q._time *= e / Q._dur), Q._dur = e, Q._tDur = P ? P < 0 ? 1e10 : S(e * (P + 1) + Q._rDelay * P) : e, i > 0 && !A && IB(Q, Q._tTime = Q._tDur * i), Q.parent && yB(Q), t || VQ(Q.parent, Q), Q;
}, _t = function(Q) {
  return Q instanceof tQ ? VQ(Q) : PB(Q, Q._dur);
}, ke = {
  _start: 0,
  endTime: hB,
  totalDuration: hB
}, EQ = function r(Q, B, t) {
  var A = Q.labels, P = Q._recent || ke, e = Q.duration() >= hQ ? P.endTime(!1) : Q._dur, i, s, v;
  return Z(B) && (isNaN(B) || B in A) ? (s = B.charAt(0), v = B.substr(-1) === "%", i = B.indexOf("="), s === "<" || s === ">" ? (i >= 0 && (B = B.replace(/=/, "")), (s === "<" ? P._start : P.endTime(P._repeat >= 0)) + (parseFloat(B.substr(1)) || 0) * (v ? (i < 0 ? P : t).totalDuration() / 100 : 1)) : i < 0 ? (B in A || (A[B] = e), A[B]) : (s = parseFloat(B.charAt(i - 1) + B.substr(i + 1)), v && t && (s = s / 100 * ($(t) ? t[0] : t).totalDuration()), i > 1 ? r(Q, B.substr(0, i - 1), t) + s : e + s)) : B == null ? e : +B;
}, zB = function(Q, B, t) {
  var A = NQ(B[1]), P = (A ? 2 : 1) + (Q < 2 ? 0 : 1), e = B[P], i, s;
  if (A && (e.duration = B[1]), e.parent = t, Q) {
    for (i = e, s = t; s && !("immediateRender" in i); )
      i = s.vars.defaults || {}, s = AQ(s.vars.inherit) && s.parent;
    e.immediateRender = AQ(i.immediateRender), Q < 2 ? e.runBackwards = 1 : e.startAt = B[P - 1];
  }
  return new K(B[0], e, B[P + 1]);
}, mQ = function(Q, B) {
  return Q || Q === 0 ? B(Q) : B;
}, dB = function(Q, B, t) {
  return t < Q ? Q : t > B ? B : t;
}, _ = function(Q, B) {
  return !Z(Q) || !(B = ve.exec(Q)) ? "" : B[1];
}, Ue = function(Q, B, t) {
  return mQ(t, function(A) {
    return dB(Q, B, A);
  });
}, ot = [].slice, pA = function(Q, B) {
  return Q && uQ(Q) && "length" in Q && (!B && !Q.length || Q.length - 1 in Q && uQ(Q[0])) && !Q.nodeType && Q !== kQ;
}, De = function(Q, B, t) {
  return t === void 0 && (t = []), Q.forEach(function(A) {
    var P;
    return Z(A) && !B || pA(A, 1) ? (P = t).push.apply(P, lQ(A)) : t.push(A);
  }) || t;
}, lQ = function(Q, B, t) {
  return y && !B && y.selector ? y.selector(Q) : Z(Q) && !t && (st || !eB()) ? ot.call((B || Ct).querySelectorAll(Q), 0) : $(Q) ? De(Q, t) : pA(Q) ? ot.call(Q, 0) : Q ? [Q] : [];
}, Tt = function(Q) {
  ...
  var B = uQ(Q) ? Q : {
    each: Q
  }, t = JQ(B.ease), A = B.from || 0, P = parseFloat(B.base) || 0, e = {}, i = A > 0 && A < 1, s = isNaN(A) || i, v = B.axis, c = A, n = A;
  return Z(A) ? c = n = {
    center: 0.5,
    edges: 0.5,
    end: 1
  }[A] || 0 : !i && s && (c = A[0], n = A[1]), function(T, z, w) {
    var o = (w || B).length, a = e[o], E, l, U, D, k, O, u, N, d;
    if (!a) {
      if (d = B.grid === "auto" ? 0 : (B.grid || [1, hQ])[1], !d) {
        for (u = -hQ; u < (u = w[d++].getBoundingClientRect().left) && d < o; )
          ;
        d < o && d--;
      }
      for (a = e[o] = [], E = s ? Math.min(d, o) * c - 0.5 : A % d, l = d === hQ ? 0 : s ? o * n / d - 0.5 : A / d | 0, u = 0, N = hQ, O = 0; O < o; O++)
        U = O % d - E, D = l - (O / d | 0), a[O] = k = v ? Math.abs(v === "y" ? D : U) : UA(U * U + D * D), k > u && (u = k), k < N && (N = k);
      A === "random" && mA(a), a.max = u - N, a.min = N, a.v = o = (parseFloat(B.amount) || parseFloat(B.each) * (d > o ? o - 1 : v ? v === "y" ? o / d : d : Math.max(d, o / d)) || 0) * (A === "edges" ? -1 : 1), a.b = o < 0 ? P - o : P, a.u = _(B.amount || B.each) || 0, t = t && o < 0 ? be(t) : t;
    }
  ...
  return function(t) {
    var A = S(Math.round(parseFloat(t) / Q) * Q * B);
    return (A - A % 1) / B + (NQ(t) ? 0 : _(t));
  };
}, yA = function(Q, B) {
  var t = $(Q), A, P;
  return !t && uQ(Q) && (A = t = Q.radius || hQ, Q.values ? (Q = lQ(Q.values), (P = !NQ(Q[0])) && (A *= A)) : Q = zt(Q.increment)), mQ(B, t ? J(Q) ? function(e) {
    return P = Q(e), Math.abs(P - e) <= A ? P : e;
  } : function(e) {
    for (var i = parseFloat(P ? e.x : e), s = parseFloat(P ? e.y : 0), v = hQ, c = 0, n = Q.length, T, z; n--; )
      P ? (T = Q[n].x - i, z = Q[n].y - s, T = T * T + z * z) : T = Math.abs(Q[n] - i), T < v && (v = T, c = n);
    return c = !A || v <= A ? Q[c] : e, P || c === e || NQ(e) ? c : c + _(e);
  } : zt(Q));
}, IA = function(Q, B, t, A) {
  return mQ($(Q) ? !B : t === !0 ? !!(t = 0) : !A, function() {
  ...
  for (var Q = arguments.length, B = new Array(Q), t = 0; t < Q; t++)
    B[t] = arguments[t];
  return function(A) {
  ...
  var A = B - Q;
  return $(Q) ? SA(Q, r(0, Q.length), B) : mQ(t, function(P) {
    return (A + (P - Q) % A) % A + Q;
  });
}, Oe = function r(Q, B, t) {
  var A = B - Q, P = A * 2;
  return $(Q) ? SA(Q, r(0, Q.length - 1), B) : mQ(t, function(e) {
    return e = (P + (e - Q) % P) % P || 0, Q + (e > A ? P - e : e);
  });
}, lB = function(Q) {
  return Q.replace(ie, function(B) {
    var t = B.indexOf("[") + 1, A = B.substring(t || 7, t ? B.indexOf("]") : B.length - 1).split(re);
    return IA(t ? A : +A[0], t ? 0 : +A[1], +A[2] || 1e-5);
  });
}, YA = function(Q, B, t, A, P) {
  var e = B - Q, i = A - t;
  return mQ(P, function(s) {
    return t + ((s - Q) / e * i || 0);
  });
}, Le = function r(Q, B, t, A) {
  var P = isNaN(Q + B) ? 0 : function(z) {
    return (1 - z) * Q + z * B;
  };
  if (!P) {
    var e = Z(Q), i = {}, s, v, c, n, T;
    if (t === !0 && (A = 1) && (t = null), e)
      Q = {
        p: Q
      }, B = {
    ...
    if (!c) {
      for (s in B)
        jt.call(i, Q, s, "get", B[s]);
      P = function(w) {
        return bt(w, i) || (e ? Q.p : Q);
      };
    }
  }
  return mQ(t, P);
}, $t = function(Q, B, t) {
  var A = Q.labels, P = hQ, e, i, s;
  for (e in A)
    i = A[e] - B, i < 0 == !!t && i && P > (i = Math.abs(i)) && (s = e, P = i);
  return s;
}, oQ = function(Q, B, t) {
  var A = Q.vars, P = A[B], e = y, i = Q._ctx, s, v, c;
  if (P)
    return s = A[B + "Params"], v = A.callbackScope || Q, t && fQ.length && fB(), i && (y = i), c = s ? P.apply(v, s) : P.call(v), y = e, c;
}, nB = function(Q) {
  return bQ(Q), Q.scrollTrigger && Q.scrollTrigger.kill(!!q), Q.progress() < 1 && oQ(Q, "onInterrupt"), Q;
}, _Q, VA = [], JA = function(Q) {
  if (Q)
    if (Q = !Q.name && Q.default || Q, dt() || Q.headless) {
      var B = Q.name, t = J(Q), A = B && !t && Q.init ? function() {
        this._props = [];
      } : Q, P = {
        init: hB,
        render: bt,
      ...
      if (eB(), Q !== A) {
        if (cQ[B])
          return;
        aQ(A, aQ(GB(Q, P), e)), tB(A.prototype, tB(P, GB(Q, e))), cQ[A.prop = B] = A, Q.targetTest && (MB.push(A), Ot[B] = 1), B = (B === "css" ? "CSS" : B.charAt(0).toUpperCase() + B.substr(1)) + "Plugin";
      }
      OA(B, A), Q.register && Q.register(rQ, A, eQ);
    } else
      VA.push(Q);
}, p = 255, oB = {
  aqua: [0, p, p],
  ...
  return Q += Q < 0 ? 1 : Q > 1 ? -1 : 0, (Q * 6 < 1 ? B + (t - B) * Q * 6 : Q < 0.5 ? t : Q * 3 < 2 ? B + (t - B) * (2 / 3 - Q) * 6 : B) * p + 0.5 | 0;
}, RA = function(Q, B, t) {
  var A = Q ? NQ(Q) ? [Q >> 16, Q >> 8 & p, Q & p] : 0 : oB.black, P, e, i, s, v, c, n, T, z, w;
  if (!A) {
    if (Q.substr(-1) === "," && (Q = Q.substr(0, Q.length - 1)), oB[Q])
      A = oB[Q];
    else if (Q.charAt(0) === "#") {
      ...
      if (A = w = Q.match(Kt), !B)
        s = +A[0] % 360 / 360, v = +A[1] / 100, c = +A[2] / 100, e = c <= 0.5 ? c * (v + 1) : c + v - c * v, P = c * 2 - e, A.length > 3 && (A[3] *= 1), A[0] = XB(s + 1 / 3, P, e), A[1] = XB(s, P, e), A[2] = XB(s - 1 / 3, P, e);
      else if (~Q.indexOf("="))
        return A = Q.match(uA), t && A.length < 4 && (A[3] = 1), A;
    } else
      A = Q.match(Kt) || oB.transparent;
    A = A.map(Number);
  }
  return B && !w && (P = A[0] / p, e = A[1] / p, i = A[2] / p, n = Math.max(P, e, i), T = Math.min(P, e, i), c = (n + T) / 2, n === T ? s = v = 0 : (z = n - T, v = c > 0.5 ? z / (2 - n - T) : z / (n + T), s = n === P ? (e - i) / z + (e < i ? 6 : 0) : n === e ? (i - P) / z + 2 : (P - e) / z + 4, s *= 60), A[0] = ~~(s + 0.5), A[1] = ~~(v * 100 + 0.5), A[2] = ~~(c * 100 + 0.5)), t && A.length < 4 && (A[3] = 1), A;
}, XA = function(Q) {
  var B = [], t = [], A = -1;
  return Q.split(GQ).forEach(function(P) {
    var e = P.match(qQ) || [];
    B.push.apply(B, e), t.push(A += e.length + 1);
  }), B.c = t, B;
}, QA = function(Q, B, t) {
  var A = "", P = (Q + A).match(GQ), e = B ? "hsla(" : "rgba(", i = 0, s, v, c, n;
  if (!P)
    return Q;
  if (P = P.map(function(T) {
    return (T = RA(T, B, 1)) && e + (B ? T[0] + "," + T[1] + "%," + T[2] + "%," + T[3] : T.join(",")) + ")";
  }), t && (c = XA(Q), s = t.c, s.join(A) !== c.c.join(A)))
    for (v = Q.replace(GQ, "1").split(qQ), n = v.length - 1; i < n; i++)
      A += v[i] + (~s.indexOf(i) ? P.shift() || e + "0,0,0,0)" : (c.length ? c : P.length ? P : t).shift());
  if (!v)
    for (v = Q.split(GQ), n = v.length - 1; i < n; i++)
      A += v[i] + P[i];
  return A + v[n];
}, GQ = function() {
...
}(), Me = /hsl[a]?\(/, KA = function(Q) {
  var B = Q.join(" "), t;
  if (GQ.lastIndex = 0, GQ.test(B))
    return t = Me.test(B), Q[1] = QA(Q[1], t), Q[0] = QA(Q[0], t, XA(Q[1])), !0;
}, kB, nQ = function() {
  var r = Date.now, Q = 500, B = 33, t = r(), A = t, P = 1e3 / 240, e = P, i = [], s, v, c, n, T, z, w = function o(a) {
    var E = r() - A, l = a === !0, U, D, k, O;
    if ((E > Q || E < 0) && (t += E - B), A += E, k = A - t, U = k - e, (U > 0 || l) && (O = ++n.frame, T = k - n.time * 1e3, n.time = k = k / 1e3, e += U + (U >= P ? 4 : P - U), D = 1), l || (s = v(o)), D)
  ...
  return n = {
    ...
    wake: function() {
      CA && (!st && dt() && (kQ = st = window, Ct = kQ.document || {}, wQ.gsap = rQ, (kQ.gsapVersions || (kQ.gsapVersions = [])).push(rQ.version), NA(FB || kQ.GreenSockGlobals || !kQ.gsap && kQ || {}), VA.forEach(JA)), c = typeof requestAnimationFrame < "u" && requestAnimationFrame, s && n.sleep(), v = c || function(a) {
        return setTimeout(a, e - n.time * 1e3 + 1 | 0);
      }, kB = 1, w(2));
    },
    sleep: function() {
      (c ? cancelAnimationFrame : clearTimeout)(s), kB = 0, v = hB;
    },
    lagSmoothing: function(a, E) {
      Q = a || 1 / 0, B = Math.min(E || 33, Q);
    },
    fps: function(a) {
      P = 1e3 / (a || 240), e = n.time * 1e3 + P;
    },
    add: function(a, E, l) {
      var U = E ? function(D, k, O, u) {
        a(D, k, O, u), n.remove(U);
      } : a;
      return n.remove(a), i[l ? "unshift" : "push"](U), eB(), U;
    },
    remove: function(a, E) {
      ~(E = i.indexOf(a)) && i.splice(E, 1) && z >= E && z--;
    },
    _listeners: i
  }, n;
}(), eB = function() {
  return !kB && nQ.wake();
}, g = {}, He = /^[\d.\-M][\d.\-,\s]/, je = /["']/g, Fe = function(Q) {
  for (var B = {}, t = Q.substr(1, Q.length - 3).split(":"), A = t[0], P = 1, e = t.length, i, s, v; P < e; P++)
    s = t[P], i = P !== e - 1 ? s.lastIndexOf(",") : s.length, v = s.substr(0, i), B[A] = isNaN(v) ? v.replace(je, "").trim() : +v, A = s.substr(i + 1).trim();
  return B;
}, fe = function(Q) {
  var B = Q.indexOf("(") + 1, t = Q.indexOf(")"), A = Q.indexOf("(", B);
  return Q.substring(B, ~A && A < t ? Q.indexOf(")", t + 1) : t);
}, Ge = function(Q) {
  var B = (Q + "").split("("), t = g[B[0]];
  return t && B.length > 1 && t.config ? t.config.apply(null, ~Q.indexOf("{") ? [Fe(B[1])] : fe(Q).split(",").map(jA)) : g._CE && He.test(Q) ? g._CE("", Q) : t;
}, be = function(Q) {
  return function(B) {
    return 1 - Q(1 - B);
  };
}, JQ = function(Q, B) {
  return Q && (J(Q) ? Q : g[Q] || Ge(Q)) || B;
}, XQ = function(Q, B, t, A) {
  ...
  return PQ(Q, function(i) {
    g[i] = wQ[i] = P, g[e = i.toLowerCase()] = t;
    for (var s in P)
  ...
  var A = B >= 1 ? B : 1, P = (t || (Q ? 0.3 : 0.45)) / (B < 1 ? B : 1), e = P / rt * (Math.asin(1 / A) || 0), i = function(c) {
    return c === 1 ? 1 : A * Math.pow(2, -10 * c) * ee((c - e) * P) + 1;
  }, s = Q === "out" ? i : Q === "in" ? function(v) {
    return 1 - i(1 - v);
  } : ZA(i);
  return P = rt / P, s.config = function(v, c) {
    return r(Q, v, c);
  }, s;
}, ZB = function r(Q, B) {
  B === void 0 && (B = 1.70158);
  ...
  }, A = Q === "out" ? t : Q === "in" ? function(P) {
    return 1 - t(1 - P);
  } : ZA(t);
  return A.config = function(P) {
    return r(Q, P);
  }, A;
};
PQ("Linear,Quad,Cubic,Quart,Quint,Strong", function(r, Q) {
  var B = Q < 5 ? Q + 1 : Q;
  XQ(r + ",Power" + (B - 1), Q ? function(t) {
    return Math.pow(t, B);
  } : function(t) {
    return t;
  }, function(t) {
    return 1 - Math.pow(1 - t, B);
  }, function(t) {
    return t < 0.5 ? Math.pow(t * 2, B) / 2 : 1 - Math.pow((1 - t) * 2, B) / 2;
  });
});
g.Linear.easeNone = g.none = g.Linear.easeIn;
XQ("Elastic", KB("in"), KB("out"), KB());
(function(r, Q) {
  var B = 1 / Q, t = 2 * B, A = 2.5 * B, P = function(i) {
    return i < B ? r * i * i : i < t ? r * Math.pow(i - 1.5 / Q, 2) + 0.75 : i < A ? r * (i -= 2.25 / Q) * i + 0.9375 : r * Math.pow(i - 2.625 / Q, 2) + 0.984375;
  };
  XQ("Bounce", function(e) {
    return 1 - P(1 - e);
  }, P);
})(7.5625, 2.75);
XQ("Expo", function(r) {
...
XQ("Sine", function(r) {
  return r === 1 ? 1 : -Pe(r * te) + 1;
});
XQ("Back", ZB("in"), ZB("out"), ZB());
g.SteppedEase = g.steps = wQ.SteppedEase = {
  config: function(Q, B) {
    Q === void 0 && (Q = 1);
    var t = 1 / Q, A = Q + (B ? 0 : 1), P = B ? 1 : 0, e = 1 - m;
    return function(i) {
      return ((A * dB(0, e, i) | 0) + P) * t;
    };
  }
};
aB.ease = g["quad.out"];
PQ("onComplete,onUpdate,onStart,onRepeat,onReverseComplete,onInterrupt", function(r) {
  return Lt += r + "," + r + "Params,";
});
var WA = function(Q, B) {
  this.id = Ae++, Q._gsap = this, this.target = Q, this.harness = B, this.get = B ? B.get : MA, this.set = B ? B.getSetter : Gt;
}, UB = /* @__PURE__ */ function() {
  function r(B) {
    this.vars = B, this._delay = +B.delay || 0, (this._repeat = B.repeat === 1 / 0 ? -2 : B.repeat || 0) && (this._rDelay = B.repeatDelay || 0, this._yoyo = !!B.yoyo || !!B.yoyoEase), this._ts = 1, PB(this, +B.duration, 1, 1), this.data = B.data, y && (this._ctx = y, y.data.push(this)), kB || nQ.wake();
  }
  var Q = r.prototype;
  return Q.delay = function(t) {
  ...
  }, Q.totalDuration = function(t) {
    return arguments.length ? (this._dirty = 0, PB(this, this._repeat < 0 ? t : (t - this._repeat * this._rDelay) / (this._repeat + 1))) : this._tDur;
  }, Q.totalTime = function(t, A) {
    if (eB(), !arguments.length)
      return this._tTime;
    var P = this._dp;
    if (P && P.smoothChildTiming && this._ts) {
      for (IB(this, t), !P._dp || P.parent || GA(P, this); P && P.parent; )
    ...
    return (this._tTime !== t || !this._dur && !A || this._initted && Math.abs(this._zTime) === m || !this._initted && this._dur && t || !t && !this._initted && (this.add || this._ptLookup)) && (this._ts || (this._pTime = t), HA(this, t, A)), this;
  }, Q.time = function(t, A) {
    return arguments.length ? this.totalTime(Math.min(this.totalDuration(), t + qt(this)) % (this._dur + this._rDelay) || (t ? this._dur : 0), A) : this._time;
  }, Q.totalProgress = function(t, A) {
    return arguments.length ? this.totalTime(this.totalDuration() * t, A) : this.totalDuration() ? Math.min(1, this._tTime / this._tDur) : this.rawTime() >= 0 && this._initted ? 1 : 0;
  }, Q.progress = function(t, A) {
    return arguments.length ? this.totalTime(this.duration() * (this._yoyo && !(this.iteration() & 1) ? 1 - t : t) + qt(this), A) : this.duration() ? Math.min(1, this._time / this._dur) : this.rawTime() > 0 ? 1 : 0;
  }, Q.iteration = function(t, A) {
    var P = this.duration() + this._rDelay;
    return arguments.length ? this.totalTime(this._time + (t - 1) * P, A) : this._repeat ? AB(this._tTime, P) + 1 : 1;
  }, Q.timeScale = function(t, A) {
    if (!arguments.length)
      return this._rts === -m ? 0 : this._rts;
    if (this._rts === t)
      return this;
    var P = this.parent && this._ts ? bB(this.parent._time, this) : this._tTime;
    return this._rts = +t || 0, this._ts = this._ps || t === -m ? 0 : this._rts, this.totalTime(dB(-Math.abs(this._delay), this.totalDuration(), P), A !== !1), yB(this), we(this);
  }, Q.paused = function(t) {
    return arguments.length ? (this._ps !== t && (this._ps = t, t ? (this._pTime = this._tTime || Math.max(-this._delay, this.rawTime()), this._ts = this._act = 0) : (eB(), this._ts = this._rts, this.totalTime(this.parent && !this.parent.smoothChildTiming ? this.rawTime() : this._tTime || this._pTime, this.progress() === 1 && Math.abs(this._zTime) !== m && (this._tTime -= m)))), this) : this._ps;
    ...
    return this._start;
  }, Q.endTime = function(t) {
    return this._start + (AQ(t) ? this.totalDuration() : this.duration()) / Math.abs(this._ts || 1);
  }, Q.rawTime = function(t) {
    var A = this.parent || this._dp;
    return A ? t && (!this._ts || this._repeat && this._time && this.totalProgress() < 1) ? this._tTime % (this._dur + this._rDelay) : this._ts ? bB(A.rawTime(t), this) : this._tTime : this._tTime;
  }, Q.revert = function(t) {
    t === void 0 && (t = ne);
    var A = q;
    return q = t, Ht(this) && (this.timeline && this.timeline.revert(t), this.totalTime(-0.01, t.suppressEvents)), this.data !== "nested" && t.kill !== !1 && this.kill(), q = A, this;
  }, Q.globalTime = function(t) {
    for (var A = this, P = arguments.length ? t : A.rawTime(); A; )
      P = A._start + P / (Math.abs(A._ts) || 1), A = A._dp;
    return !this.parent && this._sat ? this._sat.globalTime(t) : P;
  }, Q.repeat = function(t) {
    return arguments.length ? (this._repeat = t === 1 / 0 ? -2 : t, _t(this)) : this._repeat === -2 ? 1 / 0 : this._repeat;
  }, Q.repeatDelay = function(t) {
    if (arguments.length) {
      var A = this._time;
      return this._rDelay = t, _t(this), A ? this.time(A) : this;
    }
    return this._rDelay;
  }, Q.yoyo = function(t) {
    ...
    return this.play().totalTime(t ? -this._delay : 0, AQ(A)), this._dur || (this._zTime = -m), this;
  }, Q.play = function(t, A) {
    return t != null && this.seek(t, A), this.reversed(!1).paused(!1);
  }, Q.reverse = function(t, A) {
    return t != null && this.seek(t || this.totalDuration(), A), this.reversed(!0).paused(!1);
  }, Q.pause = function(t, A) {
    return t != null && this.seek(t, A), this.paused(!0);
  }, Q.resume = function() {
    ...
    var t = this.parent || this._dp, A = this._start, P;
    return !!(!t || this._ts && this._initted && t.isActive() && (P = t.rawTime(!0)) >= A && P < this.endTime(!0) - m);
  }, Q.eventCallback = function(t, A, P) {
    var e = this.vars;
    return arguments.length > 1 ? (A ? (e[t] = A, P && (e[t + "Params"] = P), t === "onUpdate" && (this._onUpdate = A)) : delete e[t], this) : e[t];
  }, Q.then = function(t) {
    var A = this, P = A._prom;
    return new Promise(function(e) {
      var i = J(t) ? t : FA, s = function() {
        var c = A.then;
        A.then = null, P && P(), J(i) && (i = i(A)) && (i.then || i === A) && (A.then = c), e(i), A.then = c;
      };
...
var tQ = /* @__PURE__ */ function(r) {
  kA(Q, r);
  function Q(t, A) {
    var P;
    return t === void 0 && (t = {}), P = r.call(this, t) || this, P.labels = {}, P.smoothChildTiming = !!t.smoothChildTiming, P.autoRemoveChildren = !!t.autoRemoveChildren, P._sort = AQ(t.sortChildren), Y && UQ(t.parent || Y, dQ(P), A), t.reversed && P.reverse(), t.paused && P.paused(!0), t.scrollTrigger && bA(dQ(P), t.scrollTrigger), P;
  }
  var B = Q.prototype;
  return B.to = function(A, P, e) {
    return zB(0, arguments, this), this;
  }, B.from = function(A, P, e) {
    return zB(1, arguments, this), this;
  }, B.fromTo = function(A, P, e, i) {
    return zB(2, arguments, this), this;
  }, B.set = function(A, P, e) {
    return P.duration = 0, P.parent = this, TB(P).repeatDelay || (P.repeat = 0), P.immediateRender = !!P.immediateRender, new K(A, P, EQ(this, e), 1), this;
  }, B.call = function(A, P, e) {
    return UQ(this, K.delayedCall(0, A, P), e);
  }, B.staggerTo = function(A, P, e, i, s, v, c) {
    return e.duration = P, e.stagger = e.stagger || i, e.onComplete = v, e.onCompleteParams = c, e.parent = this, new K(A, e, EQ(this, s)), this;
  }, B.staggerFrom = function(A, P, e, i, s, v, c) {
    return e.runBackwards = 1, TB(e).immediateRender = AQ(e.immediateRender), this.staggerTo(A, P, e, i, s, v, c);
  }, B.staggerFromTo = function(A, P, e, i, s, v, c, n) {
    return i.startAt = e, TB(i).immediateRender = AQ(i.immediateRender), this.staggerTo(A, P, i, s, v, c, n);
  }, B.render = function(A, P, e) {
    var i = this._time, s = this._dirty ? this.totalDuration() : this._tDur, v = this._dur, c = A <= 0 ? 0 : S(A), n = this._zTime < 0 != A < 0 && (this._initted || !v), T, z, w, o, a, E, l, U, D, k, O, u;
    if (this !== Y && c > s && A >= 0 && (c = s), c !== this._tTime || e || n) {
      if (i !== this._time && v && (c += this._time - i, A += this._time - i), T = c, D = this._start, U = this._ts, E = !U, n && (v || (i = this._zTime), (A || !P) && (this._zTime = A)), this._repeat) {
        if (O = this._yoyo, a = v + this._rDelay, this._repeat < -1 && A < 0)
          return this.totalTime(a * 100 + A, P, e);
        if (T = S(c % a), c === s ? (o = this._repeat, T = v) : (k = S(c / a), o = ~~k, o && o === k && (T = v, o--), T > v && (T = v)), k = AB(this._tTime, a), !i && this._tTime && k !== o && this._tTime - k * a - this._dur <= 0 && (k = o), O && o & 1 && (T = v - T, u = 1), o !== k && !this._lock) {
          var N = O && k & 1, d = N === (O && o & 1);
          if (o < k && (N = !N), i = N ? 0 : c % v ? v : c, this._lock = 1, this.render(i || (u ? 0 : S(o * a)), P, !v)._lock = 0, this._tTime = c, !P && this.parent && oQ(this, "onRepeat"), this.vars.repeatRefresh && !u && (this.invalidate()._lock = 1, k = o), i && i !== this._time || E !== !this._ts || this.vars.onRepeat && !this.parent && !this._act)
            return this;
          if (v = this._dur, s = this._tDur, d && (this._lock = 2, i = N ? v : -1e-4, this.render(i, !0), this.vars.repeatRefresh && !u && this.invalidate()), this._lock = 0, !this._ts && !E)
            return this;
        }
      }
      if (this._hasPause && !this._forcing && this._lock < 2 && (l = le(this, S(i), S(T)), l && (c -= T - (T = l._start))), this._tTime = c, this._time = T, this._act = !!U, this._initted || (this._onUpdate = this.vars.onUpdate, this._initted = 1, this._zTime = A, i = 0), !i && c && v && !P && !k && (oQ(this, "onStart"), this._tTime !== c))
        return this;
      if (T >= i && A >= 0)
        for (z = this._first; z; ) {
          if (w = z._next, (z._act || T >= z._start) && z._ts && l !== z) {
            if (z.parent !== this)
              return this.render(A, P, e);
            if (z.render(z._ts > 0 ? (T - z._start) * z._ts : (z._dirty ? z.totalDuration() : z._tDur) + (T - z._start) * z._ts, P, e), T !== this._time || !this._ts && !E) {
        ...
        z = this._last;
        for (var M = A < 0 ? A : T; z; ) {
          if (w = z._prev, (z._act || M <= z._end) && z._ts && l !== z) {
            if (z.parent !== this)
              return this.render(A, P, e);
            if (z.render(z._ts > 0 ? (M - z._start) * z._ts : (z._dirty ? z.totalDuration() : z._tDur) + (M - z._start) * z._ts, P, e || q && Ht(z)), T !== this._time || !this._ts && !E) {
              l = 0, w && (c += this._zTime = M ? -m : m);
              break;
            }
          }
          z = w;
      ...
      if (l && !P && (this.pause(), l.render(T >= i ? 0 : -m)._zTime = T >= i ? 1 : -1, this._ts))
        return this._start = D, yB(this), this.render(A, P, e);
      this._onUpdate && !P && oQ(this, "onUpdate", !0), (c === s && this._tTime >= this.totalDuration() || !c && i) && (D === this._start || Math.abs(U) !== Math.abs(this._ts)) && (this._lock || ((A || !v) && (c === s && this._ts > 0 || !c && this._ts < 0) && bQ(this, 1), !P && !(A < 0 && !i) && (c || i || !s) && (oQ(this, c === s && A >= 0 ? "onComplete" : "onReverseComplete", !0), this._prom && !(c < s && this.timeScale() > 0) && this._prom())));
    }
    return this;
  }, B.add = function(A, P) {
    var e = this;
    if (NQ(P) || (P = EQ(this, P, A)), !(A instanceof UB)) {
      if ($(A))
        return A.forEach(function(i) {
          return e.add(i, P);
        }), this;
      if (Z(A))
        return this.addLabel(A, P);
      if (J(A))
        A = K.delayedCall(0, A);
      else
        return this;
    }
    return this !== A ? UQ(this, A, P) : this;
  }, B.getChildren = function(A, P, e, i) {
    A === void 0 && (A = !0), P === void 0 && (P = !0), e === void 0 && (e = !0), i === void 0 && (i = -hQ);
    for (var s = [], v = this._first; v; )
      v._start >= i && (v instanceof K ? P && s.push(v) : (e && s.push(v), A && s.push.apply(s, v.getChildren(!0, P, e)))), v = v._next;
    return s;
    ...
    return Z(A) ? this.removeLabel(A) : J(A) ? this.killTweensOf(A) : (A.parent === this && xB(this, A), A === this._recent && (this._recent = this._last), VQ(this));
  }, B.totalTime = function(A, P) {
    return arguments.length ? (this._forcing = 1, !this._dp && this._ts && (this._start = S(nQ.time - (this._ts > 0 ? A / this._ts : (this.totalDuration() - A) / -this._ts))), r.prototype.totalTime.call(this, A, P), this._forcing = 0, this) : this._tTime;
  }, B.addLabel = function(A, P) {
    return this.labels[A] = EQ(this, P), this;
  }, B.removeLabel = function(A) {
    return delete this.labels[A], this;
  }, B.addPause = function(A, P, e) {
    var i = K.delayedCall(0, P || hB, e);
    return i.data = "isPause", this._hasPause = 1, UQ(this, i, EQ(this, A));
  }, B.removePause = function(A) {
    var P = this._first;
    for (A = EQ(this, A); P; )
      P._start === A && P.data === "isPause" && bQ(P), P = P._next;
  }, B.killTweensOf = function(A, P, e) {
    for (var i = this.getTweensOf(A, e), s = i.length; s--; )
      HQ !== i[s] && i[s].kill(A, P);
    return this;
  }, B.getTweensOf = function(A, P) {
    for (var e = [], i = lQ(A), s = this._first, v = NQ(P), c; s; )
      s instanceof K ? oe(s._targets, i) && (v ? (!HQ || s._initted && s._ts) && s.globalTime(0) <= P && s.globalTime(s.totalDuration()) > P : !P || s.isActive()) && e.push(s) : (c = s.getTweensOf(i, P)).length && e.push.apply(e, c), s = s._next;
    return e;
  }, B.tweenTo = function(A, P) {
    P = P || {};
    var e = this, i = EQ(e, A), s = P, v = s.startAt, c = s.onStart, n = s.onStartParams, T = s.immediateRender, z, w = K.to(e, aQ({
      ease: P.ease || "none",
      lazy: !1,
      immediateRender: !1,
      time: i,
      overwrite: "auto",
      duration: P.duration || Math.abs((i - (v && "time" in v ? v.time : e._time)) / e.timeScale()) || m,
      onStart: function() {
        if (e.pause(), !z) {
          var a = P.duration || Math.abs((i - (v && "time" in v ? v.time : e._time)) / e.timeScale());
          w._dur !== a && PB(w, a, 0, 1).render(w._time, !0, !0), z = 1;
        }
        c && c.apply(w, n || []);
      }
    }, P));
    return T ? w.render(0) : w;
  }, B.tweenFromTo = function(A, P, e) {
    return this.tweenTo(P, aQ({
      startAt: {
        time: EQ(this, A)
      }
    }, e));
    ...
    return this._recent;
  }, B.nextLabel = function(A) {
    return A === void 0 && (A = this._time), $t(this, EQ(this, A));
  }, B.previousLabel = function(A) {
    return A === void 0 && (A = this._time), $t(this, EQ(this, A), 1);
  }, B.currentLabel = function(A) {
    return arguments.length ? this.seek(A, !0) : this.previousLabel(this._time + m);
  }, B.shiftChildren = function(A, P, e) {
    e === void 0 && (e = 0);
    var i = this._first, s = this.labels, v;
    for (A = S(A); i; )
      i._start >= e && (i._start += A, i._end += A), i = i._next;
    if (P)
      for (v in s)
        s[v] >= e && (s[v] += A);
    return VQ(this);
    ...
    var P = 0, e = this, i = e._last, s = hQ, v, c, n;
    if (arguments.length)
      return e.timeScale((e._repeat < 0 ? e.duration() : e.totalDuration()) / (e.reversed() ? -A : A));
    if (e._dirty) {
      for (n = e.parent; i; )
        v = i._prev, i._dirty && i.totalDuration(), c = i._start, c > s && e._sort && i._ts && !e._lock ? (e._lock = 1, UQ(e, i, c - i._delay, 1)._lock = 0) : s = c, c < 0 && i._ts && (P -= c, (!n && !e._dp || n && n.smoothChildTiming) && (e._start += S(c / e._ts), e._time -= c, e._tTime -= c), e.shiftChildren(-c, !1, -1 / 0), s = 0), i._end > P && i._ts && (P = i._end), i = v;
      PB(e, e === Y && e._time > P ? e._time : P, 1, 1), e._dirty = 0;
    }
    return e._tDur;
  }, Q.updateRoot = function(A) {
    if (Y._ts && (HA(Y, bB(A, Y)), LA = nQ.frame), nQ.frame >= Zt) {
      Zt += zQ.autoSleep || 120;
      var P = Y._first;
      if ((!P || !P._ts) && zQ.autoSleep && nQ._listeners.length < 2) {
        for (; P && !P._ts; )
          P = P._next;
        P || nQ.sleep();
      }
...
var ge = function(Q, B, t, A, P, e, i) {
  var s = new eQ(this._pt, Q, B, 0, 1, tP, null, P), v = 0, c = 0, n, T, z, w, o, a, E, l;
  for (s.b = t, s.e = A, t += "", A += "", (E = ~A.indexOf("random(")) && (A = lB(A)), e && (l = [t, A], e(l, Q, B), t = l[0], A = l[1]), T = t.match(JB) || []; n = JB.exec(A); )
    w = n[0], o = A.substring(v, n.index), z ? z = (z + 1) % 5 : o.substr(-5) === "rgba(" && (z = 1), w !== T[c++] && (a = parseFloat(T[c - 1]) || 0, s._pt = {
      ...
      s: a,
      c: w.charAt(1) === "=" ? $Q(a, w) - a : parseFloat(w) - a,
      m: z && z < 4 ? Math.round : 0
    }, v = JB.lastIndex);
  return s.c = v < A.length ? A.substring(v, A.length) : "", s.fp = i, (dA.test(A) || E) && (s.e = 0), this._pt = s, s;
}, jt = function(Q, B, t, A, P, e, i, s, v, c) {
  J(A) && (A = A(P || 0, Q, e));
  var n = Q[B], T = t !== "get" ? t : J(n) ? v ? Q[B.indexOf("set") || !J(Q["get" + B.substr(3)]) ? B : "get" + B.substr(3)](v) : Q[B]() : n, z = J(n) ? v ? Ie : QP : ft, w;
  if (Z(A) && (~A.indexOf("random(") && (A = lB(A)), A.charAt(1) === "=" && (w = $Q(T, A) + (_(T) || 0), (w || w === 0) && (A = w))), !c || T !== A || wt)
    return !isNaN(T * A) && A !== "" ? (w = new eQ(this._pt, Q, B, +T || 0, A - (T || 0), typeof n == "boolean" ? Ye : BP, 0, z), v && (w.fp = v), i && w.modifier(i, this, Q), this._pt = w) : (!n && !(B in Q) && Nt(B, A), ge.call(this, Q, B, T, A, z, s || zQ.stringFilter, v));
}, pe = function(Q, B, t, A, P) {
  if (J(Q) && (Q = wB(Q, P, B, t, A)), !uQ(Q) || Q.style && Q.nodeType || $(Q) || DA(Q))
    return Z(Q) ? wB(Q, P, B, t, A) : Q;
  var e = {}, i;
  for (i in Q)
    e[i] = wB(Q[i], P, B, t, A);
  return e;
}, qA = function(Q, B, t, A, P, e) {
  var i, s, v, c;
  if (cQ[Q] && (i = new cQ[Q]()).init(P, i.rawVars ? B[Q] : pe(B[Q], A, P, e, t), t, A, e) !== !1 && (t._pt = s = new eQ(t._pt, P, Q, 0, 1, i.render, i, 0, i.priority), t !== _Q))
    for (v = t._ptLookup[t._targets.indexOf(P)], c = i._props.length; c--; )
      v[i._props[c]] = s;
  return i;
}, HQ, wt, Ft = function r(Q, B, t) {
  var A = Q.vars, P = A.ease, e = A.startAt, i = A.immediateRender, s = A.lazy, v = A.onUpdate, c = A.runBackwards, n = A.yoyoEase, T = A.keyframes, z = A.autoRevert, w = Q._dur, o = Q._startAt, a = Q._targets, E = Q.parent, l = E && E.data === "nested" ? E.vars.targets : a, U = Q._overwrite === "auto" && !Dt, D = Q.timeline, k = A.easeReverse || n, O, u, N, d, M, b, G, C, j, F, H, f, I;
  if (D && (!T || !P) && (P = "none"), Q._ease = JQ(P, aB.ease), Q._rEase = k && (JQ(k) || Q._ease), Q._from = !D && !!A.runBackwards, Q._from && (Q.ratio = 1), !D || T && !A.stagger) {
    if (C = a[0] ? YQ(a[0]).harness : 0, f = C && A[C.prop], O = GB(A, Ot), o && (o._zTime < 0 && o.progress(1), B < 0 && c && i && !z ? o.render(-1, !0) : o.revert(c && w ? LB : ce), o._lazy = 0), e) {
      if (bQ(Q._startAt = K.set(a, aQ({
        data: "isStart",
        overwrite: !1,
        parent: E,
        immediateRender: !0,
        ...
        stagger: 0
      }, e))), Q._startAt._dp = 0, Q._startAt._sat = Q, B < 0 && (q || !i && !z) && Q._startAt.revert(LB), i && w && B <= 0 && t <= 0) {
        B && (Q._zTime = B);
        return;
      }
    } else if (c && w && !o) {
    ...
    for (Q._pt = Q._ptCache = 0, s = w && AQ(s) || s && !w, u = 0; u < a.length; u++) {
      if (M = a[u], G = M._gsap || Mt(a)[u]._gsap, Q._ptLookup[u] = F = {}, vt[G.id] && fQ.length && fB(), H = l === a ? u : l.indexOf(M), C && (j = new C()).init(M, f || O, Q, H, l) !== !1 && (Q._pt = d = new eQ(Q._pt, M, j.name, 0, 1, j.render, j, 0, j.priority), j._props.forEach(function(R) {
        F[R] = d;
      }), j.priority && (b = 1)), !C || f)
        for (N in O)
          cQ[N] && (j = qA(N, O, Q, H, M, l)) ? j.priority && (b = 1) : F[N] = d = jt.call(Q, M, N, "get", O[N], H, l, 0, A.stringFilter);
      Q._op && Q._op[u] && Q.kill(M, Q._op[u]), U && Q._pt && (HQ = Q, Y.killTweensOf(M, F, Q.globalTime(B)), I = !Q.parent, HQ = 0), Q._pt && s && (vt[G.id] = 1);
    }
    b && AP(Q), Q._onInit && Q._onInit(Q);
  }
  Q._onUpdate = v, Q._initted = (!Q._op || Q._pt) && !I, T && B <= 0 && D.render(hQ, !0, !0);
}, me = function(Q, B, t, A, P, e, i, s) {
    ...
    for (v = Q._ptCache[B] = [], T = Q._ptLookup, z = Q._targets.length; z--; ) {
      if (c = T[z][B], c && c.d && c.d._pt)
        for (c = c.d._pt; c && c.p !== B && c.fp !== B; )
          c = c._next;
      if (!c)
        return wt = 1, Q.vars[B] = "+=0", Ft(Q, i), wt = 0, s ? EB(B + " not eligible for reset. Try splitting into individual properties") : 1;
      v.push(c);
    }
  for (z = v.length; z--; )
    n = v[z], c = n._pt || n, c.s = (A || A === 0) && !P ? A : c.s + (A || 0) + e * c.c, c.c = t - c.s, n.e && (n.e = X(t) + _(n.e)), n.b && (n.b = c.s + _(n.b));
}, xe = function(Q, B) {
  var t = Q[0] ? YQ(Q[0]).harness : 0, A = t && t.aliases, P, e, i, s;
  if (!A)
    return B;
  P = tB({}, B);
  for (e in A)
    if (e in P)
      for (s = A[e].split(","), i = s.length; i--; )
        P[s[i]] = P[e];
  return P;
}, ye = function(Q, B, t, A) {
  var P = B.ease || A || "power1.inOut", e, i;
  if ($(B))
    i = t[Q] || (t[Q] = []), B.forEach(function(s, v) {
      return i.push({
        t: v / (B.length - 1) * 100,
        v: s,
        e: P
    ...
    for (e in B)
      i = t[e] || (t[e] = []), e === "ease" || i.push({
        t: parseFloat(Q),
        v: B[e],
        e: P
      });
}, wB = function(Q, B, t, A, P) {
  return J(Q) ? Q.call(B, t, A, P) : Z(Q) && ~Q.indexOf("random(") ? lB(Q) : Q;
}, _A = Lt + "repeat,repeatDelay,yoyo,repeatRefresh,yoyoEase,easeReverse,autoRevert", $A = {};
PQ(_A + ",id,stagger,delay,duration,paused,scrollTrigger", function(r) {
  return $A[r] = 1;
});
var K = /* @__PURE__ */ function(r) {
  ...
  function Q(t, A, P, e) {
    var i;
    typeof A == "number" && (P.duration = A, A = P, P = null), i = r.call(this, e ? A : TB(A)) || this;
    var s = i.vars, v = s.duration, c = s.delay, n = s.immediateRender, T = s.stagger, z = s.overwrite, w = s.keyframes, o = s.defaults, a = s.scrollTrigger, E = A.parent || Y, l = ($(t) || DA(t) ? NQ(t[0]) : "length" in A) ? [t] : lQ(t), U, D, k, O, u, N, d, M;
    if (i._targets = l.length ? Mt(l) : EB("GSAP target " + t + " not found. https://gsap.com", !zQ.nullTargetWarn) || [], i._ptLookup = [], i._overwrite = z, w || T || OB(v) || OB(c)) {
      A = i.vars;
      var b = A.easeReverse || A.yoyoEase;
      if (U = i.timeline = new tQ({
        data: "nested",
        defaults: o || {},
        targets: E && E.data === "nested" ? E.vars.targets : l
      }), U.kill(), U.parent = U._dp = dQ(i), U._start = 0, T || OB(v) || OB(c)) {
        if (O = l.length, d = T && xA(T), uQ(T))
          for (u in T)
            ~_A.indexOf(u) && (M || (M = {}), M[u] = T[u]);
        for (D = 0; D < O; D++)
          k = GB(A, $A), k.stagger = 0, b && (k.easeReverse = b), M && tB(k, M), N = l[D], k.duration = +wB(v, dQ(i), D, N, l), k.delay = (+wB(c, dQ(i), D, N, l) || 0) - i._delay, !T && O === 1 && k.delay && (i._delay = c = k.delay, i._start += c, k.delay = 0), U.to(N, k, d ? d(D, N, l) : 0), U._ease = g.none;
        U.duration() ? v = c = 0 : i.timeline = 0;
      } else if (w) {
        TB(aQ(U.vars.defaults, {
          ease: "none"
        })), U._ease = JQ(w.ease || A.ease || "none");
        var G = 0, C, j, F;
        if ($(w))
          ...
          k = {};
          for (u in w)
            u === "ease" || u === "easeEach" || ye(u, w[u], k, w.easeEach);
          for (u in k)
            for (C = k[u].sort(function(H, f) {
              return H.t - f.t;
            }), G = 0, D = 0; D < C.length; D++)
              j = C[D], F = {
                ease: j.e,
                duration: (j.t - (D ? C[D - 1].t : 0)) / 100 * v
              }, F[u] = j.v, U.to(l, F, G), G += F.duration;
          U.duration() < v && U.to({}, {
      ...
      i.timeline = 0;
    return z === !0 && !Dt && (HQ = dQ(i), Y.killTweensOf(l), HQ = 0), UQ(E, dQ(i), P), A.reversed && i.reverse(), A.paused && i.paused(!0), (n || !v && !w && i._start === S(E._time) && AQ(n) && ae(dQ(i)) && E.data !== "nested") && (i._tTime = -m, i.render(Math.max(0, -c) || 0)), a && bA(dQ(i), a), i;
  }
  var B = Q.prototype;
  return B.render = function(A, P, e) {
    var i = this._time, s = this._tDur, v = this._dur, c = A < 0, n = A > s - m && !c ? s : A < m ? 0 : A, T, z, w, o, a, E, l, U;
    if (!v)
      he(this, A, P, e);
    else if (n !== this._tTime || !A || e || !this._initted && this._tTime || this._startAt && this._zTime < 0 !== c || this._lazy) {
      if (T = n, U = this.timeline, this._repeat) {
        if (o = v + this._rDelay, this._repeat < -1 && c)
          return this.totalTime(o * 100 + A, P, e);
        if (T = S(n % o), n === s ? (w = this._repeat, T = v) : (a = S(n / o), w = ~~a, w && w === a ? (T = v, w--) : T > v && (T = v)), E = this._yoyo && w & 1, E && (T = v - T), a = AB(this._tTime, o), T === i && !e && this._initted && w === a)
          return this._tTime = n, this;
        w !== a && this.vars.repeatRefresh && !E && !this._lock && T !== o && this._initted && (this._lock = e = 1, this.render(S(o * w), !0).invalidate()._lock = 0);
      }
      if (!this._initted) {
        if (gA(this, c ? A : T, e, P, n))
          return this._tTime = 0, this;
        if (i !== this._time && !(e && this.vars.repeatRefresh && w !== a))
          return this;
        if (v !== this._dur)
          return this.render(A, P, e);
      }
      if (this._rEase) {
        ...
        if (D !== this._inv) {
          var k = D ? i : v - i;
          this._inv = D, this._from && (this.ratio = 1 - this.ratio), this._invRatio = this.ratio, this._invTime = i, this._invRecip = k ? (D ? -1 : 1) / k : 0, this._invScale = D ? -this.ratio : 1 - this.ratio, this._invEase = D ? this._rEase : this._ease;
        }
        this.ratio = l = this._invRatio + this._invScale * this._invEase((T - this._invTime) * this._invRecip);
      } else
        this.ratio = l = this._ease(T / v);
      if (this._from && (this.ratio = l = 1 - l), this._tTime = n, this._time = T, !this._act && this._ts && (this._act = 1, this._lazy = 0), !i && n && !P && !a && (oQ(this, "onStart"), this._tTime !== n))
        return this;
      for (z = this._pt; z; )
        z.r(l, z.d), z = z._next;
      U && U.render(A < 0 ? A : U._dur * U._ease(T / this._dur), P, e) || this._startAt && (this._zTime = A), this._onUpdate && !P && (c && ct(this, A, P, e), oQ(this, "onUpdate")), this._repeat && w !== a && this.vars.onRepeat && !P && this.parent && oQ(this, "onRepeat"), (n === this._tDur || !n) && this._tTime === n && (c && !this._onUpdate && ct(this, A, !0, !0), (A || !v) && (n === this._tDur && this._ts > 0 || !n && this._ts < 0) && bQ(this, 1), !P && !(c && !i) && (n || i || E) && (oQ(this, n === s ? "onComplete" : "onReverseComplete", !0), this._prom && !(n < s && this.timeScale() > 0) && this._prom()));
    }
    return this;
  }, B.targets = function() {
  ...
  }, B.resetTo = function(A, P, e, i, s) {
    kB || nQ.wake(), this._ts || this.play();
    var v = Math.min(this._dur, (this._dp._time - this._start) * this._ts), c;
    return this._initted || Ft(this, v), c = this._ease(v / this._dur), me(this, A, P, e, i, c, v, s) ? this.resetTo(A, P, e, i, 1) : (IB(this, 0), this.parent || fA(this._dp, this, "_first", "_last", this._dp._sort ? "_start" : 0), this.render(0));
  }, B.kill = function(A, P) {
    if (P === void 0 && (P = "all"), !A && (!P || P === "all"))
      return this._lazy = this._pt = 0, this.parent ? nB(this) : this.scrollTrigger && this.scrollTrigger.kill(!!q), this;
    if (this.timeline) {
      var e = this.timeline.totalDuration();
      return this.timeline.killTweensOf(A, P, HQ && HQ.vars.overwrite !== !0)._first || nB(this), this.parent && e !== this.timeline.totalDuration() && PB(this, this._dur * this.timeline._tDur / e, 0, 1), this;
    }
    var i = this._targets, s = A ? lQ(A) : i, v = this._ptLookup, c = this._pt, n, T, z, w, o, a, E;
    if ((!P || P === "all") && ze(i, s))
      return P === "all" && (this._pt = 0), nB(this);
    for (n = this._op = this._op || [], P !== "all" && (Z(P) && (o = {}, PQ(P, function(l) {
      return o[l] = 1;
    }), P = o), P = xe(i, P)), E = i.length; E--; )
      if (~s.indexOf(i[E])) {
        T = v[E], P === "all" ? (n[E] = P, w = T, z = {}) : (z = n[E] = n[E] || {}, w = P);
        for (o in w)
          a = T && T[o], a && ((!("kill" in a.d) || a.d.kill(o) === !0) && xB(this, a, "_pt"), delete T[o]), z !== "all" && (z[o] = 1);
      }
    return this._initted && !this._pt && c && nB(this), this;
  }, Q.to = function(A, P) {
    return new Q(A, P, arguments[2]);
  }, Q.from = function(A, P) {
    return zB(1, arguments);
  }, Q.delayedCall = function(A, P, e, i) {
    return new Q(P, 0, {
      immediateRender: !1,
      lazy: !1,
      overwrite: !1,
  ...
  }, Q.fromTo = function(A, P, e) {
    return zB(2, arguments);
  }, Q.set = function(A, P) {
    return P.duration = 0, P.repeatDelay || (P.repeat = 0), new Q(A, P);
  }, Q.killTweensOf = function(A, P, e) {
    return Y.killTweensOf(A, P, e);
  }, Q;
}(UB);
aQ(K.prototype, {
  _targets: [],
...
PQ("staggerTo,staggerFrom,staggerFromTo", function(r) {
  K[r] = function() {
    var Q = new tQ(), B = ot.call(arguments, 0);
    return B.splice(r === "staggerFromTo" ? 5 : 4, 0, 0), Q[r].apply(Q, B);
  };
});
var ft = function(Q, B, t) {
  return Q[B] = t;
}, QP = function(Q, B, t) {
  return Q[B](t);
}, Ie = function(Q, B, t, A) {
  ...
  return Q.setAttribute(B, t);
}, Gt = function(Q, B) {
  return J(Q[B]) ? QP : ut(Q[B]) && Q.setAttribute ? Se : ft;
}, BP = function(Q, B) {
  ...
  if (!Q && B.b)
    ...
    for (; t; )
      A = t.p + (t.m ? t.m(t.s + t.c * Q) : Math.round((t.s + t.c * Q) * 1e4) / 1e4) + A, t = t._next;
    A += B.c;
  }
  B.set(B.t, B.p, A, B);
}, bt = function(Q, B) {
  ...
  for (var P = this._pt, e; P; )
    e = P._next, P.p === A && P.modifier(Q, B, t), P = e;
}, Je = function(Q) {
  for (var B = this._pt, t, A; B; )
    A = B._next, B.p === Q && !B.op || B.op === Q ? xB(this, B, "_pt") : B.dep || (t = 1), B = A;
  return !t;
}, Re = function(Q, B, t, A) {
  A.mSet(Q, B, A.m.call(A.tween, t, A.mt), A);
}, AP = function(Q) {
  for (var B = Q._pt, t, A, P, e; B; ) {
    for (t = B._next, A = P; A && A.pr > B.pr; )
      A = A._next;
    (B._prev = A ? A._prev : e) ? B._prev._next = B : P = B, (B._next = A) ? A._prev = B : e = B, B = t;
  }
  Q._pt = P;
}, eQ = /* @__PURE__ */ function() {
  function r(B, t, A, P, e, i, s, v, c) {
    this.t = t, this.s = P, this.c = e, this.p = A, this.r = i || BP, this.d = s || this, this.set = v || ft, this.pr = c || 0, this._next = B, B && (B._prev = this);
  }
  var Q = r.prototype;
  return Q.modifier = function(t, A, P) {
    this.mSet = this.mSet || this.set, this.set = Re, this.m = t, this.mt = P, this.tween = A;
  }, r;
}();
PQ(Lt + "parent,duration,ease,delay,overwrite,runBackwards,startAt,yoyo,immediateRender,repeat,repeatDelay,data,paused,reversed,lazy,callbackScope,stringFilter,id,yoyoEase,stagger,inherit,repeatRefresh,keyframes,autoRevert,scrollTrigger,easeReverse", function(r) {
  return Ot[r] = 1;
});
wQ.TweenMax = wQ.TweenLite = K;
wQ.TimelineLite = wQ.TimelineMax = tQ;
Y = new tQ({
  sortChildren: !1,
  defaults: aB,
...
zQ.stringFilter = KA;
var RQ = [], HB = {}, Xe = [], BA = 0, Ke = 0, WB = function(Q) {
  return (HB[Q] || Xe).map(function(B) {
    return B();
  });
}, at = function() {
  var Q = Date.now(), B = [];
  Q - BA > 2 && (WB("matchMediaInit"), RQ.forEach(function(t) {
    var A = t.queries, P = t.conditions, e, i, s, v;
    for (i in A)
      e = kQ.matchMedia(A[i]).matches, e && (s = 1), e !== P[i] && (P[i] = e, v = 1);
    v && (t.revert(), s && B.push(t));
  }), WB("matchMediaRevert"), B.forEach(function(t) {
    return t.onMatch(t, function(A) {
      return t.add(null, A);
    });
  }), BA = Q, WB("matchMedia"));
}, PP = /* @__PURE__ */ function() {
  function r(B, t) {
    this.selector = t && Tt(t), this.data = [], this._r = [], this.isReverted = !1, this.id = Ke++, B && this.add(B);
  }
  var Q = r.prototype;
  return Q.add = function(t, A, P) {
    J(t) && (P = A, A = t, t = J);
    var e = this, i = function() {
      var v = y, c = e.selector, n;
      return v && v !== e && v.data.push(e), P && (e.selector = Tt(P)), y = e, n = A.apply(e, arguments), J(n) && e._r.push(n), y = v, e.selector = c, e.isReverted = !1, n;
    };
    return e.last = i, t === J ? i(e, function(s) {
      return e.add(null, s);
    }) : t ? e[t] = i : i;
  }, Q.ignore = function(t) {
    var A = y;
    ...
    return this.data.forEach(function(A) {
      return A instanceof r ? t.push.apply(t, A.getTweens()) : A instanceof K && !(A.parent && A.parent.data === "nested") && t.push(A);
    }), t;
  }, Q.clear = function() {
    ...
    if (t ? function() {
      ...
      for (i.map(function(c) {
        ...
        v = P.data[s], v instanceof tQ ? v.data !== "nested" && (v.scrollTrigger && v.scrollTrigger.revert(), v.kill()) : !(v instanceof K) && v.revert && v.revert(t);
      ...
      for (var e = RQ.length; e--; )
        RQ[e].id === this.id && RQ.splice(e, 1);
  }, Q.revert = function(t) {
    this.kill(t || {});
  }, r;
}(), Ze = /* @__PURE__ */ function() {
  function r(B) {
    this.contexts = [], this.scope = B, y && y.data.push(this);
  }
  var Q = r.prototype;
  return Q.add = function(t, A, P) {
    uQ(t) || (t = {
    ...
    var e = new PP(0, P || this.scope), i = e.conditions = {}, s, v, c;
    y && !e.selector && (e.selector = y.selector), this.contexts.push(e), A = e.add("onMatch", A), e.queries = t;
    for (v in t)
      v === "all" ? c = 1 : (s = kQ.matchMedia(t[v]), s && (RQ.indexOf(e) < 0 && RQ.push(e), (i[v] = s.matches) && (c = 1), s.addListener ? s.addListener(at) : s.addEventListener("change", at)));
  ...
  getTweensOf: function(Q, B) {
    return Y.getTweensOf(Q, B);
  },
  getProperty: function(Q, B, t, A) {
    Z(Q) && (Q = lQ(Q)[0]);
    var P = YQ(Q || {}).get, e = t ? FA : jA;
    return t === "native" && (t = ""), Q && (B ? e((cQ[B] && cQ[B].get || P)(Q, B, t, A)) : function(i, s, v) {
      return e((cQ[i] && cQ[i].get || P)(Q, i, s, v));
    });
  },
  quickSetter: function(Q, B, t) {
    if (Q = lQ(Q), Q.length > 1) {
      var A = Q.map(function(c) {
        return rQ.quickSetter(c, B, t);
      }), P = A.length;
      return function(c) {
        for (var n = P; n--; )
          A[n](c);
    ...
    Q = Q[0] || {};
    var e = cQ[B], i = YQ(Q), s = i.harness && (i.harness.aliases || {})[B] || B, v = e ? function(c) {
      var n = new e();
      _Q._pt = 0, n.init(Q, t ? c + t : c, _Q, 0, [Q]), n.render(1, n), _Q._pt && bt(1, _Q);
    } : i.set(Q, s);
    return e ? v : function(c) {
      return v(Q, s, t ? c + t : c, i, 1);
    };
  },
  quickTo: function(Q, B, t) {
    var A, P = rQ.to(Q, aQ((A = {}, A[B] = "+=0.1", A.paused = !0, A.stagger = 0, A), t || {})), e = function(s, v, c) {
  ...
  defaults: function(Q) {
    return Q && Q.ease && (Q.ease = JQ(Q.ease, aB.ease)), Wt(aB, Q || {});
  },
  config: function(Q) {
    return Wt(zQ, Q || {});
  },
  registerEffect: function(Q) {
    var B = Q.name, t = Q.effect, A = Q.plugins, P = Q.defaults, e = Q.extendTimeline;
    (A || "").split(",").forEach(function(i) {
      return i && !cQ[i] && !wQ[i] && EB(B + " effect requires " + i + " plugin.");
    }), RB[B] = function(i, s, v) {
      return t(lQ(i), aQ(s || {}, P), v);
    }, e && (tQ.prototype[B] = function(i, s, v) {
      return this.add(RB[B](i, uQ(s) ? s : (v = s) && {}, this), v);
    });
  },
  ...
  parseEase: function(Q, B) {
    return arguments.length ? JQ(Q, B) : g;
  },
  getById: function(Q) {
    return Y.getById(Q);
  },
  exportRoot: function(Q, B) {
    Q === void 0 && (Q = {});
    var t = new tQ(Q), A, P;
    for (t.smoothChildTiming = AQ(Q.smoothChildTiming), Y.remove(t), t._dp = 0, t._time = t._tTime = Y._time, A = Y._first; A; )
      P = A._next, (B || !(!A._dur && A instanceof K && A.vars.onComplete === A._targets[0])) && UQ(t, A, A._start - A._delay), A = P;
    return UQ(Y, t, 0), t;
  },
  context: function(Q, B) {
    return Q ? new PP(Q, B) : y;
  },
  ...
  addEventListener: function(Q, B) {
    var t = HB[Q] || (HB[Q] = []);
    ~t.indexOf(B) || t.push(B);
  },
  removeEventListener: function(Q, B) {
    var t = HB[Q], A = t && t.indexOf(B);
    A >= 0 && t.splice(A, 1);
  },
  utils: {
    wrap: Ne,
...
PQ("to,from,fromTo,delayedCall,set,killTweensOf", function(r) {
  return gB[r] = K[r];
});
nQ.add(tQ.updateRoot);
_Q = gB.to({}, {
  duration: 0
});
var We = function(Q, B) {
  for (var t = Q._pt; t && t.p !== B && t.op !== B && t.fp !== B; )
    t = t._next;
  return t;
}, qe = function(Q, B) {
  var t = Q._targets, A, P, e;
  for (A in B)
    for (P = t.length; P--; )
      e = Q._ptLookup[P][A], e && (e = e.d) && (e._pt && (e = We(e, A)), e && e.modifier && e.modifier(B[A], Q, t[P], A));
}, qB = function(Q, B) {
  return {
    name: Q,
    headless: 1,
...
}, rQ = gB.registerPlugin({
  name: "attr",
  init: function(Q, B, t, A, P) {
    var e, i, s;
    this.tween = t;
    for (e in B)
      s = Q.getAttribute(e) || "", i = this.add(Q, "setAttribute", (s || 0) + "", B[e], A, P, 0, 0, e), i.op = e, i.b = s, this._props.push(e);
  },
  render: function(Q, B) {
    for (var t = B._pt; t; )
      q ? t.set(t.t, t.p, t.b, t) : t.r(Q, t.d), t = t._next;
  }
}, {
  name: "endArray",
  headless: 1,
  init: function(Q, B) {
    for (var t = B.length; t--; )
      this.add(Q, t, Q[t] || 0, B[t], 0, 0, 0, 0, 0, 1);
  }
}, qB("roundProps", zt), qB("modifiers"), qB("snap", yA)) || gB;
K.version = tQ.version = rQ.version = "3.15.0";
CA = 1;
dt() && eB();
g.Power0;
g.Power1;
 ...
 * @author: Jack Doyle, jack@greensock.com
*/
var tA, jQ, QB, gt, SQ, AA, pt, _e = function() {
  return typeof window < "u";
}, OQ = {}, IQ = 180 / Math.PI, BB = Math.PI / 180, KQ = Math.atan2, PA = 1e8, mt = /([A-Z])/g, $e = /(left|right|width|margin|padding|x)/i, Qi = /[\s,\(]\S/, DQ = {
  ...
  var t = B.s + B.c * Q;
  B.set(B.t, B.p, ~~(t + (t < 0 ? -0.5 : 0.5)) + B.u, B);
}, eP = function(Q, B) {
  ...
  return Q.style.setProperty(B, t);
}, ri = function(Q, B, t) {
  return Q._gsap[B] = t;
}, si = function(Q, B, t) {
  return Q._gsap.scaleX = Q._gsap.scaleY = t;
}, vi = function(Q, B, t, A, P) {
  var e = Q._gsap;
  e.scaleX = e.scaleY = t, e.renderTransform(P, e);
}, ci = function(Q, B, t, A, P) {
  var e = Q._gsap;
  e[B] = t, e.renderTransform(P, e);
}, V = "transform", iQ = V + "Origin", ni = function r(Q, B) {
  var t = this, A = this.target, P = A.style, e = A._gsap;
  if (Q in OQ && P) {
    if (this.tfm = this.tfm || {}, Q !== "transform")
      Q = DQ[Q] || Q, ~Q.indexOf(",") ? Q.split(",").forEach(function(i) {
        return t.tfm[i] = CQ(A, i);
      }) : this.tfm[Q] = e.x ? e[Q] : CQ(A, Q), Q === iQ && (this.tfm.zOrigin = e.zOrigin);
    else
      return DQ.transform.split(",").forEach(function(i) {
        return r.call(t, i, B);
      });
    if (this.props.indexOf(V) >= 0)
      return;
    e.svg && (this.svgo = A.getAttribute("data-svg-origin"), this.props.push(iQ, B, "")), Q = V;
  }
  (P || B) && this.props.push(Q, B, P[Q]);
}, rP = function(Q) {
  Q.translate && (Q.removeProperty("translate"), Q.removeProperty("scale"), Q.removeProperty("rotate"));
}, oi = function() {
  var Q = this.props, B = this.target, t = B.style, A = B._gsap, P, e;
  for (P = 0; P < Q.length; P += 3)
    Q[P + 1] ? Q[P + 1] === 2 ? B[Q[P]](Q[P + 2]) : B[Q[P]] = Q[P + 2] : Q[P + 2] ? t[Q[P]] = Q[P + 2] : t.removeProperty(Q[P].substr(0, 2) === "--" ? Q[P] : Q[P].replace(mt, "-$1").toLowerCase());
  if (this.tfm) {
    for (e in this.tfm)
      A[e] = this.tfm[e];
    A.svg && (A.renderTransform(), B.setAttribute("data-svg-origin", this.svgo || "")), P = pt(), (!P || !P.isStart) && !t[V] && (rP(t), A.zOrigin && t[iQ] && (t[iQ] += " " + A.zOrigin + "px", A.zOrigin = 0, A.renderTransform()), A.uncache = 1);
  }
}, sP = function(Q, B) {
  var t = {
    target: Q,
  ...
  return Q._gsap || rQ.core.getCache(Q), B && Q.style && Q.nodeType && B.split(",").forEach(function(A) {
    return t.save(A);
  }), t;
}, vP, ht = function(Q, B) {
  var t = jQ.createElementNS ? jQ.createElementNS((B || "http://www.w3.org/1999/xhtml").replace(/^https/, "http"), Q) : jQ.createElement(Q);
  return t && t.style ? t : jQ.createElement(Q);
}, TQ = function r(Q, B, t) {
  var A = getComputedStyle(Q);
  return A[B] || A.getPropertyValue(B.replace(mt, "-$1").toLowerCase()) || A.getPropertyValue(B) || !t && r(Q, iB(B) || B, 1) || "";
}, eA = "O,Moz,ms,Ms,Webkit".split(","), iB = function(Q, B, t) {
  var A = B || SQ, P = A.style, e = 5;
  if (Q in P && !t)
    return Q;
  for (Q = Q.charAt(0).toUpperCase() + Q.substr(1); e-- && !(eA[e] + Q in P); )
    ;
  return e < 0 ? null : (e === 3 ? "ms" : e >= 0 ? eA[e] : "") + Q;
}, lt = function() {
  _e() && window.document && (tA = window, jQ = tA.document, QB = jQ.documentElement, SQ = ht("div") || {
    style: {}
  }, ht("div"), V = iB(V), iQ = V + "Origin", SQ.style.cssText = "border-width:0;line-height:0;position:absolute;padding:0", vP = !!iB("perspective"), pt = rQ.core.reverting, gt = 1);
}, iA = function(Q) {
  var B = Q.ownerSVGElement, t = ht("svg", B && B.getAttribute("xmlns") || "http://www.w3.org/2000/svg"), A = Q.cloneNode(!0), P;
  A.style.display = "block", t.appendChild(A), QB.appendChild(t);
  try {
    P = A.getBBox();
  } catch {
  }
  return t.removeChild(A), QB.removeChild(t), P;
}, rA = function(Q, B) {
  for (var t = B.length; t--; )
    if (Q.hasAttribute(B[t]))
      return Q.getAttribute(B[t]);
}, cP = function(Q) {
  var B, t;
  try {
    B = Q.getBBox();
  ...
  return B && (B.width || B.height) || t || (B = iA(Q)), B && !B.width && !B.x && !B.y ? {
    x: +rA(Q, ["x", "cx", "x1"]) || 0,
    y: +rA(Q, ["y", "cy", "y1"]) || 0,
    width: 0,
    height: 0
  ...
  return !!(Q.getCTM && (!Q.parentNode || Q.ownerSVGElement) && cP(Q));
}, gQ = function(Q, B) {
  if (B) {
    var t = Q.style, A;
    B in OQ && B !== iQ && (B = V), t.removeProperty ? (A = B.substr(0, 2), (A === "ms" || B.substr(0, 6) === "webkit") && (B = "-" + B), t.removeProperty(A === "--" ? B : B.replace(mt, "-$1").toLowerCase())) : t.removeAttribute(B);
  }
}, FQ = function(Q, B, t, A, P, e) {
  var i = new eQ(Q._pt, B, t, 0, 1, e ? iP : eP);
  return Q._pt = i, i.b = A, i.e = P, Q._props.push(t), i;
}, sA = {
  deg: 1,
  rad: 1,
  turn: 1
  ...
  grid: 1,
  flex: 1
}, pQ = function r(Q, B, t, A) {
  var P = parseFloat(t) || 0, e = (t + "").trim().substr((P + "").length) || "px", i = SQ.style, s = $e.test(B), v = Q.tagName.toLowerCase() === "svg", c = (v ? "client" : "offset") + (s ? "Width" : "Height"), n = 100, T = A === "px", z = A === "%", w, o, a, E;
  if (A === e || !P || sA[A] || sA[e])
    return P;
  if (e !== "px" && !T && (P = r(Q, B, t, "px")), E = Q.getCTM && nP(Q), (z || e === "%") && (OQ[B] || ~B.indexOf("adius")))
    return w = E ? Q.getBBox()[s ? "width" : "height"] : Q[c], X(z ? P / w * n : P / 100 * w);
  if (i[s ? "width" : "height"] = n + (T ? e : A), o = A !== "rem" && ~B.indexOf("adius") || A === "em" && Q.appendChild && !v ? Q : Q.parentNode, E && (o = (Q.ownerSVGElement || {}).parentNode), (!o || o === jQ || !o.appendChild) && (o = jQ.body), a = o._gsap, a && z && a.width && s && a.time === nQ.time && !a.uncache)
    return X(P / a.width * n);
  if (z && (B === "height" || B === "width")) {
    var l = Q.style[B];
    Q.style[B] = n + A, w = Q[c], l ? Q.style[B] = l : gQ(Q, B);
  } else
    (z || e === "%") && !Ti[TQ(o, "display")] && (i.position = TQ(Q, "position")), o === Q && (i.position = "static"), o.appendChild(SQ), w = SQ[c], o.removeChild(SQ), i.position = "absolute";
  return s && z && (a = YQ(o), a.time = nQ.time, a.width = o[c]), X(T ? w * P / n : w && P ? n / w * P : 0);
}, CQ = function(Q, B, t, A) {
  var P;
  return gt || lt(), B in DQ && B !== "transform" && (B = DQ[B], ~B.indexOf(",") && (B = B.split(",")[0])), OQ[B] && B !== "transform" ? (P = uB(Q, A), P = B !== "transformOrigin" ? P[B] : P.svg ? P.origin : mB(TQ(Q, iQ)) + " " + P.zOrigin + "px") : (P = Q.style[B], (!P || P === "auto" || A || ~(P + "").indexOf("calc(")) && (P = pB[B] && pB[B](Q, B, t) || TQ(Q, B) || MA(Q, B) || (B === "opacity" ? 1 : 0))), t && !~(P + "").trim().indexOf(" ") ? pQ(Q, B, P, t) + t : P;
}, zi = function(Q, B, t, A) {
  if (!t || t === "none") {
    var P = iB(B, Q, 1), e = P && TQ(Q, P, 1);
    e && e !== t ? (B = P, t = e) : B === "borderColor" && (t = TQ(Q, "borderTopColor"));
  }
  var i = new eQ(this._pt, Q.style, B, 0, 1, tP), s = 0, v = 0, c, n, T, z, w, o, a, E, l, U, D, k;
  if (i.b = t, i.e = A, t += "", A += "", A.substring(0, 6) === "var(--" && (A = TQ(Q, A.substring(4, A.indexOf(")")))), A === "auto" && (o = Q.style[B], Q.style[B] = A, A = TQ(Q, B) || A, o ? Q.style[B] = o : gQ(Q, B)), c = [t, A], KA(c), t = c[0], A = c[1], T = t.match(qQ) || [], k = A.match(qQ) || [], k.length) {
    for (; n = qQ.exec(A); )
      a = n[0], l = A.substring(s, n.index), w ? w = (w + 1) % 5 : (l.substr(-5) === "rgba(" || l.substr(-5) === "hsla(") && (w = 1), a !== (o = T[v++] || "") && (z = parseFloat(o) || 0, D = o.substr((z + "").length), a.charAt(1) === "=" && (a = $Q(z, a) + D), E = parseFloat(a), U = a.substr((E + "").length), s = qQ.lastIndex - U.length, U || (U = U || zQ.units[B] || D, s === A.length && (A += U, i.e += U)), D !== U && (z = pQ(Q, B, o, U) || 0), i._pt = {
        _next: i._pt,
        p: l || v === 1 ? l : ",",
        //note: SVG spec allows omission of comma/space when a negative sign is wedged between two numbers, like 2.5-5.3 instead of 2.5,-5.3 but when tweening, the negative value may switch to positive, so we insert the comma just in case.
        s: z,
    ...
    i.r = B === "display" && A === "none" ? iP : eP;
  return dA.test(A) && (i.e = 0), this._pt = i, i;
}, vA = {
  top: "0%",
  bottom: "100%",
  left: "0%",
  ...
  var B = Q.split(" "), t = B[0], A = B[1] || "50%";
  return (t === "top" || t === "bottom" || A === "left" || A === "right") && (Q = t, t = A, A = Q), B[0] = vA[t] || t, B[1] = vA[A] || A, B.join(" ");
}, ai = function(Q, B) {
  if (B.tween && B.tween._time === B.tween._dur) {
    var t = B.t, A = t.style, P = B.u, e = t._gsap, i, s, v;
    if (P === "all" || P === !0)
      A.cssText = "", s = 1;
    else
      for (P = P.split(","), v = P.length; --v > -1; )
        i = P[v], OQ[i] && (s = 1, i = i === "transformOrigin" ? iQ : V), gQ(t, i);
    s && (gQ(t, V), e && (e.svg && t.removeAttribute("transform"), A.scale = A.rotate = A.translate = "none", uB(t, 1), e.uncache = 1, rP(A)));
  }
}, pB = {
  clearProps: function(Q, B, t, A, P) {
    if (P.data !== "isFromStart") {
      var e = Q._pt = new eQ(Q._pt, B, t, 0, 0, ai);
      return e.u = A, e.pr = -10, e.tween = P, Q._props.push(t), 1;
    }
  }
  ...
  */
}, DB = [1, 0, 0, 1, 0, 0], oP = {}, TP = function(Q) {
  return Q === "matrix(1, 0, 0, 1, 0, 0)" || Q === "none" || !Q;
}, cA = function(Q) {
  var B = TQ(Q, V);
  return TP(B) ? DB : B.substr(7).match(uA).map(X);
}, xt = function(Q, B) {
  var t = Q._gsap || YQ(Q), A = Q.style, P = cA(Q), e, i, s, v;
  return t.svg && Q.getAttribute("transform") ? (s = Q.transform.baseVal.consolidate().matrix, P = [s.a, s.b, s.c, s.d, s.e, s.f], P.join(",") === "1,0,0,1,0,0" ? DB : P) : (P === DB && !Q.offsetParent && Q !== QB && !t.svg && (s = A.display, A.display = "block", e = Q.parentNode, (!e || !Q.offsetParent && !Q.getBoundingClientRect().width) && (v = 1, i = Q.nextElementSibling, QB.appendChild(Q)), P = cA(Q), s ? A.display = s : gQ(Q, "display"), v && (i ? e.insertBefore(Q, i) : e ? e.appendChild(Q) : QB.removeChild(Q))), B && P.length > 6 ? [P[0], P[1], P[4], P[5], P[12], P[13]] : P);
}, kt = function(Q, B, t, A, P, e) {
  var i = Q._gsap, s = P || xt(Q, !0), v = i.xOrigin || 0, c = i.yOrigin || 0, n = i.xOffset || 0, T = i.yOffset || 0, z = s[0], w = s[1], o = s[2], a = s[3], E = s[4], l = s[5], U = B.split(" "), D = parseFloat(U[0]) || 0, k = parseFloat(U[1]) || 0, O, u, N, d;
  t ? s !== DB && (u = z * a - w * o) && (N = D * (a / u) + k * (-o / u) + (o * l - a * E) / u, d = D * (-w / u) + k * (z / u) - (z * l - w * E) / u, D = N, k = d) : (O = cP(Q), D = O.x + (~U[0].indexOf("%") ? D / 100 * O.width : D), k = O.y + (~(U[1] || U[0]).indexOf("%") ? k / 100 * O.height : k)), A || A !== !1 && i.smooth ? (E = D - v, l = k - c, i.xOffset = n + (E * z + l * o) - E, i.yOffset = T + (E * w + l * a) - l) : i.xOffset = i.yOffset = 0, i.xOrigin = D, i.yOrigin = k, i.smooth = !!A, i.origin = B, i.originIsAbsolute = !!t, Q.style[iQ] = "0px 0px", e && (FQ(e, i, "xOrigin", v, D), FQ(e, i, "yOrigin", c, k), FQ(e, i, "xOffset", n, i.xOffset), FQ(e, i, "yOffset", T, i.yOffset)), Q.setAttribute("data-svg-origin", D + " " + k);
}, uB = function(Q, B) {
  var t = Q._gsap || new WA(Q);
  if ("x" in t && !B && !t.uncache)
    return t;
  var A = Q.style, P = t.scaleX < 0, e = "px", i = "deg", s = getComputedStyle(Q), v = TQ(Q, iQ) || "0", c, n, T, z, w, o, a, E, l, U, D, k, O, u, N, d, M, b, G, C, j, F, H, f, I, R, QQ, sQ, W, x, BQ, vQ;
  return c = n = T = o = a = E = l = U = D = 0, z = w = 1, t.svg = !!(Q.getCTM && nP(Q)), s.translate && ((s.translate !== "none" || s.scale !== "none" || s.rotate !== "none") && (A[V] = (s.translate !== "none" ? "translate3d(" + (s.translate + " 0 0").split(" ").slice(0, 3).join(", ") + ") " : "") + (s.rotate !== "none" ? "rotate(" + s.rotate + ") " : "") + (s.scale !== "none" ? "scale(" + s.scale.split(" ").join(",") + ") " : "") + (s[V] !== "none" ? s[V] : "")), A.scale = A.rotate = A.translate = "none"), u = xt(Q, t.svg), t.svg && (t.uncache ? (I = Q.getBBox(), v = t.xOrigin - I.x + "px " + (t.yOrigin - I.y) + "px", f = "") : f = !B && Q.getAttribute("data-svg-origin"), kt(Q, f || v, !!f || t.originIsAbsolute, t.smooth !== !1, u)), k = t.xOrigin || 0, O = t.yOrigin || 0, u !== DB && (b = u[0], G = u[1], C = u[2], j = u[3], c = F = u[4], n = H = u[5], u.length === 6 ? (z = Math.sqrt(b * b + G * G), w = Math.sqrt(j * j + C * C), o = b || G ? KQ(G, b) * IQ : 0, l = C || j ? KQ(C, j) * IQ + o : 0, l && (w *= Math.abs(Math.cos(l * BB))), t.svg && (c -= k - (k * b + O * C), n -= O - (k * G + O * j))) : (vQ = u[6], x = u[7], QQ = u[8], sQ = u[9], W = u[10], BQ = u[11], c = u[12], n = u[13], T = u[14], N = KQ(vQ, W), a = N * IQ, N && (d = Math.cos(-N), M = Math.sin(-N), f = F * d + QQ * M, I = H * d + sQ * M, R = vQ * d + W * M, QQ = F * -M + QQ * d, sQ = H * -M + sQ * d, W = vQ * -M + W * d, BQ = x * -M + BQ * d, F = f, H = I, vQ = R), N = KQ(-C, W), E = N * IQ, N && (d = Math.cos(-N), M = Math.sin(-N), f = b * d - QQ * M, I = G * d - sQ * M, R = C * d - W * M, BQ = j * M + BQ * d, b = f, G = I, C = R), N = KQ(G, b), o = N * IQ, N && (d = Math.cos(N), M = Math.sin(N), f = b * d + G * M, I = F * d + H * M, G = G * d - b * M, H = H * d - F * M, b = f, F = I), a && Math.abs(a) + Math.abs(o) > 359.9 && (a = o = 0, E = 180 - E), z = X(Math.sqrt(b * b + G * G + C * C)), w = X(Math.sqrt(H * H + vQ * vQ)), N = KQ(F, H), l = Math.abs(N) > 2e-4 ? N * IQ : 0, D = BQ ? 1 / (BQ < 0 ? -BQ : BQ) : 0), t.svg && (f = Q.getAttribute("transform"), t.forceCSS = Q.setAttribute("transform", "") || !TP(TQ(Q, V)), f && Q.setAttribute("transform", f))), Math.abs(l) > 90 && Math.abs(l) < 270 && (P ? (z *= -1, l += o <= 0 ? 180 : -180, o += o <= 0 ? 180 : -180) : (w *= -1, l += l <= 0 ? 180 : -180)), B = B || t.uncache, t.x = c - ((t.xPercent = c && (!B && t.xPercent || (Math.round(Q.offsetWidth / 2) === Math.round(-c) ? -50 : 0))) ? Q.offsetWidth * t.xPercent / 100 : 0) + e, t.y = n - ((t.yPercent = n && (!B && t.yPercent || (Math.round(Q.offsetHeight / 2) === Math.round(-n) ? -50 : 0))) ? Q.offsetHeight * t.yPercent / 100 : 0) + e, t.z = T + e, t.scaleX = X(z), t.scaleY = X(w), t.rotation = X(o) + i, t.rotationX = X(a) + i, t.rotationY = X(E) + i, t.skewX = l + i, t.skewY = U + i, t.transformPerspective = D + e, (t.zOrigin = parseFloat(v.split(" ")[2]) || !B && t.zOrigin || 0) && (A[iQ] = mB(v)), t.xOffset = t.yOffset = 0, t.force3D = zQ.force3D, t.renderTransform = t.svg ? hi : vP ? zP : Ei, t.uncache = 0, t;
}, mB = function(Q) {
  ...
  B.z = "0px", B.rotationY = B.rotationX = "0deg", B.force3D = 0, zP(Q, B);
}, xQ = "0deg", sB = "0px", yQ = ") ", zP = function(Q, B) {
  var t = B || this, A = t.xPercent, P = t.yPercent, e = t.x, i = t.y, s = t.z, v = t.rotation, c = t.rotationY, n = t.rotationX, T = t.skewX, z = t.skewY, w = t.scaleX, o = t.scaleY, a = t.transformPerspective, E = t.force3D, l = t.target, U = t.zOrigin, D = "", k = E === "auto" && Q && Q !== 1 || E === !0;
  if (U && (n !== xQ || c !== xQ)) {
    var O = parseFloat(c) * BB, u = Math.sin(O), N = Math.cos(O), d;
    O = parseFloat(n) * BB, d = Math.cos(O), e = _B(l, e, u * d * -U), i = _B(l, i, -Math.sin(O) * -U), s = _B(l, s, N * d * -U + U);
  }
  a !== sB && (D += "perspective(" + a + yQ), (A || P) && (D += "translate(" + A + "%, " + P + "%) "), (k || e !== sB || i !== sB || s !== sB) && (D += s !== sB || k ? "translate3d(" + e + ", " + i + ", " + s + ") " : "translate(" + e + ", " + i + yQ), v !== xQ && (D += "rotate(" + v + yQ), c !== xQ && (D += "rotateY(" + c + yQ), n !== xQ && (D += "rotateX(" + n + yQ), (T !== xQ || z !== xQ) && (D += "skew(" + T + ", " + z + yQ), (w !== 1 || o !== 1) && (D += "scale(" + w + ", " + o + yQ), l.style[V] = D || "translate(0, 0)";
}, hi = function(Q, B) {
  var t = B || this, A = t.xPercent, P = t.yPercent, e = t.x, i = t.y, s = t.rotation, v = t.skewX, c = t.skewY, n = t.scaleX, T = t.scaleY, z = t.target, w = t.xOrigin, o = t.yOrigin, a = t.xOffset, E = t.yOffset, l = t.forceCSS, U = parseFloat(e), D = parseFloat(i), k, O, u, N, d;
  s = parseFloat(s), v = parseFloat(v), c = parseFloat(c), c && (c = parseFloat(c), v += c, s += c), s || v ? (s *= BB, v *= BB, k = Math.cos(s) * n, O = Math.sin(s) * n, u = Math.sin(s - v) * -T, N = Math.cos(s - v) * T, v && (c *= BB, d = Math.tan(v - c), d = Math.sqrt(1 + d * d), u *= d, N *= d, c && (d = Math.tan(c), d = Math.sqrt(1 + d * d), k *= d, O *= d)), k = X(k), O = X(O), u = X(u), N = X(N)) : (k = n, N = T, O = u = 0), (U && !~(e + "").indexOf("px") || D && !~(i + "").indexOf("px")) && (U = pQ(z, "x", e, "px"), D = pQ(z, "y", i, "px")), (w || o || a || E) && (U = X(U + w - (w * k + o * u) + a), D = X(D + o - (w * O + o * N) + E)), (A || P) && (d = z.getBBox(), U = X(U + A / 100 * d.width), D = X(D + P / 100 * d.height)), d = "matrix(" + k + "," + O + "," + u + "," + N + "," + U + "," + D + ")", z.setAttribute("transform", d), l && (z.style[V] = d);
}, li = function(Q, B, t, A, P) {
  var e = 360, i = Z(P), s = parseFloat(P) * (i && ~P.indexOf("rad") ? IQ : 1), v = s - A, c = A + v + "deg", n, T;
  return i && (n = P.split("_")[1], n === "short" && (v %= e, v !== v % (e / 2) && (v += v < 0 ? e : -e)), n === "cw" && v < 0 ? v = (v + e * PA) % e - ~~(v / e) * e : n === "ccw" && v > 0 && (v = (v - e * PA) % e - ~~(v / e) * e)), Q._pt = T = new eQ(Q._pt, B, t, A, v, Bi), T.e = c, T.u = "deg", Q._props.push(t), T;
}, nA = function(Q, B) {
  for (var t in B)
    Q[t] = B[t];
  return Q;
}, ki = function(Q, B, t) {
  var A = nA({}, t._gsap), P = "perspective,force3D,transformOrigin,svgOrigin", e = t.style, i, s, v, c, n, T, z, w;
  A.svg ? (v = t.getAttribute("transform"), t.setAttribute("transform", ""), e[V] = B, i = uB(t, 1), gQ(t, V), t.setAttribute("transform", v)) : (v = getComputedStyle(t)[V], e[V] = B, i = uB(t, 1), e[V] = v);
  for (s in OQ)
    v = A[s], c = i[s], v !== c && P.indexOf(s) < 0 && (z = _(v), w = _(c), n = z !== w ? pQ(t, s, v, w) : parseFloat(v), T = parseFloat(c), Q._pt = new eQ(Q._pt, i, s, n, T - n, Et), Q._pt.u = w || 0, Q._props.push(s));
  nA(i, A);
};
PQ("padding,margin,Width,Radius", function(r, Q) {
  var B = "Top", t = "Right", A = "Bottom", P = "Left", e = (Q < 3 ? [B, t, A, P] : [B + P, B + t, A + t, A + P]).map(function(i) {
    return Q < 2 ? r + i : "border" + i + r;
  });
  pB[Q > 1 ? "border" + r : r] = function(i, s, v, c, n) {
    var T, z;
...
var wP = {
  ...
  init: function(Q, B, t, A, P) {
    var e = this._props, i = Q.style, s = t.vars.startAt, v, c, n, T, z, w, o, a, E, l, U, D, k, O, u, N, d;
    gt || lt(), this.styles = this.styles || sP(Q), N = this.styles.props, this.tween = t;
    for (o in B)
      if (o !== "autoRound" && (c = B[o], !(cQ[o] && qA(o, B, t, A, Q, P)))) {
        if (z = typeof c, w = pB[o], z === "function" && (c = c.call(t, A, Q, P), z = typeof c), z === "string" && ~c.indexOf("random(") && (c = lB(c)), w)
          w(this, Q, o, c, t) && (u = 1);
        else if (o.substr(0, 2) === "--")
          v = (getComputedStyle(Q).getPropertyValue(o) + "").trim(), c += "", GQ.lastIndex = 0, GQ.test(v) || (a = _(v), E = _(c), E ? a !== E && (v = pQ(Q, o, v, E) + E) : a && (c += a)), this.add(i, "setProperty", v, c, A, P, 0, 0, o), e.push(o), N.push(o, 0, i[o]);
        else if (z !== "undefined") {
          if (s && o in s ? (v = typeof s[o] == "function" ? s[o].call(t, A, Q, P) : s[o], Z(v) && ~v.indexOf("random(") && (v = lB(v)), _(v + "") || v === "auto" || (v += zQ.units[o] || _(CQ(Q, o)) || ""), (v + "").charAt(1) === "=" && (v = CQ(Q, o))) : v = CQ(Q, o), T = parseFloat(v), l = z === "string" && c.charAt(1) === "=" && c.substr(0, 2), l && (c = c.substr(2)), n = parseFloat(c), o in DQ && (o === "autoAlpha" && (T === 1 && CQ(Q, "visibility") === "hidden" && n && (T = 0), N.push("visibility", 0, i.visibility), FQ(this, i, "visibility", T ? "inherit" : "hidden", n ? "inherit" : "hidden", !n)), o !== "scale" && o !== "transform" && (o = DQ[o], ~o.indexOf(",") && (o = o.split(",")[0]))), U = o in OQ, U) {
            ...
            if (D || (k = Q._gsap, k.renderTransform && !B.parseTransform || uB(Q, B.parseTransform), O = B.smoothOrigin !== !1 && k.smooth, D = this._pt = new eQ(this._pt, i, V, 0, 1, k.renderTransform, k, 0, -1), D.dep = 1), o === "scale")
              this._pt = new eQ(this._pt, k, "scaleY", k.scaleY, (l ? $Q(k.scaleY, l + n) : n) - k.scaleY || 0, Et), this._pt.u = 0, e.push("scaleY", o), o += "X";
            else if (o === "transformOrigin") {
              N.push(iQ, 0, i[iQ]), c = wi(c), k.svg ? kt(Q, c, 0, O, 0, this) : (E = parseFloat(c.split(" ")[2]) || 0, E !== k.zOrigin && FQ(this, k, "zOrigin", k.zOrigin, E), FQ(this, i, o, mB(v), mB(c)));
              continue;
            } else if (o === "svgOrigin") {
              kt(Q, c, 1, O, 0, this);
              continue;
            } else if (o in oP) {
              li(this, k, o, T, l ? $Q(T, l + c) : c);
              continue;
          ...
          } else o in i || (o = iB(o) || o);
          if (U || (n || n === 0) && (T || T === 0) && !Qi.test(c) && o in i)
            a = (v + "").substr((T + "").length), n || (n = 0), E = _(c) || (o in zQ.units ? zQ.units[o] : a), a !== E && (T = pQ(Q, o, v, E)), this._pt = new eQ(this._pt, U ? k : i, o, T, (l ? $Q(T, l + n) : n) - T, !U && (E === "px" || o === "zIndex") && B.autoRound !== !1 ? Pi : Et), this._pt.u = E || 0, U && d !== c ? (this._pt.b = v, this._pt.e = d, this._pt.r = Ai) : a !== E && E !== "%" && (this._pt.b = v, this._pt.r = ti);
          else if (o in i)
            zi.call(this, Q, o, v, l ? l + c : c);
          else if (o in Q)
            this.add(Q, o, v || Q[o], l ? l + c : c, A, P);
  ...
  get: CQ,
  aliases: DQ,
  getSetter: function(Q, B, t) {
    var A = DQ[B];
    return A && A.indexOf(",") < 0 && (B = A), B in OQ && B !== iQ && (Q._gsap.x || CQ(Q, "x")) ? t && AA === t ? B === "scale" ? si : ri : (AA = t || {}) && (B === "scale" ? vi : ci) : Q.style && !ut(Q.style[B]) ? ei : ~B.indexOf("-") ? ii : Gt(Q, B);
  },
  core: {
    _removeProperty: gQ,
    _getMatrix: xt
...
function di(r) {
  const Q = Math.max(1e-3, r), B = Q * 1.6, t = Q * 0.7, A = new h.ConeGeometry(t, B, 3, 1, !1);
  return A.rotateX(-Math.PI / 2), A.translate(0, 0, B / 2), A;
}
function Ci(r) {
  const B = Math.max(1e-3, r) * 0.65, t = new h.SphereGeometry(B, 18, 12);
  return t.translate(0, 0, B), t;
}
function Ni(r, Q) {
  r.computeBoundingBox();
  const B = r.boundingBox, t = Math.max(1e-6, B.max.z - B.min.z), A = r.getAttribute("position"), P = new Float32Array(A.count * 2);
  for (let e = 0; e < A.count; e++) {
    const i = A.getX(e), s = A.getY(e), v = A.getZ(e);
    P[e * 2] = Math.atan2(s, i) / (2 * Math.PI) + 0.5, P[e * 2 + 1] = (v - B.min.z) / t * Q;
  }
  r.setAttribute("uv", new h.BufferAttribute(P, 2));
}
function EP(r) {
  const Q = atob(Ui), B = new Uint8Array(Q.length);
  for (let v = 0; v < Q.length; v++) B[v] = Q.charCodeAt(v);
  const A = new CP().parse(B.buffer);
  A.rotateX(-Math.PI / 2), A.computeBoundingBox();
  const P = A.boundingBox, e = new h.Vector3();
  P.getSize(e);
  ...
  return A.translate(
    ...
    -(s.min.y + s.max.y) / 2,
    -s.min.z
  ), Ni(A, 6), A.computeVertexNormals(), A;
}
let $B = null;
function Oi() {
  if ($B) return $B;
  ...
  t.fillStyle = "#808080", t.fillRect(0, 0, r, Q);
  const A = 10;
  for (let i = 0; i < A; i++) {
    const s = i / A * r, v = i % 2 === 0 ? 150 : 90;
    t.fillStyle = `rgb(${v},${v},${v})`, t.fillRect(s, 0, r / A / 2, Q);
  }
  const P = t.getImageData(0, 0, r, Q);
  ...
  return e.wrapS = h.RepeatWrapping, e.wrapT = h.RepeatWrapping, $B = e, e;
}
function Li(r, Q) {
  const B = Math.max(1e-3, r), t = B * 1.6, A = B * 0.08, P = B * 0.187, e = CB(Q), i = new h.CylinderGeometry(0, A, t, 8, 1, !1);
  i.rotateX(-Math.PI / 2), i.translate(0, 0, t / 2);
  const s = new h.CylinderGeometry(0, P, t, 8, 1, !0);
  s.rotateX(-Math.PI / 2), s.translate(0, 0, t / 2);
  const v = new h.MeshBasicMaterial({
    color: new h.Color(TA),
    transparent: !0,
    opacity: e,
    depthTest: !1,
    depthWrite: !1,
...
function Mi(r) {
  ...
  return r.bit.type === "triangle" ? di(Q) : r.bit.type === "drill" ? EP(Q) : Ci(Q);
}
function hP(r) {
  return r.bit.colorSource === "custom" ? r.bit.color : Di;
}
function Hi(r) {
  const Q = CB(r.bit.opacity), B = EP(Math.max(1e-3, r.bit.size)), t = new h.Color(hP(r)), A = Oi(), P = new h.MeshStandardMaterial({
    color: t,
    metalness: 0.8,
    roughness: 0.35,
    roughnessMap: A,
...
function ji(r) {
  const Q = CB(r.bit.opacity), B = Mi(r), t = new h.MeshBasicMaterial({
    color: new h.Color(aP),
    transparent: Q < 1,
    opacity: Q,
    side: h.DoubleSide,
    depthTest: !1,
...
function zA(r) {
  return r.bit.type === "laser" ? Li(r.bit.size, r.bit.opacity) : r.bit.type === "drill" ? Hi(r) : ji(r);
}
function wA(r) {
...
function Fi(r) {
  ...
  let B = r, t = zA(B);
  Q.add(t);
  let A = null, P = !1, e = null, i = null;
  const s = (n) => {
    const T = Math.max(1, n);
    return jB.to(t.rotation, {
      z: `+=${Math.PI * 2}`,
      duration: 60 / T,
      repeat: -1,
      ease: "none",
  ...
  e = s(B.bit.spinRpm);
  const v = (n) => {
    const T = n.bit.type, z = B.bit.type, w = n.bit.size, o = B.bit.size, a = T !== z, E = w !== o, l = CB(n.bit.opacity);
    if (a || E)
      Q.remove(t), wA(t), t = zA(n), Q.add(t), i == null || i.kill(), e == null || e.kill(), e = s(n.bit.spinRpm), P && e.timeScale(1).play();
    else if (T === "laser") {
      const D = t, [k, O] = D.children, u = k.material, N = O.material;
      u.opacity = l, u.needsUpdate = !0, N.opacity = l * 0.3, N.needsUpdate = !0;
    } else {
      const k = t.material, O = new h.Color(T === "drill" ? hP(n) : aP);
  ...
  return {
    ...
    setOptions: (n) => v(n),
    setTarget: (n, T) => {
      const z = !!(T != null && T.immediate), w = Number(n.x) || 0, o = Number(n.y) || 0, a = Number(n.z) || 0;
      if (n.a, z || B.bit.tweenMs <= 0) {
        A = null, Q.position.set(w, o, a);
        return;
      }
      A = {
    ...
    update: (n) => {
      if (!A) return;
      const T = (n - A.startedAt) / A.duration, z = ui(T);
      Q.position.set(
    ...
    dispose: () => {
      i == null || i.kill(), e == null || e.kill(), Q.remove(t), wA(t);
    }
  };
}
const fi = 25.4, Gi = 500, bi = 300;
class Vi {
  constructor(Q) {
    this.viewCube = null, this.resizeObserver = null, this.onWindowResize = null, this.animationFrameId = null, this.gridGroup = null, this.axesGroup = null, this.gridLabelsGroup = null, this.boundingBoxGroup = null, this.machineBedGroup = null, this.bitMarker = null, this.preLaserBitType = "drill", this.toolpathStreams = [], this.toolpathCutBucketCount = 1, this.toolpathRotationA = 0, this.lastBitPosition = { x: 0, y: 0, z: 0, a: 0 }, this.sim3dHandle = null, this.currentLines = [], this.linePositions = null, this.renderSequence = 0, this.currentBounds = null, this.cameraFocusTransition = null, this.cameraFollowRequested = !1, this.cameraFollowInterrupted = !1, this.cameraFollowOffset = null, this.id = Q.id, this.container = Q.container, this.callbacks = Q.callbacks ?? {}, this.options = EA(fP, Q.options), this.canvas = document.createElement("canvas"), this.canvas.style.width = "100%", this.canvas.style.height = "100%", this.canvas.style.display = "block", this.container.appendChild(this.canvas), KP(this.container), this.renderer = new h.WebGLRenderer({
      canvas: this.canvas,
    ...
    const t = new h.DirectionalLight(16777215, 0.5);
    t.position.set(-1, 1, 1), this.scene.add(t), this.toolpathRoot = new h.Group(), this.toolpathRoot.name = "gviewer:toolpath-root", this.scene.add(this.toolpathRoot), this.camera = new h.PerspectiveCamera(this.options.camera.fov, 1, 0.1, 1e5), this.camera.up.set(0, 0, 1), this.camera.position.set(
      this.options.camera.initialPosition.x,
      this.options.camera.initialPosition.y,
      this.options.camera.initialPosition.z
    ), this.controls = new kP(this.camera, this.renderer.domElement), this.controls.enableDamping = this.options.camera.orbit.enableDamping, this.controls.addEventListener("start", () => {
      this.cameraFollowRequested && (this.cameraFollowInterrupted = !0);
    }), this.controls.update(), this.viewCubeCorrection = new h.Matrix4().makeRotationX(h.MathUtils.degToRad(90)), this.viewCube = new xP({
      ...
      onSelectView: (A) => {
        ...
        this.startSnapToView(A, P, Math.max(1e-6, e), 400);
      }
  ...
  setBitPosition(Q, B) {
    ...
    if (this.ensureBitMarker(), this.lastBitPosition = { ...this.lastBitPosition, ...Q }, (t = this.bitMarker) == null || t.setTarget(Q, B), this.cameraFollowRequested && !this.cameraFollowInterrupted && this.cameraFollowOffset) {
      const A = new h.Vector3(this.lastBitPosition.x, this.lastBitPosition.y, this.controls.target.z), P = A.clone().add(this.cameraFollowOffset);
      this.startCameraLerp(A, P, bi, "linear");
    }
  }
  /**
  ...
  setCameraFollowEnabled(Q) {
    ...
    const B = new h.Vector3(this.lastBitPosition.x, this.lastBitPosition.y, this.controls.target.z), t = B.clone().add(this.cameraFollowOffset);
    this.startCameraLerp(B, t, Gi);
  }
  setBitVisible(Q) {
    var B;
  ...
  hideUntilLine(Q, B) {
    const t = B ?? this.options.progress.mode, A = Math.floor(Q);
    for (const P of this.toolpathStreams) {
      const e = A < 0 ? 0 : P.prefixEndVertex[Math.min(A, P.prefixEndVertex.length - 1)];
      t === "grey" ? (P.line.geometry.setDrawRange(0, P.totalVertices), HP({ stream: P, nextCursorVertex: e, options: this.options })) : P.line.geometry.setDrawRange(e, Math.max(0, P.totalVertices - e));
    }
  }
  seekToLine(Q, B) {
    if (this.hideUntilLine(Q, B), this.linePositions && this.currentLines.length > 0) {
      const t = Math.max(0, Math.min(Math.floor(Q), this.currentLines.length - 1)), A = {
        x: this.linePositions[t * 4],
        y: this.linePositions[t * 4 + 1],
        z: this.linePositions[t * 4 + 2],
        a: this.linePositions[t * 4 + 3]
      };
      this.setBitPosition(A, { immediate: !0 });
    }
    if (this.sim3dHandle) {
      const t = Math.max(0, Math.min(Math.floor(Q), this.currentLines.length - 1));
      if (t !== this.sim3dHandle.currentLine) {
        const A = SB(t, this.sim3dHandle.data);
        YB(A, this.sim3dHandle.data.resolution, this.options.sim3d.erosionPasses, this.sim3dHandle.data.slabBounds.zTop), VB(this.sim3dHandle.slab, A), this.sim3dHandle.currentLine = t;
      }
    }
  }
  showAll() {
  ...
  snapCameraToView(Q, B = {}) {
    const t = Math.max(0, Math.floor(B.durationMs ?? 240)), A = this.controls.target.clone(), P = this.camera.position.distanceTo(A), e = Math.max(1e-6, B.distance ?? P);
    this.startSnapToView(Q, A, e, t);
  }
  /**
  ...
  screenToWorld(Q, B, t) {
    ...
    const P = new h.Vector2(
      ...
      -((B - A.top) / A.height) * 2 + 1
    ), e = new h.Raycaster();
    e.setFromCamera(P, this.camera);
    const i = (t == null ? void 0 : t.planeZ) ?? this.lastBitPosition.z ?? 0, s = new h.Plane(new h.Vector3(0, 0, 1), -i), v = new h.Vector3();
   ...
   */
  worldToScreen(Q, B, t = 0) {
    const A = this.canvas.getBoundingClientRect();
    if (A.width === 0 || A.height === 0)
      return null;
    const P = new h.Vector3(Q, B, t).project(this.camera);
    return {
      x: A.left + (P.x + 1) / 2 * A.width,
      y: A.top + (1 - P.y) / 2 * A.height
    };
  }
  startSnapToView(Q, B, t, A) {
    const P = Rt(Q), e = B.clone().add(P.multiplyScalar(t)), i = Math.max(1, t);
    this.camera.near = i / 1e3, this.camera.far = i * 50, this.camera.updateProjectionMatrix(), this.startCameraLerp(B, e, A);
  }
  ...
  startCameraLerp(Q, B, t, A = "easeInOutCubic") {
  ...
  async loadFromUrl(Q, B = {}) {
    ...
    const t = await fetch(Q, { signal: B.signal });
    if (!t.ok)
      throw this.emitProgress({ state: "hidden" }), new Error(`Failed to load gcode: ${t.statusText}`);
  ...
  async loadFromWorkerData(Q) {
    this.currentLines = [];
    const { rapid: B, cut: t } = UP(Q), A = (Q.toolchangeCount ?? 0) > 0;
  ...
  setOptions(Q) {
    var T, z, w, o, a, E, l, U, D, k, O, u, N, d, M, b, G, C, j, F, H, f, I, R, QQ, sQ, W, x, BQ, vQ, rB, yt, It, St;
    const B = this.options;
    this.options = EA(this.options, Q), (B.render.antialias !== this.options.render.antialias || B.render.theme.background !== this.options.render.theme.background) && this.renderer.setClearColor(this.options.render.theme.background, 1), (B.render.theme.opacity !== this.options.render.theme.opacity || B.render.theme.rapidOpacity !== this.options.render.theme.rapidOpacity) && this.refreshToolpathOpacities(), B.camera.fov !== this.options.camera.fov && (this.camera.fov = this.options.camera.fov, this.camera.updateProjectionMatrix()), B.camera.orbit.enableDamping !== this.options.camera.orbit.enableDamping && (this.controls.enableDamping = this.options.camera.orbit.enableDamping);
    const t = ((T = B.grid.bounds) == null ? void 0 : T.min.x) !== ((z = this.options.grid.bounds) == null ? void 0 : z.min.x) || ((w = B.grid.bounds) == null ? void 0 : w.min.y) !== ((o = this.options.grid.bounds) == null ? void 0 : o.min.y) || ((a = B.grid.bounds) == null ? void 0 : a.max.x) !== ((E = this.options.grid.bounds) == null ? void 0 : E.max.x) || ((l = B.grid.bounds) == null ? void 0 : l.max.y) !== ((U = this.options.grid.bounds) == null ? void 0 : U.max.y), A = B.units !== this.options.units || B.grid.sizeX !== this.options.grid.sizeX || B.grid.sizeY !== this.options.grid.sizeY || B.grid.axisDepth !== this.options.grid.axisDepth || t, P = B.render.theme.colors.grid.major !== this.options.render.theme.colors.grid.major || B.render.theme.colors.grid.minor !== this.options.render.theme.colors.grid.minor || B.render.theme.colors.axes.x !== this.options.render.theme.colors.axes.x || B.render.theme.colors.axes.y !== this.options.render.theme.colors.axes.y || B.render.theme.colors.axes.z !== this.options.render.theme.colors.axes.z;
    (A || P) && this.renderGridAndAxes(), (B.grid.labels !== this.options.grid.labels || B.units !== this.options.units || B.grid.sizeX !== this.options.grid.sizeX || B.grid.sizeY !== this.options.grid.sizeY || t || P) && this.refreshGridLabels(), (B.boundingBox.visible !== this.options.boundingBox.visible || B.boundingBox.labels !== this.options.boundingBox.labels || B.units !== this.options.units || B.render.theme.colors.boundingBox !== this.options.render.theme.colors.boundingBox) && this.refreshBoundingBox(), (B.machineBed.visible !== this.options.machineBed.visible || ((D = B.machineBed.min) == null ? void 0 : D.x) !== ((k = this.options.machineBed.min) == null ? void 0 : k.x) || ((O = B.machineBed.min) == null ? void 0 : O.y) !== ((u = this.options.machineBed.min) == null ? void 0 : u.y) || ((N = B.machineBed.max) == null ? void 0 : N.x) !== ((d = this.options.machineBed.max) == null ? void 0 : d.x) || ((M = B.machineBed.max) == null ? void 0 : M.y) !== ((b = this.options.machineBed.max) == null ? void 0 : b.y) || ((C = (G = B.machineBed.keepout) == null ? void 0 : G.min) == null ? void 0 : C.x) !== ((F = (j = this.options.machineBed.keepout) == null ? void 0 : j.min) == null ? void 0 : F.x) || ((f = (H = B.machineBed.keepout) == null ? void 0 : H.min) == null ? void 0 : f.y) !== ((R = (I = this.options.machineBed.keepout) == null ? void 0 : I.min) == null ? void 0 : R.y) || ((sQ = (QQ = B.machineBed.keepout) == null ? void 0 : QQ.max) == null ? void 0 : sQ.x) !== ((x = (W = this.options.machineBed.keepout) == null ? void 0 : W.max) == null ? void 0 : x.x) || ((vQ = (BQ = B.machineBed.keepout) == null ? void 0 : BQ.max) == null ? void 0 : vQ.y) !== ((yt = (rB = this.options.machineBed.keepout) == null ? void 0 : rB.max) == null ? void 0 : yt.y) || B.render.theme.colors.machineBed !== this.options.render.theme.colors.machineBed || B.render.theme.colors.machineBedKeepout !== this.options.render.theme.colors.machineBedKeepout) && this.refreshMachineBed();
    ...
    if (B.sim3d.erosionPasses !== this.options.sim3d.erosionPasses && this.sim3dHandle) {
      const lP = this.sim3dHandle.currentLine, Yt = SB(lP, this.sim3dHandle.data);
      YB(Yt, this.sim3dHandle.data.resolution, this.options.sim3d.erosionPasses, this.sim3dHandle.data.slabBounds.zTop), VB(this.sim3dHandle.slab, Yt);
    }
  ...
  resize() {
    ...
    this.renderer.setSize(B, t, !1), this.camera.aspect = B / t, this.camera.updateProjectionMatrix();
  }
  focusToModel() {
  ...
  emitProgress(Q) {
    var B, t;
    (t = (B = this.callbacks).onProgress) == null || t.call(B, { id: this.id, ...Q });
  }
  emitBoundsChanged() {
    var Q, B;
  ...
  startAnimationLoop() {
    const Q = () => {
      ...
      this.controls.update(), this.updateCameraFocusTransition(), this.updateViewCubeRotation(), (t = this.bitMarker) == null || t.update(B), this.renderer.render(this.scene, this.camera);
    };
    Q();
  }
  ensureBitMarker() {
    this.bitMarker || (this.bitMarker = Fi(this.options), this.scene.add(this.bitMarker.object));
  }
  worldSizes() {
    const Q = this.options.units === "in" ? fi : 1;
    return {
  ...
  renderGridAndAxes() {
    const { sizeXMm: Q, sizeYMm: B, axisDepthMm: t, bounds: A } = this.worldSizes();
    this.setGridGroup(
      qP({
  ...
  refreshGridLabels() {
    ...
    const { sizeXMm: Q, sizeYMm: B, bounds: t } = this.worldSizes();
    this.setGridLabelsGroup(
      Be({ sizeXMm: Q, sizeYMm: B, bounds: t, units: this.options.units, theme: this.options.render.theme })
    );
  }
  refreshBoundingBox() {
    ...
    this.setBoundingBoxGroup(SP(this.currentBounds, this.options));
  }
  refreshMachineBed() {
    const { visible: Q, min: B, max: t, keepout: A } = this.options.machineBed;
    if (!Q || !B || !t) {
      this.setMachineBedGroup(null);
      return;
    }
    this.setMachineBedGroup(RP(B, t, this.options, A));
  }
  setBitMarker(Q) {
  ...
  setGridGroup(Q) {
    this.gridGroup && (this.scene.remove(this.gridGroup), _P(this.gridGroup), this.gridGroup = null), Q && (this.gridGroup = Q, this.scene.add(Q));
  }
  setAxesGroup(Q) {
    this.axesGroup && (this.scene.remove(this.axesGroup), Qe(this.axesGroup), this.axesGroup = null), Q && (this.axesGroup = Q, this.scene.add(Q));
  }
  setGridLabelsGroup(Q) {
    this.gridLabelsGroup && (this.scene.remove(this.gridLabelsGroup), yP(this.gridLabelsGroup), this.gridLabelsGroup = null), Q && (this.gridLabelsGroup = Q, this.scene.add(Q));
  }
  setBoundingBoxGroup(Q) {
    this.boundingBoxGroup && (this.scene.remove(this.boundingBoxGroup), VP(this.boundingBoxGroup), this.boundingBoxGroup = null), Q && (this.boundingBoxGroup = Q, this.scene.add(Q));
  }
  setMachineBedGroup(Q) {
    this.machineBedGroup && (this.scene.remove(this.machineBedGroup), XP(this.machineBedGroup), this.machineBedGroup = null), Q && (this.machineBedGroup = Q, this.scene.add(Q));
  }
  setGeometryEmpty() {
    this.setSim3dHandle(null), OP(this.toolpathRoot, this.toolpathStreams), this.toolpathStreams = [], this.toolpathCutBucketCount = 1, this.linePositions = null, this.currentBounds = null, this.emitBoundsChanged(), this.refreshBoundingBox();
  }
  setToolpathGeometry(Q) {
    ...
    const B = [
      {
        ...
        opacity: aA(this.options.render.theme.rapidOpacity ?? 0.3)
      },
      ...Q.cuts.map((P, e) => ({
        kind: "cut",
        cutBucketIndex: e,
    ...
    ], { streams: t, bounds: A } = NP({
      specs: B,
      options: this.options,
      scene: this.toolpathRoot
    });
    this.toolpathStreams = t, this.currentBounds = A ? A.clone() : null, this.emitBoundsChanged(), this.refreshBoundingBox(), this.setToolpathRotationA(this.toolpathRotationA);
  }
  ...
  cutBucketOpacity(Q) {
    const B = this.options.render.theme.opacity, t = this.toolpathCutBucketCount;
    if (t <= 1)
      return B;
    const A = Q / (t - 1);
    return aA(B * A);
  ...
  buildLinePositions() {
    ...
    for (let i = 0; i < Q; i++) {
      ...
      s && e.processLine(s), this.linePositions[i * 4] = B, this.linePositions[i * 4 + 1] = t, this.linePositions[i * 4 + 2] = A, this.linePositions[i * 4 + 3] = P;
    }
  }
  async renderScene() {
    ...
    try {
      ...
      const B = await DP(this.currentLines, {
        ...
        batch: {
          ...
          onProgress: (t, A) => {
            Q === this.renderSequence && this.emitProgress({ state: "determinate", label: "Building geometry...", processed: t, total: A });
          }
        }
      });
  ...
  setSim3dHandle(Q) {
    this.sim3dHandle && (this.scene.remove(this.sim3dHandle.slab.group), mP(this.sim3dHandle.slab), this.sim3dHandle = null), Q && (this.sim3dHandle = Q, this.scene.add(Q.slab.group));
  }
  ...
  async buildAndApplySim3d(Q) {
    ...
    const B = this.options.sim3d.toolDiameter / 2, t = this.options.sim3d.resolution;
    let A;
    try {
      A = await GP(this.currentLines, B, t, {
        arcSegments: this.options.geometry.arcSegments,
        batch: {
          shouldAbort: () => Q !== this.renderSequence,
          onProgress: (i, s) => {
    ...
    const P = SB(0, A);
    YB(P, t, this.options.sim3d.erosionPasses, A.slabBounds.zTop);
    const e = pP(A.slabBounds, t);
    VB(e, P), this.setSim3dHandle({ data: A, slab: e, currentLine: 0 });
  }
  startCameraFocus(Q) {
    ...
    Q.getSize(t);
    const A = Math.max(t.x, t.y, 1) / 2, P = h.MathUtils.degToRad(this.camera.fov), e = A / Math.tan(P / 2) * 1.25 + t.z, i = Rt("front-top-left"), s = B.clone().add(i.multiplyScalar(e)), v = Math.max(t.x, t.y, t.z, 1);
  ...
  updateCameraFocusTransition() {
    if (!this.cameraFocusTransition)
      return;
    const B = performance.now() - this.cameraFocusTransition.startedAt, t = Math.min(1, Math.max(0, B / this.cameraFocusTransition.duration)), A = this.cameraFocusTransition.easing === "linear" ? t : ZP(t);
    if (this.camera.position.lerpVectors(this.cameraFocusTransition.fromPosition, this.cameraFocusTransition.toPosition, A), this.controls.target.lerpVectors(this.cameraFocusTransition.fromTarget, this.cameraFocusTransition.toTarget, A), t >= 1) {
  ...
  updateViewCubeRotation() {
    ...
    const t = WP(this.camera.position, this.controls.target);
    this.viewCube.setActiveFace(t);
  }
}
function aA(r) {
  return Number.isFinite(r) ? Math.max(0, Math.min(1, r)) : 0;
...
function EA(r, Q) {
  ...
  if (!Q)
    return { ...r };
  const B = gi(r.render.theme, (t = Q.render) == null ? void 0 : t.theme);
  return {
    ...r,
    ...Q,
    mode: { ...r.mode, ...Q.mode },
...
function gi(r, Q) {
  ...
  return Q ? {
    ...
    colors: {
      ...
      axes: { ...r.colors.axes, ...(t = Q.colors) == null ? void 0 : t.axes }
    }
  } : r;
}
const pi = {
...
class Ji {
  constructor(Q, B) {
    this.crosshairPos = null, this.crosshairVisible = !1, this.pathEls = [], this.segmentGroups = [], this.workerMode = !1, this.viewBox = { x: 0, y: 0, w: 100, h: 100 }, this.rotX = vB, this.rotY = cB, this.centerX = 0, this.centerY = 0, this.centerZ = 0, this.focalLength = 500, this.cosRotX = 1, this.sinRotX = 0, this.cosRotY = 1, this.sinRotY = 0, this.dragMode = "none", this.activePointers = /* @__PURE__ */ new Map(), this.pinchLastDist = 0, this.pinchLastMid = { x: 0, y: 0 }, this.rafPending = !1, this.overlayRafPending = !1, this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.bounds = { minX: 0, minY: 0, maxX: 0, maxY: 0, minZ: 0, maxZ: 0, empty: !0 }, this.hasZInfo = !0, this.project = (t, A, P) => {
      if (this.options.projectionMode === "top")
        return { x: t, y: -A };
      const e = t - this.centerX, i = A - this.centerY, s = P - this.centerZ, v = e * this.cosRotY + s * this.sinRotY, c = i, n = -e * this.sinRotY + s * this.cosRotY, T = v, z = c * this.cosRotX - n * this.sinRotX, w = c * this.sinRotX + n * this.cosRotX;
      ...
      t.preventDefault();
      const A = t.deltaY > 0 ? 1.1 : 1 / 1.1, P = this.svgPoint(t.clientX, t.clientY), { x: e, y: i, w: s, h: v } = this.viewBox;
      this.viewBox = {
        x: P.x - (P.x - e) * A,
        y: P.y - (P.y - i) * A,
        w: s * A,
      ...
      if (this.activePointers.set(t.pointerId, { x: t.clientX, y: t.clientY }), this.svg.setPointerCapture(t.pointerId), t.preventDefault(), this.activePointers.size === 1)
        this.dragMode = "pan", this.svg.style.cursor = "grabbing";
      else if (this.activePointers.size === 2) {
        this.dragMode = "pan", this.svg.style.cursor = "move";
        const [A, P] = Array.from(this.activePointers.values());
        this.pinchLastDist = Math.hypot(P.x - A.x, P.y - A.y), this.pinchLastMid = { x: (A.x + P.x) / 2, y: (A.y + P.y) / 2 };
      }
    }, this.onPointerMove = (t) => {
      if (!this.activePointers.has(t.pointerId)) return;
      const A = this.activePointers.get(t.pointerId);
      if (this.activePointers.set(t.pointerId, { x: t.clientX, y: t.clientY }), this.activePointers.size === 2) {
        ...
        if (this.pinchLastDist > 0) {
          const w = this.pinchLastDist / v, o = this.svgPoint(c.x, c.y), { x: a, y: E, w: l, h: U } = this.viewBox;
          this.viewBox = {
            x: o.x - (o.x - a) * w,
            y: o.y - (o.y - E) * w,
            w: l * w,
      ...
      const P = t.clientX - A.x, e = t.clientY - A.y;
      ...
      if (this.activePointers.delete(t.pointerId), this.svg.releasePointerCapture(t.pointerId), this.activePointers.size === 1) {
      ...
      t.preventDefault();
    }, this.options = { ...pi, ...B }, this.svg = document.createElementNS("http://www.w3.org/2000/svg", "svg"), this.svg.style.cssText = "width:100%;height:100%;display:block;cursor:grab;user-select:none;touch-action:none;", this.svg.setAttribute("preserveAspectRatio", "xMidYMid meet"), this.svg.setAttribute("xmlns", "http://www.w3.org/2000/svg"), this.bboxPath = Qt(), this.pathLayer = document.createElementNS("http://www.w3.org/2000/svg", "g"), this.bboxLabelX = Bt("middle", "hanging"), this.bboxLabelY = Bt("start", "middle"), this.bboxLabelZ = Bt("middle", "auto"), this.originMarker = document.createElementNS("http://www.w3.org/2000/svg", "circle"), this.originMarker.setAttribute("stroke-opacity", "0.4"), this.originMarker.setAttribute("visibility", "hidden"), this.crosshairEl = Qt(), this.crosshairEl.setAttribute("fill", "none"), this.crosshairEl.setAttribute("visibility", "hidden"), this.svg.appendChild(this.bboxPath), this.svg.appendChild(this.pathLayer), this.svg.appendChild(this.bboxLabelX), this.svg.appendChild(this.bboxLabelY), this.svg.appendChild(this.bboxLabelZ), this.svg.appendChild(this.originMarker), this.svg.appendChild(this.crosshairEl), Q.appendChild(this.svg), this.applyOptions(), this.bindEvents();
  }
  // ── Public API ────────────────────────────────────────────────────────────
  loadFromLines(Q) {
    const { rapid: B, cutting: t } = uP(Q, {
      arcSegments: this.options.arcSegments
    });
    if (this.rapidVerts = B, this.cutVerts = t, this.workerMode = !1, this.syncSegmentGroupsFromLines(), this.hasZInfo = !0, this.bounds = At([
      { verts: B, stride: 6 },
      { verts: t, stride: 6 }
    ]), !this.bounds.empty) {
    ...
    this.rotX = vB, this.rotY = cB, this.updateTrig(), this.fitView(), this.rebuildAndRender();
  }
  loadFromFile(Q) {
    return new Promise((B, t) => {
      ...
      }, A.onerror = () => t(new Error("FileReader error")), A.readAsText(Q);
    });
  }
  loadFromText(Q) {
  ...
  loadFromWorkerData(Q) {
    const B = dP(Q);
    if (this.workerMode = !0, this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.segmentGroups = B.map((t) => ({
      color: t.hexColor,
      opacity: t.opacity,
      verts: t.positions,
      stride: 6
    })), this.hasZInfo = !0, this.bounds = At(B.map((t) => ({ verts: t.positions, stride: 6 }))), !this.bounds.empty) {
      this.centerX = (this.bounds.minX + this.bounds.maxX) / 2, this.centerY = (this.bounds.minY + this.bounds.maxY) / 2, this.centerZ = (this.bounds.minZ + this.bounds.maxZ) / 2;
      const t = Math.hypot(
        this.bounds.maxX - this.bounds.minX,
        this.bounds.maxY - this.bounds.minY,
  ...
  loadFromPrecomputedGroups(Q, B) {
    this.workerMode = !0, this.rapidVerts = new Float32Array(0), this.cutVerts = new Float32Array(0), this.segmentGroups = Q.map((A) => ({
      ...
      stride: A.stride ?? 6
    })), this.bounds = At(this.segmentGroups);
    const t = (B == null ? void 0 : B.minZ) !== void 0 && (B == null ? void 0 : B.maxZ) !== void 0;
    if (t && !this.bounds.empty && (this.bounds.minZ = B.minZ, this.bounds.maxZ = B.maxZ), this.hasZInfo = t || this.segmentGroups.every((A) => A.stride === 6), !this.bounds.empty) {
      this.centerX = (this.bounds.minX + this.bounds.maxX) / 2, this.centerY = (this.bounds.minY + this.bounds.maxY) / 2, this.centerZ = (this.bounds.minZ + this.bounds.maxZ) / 2;
      const A = Math.hypot(
        this.bounds.maxX - this.bounds.minX,
        this.bounds.maxY - this.bounds.minY,
  ...
  rebuildToolpaths(Q = !1) {
    const B = Q ? mi : L, t = String(this.options.strokeWidth), A = this.simplifyTolerance();
    for (; this.pathEls.length < this.segmentGroups.length; ) {
      const P = Qt();
      this.pathLayer.appendChild(P), this.pathEls.push(P);
    }
    for (; this.pathEls.length > this.segmentGroups.length; )
      this.pathLayer.removeChild(this.pathEls.pop());
    for (let P = 0; P < this.segmentGroups.length; P++) {
      const { color: e, opacity: i, verts: s, stride: v } = this.segmentGroups[P], c = this.pathEls[P];
      c.setAttribute("stroke", e), c.setAttribute("stroke-opacity", String(i)), c.setAttribute("stroke-width", t), c.setAttribute("stroke-linecap", "round"), c.setAttribute("stroke-linejoin", "round"), c.setAttribute(
        "d",
        v === 4 ? xi(s, B, A) : yi(s, this.project, B, A)
      );
    }
  }
  ...
  fitView() {
    ...
    const { minX: Q, maxX: B, minY: t, maxY: A, minZ: P, maxZ: e } = this.bounds, i = this.project, s = i(Q, t, P), v = i(B, t, P), c = i(B, A, P), n = i(Q, A, P), T = i(Q, t, e), z = i(B, t, e), w = i(B, A, e), o = i(Q, A, e), a = Math.hypot(v.x - s.x, v.y - s.y), E = Math.hypot(n.x - s.x, n.y - s.y), l = Math.hypot(T.x - s.x, T.y - s.y), U = Math.max(a, E, l) * 0.07, D = U * 0.5, k = tt(LQ(s, v), LQ(c, n), D), O = [s, v, c, n, T, z, w, o], u = Math.min(...O.map((x) => x.y)), N = Math.max(...O.map((x) => x.y)), d = Math.min(...O.map((x) => x.x)), M = Math.max(...O.map((x) => x.x)), b = { x: M + D, y: (u + N) / 2 }, G = { x: b.x + U * 5, y: b.y }, C = { x: (d + M) / 2, y: u - U }, j = [s, v, c, n, T, z, w, o, k, b, G, C];
  ...
  applyViewBox() {
    const { x: Q, y: B, w: t, h: A } = this.viewBox;
    this.svg.setAttribute("viewBox", `${L(Q)} ${L(B)} ${L(t)} ${L(A)}`);
  }
  applyOptions() {
    const { boundingBoxColor: Q, strokeWidth: B, originColor: t } = this.options;
    ...
    this.originMarker.setAttribute("fill", t), this.originMarker.setAttribute("stroke", "#000000"), this.crosshairEl.setAttribute("stroke", this.options.crosshairColor);
  }
  renderBbox() {
    if (this.bounds.empty) {
    ...
    const { minX: Q, maxX: B, minY: t, maxY: A, minZ: P, maxZ: e } = this.bounds, i = this.project;
    if (this.options.projectionMode === "top") {
      const C = i(Q, t, 0), j = i(B, t, 0), F = i(B, A, 0), H = i(Q, A, 0);
      ...
      const sQ = Math.min(C.y, j.y, F.y, H.y), W = Math.max(C.y, j.y, F.y, H.y), x = Math.max(C.x, j.x, F.x, H.x), BQ = Math.min(C.x, j.x, F.x, H.x), vQ = { x: x + QQ, y: (sQ + W) / 2 };
      if (ZQ(this.bboxLabelY, vQ, vQ, R, `Y: ${WQ(A - t)}`), this.hasZInfo) {
        const rB = { x: (BQ + x) / 2, y: sQ - R };
        ZQ(this.bboxLabelZ, rB, rB, R, `Z: ${WQ(e - P)}`);
      } else
        this.bboxLabelZ.setAttribute("visibility", "hidden");
      return;
    }
    const s = i(Q, t, P), v = i(B, t, P), c = i(B, A, P), n = i(Q, A, P), T = i(Q, t, e), z = i(B, t, e), w = i(B, A, e), o = i(Q, A, e), a = [
    ...
    this.bboxPath.setAttribute("d", a);
    const E = Math.hypot(v.x - s.x, v.y - s.y), l = Math.hypot(n.x - s.x, n.y - s.y), U = Math.hypot(T.x - s.x, T.y - s.y), D = Math.max(E, l, U) * 0.07, k = D * 0.5;
    ZQ(this.bboxLabelX, LQ(s, v), tt(LQ(s, v), LQ(c, n), k), D, `X: ${WQ(B - Q)}`);
    const O = [s, v, c, n, T, z, w, o], u = Math.min(...O.map((C) => C.y)), N = Math.max(...O.map((C) => C.y)), d = Math.min(...O.map((C) => C.x)), M = Math.max(...O.map((C) => C.x)), b = { x: M + k, y: (u + N) / 2 };
    ZQ(this.bboxLabelY, b, b, D, `Y: ${WQ(A - t)}`);
    const G = { x: (d + M) / 2, y: u - D };
    ZQ(this.bboxLabelZ, G, G, D, `Z: ${WQ(e - P)}`);
  }
  renderOriginMarker() {
  ...
  renderCrosshairMarker() {
    ...
    const { x: Q, y: B, z: t } = this.crosshairPos, A = this.project(Q, B, t), P = Math.min(this.viewBox.w, this.viewBox.h) * 0.025, e = P * 0.25, i = A.x, s = A.y, v = [
  ...
  svgPoint(Q, B) {
    const t = this.svg.getBoundingClientRect(), { x: A, y: P, w: e, h: i } = this.viewBox;
    return {
      x: A + (Q - t.left) / t.width * e,
      y: P + (B - t.top) / t.height * i
    };
  }
}
function Qt() {
...
function ZQ(r, Q, B, t, A) {
  r.setAttribute("x", L(B.x)), r.setAttribute("y", L(B.y)), r.setAttribute("font-size", L(t)), r.textContent = A, r.setAttribute("visibility", "visible");
}
function LQ(r, Q) {
  return { x: (r.x + Q.x) / 2, y: (r.y + Q.y) / 2 };
}
function tt(r, Q, B) {
  const t = r.x - Q.x, A = r.y - Q.y, P = Math.hypot(t, A) || 1;
  return { x: r.x + t / P * B, y: r.y + A / P * B };
}
function L(r) {
  return r.toFixed(2);
}
...
function xi(r, Q, B = 0) {
  ...
  const t = [], A = 1e-6, P = B * B;
  let e = NaN, i = NaN, s = NaN, v = NaN, c = NaN, n = NaN, T = !1;
  for (let z = 0; z + 3 < r.length; z += 4) {
    const w = r[z], o = -r[z + 1], a = r[z + 2], E = -r[z + 3];
    if (Math.abs(w - e) < A && Math.abs(o - i) < A) {
      const l = a - s, U = E - v;
      P > 0 && l * l + U * U < P ? (c = a, n = E, T = !0) : (t.push(`L${Q(a)} ${Q(E)}`), s = a, v = E, T = !1);
    } else
      T && (t.push(`L${Q(c)} ${Q(n)}`), T = !1), t.push(`M${Q(w)} ${Q(o)}L${Q(a)} ${Q(E)}`), s = a, v = E;
    e = a, i = E;
  }
  return T && t.push(`L${Q(c)} ${Q(n)}`), t.join("");
}
function yi(r, Q, B, t = 0) {
  if (r.length === 0) return "";
  const A = [], P = 1e-6, e = t * t;
  let i = NaN, s = NaN, v = NaN, c = NaN, n = NaN, T = NaN, z = !1;
  for (let w = 0; w + 5 < r.length; w += 6) {
    const o = Q(r[w], r[w + 1], r[w + 2]), a = Q(r[w + 3], r[w + 4], r[w + 5]);
    if (Math.abs(o.x - i) < P && Math.abs(o.y - s) < P) {
      const E = a.x - v, l = a.y - c;
      e > 0 && E * E + l * l < e ? (n = a.x, T = a.y, z = !0) : (A.push(`L${B(a.x)} ${B(a.y)}`), v = a.x, c = a.y, z = !1);
    } else
      z && (A.push(`L${B(n)} ${B(T)}`), z = !1), A.push(`M${B(o.x)} ${B(o.y)}L${B(a.x)} ${B(a.y)}`), v = a.x, c = a.y;
    i = a.x, s = a.y;
  }
  return z && A.push(`L${B(n)} ${B(T)}`), A.join("");
}
function At(r) {
  let Q = 1 / 0, B = 1 / 0, t = 1 / 0, A = -1 / 0, P = -1 / 0, e = -1 / 0, i = !0, s = !1;
  for (const { verts: v, stride: c } of r)
    if (c === 4)
      for (let n = 0; n + 3 < v.length; n += 4) {
        const T = v[n], z = v[n + 1], w = v[n + 2], o = v[n + 3];
        T < Q && (Q = T), z < B && (B = z), T > A && (A = T), z > P && (P = z), w < Q && (Q = w), o < B && (B = o), w > A && (A = w), o > P && (P = o), i = !1;
      }
    else
      for (let n = 0; n + 5 < v.length; n += 6) {
        const T = v[n], z = v[n + 1], w = v[n + 2], o = v[n + 3], a = v[n + 4], E = v[n + 5];
        T < Q && (Q = T), z < B && (B = z), w < t && (t = w), T > A && (A = T), z > P && (P = z), w > e && (e = w), o < Q && (Q = o), a < B && (B = a), E < t && (t = E), o > A && (A = o), a > P && (P = a), E > e && (e = E), i = !1, s = !0;
      }
  return !i && !s && (t = 0, e = 0), { minX: Q, minY: B, maxX: A, maxY: P, minZ: t, maxZ: e, empty: i };
}
export {
  Ji as G,
  xP as V,
