...
import { OrbitControls as lP } from "three/examples/jsm/controls/OrbitControls.js";
import { GCodeVirtualizer as AA, buildWorkerToolpathStreams as UP, buildToolpathGeometryFromLinesBatched as DP, buildMovementVerticesFromLines as uP, buildWorkerSegmentGroups as CP } from "./gviewer.js";
import { STLLoader as dP } from "three/examples/jsm/loaders/STLLoader.js";
function NP(r) {
  const Q = [];
  let B = null;
...
async function fP(r, Q, B, A) {
  ...
  if (r.length === 0) {
    const O = {
      ...
      zTop: 1,
      zBot: 0
    }, G = new Float32Array(t * t).fill(O.zTop), f = /* @__PURE__ */ new Map();
    return f.set(0, new Float32Array(G)), { moves: [], checkpoints: f, slabBounds: O, resolution: t, toolRadius: Q };
  }
  let c = 1 / 0, n = -1 / 0, T = 1 / 0, z = -1 / 0, w = 1 / 0, o = -1 / 0;
  const E = new AA({
    onLinearMove(O) {
      if (O.modals.motion === "G0") return;
      const G = O.transformedStart ?? O.start, f = O.transformedEnd ?? O.end;
      for (const H of [G, f])
        H.X < c && (c = H.X), H.X > n && (n = H.X), H.Y < T && (T = H.Y), H.Y > z && (z = H.Y), H.Z < w && (w = H.Z), H.Z > o && (o = H.Z);
    },
    onArcMove(O) {
      const G = O.transformedStart ?? O.start, f = O.transformedEnd ?? O.end, H = O.transformedMax ?? O.max;
  ...
  const C = new AA({
    onLinearMove(O) {
      const G = O.transformedStart ?? O.start, f = O.transformedEnd ?? O.end, H = {
        lineIndex: d,
        x0: G.X,
        y0: G.Y,
        z0: G.Z,
      ...
      N.push(H), PA(H, D, U, Q, t);
    },
    onArcMove(O) {
      const G = O.transformedStart ?? O.start, f = O.transformedEnd ?? O.end, H = O.transformedCenter ?? O.center, F = bP(G, f, H, O.plane, O.motion, P);
      for (const J of F) {
        const _ = { lineIndex: d, ...J };
        N.push(_), PA(_, D, U, Q, t);
      }
    }
  });
  for (let O = 0; O < r.length && !s(); O++) {
    d = O;
    const G = r[O];
    G !== void 0 && C.processLine(G), O - u >= e && (l.set(O, new Float32Array(D)), u = O), O % i === 0 && O > 0 && (v == null || v(O, r.length), await new Promise((f) => setTimeout(f, 0)));
  }
  return v == null || v(r.length, r.length), { moves: N, checkpoints: l, slabBounds: U, resolution: t, toolRadius: Q };
}
function PA(r, Q, B, A, t) {
  ...
  for (let z = 0; z <= v; z++) {
    ...
    for (let D = -c; D <= c; D++) {
      const l = h + D;
      if (l < 0 || l >= t) continue;
      const N = B.xMin + (l + 0.5) * P, u = (N - o) * (N - o);
      if (!(u > T))
        for (let d = -n; d <= n; d++) {
          const C = U + d;
          if (C < 0 || C >= t) continue;
...
function bP(r, Q, B, A, t, P) {
  ...
  let o = { ...r };
  for (let E = 1; E <= e; E++) {
    const k = E / e, h = n + z * k, U = k, D = B[i] + c * Math.cos(h), l = B[s] + c * Math.sin(h), N = r[v] + (Q[v] - r[v]) * U, u = { X: 0, Y: 0, Z: 0 };
    u[i] = D, u[s] = l, u[v] = N, w.push({
      x0: o.X,
      y0: o.Y,
      z0: o.Z,
      x1: u.X,
...
function pP(r, Q) {
  ...
  l.rotateX(Math.PI / 2);
  const N = new a.Mesh(l, n);
  N.position.set(e, r.yMin, s), B.add(N);
  const u = new a.PlaneGeometry(P, t);
  u.rotateY(-Math.PI / 2);
  const d = new a.Mesh(u, n);
  d.position.set(r.xMax, i, s), B.add(d);
...
function RP(r, Q, B, A) {
  const t = new a.Group();
  if (t.add(VA(r, Q, B.render.theme.colors.machineBed)), A) {
    const P = B.render.theme.colors.machineBedKeepout;
    t.add(VA(A.min, A.max, P)), t.add(JP(A.min, A.max, P));
  }
  return t;
}
function XP(r) {
...
function qP(r) {
  const { minX: Q, maxX: B, minY: A, maxY: t } = kA(r), P = r.units === "mm" ? 10 : kt, e = [], i = [], s = (w, o, E, k, h) => {
    h.push(w, o, 0, E, k, 0);
  ...
  const z = new a.Group();
  return i.length > 0 && z.add(RA(i, r.theme.colors.grid.minor, 0.25)), e.length > 0 && z.add(RA(e, r.theme.colors.grid.major, 0.4)), z;
}
function RA(r, Q, B) {
...
function $P(r) {
  const Q = new a.Group(), { minX: B, maxX: A, minY: t, maxY: P } = kA({
    sizeXMm: r.sizeXWorld,
    sizeYMm: r.sizeYWorld,
    bounds: r.bounds
  }), e = Math.max(1, r.depthWorld), i = Math.max(1, Math.min(Math.max(A - B, P - t) / 24, 30)), s = i * 0.6, v = (k) => new a.LineDashedMaterial({
    color: new a.Color(k),
    dashSize: i,
    gapSize: s,
    transparent: !0,
  ...
  const z = Math.max(2, i), w = LQ("X", r.theme.colors.axes.x, { size: 5 });
  w.position.set(A + z, 0, 0);
  const o = LQ("Y", r.theme.colors.axes.y, { size: 5 });
  o.position.set(0, P + z, 0);
  const E = LQ("Z", r.theme.colors.axes.z, { size: 5 });
...
function Be(r) {
  const Q = new a.Group(), { minX: B, maxX: A, minY: t, maxY: P } = kA(r), e = r.units === "mm" ? 10 : 1, i = e * 2, s = r.units === "mm" ? 1 : kt, v = 0.5, c = 4, n = 0.1, T = 5, z = r.theme.colors.axes.x, w = r.theme.colors.axes.y, o = { opacity: v, size: c };
  for (let E = e; ; E += i) {
    const k = E * s;
    if (k > A + 1e-6)
      break;
    const h = LQ(String(E), z, o);
    h.position.set(k, T, n), Q.add(h);
  }
  for (let E = e; ; E += i) {
    const k = E * s;
    if (-k < B - 1e-6)
  ...
  for (let E = e; ; E += i) {
    ...
    if (k > P + 1e-6)
      break;
    const h = LQ(String(E), w, o);
    h.position.set(-T, k, n), Q.add(h);
  }
  for (let E = e; ; E += i) {
    const k = E * s;
    if (-k < t - 1e-6)
...
var cQ = {
  ...
  overwrite: !1,
  delay: 0
}, lA, Z, x, wQ = 1e8, m = 1 / wQ, eA = Math.PI * 2, Ae = eA / 4, te = 0, Ut = Math.sqrt, Pe = Math.cos, ee = Math.sin, K = function(Q) {
...
}, q = Array.isArray, ie = /random\([^)]+\)/g, re = /,\s*/g, XA = /(?:-?\.?\d|\.)+/gi, ut = /[-+=.]*\d+[.e\-+]*\d*[e\-+]*\d*/g, KQ = /[-+=.]*\d+[.e-]*\d*[a-z%]*/g, SB = /[-+=.]*\d+\.?\d*(?:e-|e\+)?\d*/gi, Ct = /[+-]=-?[.\d]+/, se = /[^,'"\[\]\s]+/gi, ve = /^[+\-=e\s\d]*\d+[.\d]*([a-z]*|%)\s*$/i, I, lQ, iA, uA, nQ = {}, MB = {}, dt, Nt = function(Q) {
  return (MB = $Q(Q, nQ)) && eQ;
}, CA = function(Q, B) {
  ...
  suppressEvents: !0
}, dA = {}, FQ = [], rA = {}, Lt, iQ = {}, YB = {}, KA = 30, NB = [], NA = "", OA = function(Q) {
  var B = Q[0], A, t;
  if (uQ(B) || V(B) || (Q = [Q]), !(A = (B._gsap || {}).harness)) {
    for (t = NB.length; t-- && !NB[t].targetTest(B); )
      ;
    A = NB[t];
  }
  for (t = Q.length; t--; )
  ...
  return Q._gsap || OA(EQ(Q))[0]._gsap;
}, Mt = function(Q, B, A) {
  return (A = Q[B]) && V(A) ? Q[B]() : UA(A) && Q.getAttribute && Q.getAttribute(B) || A;
}, AQ = function(Q, B) {
  ...
  var Q = FQ.length, B = FQ.slice(0), A, t;
  for (rA = {}, FQ.length = 0, A = 0; A < Q; A++)
    t = B[A], t && t._lazy && (t.render(t._lazy[0], t._lazy[1], !0)._lazy = 0);
}, LA = function(Q) {
  return !!(Q._initted || Q._startAt || Q.add);
}, Ht = function(Q, B, A, t) {
  FQ.length && !Z && HB(), Q.render(B, A, !!(Z && B < 0 && LA(Q))), FQ.length && !Z && HB();
}, jt = function(Q) {
  var B = parseFloat(Q);
  return (B || B === 0) && (Q + "").match(se).length < 2 ? B : K(Q) ? Q.trim() : Q;
}, Ft = function(Q) {
  return Q;
}, oQ = function(Q, B) {
  for (var A in B)
  ...
  var B = Q.parent || I, A = Q.keyframes ? Te(q(Q.keyframes)) : oQ;
  if (BQ(Q.inherit))
    for (; B; )
  ...
  return Q._startAt && (Z ? Q._startAt.revert(dB) : Q.vars.immediateRender && !Q.vars.autoRevert || Q._startAt.render(B, !0, t));
}, Ee = function r(Q) {
  ...
  var A;
  if ((B._time || !B._dur && B._initted || B._start < Q._time && (B._dur || !B.add)) && (A = FB(Q.rawTime(), B), (!B._dur || UB(0, B.totalDuration(), A) - B._tTime > m) && B.render(A, !0)), YQ(Q, B)._dp && Q._initted && Q._time >= Q._dur && Q._ts) {
  ...
  return B.parent && fQ(B), B._start = y((NQ(A) ? A : A || Q !== I ? zQ(Q, A, B) : Q._time) + B._delay), B._end = y(B._start + (B.totalDuration() / Math.abs(B.timeScale()) || 0)), Gt(Q, B, "_first", "_last", Q._sort ? "_start" : 0), vA(B) || (Q._recent = B), t || ft(Q, B), Q._ts < 0 && mB(Q, Q._tTime), Q;
}, bt = function(Q, B) {
  return (nQ.ScrollTrigger || CA("scrollTrigger", B)) && nQ.ScrollTrigger.create(B, Q);
}, gt = function(Q, B, A, t, P) {
  if (HA(Q, B, P), !Q._initted)
    return 1;
  if (!A && Q._pt && !Z && (Q._dur && Q.vars.lazy !== !1 || !Q._dur && Q.vars.lazy) && Lt !== rQ.frame)
    return FQ.push(Q), Q._lazy = [P, t], 1;
}, ae = function r(Q) {
  var B = Q.parent;
  ...
  var P = Q.ratio, e = B < 0 || !B && (!Q._start && ae(Q) && !(!Q._initted && vA(Q)) || (Q._ts < 0 || Q._dp._ts < 0) && !vA(Q)) ? 0 : 1, i = Q._rDelay, s = 0, v, c, n;
  if (i && Q._repeat && (s = UB(0, Q._tDur, B), c = QB(s, i), Q._yoyo && c & 1 && (e = 1 - e), c !== QB(Q._tTime, i) && (P = 1 - e, Q.vars.repeatRefresh && Q._initted && Q.invalidate())), e !== P || Z || t || Q._zTime === m || !B && Q._zTime) {
    ...
    for (n = Q._zTime, Q._zTime = B || (A ? m : 0), A || (A = B && !n), Q.ratio = e, Q._from && (e = 1 - e), Q._time = 0, Q._tTime = s, v = Q._pt; v; )
      v.r(e, v.d), v = v._next;
    B < 0 && sA(Q, B, A, !0), Q._onUpdate && !A && sQ(Q, "onUpdate"), s && Q._repeat && !A && Q.parent && sQ(Q, "onRepeat"), (B >= Q._tDur || B < 0) && Q.ratio === e && (e && fQ(Q, 1), !A && !Z && (sQ(Q, e ? "onComplete" : "onReverseComplete", !0), Q._prom && Q._prom()));
  ...
  return i && !t && (Q._time *= e / Q._dur), Q._dur = e, Q._tDur = P ? P < 0 ? 1e10 : y(e * (P + 1) + Q._rDelay * P) : e, i > 0 && !t && mB(Q, Q._tTime = Q._tDur * i), Q.parent && pB(Q), A || YQ(Q.parent, Q), Q;
}, qA = function(Q) {
  ...
  var t = Q.labels, P = Q._recent || le, e = Q.duration() >= wQ ? P.endTime(!1) : Q._dur, i, s, v;
  ...
  return new X(B[0], e, B[P + 1]);
}, pQ = function(Q, B) {
  return Q || Q === 0 ? B(Q) : B;
}, UB = function(Q, B, A) {
  return A < Q ? Q : A > B ? B : A;
}, W = function(Q, B) {
  return !K(Q) || !(B = ve.exec(Q)) ? "" : B[1];
}, Ue = function(Q, B, A) {
  return pQ(A, function(t) {
    return UB(Q, B, t);
  });
}, cA = [].slice, pt = function(Q, B) {
  return Q && uQ(Q) && "length" in Q && (!B && !Q.length || Q.length - 1 in Q && uQ(Q[0])) && !Q.nodeType && Q !== lQ;
}, De = function(Q, B, A) {
  return A === void 0 && (A = []), Q.forEach(function(t) {
    var P;
    return K(t) && !B || pt(t, 1) ? (P = A).push.apply(P, EQ(t)) : A.push(t);
  }) || A;
}, EQ = function(Q, B, A) {
  return x && !B && x.selector ? x.selector(Q) : K(Q) && !A && (iA || !AB()) ? cA.call((B || uA).querySelectorAll(Q), 0) : q(Q) ? De(Q, A) : pt(Q) ? cA.call(Q, 0) : Q ? [Q] : [];
}, nA = function(Q) {
  return Q = EQ(Q)[0] || zB("Invalid scope") || {}, function(B) {
    var A = Q.current || Q.nativeElement || Q;
    return EQ(B, A.querySelectorAll ? A : A === Q ? zB("Invalid scope") || uA.createElement("div") : Q);
  };
}, mt = function(Q) {
  ...
  return K(t) ? c = n = {
    ...
    var o = (w || B).length, E = e[o], k, h, U, D, l, N, u, d, C;
    if (!E) {
      if (C = B.grid === "auto" ? 0 : (B.grid || [1, wQ])[1], !C) {
        for (u = -wQ; u < (u = w[C++].getBoundingClientRect().left) && C < o; )
          ;
        C < o && C--;
      }
      for (E = e[o] = [], k = s ? Math.min(C, o) * c - 0.5 : t % C, h = C === wQ ? 0 : s ? o * n / C - 0.5 : t / C | 0, u = 0, d = wQ, N = 0; N < o; N++)
        U = N % C - k, D = h - (N / C | 0), E[N] = l = v ? Math.abs(v === "y" ? D : U) : Ut(U * U + D * D), l > u && (u = l), l < d && (d = l);
      t === "random" && mt(E), E.max = u - d, E.min = d, E.v = o = (parseFloat(B.amount) || parseFloat(B.each) * (C > o ? o - 1 : v ? v === "y" ? o / C : C : Math.max(C, o / C)) || 0) * (t === "edges" ? -1 : 1), E.b = o < 0 ? P - o : P, E.u = W(B.amount || B.each) || 0, A = A && o < 0 ? be(A) : A;
    }
  ...
  var A = q(Q), t, P;
  return !A && uQ(Q) && (t = A = Q.radius || wQ, Q.values ? (Q = EQ(Q.values), (P = !NQ(Q[0])) && (t *= t)) : Q = oA(Q.increment)), pQ(B, A ? V(Q) ? function(e) {
    return P = Q(e), Math.abs(P - e) <= t ? P : e;
  } : function(e) {
    for (var i = parseFloat(P ? e.x : e), s = parseFloat(P ? e.y : 0), v = wQ, c = 0, n = Q.length, T, z; n--; )
      P ? (T = Q[n].x - i, z = Q[n].y - s, T = T * T + z * z) : T = Math.abs(Q[n] - i), T < v && (v = T, c = n);
    return c = !t || v <= t ? Q[c] : e, P || c === e || NQ(e) ? c : c + W(e);
  } : oA(Q));
}, It = function(Q, B, A, t) {
  return pQ(q(Q) ? !B : A === !0 ? !!(A = 0) : !t, function() {
  ...
  var t = B - Q;
  return q(Q) ? St(Q, r(0, Q.length), B) : pQ(A, function(P) {
    return (t + (P - Q) % t) % t + Q;
  });
}, Oe = function r(Q, B, A) {
  var t = B - Q, P = t * 2;
  return q(Q) ? St(Q, r(0, Q.length - 1), B) : pQ(A, function(e) {
  ...
  if (P)
    return s = t[B + "Params"], v = t.callbackScope || Q, A && FQ.length && HB(), i && (x = i), c = s ? P.apply(v, s) : P.call(v), x = e, c;
}, sB = function(Q) {
  return fQ(Q), Q.scrollTrigger && Q.scrollTrigger.kill(!!Z), Q.progress() < 1 && sQ(Q, "onInterrupt"), Q;
}, ZQ, Vt = [], Jt = function(Q) {
  if (Q)
    if (Q = !Q.name && Q.default || Q, DA() || Q.headless) {
      var B = Q.name, A = V(Q), t = B && !A && Q.init ? function() {
        this._props = [];
      } : Q, P = {
        init: wB,
        render: GA,
      ...
      if (AB(), Q !== t) {
        if (iQ[B])
          return;
        oQ(t, oQ(jB(Q, P), e)), $Q(t.prototype, $Q(P, jB(Q, e))), iQ[t.prop = B] = t, Q.targetTest && (NB.push(t), dA[B] = 1), B = (B === "css" ? "CSS" : B.charAt(0).toUpperCase() + B.substr(1)) + "Plugin";
      }
      Ot(B, t), Q.register && Q.register(eQ, t, tQ);
    } else
      Vt.push(Q);
}, p = 255, vB = {
  aqua: [0, p, p],
  ...
  if (GQ.lastIndex = 0, GQ.test(B))
    return A = Me.test(B), Q[1] = $A(Q[1], A), Q[0] = $A(Q[0], A, Xt(Q[1])), !0;
}, aB, rQ = function() {
  var r = Date.now, Q = 500, B = 33, A = r(), t = A, P = 1e3 / 240, e = P, i = [], s, v, c, n, T, z, w = function o(E) {
    var k = r() - t, h = E === !0, U, D, l, N;
    if ((k > Q || k < 0) && (A += k - B), t += k, l = t - A, U = l - e, (U > 0 || h) && (N = ++n.frame, T = l - n.time * 1e3, n.time = l = l / 1e3, e += U + (U >= P ? 4 : P - U), D = 1), h || (s = v(o)), D)
  ...
  return n = {
    ...
    wake: function() {
      dt && (!iA && DA() && (lQ = iA = window, uA = lQ.document || {}, nQ.gsap = eQ, (lQ.gsapVersions || (lQ.gsapVersions = [])).push(eQ.version), Nt(MB || lQ.GreenSockGlobals || !lQ.gsap && lQ || {}), Vt.forEach(Jt)), c = typeof requestAnimationFrame < "u" && requestAnimationFrame, s && n.sleep(), v = c || function(E) {
    ...
    add: function(E, k, h) {
      var U = k ? function(D, l, N, u) {
        E(D, l, N, u), n.remove(U);
      } : E;
  ...
  return P = eA / P, s.config = function(v, c) {
    return r(Q, v, c);
  }, s;
}, RB = function r(Q, B) {
  B === void 0 && (B = 1.70158);
...
var Wt = function(Q, B) {
  ...
  return Q.delay = function(A) {
    ...
    if (P && P.smoothChildTiming && this._ts) {
      for (mB(this, A), !P._dp || P.parent || ft(P, this); P && P.parent; )
    ...
    return arguments.length ? this.totalTime(Math.min(this.totalDuration(), A + WA(this)) % (this._dur + this._rDelay) || (A ? this._dur : 0), t) : this._time;
    ...
    return arguments.length ? this.totalTime(this.duration() * (this._yoyo && !(this.iteration() & 1) ? 1 - A : A) + WA(this), t) : this.duration() ? Math.min(1, this._time / this._dur) : this.rawTime() > 0 ? 1 : 0;
    ...
    return this._rts = +A || 0, this._ts = this._ps || A === -m ? 0 : this._rts, this.totalTime(UB(-Math.abs(this._delay), this.totalDuration(), P), t !== !1), pB(this), we(this);
  }, Q.paused = function(A) {
    ...
    return arguments.length ? (this._repeat = A === 1 / 0 ? -2 : A, qA(this)) : this._repeat === -2 ? 1 / 0 : this._repeat;
  }, Q.repeatDelay = function(A) {
    if (arguments.length) {
      var t = this._time;
      return this._rDelay = A, qA(this), t ? this.time(t) : this;
    }
    return this._rDelay;
  }, Q.yoyo = function(A) {
    return arguments.length ? (this._yoyo = A, this) : this._yoyo;
  }, Q.seek = function(A, t) {
    return this.totalTime(zQ(this, A), BQ(t));
    ...
    return new Promise(function(e) {
      var i = V(A) ? A : Ft, s = function() {
        var c = t.then;
        t.then = null, P && P(), V(i) && (i = i(t)) && (i.then || i === t) && (t.then = c), e(i), t.then = c;
      };
...
var QQ = /* @__PURE__ */ function(r) {
  ...
  function Q(A, t) {
    var P;
    return A === void 0 && (A = {}), P = r.call(this, A) || this, P.labels = {}, P.smoothChildTiming = !!A.smoothChildTiming, P.autoRemoveChildren = !!A.autoRemoveChildren, P._sort = BQ(A.sortChildren), I && UQ(A.parent || I, CQ(P), t), A.reversed && P.reverse(), A.paused && P.paused(!0), A.scrollTrigger && bt(CQ(P), A.scrollTrigger), P;
  }
  var B = Q.prototype;
  return B.to = function(t, P, e) {
    ...
    return P.duration = 0, P.parent = this, cB(P).repeatDelay || (P.repeat = 0), P.immediateRender = !!P.immediateRender, new X(t, P, zQ(this, e), 1), this;
  }, B.call = function(t, P, e) {
    return UQ(this, X.delayedCall(0, t, P), e);
  }, B.staggerTo = function(t, P, e, i, s, v, c) {
    return e.duration = P, e.stagger = e.stagger || i, e.onComplete = v, e.onCompleteParams = c, e.parent = this, new X(t, e, zQ(this, s)), this;
    ...
    var i = this._time, s = this._dirty ? this.totalDuration() : this._tDur, v = this._dur, c = t <= 0 ? 0 : y(t), n = this._zTime < 0 != t < 0 && (this._initted || !v), T, z, w, o, E, k, h, U, D, l, N, u;
    if (this !== I && c > s && t >= 0 && (c = s), c !== this._tTime || e || n) {
      if (i !== this._time && v && (c += this._time - i, t += this._time - i), T = c, D = this._start, U = this._ts, k = !U, n && (v || (i = this._zTime), (t || !P) && (this._zTime = t)), this._repeat) {
        if (N = this._yoyo, E = v + this._rDelay, this._repeat < -1 && t < 0)
          return this.totalTime(E * 100 + t, P, e);
        if (T = y(c % E), c === s ? (o = this._repeat, T = v) : (l = y(c / E), o = ~~l, o && o === l && (T = v, o--), T > v && (T = v)), l = QB(this._tTime, E), !i && this._tTime && l !== o && this._tTime - l * E - this._dur <= 0 && (l = o), N && o & 1 && (T = v - T, u = 1), o !== l && !this._lock) {
          var d = N && l & 1, C = d === (N && o & 1);
          if (o < l && (d = !d), i = d ? 0 : c % v ? v : c, this._lock = 1, this.render(i || (u ? 0 : y(o * E)), P, !v)._lock = 0, this._tTime = c, !P && this.parent && sQ(this, "onRepeat"), this.vars.repeatRefresh && !u && (this.invalidate()._lock = 1, l = o), i && i !== this._time || k !== !this._ts || this.vars.onRepeat && !this.parent && !this._act)
            return this;
          if (v = this._dur, s = this._tDur, C && (this._lock = 2, i = d ? v : -1e-4, this.render(i, !0), this.vars.repeatRefresh && !u && this.invalidate()), this._lock = 0, !this._ts && !k)
            return this;
        }
      }
      if (this._hasPause && !this._forcing && this._lock < 2 && (h = ke(this, y(i), y(T)), h && (c -= T - (T = h._start))), this._tTime = c, this._time = T, this._act = !!U, this._initted || (this._onUpdate = this.vars.onUpdate, this._initted = 1, this._zTime = t, i = 0), !i && c && v && !P && !l && (sQ(this, "onStart"), this._tTime !== c))
        return this;
      if (T >= i && t >= 0)
        for (z = this._first; z; ) {
      ...
      this._onUpdate && !P && sQ(this, "onUpdate", !0), (c === s && this._tTime >= this.totalDuration() || !c && i) && (D === this._start || Math.abs(U) !== Math.abs(this._ts)) && (this._lock || ((t || !v) && (c === s && this._ts > 0 || !c && this._ts < 0) && fQ(this, 1), !P && !(t < 0 && !i) && (c || i || !s) && (sQ(this, c === s && t >= 0 ? "onComplete" : "onReverseComplete", !0), this._prom && !(c < s && this.timeScale() > 0) && this._prom())));
    }
    return this;
  }, B.add = function(t, P) {
    var e = this;
    if (NQ(P) || (P = zQ(this, P, t)), !(t instanceof hB)) {
      if (q(t))
    ...
    t === void 0 && (t = !0), P === void 0 && (P = !0), e === void 0 && (e = !0), i === void 0 && (i = -wQ);
    for (var s = [], v = this._first; v; )
      v._start >= i && (v instanceof X ? P && s.push(v) : (e && s.push(v), t && s.push.apply(s, v.getChildren(!0, P, e)))), v = v._next;
    return s;
    ...
    return K(t) ? this.removeLabel(t) : V(t) ? this.killTweensOf(t) : (t.parent === this && gB(this, t), t === this._recent && (this._recent = this._last), YQ(this));
    ...
    return this.labels[t] = zQ(this, P), this;
  }, B.removeLabel = function(t) {
    return delete this.labels[t], this;
  }, B.addPause = function(t, P, e) {
    var i = X.delayedCall(0, P || wB, e);
    return i.data = "isPause", this._hasPause = 1, UQ(this, i, zQ(this, t));
    ...
    for (var e = [], i = EQ(t), s = this._first, v = NQ(P), c; s; )
      s instanceof X ? oe(s._targets, i) && (v ? (!MQ || s._initted && s._ts) && s.globalTime(0) <= P && s.globalTime(s.totalDuration()) > P : !P || s.isActive()) && e.push(s) : (c = s.getTweensOf(i, P)).length && e.push.apply(e, c), s = s._next;
    return e;
  }, B.tweenTo = function(t, P) {
    P = P || {};
    var e = this, i = zQ(e, t), s = P, v = s.startAt, c = s.onStart, n = s.onStartParams, T = s.immediateRender, z, w = X.to(e, oQ({
      ease: P.ease || "none",
      lazy: !1,
      immediateRender: !1,
      time: i,
    ...
    return t === void 0 && (t = this._time), _A(this, zQ(this, t));
  }, B.previousLabel = function(t) {
    return t === void 0 && (t = this._time), _A(this, zQ(this, t), 1);
    ...
    var P = 0, e = this, i = e._last, s = wQ, v, c, n;
    if (arguments.length)
      return e.timeScale((e._repeat < 0 ? e.duration() : e.totalDuration()) / (e.reversed() ? -t : t));
    if (e._dirty) {
      for (n = e.parent; i; )
        v = i._prev, i._dirty && i.totalDuration(), c = i._start, c > s && e._sort && i._ts && !e._lock ? (e._lock = 1, UQ(e, i, c - i._delay, 1)._lock = 0) : s = c, c < 0 && i._ts && (P -= c, (!n && !e._dp || n && n.smoothChildTiming) && (e._start += y(c / e._ts), e._time -= c, e._tTime -= c), e.shiftChildren(-c, !1, -1 / 0), s = 0), i._end > P && i._ts && (P = i._end), i = v;
      BB(e, e === I && e._time > P ? e._time : P, 1, 1), e._dirty = 0;
    }
    return e._tDur;
  }, Q.updateRoot = function(t) {
    if (I._ts && (Ht(I, FB(t, I)), Lt = rQ.frame), rQ.frame >= KA) {
      KA += cQ.autoSleep || 120;
      var P = I._first;
...
var ge = function(Q, B, A, t, P, e, i) {
  var s = new tQ(this._pt, Q, B, 0, 1, AP, null, P), v = 0, c = 0, n, T, z, w, o, E, k, h;
  ...
  return s.c = v < t.length ? t.substring(v, t.length) : "", s.fp = i, (Ct.test(t) || k) && (s.e = 0), this._pt = s, s;
}, MA = function(Q, B, A, t, P, e, i, s, v, c) {
  V(t) && (t = t(P || 0, Q, e));
  var n = Q[B], T = A !== "get" ? A : V(n) ? v ? Q[B.indexOf("set") || !V(Q["get" + B.substr(3)]) ? B : "get" + B.substr(3)](v) : Q[B]() : n, z = V(n) ? v ? Ie : QP : jA, w;
  if (K(t) && (~t.indexOf("random(") && (t = EB(t)), t.charAt(1) === "=" && (w = WQ(T, t) + (W(T) || 0), (w || w === 0) && (t = w))), !c || T !== t || TA)
    return !isNaN(T * t) && t !== "" ? (w = new tQ(this._pt, Q, B, +T || 0, t - (T || 0), typeof n == "boolean" ? Ye : BP, 0, z), v && (w.fp = v), i && w.modifier(i, this, Q), this._pt = w) : (!n && !(B in Q) && CA(B, t), ge.call(this, Q, B, T, t, z, s || cQ.stringFilter, v));
}, pe = function(Q, B, A, t, P) {
  if (V(Q) && (Q = oB(Q, P, B, A, t)), !uQ(Q) || Q.style && Q.nodeType || q(Q) || Dt(Q))
  ...
  var i, s, v, c;
  if (iQ[Q] && (i = new iQ[Q]()).init(P, i.rawVars ? B[Q] : pe(B[Q], t, P, e, A), A, t, e) !== !1 && (A._pt = s = new tQ(A._pt, P, Q, 0, 1, i.render, i, 0, i.priority), A !== ZQ))
    for (v = A._ptLookup[A._targets.indexOf(P)], c = i._props.length; c--; )
      v[i._props[c]] = s;
  return i;
}, MQ, TA, HA = function r(Q, B, A) {
  var t = Q.vars, P = t.ease, e = t.startAt, i = t.immediateRender, s = t.lazy, v = t.onUpdate, c = t.runBackwards, n = t.yoyoEase, T = t.keyframes, z = t.autoRevert, w = Q._dur, o = Q._startAt, E = Q._targets, k = Q.parent, h = k && k.data === "nested" ? k.vars.targets : E, U = Q._overwrite === "auto" && !lA, D = Q.timeline, l = t.easeReverse || n, N, u, d, C, M, b, j, O, G, f, H, F, J;
  if (D && (!T || !P) && (P = "none"), Q._ease = VQ(P, TB.ease), Q._rEase = l && (VQ(l) || Q._ease), Q._from = !D && !!t.runBackwards, Q._from && (Q.ratio = 1), !D || T && !t.stagger) {
    if (O = E[0] ? SQ(E[0]).harness : 0, F = O && t[O.prop], N = jB(t, dA), o && (o._zTime < 0 && o.progress(1), B < 0 && c && i && !z ? o.render(-1, !0) : o.revert(c && w ? dB : ce), o._lazy = 0), e) {
      if (fQ(Q._startAt = X.set(E, oQ({
        data: "isStart",
        overwrite: !1,
        parent: k,
        immediateRender: !0,
        ...
        stagger: 0
      }, e))), Q._startAt._dp = 0, Q._startAt._sat = Q, B < 0 && (Z || !i && !z) && Q._startAt.revert(dB), i && w && B <= 0 && A <= 0) {
        B && (Q._zTime = B);
        return;
      }
    } else if (c && w && !o) {
      ...
      }, N), F && (d[O.prop] = F), fQ(Q._startAt = X.set(E, d)), Q._startAt._dp = 0, Q._startAt._sat = Q, B < 0 && (Z ? Q._startAt.revert(dB) : Q._startAt.render(-1, !0)), Q._zTime = B, !i)
        r(Q._startAt, m, m);
      else if (!B)
        return;
    }
    for (Q._pt = Q._ptCache = 0, s = w && BQ(s) || s && !w, u = 0; u < E.length; u++) {
      if (M = E[u], j = M._gsap || OA(E)[u]._gsap, Q._ptLookup[u] = f = {}, rA[j.id] && FQ.length && HB(), H = h === E ? u : h.indexOf(M), O && (G = new O()).init(M, F || N, Q, H, h) !== !1 && (Q._pt = C = new tQ(Q._pt, M, G.name, 0, 1, G.render, G, 0, G.priority), G._props.forEach(function(_) {
        f[_] = C;
      }), G.priority && (b = 1)), !O || F)
        for (d in N)
          iQ[d] && (G = qt(d, N, Q, H, M, h)) ? G.priority && (b = 1) : f[d] = C = MA.call(Q, M, d, "get", N[d], H, h, 0, t.stringFilter);
      Q._op && Q._op[u] && Q.kill(M, Q._op[u]), U && Q._pt && (MQ = Q, I.killTweensOf(M, f, Q.globalTime(B)), J = !Q.parent, MQ = 0), Q._pt && s && (rA[j.id] = 1);
    }
    b && tP(Q), Q._onInit && Q._onInit(Q);
  }
  Q._onUpdate = v, Q._initted = (!Q._op || Q._pt) && !J, T && B <= 0 && D.render(wQ, !0, !0);
    ...
    for (v = Q._ptCache[B] = [], T = Q._ptLookup, z = Q._targets.length; z--; ) {
      if (c = T[z][B], c && c.d && c.d._pt)
        for (c = c.d._pt; c && c.p !== B && c.fp !== B; )
          c = c._next;
      if (!c)
        return TA = 1, Q.vars[B] = "+=0", HA(Q, i), TA = 0, s ? zB(B + " not eligible for reset. Try splitting into individual properties") : 1;
      v.push(c);
    }
  for (z = v.length; z--; )
    n = v[z], c = n._pt || n, c.s = (t || t === 0) && !P ? t : c.s + (t || 0) + e * c.c, c.c = A - c.s, n.e && (n.e = R(A) + W(n.e)), n.b && (n.b = c.s + W(n.b));
}, xe = function(Q, B) {
  ...
  return V(Q) ? Q.call(B, A, t, P) : K(Q) && ~Q.indexOf("random(") ? EB(Q) : Q;
}, _t = NA + "repeat,repeatDelay,yoyo,repeatRefresh,yoyoEase,easeReverse,autoRevert", $t = {};
AQ(_t + ",id,stagger,delay,duration,paused,scrollTrigger", function(r) {
  return $t[r] = 1;
});
var X = /* @__PURE__ */ function(r) {
  ...
  function Q(A, t, P, e) {
    ...
    var s = i.vars, v = s.duration, c = s.delay, n = s.immediateRender, T = s.stagger, z = s.overwrite, w = s.keyframes, o = s.defaults, E = s.scrollTrigger, k = t.parent || I, h = (q(A) || Dt(A) ? NQ(A[0]) : "length" in t) ? [A] : EQ(A), U, D, l, N, u, d, C, M;
    if (i._targets = h.length ? OA(h) : zB("GSAP target " + A + " not found. https://gsap.com", !cQ.nullTargetWarn) || [], i._ptLookup = [], i._overwrite = z, w || T || CB(v) || CB(c)) {
      t = i.vars;
      ...
      if (U = i.timeline = new QQ({
      ...
      }), U.kill(), U.parent = U._dp = CQ(i), U._start = 0, T || CB(v) || CB(c)) {
        if (N = h.length, C = T && xt(T), uQ(T))
          for (u in T)
            ~_t.indexOf(u) && (M || (M = {}), M[u] = T[u]);
        for (D = 0; D < N; D++)
          l = jB(t, $t), l.stagger = 0, b && (l.easeReverse = b), M && $Q(l, M), d = h[D], l.duration = +oB(v, CQ(i), D, d, h), l.delay = (+oB(c, CQ(i), D, d, h) || 0) - i._delay, !T && N === 1 && l.delay && (i._delay = c = l.delay, i._start += c, l.delay = 0), U.to(d, l, C ? C(D, d, h) : 0), U._ease = g.none;
        ...
        if (q(w))
          ...
          l = {};
          for (u in w)
            u === "ease" || u === "easeEach" || ye(u, w[u], l, w.easeEach);
          for (u in l)
            for (O = l[u].sort(function(H, F) {
              return H.t - F.t;
            }), j = 0, D = 0; D < O.length; D++)
              G = O[D], f = {
                ease: G.e,
                duration: (G.t - (D ? O[D - 1].t : 0)) / 100 * v
      ...
      i.timeline = 0;
    return z === !0 && !lA && (MQ = CQ(i), I.killTweensOf(h), MQ = 0), UQ(k, CQ(i), P), t.reversed && i.reverse(), t.paused && i.paused(!0), (n || !v && !w && i._start === y(k._time) && BQ(n) && Ee(CQ(i)) && k.data !== "nested") && (i._tTime = -m, i.render(Math.max(0, -c) || 0)), E && bt(CQ(i), E), i;
  }
  var B = Q.prototype;
  return B.render = function(t, P, e) {
    ...
    if (!v)
      ...
      for (z = this._pt; z; )
        z.r(h, z.d), z = z._next;
      U && U.render(t < 0 ? t : U._dur * U._ease(T / this._dur), P, e) || this._startAt && (this._zTime = t), this._onUpdate && !P && (c && sA(this, t, P, e), sQ(this, "onUpdate")), this._repeat && w !== E && this.vars.onRepeat && !P && this.parent && sQ(this, "onRepeat"), (n === this._tDur || !n) && this._tTime === n && (c && !this._onUpdate && sA(this, t, !0, !0), (t || !v) && (n === this._tDur && this._ts > 0 || !n && this._ts < 0) && fQ(this, 1), !P && !(c && !i) && (n || i || k) && (sQ(this, n === s ? "onComplete" : "onReverseComplete", !0), this._prom && !(n < s && this.timeScale() > 0) && this._prom()));
    }
    return this;
  }, B.targets = function() {
    ...
    return this._initted || HA(this, v), c = this._ease(v / this._dur), me(this, t, P, e, i, c, v, s) ? this.resetTo(t, P, e, i, 1) : (mB(this, 0), this.parent || Gt(this._dp, this, "_first", "_last", this._dp._sort ? "_start" : 0), this.render(0));
  }, B.kill = function(t, P) {
    ...
    if (this.timeline) {
      ...
      return this.timeline.killTweensOf(t, P, MQ && MQ.vars.overwrite !== !0)._first || sB(this), this.parent && e !== this.timeline.totalDuration() && BB(this, this._dur * this.timeline._tDur / e, 0, 1), this;
    }
    var i = this._targets, s = t ? EQ(t) : i, v = this._ptLookup, c = this._pt, n, T, z, w, o, E, k;
    if ((!P || P === "all") && ze(i, s))
      return P === "all" && (this._pt = 0), sB(this);
    for (n = this._op = this._op || [], P !== "all" && (K(P) && (o = {}, AQ(P, function(h) {
      return o[h] = 1;
    }), P = o), P = xe(i, P)), k = i.length; k--; )
      if (~s.indexOf(i[k])) {
...
AQ("staggerTo,staggerFrom,staggerFromTo", function(r) {
  X[r] = function() {
    var Q = new QQ(), B = cA.call(arguments, 0);
...
var jA = function(Q, B, A) {
  ...
  return V(Q[B]) ? QP : UA(Q[B]) && Q.setAttribute ? Se : jA;
}, BP = function(Q, B) {
  ...
  function r(B, A, t, P, e, i, s, v, c) {
    this.t = A, this.s = P, this.c = e, this.p = t, this.r = i || BP, this.d = s || this, this.set = v || jA, this.pr = c || 0, this._next = B, B && (B._prev = this);
  }
  var Q = r.prototype;
...
AQ(NA + "parent,duration,ease,delay,overwrite,runBackwards,startAt,yoyo,immediateRender,repeat,repeatDelay,data,paused,reversed,lazy,callbackScope,stringFilter,id,yoyoEase,stagger,inherit,repeatRefresh,keyframes,autoRevert,scrollTrigger,easeReverse", function(r) {
  return dA[r] = 1;
});
nQ.TweenMax = nQ.TweenLite = X;
...
cQ.stringFilter = Kt;
var JQ = [], OB = {}, Xe = [], Qt = 0, Ke = 0, XB = function(Q) {
  return (OB[Q] || Xe).map(function(B) {
    return B();
  });
}, zA = function() {
  var Q = Date.now(), B = [];
  Q - Qt > 2 && (XB("matchMediaInit"), JQ.forEach(function(A) {
    var t = A.queries, P = A.conditions, e, i, s, v;
    for (i in t)
      e = lQ.matchMedia(t[i]).matches, e && (s = 1), e !== P[i] && (P[i] = e, v = 1);
  ...
  function r(B, A) {
    this.selector = A && nA(A), this.data = [], this._r = [], this.isReverted = !1, this.id = Ke++, B && this.add(B);
  }
  var Q = r.prototype;
  return Q.add = function(A, t, P) {
    ...
    var e = this, i = function() {
      ...
      return v && v !== e && v.data.push(e), P && (e.selector = nA(P)), x = e, n = t.apply(e, arguments), V(n) && e._r.push(n), x = v, e.selector = c, e.isReverted = !1, n;
    };
    return e.last = i, A === V ? i(e, function(s) {
      return e.add(null, s);
    }) : A ? e[A] = i : i;
  }, Q.ignore = function(A) {
    var t = x;
    ...
    return this.data.forEach(function(t) {
      return t instanceof r ? A.push.apply(A, t.getTweens()) : t instanceof X && !(t.parent && t.parent.data === "nested") && A.push(t);
    }), A;
  }, Q.clear = function() {
    ...
    if (A ? function() {
      ...
      for (i.map(function(c) {
        ...
        v = P.data[s], v instanceof QQ ? v.data !== "nested" && (v.scrollTrigger && v.scrollTrigger.revert(), v.kill()) : !(v instanceof X) && v.revert && v.revert(A);
  ...
  return Q.add = function(A, t, P) {
    ...
    for (v in A)
      v === "all" ? c = 1 : (s = lQ.matchMedia(A[v]), s && (JQ.indexOf(e) < 0 && JQ.push(e), (i[v] = s.matches) && (c = 1), s.addListener ? s.addListener(zA) : s.addEventListener("change", zA)));
  ...
  defaults: function(Q) {
    return Q && Q.ease && (Q.ease = VQ(Q.ease, TB.ease)), ZA(TB, Q || {});
  },
  config: function(Q) {
    return ZA(cQ, Q || {});
  },
  registerEffect: function(Q) {
    ...
    (t || "").split(",").forEach(function(i) {
      ...
      return A(EQ(i), oQ(s || {}, P), v);
    }, e && (QQ.prototype[B] = function(i, s, v) {
      return this.add(YB[B](i, uQ(s) ? s : (v = s) && {}, this), v);
    });
  },
  ...
  exportRoot: function(Q, B) {
    ...
    for (A.smoothChildTiming = BQ(Q.smoothChildTiming), I.remove(A), A._dp = 0, A._time = A._tTime = I._time, t = I._first; t; )
      P = t._next, (B || !(!t._dur && t instanceof X && t.vars.onComplete === t._targets[0])) && UQ(A, t, t._start - t._delay), t = P;
    return UQ(I, A, 0), A;
  },
  context: function(Q, B) {
    return Q ? new PP(Q, B) : x;
  },
...
}, KB("roundProps", oA), KB("modifiers"), KB("snap", yt)) || GB;
X.version = QQ.version = eQ.version = "3.15.0";
dt = 1;
DA() && AB();
g.Power0;
g.Power1;
 ...
 * @author: Jack Doyle, jack@greensock.com
*/
var Bt, HQ, qQ, fA, IQ, At, bA, _e = function() {
  return typeof window < "u";
}, OQ = {}, yQ = 180 / Math.PI, _Q = Math.PI / 180, XQ = Math.atan2, tt = 1e8, gA = /([A-Z])/g, $e = /(left|right|width|margin|padding|x)/i, Qi = /[\s,\(]\S/, DQ = {
...
}, S = "transform", PQ = S + "Origin", ni = function r(Q, B) {
  ...
  if (Q in OQ && P) {
    if (this.tfm = this.tfm || {}, Q !== "transform")
      ...
      }) : this.tfm[Q] = e.x ? e[Q] : dQ(t, Q), Q === PQ && (this.tfm.zOrigin = e.zOrigin);
    else
  ...
  if (this.tfm) {
    for (e in this.tfm)
      t[e] = this.tfm[e];
    t.svg && (t.renderTransform(), B.setAttribute("data-svg-origin", this.svgo || "")), P = bA(), (!P || !P.isStart) && !A[S] && (rP(A), t.zOrigin && A[PQ] && (A[PQ] += " " + t.zOrigin + "px", t.zOrigin = 0, t.renderTransform()), t.uncache = 1);
  }
}, sP = function(Q, B) {
  var A = {
    target: Q,
  ...
  var A = HQ.createElementNS ? HQ.createElementNS((B || "http://www.w3.org/1999/xhtml").replace(/^https/, "http"), Q) : HQ.createElement(Q);
  return A && A.style ? A : HQ.createElement(Q);
}, vQ = function r(Q, B, A) {
  var t = getComputedStyle(Q);
  return t[B] || t.getPropertyValue(B.replace(gA, "-$1").toLowerCase()) || t.getPropertyValue(B) || !A && r(Q, tB(B) || B, 1) || "";
}, Pt = "O,Moz,ms,Ms,Webkit".split(","), tB = function(Q, B, A) {
  var t = B || IQ, P = t.style, e = 5;
  if (Q in P && !A)
    return Q;
  for (Q = Q.charAt(0).toUpperCase() + Q.substr(1); e-- && !(Pt[e] + Q in P); )
    ;
  return e < 0 ? null : (e === 3 ? "ms" : e >= 0 ? Pt[e] : "") + Q;
}, aA = function() {
  _e() && window.document && (Bt = window, HQ = Bt.document, qQ = HQ.documentElement, IQ = EA("div") || {
    style: {}
  }, EA("div"), S = tB(S), PQ = S + "Origin", IQ.style.cssText = "border-width:0;line-height:0;position:absolute;padding:0", vP = !!tB("perspective"), bA = eQ.core.reverting, fA = 1);
}, et = function(Q) {
  ...
  return B && (B.width || B.height) || A || (B = et(Q)), B && !B.width && !B.x && !B.y ? {
    x: +it(Q, ["x", "cx", "x1"]) || 0,
    y: +it(Q, ["y", "cy", "y1"]) || 0,
    width: 0,
    height: 0
...
}, jQ = function(Q, B, A, t, P, e) {
  var i = new tQ(Q._pt, B, A, 0, 1, e ? iP : eP);
  ...
  grid: 1,
  flex: 1
}, gQ = function r(Q, B, A, t) {
  var P = parseFloat(A) || 0, e = (A + "").trim().substr((P + "").length) || "px", i = IQ.style, s = $e.test(B), v = Q.tagName.toLowerCase() === "svg", c = (v ? "client" : "offset") + (s ? "Width" : "Height"), n = 100, T = t === "px", z = t === "%", w, o, E, k;
  if (t === e || !P || rt[t] || rt[e])
    return P;
  if (e !== "px" && !T && (P = r(Q, B, A, "px")), k = Q.getCTM && nP(Q), (z || e === "%") && (OQ[B] || ~B.indexOf("adius")))
    return w = k ? Q.getBBox()[s ? "width" : "height"] : Q[c], R(z ? P / w * n : P / 100 * w);
  if (i[s ? "width" : "height"] = n + (T ? e : t), o = t !== "rem" && ~B.indexOf("adius") || t === "em" && Q.appendChild && !v ? Q : Q.parentNode, k && (o = (Q.ownerSVGElement || {}).parentNode), (!o || o === HQ || !o.appendChild) && (o = HQ.body), E = o._gsap, E && z && E.width && s && E.time === rQ.time && !E.uncache)
    return R(P / E.width * n);
  if (z && (B === "height" || B === "width")) {
    var h = Q.style[B];
    Q.style[B] = n + t, w = Q[c], h ? Q.style[B] = h : bQ(Q, B);
  } else
    (z || e === "%") && !Ti[vQ(o, "display")] && (i.position = vQ(Q, "position")), o === Q && (i.position = "static"), o.appendChild(IQ), w = IQ[c], o.removeChild(IQ), i.position = "absolute";
  return s && z && (E = SQ(o), E.time = rQ.time, E.width = o[c]), R(T ? w * P / n : w && P ? n / w * P : 0);
}, dQ = function(Q, B, A, t) {
  var P;
  return fA || aA(), B in DQ && B !== "transform" && (B = DQ[B], ~B.indexOf(",") && (B = B.split(",")[0])), OQ[B] && B !== "transform" ? (P = lB(Q, t), P = B !== "transformOrigin" ? P[B] : P.svg ? P.origin : bB(vQ(Q, PQ)) + " " + P.zOrigin + "px") : (P = Q.style[B], (!P || P === "auto" || t || ~(P + "").indexOf("calc(")) && (P = fB[B] && fB[B](Q, B, A) || vQ(Q, B) || Mt(Q, B) || (B === "opacity" ? 1 : 0))), A && !~(P + "").trim().indexOf(" ") ? gQ(Q, B, P, A) + A : P;
}, zi = function(Q, B, A, t) {
  if (!A || A === "none") {
    var P = tB(B, Q, 1), e = P && vQ(Q, P, 1);
    e && e !== A ? (B = P, A = e) : B === "borderColor" && (A = vQ(Q, "borderTopColor"));
  }
  var i = new tQ(this._pt, Q.style, B, 0, 1, AP), s = 0, v = 0, c, n, T, z, w, o, E, k, h, U, D, l;
  if (i.b = A, i.e = t, A += "", t += "", t.substring(0, 6) === "var(--" && (t = vQ(Q, t.substring(4, t.indexOf(")")))), t === "auto" && (o = Q.style[B], Q.style[B] = t, t = vQ(Q, B) || t, o ? Q.style[B] = o : bQ(Q, B)), c = [A, t], Kt(c), A = c[0], t = c[1], T = A.match(KQ) || [], l = t.match(KQ) || [], l.length) {
    for (; n = KQ.exec(t); )
      E = n[0], h = t.substring(s, n.index), w ? w = (w + 1) % 5 : (h.substr(-5) === "rgba(" || h.substr(-5) === "hsla(") && (w = 1), E !== (o = T[v++] || "") && (z = parseFloat(o) || 0, D = o.substr((z + "").length), E.charAt(1) === "=" && (E = WQ(z, E) + D), k = parseFloat(E), U = E.substr((k + "").length), s = KQ.lastIndex - U.length, U || (U = U || cQ.units[B] || D, s === t.length && (t += U, i.e += U)), D !== U && (z = gQ(Q, B, o, U) || 0), i._pt = {
        _next: i._pt,
        p: h || v === 1 ? h : ",",
        //note: SVG spec allows omission of comma/space when a negative sign is wedged between two numbers, like 2.5-5.3 instead of 2.5,-5.3 but when tweening, the negative value may switch to positive, so we insert the comma just in case.
        s: z,
  ...
  return Ct.test(t) && (i.e = 0), this._pt = i, i;
}, st = {
  top: "0%",
  bottom: "100%",
  left: "0%",
  ...
  return (A === "top" || A === "bottom" || t === "left" || t === "right") && (Q = A, A = t, t = Q), B[0] = st[A] || A, B[1] = st[t] || t, B.join(" ");
}, Ei = function(Q, B) {
  if (B.tween && B.tween._time === B.tween._dur) {
    ...
    if (P === "all" || P === !0)
      ...
      for (P = P.split(","), v = P.length; --v > -1; )
        i = P[v], OQ[i] && (s = 1, i = i === "transformOrigin" ? PQ : S), bQ(A, i);
    s && (bQ(A, S), e && (e.svg && A.removeAttribute("transform"), t.scale = t.rotate = t.translate = "none", lB(A, 1), e.uncache = 1, rP(t)));
  }
}, fB = {
  clearProps: function(Q, B, A, t, P) {
    if (P.data !== "isFromStart") {
      var e = Q._pt = new tQ(Q._pt, B, A, 0, 0, Ei);
  ...
  */
}, kB = [1, 0, 0, 1, 0, 0], oP = {}, TP = function(Q) {
  return Q === "matrix(1, 0, 0, 1, 0, 0)" || Q === "none" || !Q;
}, vt = function(Q) {
  var B = vQ(Q, S);
  return TP(B) ? kB : B.substr(7).match(ut).map(R);
}, pA = function(Q, B) {
  var A = Q._gsap || SQ(Q), t = Q.style, P = vt(Q), e, i, s, v;
  return A.svg && Q.getAttribute("transform") ? (s = Q.transform.baseVal.consolidate().matrix, P = [s.a, s.b, s.c, s.d, s.e, s.f], P.join(",") === "1,0,0,1,0,0" ? kB : P) : (P === kB && !Q.offsetParent && Q !== qQ && !A.svg && (s = t.display, t.display = "block", e = Q.parentNode, (!e || !Q.offsetParent && !Q.getBoundingClientRect().width) && (v = 1, i = Q.nextElementSibling, qQ.appendChild(Q)), P = vt(Q), s ? t.display = s : bQ(Q, "display"), v && (i ? e.insertBefore(Q, i) : e ? e.appendChild(Q) : qQ.removeChild(Q))), B && P.length > 6 ? [P[0], P[1], P[4], P[5], P[12], P[13]] : P);
}, hA = function(Q, B, A, t, P, e) {
  var i = Q._gsap, s = P || pA(Q, !0), v = i.xOrigin || 0, c = i.yOrigin || 0, n = i.xOffset || 0, T = i.yOffset || 0, z = s[0], w = s[1], o = s[2], E = s[3], k = s[4], h = s[5], U = B.split(" "), D = parseFloat(U[0]) || 0, l = parseFloat(U[1]) || 0, N, u, d, C;
  A ? s !== kB && (u = z * E - w * o) && (d = D * (E / u) + l * (-o / u) + (o * h - E * k) / u, C = D * (-w / u) + l * (z / u) - (z * h - w * k) / u, D = d, l = C) : (N = cP(Q), D = N.x + (~U[0].indexOf("%") ? D / 100 * N.width : D), l = N.y + (~(U[1] || U[0]).indexOf("%") ? l / 100 * N.height : l)), t || t !== !1 && i.smooth ? (k = D - v, h = l - c, i.xOffset = n + (k * z + h * o) - k, i.yOffset = T + (k * w + h * E) - h) : i.xOffset = i.yOffset = 0, i.xOrigin = D, i.yOrigin = l, i.smooth = !!t, i.origin = B, i.originIsAbsolute = !!A, Q.style[PQ] = "0px 0px", e && (jQ(e, i, "xOrigin", v, D), jQ(e, i, "yOrigin", c, l), jQ(e, i, "xOffset", n, i.xOffset), jQ(e, i, "yOffset", T, i.yOffset)), Q.setAttribute("data-svg-origin", D + " " + l);
}, lB = function(Q, B) {
  var A = Q._gsap || new Wt(Q);
  if ("x" in A && !B && !A.uncache)
    return A;
  var t = Q.style, P = A.scaleX < 0, e = "px", i = "deg", s = getComputedStyle(Q), v = vQ(Q, PQ) || "0", c, n, T, z, w, o, E, k, h, U, D, l, N, u, d, C, M, b, j, O, G, f, H, F, J, _, aQ, hQ, $, Y, TQ, kQ;
  return c = n = T = o = E = k = h = U = D = 0, z = w = 1, A.svg = !!(Q.getCTM && nP(Q)), s.translate && ((s.translate !== "none" || s.scale !== "none" || s.rotate !== "none") && (t[S] = (s.translate !== "none" ? "translate3d(" + (s.translate + " 0 0").split(" ").slice(0, 3).join(", ") + ") " : "") + (s.rotate !== "none" ? "rotate(" + s.rotate + ") " : "") + (s.scale !== "none" ? "scale(" + s.scale.split(" ").join(",") + ") " : "") + (s[S] !== "none" ? s[S] : "")), t.scale = t.rotate = t.translate = "none"), u = pA(Q, A.svg), A.svg && (A.uncache ? (J = Q.getBBox(), v = A.xOrigin - J.x + "px " + (A.yOrigin - J.y) + "px", F = "") : F = !B && Q.getAttribute("data-svg-origin"), hA(Q, F || v, !!F || A.originIsAbsolute, A.smooth !== !1, u)), l = A.xOrigin || 0, N = A.yOrigin || 0, u !== kB && (b = u[0], j = u[1], O = u[2], G = u[3], c = f = u[4], n = H = u[5], u.length === 6 ? (z = Math.sqrt(b * b + j * j), w = Math.sqrt(G * G + O * O), o = b || j ? XQ(j, b) * yQ : 0, h = O || G ? XQ(O, G) * yQ + o : 0, h && (w *= Math.abs(Math.cos(h * _Q))), A.svg && (c -= l - (l * b + N * O), n -= N - (l * j + N * G))) : (kQ = u[6], Y = u[7], aQ = u[8], hQ = u[9], $ = u[10], TQ = u[11], c = u[12], n = u[13], T = u[14], d = XQ(kQ, $), E = d * yQ, d && (C = Math.cos(-d), M = Math.sin(-d), F = f * C + aQ * M, J = H * C + hQ * M, _ = kQ * C + $ * M, aQ = f * -M + aQ * C, hQ = H * -M + hQ * C, $ = kQ * -M + $ * C, TQ = Y * -M + TQ * C, f = F, H = J, kQ = _), d = XQ(-O, $), k = d * yQ, d && (C = Math.cos(-d), M = Math.sin(-d), F = b * C - aQ * M, J = j * C - hQ * M, _ = O * C - $ * M, TQ = G * M + TQ * C, b = F, j = J, O = _), d = XQ(j, b), o = d * yQ, d && (C = Math.cos(d), M = Math.sin(d), F = b * C + j * M, J = f * C + H * M, j = j * C - b * M, H = H * C - f * M, b = F, f = J), E && Math.abs(E) + Math.abs(o) > 359.9 && (E = o = 0, k = 180 - k), z = R(Math.sqrt(b * b + j * j + O * O)), w = R(Math.sqrt(H * H + kQ * kQ)), d = XQ(f, H), h = Math.abs(d) > 2e-4 ? d * yQ : 0, D = TQ ? 1 / (TQ < 0 ? -TQ : TQ) : 0), A.svg && (F = Q.getAttribute("transform"), A.forceCSS = Q.setAttribute("transform", "") || !TP(vQ(Q, S)), F && Q.setAttribute("transform", F))), Math.abs(h) > 90 && Math.abs(h) < 270 && (P ? (z *= -1, h += o <= 0 ? 180 : -180, o += o <= 0 ? 180 : -180) : (w *= -1, h += h <= 0 ? 180 : -180)), B = B || A.uncache, A.x = c - ((A.xPercent = c && (!B && A.xPercent || (Math.round(Q.offsetWidth / 2) === Math.round(-c) ? -50 : 0))) ? Q.offsetWidth * A.xPercent / 100 : 0) + e, A.y = n - ((A.yPercent = n && (!B && A.yPercent || (Math.round(Q.offsetHeight / 2) === Math.round(-n) ? -50 : 0))) ? Q.offsetHeight * A.yPercent / 100 : 0) + e, A.z = T + e, A.scaleX = R(z), A.scaleY = R(w), A.rotation = R(o) + i, A.rotationX = R(E) + i, A.rotationY = R(k) + i, A.skewX = h + i, A.skewY = U + i, A.transformPerspective = D + e, (A.zOrigin = parseFloat(v.split(" ")[2]) || !B && A.zOrigin || 0) && (t[PQ] = bB(v)), A.xOffset = A.yOffset = 0, A.force3D = cQ.force3D, A.renderTransform = A.svg ? hi : vP ? zP : ai, A.uncache = 0, A;
}, bB = function(Q) {
  return (Q = Q.split(" "))[0] + " " + Q[1];
}, ZB = function(Q, B, A) {
  var t = W(B);
  return R(parseFloat(B) + parseFloat(gQ(Q, "x", A + "px", t))) + t;
}, ai = function(Q, B) {
  B.z = "0px", B.rotationY = B.rotationX = "0deg", B.force3D = 0, zP(Q, B);
}, mQ = "0deg", PB = "0px", xQ = ") ", zP = function(Q, B) {
  ...
  if (U && (n !== mQ || c !== mQ)) {
    var N = parseFloat(c) * _Q, u = Math.sin(N), d = Math.cos(N), C;
    N = parseFloat(n) * _Q, C = Math.cos(N), e = ZB(h, e, u * C * -U), i = ZB(h, i, -Math.sin(N) * -U), s = ZB(h, s, d * C * -U + U);
  }
  ...
  var A = B || this, t = A.xPercent, P = A.yPercent, e = A.x, i = A.y, s = A.rotation, v = A.skewX, c = A.skewY, n = A.scaleX, T = A.scaleY, z = A.target, w = A.xOrigin, o = A.yOrigin, E = A.xOffset, k = A.yOffset, h = A.forceCSS, U = parseFloat(e), D = parseFloat(i), l, N, u, d, C;
  s = parseFloat(s), v = parseFloat(v), c = parseFloat(c), c && (c = parseFloat(c), v += c, s += c), s || v ? (s *= _Q, v *= _Q, l = Math.cos(s) * n, N = Math.sin(s) * n, u = Math.sin(s - v) * -T, d = Math.cos(s - v) * T, v && (c *= _Q, C = Math.tan(v - c), C = Math.sqrt(1 + C * C), u *= C, d *= C, c && (C = Math.tan(c), C = Math.sqrt(1 + C * C), l *= C, N *= C)), l = R(l), N = R(N), u = R(u), d = R(d)) : (l = n, d = T, N = u = 0), (U && !~(e + "").indexOf("px") || D && !~(i + "").indexOf("px")) && (U = gQ(z, "x", e, "px"), D = gQ(z, "y", i, "px")), (w || o || E || k) && (U = R(U + w - (w * l + o * u) + E), D = R(D + o - (w * N + o * d) + k)), (t || P) && (C = z.getBBox(), U = R(U + t / 100 * C.width), D = R(D + P / 100 * C.height)), C = "matrix(" + l + "," + N + "," + u + "," + d + "," + U + "," + D + ")", z.setAttribute("transform", C), h && (z.style[S] = C);
}, ki = function(Q, B, A, t, P) {
  var e = 360, i = K(P), s = parseFloat(P) * (i && ~P.indexOf("rad") ? yQ : 1), v = s - t, c = t + v + "deg", n, T;
  return i && (n = P.split("_")[1], n === "short" && (v %= e, v !== v % (e / 2) && (v += v < 0 ? e : -e)), n === "cw" && v < 0 ? v = (v + e * tt) % e - ~~(v / e) * e : n === "ccw" && v > 0 && (v = (v - e * tt) % e - ~~(v / e) * e)), Q._pt = T = new tQ(Q._pt, B, A, t, v, Bi), T.e = c, T.u = "deg", Q._props.push(A), T;
}, ct = function(Q, B) {
  for (var A in B)
    Q[A] = B[A];
  return Q;
}, li = function(Q, B, A) {
  var t = ct({}, A._gsap), P = "perspective,force3D,transformOrigin,svgOrigin", e = A.style, i, s, v, c, n, T, z, w;
  t.svg ? (v = A.getAttribute("transform"), A.setAttribute("transform", ""), e[S] = B, i = lB(A, 1), bQ(A, S), A.setAttribute("transform", v)) : (v = getComputedStyle(A)[S], e[S] = B, i = lB(A, 1), e[S] = v);
  for (s in OQ)
    v = t[s], c = i[s], v !== c && P.indexOf(s) < 0 && (z = W(v), w = W(c), n = z !== w ? gQ(A, s, v, w) : parseFloat(v), T = parseFloat(c), Q._pt = new tQ(Q._pt, i, s, n, T - n, wA), Q._pt.u = w || 0, Q._props.push(s));
  ct(i, t);
};
...
var wP = {
  ...
  init: function(Q, B, A, t, P) {
    var e = this._props, i = Q.style, s = A.vars.startAt, v, c, n, T, z, w, o, E, k, h, U, D, l, N, u, d, C;
    fA || aA(), this.styles = this.styles || sP(Q), d = this.styles.props, this.tween = A;
    for (o in B)
      if (o !== "autoRound" && (c = B[o], !(iQ[o] && qt(o, B, A, t, Q, P)))) {
        if (z = typeof c, w = fB[o], z === "function" && (c = c.call(A, t, Q, P), z = typeof c), z === "string" && ~c.indexOf("random(") && (c = EB(c)), w)
          w(this, Q, o, c, A) && (u = 1);
        else if (o.substr(0, 2) === "--")
          v = (getComputedStyle(Q).getPropertyValue(o) + "").trim(), c += "", GQ.lastIndex = 0, GQ.test(v) || (E = W(v), k = W(c), k ? E !== k && (v = gQ(Q, o, v, k) + k) : E && (c += E)), this.add(i, "setProperty", v, c, t, P, 0, 0, o), e.push(o), d.push(o, 0, i[o]);
        else if (z !== "undefined") {
          if (s && o in s ? (v = typeof s[o] == "function" ? s[o].call(A, t, Q, P) : s[o], K(v) && ~v.indexOf("random(") && (v = EB(v)), W(v + "") || v === "auto" || (v += cQ.units[o] || W(dQ(Q, o)) || ""), (v + "").charAt(1) === "=" && (v = dQ(Q, o))) : v = dQ(Q, o), T = parseFloat(v), h = z === "string" && c.charAt(1) === "=" && c.substr(0, 2), h && (c = c.substr(2)), n = parseFloat(c), o in DQ && (o === "autoAlpha" && (T === 1 && dQ(Q, "visibility") === "hidden" && n && (T = 0), d.push("visibility", 0, i.visibility), jQ(this, i, "visibility", T ? "inherit" : "hidden", n ? "inherit" : "hidden", !n)), o !== "scale" && o !== "transform" && (o = DQ[o], ~o.indexOf(",") && (o = o.split(",")[0]))), U = o in OQ, U) {
            if (this.styles.save(o), C = c, z === "string" && c.substring(0, 6) === "var(--") {
              if (c = vQ(Q, c.substring(4, c.indexOf(")"))), c.substring(0, 5) === "calc(") {
                ...
                Q.style.perspective = c, c = vQ(Q, "perspective"), M ? Q.style.perspective = M : bQ(Q, "perspective");
              }
              n = parseFloat(c);
            }
            if (D || (l = Q._gsap, l.renderTransform && !B.parseTransform || lB(Q, B.parseTransform), N = B.smoothOrigin !== !1 && l.smooth, D = this._pt = new tQ(this._pt, i, S, 0, 1, l.renderTransform, l, 0, -1), D.dep = 1), o === "scale")
              this._pt = new tQ(this._pt, l, "scaleY", l.scaleY, (h ? WQ(l.scaleY, h + n) : n) - l.scaleY || 0, wA), this._pt.u = 0, e.push("scaleY", o), o += "X";
            else if (o === "transformOrigin") {
              d.push(PQ, 0, i[PQ]), c = wi(c), l.svg ? hA(Q, c, 0, N, 0, this) : (k = parseFloat(c.split(" ")[2]) || 0, k !== l.zOrigin && jQ(this, l, "zOrigin", l.zOrigin, k), jQ(this, i, o, bB(v), bB(c)));
              continue;
            } else if (o === "svgOrigin") {
              hA(Q, c, 1, N, 0, this);
              continue;
            } else if (o in oP) {
              ki(this, l, o, T, h ? WQ(T, h + c) : c);
              continue;
            } else if (o === "smoothOrigin") {
              jQ(this, l, "smooth", l.smooth, c);
              continue;
            } else if (o === "force3D") {
              l[o] = c;
              continue;
          ...
          if (U || (n || n === 0) && (T || T === 0) && !Qi.test(c) && o in i)
            E = (v + "").substr((T + "").length), n || (n = 0), k = W(c) || (o in cQ.units ? cQ.units[o] : E), E !== k && (T = gQ(Q, o, v, k)), this._pt = new tQ(this._pt, U ? l : i, o, T, (h ? WQ(T, h + n) : n) - T, !U && (k === "px" || o === "zIndex") && B.autoRound !== !1 ? Pi : wA), this._pt.u = k || 0, U && C !== c ? (this._pt.b = v, this._pt.e = C, this._pt.r = ti) : E !== k && k !== "%" && (this._pt.b = v, this._pt.r = Ai);
          else if (o in i)
            zi.call(this, Q, o, v, h ? h + c : c);
          else if (o in Q)
          ...
          U || (o in i ? d.push(o, 0, i[o]) : typeof Q[o] == "function" ? d.push(o, 2, Q[o]()) : d.push(o, 1, v || Q[o])), e.push(o);
        }
      }
    u && tP(this);
  },
  ...
  getSetter: function(Q, B, A) {
    var t = DQ[B];
    return t && t.indexOf(",") < 0 && (B = t), B in OQ && B !== PQ && (Q._gsap.x || dQ(Q, "x")) ? A && At === A ? B === "scale" ? si : ri : (At = A || {}) && (B === "scale" ? vi : ci) : Q.style && !UA(Q.style[B]) ? ei : ~B.indexOf("-") ? ii : FA(Q, B);
  },
  core: {
    _removeProperty: bQ,
    _getMatrix: pA
...
eQ.core.getStyleSaver = sP;
(function(r, Q, B, A) {
  var t = AQ(r + "," + Q + "," + B, function(P) {
    OQ[P] = 1;
  });
  AQ(Q, function(P) {
    cQ.units[P] = "deg", oP[P] = 1;
  }), DQ[t[13]] = r + "," + Q, AQ(A, function(P) {
    var e = P.split(":");
    DQ[e[1]] = t[e[0]];
  });
...
function Mi(r) {
  ...
  return r.bit.type === "triangle" ? Ci(Q) : r.bit.type === "drill" ? aP(Q) : di(Q);
}
function hP(r) {
  return r.bit.colorSource === "custom" ? r.bit.color : Di;
}
function Hi(r) {
  const Q = DB(r.bit.opacity), B = aP(Math.max(1e-3, r.bit.size)), A = new a.Color(hP(r)), t = Oi(), P = new a.MeshStandardMaterial({
    color: A,
    metalness: 0.8,
    roughness: 0.35,
    roughnessMap: t,
...
function ji(r) {
  const Q = DB(r.bit.opacity), B = Mi(r), A = new a.MeshBasicMaterial({
...
function Fi(r) {
  ...
  const v = (n) => {
    ...
    if (E || k)
      Q.remove(A), zt(A), A = Tt(n), Q.add(A), i == null || i.kill(), e == null || e.kill(), e = s(n.bit.spinRpm), P && e.timeScale(1).play();
    else if (T === "laser") {
      const D = A, [l, N] = D.children, u = l.material, d = N.material;
      u.opacity = h, u.needsUpdate = !0, d.opacity = h * 0.3, d.needsUpdate = !0;
    } else {
      const l = A.material, N = new a.Color(T === "drill" ? hP(n) : EP);
      l.color = N, T === "drill" && (l.emissive = N), l.opacity = h, l.transparent = h < 1, l.needsUpdate = !0;
    }
  ...
  return {
    ...
    dispose: () => {
      i == null || i.kill(), e == null || e.kill(), Q.remove(A), zt(A);
    }
  };
}
...
class Yi {
  ...
  seekToLine(Q, B) {
    ...
    if (this.sim3dHandle) {
      ...
      if (A !== this.sim3dHandle.currentLine) {
        const t = xB(A, this.sim3dHandle.data);
  ...
  setOptions(Q) {
    var T, z, w, o, E, k, h, U, D, l, N, u, d, C, M, b, j, O, G, f, H, F, J, _, aQ, hQ, $, Y, TQ, kQ, mA, xA, yA, IA;
    const B = this.options;
    this.options = Et(this.options, Q), (B.render.antialias !== this.options.render.antialias || B.render.theme.background !== this.options.render.theme.background) && this.renderer.setClearColor(this.options.render.theme.background, 1), (B.render.theme.opacity !== this.options.render.theme.opacity || B.render.theme.rapidOpacity !== this.options.render.theme.rapidOpacity) && this.refreshToolpathOpacities(), B.camera.fov !== this.options.camera.fov && (this.camera.fov = this.options.camera.fov, this.camera.updateProjectionMatrix()), B.camera.orbit.enableDamping !== this.options.camera.orbit.enableDamping && (this.controls.enableDamping = this.options.camera.orbit.enableDamping);
    const A = ((T = B.grid.bounds) == null ? void 0 : T.min.x) !== ((z = this.options.grid.bounds) == null ? void 0 : z.min.x) || ((w = B.grid.bounds) == null ? void 0 : w.min.y) !== ((o = this.options.grid.bounds) == null ? void 0 : o.min.y) || ((E = B.grid.bounds) == null ? void 0 : E.max.x) !== ((k = this.options.grid.bounds) == null ? void 0 : k.max.x) || ((h = B.grid.bounds) == null ? void 0 : h.max.y) !== ((U = this.options.grid.bounds) == null ? void 0 : U.max.y), t = B.units !== this.options.units || B.grid.sizeX !== this.options.grid.sizeX || B.grid.sizeY !== this.options.grid.sizeY || B.grid.axisDepth !== this.options.grid.axisDepth || A, P = B.render.theme.colors.grid.major !== this.options.render.theme.colors.grid.major || B.render.theme.colors.grid.minor !== this.options.render.theme.colors.grid.minor || B.render.theme.colors.axes.x !== this.options.render.theme.colors.axes.x || B.render.theme.colors.axes.y !== this.options.render.theme.colors.axes.y || B.render.theme.colors.axes.z !== this.options.render.theme.colors.axes.z;
    (t || P) && this.renderGridAndAxes(), (B.grid.labels !== this.options.grid.labels || B.units !== this.options.units || B.grid.sizeX !== this.options.grid.sizeX || B.grid.sizeY !== this.options.grid.sizeY || A || P) && this.refreshGridLabels(), (B.boundingBox.visible !== this.options.boundingBox.visible || B.boundingBox.labels !== this.options.boundingBox.labels || B.units !== this.options.units || B.render.theme.colors.boundingBox !== this.options.render.theme.colors.boundingBox) && this.refreshBoundingBox(), (B.machineBed.visible !== this.options.machineBed.visible || ((D = B.machineBed.min) == null ? void 0 : D.x) !== ((l = this.options.machineBed.min) == null ? void 0 : l.x) || ((N = B.machineBed.min) == null ? void 0 : N.y) !== ((u = this.options.machineBed.min) == null ? void 0 : u.y) || ((d = B.machineBed.max) == null ? void 0 : d.x) !== ((C = this.options.machineBed.max) == null ? void 0 : C.x) || ((M = B.machineBed.max) == null ? void 0 : M.y) !== ((b = this.options.machineBed.max) == null ? void 0 : b.y) || ((O = (j = B.machineBed.keepout) == null ? void 0 : j.min) == null ? void 0 : O.x) !== ((f = (G = this.options.machineBed.keepout) == null ? void 0 : G.min) == null ? void 0 : f.x) || ((F = (H = B.machineBed.keepout) == null ? void 0 : H.min) == null ? void 0 : F.y) !== ((_ = (J = this.options.machineBed.keepout) == null ? void 0 : J.min) == null ? void 0 : _.y) || ((hQ = (aQ = B.machineBed.keepout) == null ? void 0 : aQ.max) == null ? void 0 : hQ.x) !== ((Y = ($ = this.options.machineBed.keepout) == null ? void 0 : $.max) == null ? void 0 : Y.x) || ((kQ = (TQ = B.machineBed.keepout) == null ? void 0 : TQ.max) == null ? void 0 : kQ.y) !== ((xA = (mA = this.options.machineBed.keepout) == null ? void 0 : mA.max) == null ? void 0 : xA.y) || B.render.theme.colors.machineBed !== this.options.render.theme.colors.machineBed || B.render.theme.colors.machineBedKeepout !== this.options.render.theme.colors.machineBedKeepout) && this.refreshMachineBed();
    ...
    if (B.sim3d.erosionPasses !== this.options.sim3d.erosionPasses && this.sim3dHandle) {
      const kP = this.sim3dHandle.currentLine, SA = xB(kP, this.sim3dHandle.data);
      yB(SA, this.sim3dHandle.data.resolution, this.options.sim3d.erosionPasses, this.sim3dHandle.data.slabBounds.zTop), IB(this.sim3dHandle.slab, SA);
    }
    ...
    const B = [
      {
        ...
        opacity: wt(this.options.render.theme.rapidOpacity ?? 0.3)
      },
      ...Q.cuts.map((P, e) => ({
        kind: "cut",
        cutBucketIndex: e,
...
class Vi {
  ...
  fitView() {
    ...
    const { minX: Q, maxX: B, minY: A, maxY: t, minZ: P, maxZ: e } = this.bounds, i = this.project, s = i(Q, A, P), v = i(B, A, P), c = i(B, t, P), n = i(Q, t, P), T = i(Q, A, e), z = i(B, A, e), w = i(B, t, e), o = i(Q, t, e), E = Math.hypot(v.x - s.x, v.y - s.y), k = Math.hypot(n.x - s.x, n.y - s.y), h = Math.hypot(T.x - s.x, T.y - s.y), U = Math.max(E, k, h) * 0.07, D = U * 0.5, l = at(rB(s, v), rB(c, n), D), N = [s, v, c, n, T, z, w, o], u = Math.min(...N.map((Y) => Y.y)), d = Math.max(...N.map((Y) => Y.y)), C = Math.min(...N.map((Y) => Y.x)), M = Math.max(...N.map((Y) => Y.x)), b = { x: M + D, y: (u + d) / 2 }, j = { x: b.x + U * 5, y: b.y }, O = { x: (C + M) / 2, y: u - U }, G = [s, v, c, n, T, z, w, o, l, b, j, O];
    let f = 1 / 0, H = 1 / 0, F = -1 / 0, J = -1 / 0;
    for (const Y of G)
      Y.x < f && (f = Y.x), Y.y < H && (H = Y.y), Y.x > F && (F = Y.x), Y.y > J && (J = Y.y);
    const _ = F - f, aQ = J - H, hQ = Math.max(_, aQ) * 0.08, $ = this.options.padding + hQ;
    this.viewBox = {
      x: f - $,
      y: H - $,
      w: _ + $ * 2,
  ...
  renderBbox() {
    ...
    $B(this.bboxLabelX, rB(s, v), at(rB(s, v), rB(c, n), l), D, `X: ${QA(B - Q)}`);
    const N = [s, v, c, n, T, z, w, o], u = Math.min(...N.map((O) => O.y)), d = Math.max(...N.map((O) => O.y)), C = Math.min(...N.map((O) => O.x)), M = Math.max(...N.map((O) => O.x)), b = { x: M + l, y: (u + d) / 2 };
